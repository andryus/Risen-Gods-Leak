...
namespace MMAP
{
    ...
    void TerrainBuilder::loadStaticGameObject(GameObjectBounds& bounds, VMAP::WorldModel& model, MeshData& meshData, bool isM2)
    {
        float scale = bounds.size;
        G3D::Matrix3 rotation = bounds.rotation;
        G3D::Vector3 position = bounds.iPos;
        ...
        for (std::vector<GroupModel>::iterator it = groupModels.begin(); it != groupModels.end(); ++it)
        {
            ...
            for (std::vector<G3D::Vector3>::iterator it = tempVertices.begin(); it != tempVertices.end(); ++it)
            {
                G3D::Vector3 v((*it) * rotation * scale + position);
                transformedVertices.push_back(v);
            }
