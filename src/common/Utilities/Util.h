...
COMMON_API int32 MoneyStringToMoney(const std::string& moneyString);
COMMON_API std::string moneyToMoneyString(int32 money);

COMMON_API tm* localtime_r(const std::time_t* time, tm *result);
...
COMMON_API std::string FormatDuration(uint64 durationInSecs, bool shortText = false, bool hoursOnly = false);
template<typename Rep, typename Period>
std::string FormatDuration(std::chrono::duration<Rep, Period> duration, bool shortText = false, bool hoursOnly = false)
{
...
COMMON_API std::string TimeToTimestampStr(std::time_t t);
...
COMMON_API bool WStrToUtf8(std::wstring wstr, std::string& utf8str);
...
COMMON_API std::wstring GetMainPartOfName(std::wstring wname, uint32 declension);
...
COMMON_API bool Utf8FitTo(const std::string& str, std::wstring search);
