...
std::string secsToTimeString(uint64 durationInSecs, bool shortText, bool hoursOnly)
{
    return FormatDuration(durationInSecs, shortText, hoursOnly);
}

std::string FormatDuration(uint64 durationInSecs, bool shortText, bool hoursOnly)
{
    uint64 secs = durationInSecs % MINUTE;
    uint64 minutes = durationInSecs % HOUR / MINUTE;
    uint64 hours = durationInSecs % DAY / HOUR;
    uint64 days = durationInSecs / DAY;
...
std::string moneyToMoneyString(int32 money){
    ...
    {
        int8 s = (int8)(money / 100);
        int8 k = (int8)money - (s * 100);
        os << int32(s) << "s " << int32(k) << "c";
    }
    else    
    {
        int32 g = (int32)(money / 10000);
        int8 s = (int8)((money - g * 10000) / 100);
        int8 k = (int8) money - (g * 10000 + s * 100);
        os << g << "g " << int32(s) << "s " << int32(k) << "c";
    }
    return os.str();
}
