...
#   include "Graylog/AppenderGelf.h"
#endif
...
void Log::CreateLoggerFromConfig(std::string const& appenderName)
{
    ...
    {
        std::string mirror = sConfigMgr->GetStringDefault("RG.Logging.Mirror", "");
        if (!mirror.empty())
        {
            if (Appender* appender = GetAppenderByName(mirror))
            {
                logger.addAppender(appender->getId(), appender);
            }
            else
                fprintf(stderr, "Error while configuring Appender %s for mirror logging. Appender does not exist", mirror.c_str());
        }
        else
            fprintf(stdout, "RG: Logging: Gelf mirror is deactivated");
    }
#endif
}
