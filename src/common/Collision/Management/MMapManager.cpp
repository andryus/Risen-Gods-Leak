...
namespace MMAP
{
    ...
    dtNavMeshQuery const* MMapManager::GetNavMeshQuery(uint32 mapId)
    {
        ...
        MMapDataSet::const_iterator mmapItr = GetMMapData(mapId);
        if (mmapItr == loadedMMaps.end())
            return NULL;

        MMapData* mmap = mmapItr->second;

        auto navMeshItr = mmap->navMeshQueries.find(std::this_thread::get_id());
        if (navMeshItr != mmap->navMeshQueries.end())
            return navMeshItr->second;
        ...
        if (dtStatusFailed(query->init(mmap->navMesh, 1024)))
        {
            ...
            TC_LOG_ERROR("maps", "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId %03u", mapId);
            return NULL;
        }

        TC_LOG_DEBUG("maps", "MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId %03u", mapId);
        mmap->navMeshQueries.insert(std::pair<std::thread::id, dtNavMeshQuery*>(std::this_thread::get_id(), query));
    ...
    bool MMapManager::loadGameObject(uint32 displayId)
    {
        ...
        // load and init dtNavMesh - read parameters from file
        uint32 pathLen = sConfigMgr->GetStringDefault("DataDir", ".").length() + strlen("/mmaps/go%04u.mmap") + 1;
        char *fileName = new char[pathLen];
        snprintf(fileName, pathLen, (sConfigMgr->GetStringDefault("DataDir", ".") + "/mmaps/go%04u.mmap").c_str(), displayId);
    ...
    dtNavMesh const * MMAP::MMapManager::GetModelNavMesh(uint32 displayId)
    {
        auto itr = loadedModels.find(displayId);
        if (loadedModels.find(displayId) == loadedModels.end() || !itr->second)
            return NULL;

        return itr->second->navMesh;
    }

    dtNavMeshQuery const* MMAP::MMapManager::GetModelNavMeshQuery(uint32 displayId)
    {
        if (loadedModels.find(displayId) == loadedModels.end())
            return NULL;
        ...
        {
            dtFreeNavMeshQuery(query);
            TC_LOG_ERROR("maps", "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for displayid %03u", displayId);
            return NULL;
        }

        TC_LOG_DEBUG("maps", "MMAP:GetNavMeshQuery: created dtNavMeshQuery for displayid %03u", displayId);
        mmap->navMeshQueries.insert(std::pair<std::thread::id, dtNavMeshQuery*>(std::this_thread::get_id(), query));
        return query;
    }
}
