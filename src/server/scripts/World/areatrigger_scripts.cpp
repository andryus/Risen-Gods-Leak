...
class AreaTrigger_at_coilfang_waterfall : public AreaTriggerScript
{
    public:
        ...
        {
            if (GameObject* go = player->FindNearestGameObject(GO_COILFANG_WATERFALL, 35.0f))
    ...
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
        {
            if (!(player->GetQuestStatus(QUEST_LAST_RITES) == QUEST_STATUS_INCOMPLETE ||
                ...
                player->GetQuestStatus(QUEST_LAST_RITES) == QUEST_STATUS_REWARDED ||
            ...
            switch (trigger->id)
            {
                case 5332:
                    ...
                    break;
                case 5338: 
                    pPosition = WorldLocation(571, 3802.50f, 3589.72f, 48.945f, 1.143233f);
                    break;
                case 5334:
                ...
                case 5340:
                    if (player->GetQuestStatus(QUEST_LAST_RITES) == QUEST_STATUS_INCOMPLETE ||
                        ...
                        pPosition = WorldLocation(571, 3730.274902f, 3564.420410f, 341.603943f);
                    break;
                default:
                    return false;
            }
    ...
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
        {
            if (!player->isDead() && (player->GetQuestStatus(QUEST_MEETING_A_GREAT_ONE) != QUEST_STATUS_NONE ||
                (player->GetQuestStatus(QUEST_THE_MAKERS_OVERLOOK) == QUEST_STATUS_REWARDED)))
            {
                switch (trigger->id)
                {
                    case AT_SHOLAZAR:
...
class AreaTrigger_at_brewfest : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
        {
            ...
            if (game::time::GetGameTime() - _triggerTimes[triggerId] < AREATRIGGER_TALK_COOLDOWN)
                return false;
            ...
            _triggerTimes[triggerId] = game::time::GetGameTime();
            return false;
        }
...
class AreaTrigger_at_area_52_entrance : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
        {
            ...
            uint32 triggerId = trigger->id;
            if (game::time::GetGameTime() - _triggerTimes[trigger->id] < SUMMON_COOLDOWN)
                return false;
            ...
            _triggerTimes[trigger->id] = game::time::GetGameTime();
            return false;
        }
...
enum VoltarusAreaTrigger
{
    ITEM_SCEPTER_OF_DOMINATION = 39319
};

class AreaTrigger_at_voltarus_teleporter : public AreaTriggerScript
{
    public:
        AreaTrigger_at_voltarus_teleporter() : AreaTriggerScript("at_voltarus_teleporter") { }

        bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/)
        {
            if (player->IsAlive() && player->GetItemCount(ITEM_SCEPTER_OF_DOMINATION) > 0)
            {
                player->TeleportTo(571, 6252.87f, -1964.71f, 484.78f, 5.261f);
                return true;
            }
...
enum AnubarUnderlordMisc
{
    QUEST_FROM_THE_DEPTHS_OF_AZJOL = 12036,
    NPC_ANUB_AR_UNDERLORD          = 26605,
    DATA_START_EVENT               = 1
};

class AreaTrigger_at_anub_ar_underlord : public AreaTriggerScript
{
    public:
        AreaTrigger_at_anub_ar_underlord() : AreaTriggerScript("at_anub_ar_underlord") { }

        bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/) override
        {
            if (!player->isDead() && player->GetQuestStatus(QUEST_FROM_THE_DEPTHS_OF_AZJOL) == QUEST_STATUS_INCOMPLETE)
                player->CompleteQuest(QUEST_FROM_THE_DEPTHS_OF_AZJOL);

            if (Creature* underlord = player->FindNearestCreature(NPC_ANUB_AR_UNDERLORD, 20.0f))
                underlord->AI()->SetData(DATA_START_EVENT, DATA_START_EVENT);
...
enum MistWhisperTreasure
{
    ...
    NPC_ZEPTEK_THE_DESTROYER = 28399
};

Position const TARTEK_SUMMON_POS = { 6709.02f, 5169.21f, -20.8878f, 4.91029f };
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/) override
    {
        if (player->GetQuestStatus(QUEST_LOST_MISTWHISPER_TREASURE) == QUEST_STATUS_INCOMPLETE)
        {
            // Check If Player has already killed Tartek
            if (uint16 questslot = player->FindQuestSlot(QUEST_LOST_MISTWHISPER_TREASURE)) 
            {
                if (questslot < MAX_QUEST_LOG_SIZE && player->GetQuestSlotCounter(questslot, 0) != 1) 
                {
                    if (!player->FindNearestCreature(NPC_WARLORD_TARTEK, 100.0f, true) && !player->FindNearestCreature(NPC_ZEPTEK_THE_DESTROYER, 100.0f, true))
                    {
                        player->SummonCreature(NPC_WARLORD_TARTEK, TARTEK_SUMMON_POS, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120000);
                        return true;
                    }
                }
            } 
...
void AddSC_areatrigger_scripts()
{
    ...
    new AreaTrigger_at_voltarus_teleporter();
    new AreaTrigger_at_anub_ar_underlord();
