...
public:
    ...
    bool OnExpire(Player* player, ItemTemplate const* /*pItemProto*/) override
    {
        ...
        if (msg == EQUIP_ERR_OK)
        {
            Item* item = player->StoreNewItem(dest, 39883, true, Item::GenerateItemRandomPropertyId(39883));
...
public:
    ...
    bool OnExpire(Player* player, ItemTemplate const* /*pItemProto*/) override
    {
        ...
        if (msg == EQUIP_ERR_OK)
        {
            Item* item = player->StoreNewItem(dest, 44718, true, Item::GenerateItemRandomPropertyId(44718));
            // RG-Custom ItemLogging
            RG_LOG<ItemLogModule>(item, RG::Logging::ItemLogType::ADDED, 1);
        }
        return true;
...
enum SlipperyEelMisc
{
    SPELL_FOOD = 43180
};

class item_slippery_eel : public ItemScript
{
    public:
        item_slippery_eel() : ItemScript("item_slippery_eel") { }

        bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/) override
        {
            player->AddAura(SPELL_FOOD, player);
