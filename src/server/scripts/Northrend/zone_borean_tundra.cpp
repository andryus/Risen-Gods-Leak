    ...
    struct npc_sinkhole_kill_creditAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (phaseTimer <= diff)
            {
                ...
                {
                    case 1:
                        DoCastSelf(SPELL_EXPLODE_CART, true);
                        DoCastSelf(SPELL_SUMMON_CART, true);
                    ...
                    case 3:
                        ...
                        break;
                    case 5:
                        DoCastSelf(SPELL_SUMMON_WORM, true);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_ACES_HIGH) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_ACES_HIGH_DAILY) == QUEST_STATUS_INCOMPLETE) //It's the same dragon for both quests.
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_C_1, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (player->GetQuestStatus(QUEST_SPIRITS_WATCH_OVER_US) == QUEST_STATUS_INCOMPLETE)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_I, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_ESCAPE_WINTERFIN_CAVERNS)
        {
            ...
            creature->setFaction(FACTION_ESCORTEE_N);
    ...
    struct npc_nexus_drake_hatchlingAI : public FollowerAI //The spell who makes the npc follow the player is missing, also we can use FollowerAI!
    {
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_DRAKE_HARPOON && caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                DoCastSelf(SPELL_RED_DRAGONBLOOD, true);
            }
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            if (WithRedDragonBlood && HarpoonerGUID && !me->HasAura(SPELL_RED_DRAGONBLOOD))
            {
                ...
                {
                    EnterEvadeMode();
                    // Must be set again because of Reset() call in EnterEvadeMode()
                    HarpoonerGUID = pHarpooner->GetGUID();
                    if (!HasFollowState(STATE_FOLLOW_INPROGRESS))
                        StartFollow(pHarpooner, 35, NULL);

                    DoCastSelf(SPELL_SUBDUED, true);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_LAST_RITES) == QUEST_STATUS_INCOMPLETE && creature->GetAreaId() == 4128)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_T, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
    ...
    struct npc_beryl_sorcererAI : public FollowerAI
    {
        ...
        void SpellHit(Unit* pCaster, const SpellInfo* pSpell) override
        {
            if (pSpell->Id == SPELL_ARCANE_CHAINS && pCaster->GetTypeId() == TYPEID_PLAYER && !HealthAbovePct(50) && !bEnslaved)
            {
                ...
                DoCastSelf(SPELL_COSMETIC_ENSLAVE_CHAINS_SELF, true);
                me->setActive(true);
    ...
    struct npc_imprisoned_beryl_sorcererAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (rebuff <= diff)
            {
                ...
                {
                    DoCastSelf(SPELL_COSMETIC_ENSLAVE_CHAINS_SELF);
                }
                rebuff = 180000;
            }
            else
...
enum MootooTheYounger
{
    ...
    EVENT_SAY_ESCORT_RANDOM     = 1,
    EVENT_DESPAWN               = 2,

    NPC_ELDAR_MOOTOO            = 25503,
...
public:
    ...
    struct npc_mootoo_the_youngerAI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            {
                case 1:
                    events.ScheduleEvent(EVENT_SAY_ESCORT_RANDOM, 30s);
                    break;
                case 10:
                ...
                case 21:
                    Talk(SAY_5, me);
                    if (Creature* eldarmootoo = me->FindNearestCreature(NPC_ELDAR_MOOTOO, 50.0f, true)) 
                    {
                        eldarmootoo->AI()->Talk(SAY_ELDAR_1, player);
                        eldarmootoo->AI()->TalkWithDelay(2s, SAY_ELDAR_2, player->GetGUID());
                    }
        ...
        void UpdateEscortAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                case EVENT_SAY_ESCORT_RANDOM:
                    Talk(SAY_RANDOM);
                    events.ScheduleEvent(EVENT_SAY_ESCORT_RANDOM, 20s, 30s);
                    break;
                case EVENT_DESPAWN:
                    me->DisappearAndDie();
                    break;
...
enum BonkerTogglevolt
{
    ...
    GO_BALL_AND_CHAIN           = 175544,

    EVENT_SAY_RANDOM            = 1,

    SAY_BONKER_RANDOM           = 0,
    SAY_BONKER_ESCORT_START     = 1,
    SAY_BONKER_ESCORT1          = 2,
    SAY_BONKER_ESCORT2          = 3,
    SAY_BONKER_ESCORT_FINISHED  = 4,
    SAY_BONKER_AGGRO            = 5
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
    {
        ...
        {
            ENSURE_AI(npc_escortAI, (creature->AI()))->Start(true, true, player->GetGUID(), quest);
            creature->SetStandState(UNIT_STAND_STATE_STAND);
            creature->AI()->Talk(SAY_BONKER_ESCORT_START, player);
            creature->AI()->TalkWithDelay(3s, SAY_BONKER_ESCORT1);
            creature->SetImmuneToNPC(false);
            creature->SetImmuneToPC(true);
            
            if (GameObject* go = creature->FindNearestGameObject(GO_BALL_AND_CHAIN, 10.0f))
    ...
    struct npc_bonker_togglevoltAI : public npc_escortAI
    {
        ...
        void Reset() override
        {
            ...
            events.ScheduleEvent(EVENT_SAY_RANDOM, 60s, 120s);
            // respawn GO Ball And Chains
            me->SummonGameObject(GO_BALL_AND_CHAIN, 4414.002f, 5368.281f, -15.58675f, 3.595379f, 0, 0, -0.97437f, 0.2249513f, 120);
        }
        ...
        void UpdateEscortAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent()) 
            {
                switch (eventId) 
                {
                case EVENT_SAY_RANDOM:
                    Talk(SAY_BONKER_RANDOM);
                    events.ScheduleEvent(EVENT_SAY_RANDOM, 60s, 120s);
                    break;
                default:
                    break;
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            {
                case 2:
                    Talk(SAY_BONKER_ESCORT2);
                    break;
                case 29:
                    Talk(SAY_BONKER_ESCORT_FINISHED, player);
    ...
    struct npc_warmage_coldarraAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            if (m_uiTimer <= uiDiff)
            {
                ...
                me->GetCreatureListWithEntryInGrid(orbList, NPC_TRANSITUS_SHIELD_DUMMY, 32.0f);
...
enum Gossipitems
{
    GOSSIP_ITEM_QUEST_THE_HUNT_IS_ON            = 60197,
    GOSSIP_ITEM_TOM_HEGGER                      = 0,
    GOSSIP_ITEM_GUARD_MITCHELLS                 = 1,
    GOSSIP_ITEM_SALTY_JOHN_THORPE               = 2
};
...
public:
    ...
    struct npc_hidden_cultistAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (who->HasAura(SPELL_RIGHTEOUS_VISION))
            {
                if (!me->HasAuraEffect(SPELL_SHADOWFORM, 0)) 
                {
                ...
                switch (uiEventPhase)
                {
                    case 1:
                        switch (me->GetEntry())
                        {
                            case NPC_SALTY_JOHN_THORPE:
                                if (Player* player = ObjectAccessor::GetPlayer(*me, uiPlayerGUID))
                                    Talk(SAY_HIDDEN_CULTIST_1, player);
                                uiEventTimer = 5000;
                                uiEventPhase = 2;
                                break;
                            case NPC_GUARD_MITCHELLS:
                                if (Player* player = ObjectAccessor::GetPlayer(*me, uiPlayerGUID))
                                    Talk(SAY_HIDDEN_CULTIST_2, player);
                                uiEventTimer = 5000;
                                uiEventPhase = 2;
                                break;
                            case NPC_TOM_HEGGER:
                    ...
                    case 2:
                        switch (me->GetEntry())
                        {
                            case NPC_SALTY_JOHN_THORPE:
                                if (Player* player = ObjectAccessor::GetPlayer(*me, uiPlayerGUID))
                                    Talk(SAY_HIDDEN_CULTIST_4, player);
                                if (Player* player = ObjectAccessor::GetPlayer(*me, uiPlayerGUID))
                                    me->SetFacingToObject(player);
                                uiEventTimer = 3000;
                                uiEventPhase = 3;
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->HasAura(SPELL_RIGHTEOUS_VISION) && player->GetQuestStatus(QUEST_THE_HUNT_IS_ON) == QUEST_STATUS_INCOMPLETE)
            player->AddGossipItem(GOSSIP_ITEM_QUEST_THE_HUNT_IS_ON, charGossipItem, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
struct npc_enkilah_abominationAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        _events.ScheduleEvent(EVENT_HOOK, 1s);
    }
            ...
            switch (eventId)
            {
                ...
                case EVENT_HOOK:
                    if (Unit* victim = me->GetVictim())
                        if (me->GetDistance(victim->GetPosition()) > me->GetCombatReach())
                            DoCastVictim(SPELL_SCOURGE_HOOK, false);
                    _events.ScheduleEvent(EVENT_HOOK, 1s);
                    break;
                default:
                    break;
...
enum NotSoBig
{
    ...
    SPELL_AURA_NOTSOBIG_5 = 45683,
};

constexpr uint32 NotSoBigAuras[] = { SPELL_AURA_NOTSOBIG_1, SPELL_AURA_NOTSOBIG_2, SPELL_AURA_NOTSOBIG_3, SPELL_AURA_NOTSOBIG_4, SPELL_AURA_NOTSOBIG_5 };

class spell_magnataur_on_death1_SpellScript : public SpellScript
{
    PrepareSpellScript(spell_magnataur_on_death1_SpellScript);
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        ...
        if (target)
        {
            // check if there is a random (triggered) aura
            for (auto itr : NotSoBigAuras)
            {
                if (target->HasAura(itr))
                {
                    triggeredaura = target->GetAura(itr);
                    break;
                }
            }
            ...
            if (triggeredaura)
            {
                target->RemoveAura(triggeredaura);
            }
        }
    }
    ...
    void Register()
    {
        OnEffectHitTarget += SpellEffectFn(spell_magnataur_on_death1_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
void AddSC_borean_tundra()
{
    ...
    new CreatureScriptLoaderEx<npc_enkilah_abominationAI>("npc_enkilah_abomination");
    new SpellScriptLoaderEx<spell_magnataur_on_death1_SpellScript>("spell_magnataur_on_death1");
}
