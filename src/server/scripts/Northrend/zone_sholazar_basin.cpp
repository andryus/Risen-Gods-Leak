...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            ...
            case GOSSIP_ACTION_INFO_DEF+2:
                ...
                creature->ApplySpellImmune(0,IMMUNITY_ID, SPELL_FREANZYHEARTS_FURY, true); //If not, NPC will pushed away
                player->NearTeleportTo(5410.408f, 4695.524f, -122.176f, 4.717f);
                creature->AI()->DoAction(ACTION_START_VEKJIK);
                break;
        }
    ...
    struct npc_vekjikAI : public ScriptedAI
    {
        ...
        {
            if (action == ACTION_START_VEKJIK)
                events.ScheduleEvent(EVENT_CAST_KNOCKBACK, 1 * IN_MILLISECONDS);
        }
                ...
                switch (eventId)
                {
                    case EVENT_CAST_KNOCKBACK:
                        if (Player* player = me->FindNearestPlayer(10.0f))
                            me->CastSpell(player, SPELL_FREANZYHEARTS_FURY);
                        break;
                    default:
                        break;
                }
    ...
    struct npc_jungle_punch_targetAI : public ScriptedAI
    {
        ...
        void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
        {
            ...
            for (uint8 i = 0; i < 3; ++i)
            {
                ...
                player->MonsterSay(sObjectMgr->GetTrinityStringForDBCLocale(SAY_OFFER), LANG_UNIVERSAL, player);
                sayStep = 1;
                break;
            }
        }
...
public:
    ...
    struct npc_vics_flying_machineAI : public VehicleAI
    {
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (Creature* pilot = me->FindNearestCreature(NPC_PILOT, 10))
                switch (id)
                {
                    case 5:
...
public:
    ...
    struct npc_shardhorn_rhinoAI : public ScriptedAI
    {
        npc_shardhorn_rhinoAI(Creature* creature) : ScriptedAI(creature) { }


        void Reset() override
        {
            boarded = false;
            uiGoreTimer = irand(5000, 9000);
            uiSearchTimer = 7000;
        }
        ...
        {
            if (who)
                DoCast(who, SPELL_RHINO_CHARGE, true);

            if (Unit* bird = me->GetVehicleKit()->GetPassenger(0))
            {
                bird->ExitVehicle();
                boarded = false;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (!boarded && !me->IsInCombat() && me->IsVehicle())
            {
                if (uiSearchTimer <= diff)
                {
                    if (Creature* bird = me->FindNearestCreature(NPC_TICKBIRD, 25.0f, true))
                    {
                        bird->EnterVehicle(me, 0);
                        boarded = true;
                    }
                    else
                        uiSearchTimer = irand(7000, 12000);
                }
                else uiSearchTimer -= diff;
            }
            ...
            if (uiGoreTimer <= diff)
            {
                DoCastVictim(SPELL_GORE, true);
                uiGoreTimer = irand(7000, 12000);
            }
