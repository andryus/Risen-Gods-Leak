...
class npc_tyrannus_double : public CreatureScript
{
    public:
        npc_tyrannus_double() : CreatureScript("npc_tyrannus_double") { }

    struct npc_tyrannus_doubleAI : public ScriptedAI
    {
        npc_tyrannus_doubleAI(Creature* creature) : ScriptedAI(creature) { }

        EventMap events;

        void Reset()
        {
            events.ScheduleEvent(EVENT_SAY, urand(120, 360)*IN_MILLISECONDS);
        }
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            {
                case EVENT_SAY:
                     if (Creature* eye = me->FindNearestCreature(NPC_EYE_OF_THE_LICHKING, 1000.0f))
                      {
                            sCreatureTextMgr->SendChat(eye, SAY_TEXT, NULL, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_MAP);
                      }
                    events.ScheduleEvent(EVENT_SAY, urand(360, 450)*IN_MILLISECONDS);
                    break;
            }
        }
    };
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_tyrannus_doubleAI(creature);
    }
};
