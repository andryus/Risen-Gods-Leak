...
class boss_prince_taldaram_icc : public CreatureScript
{
    public:
        ...
        struct boss_prince_taldaramAI : public BossAI
        {
            ...
            {
                summons.Summon(summon);
                Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 150.0f, true); // first try at distance
                if (!target)
        ...
        void DoAction(int32 action)
        {
            switch(action)
            {
                ...
                case ACTION_KINETIC_BOMB_JUMP:
                {
                    ...
                    float positionZ = (me->GetPositionZ() + 7.0f) > spawnZ ? spawnZ : (me->GetPositionZ() + 7.0f);
                    me->GetMotionMaster()->MovePoint(ACTION_KINETIC_BOMB_JUMP, me->GetPositionX(), me->GetPositionY(), positionZ);
                    break; 
                }
            }
        }
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            switch (events.ExecuteEvent()) 
            {
                case EVENT_SHOCK_VORTEX_AURA:
                    DoCast(me, SPELL_SHOCK_VORTEX_PERIODIC, true);
                    break;
                default:
                    break;
            }
