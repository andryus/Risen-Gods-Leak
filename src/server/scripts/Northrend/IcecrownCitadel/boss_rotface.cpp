...
enum Events
{
    // Rotface
    EVENT_SLIME_SPRAY       = 1,
    EVENT_MUTATED_INFECTION = 2,

    // Precious
    EVENT_DECIMATE          = 3,
    EVENT_MORTAL_WOUND      = 4,
    EVENT_SUMMON_ZOMBIES    = 5,

    EVENT_STICKY_OOZE       = 6,
    EVENT_ENRAGE            = 7,
    EVENT_DOUBLE_DAMAGE     = 8,
    EVENT_NORMAL_DAMAGE     = 9,
    EVENT_EXPLODE           = 10,
    EVENT_OOZE_SUMMON       = 11
};
...
class boss_rotface : public CreatureScript
{
    public:
        ...
        struct boss_rotfaceAI : public BossAI
        {
            ...
            {
                _Reset();
                DoRemoveAuraOnAllPlayerInInstanceRotface(instance, MUTATED_INFECTION);
            ...
            void JustDied(Unit* /*killer*/)
            {
                ...
                me->GetCreatureListWithEntryInGrid(AddList, NPC_LITTLE_OOZE, 200.0f);
                me->GetCreatureListWithEntryInGrid(AddList, NPC_BIG_OOZE, 200.0f);
                if (!AddList.empty())
...
class npc_little_ooze : public CreatureScript
{
    public:
        ...
        struct npc_little_oozeAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* /*who*/)
            {
                events.ScheduleEvent(EVENT_OOZE_SUMMON, 0.1*IN_MILLISECONDS);   
            }
            ...
            void IsSummonedBy(Unit* /*summoner*/)
            {
                ...
                events.ScheduleEvent(EVENT_STICKY_OOZE, 5*IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_STICKY_OOZE:
                            ...
                            events.ScheduleEvent(EVENT_STICKY_OOZE, 15*IN_MILLISECONDS);
                            break;
                        case EVENT_OOZE_SUMMON:
                            if(Unit* target = SelectTarget(SELECT_TARGET_NEAREST))
                                me->getThreatManager().doAddThreat(target, 500000.0f);                       
                            break;
                        default:
                            break;
                    }
    ...
    public:
        ...
        struct npc_big_oozeAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                {
                    if (UnstableAura->GetStackAmount() >= 5 && !Explode)
                    {
                        events.ScheduleEvent(EVENT_EXPLODE, 10);
                        me->SetObjectScale(1.5f);
                        Talk(SAY_GROW_UP_FOUR);
                        Explode = true;
                    }
                    else if (UnstableAura->GetStackAmount() == 2 && !emote_one)
                    {
                        Talk(SAY_GROW_UP_ONE);
                        emote_one = true;               
                    }
                    else if (UnstableAura->GetStackAmount() == 3 && !emote_two)
                    {
                        me->SetObjectScale(1.3f);
                        Talk(SAY_GROW_UP_TWO);
                        emote_two = true;                
                    }
                    else if (UnstableAura->GetStackAmount() == 4 && !emote_three)
                    {
                        me->SetObjectScale(1.4f);
                        Talk(SAY_GROW_UP_THREE);
                        emote_three = true;
                    }
                }
    ...
    public:
        ...
        struct npc_precious_iccAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                _events.ScheduleEvent(EVENT_MORTAL_WOUND, 2*IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_SUMMON_ZOMBIES, urand(20*IN_MILLISECONDS, 22*IN_MILLISECONDS));
                _events.ScheduleEvent(EVENT_DOUBLE_DAMAGE, 4*IN_MILLISECONDS);
                _summons.DespawnAll();
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_DECIMATE:
                            ...
                            _events.ScheduleEvent(EVENT_DECIMATE, urand(20, 25)*IN_MILLISECONDS);
                            break;
                        case EVENT_MORTAL_WOUND:
                            DoCastVictim(SPELL_MORTAL_WOUND, true);
                            _events.ScheduleEvent(EVENT_MORTAL_WOUND, urand(2, 4)*IN_MILLISECONDS);
                            break;
                        case EVENT_SUMMON_ZOMBIES:
                            Talk(EMOTE_PRECIOUS_ZOMBIES);
                            for (uint32 i = 0; i < 11; ++i)
                                DoCast(me, SPELL_AWAKEN_PLAGUED_ZOMBIES, false);
                            _events.ScheduleEvent(EVENT_SUMMON_ZOMBIES, urand(20, 22)*IN_MILLISECONDS);
                            break;
                        case EVENT_DOUBLE_DAMAGE:
                            DoubleDamage();
                            _events.ScheduleEvent(EVENT_NORMAL_DAMAGE, 5*IN_MILLISECONDS);
                            break;
                        case EVENT_NORMAL_DAMAGE:
                            NormalDamage();
                            _events.ScheduleEvent(EVENT_DOUBLE_DAMAGE, 10*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
