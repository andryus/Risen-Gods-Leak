    ...
    public:
        ...
        struct npc_crok_scourgebaneAI : public npc_escortAI
        {
            ...
            void SetData(uint32 type, uint32 data)
            {
                if (type == 1 && data == 1)
                {
                    if (!DoorEventStart)
                    {
                        DoorEvent = true;
                        DoorEventStart = true;
                        _events.ScheduleEvent(EVENT_PORT_SUMMON, 0.1*IN_MILLISECONDS);
                    }
                }
            }
            ...
            void UpdateEscortAI(uint32 const diff)
            {
                ...
                if (!UpdateVictim() && !_isEventActive && !DoorEvent)
                    return;
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_HEALTH_CHECK:
                            ...
                            break;
                        case EVENT_PORT_SUMMON:
                            me->RemoveAura(SPELL_INVISIBILITY);
                            me->setFaction(FACTION_FRIENDLY_TO_ALL);
                            me->CastSpell(me, SPELL_SIMPLE_TELEPORT, true);
                            _events.ScheduleEvent(EVENT_PORT_TALK, 4*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_TALK: 
                            me->GetMotionMaster()->MovePoint(10, 4221.423828f, 2769.398682f, 350.964111f);
                            _events.ScheduleEvent(EVENT_PORT_DESPAWN, 7*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_DESPAWN:
                            Talk(SAY_PORT_SUMMON_TWO);
                            _events.ScheduleEvent(EVENT_PORT_DESPAWN_TWO, 5*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_DESPAWN_TWO: 
                            me->GetMotionMaster()->MovePoint(11, 4264.242676f, 2769.279053f, 349.389893f);
            ...
            void SetData(uint32 type, uint32 data)
            {
                if (type == 1 && data == 1)
                {
                    if (!DoorEventStart)
                    {
                        DoorEventStart = true;
                        DoorEvent = true;
                        Events.ScheduleEvent(EVENT_PORT_SUMMON, 0.1*IN_MILLISECONDS);
                    }
                }
            }
                ...
                while (uint32 eventId = Events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PORT_SUMMON:
                            ...
                            me->setFaction(FACTION_FRIENDLY_TO_ALL);
                            me->CastSpell(me, SPELL_SIMPLE_TELEPORT, true);
                            Events.ScheduleEvent(EVENT_PORT_DESPAWN, 22*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_DESPAWN:  
                            me->GetMotionMaster()->MovePoint(0, 4259.630371f, 2780.150146f, 349.168060f);
                            me->DespawnOrUnsummon(10*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                ...
                while (uint32 eventId = Events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PORT_SUMMON:
                            ...
                            me->setFaction(FACTION_FRIENDLY_TO_ALL);
                            me->CastSpell(me, SPELL_SIMPLE_TELEPORT, true);
                            Events.ScheduleEvent(EVENT_PORT_TALK, 3*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_TALK:
                            Talk(SAY_PORT_SUMMON);
                            Events.ScheduleEvent(EVENT_PORT_DESPAWN, 18*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_DESPAWN:
                            me->GetMotionMaster()->MovePoint(0, 4264.074707f, 2775.119141f, 349.275696f);
                            me->DespawnOrUnsummon(10*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                ...
                while (uint32 eventId = Events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PORT_SUMMON:
                            ...
                            break;
                        case EVENT_PORT_DESPAWN: 
                            me->GetMotionMaster()->MovePoint(0, 4264.130371f, 2764.516602f, 349.295441f);
                            me->DespawnOrUnsummon(10*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                ...
                while (uint32 eventId = Events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PORT_SUMMON:
                            ...
                            me->setFaction(FACTION_FRIENDLY_TO_ALL);
                            me->CastSpell(me, SPELL_SIMPLE_TELEPORT, true);
                            Events.ScheduleEvent(EVENT_PORT_TALK, 9*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_TALK:
                            Talk(SAY_PORT_SUMMON);
                            Events.ScheduleEvent(EVENT_PORT_DESPAWN, 13*IN_MILLISECONDS);
                            break;
                        case EVENT_PORT_DESPAWN:
                            me->GetMotionMaster()->MovePoint(0, 4258.731445f, 2758.809814f, 349.169281f);
                            me->DespawnOrUnsummon(10*IN_MILLISECONDS);
                            break;
                        default:
                            break;
...
class npc_lights_hammer_champions : public CreatureScript
{
    public:
        ...
        struct npc_lights_hammer_championsAI : public SmartAI
        {
            ...
            bool CanAIAttack(Unit const* target) const
            {
                // reset if npcs are in room 2 (otherwise the npcs in the startroom help the players till marrowgar)
                return (me->GetPositionX() < -142.08f) == (target->GetPositionX() < -142.08f) && SmartAI::CanAIAttack(target);
            }
        };
    ...
    public:
        ...
        class spell_icc_giant_insect_swarm_AuraScript : public AuraScript
        {
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                // only spawn on every second aura tick
                doSpawnOnTick = !doSpawnOnTick;
                if (!doSpawnOnTick)
                    return;
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/)
        {
            if (InstanceScript* instance = player->GetInstanceScript())
                if (instance->GetData(DATA_PUTRICIDE_TRAP) == 0)
                {
                    ...
                    GetCreatureListWithEntryInGrid(creatures, player, NPC_PUTRICIDE_TRAP, 100.0f);
