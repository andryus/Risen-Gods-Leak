            ...
            void MovementInform(uint32 type, uint32 point)
            {
                ...
                switch (point)
                {
                    ...
                    case POINT_AIR_PHASE:
                        ...
                        events.ScheduleEvent(EVENT_FROST_BOMB, 9*IN_MILLISECONDS);
                        break;
                    case POINT_AIR_PHASE_FAR:
                        me->SetFacingTo(float(M_PI));
                        events.ScheduleEvent(EVENT_LAND, 30*IN_MILLISECONDS);
                        break;
                    case POINT_LAND:
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_BLISTERING_COLD_YELL:
                            ...
                            events.ScheduleEvent(EVENT_ICY_GRIP, urand(90, 110)*IN_MILLISECONDS, EVENT_GROUP_LAND_PHASE);
                            break;
                        ...
                        case EVENT_FROST_BOMB:
                        {
                            ...
                            events.ScheduleEvent(EVENT_FROST_BOMB, urand(9, 10)*IN_MILLISECONDS);
                            break;
                        }
                        case EVENT_LAND:
                        {
                        ...
                        case EVENT_THIRD_PHASE_CHECK:
                        {
                            if (!_isInAirPhase)
                            {
                                ...
                                events.RescheduleEvent(EVENT_ICY_GRIP, urand(90, 110)*IN_MILLISECONDS);
    ...
    public:
        ...
        class spell_sindragosa_icy_grip_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
                PreventHitDefaultEffect(effIndex);
                if (GetHitUnit())
                    if (!GetHitUnit()->isDead() && GetHitUnit()->GetTypeId() == TYPEID_PLAYER)
                    {
                        if (GetCaster())
                        {
                            float x, y, z;
                            ...
                            GetHitUnit()->GetMotionMaster()->MoveJump(x, y, z, speedXY, 1.0f);
                        }
                    }
            }
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
            {
                if (Unit* target = GetTarget())
                    if (target->GetHealthPct() == 100.0f)
                        target->RemoveAura(SPELL_GLACIAL_STRIKE);
            }
