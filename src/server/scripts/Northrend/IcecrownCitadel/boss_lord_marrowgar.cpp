    ...
    public:
        ...
        struct boss_lord_marrowgarAI : public BossAI
        {
            ...
            void BoneSpikeDespawn()
            {
                ...
                instance->DoRemoveAurasOnPlayerAndPets(SPELL_IMPALED);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_BONE_STORM_MOVE:
                        {
                            ...
                            Unit* unit = SelectTarget(roll_chance_i(90) ? SELECT_TARGET_FARTHEST : SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(me));
                            if (!unit)
                                unit = SelectTarget(SELECT_TARGET_FARTHEST, 0, 0.0f, true);
                            if (unit)
    ...
    public:
        ...
        class spell_marrowgar_coldflame_SpellScript : public SpellScript
        {
            ...
            void SelectTarget(std::list<WorldObject*>& targets)
            {
                ...
                if (!target)
                    target = GetCaster()->GetAI()->SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true, -SPELL_IMPALED); // or the tank if its solo
                if (!target)
                    return;
            ...
            void HandleScriptEffect(SpellEffIndex effIndex)
            {
                ...
                for (uint8 i = 0; i < 4; ++i)
                {
                    ...
                    switch (i)
                    {
                        case 0: o = 0.0f; break;
                        case 1: o = M_PI; break;
                        case 2: o = M_PI + (M_PI/2); break;
                        case 3: o = M_PI*2; break;
                        default: break;
                    }
                    x += 7.0f * cos(o);
                    y += 7.0f * sin(o);
...
class spell_marrowgar_bone_storm : public SpellScriptLoader
{
    public:
        ...
        class spell_marrowgar_bone_storm_SpellScript : public SpellScript
        {
            ...
            {
                if (GetHitUnit()->GetExactDist2d(GetCaster()) < 4.5f)
                    SetHitDamage(int32(GetHitDamage() / 1.5));
                else if (GetHitUnit()->GetExactDist2d(GetCaster()) > 6.0f)
