        ...
        struct npc_commander_eligor_dawnbringerAI : public ScriptedAI
        {
            ...
            void StoreTargets()
            {
                ...
                for (uint8 ii = 0; ii < 3; ++ii)
                {
                    ...
                    me->GetCreatureListWithEntryInGrid(creatureList, AudienceMobs[ii], 15.0f);
...
class npc_wyrmrest_defender : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_1, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        ...
        struct npc_wyrmrest_defenderAI : public VehicleAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
                    DoCastSelf(SPELL_DEFENDER_ON_LOW_HEALTH_EMOTE);
                    hpWarningReady = false;
                }

                if (me->GetZoneId() != DRAGONBLIGHT)
                {
                    me->RemoveAura(SPELL_WYRMREST_DEFENDER_MOUNT);
                    me->RemoveAura(SPELL_FLY_AURA);
                };
