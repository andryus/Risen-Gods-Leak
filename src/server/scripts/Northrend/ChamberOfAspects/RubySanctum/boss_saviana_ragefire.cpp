    ...
    public:
        ...
        struct boss_saviana_ragefireAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_FLIGHT, 27500);
            }
            ...
            void MovementInform(uint32 type, uint32 point) override
            {
                ...
                {
                    case POINT_FLIGHT:
                        me->SetOrientation(SavianaRagefireFlyOutPos.GetOrientation());
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_FLIGHT:
                        {
                            ...
                            Position pos;
                            pos.Relocate(me);
                            float xOffset = SavianaRagefireFlyOutPos.GetPositionX() - me->GetPositionX();
                            float yOffset = SavianaRagefireFlyOutPos.GetPositionY() - me->GetPositionY();
                            pos.m_positionX += xOffset * 0.2f;
                            pos.m_positionY += yOffset * 0.2f;
                            pos.m_positionZ += 10.0f;
                            me->GetMotionMaster()->MoveTakeoff(POINT_TAKEOFF, pos);
                            events.ScheduleEvent(EVENT_FLIGHT, 47500);
