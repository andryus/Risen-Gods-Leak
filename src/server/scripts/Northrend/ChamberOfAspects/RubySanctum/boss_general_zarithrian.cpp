...
enum Events
{
    ...
    EVENT_SUMMON_ADDS_WAVE_1        = 3,
    EVENT_SUMMON_ADDS_WAVE_2        = 4,
    // Onyx Flamecaller
    EVENT_BLAST_NOVA                = 5,
    EVENT_LAVA_GOUT                 = 6,
};
    ...
    public:
        ...
        struct boss_general_zarithrianAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_SUMMON_ADDS_WAVE_1, 40000);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SUMMON_ADDS_WAVE_1:
                        {
                            ...
                            if (Is25ManRaid())
                                events.ScheduleEvent(EVENT_SUMMON_ADDS_WAVE_2, 3000);

                            Talk(SAY_ADDS);
                            events.ScheduleEvent(EVENT_SUMMON_ADDS_WAVE_1, 42000);
                            break;
                        }
                        case EVENT_SUMMON_ADDS_WAVE_2:
                            if (Creature* stalker3 = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ZARITHRIAN_SPAWN_STALKER_1)))
                                stalker3->CastSpell(stalker3, SPELL_SUMMON_FLAMECALLER, false);

                            if (Creature* stalker4 = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ZARITHRIAN_SPAWN_STALKER_2)))
                                stalker4->CastSpell(stalker4, SPELL_SUMMON_FLAMECALLER, false);

                            break;
                        case EVENT_INTIDMDATING_ROAR:
                            DoCastSelf(SPELL_INTIMIDATING_ROAR, true);
                            events.ScheduleEvent(EVENT_INTIDMDATING_ROAR, 42000);
                            break;
                        case EVENT_CLEAVE:
