...
enum Events
{
    ...
    EVENT_ACTIVATE_INFERNOS,
    EVENT_TWILIGHT_MENDING,

    // Living inferno
    EVENT_SPAWN_LIVING_EMBER,

    // Twilight Cutter
    EVENT_TWILIGHT_CUTTER_START_1,
    EVENT_TWILIGHT_CUTTER_START_2,

    // Twilight Cutter
    EVENT_TWILIGHT_CUTTER_START = 23,

    // HACK!
    EVENT_CHECK_PLAYER_IN_FIGHT = 24
};
...
enum Actions
{
    ...
    ACTION_ACTIVATE_EMBERS      = 4,
    ACTION_ACTIVATE_INFERNOS    = 5,
...
{
    CORPOREALITY_INCREASE           = 0,
    CORPOREALITY_DECREASE           = 1
};

Position const HalionSpawnPos = { 3157.37f, 533.995f, 72.8887f, 3.159f };
...
CorporealityEntry const _corporealityReference[MAX_CORPOREALITY_STATE] = {
    {74836, 74831}, // mostly material
    {74835, 74830},
    {74834, 74829},
    {74833, 74828},
    {74832, 74827},
    {74826, 74826}, // balanced
    {74827, 74832},
    {74828, 74833},
    {74829, 74834},
    {74830, 74835},
    {74831, 74836}  // mostly twilight
};
...
struct generic_halionAI : public BossAI
{
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_CHECK_PLAYER_IN_FIGHT, 3000);
        events.ScheduleEvent(EVENT_SET_AGRESSIVE, 1500);
    }
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_CLEAVE:
                ...
                events.ScheduleEvent(EVENT_CLEAVE, urand(7000, 11000));
                break;
            case EVENT_TAIL_LASH:
                DoCastAOE(SPELL_TAIL_LASH);
                events.ScheduleEvent(EVENT_TAIL_LASH, urand(13000, 15000));
                break;
            case EVENT_BREATH:
                DoCastSelf(me->GetEntry() == NPC_HALION ? SPELL_FLAME_BREATH : SPELL_DARK_BREATH);
                events.ScheduleEvent(EVENT_BREATH, urand(18000, 25000));
                break;
            case EVENT_SET_AGRESSIVE:
                me->SetReactState(REACT_AGGRESSIVE);
                break;
            case EVENT_CHECK_PLAYER_IN_FIGHT:
            {
                bool shouldEvade = true;
                for (Player& player : me->GetMap()->GetPlayers())
                    if (!player.IsGameMaster() && player.IsAlive() && player.IsInCombat())
                        {
                            shouldEvade = false;
                            if (player.GetExactDist2d(&HalionSpawnPos) >= 50.5f)
                                player.NearTeleportTo(HalionSpawnPos.GetPositionX(), HalionSpawnPos.GetPositionY(), HalionSpawnPos.GetPositionZ(), 0.0f);
                        }
                if (shouldEvade)
                    EnterEvadeMode();
                events.ScheduleEvent(EVENT_CHECK_PLAYER_IN_FIGHT, 3000);
    ...
    void UpdateAI(uint32 diff) override
    {
        if (!UpdateVictim())
        {
            if (Creature* controller = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_HALION_CONTROLLER)))
                controller->AI()->SetData(DATA_EVADE_METHOD, me->GetEntry() == NPC_HALION ? 0 : 1);
            return;
        }
    ...
    public:
        ...
        struct boss_halionAI : public generic_halionAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.RescheduleEvent(EVENT_CHECK_PLAYER_IN_FIGHT, 3000);
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
                    // still update threat-list for evade mechanic
                    if (!UpdateVictim())
                        if (Creature* controller = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_HALION_CONTROLLER)))
                            controller->AI()->SetData(DATA_EVADE_METHOD, 0);
                    return;
                }
            ...
            void ExecuteEvent(uint32 eventId) override
            {
                switch (eventId)
                {
                    ...
                    {
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true, false, -SPELL_TWILIGHT_REALM))
    ...
    public:
        ...
        struct boss_twilight_halionAI : public generic_halionAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_SOUL_CONSUMPTION, 25000);
    ...
    public:
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_ACTIVATE_EMBERS:
                        ...
                        break;
                    case ACTION_ACTIVATE_INFERNOS:
                        _events.ScheduleEvent(EVENT_ACTIVATE_INFERNOS, 1000);
                        break;
                    default:
                        break;
                }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SHADOW_PULSARS_SHOOT:
                            ...
                            _events.ScheduleEvent(EVENT_SHADOW_PULSARS_SHOOT, urand(30, 35) * IN_MILLISECONDS);
                            break;
                        case EVENT_CHECK_CORPOREALITY:
                            UpdateCorporeality();
                            _events.ScheduleEvent(EVENT_CHECK_CORPOREALITY, 15 * IN_MILLISECONDS);
                            break;
                        case EVENT_ACTIVATE_EMBERS:
                            _summons.DoZoneInCombat(NPC_LIVING_EMBER);
                            break;
                        case EVENT_ACTIVATE_INFERNOS:
                            _summons.DoZoneInCombat(NPC_LIVING_INFERNO);
                            break;
                        case EVENT_TWILIGHT_MENDING:
                            if (ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_HALION))) // Just check if physical Halion is spawned
                                if (ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_TWILIGHT_HALION)))
                    ...
                    case DATA_FIGHT_PHASE:
                        ...
                        switch (value)
                        {
                            case PHASE_ONE:
                                ...
                                break;
                            case PHASE_TWO:
                                _events.ScheduleEvent(EVENT_SHADOW_PULSARS_SHOOT, urand(35, 40) * IN_MILLISECONDS);
                                break;
                            default:
                                break;
                        }
        ...
        private:
            void UpdateCorporeality()
            {
                ...
                {
                    _events.ScheduleEvent(EVENT_TWILIGHT_MENDING, 100ms);
                    _twilightDamageTaken = 0;
                    _materialDamageTaken = 0;
                    return;
                }

                CorporealityEvent action = (_materialDamageTaken < _twilightDamageTaken) ? CORPOREALITY_DECREASE : CORPOREALITY_INCREASE;
                ...
                _instance->DoUpdateWorldState(WORLDSTATE_CORPOREALITY_MATERIAL, 100 - _materialCorporealityValue * 10);
                _instance->DoUpdateWorldState(WORLDSTATE_CORPOREALITY_TWILIGHT, _materialCorporealityValue * 10);
        ...
        struct npc_orb_carrierAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_TWILIGHT_CUTTER_START_1:
                        {
                        ...
                        case EVENT_TWILIGHT_CUTTER_START_2:
                        {
                            Vehicle* vehicle = me->GetVehicleKit();
            ...
            void DoAction(int32 action) override
            {
                if (action == ACTION_SHOOT)
                {
                    ...
                    _events.ScheduleEvent(EVENT_TWILIGHT_CUTTER_START_1, 5250);
                    if (IsHeroic())
                        _events.ScheduleEvent(EVENT_TWILIGHT_CUTTER_START_2, 6 * IN_MILLISECONDS);
                }
            }
        private:
    ...
    public:
        ...
        struct npc_meteor_strike_initialAI : public ScriptedAI
        {
                ...
                switch (action)
                {
                    case ACTION_METEOR_STRIKE_AOE:
                        ...
                        for (std::list<ObjectGuid>::iterator itr = _meteorList.begin(); itr != _meteorList.end(); ++itr)
                            if (Creature* meteor = ObjectAccessor::GetCreature(*me, *itr))
                                meteor->AI()->DoAction(ACTION_METEOR_STRIKE_BURN);
                        break;
                }
            }
            ...
            void IsSummonedBy(Unit* summoner) override
            {
                ...
                DoCastSelf(SPELL_BIRTH_NO_VISUAL); // Unknown purpose
    ...
    public:
        ...
        struct npc_meteor_strikeAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
                    Position pos;
                    me->GetNearPosition(pos, 5.0f, frand(-static_cast<float>(M_PI / 6.0f), static_cast<float>(M_PI / 6.0f)));
                    if (Creature* flame = me->SummonCreature(NPC_METEOR_STRIKE_FLAME, pos, TEMPSUMMON_TIMED_DESPAWN, 25000))
                        flame->AI()->SetGuidData(me->GetGUID());
                }
            }
...
public:
    ...
    struct npc_meteor_strike_flameAI : public ScriptedAI
    {
        ...
        {
            _rootOwnerGuid = guid;
            if (me->GetExactDist2d(&HalionSpawnPos) < 50.0f)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            Position pos;
            me->GetNearPosition(pos, 5.0f, frand(-static_cast<float>(M_PI / 3.f), static_cast<float>(M_PI / 3.f)));
            if (Creature* flame = me->SummonCreature(NPC_METEOR_STRIKE_FLAME, pos, TEMPSUMMON_TIMED_DESPAWN, 25000))
                flame->AI()->SetGuidData(_rootOwnerGuid);
        }
    ...
    public:
        ...
        struct npc_living_infernoAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* /*summoner*/) override
            {
                ...
                DoCastSelf(SPELL_BLAZING_AURA, true);
                ...
                if (me->GetMap()->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
                    _events.ScheduleEvent(EVENT_SPAWN_LIVING_EMBER, 3000);

                if (InstanceScript* instance = me->GetInstanceScript())
                    if (Creature* controller = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_HALION_CONTROLLER)))
                    {
                        controller->AI()->DoAction(ACTION_ACTIVATE_EMBERS);
                        controller->AI()->DoAction(ACTION_ACTIVATE_INFERNOS);
                        controller->AI()->JustSummoned(me);
                    }
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                _events.Update(diff);
                if (_events.ExecuteEvent() != EVENT_SPAWN_LIVING_EMBER)
                    return;

                DoCastSelf(SPELL_SPAWN_LIVING_EMBERS, true);
            }
...
class go_twilight_portal_hallion : public GameObjectScript
{
    public:
        go_twilight_portal_hallion() : GameObjectScript("go_twilight_portal_hallion") { }

        struct go_twilight_portal_hallionAI : public GameObjectAI
        {
            go_twilight_portal_hallionAI(GameObject* gameobject) : GameObjectAI(gameobject),
            ...
            {
                if (Creature* halion = ObjectAccessor::GetCreature(*player, _instance->GetGuidData(DATA_HALION)))
                    halion->GetThreatManager().ModifyThreatByPercent(player, -100);
                if (Creature* twilightHalion = ObjectAccessor::GetCreature(*player, _instance->GetGuidData(DATA_TWILIGHT_HALION)))
                    twilightHalion->GetThreatManager().ModifyThreatByPercent(player, -100);
                if (_spellId != 0)
        ...
        {
            return GetRubySanctumAI<go_twilight_portal_hallionAI>(gameobject);
        }
};
        ...
        class spell_halion_combustion_consumption_periodic_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* aurEff)
            {
                if (aurEff->GetEffIndex() == EFFECT_0)
                {
                    ...
                    {
                        uint32 auraId = GetAura()->GetId();
                        uint32 triggerSpellId = GetSpellInfo()->Effects[EFFECT_0].TriggerSpell;

                        if (Aura* scaleAura = caster->GetAura(SPELL_SCALE_AURA))
                        {
                            int32 stackAmount = scaleAura->GetStackAmount();
                            caster->CastCustomSpell(triggerSpellId, SPELLVALUE_RADIUS_MOD, 10000 + stackAmount * (auraId == 74629 ? 6000 : 5000), (Unit*) nullptr, TRIGGERED_FULL_MASK, nullptr, aurEff, caster->GetGUID());
                        }
                        else
                            caster->CastCustomSpell(triggerSpellId, SPELLVALUE_RADIUS_MOD, 10000, (Unit*) nullptr, TRIGGERED_FULL_MASK, nullptr, aurEff, caster->GetGUID());
                    }
                }
            }
...
class TwilightCutterSelector
{
    public:
        ...
        {
            return !unit->IsInBetween(_caster, _channelTarget, 2.7f);
        }
...
void AddSC_boss_halion()
{
    ...
    new go_twilight_portal_hallion();
