    ...
    public:
        ...
        struct instance_ruby_sanctum_InstanceMapScript : public InstanceScript
        {
            ...
            void OnPlayerEnter(Player* player) override
            {
                ...
                if (GetBossState(DATA_HALION) != IN_PROGRESS)
                {
                    // debuffs in bossfight, which are not removed after dc/servercrash
                    player->RemoveAurasDueToSpell(74562);
                    player->RemoveAurasDueToSpell(74792);
                    player->RemoveAurasDueToSpell(74807);
                    player->RemoveAurasDueToSpell(74567);
                    player->RemoveAurasDueToSpell(74795);
                }
            }
                        ...
                        if (state == IN_PROGRESS)
                        {
                            ...
                            for (std::list<ObjectGuid>::const_iterator itr = trashList.begin(); itr != trashList.end(); ++itr)
                            {
                                if (Creature* temp = instance->GetCreature((*itr)))
                                    if (temp->IsAlive())
                                        temp->EngageWithTarget(halion->GetVictim());
                            }
                        }
