    ...
    public:
        ...
        struct boss_baltharus_the_warbornAI : public BossAI
        {
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_CLONE:
                    {
                        ...
                        if (spell)
                            events.ScheduleEvent(EVENT_CLONE, spell->GetCastTime() + 100);
                        else
                        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (!introPhase)
                {
                    uint32 newHealth = instance->GetData(DATA_BALTHARUS_SHARED_HEALTH);
                    me->LowerPlayerDamageReq(me->GetHealth() - newHealth);
