...
public:
    ...
    struct npc_brunnhildar_prisonerAI : public ScriptedAI
    {
        ...
        {
            freed = false;
            DoCastSelf(SPELL_ICE_PRISON, true);
        }
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell) override
        {
            ...
            {
                DoCastSelf(SPELL_FREE_PRISONER, true);
                me->CastSpell(caster, SPELL_RIDE_DRAKE, true);
                DoCastSelf(SPELL_SHARD_IMPACT, true);
                freed = true;
            }
        }
    };
                    ...
                    if (Vehicle* vehicle = me->GetVehicleKit())
                        if (Unit* player = vehicle->GetPassenger(0))
                            if (player->GetTypeId() == TYPEID_PLAYER)
                            {
                                ...
                                DoCastSelf(SPELL_KILL_CREDIT_DRAKE, true);
                                player->ExitVehicle();
                            }
                    break;
            }
    ...
    public:
        ...
        struct npc_hyldsmeet_drakeriderAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* killer)
            {
                ...
                {
                    player->KilledMonsterCredit(NPC_DRAKKENSRYD_KC);
                }
            }
    ...
    public:
        ...
        class npc_hyldsmeet_protodrakeAI : public CreatureAI
        {
            public:
                ...
                void UpdateAI(uint32 diff) override
                {
                    if (!me->isMoving() && !me->IsSummon())
                    {
                        auto i_path = sWaypointMgr->GetPath(me->GetWaypointPath());
                        if (i_path)
                        {
                            ...
                            me->NearTeleportTo(points.begin()->x, points.begin()->y, points.begin()->z, 0.f);
