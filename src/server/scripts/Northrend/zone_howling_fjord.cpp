...
static float afSummon[] = { 812.22f, -4709.84f, -96.143f };
...
## npc_dragonflayer_prisoner_cagedAI
######*/
...
enum PrisonersOfWyrmskull
{
    ...
    EVENT_SAY_FREE_TEXT         = 1,
    EVENT_SAY_RANDOM,
    EVENT_START_FOLLOW,

    ACTION_CAGE_OPENED          = 1,

    TEXT_SAY_RANDOM             = 0,
    TEXT_SAY_FREE_TEXT,          
   
    SPELL_PRISONER_FREE         = 43006,
    SPELL_PRISONER_CREDIT       = 43094,

    SPELL_SUMMON_MAGE           = 43028,
    SPELL_SUMMON_PRIEST         = 43030,
    SPELL_SUMMON_PALA           = 43032,
    SPELL_SUMMON_WARRIOR        = 42984,

};

constexpr uint32 prisonerentry[] =
{
    NPC_PRISONER_PRIEST,
    NPC_PRISONER_MAGE,
...
struct go_dragonflayer_cageAI : public GameObjectAI
{
    ...
    void Reset() override
    {
        go->setActive(true);
        prisonerGUID.Clear();
        prisonerGUID = go->SummonCreature(prisonerentry[urand(0, 3)], *go)->GetGUID();
    }

    void OnStateChanged(uint32 state, Unit* unit) override
    {
        if (state == GO_STATE_ACTIVE_ALTERNATIVE) // Cage open 
        {
            if (Creature* prisoner = ObjectAccessor::GetCreature(*go, prisonerGUID)) 
            {
                if (unit->GetTypeId() == TYPEID_PLAYER) // Player opened cage
                {
                    unit->ToPlayer()->CastSpell(prisoner, SPELL_PRISONER_FREE, true);
                }
            }
        }
        if (state == GO_STATE_READY) // Cage Closed - Respawn Prisoner
        {
            Reset();
        }
    }
...
// npc_dragonflayer_prisoner_caged

struct npc_dragonflayer_prisoner_cagedAI : public ScriptedAI 
{
    npc_dragonflayer_prisoner_cagedAI(Creature* creature) : ScriptedAI(creature) { }

    void Reset() override 
    {
        playerGUID.Clear();
        events.ScheduleEvent(EVENT_SAY_RANDOM, (std::chrono::seconds)((urand(60, 120))));
    }

    void SpellHit(Unit* caster, SpellInfo const* spell) override 
    { 
        if (spell->Id == SPELL_PRISONER_FREE) // Player casts this spell on cage opened
        {
            if (caster->GetTypeId() == TYPEID_PLAYER) 
            {
                playerGUID = caster->GetGUID();
            }
            events.Reset(); // Delete Scheduled Random Text Say
            events.ScheduleEvent(EVENT_SAY_FREE_TEXT, 1s);
            events.ScheduleEvent(EVENT_START_FOLLOW, 5s);
        }
    }
    ...
    void UpdateAI(uint32 diff) override 
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
                {
                    Talk(TEXT_SAY_RANDOM);
                    events.RescheduleEvent(EVENT_SAY_RANDOM, (std::chrono::seconds)((urand(60, 120))));
                }
            }
            break;
            case EVENT_SAY_FREE_TEXT:
            {
                Talk(TEXT_SAY_FREE_TEXT);
            }
            break;
            case EVENT_START_FOLLOW:
            {
                ...
                if (!playerGUID.Empty)
                {
                    ...
                    {
                        me->CastSpell(player, SPELL_PRISONER_CREDIT, true);
                        switch (me->GetEntry()) 
                        {
                        case NPC_PRISONER_PRIEST :
                            spellid = SPELL_SUMMON_PRIEST;
                            break;
                        case NPC_PRISONER_MAGE :
                            spellid = SPELL_SUMMON_MAGE;
                            break;
                        case NPC_PRISONER_PALADIN:
                            spellid = SPELL_SUMMON_PALA;
                            break;
                        case NPC_PRISONER_WARRIOR :
                            spellid = SPELL_SUMMON_WARRIOR;
                            break;
                        default: spellid = 0;
                            break;
                        }
...
void AddSC_howling_fjord()
{
    ...
    new GameObjectScriptLoaderEx<go_dragonflayer_cageAI>("go_dragonflayer_cage");
    new CreatureScriptLoaderEx<npc_dragonflayer_prisoner_cagedAI>("npc_dragonflayer_prisoner_caged");
 }
