    ...
    struct npc_drakuru_shacklesAI : public ScriptedAI
    {
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_UNLOCK_SHACKLE)
            {
                ...
                {
                    caster->ToPlayer()->KilledMonsterCredit(NPC_RAGECLAW);
                    me->setDeathState(JUST_DIED);
                    if (Creature* rageclaw = me->FindNearestCreature(NPC_RAGECLAW, 5.0f, true))
                        me->AI()->DoCast(rageclaw, SPELL_FREE_RAGECLAW, true);
                }
            }
        }
    };
...
class spell_q12861_unlock_shackle : public SpellScriptLoader
{
public:
    spell_q12861_unlock_shackle() : SpellScriptLoader("spell_q12861_unlock_shackle") { }

    class spell_q12861_unlock_shackle_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q12861_unlock_shackle_SpellScript);

        SpellCastResult CheckRequirement()
        {
            if (Creature* target = GetCaster()->FindNearestCreature(NPC_RAGECLAW, 3.0f))
                if (!target->HasAura(SPELL_UNSHACKLED))
        ...
        {
            OnCheckCast += SpellCheckCastFn(spell_q12861_unlock_shackle_SpellScript::CheckRequirement);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_q12861_unlock_shackle_SpellScript();
    }
};
...
public:
    ...
    struct npc_captured_rageclawAI : public ScriptedAI
    {
        ...
        {
            me->setFaction(35);
            me->HandleEmoteCommand(ANIM_EMOTE_KNEEL);

            if (Creature* shackles = me->FindNearestCreature(NPC_DRAKURU_SHACKLES, 5.0f, true))
            {
                shackles->AI()->DoCastSelf(SPELL_LEFT_CHAIN, true);
                shackles->AI()->DoCastSelf(SPELL_RIGHT_CHAIN, true);
            }            
        }
        ...
        void SpellHit(Unit* /*caster*/, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_FREE_RAGECLAW)
            {
                ...
                DoCastSelf(SPELL_UNSHACKLED, true);
                Talk(SAY_RAGECLAW);
        ...
        class spell_random_ingredient_SpellScript : public SpellScript
        {
            ...
            void HandleScriptEffect(SpellEffIndex /* effIndex */)
            {
                if (Player* player = GetHitPlayer())
                {
                    ...
                    if (Creature* finklestein = player->FindNearestCreature(NPC_FINKLESTEIN, 25.0f))
                    {
            ...
            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE)
                    if (target->HasAura(SPELL_ALCHEMIST_APPRENTICE_INVISBUFF))
                        if (Creature* finklestein = target->FindNearestCreature(NPC_FINKLESTEIN, 100.0f))
                        {
...
void AddSC_zuldrak()
{
    ...
    new spell_q12861_unlock_shackle();
