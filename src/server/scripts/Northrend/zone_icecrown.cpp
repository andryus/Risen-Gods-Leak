...
public:
    ...
    struct npc_vereth_the_cunningAI : public ScriptedAI
    {
...

        {
            ...
            if (who->GetEntry() == NPC_LITHE_STALKER_QUEST && me->IsWithinDistInMap(who, 5.0f))
...
enum BlessedBanner
{
    ...
    HALOF_SAY_SPAWN                     = 0, // "THE MASTER'S WILL BE DONE!"
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_START_FIGHT:
                    {
                        if (Creature* LK = me->FindNearestCreature(NPC_LK, 100))
                        ...
                        if (Creature* tempsum = DoSummon(NPC_SCOURGE_DRUDGE, Mason3Pos[0]))
                            {
                                ...
                                tempsum->AI()->AttackStart(me->FindNearestCreature(NPC_BLESSED_BANNER, 100));
                            }
                        if (urand(0, 1) == 0)
                        {
                ...
                case EVENT_HALOF:
                    {
                        ...
                        if (Creature* tempsum = DoSummon(NPC_HALOF_THE_DEATHBRINGER, DalforsPos[0]))
                        {
                            ...
                            tempsum->SetHomePosition(DalforsPos[2]);
                            tempsum->AI()->AttackStart(me->FindNearestCreature(NPC_BLESSED_BANNER, 100));
                            tempsum->AI()->Talk(HALOF_SAY_SPAWN);
                        }
                    }
                    break;
                case EVENT_ENDED:
            ...
            if (PhaseCount == 8)
                if (Creature* Halof = ObjectAccessor::GetCreature(*me, guidHalof))
                    if (Halof->isDead())
                    {
                        DoCastSelf(SPELL_CRUSADERS_SPIRE_VICTORY, true);
...
enum NjorndarProDrake
{
    SPELL_NJORNDAR_PROTO_DRAKE  = 57401,
    AREA_NJORNDAR_VILLAGE       = 4526,
...
public:
    ...
    struct npc_njorndar_proto_drakeAI : public VehicleAI
    {
        npc_njorndar_proto_drakeAI(Creature* creature) : VehicleAI(creature) { }
        ...
        void PassengerBoarded(Unit* who, int8 /*seatId*/, bool apply)
        {
            ...
            {
                if (!apply)
                    who->RemoveAurasDueToSpell(SPELL_NJORNDAR_PROTO_DRAKE);
                else
                {
                    _playerGUID = who->GetGUID();
                    _events.ScheduleEvent(EVENT_CHECK_AREA, Seconds(3));
                }
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (_events.ExecuteEvent())
            {
                case EVENT_CHECK_AREA:
                    if (me->GetAreaId() != AREA_NJORNDAR_VILLAGE)
                    {
                        if (++_countAreaCheck > 4)
                        {
