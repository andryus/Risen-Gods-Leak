...
public:
    ...
    struct npc_emilyAI : public npc_escortAI
    {
        ...
        {
            if (Creature* Mrfloppy = me->FindNearestCreature(NPC_MRFLOPPY, 50.0f))
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            {
                case 9:
                    if (Creature* Mrfloppy = me->FindNearestCreature(NPC_MRFLOPPY, 100.0f))
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_PERILOUS_ADVENTURE)
        {
            ...
            if (Creature* Mrfloppy = creature->FindNearestCreature(NPC_MRFLOPPY, 180.0f))
...
public:
    ...
    struct npc_mrfloppyAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* Who) override
        {
            if (Creature* Emily = me->FindNearestCreature(NPC_EMILY, 50.0f))
            {
                switch (Who->GetEntry())
                {
        ...
        void SpellHit(Unit* Caster, const SpellInfo* Spell) override
        {
             if (Spell->Id == SPELL_OUTHOUSE_GROANS)
             {
                ...
                if (_counter < 5)
                    ...
                    _counter = 0;
                DoCastSelf(SPELL_DUST_FIELD, true);
                switch (_gender)
                {
                    case GENDER_FEMALE:
...
public:
    ...
    struct npc_rocket_propelled_warheadAI : public VehicleAI
    {
        ...
        void PassengerBoarded(Unit* who, int8 /*seatId*/, bool apply) override
        {
            ...
            {
                DoCastSelf(SPELL_VEHICLE_WARHEAD_FUSE);
        ...
        void FinishQuest(bool success, uint32 faction)
        {
            ...
            if (success)
                DoCastSelf(faction == ALLIANCE ? SPELL_ALLIANCE_KILL_CREDIT_TORPEDO : SPELL_HORDE_KILL_CREDIT_TORPEDO);

            DoCastSelf(SPELL_DETONATE);
            me->RemoveAllAuras();
            me->SetVisible(false);
