            ...
            if (Phase == 1 || Phase == 3)
            {
                ...
                if (EntangleTimer <= diff)
                {
                    if (!Entangle)
                    {
                        ...
                        {
                            float x, y, z;
                            me->GetVictim()->GetNearPoint(me, x, y, z, me->GetCombatReach(), 8.f, me->GetVictim()->GetAngle(me));
                ...
                if (CheckTimer <= diff)
                {
                    ...
                    {
                        CastShootOrMultishot();
                        if (me->HasUnitState(UNIT_STATE_CHASE) && !me->IsWithinMeleeRange(me->GetVictim()))
                            me->GetMotionMaster()->Clear();
                    }
