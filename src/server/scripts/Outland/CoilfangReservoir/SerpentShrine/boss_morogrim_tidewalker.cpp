...
public:
    ...
    struct boss_morogrim_tidewalkerAI : public ScriptedAI
    {
        ...
        {
            Map::PlayerList list = me->GetMap()->GetPlayers();
            Playercount = list.size();
            StartEvent();
            if (me->GetVictim())
            {
                std::list<Creature*> HelperList;
                me->GetCreatureListWithEntryInGrid(HelperList, NPC_TIDEWALKER_SHAMAN,      100.0f);
                me->GetCreatureListWithEntryInGrid(HelperList, NPC_TIDEWALKER_DEPTH_SEER,  100.0f);
                me->GetCreatureListWithEntryInGrid(HelperList, NPC_TIDEWALKER_HYDROMANCER, 100.0f);
                me->GetCreatureListWithEntryInGrid(HelperList, NPC_TIDEWALKER_HARPOONER,   100.0f);
                if (!HelperList.empty())
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (HomePositionCheckTimer <= diff)
            {
                if (me->GetDistance(me->GetHomePosition()) > 87.0f)
                    EnterEvadeMode();
                HomePositionCheckTimer = 2000;
            }
            else
                HomePositionCheckTimer -= diff;
