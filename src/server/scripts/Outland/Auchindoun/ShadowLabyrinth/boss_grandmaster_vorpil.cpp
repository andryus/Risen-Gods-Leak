                        ...
                        case EVENT_DRAW_SHADOWS:
                            {
                                ...
                                me->NearTeleportTo(VorpilPosition.GetPositionX(), VorpilPosition.GetPositionY(), VorpilPosition.GetPositionZ(), VorpilPosition.GetOrientation());
    ...
    public:
        ...
        struct npc_voidtravelerAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                if (_moveTimer <= diff)
                {
                    Creature* Vorpil = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_GRANDMASTER_VORPIL)));
                    if (!Vorpil)
                        return;
                    ...
                    {
                        DoCastSelf(SPELL_SACRIFICE, false);
                        _sacrificed = true;
                        _moveTimer = 500;
                        return;
                    }
