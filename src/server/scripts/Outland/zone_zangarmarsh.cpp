...
enum AshyenAndKeleth
{
    ...
    GOSSIP_ITEM_BLESS_ASH       = 7559,
    GOSSIP_ITEM_BLESS_KEL       = 7560,
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (player->GetReputationRank(942) > REP_NEUTRAL)
        {
            if (creature->GetEntry() == NPC_ASHYEN)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_BLESS_ASH, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

            if (creature->GetEntry() == NPC_KELETH)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_BLESS_KEL, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        }
...
enum ElderKurutiMisc
{
    GOSSIP_ITEM_KUR = 7582
};
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (player->GetQuestStatus(9803) == QUEST_STATUS_INCOMPLETE)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_KUR, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_KUR, 1), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                player->SEND_GOSSIP_MENU(9227, creature->GetGUID());
                break;
            case GOSSIP_ACTION_INFO_DEF + 1:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_ITEM_KUR, 2), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
