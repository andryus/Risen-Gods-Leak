...
enum Spells
{
    ...
    //Phase 2 Weapon Spells
    SPELL_W_SHIELD_BASH                  = 36988,
    SPELL_W_SHOOT                        = 36980,
    SPELL_W_MULTISHOOT                   = 36979,
    SPELL_W_FROSTNOVA                    = 36989,
    SPELL_W_FROSTBOLD                    = 36990,
    SPELL_W_TRASH                        = 3391,
    SPELL_W_HOLY_NOVA                    = 36985,
    SPELL_W_HEAL                         = 36983,
    SPELL_W_REND                         = 36991,
    SPELL_W_WHIRLWIND                    = 36981,
    SPELL_W_BLINK                        = 36994,
...
enum eWeapons
{
    eWeapon_StaffOfDisintegration    = 21274,
    eWeapon_NetherstrandLongbow      = 21268,
    eWeapon_CosmicInfuser            = 21270,
    eWeapon_WarpSlicer               = 21272,
    eWeapon_Devastation              = 21269,
    eWeapon_PhaseshiftBulwark        = 21273,
    eWeapon_InfinityBlades           = 21271,
};

enum ItemIDs
{
    ITEM_STAFF_OF_DISINTEGRATION     = 30313,
    ITEM_NETHERSTRAND_LOBGBOW        = 30318,
    ITEM_COSMETIC_INFUSER            = 30317,
    ITEM_WARP_SLICER                 = 30311,
    ITEM_DEAVASTATION                = 30316,
    ITEM_PHASESHIFT_BULWARK          = 30314,
    ITEM_INVINITY_BLADE              = 30312
};
...
static const Position afGravityPos{ 795.0f, 0.0f, 70.0f };
...
struct advisorbase_ai : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        if (instance->GetData(DATA_KAELTHASEVENT) == 1 || instance->GetData(DATA_KAELTHASEVENT) == 3)
            if (Creature* Kaelthas = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_KAELTHAS))))
    ...
    void Revive(Unit* /*Target*/)
    {
        ...
        DoCastSelf(SPELL_RES_VISUAL, false);
        DelayRes_Timer = 2000;
    }
...
class boss_kaelthas : public CreatureScript
{
    public:
        ...
        struct boss_kaelthasAI : public BossAI
        {
            ...
            void StartEvent()
            {
                m_auiAdvisorGuid[0] = ObjectGuid(instance->GetGuidData(DATA_THALADREDTHEDARKENER));
                m_auiAdvisorGuid[1] = ObjectGuid(instance->GetGuidData(DATA_LORDSANGUINAR));
                m_auiAdvisorGuid[2] = ObjectGuid(instance->GetGuidData(DATA_GRANDASTROMANCERCAPERNIAN));
                m_auiAdvisorGuid[3] = ObjectGuid(instance->GetGuidData(DATA_MASTERENGINEERTELONICUS));
            ...
            void JustDied(Unit* killer) override
            {
                ...
                me->NearTeleportTo(me->GetHomePosition().GetPositionX(), me->GetHomePosition().GetPositionY(), me->GetHomePosition().GetPositionZ(), me->GetOrientation());
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch (Phase)
                {
                        ...
                        switch (PhaseSubphase)
                        {
                            ...
                            case 0:
                                if (Phase_Timer <= diff)
                                {
                                    ...
                                    {
                                        player.DestroyItemCount(ITEM_STAFF_OF_DISINTEGRATION, 1, true);
                                        player.DestroyItemCount(ITEM_NETHERSTRAND_LOBGBOW, 1, true);
                                        player.DestroyItemCount(ITEM_COSMETIC_INFUSER, 1, true);
                                        player.DestroyItemCount(ITEM_WARP_SLICER, 1, true);
                                        player.DestroyItemCount(ITEM_DEAVASTATION, 1, true);
                                        player.DestroyItemCount(ITEM_PHASESHIFT_BULWARK, 1, true);
                                        player.DestroyItemCount(ITEM_INVINITY_BLADE, 1, true);
                                    }
                    ...
                    case 2:
                    {
                        ...
                        if (PhaseSubphase == 1)
                        {
                            DoCastSelf(SPELL_SUMMON_WEAPONS, false);
                            ...
                            {
                                DoCastSelf(m_auiSpellSummonWeapon[i], true);
                                //Weapon = (Creature*)(Unit::GetUnit((*Weapon), m_auiAdvisorGuid[i]));
                                //JustSummoned(Weapon);
                            }

                            Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0);
                            /* Double Code make no sense
                            for (uint32 i = 0; i < uiMaxWeapon; ++i)
                            {
                                DoCastSelf(m_auiSpellSummonWeapon[i], true);
                            } */
                    ...
                    case 6:
                    {
                        ...
                        {
                            DoCastSelf(SPELL_PHOENIX_ANIMATION);
                        ...
                        if (Phase == 4)
                        {
                            if (HealthBelowPct(50))
                            {
                                ...
                                me->StopMoving();
                                me->InterruptNonMeleeSpells(false);
                                me->NearTeleportTo(afGravityPos);
                                DoCastSelf(SPELL_FULLPOWER);
                                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                            }
                                    ...
                                    case 0:
                                        ...
                                        for (i = threatlist.begin(); i != threatlist.end(); ++i)
                                        {
                                            ...
                                            if (unit && (unit->GetTypeId() == TYPEID_PLAYER))
                                            {
                                                ...
                                                DoTeleportPlayer(unit, afGravityPos.GetPositionX(), afGravityPos.GetPositionY(), afGravityPos.GetPositionZ(), unit->GetOrientation());
                                            }
                                        }
    ...
    public:
        ...
        struct npc_kael_flamestrikeAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                if (!Casting)
                {
                    DoCastSelf(SPELL_FLAME_STRIKE_VIS);
                    Casting = true;
                }
                ...
                if (Timer <= diff)
                {
                    ...
                    {
                        me->InterruptNonMeleeSpells(false);
                        DoCastSelf(SPELL_FLAME_STRIKE_DMG);
                    }
                    else
                        me->KillSelf();
...
//Fertig
class weapon_StaffOfDisintegration : public CreatureScript
{
    public:

        weapon_StaffOfDisintegration()
            : CreatureScript("weapon_StaffOfDisintegration")
        {
        }
        struct weapon_StaffOfDisintegrationAI : public ScriptedAI
        {
            weapon_StaffOfDisintegrationAI(Creature* creature) : ScriptedAI(creature) {}

            uint32 Frostbold_Timer;
            uint32 Frostnova_Timer;

            void Reset()
            {
                Frostbold_Timer = 1000;
                Frostnova_Timer = 24000;
            }
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                //Do Frostbold as Basic-Attack
                if (Frostbold_Timer <= diff)
                {
                    DoCastVictim(SPELL_W_FROSTBOLD);
                    Frostbold_Timer = 1000;
                }
                else
                    Frostbold_Timer -= diff;

                //Do FrostNova
                if (Frostnova_Timer <= diff)
                {
                    DoCast(SPELL_W_FROSTNOVA);
                    Frostnova_Timer = 24000;
                }
                else 
        ...
        {
            return new weapon_StaffOfDisintegrationAI(creature);
        }
};

//Fertig
class weapon_NetherstrandLongbow : public CreatureScript
{
    public:

        weapon_NetherstrandLongbow()
            : CreatureScript("weapon_NetherstrandLongbow")
        {
        }
        struct weapon_NetherstrandLongbowAI : public ScriptedAI
        {
            weapon_NetherstrandLongbowAI(Creature* creature) : ScriptedAI(creature) {}
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                //DO Aggro reset if get hit by meele attack
                            if (me->IsWithinMeleeRange(me->GetVictim()))
                            {
                                //if in melee range blink away
                                    DoCast(SPELL_W_BLINK);
                                
                                //give aggro to someone
                                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                {
                                    AddThreat(target, 5000000.0f);
                                }
                        ...
                        if(me->GetVictim()->IsInRange(me,2,25,true))
                        {
                            me->StopMoving();
                        }
                    //}
                //Do Shoot as Basic-Attack
                if (Shoot_Timer <= diff)
                {
                    DoCastVictim(SPELL_W_SHOOT);
                    Shoot_Timer = 1400;
                }
                else
                    Shoot_Timer -= diff;
                ...
                {
                    //DoCastVictim(SPELL_W_MULTISHOOT);
                    DoCastVictim(SPELL_W_MULTISHOOT);
                    MultiShoot_Timer = 10000;
                }
                else
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new weapon_NetherstrandLongbowAI(creature);
        }
};

//Fertig heal evtl fehlerhaft
class weapon_CosmicInfuser : public CreatureScript
{
    public:

        weapon_CosmicInfuser()
            : CreatureScript("weapon_CosmicInfuser")
        {
        }
        struct weapon_CosmicInfuserAI : public ScriptedAI
        {
            weapon_CosmicInfuserAI(Creature* creature) : ScriptedAI(creature) {}
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                {
                  DoCastVictim(SPELL_W_HOLY_NOVA);
                  HolyNova_Timer = 5000;
                ...
                if (GreaterHeal_Timer <= diff)
                {
                    ...
                    //1 kontrollieren
                    //+++++++++++++++
                    _healRandom = urand(0, 6);
                    if(_healRandom == 0)
                    {
                        if (Creature* waffe = me->FindNearestCreature(eWeapon_StaffOfDisintegration, 30.0f))
                        DoCast(waffe, SPELL_W_HEAL);
                    }
                    else if (_healRandom == 1)
                    {
                        if (Creature* waffe = me->FindNearestCreature(eWeapon_CosmicInfuser, 30.0f))
                        DoCast(waffe, SPELL_W_HEAL);
                    }
                    else if (_healRandom == 2)
                    {
                        if (Creature* waffe = me->FindNearestCreature(eWeapon_WarpSlicer, 30.0f))
                        DoCast(waffe, SPELL_W_HEAL);
                    }
                    else if (_healRandom == 3)
                    {
                        if (Creature* waffe = me->FindNearestCreature(eWeapon_Devastation, 30.0f))
                        DoCast(waffe, SPELL_W_HEAL);
                    }
                    else if (_healRandom == 4)
                    {
                        if (Creature* waffe = me->FindNearestCreature(eWeapon_PhaseshiftBulwark, 30.0f))
                        DoCast(waffe, SPELL_W_HEAL);
                    }
                    else if (_healRandom == 5)
                    {
                        if (Creature* waffe = me->FindNearestCreature(eWeapon_InfinityBlades, 30.0f))
                        DoCast(waffe, SPELL_W_HEAL);
                    }
                    else
                    {
                        if (Creature* waffe = me->FindNearestCreature(eWeapon_NetherstrandLongbow, 30.0f))
                        DoCast(waffe, SPELL_W_HEAL);
                    }

                    GreaterHeal_Timer = urand(10000, 20000);
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new weapon_CosmicInfuserAI(creature);
        }
};

//Fertig
class weapon_WarpSlicer : public CreatureScript
{
    public:

        weapon_WarpSlicer()
            : CreatureScript("weapon_WarpSlicer")
        {
        }
        struct weapon_WarpSlicerAI : public ScriptedAI
        {
            weapon_WarpSlicerAI(Creature* creature) : ScriptedAI(creature) {}
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new weapon_WarpSlicerAI(creature);
        }
};

//Fertig
class weapon_Devastation : public CreatureScript
{
    public:

        weapon_Devastation()
            : CreatureScript("weapon_Devastation")
        {
        }
        struct weapon_DevastationAI : public ScriptedAI
        {
            weapon_DevastationAI(Creature* creature) : ScriptedAI(creature) {}

            uint32 Whirlwind_Timer;

            void Reset()
            {
                Whirlwind_Timer = 11000;
            }
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                {
                    DoCastVictim(SPELL_W_WHIRLWIND);
                    Whirlwind_Timer = urand(10000, 12000);
                 } 
                else 
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new weapon_DevastationAI(creature);
        }
};

//Fertig
class weapon_PhaseshiftBulwark : public CreatureScript
{
    public:

        weapon_PhaseshiftBulwark()
            : CreatureScript("weapon_PhaseshiftBulwark")
        {
        }
        struct weapon_PhaseshiftBulwarkAI : public ScriptedAI
        {
            weapon_PhaseshiftBulwarkAI(Creature* creature) : ScriptedAI(creature)
            {
                // Shield Spike
                DoCastSelf(37016,true); 
            }
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                if (Shield_Bash_Timer <= diff)
                {
                    if(me->GetVictim()->IsNonMeleeSpellCast(false))
                    {
                        DoCastVictim(SPELL_W_SHIELD_BASH);
                        Shield_Bash_Timer = 12000;
                    }
                }
                else
                    Shield_Bash_Timer -= diff;
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new weapon_PhaseshiftBulwarkAI(creature);
        }
};

//Fertig
class weapon_InfinityBlades : public CreatureScript
{
    public:

        weapon_InfinityBlades()
            : CreatureScript("weapon_InfinityBlades")
        {
        }
        struct weapon_InfinityBladesAI : public ScriptedAI
        {
            weapon_InfinityBladesAI(Creature* creature) : ScriptedAI(creature) {}
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new weapon_InfinityBladesAI(creature);
        }
};
...
void AddSC_boss_kaelthas()
{
    ...
    new weapon_StaffOfDisintegration();
    new weapon_NetherstrandLongbow();
    new weapon_CosmicInfuser();
    new weapon_WarpSlicer();
    new weapon_Devastation();
    new weapon_PhaseshiftBulwark();
    new weapon_InfinityBlades();
}
