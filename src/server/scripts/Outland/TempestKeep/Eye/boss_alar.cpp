...
enum Spells
{
    ...
    SPELL_DIVE_BOMB              = 35181, // after watching tonns of movies, set cooldown to 40+rand32()%5.
    ...
    AURA_ASHTONGUE_RUSE          = 39527, // Aura needed for questcomplete 'Ruse of the Ashtongue'
    QUEST_RUSE_OF_THE_ASHTONGUE  = 10946  // Questid
};
        ...
        struct boss_alarAI : public BossAI
        {
            ...
            void CompleteQuest()
            {
                ...
                for (Player& player : map->GetPlayers())
                    if (me->IsInRange(&player, 0.0f, 100.0f) && player.HasAura(AURA_ASHTONGUE_RUSE))
                        player.CompleteQuest(QUEST_RUSE_OF_THE_ASHTONGUE);
            }
                    ...
                    if (WaitTimer)
                    {
                        if (WaitTimer <= diff)
                        {
                            ...
                            case WE_PLATFORM:
                                ...
                                break;
                            case WE_QUILL:
                                DoCastSelf(SPELL_FLAME_QUILLS, true);
                            ...
                            case WE_METEOR:
                                ...
                                DoCastSelf(SPELL_DIVE_BOMB_VISUAL, false);
                                WaitEvent = WE_DIVE;
                                WaitTimer = 4000;
                                return;
                            case WE_DIVE:
                            ...
                            case WE_SUMMON:
                                ...
                                me->SetDisplayId(me->GetNativeDisplayId());
                                DoCastSelf(SPELL_REBIRTH_2, true);
                                break;
                            case WE_DUMMY:
                            default:
                                break;
        ...
        struct npc_ember_of_alarAI : public ScriptedAI
        {
            ...
            void DamageTaken(Unit* killer, uint32& damage) override
            {
                if (damage >= me->GetHealth() && killer != me && !toDie)
                {
                    ...
                    {
                        if (Unit* Alar = ObjectAccessor::GetUnit(*me, ObjectGuid(instance->GetGuidData(DATA_ALAR))))
                        {
