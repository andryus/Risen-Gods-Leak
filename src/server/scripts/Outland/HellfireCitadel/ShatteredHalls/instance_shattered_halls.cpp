    ...
    public:
        ...
        struct instance_shattered_halls_InstanceMapScript : public InstanceScript
        {
            instance_shattered_halls_InstanceMapScript(Map* map) : InstanceScript(map)
            {
                ...
                nethekurseDoor1GUID.Clear();
                nethekurseDoor2GUID.Clear();
                executionTimer = 0;
                executed = 0;
                _team = 0;
            }
    ...
    public:
        ...
        struct boss_shattered_executionerAI : public BossAI
        {
            ...
            void JustDied(Unit* killer) override
            {
                ...
                {
                    uint32 qId = player.GetTeam() == ALLIANCE ? QUEST_IMPRISONED_A : QUEST_IMPRISONED_H;
                    if (player.GetQuestStatus(qId) != QUEST_STATUS_NONE && player.GetQuestStatus(qId) != QUEST_STATUS_FAILED)
