...
class npc_abyssal : public CreatureScript
{
    public: 
        ...
        struct npc_abyssalAI : public ScriptedAI
        {
            ...
            {
                trigger = 0;
                Despawn_Timer = 60 * IN_MILLISECONDS;
            }
           
            void Reset() override
            {
                FireBlast_Timer = 6 * IN_MILLISECONDS;
            }
            ...
            void SpellHit(Unit*, const SpellInfo* spell) override
            {
                ...
                {
                    DoCastSelf(SPELL_BLAZE_TRAP, true);
                    me->SetVisible(false);
                    Despawn_Timer = 130 * IN_MILLISECONDS;
                }
            }
            ...
            {
                trigger = _trigger;
                me->SetDisplayId(MODEL_ABYSSAL);
                if (trigger == 1) //debris
                {
                    me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                    DoCastSelf(SPELL_DEBRIS_VISUAL, true);
                    FireBlast_Timer = 5 * IN_MILLISECONDS;
                    Despawn_Timer = 10 * IN_MILLISECONDS;
                }
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                if (trigger)
                {
                    if (trigger == 1)
                    {
                        ...
                        {
                            DoCastSelf(SPELL_DEBRIS_DAMAGE, true);
                            trigger = 3;
                        }
                ...
                if (FireBlast_Timer <= diff)
                {
                    ...
                    FireBlast_Timer = urand(5, 15) * IN_MILLISECONDS;
                }
                else FireBlast_Timer -= diff;
...
{
    public: boss_magtheridon() : CreatureScript("boss_magtheridon") { }
        ...
        struct boss_magtheridonAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                Berserk_Timer   =          1320 * IN_MILLISECONDS;
                Quake_Timer     =            40 * IN_MILLISECONDS;
                Debris_Timer    =            10 * IN_MILLISECONDS;
                Blaze_Timer     = urand(10, 30) * IN_MILLISECONDS;
                BlastNova_Timer =            60 * IN_MILLISECONDS;
                Cleave_Timer    =            15 * IN_MILLISECONDS;
                RandChat_Timer  =            90 * IN_MILLISECONDS;
                ...
                DoCastSelf(SPELL_SHADOW_CAGE_C, true); 
                ...
                me->GetCreatureListWithEntryInGrid(HelperList, NPC_CHANNELLER, 1000.0f);
                if (!HelperList.empty())
                {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (Cleave_Timer <= diff)
                {
                    ...
                    Cleave_Timer = 10 * IN_MILLISECONDS;
                }
                else
                    Cleave_Timer -= diff;
                ...
                if (Quake_Timer <= diff)
                {
                    ...
                    {
                        DoCastSelf(SPELL_QUAKE_TRIGGER, true);
                        Quake_Timer = 50 * IN_MILLISECONDS;
                    }
                }
                else
                    Quake_Timer -= diff;
                ...
                if (Blaze_Timer <= diff)
                {
                    ...
                    Blaze_Timer = urand(20, 40) * IN_MILLISECONDS;
                }
                else
                    Blaze_Timer -= diff;
                    ...
                    && !me->HasUnitState(UNIT_STATE_STUNNED)) // shadow cage and earthquake
                {
                    ...
                    DoCastSelf(SPELL_DEBRIS_KNOCKDOWN, true);
...
class npc_hellfire_channeler : public CreatureScript
{
    public:
        ...
        struct npc_hellfire_channelerAI : public ScriptedAI
        {
            ...
            {
                _events.Reset();
                _events.ScheduleEvent(EVENT_SHADOWBOLT,   urand(8, 10) * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_DARK_MENDING,           10 * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_FEAR,        urand(15, 20) * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_INFERNAL,    urand(10, 50) * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_CHECK,                   5 * IN_MILLISECONDS);

                me->SetReactState(REACT_PASSIVE);
                DoCastSelf(SPELL_SHADOW_GRASP_C, false);
            }
            ...
            void DamageTaken(Unit*, uint32 &damage) override
            {
                if (damage >= me->GetHealth())
                    DoCastSelf(SPELL_SOUL_TRANSFER, true);
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SHADOWBOLT:
                            ...
                            _events.ScheduleEvent(EVENT_SHADOWBOLT, urand(10, 20) * IN_MILLISECONDS);
                            break;
                        case EVENT_DARK_MENDING:
                            if (HealthBelowPct(50))
                                DoCastSelf(SPELL_DARK_MENDING);
                            _events.ScheduleEvent(EVENT_DARK_MENDING, urand(10, 20) * IN_MILLISECONDS);
                            break;
                        case EVENT_FEAR:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1))
                                DoCast(target, SPELL_FEAR);
                            _events.ScheduleEvent(EVENT_FEAR, urand(25, 40) * IN_MILLISECONDS);
                            break;
                        case EVENT_INFERNAL:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                DoCast(target, SPELL_BURNING_ABYSSAL, true);
                            _events.ScheduleEvent(EVENT_INFERNAL, urand(30, 40) * IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        if (instance->GetData(DATA_MAGTHERIDON_EVENT) != IN_PROGRESS)
            return true;
        Creature* Magtheridon =ObjectAccessor::GetCreature(*go, ObjectGuid(instance->GetGuidData(DATA_MAGTHERIDON)));
