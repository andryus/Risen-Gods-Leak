...
class spell_twisted_reflectionAuraScript : public AuraScript
{
    PrepareAuraScript(spell_twisted_reflectionAuraScript);

    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& procInfo)
    {
        GetCaster()->CastSpell(procInfo.GetActor(), SPELL_TWISTED_REFLECTION_HEAL);
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_twisted_reflectionAuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
void AddSC_boss_doomlordkazzak()
{
    ...
    new SpellScriptLoaderEx<spell_twisted_reflectionAuraScript>("spell_twisted_reflection");
}
