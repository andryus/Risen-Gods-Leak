    ...
    struct npc_unkor_the_ruthlessAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (CanDoQuest)
            {
                ...
                {
                    //DoCastSelf(SPELL_QUID9889);        //not using spell for now
                    DoNice();
                }
                else
                {
        ...
        void DamageTaken(Unit* done_by, uint32 &damage) override
        {
            if (done_by && done_by->GetTypeId() == TYPEID_PLAYER)
                if (me->GetHealth() <= damage)
                    if (rand32()%100 < 75)
                        //Summon Wood Mites
                        DoCastSelf(SPELL_SUMMON_WOOD_MITES, true);
        }
    };
};
            ...
            if (done_by->GetTypeId() == TYPEID_PLAYER)
                if (me->GetHealth() <= damage)
                    if (rand32() % 100 < 75)
                        ...
                        DoCastSelf(SPELL_SUMMON_LOTS_OF_WOOD_MITES, true);
        }
    };
};
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            switch (waypointId)
            {
                ...
                case 30:
                    ...
                    break;
                case 31:
                    DoCastSelf(SPELL_TRAVEL_FORM_CAT);
                    me->SetWalk(false);
                    me->SetImmuneToNPC(true);
                    me->setFaction(FACTION_ORIGIN);
                    break;
            }
        }
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        if ((player->GetQuestStatus(ADVERSARIAL_BLOOD) == QUEST_STATUS_INCOMPLETE) || player->GetQuestRewardStatus(ADVERSARIAL_BLOOD))
        {
            ...
            player->SEND_GOSSIP_MENU(GOSSIP_MENU_TEXT_SKULL_PILE, go->GetGUID());
        }       
        player->SEND_GOSSIP_MENU(GOSSIP_MENU_TEXT_SKULL_PILE, go->GetGUID());
        return true;
    }
