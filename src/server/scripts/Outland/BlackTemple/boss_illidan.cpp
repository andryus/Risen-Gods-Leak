        ...
        void EnrageCheck()
        {
            if (Creature* glaive = ObjectAccessor::GetCreature(*me, GlaiveGUID))
            {
                if (!me->IsWithinDistInMap(glaive, FLAME_ENRAGE_DISTANCE))
                {
                    ...
                    DoCastSelf(SPELL_FLAME_ENRAGE, true);
                    ResetThreatList();
    ...
    struct boss_illidan_stormrageAI : public BossAI
    {
        ...
        void EnterEvadeMode(EvadeReason /*why*/) override
        {
            if (Creature* akama = ObjectAccessor::GetCreature(*me, AkamaGUID))
                akama->AI()->DoAction(ACTION_EVADE);
        ...
        void SummonMaiev()
        {
            ...
            DoCastSelf(SPELL_SUMMON_MAIEV, true);
        ...
        void HandleTransformSequence()
        {
            ...
            if (DemonTransformation[TransformCount].aura)
                DoCastSelf(DemonTransformation[TransformCount].aura, true);
...
public:
    ...
    struct npc_akama_illidanAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (!JustCreated) // open all doors, raid wiped
            {
                instance->HandleGameObject(instance->GetGuidData(DATA_GO_ILLIDAN_GATE), true);
                WalkCount = 1; // skip first wp

                instance->HandleGameObject(instance->GetGuidData(DATA_GO_ILLIDAN_DOOR_R), true);
                instance->HandleGameObject(instance->GetGuidData(DATA_GO_ILLIDAN_DOOR_L), true);
            }
        ...
        void DamageTaken(Unit* done_by, uint32 &damage) override
        {
            if (damage > me->GetHealth() || done_by->GetGUID() != instance->GetGuidData(DATA_ILLIDAN_STORMRAGE))
                damage = 0;
        }
        ...
        void BeginTalk()
        {
            ...
            instance->HandleGameObject(instance->GetGuidData(DATA_GO_ILLIDAN_DOOR_R), false);
            instance->HandleGameObject(instance->GetGuidData(DATA_GO_ILLIDAN_DOOR_L), false);
        ...
        void BeginChannel()
        {
            ...
            if (!JustCreated)
                return;
            float x, y, z;
            if (GameObject* gate = ObjectAccessor::GetGameObject(*me, instance->GetGuidData(DATA_GO_ILLIDAN_GATE)))
        ...
        void HandleWalkSequence()
        {
            ...
            {
            case 6:
                instance->GetGuidData(DATA_GO_ILLIDAN_DOOR_R);
                instance->GetGuidData(DATA_GO_ILLIDAN_DOOR_L);
                break;
            case 8:
                if (Phase == PHASE_WALK)
...
void boss_illidan_stormrage::boss_illidan_stormrageAI::HandleTalkSequence()
{
    ...
    case 14:
        ...
        break;
    case 15:
        DoCastSelf(SPELL_DEATH); // Animate his kneeling + stun him
        summons.DespawnAll();
        break;
    case 17:
        if (Creature* akama = ObjectAccessor::GetCreature(*me, AkamaGUID))
        {
            ...
            {
                akama->GetMotionMaster()->MoveChase(me); // Illidan must not die until Akama arrives.
            }
        }
        break;
            ...
            if (who && (who->GetTypeId() != TYPEID_PLAYER))
            {
                if (who->GetEntry() == ILLIDAN_STORMRAGE) // Check if who is Illidan
                {
                    if (!IllidanGUID && me->IsWithinDistInMap(who, 3) && (!who->HasAura(SPELL_CAGED)))
                    {
                        ...
                        // if (GameObject* CageTrap = instance->GetGameObject(instance->GetGuidData(CageTrapGUID)))
...
public:
    ...
    struct shadow_demonAI : public ScriptedAI
    {
        ...
        {
            TargetGUID.Clear();
            DoCastSelf(SPELL_SHADOW_DEMON_PASSIVE, true);
        }
...
public:
    ...
    struct npc_parasitic_shadowfiendAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            CheckTimer = 5000;
            DoCastSelf(SPELL_SHADOWFIEND_PASSIVE, true);
        }
