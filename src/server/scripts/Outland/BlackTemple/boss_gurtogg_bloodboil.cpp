        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (Phase1)
            {
                if (BewilderingStrikeTimer <= diff)
                {
                    ...
                    float mt_threat = GetThreat(me->GetVictim());
                ...
                if (EjectTimer <= diff)
                {
                    ...
                    ModifyThreatByPercent(me->GetVictim(), -40);
                    EjectTimer = 15000;
            ...
            if (PhaseChangeTimer <= diff)
            {
                if (Phase1)
                {
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
                    {
                        ...
                        DoCastSelf(SPELL_INSIGNIFIGANCE, true);
