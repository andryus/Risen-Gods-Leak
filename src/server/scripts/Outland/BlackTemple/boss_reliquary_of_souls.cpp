...
public:
    ...
    struct npc_enslaved_soulAI : public ScriptedAI
    {
        ...
        void Reset() override { ReliquaryGUID.Clear(); }

        void EnterCombat(Unit* /*who*/) override
        {
            DoCastSelf(ENSLAVED_SOUL_PASSIVE, true);
            DoZoneInCombat();
        }
...
public:
    ...
    struct boss_reliquary_of_soulsAI : public BossAI
    {
        ...
        // Override JustDied cause summons.DespawnAll() would despawn the last essence so no loot would be granted
        void JustDied(Unit* /*killer*/) override
        {
            events.Reset();
        ...
        void MergeThreatList(Creature* target)
        {
            ...
            for (ThreatContainer::StorageType::const_iterator itr = threatlist.begin(); itr != threatlist.end(); ++itr)
            {
                ...
                if (unit)
                {
                    ...
                    AddThreat(unit, threat);       // This makes it so that the unit has the same amount of threat in Reliquary's threatlist as in the target creature's (One of the Essences).
                }
            }
        }
            ...
            if (Timer <= diff)
            {
                ...
                case 2:
                    ...
                    if (Creature* Summon = DoSpawnCreature(23417+Phase, 0, 0, 0, 0, TEMPSUMMON_DEAD_DESPAWN, 0))
                    {
                        ...
                        Summon->AI()->AttackStart(SelectTarget(SELECT_TARGET_MAXTHREAT, 0));
    ...
    struct boss_essence_of_sufferingAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            if (!me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE))
            {
                Talk(SUFF_SAY_FREED);
                DoZoneInCombat();
                DoCastSelf(AURA_OF_SUFFERING, true); // linked aura need core support
                DoCastSelf(ESSENCE_OF_SUFFERING_PASSIVE, true);
                DoCastSelf(ESSENCE_OF_SUFFERING_PASSIVE2, true);
            }
        }
...
public:
    ...
    struct boss_essence_of_desireAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            Talk(DESI_SAY_FREED);
            DoZoneInCombat();
            DoCastSelf(AURA_OF_DESIRE, true);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (RuneShieldTimer <= diff)
            {
                ...
                DoCastSelf(SPELL_RUNE_SHIELD, true);
                SoulShockTimer += 2000;
                DeadenTimer += 2000;
                RuneShieldTimer = 60000;
...
public:
    ...
    struct boss_essence_of_angerAI : public ScriptedAI
    {
        ...
        std::list<ObjectGuid> SpiteTargetGUID;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (CheckTankTimer <= diff)
            {
                if (me->EnsureVictim()->GetGUID() != AggroTargetGUID)
                {
                    ...
                    DoCastSelf(SPELL_SELF_SEETHE, true);
                    AggroTargetGUID = me->EnsureVictim()->GetGUID();
                }
                CheckTankTimer = 2000;
