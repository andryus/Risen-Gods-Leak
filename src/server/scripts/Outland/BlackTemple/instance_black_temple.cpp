            ...
            void OnGameObjectCreate(GameObject* go) override
            {
                switch (go->GetEntry())
                {
                    ...
                    case GO_ILLIDAN_GATE:
                        ...
                        if (GetBossState(DATA_ILLIDAN_STORMRAGE) != DONE)
                            HandleGameObject(IllidanGateGUID, false);
                        break;
                    case GO_ILLIDAN_DOOR_R:
                        IllidanDoorGUIDs[0] = go->GetGUID();
                        if (GetBossState(DATA_ILLIDAN_STORMRAGE) != DONE)
                            HandleGameObject(IllidanDoorGUIDs[0], false);
                        break;
                    case GO_ILLIDAN_DOOR_L:
                        IllidanDoorGUIDs[1] = go->GetGUID();
                        if (GetBossState(DATA_ILLIDAN_STORMRAGE) != DONE)
                            HandleGameObject(IllidanDoorGUIDs[1], false);
                        break;
                    default:
                        break;
                }
