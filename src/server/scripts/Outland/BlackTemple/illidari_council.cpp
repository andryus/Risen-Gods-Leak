    ...
    struct boss_gathios_the_shattererAI : public boss_illidari_councilAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (SealTimer <= diff)
            {
                switch (urand(0, 1))
                {
                    case 0: DoCastSelf(SPELL_SEAL_OF_COMMAND);  break;
                    case 1: DoCastSelf(SPELL_SEAL_OF_BLOOD);    break;
                }
                SealTimer = 40000;
            } else SealTimer -= diff;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            else
            {
                if (VanishTimer <= diff)                          // Become attackable and poison current target
                {
                    ...
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                    ResetThreatList();
                    AddThreat(target, 3000.0f);      // Make Veras attack his target for a while, he will cast Envenom 4 seconds after.
