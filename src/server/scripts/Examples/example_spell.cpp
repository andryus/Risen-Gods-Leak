...
class spell_ex_proc : public SpellScriptLoader
{
    public:
        spell_ex_proc() : SpellScriptLoader("spell_ex_proc") { }

        class spell_ex_proc_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_ex_proc_AuraScript);
            ...
            {
                // check if the proc can be executed, return false if the aura should not proc
                return true;
            }

            void Prepare(ProcEventInfo& /*eventInfo*/)
            {
                // do stuff before proc
            ...
            void HandleEffectProc(AuraEffect const* /*aurEff*/, ProcEventInfo& /*eventInfo*/)
            {
                ...
                // do stuff on effect proc
            }

            void HandleAfterEffectProc(AuraEffect const* /*aurEff*/, ProcEventInfo& /*eventInfo*/)
            {
                // do stuff after effect proc
            }

            void Register()
            {
                DoCheckProc += AuraCheckProcFn(spell_ex_proc_AuraScript::Check);
                DoPrepareProc += AuraProcFn(spell_ex_proc_AuraScript::Prepare);
                OnProc += AuraProcFn(spell_ex_proc_AuraScript::HandleProc);
                OnEffectProc += AuraEffectProcFn(spell_ex_proc_AuraScript::HandleEffectProc, EFFECT_0, SPELL_AURA_DUMMY);
                AfterEffectProc += AuraEffectProcFn(spell_ex_proc_AuraScript::HandleAfterEffectProc, EFFECT_0, SPELL_AURA_DUMMY);
                AfterProc += AuraProcFn(spell_ex_proc_AuraScript::HandleAfterProc);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_ex_proc_AuraScript();
        }
};
