...
void OutdoorPvPNA::HandleKill(Player* killer, Unit* killed)
{
    ...
    {
        killer->KilledMonsterCredit(NA_CREDIT_MARKER, ObjectGuid::Empty); // 0 guid, btw it isn't even used in killedmonster function :S
...
uint32 OPvPCapturePointNA::GetAliveGuardsCount()
{
    ...
    for (std::map<uint32, ObjectGuid>::iterator itr = m_Creatures.begin(); itr != m_Creatures.end(); ++itr)
    {
        ...
        case NA_NPC_GUARD_15:
            if (Creature const* const cr = m_capturePoint->GetMap()->GetCreature(itr->second))
                if (cr->IsAlive())
                    ++cnt;
            break;
        default:
...
bool OPvPCapturePointNA::HandleCustomSpell(Player* player, uint32 spellId, GameObject* /*go*/)
{
    ...
    if (retval)
    {
        ...
        RG_LOG<ItemLogModule>(item, RG::Logging::ItemLogType::ADDED, count);
