...
public:
    ...
    struct npc_kiljaeden_controllerAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            summons.DespawnAll();
            me->SummonCreature(25588, 1684.7f,  614.42f, 28.058f,  0.698392f, TEMPSUMMON_MANUAL_DESPAWN);
            me->SummonCreature(25588, 1682.95f, 637.75f, 27.9231f, 5.71709f,  TEMPSUMMON_MANUAL_DESPAWN);
            me->SummonCreature(25588, 1707.61f, 612.15f, 27.7946f, 1.99037f,  TEMPSUMMON_MANUAL_DESPAWN);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!bSummonedDeceivers)
            {
                ...
                DoCastSelf(SPELL_ANVEENA_ENERGY_DRAIN);
...
public:
    ...
    struct boss_kiljaedenAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Creature* controler = me->FindNearestCreature(NPC_KILJAEDEN_CONTROLLER, 1000.0f))
    ...
    struct npc_hand_of_the_deceiverAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (HealthBelowPct(20) && !me->HasAura(SPELL_SHADOW_INFUSION, ObjectGuid::Empty))
                DoCastSelf(SPELL_SHADOW_INFUSION, true);
    ...
    struct npc_volatile_felfire_fiendAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* /*done_by*/, uint32 &damage) override
        {
            if (damage > me->GetHealth())
                DoCastSelf(SPELL_FELFIRE_FISSION, true);
        }
    ...
    struct npc_armageddonAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (uiTimer <= diff)
            {
                ...
                {
                    case 0:
                        DoCastSelf(SPELL_ARMAGEDDON_VISUAL, true);
                        ++spell;
                        break;
                    case 1:
                        DoCastSelf(SPELL_ARMAGEDDON_VISUAL2, true);
                        uiTimer = 9000;
                        ++spell;
                        break;
                    case 2:
                        DoCastSelf(SPELL_ARMAGEDDON_TRIGGER, true);
                        ++spell;
                        uiTimer = 5000;
                        break;
                    case 3:
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (victimClass)
            {
                ...
                case CLASS_PRIEST:
                    ...
                    if (uiTimer[2] <= diff)
                    {
                        DoCastSelf(SPELL_SR_RENEW, false);
