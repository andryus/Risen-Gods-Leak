...
public:
    ...
    struct boss_felmystAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            me->setActive(true);
            DoZoneInCombat();
            DoCastSelf(AURA_SUNWELL_RADIANCE, true);
            DoCastSelf(AURA_NOXIOUS_FUMES, true);
        ...
        void HandleFlightSequence()
        {
            switch (uiFlightCount)
            {
                ...
                {
                    DespawnSummons(NPC_VAPOR_TRAIL);
                    //DoCastSelf(SPELL_VAPOR_SELECT); need core support
                ...
                case 5:
                {
                    if ((uiBreathCount == 0) | (uiBreathCount == 2))
                        if (Unit* target = me->FindNearestCreature(NPC_TRIGGER_LEFT, 1000.0f, true))
                        {
                    ...
                    if ((uiBreathCount == 1) | (uiBreathCount == 3))
                        if (Unit* target = me->FindNearestCreature(NPC_TRIGGER_RIGHT, 1000.0f, true))
                        {
                            breathX = target->GetPositionX();
                            breathY = target->GetPositionY();
                            float x, y, z;
                         ...
                         //if (!target)
                         // target = ObjectAccessor::GetUnit(*me, instance ? instance->GetGuidData(DATA_PLAYER_GUID) : ObjectGuid::Empty);
                }
                case 6:
                ...
                case 7:
                {
                    DoCastSelf(SPELL_FOG_BREATH, true);
                    float x, y, z;
            ...
            if (phase == PHASE_GROUND)
            {
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_CORROSION:
                        ...
                        break;
                    case EVENT_GAS_NOVA:
                        DoCastSelf(SPELL_GAS_NOVA, false);
        ...
        void DespawnSummons(uint32 entry)
        {
            ...
            me->VisitAnyNearbyObject<Creature, Trinity::ContainerAction>(me->GetGridActivationRange(), templist, check);
...
public:
    ...
    struct npc_felmyst_vaporAI : public ScriptedAI
    {
        ...
        {
            DoZoneInCombat();
            //DoCastSelf(SPELL_VAPOR_FORCE, true); core bug
        }
...
public:
    ...
    struct npc_felmyst_trailAI : public ScriptedAI
    {
        ...
        {
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            DoCastSelf(SPELL_TRAIL_TRIGGER, true);
