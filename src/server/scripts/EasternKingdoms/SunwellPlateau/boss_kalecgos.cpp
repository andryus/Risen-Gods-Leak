...
#define MAX_PLAYERS_IN_SPECTRAL_REALM 13 //over this, teleport object won't work, 0 disables check
    ...
    struct boss_kalecgosAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            else
            {
                ...
                {
                   DoCast(SelectTarget(SELECT_TARGET_RANDOM, 1), SPELL_SPECTRAL_BLAST);
    ...
    struct boss_kalecAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (!me->HasAura(AURA_SPECTRAL_INVISIBILITY))
                DoCastSelf(AURA_SPECTRAL_INVISIBILITY, true);
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        {
            if (player.GetPositionZ() < DEMON_REALM_Z + 5)
                ++SpectralPlayers;
        }
...
public:
    ...
    struct boss_sathrovarrAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            DoCastSelf(AURA_DEMONIC_VISUAL, true);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (CheckTimer <= diff)
            {
                ...
                if (!isBanished && HealthBelowPct(1))
                {
                    if (Kalecgos)
                    {
                        ...
                        {
                            me->SetPosition(1704.138184f, 901.663269f, DRAGON_REALM_Z, 1.520955f);
                            TeleportAllPlayersBack();
            ...
            if (ResetThreat <= diff)
            {
               for (std::list<HostileReference*>::const_iterator itr = me->GetThreatManager().getThreatList().begin(); itr != 
                   me->GetThreatManager().getThreatList().end(); ++itr)
                {
