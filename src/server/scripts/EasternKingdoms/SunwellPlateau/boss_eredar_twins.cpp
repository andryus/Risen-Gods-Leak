...
public:
    ...
    struct boss_sacrolashAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (!me->IsInCombat())
            {
                ShadowbladesTimer    =  10*IN_MILLISECONDS;
                ShadownovaTimer      =  30*IN_MILLISECONDS;
                ConfoundingblowTimer =  25*IN_MILLISECONDS;
                ShadowimageTimer     =  20*IN_MILLISECONDS;
                ConflagrationTimer   =  30*IN_MILLISECONDS;
                EnrageTimer          = 360*IN_MILLISECONDS;
                FireblastTimer       =  26*IN_MILLISECONDS;
                SisterDeath = false;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (SisterDeath)
            {
                ...
                {
                    DoCastSelf(SPELL_FIREBLAST, true);
                    FireblastTimer = 26*IN_MILLISECONDS;                   
            ...
            if (ShadowimageTimer <= diff)
            {
                ...
                for (uint8 i = 0; i<3; ++i)
                {
                    ...
                    if (temp && target)
                    {
                        AddThreat(target, 1000000, temp); //don't change target(healers)
...
public:
    ...
    struct boss_alythessAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (!me->IsInCombat())
            {
                ConflagrationTimer =  45*IN_MILLISECONDS;
                BlazeTimer         = 0.1*IN_MILLISECONDS;
                PyrogenicsTimer    =  15*IN_MILLISECONDS;
                ShadownovaTimer    =  40*IN_MILLISECONDS;
                EnrageTimer        = 360*IN_MILLISECONDS;
                FlamesearTimer     =  15*IN_MILLISECONDS;
                IntroYellTimer     =  10*IN_MILLISECONDS;
                FireblastTimer     =  26*IN_MILLISECONDS;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (PyrogenicsTimer <= diff)
            {
                if (!me->IsNonMeleeSpellCast(false))
                {
                    DoCastSelf(SPELL_PYROGENICS, true);
