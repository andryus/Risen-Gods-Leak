...
public:
    ...
    struct npc_barnesAI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            {
                case 0:
                    DoCastSelf(SPELL_TUXEDO, false);
                    instance->DoUseDoorOrButton(ObjectGuid(instance->GetGuidData(DATA_GO_STAGEDOORLEFT)));
                    break;
                case 4:
                    TalkCount = 0;
                ...
                case 9:
                    PrepareEncounter();
                    instance->DoUseDoorOrButton(ObjectGuid(instance->GetGuidData(DATA_GO_CURTAINS)));
                    break;
            }
        }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            ...
            case GOSSIP_ACTION_INFO_DEF+3:
                ...
                TC_LOG_DEBUG("scripts", "player (GUID " UI64FMTD ") manually set Opera event to EVENT_OZ", player->GetGUID().GetRawValue());
                break;
            case GOSSIP_ACTION_INFO_DEF+4:
                player->CLOSE_GOSSIP_MENU();
                pBarnesAI->m_uiEventId = EVENT_HOOD;
                TC_LOG_DEBUG("scripts", "player (GUID " UI64FMTD ") manually set Opera event to EVENT_HOOD", player->GetGUID().GetRawValue());
                break;
            case GOSSIP_ACTION_INFO_DEF+5:
                player->CLOSE_GOSSIP_MENU();
                pBarnesAI->m_uiEventId = EVENT_RAJ;
                TC_LOG_DEBUG("scripts", "player (GUID " UI64FMTD ") manually set Opera event to EVENT_RAJ", player->GetGUID().GetRawValue());
                break;
        }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (InstanceScript* instance = creature->GetInstanceScript())
        {
            ...
            if (instance->GetBossState(DATA_ARAN) == DONE)
...
public:
    ...
    struct npc_image_of_medivhAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            me->GetCreatureListWithEntryInGrid(creaturelist, NPC_IMAGE_OF_MEDIVH, 50.0f);
