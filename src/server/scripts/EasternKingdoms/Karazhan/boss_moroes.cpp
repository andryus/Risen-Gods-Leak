        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!Enrage)
            {
                ...
                if (Blind_Timer <= diff)
                {
                    ...
                    SelectTargetList(targets, 5, SELECT_TARGET_RANDOM, 0, me->GetCombatReach()*5, true);
...
struct boss_moroes_guestAI : public ScriptedAI
{
    ...
    void AcquireGUID()
    {
        if (Creature* Moroes = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_MOROES))))
            for (uint8 i = 0; i < 4; ++i)
...
public:
    ...
    struct boss_baron_rafe_dreugerAI : public boss_moroes_guestAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            {
                DoCastSelf(SPELL_SEALOFCOMMAND);
