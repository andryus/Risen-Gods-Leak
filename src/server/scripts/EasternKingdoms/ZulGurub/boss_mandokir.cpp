    ...
    public:
        ...
        struct boss_mandokirAI : public BossAI
        {
            ...
            void Reset() override
            {
                BossAI::Reset();
                killCount = 0;
                events.ScheduleEvent(EVENT_CHECK_START, 1 * IN_MILLISECONDS);
                if (!me->FindNearestCreature(NPC_VILEBRANCH_SPEAKER, 2000.0f))
                    me->SetImmuneToAll(false);
                me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_PVP_ATTACKABLE | UNIT_FLAG_NOT_ATTACKABLE_1);
                if (Creature* speaker = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(NPC_VILEBRANCH_SPEAKER))))
                    if (!speaker->IsAlive())
                        speaker->Respawn(true); // Does not seem to work after reset
                summons.DespawnAll();
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_OVERPOWER, urand(7, 9)*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_MORTAL_STRIKE, urand(12, 18)*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_WHIRLWIND, urand(24, 30)*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CHECK_OHGAN, 1*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_WATCH_PLAYER, urand(13, 15)*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CHARGE_PLAYER, urand(33, 38)*IN_MILLISECONDS);
            ...
            void KilledUnit(Unit* victim) override
            {
                ...
                {
                    Talk(SAY_DING_KILL);
                    if (Creature* jindo = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_JINDO))))
                        if (jindo->IsAlive())
                ...
                if (me->IsInCombat())
                    if (Creature* spirit = me->FindNearestCreature(NPC_CHAINED_SPIRT, 50.0f))
                    {
                        victim->ToPlayer()->ResurrectPlayer(100.0f);
                        spirit->AI()->Talk(SAY_SPIRIT_RELEASE, victim);
                        spirit->DespawnOrUnsummon(2 * IN_MILLISECONDS);
                    }
            }
                            ...
                            switch (eventId)
                            {
                                case EVENT_CHECK_START:
                                    if (instance->GetBossState(DATA_MANDOKIR) == SPECIAL)
                                    {
                                        ...
                                        events.ScheduleEvent(EVENT_STARTED, 6*IN_MILLISECONDS);
                                    }
                                    else
                                        events.ScheduleEvent(EVENT_CHECK_START, 1*IN_MILLISECONDS);
                                    break;
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_OVERPOWER:
                            ...
                            events.ScheduleEvent(EVENT_OVERPOWER, urand(6, 12)*IN_MILLISECONDS);
                            break;
                        ...
                        case EVENT_WHIRLWIND:
                            ...
                            events.ScheduleEvent(EVENT_WHIRLWIND, urand(22, 26)*IN_MILLISECONDS);
                            break;
                        case EVENT_CHECK_OHGAN:
                            if (!me->FindNearestCreature(NPC_OHGAN, 2000.0f))
                            {
                        ...
                        case EVENT_WATCH_PLAYER:
                            ...
                            events.ScheduleEvent(EVENT_WATCH_PLAYER, urand(12, 15)*IN_MILLISECONDS);
                            break;
                        case EVENT_CHARGE_PLAYER:
                            DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 40, true), SPELL_CHARGE);
                            events.ScheduleEvent(EVENT_CHARGE_PLAYER, urand(22, 30)*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
...
class npc_ohgan : public CreatureScript
{
    public:
        ...
        struct npc_ohganAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                _events.ScheduleEvent(EVENT_SUNDERARMOR, 5*IN_MILLISECONDS);
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SUNDERARMOR:
                            DoCastVictim(SPELL_SUNDERARMOR, true);
                            _events.ScheduleEvent(EVENT_SUNDERARMOR, urand(10, 15)*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
...
class npc_vilebranch_speaker : public CreatureScript
{
    public:
        ...
        struct npc_vilebranch_speakerAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                _events.ScheduleEvent(EVENT_DEMORALIZING_SHOUT, urand(2, 4)*IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_CLEAVE, urand(5, 8)*IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_DEMORALIZING_SHOUT:
                            DoCastSelf(SPELL_DEMORALIZING_SHOUT, true);
                            _events.ScheduleEvent(EVENT_SUNDERARMOR, urand(22, 30)*IN_MILLISECONDS);
                            break;
                        case EVENT_CLEAVE:
                            DoCastVictim(SPELL_CLEAVE, true);
                            _events.ScheduleEvent(EVENT_SUNDERARMOR, urand(6, 9)*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
