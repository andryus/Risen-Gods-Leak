    ...
    public:
        ...
        struct boss_hazzarahAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_MANABURN,   urand(4, 10)*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SLEEP,     urand(10, 18)*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_ILLUSIONS, urand(10, 18)*IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MANABURN:
                            ...
                            events.ScheduleEvent(EVENT_MANABURN, urand(8, 16)*IN_MILLISECONDS);
                            break;
                        case EVENT_SLEEP:
                            DoCastVictim(SPELL_SLEEP, true);
                            events.ScheduleEvent(EVENT_SLEEP, urand(12, 20)*IN_MILLISECONDS);
                            break;
                        case EVENT_ILLUSIONS:
                            ...
                            events.ScheduleEvent(EVENT_ILLUSIONS, urand(15, 25)*IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
