                    ...
                    switch (eventId)
                    {
                        case EVENT_SPAWN_START_SPIDERS:
                            ...
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                            {
                                ...
                                Spider = me->SummonCreature(15041, target->GetPositionX()+10, target->GetPositionY(), target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
                                if (Spider)
                                ...
                                if (Spider)
                                    Spider->AI()->AttackStart(target);
                                Spider = me->SummonCreature(15041, target->GetPositionX()-10, target->GetPositionY()-10, target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
                                if (Spider)
                                    Spider->AI()->AttackStart(target);
                            }
                        ...
                        {
                            Talk(SAY_TRANSFORM);
                            DoCastSelf(SPELL_SPIDER_FORM); // SPELL_AURA_TRANSFORM
                            /*
