    ...
    struct npc_kalecgosAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 type, uint32 pointId) override
        {
            ...
            if (pointId == POINT_ID_PREPARE_LANDING)
            {
                ...
                me->SetHover(false);
                events.ScheduleEvent(EVENT_KALECGOS_LANDING, 2 * IN_MILLISECONDS);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_KALECGOS_LANDING:
                    ...
                    events.ScheduleEvent(EVENT_KALECGOS_TRANSFORM, 1 * IN_MILLISECONDS);
                    break;
                case EVENT_KALECGOS_TRANSFORM:
                    DoCastSelf(SPELL_ORB_KILL_CREDIT, true);
                    DoCastSelf(SPELL_TRANSFORM_VISUAL, false);
                    DoCastSelf(SPELL_KALECGOS_TRANSFORM, false);
