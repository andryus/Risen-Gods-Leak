...
public:
    ...
    struct npc_grimstoneAI : public npc_escortAI
    {
        ...
        void SetArenaSpoilsPhase(uint32 phase)
        {
            if(instance)
            {
                if(uint64 chestGUID = instance->GetData64(DATA_GO_ARENA_SPOILS))
                                ...
                                if (!players.isEmpty())
                                {
                                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                    {
                                        if (Player* player = itr->GetSource())
                                        {
                                            ...
                                            {
                                                player->KilledMonsterCredit(16166, 0);
                                            }
                                        }
                                    }
                                }
    ...
    public:
        ...
        struct npc_hurley_blackbreathAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                {
                    DoCast(me, SPELL_DRUNKEN_RAGE); //is defined in script of Racknot
                    _enraged = true;
                }
    ...
    public:
        ...
        struct npc_heart_of_the_mountain_triggerAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                {
                    (*itr)->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE|UNIT_FLAG_IMMUNE_TO_PC|UNIT_FLAG_NON_ATTACKABLE);
                }
