...
public:
    ...
    struct boss_chromaggusAI : public BossAI
    {
        ...
        {
            _JustDied();
            if (GameObject* exitdoor = me->FindNearestGameObject(GO_CHROMAGGUS_EXIT, 100.0f))
            {
                exitdoor->UseDoorOrButton();
            }
        }
...
public:
    ...
    void OnGameObjectStateChanged(GameObject* go, uint32 state)
    {
        ...
        if (GameObject* chromaggusdoor = go->FindNearestGameObject(GO_CHROMAGGUS_DOOR, 100.0f))
        {
