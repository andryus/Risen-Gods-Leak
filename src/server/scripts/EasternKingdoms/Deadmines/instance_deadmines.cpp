...
Position const YellTriggerSummonPos =
{
    -99.96f, -670.45f, 7.43f, 0.0f
};
...
class instance_deadmines : public InstanceMapScript
{
    public:
        ...
        struct instance_deadmines_InstanceMapScript : public InstanceScript
        {
            ...
            virtual void Update(uint32 diff) override
            {
                ...
                switch (State)
                {
                    case CANNON_GUNPOWDER_USED:
                        ...
                        if (Unit* proxy = instance->SummonCreature(NPC_MR_SMITE_PROXY, YellTriggerSummonPos))
                        {
                            proxy->ToCreature()->AI()->Talk(SAY_MR_SMITE_ALARM1);
                        }
                    ...
                    case CANNON_BLAST_INITIATED:
                        ...
                        if (CannonBlast_Timer <= diff)
                        {
                            ...
                            if (Unit* proxy = instance->SummonCreature(NPC_MR_SMITE_PROXY, YellTriggerSummonPos))
                            {
                                proxy->ToCreature()->AI()->Talk(SAY_MR_SMITE_ALARM2);
                            }
            ...
            void RemoveDoor()
            {
                if (GameObject* DefiasDoor = instance->GetGameObject(IronCladDoorGUID))
                    DefiasDoor->RemoveFromWorld(); // never respawn
            }
