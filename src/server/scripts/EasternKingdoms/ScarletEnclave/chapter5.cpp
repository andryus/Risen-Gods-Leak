...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF+1)
        {
            ...
            if (npc_highlord_darion_mograine::npc_highlord_darion_mograineAI* _ai = CAST_AI(npc_highlord_darion_mograine::npc_highlord_darion_mograineAI, creature->AI()))
            {
                _ai->uiStep = 1;
                _ai->Start();
                _ai->SetDespawnAtFar(false);
            }
        }
        return true;
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_THE_LIGHT_OF_DAWN) == QUEST_STATUS_INCOMPLETE
            || player->GetQuestStatus(QUEST_THE_LIGHT_OF_DAWN) == QUEST_STATUS_COMPLETE)
            player->AddGossipItem(GOSSIP_OPTION_MENU_ID);
    ...
    struct npc_highlord_darion_mograineAI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            switch (waypointId)
            {
                ...
                case 1:
                    ...
                    me->Dismount();
                    DoCastSelf(SPELL_THE_MIGHT_OF_MOGRAINE, true); // need to fix, on player only
                ...
                case 2:
                    me->SetWalk(true);
                    DoCastSelf(SPELL_THE_LIGHT_OF_DAWN);
                    break;
                case 3:
                {
        ...
        void UpdateAI(uint32 diff) override
        {
                ...
                if (uiPhase_timer <= diff)
                {
                    ...
                    switch (uiStep)
                    {
                        ...
                        case 2:
                            ...
                            if (Creature* pKoltira = me->FindNearestCreature(NPC_KOLTIRA_DEATHWEAVER, 50.0f))
                                uiKoltiraGUID = pKoltira->GetGUID();
                            if (Creature* pOrbaz = me->FindNearestCreature(NPC_ORBAZ_BLOODBANE, 50.0f))
                                uiOrbazGUID = pOrbaz->GetGUID();
                            if (Creature* pThassarian = me->FindNearestCreature(NPC_THASSARIAN, 50.0f))
                        ...
                        case 33: // Darion supports to jump to lich king here
                            if (ObjectAccessor::GetCreature(*me, uiLichKingGUID))
                                DoCastSelf(SPELL_MOGRAINE_CHARGE); // jumping charge
                            ...
                            //    go->SetPhaseMask(0, true);
                            {
                                ...
                                for (std::list<Player*>::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                    if (Player* player = *itr)
                                        if (player->IsAlive() && me->IsWithinDistInMap(player, 50))
                                            player->CastSpell(player, SPELL_THE_LIGHT_OF_DAWN_Q, false);
                            }
                            me->SetVisible(false); // respawns another Darion for quest turn in
                            me->SummonCreature(NPC_HIGHLORD_DARION_MOGRAINE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 600000);
                            JumpToNextStep(1000);
                            break;
            ...
            else
            {
                ...
                {
                    DoCastSelf(SPELL_ANTI_MAGIC_ZONE1);
