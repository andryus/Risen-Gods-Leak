    ...
    struct npc_unworthy_initiateAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id == 1)
            {
                wait_timer = 5000;
                me->LoadEquipment(1);
                DoCastSelf(SPELL_DK_INITIATE_VISUAL, true);
            ...
            case PHASE_CHAINED:
                if (!anchorGUID)
                {
                    if (checkTimer <= diff)
                    {
                        ...
                        {
                            anchor->AI()->SetGuidData(me->GetGUID());
...
public:
    ...
    struct npc_unworthy_initiate_anchorAI : public PassiveAI
    {
        ...
        void SetGuidData(ObjectGuid guid, uint32 /*id*/)
        {
            if (!prisonerGUID)
                prisonerGUID = guid;
        }
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        if (Creature* anchor = go->FindNearestCreature(29521, 15))
            if (ObjectGuid prisonerGUID = anchor->AI()->GetGuidData())
...
public:
    ...
    {
        player->PlayerTalkClass->ClearMenus();
        player->AddAura(SPELL_REALM_OF_SHADOWS, player);
        return true;
    }
        ...
        struct npc_scarlet_miner_cartAI : public PassiveAI
        {
            ...
            void JustSummoned(Creature* summon) override
            {
                if (summon->GetEntry() == NPC_MINER)
                {
                    ...
                    summon->AI()->SetGuidData(_playerGUID);
                }
            }
...
class npc_scarlet_miner : public CreatureScript
{
    public:
        ...
        struct npc_scarlet_minerAI : public npc_escortAI
        {
            ...
            void SetGuidData(ObjectGuid guid, uint32 /*id = 0*/)
            {
                InitWaypoint();
                ...
                switch (waypointId)
                {
                    case 1:
                        ...
                        if (Unit* car = ObjectAccessor::GetCreature(*me, carGUID))
                        {
                            ...
                            car->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        }
                    ...
                    case 17:
                        if (Unit* car = ObjectAccessor::GetCreature(*me, carGUID))
                        {
                            ...
                            car->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        if (Player* player = GetPlayerForEscort())
        ...
        class spell_recall_eye_of_acherus_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* owner = caster->GetOwner())
                    owner->RemoveAura(SPELL_THE_EYE_OF_ACHERUS);
            }

            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_recall_eye_of_acherus_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };
...
class spell_deliver_stolen_horse : public SpellScriptLoader
{
    public:
        ...
        class spell_deliver_stolen_horse_SpellScript : public SpellScript
        {
            ...
            {
                Unit* caster = GetCaster();
                if (caster->FindNearestCreature(NPC_SALANAR_THE_HORSEMAN, 5.0f))
                {
            ...
            void Register()
            {
                OnCheckCast += SpellCheckCastFn(spell_deliver_stolen_horse_SpellScript::CheckCast);
                OnEffectHitTarget += SpellEffectFn(spell_deliver_stolen_horse_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };
