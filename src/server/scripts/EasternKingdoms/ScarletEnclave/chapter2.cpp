...
class spell_chapter2_penance : public SpellScriptLoader
{
    public:
        spell_chapter2_penance() : SpellScriptLoader("spell_chapter2_penance") { }

        class spell_chapter2_penance_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_chapter2_penance_SpellScript);

            void FilterTargets(std::list<WorldObject*>& targetList)
            {
                targetList.remove_if(NpcCheckPredicate());
            }

            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_chapter2_penance_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_chapter2_penance_SpellScript();
        }
};
                    ...
                    switch (player->getRace())
                    {
                        case RACE_HUMAN:
                            switch (ExecuteSpeech_Counter)
                            {
                                ...
                                case 8:
                                    if (Creature* Plaguefist = me->FindNearestCreature(NPC_PLAGUEFIST, 85.0f))
