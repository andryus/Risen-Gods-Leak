...
enum Waypoints
{
    WP_SCARLET_TRAINEE_ONE   = 657500,
    WP_SCARLET_TRAINEE_TWO   = 657501
};
...
class boss_herod : public CreatureScript
{
    public:
        ...
        struct boss_herodAI : public BossAI
        {
            ...
            {
                Talk(SAY_AGGRO);
                DoCastSelf(SPELL_RUSHINGCHARGE);
            ...
            void JustDied(Unit* killer) override
            {
                ...
                {
                    if (Creature* guard = me->SummonCreature(NPC_SCARLET_TRAINEE, 1939.18f, -431.58f, 17.09f, 6.22f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 600000))
                    {
                        guard->GetMotionMaster()->MovePath(WP_SCARLET_TRAINEE_ONE, false);

                        // Only the first mob needs to yell
                        if (!i)
                            guard->AI()->Talk(SAY_DEFEAT);
                    }
                }
                ...
                {
                    if (Creature* guard = me->SummonCreature(NPC_SCARLET_TRAINEE, 1939.18f, -431.58f, 17.09f, 6.22f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 600000))
                        guard->GetMotionMaster()->MovePath(WP_SCARLET_TRAINEE_TWO, false);
                }
            }
