                    ...
                    switch (urand(0, 5))
                    {
                        case ROOM_HALL_OF_SECRETS:
                            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                                if (GameObject::GetGameObject(*caster, instance->GetGuidData(GO_GATE_RAVENIAN))->GetGoState() == GO_STATE_ACTIVE)
                                    spell_to_cast = SPELL_SHADOW_PORTAL_HALLOFSECRETS;
                            break;
                        case ROOM_HALL_OF_THE_DAMNED:
                            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                                if (GameObject::GetGameObject(*caster, instance->GetGuidData(GO_GATE_THEOLEN))->GetGoState() == GO_STATE_ACTIVE)
                                    spell_to_cast = SPELL_SHADOW_PORTAL_HALLOFTHEDAMNED;
                            break;
                        case ROOM_THE_COVEN:
                            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                                if (GameObject::GetGameObject(*caster, instance->GetGuidData(GO_GATE_MALICIA))->GetGoState() == GO_STATE_ACTIVE)
                                    spell_to_cast = SPELL_SHADOW_PORTAL_THECOVEN;
                            break;
                        case ROOM_THE_SHADOW_VAULT:
                            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                                if (GameObject::GetGameObject(*caster, instance->GetGuidData(GO_GATE_ILLUCIA))->GetGoState() == GO_STATE_ACTIVE)
                                    spell_to_cast = SPELL_SHADOW_PORTAL_THESHADOWVAULT;
                            break;
                        case ROOM_BAROV_FAMILY_VAULT:
                            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                                if (GameObject::GetGameObject(*caster, instance->GetGuidData(GO_GATE_BAROV))->GetGoState() == GO_STATE_ACTIVE)
                                    spell_to_cast = SPELL_SHADOW_PORTAL_BAROVFAMILYVAULT;
                            break;
                        case ROOM_VAULT_OF_THE_RAVENIAN:
                            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                                if (GameObject::GetGameObject(*caster, instance->GetGuidData(GO_GATE_POLKELT))->GetGoState() == GO_STATE_ACTIVE)
        ...
        class spell_shadow_portal_rooms_SpellScript : public SpellScript
        {
            ...
            void HandleSendEvent(SpellEffIndex effIndex)
            {
                ...
                if (gate_to_close && (GetCaster()->GetMap()->GetId() == 289))
                {
                    ...
                    if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                        if (GameObject* gate = GameObject::GetGameObject(*caster, instance->GetGuidData(gate_to_close)))
                            gate->SetGoState(GO_STATE_READY);
                }
            }
