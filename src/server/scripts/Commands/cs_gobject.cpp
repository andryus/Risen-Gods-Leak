...
namespace chat { namespace command { namespace handler {
    ...
    bool HandleGameObjectActivateCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    //spawn go
    bool HandleGameObjectAddCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        GameObject* object = GameObject::Create(objectInfo->entry, *map, player->GetPhaseMaskForSpawn(), Position(x, y, z, o), G3D::Quat(0, 0, rot2, rot3), 0);
        if (!object)
            return false;
        ...
        uint32 guidLow = object->GetSpawnId();

        delete object;

        // this will generate a new guid if the object is in an instance
        object = GameObject::LoadFromDB(guidLow, *map);
        if (!object)
            return false;
    ...
    // add go, temp only
    bool HandleGameObjectAddTempCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleGameObjectTargetCommand(ChatHandler* handler, char const* args)
    {
        Player* player = handler->GetSession()->GetPlayer();
        QueryResult result;
    ...
    bool HandleGameObjectDeleteCommand(ChatHandler* handler, char const* args)
    {
    ...
    bool HandleGameObjectTurnCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_COMMAND_TURNOBJMESSAGE, object->GetGUID().GetCounter(), object->GetGOInfo()->name.c_str(), object->GetGUID().GetCounter());

        return true;
    }

    //move selected object
    bool HandleGameObjectMoveCommand(ChatHandler* handler, char const* args)
    {
        // number or [name] Shift-click form |color|Hgameobject:go_guid|h[name]|h|r
        char* id = handler->extractKeyFromLink((char*)args, "Hgameobject");
        if (!id)
    ...
    bool HandleGameObjectSetPhaseCommand(ChatHandler* handler, char const* args)
    {
        // number or [name] Shift-click form |color|Hgameobject:go_id|h[name]|h|r
        char* id = handler->extractKeyFromLink((char*)args, "Hgameobject");
        if (!id)
    ...
    bool HandleGameObjectNearCommand(ChatHandler* handler, char const* args)
    {
    ...
    bool HandleGameObjectInfoCommand(ChatHandler* handler, char const* args)
    {
        uint32 entry = 0;
        uint32 type = 0;
        uint32 displayId = 0;
        ...
        GameObject* go = handler->getSelectedObject()->ToGameObject();

        if (!go)
            return false;

        int64 curRespawnDelay = 0;
        curRespawnDelay = go->GetRespawnTimeEx() - time(NULL);
        if (curRespawnDelay < 0)
            curRespawnDelay = 0;
        std::string curRespawnDelayStr = secsToTimeString(uint64(curRespawnDelay), true);
        std::string defRespawnDelayStr = secsToTimeString(go->GetRespawnDelay(), true);
        ...
        GameObjectModel* model = go->m_model;
        if (model)
        {
            Player* player = handler->GetSession()->GetPlayer();
            float scale = go->GetObjectScale();
            G3D::Vector3 iPos(go->GetPositionX(), go->GetPositionY(), go->GetPositionZ());
            G3D::Matrix3 rotation = go->GetGOData() != nullptr ? G3D::Matrix3(go->GetGOData()->rotation) : G3D::Matrix3::fromEulerAnglesZYX(go->GetOrientation(), 0, 0);
            ModelList modelList = GetGameObjectModelList();
            ...
            if (it != modelList.end())
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    G3D::Vector3 pos((rotation * box.corner(i) * scale) + iPos);
                    player->SummonCreature(VISUAL_WAYPOINT, pos.x - 0.4f, pos.y, pos.z - 1.5f, 0, TEMPSUMMON_TIMED_DESPAWN, 9000);
                }
            }
        }
    ...
    bool HandleGameObjectSetStateCommand(ChatHandler* handler, char const* args)
    {
        // number or [name] Shift-click form |color|Hgameobject:go_id|h[name]|h|r
        char* id = handler->extractKeyFromLink((char*)args, "Hgameobject");
        if (!id)
...
void AddSC_gobject_commandscript()
{
    ...
    new LegacyCommandScript("cmd_gobject_activate",     HandleGameObjectActivateCommand,    false);
    new LegacyCommandScript("cmd_gobject_delete",       HandleGameObjectDeleteCommand,      false);
    new LegacyCommandScript("cmd_gobject_info",         HandleGameObjectInfoCommand,        false);
    new LegacyCommandScript("cmd_gobject_move",         HandleGameObjectMoveCommand,        false);
    new LegacyCommandScript("cmd_gobject_near",         HandleGameObjectNearCommand,        false);
    new LegacyCommandScript("cmd_gobject_target",       HandleGameObjectTargetCommand,      false);
    new LegacyCommandScript("cmd_gobject_turn",         HandleGameObjectTurnCommand,        false);

    new LegacyCommandScript("cmd_gobject_add",          HandleGameObjectAddCommand,         false);
    new LegacyCommandScript("cmd_gobject_add_temp",     HandleGameObjectAddTempCommand,     false);

    new LegacyCommandScript("cmd_gobject_set_phase",    HandleGameObjectSetPhaseCommand,    false);
    new LegacyCommandScript("cmd_gobject_set_state",    HandleGameObjectSetStateCommand,    false);
}
