...
namespace chat { namespace command { namespace handler {
    ...
    bool HandleLearnCommand(ChatHandler* handler, char const* args)
    {
    ...
    bool HandleLearnAllGMCommand(ChatHandler* handler, char const* /*args*/)
    {
    ...
    bool HandleLearnAllSupportCommand(ChatHandler* handler, char const* /*args*/)
    {
        Player* targetPlayer = handler->getSelectedPlayerOrSelf();

        if (!targetPlayer || targetPlayer->GetSession()->GetSecurity() < GMLEVEL_RG_TEAM)
        {
            handler->SendSysMessage(LANG_PLAYER_NOT_FOUND);
            handler->SetSentErrorMessage(true);
            return false;
        ...
        constexpr uint32 supportSpells[] =
        {
            27254, // Damage 8
            27255, // Damage 9
            27258, // Damage 10
            ...
            78862, // Twilight Pulse
            48788, // Lay on Hands (Rank 5)
            70859, // Upper Spire Teleport
        };

        for (uint32 spellId : supportSpells)
        {
    ...
    bool HandleLearnAllMySpellsCommand(ChatHandler* handler, char const* /*args*/)
    {
    ...
    bool HandleLearnAllMyTalentsCommand(ChatHandler* handler, char const* /*args*/)
    {
    ...
    bool HandleLearnAllMyClassCommand(ChatHandler* handler, char const* /*args*/)
    {
    ...
    bool HandleLearnAllMyPetTalentsCommand(ChatHandler* handler, char const* /*args*/)
    {
        Player* player = handler->GetSession()->GetPlayer();
    ...
    bool HandleLearnAllLangCommand(ChatHandler* handler, char const* /*args*/)
    {
        // skipping UNIVERSAL language (0)
        for (uint8 i = 1; i < LANGUAGES_COUNT; ++i)
            handler->GetSession()->GetPlayer()->LearnSpell(GetLanguageDescByIndex(i).spell_id, false);

        handler->SendSysMessage(LANG_COMMAND_LEARN_ALL_LANG);
        return true;
    }

    bool HandleLearnAllDefaultCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
    ...
    bool HandleLearnAllCraftsCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
        if (!handler->extractPlayerTarget((char*)args, &target))
            return false;
    ...
    bool HandleLearnAllRecipesCommand(ChatHandler* handler, char const* args)
    {
    ...
    bool HandleUnLearnCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (target->HasSpell(spellId))
        {
            ...
            // if the spell is removed, send new talents info to the target client
...
void AddSC_learn_commandscript()
{
    ...
    new LegacyCommandScript("cmd_learn",                    HandleLearnCommand,                 false);
    new LegacyCommandScript("cmd_learn_all_my_class",       HandleLearnAllMyClassCommand,       false);
    new LegacyCommandScript("cmd_learn_all_my_pettalents",  HandleLearnAllMyPetTalentsCommand,  false);
    new LegacyCommandScript("cmd_learn_all_my_spells",      HandleLearnAllMySpellsCommand,      false);
    new LegacyCommandScript("cmd_learn_all_my_talents",     HandleLearnAllMyTalentsCommand,     false);

    new LegacyCommandScript("cmd_learn_all_gm",             HandleLearnAllGMCommand,            false);
    new LegacyCommandScript("cmd_learn_all_support",        HandleLearnAllSupportCommand,       false);
    new LegacyCommandScript("cmd_learn_all_crafts",         HandleLearnAllCraftsCommand,        false);
    new LegacyCommandScript("cmd_learn_all_default",        HandleLearnAllDefaultCommand,       false);
    new LegacyCommandScript("cmd_learn_all_lang",           HandleLearnAllLangCommand,          false);
    new LegacyCommandScript("cmd_learn_all_recipes",        HandleLearnAllRecipesCommand,       false);

    new LegacyCommandScript("cmd_unlearn",                  HandleUnLearnCommand,               false);
}
