...
namespace chat { namespace command { namespace handler {
    ...
    bool HandleTeleAddCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleTeleDeleteCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;

        // id, or string, or [name] Shift-click form |color|Htele:id|h[name]|h|r
        GameTele const* tele = handler->extractGameTeleFromLink((char*)args, true);
        if (!tele)
        {
    ...
    bool HandleTeleNameCommand(ChatHandler* handler, const char* args)
    {
        char* nameStr;
        char* teleStr;
        ...
        if (strcmp(teleStr, "$home") == 0)    // References target's homebind
        {
            ...
            else
            {
                ...
                if (resultDB)
                {
                    ...
                    Player::SavePositionInDB(trans, mapId, posX, posY, posZ, 0, zoneId, target_guid);
        ...
        else
        {
            ...
            auto trans = CharacterDatabase.BeginTransaction();
            Player::SavePositionInDB(trans, tele->mapId, tele->position_x, tele->position_y, tele->position_z, tele->orientation,
    ...
    bool HandleTeleGroupCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
...
void AddSC_tele_commandscript()
{
    ...
    new LegacyCommandScript("cmd_teleport",         HandleTeleCommand,          false);
    new LegacyCommandScript("cmd_teleport_add",     HandleTeleAddCommand,       false);
    new LegacyCommandScript("cmd_teleport_delete",  HandleTeleDeleteCommand,    true);
    new LegacyCommandScript("cmd_teleport_name",    HandleTeleNameCommand,      true);
    new LegacyCommandScript("cmd_teleport_group",   HandleTeleGroupCommand,     false);
}
