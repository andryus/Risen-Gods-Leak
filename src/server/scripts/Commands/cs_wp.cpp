...
namespace chat { namespace command { namespace handler {
    ...
    */
    bool HandleWpAddCommand(ChatHandler* handler, const char* args)
    {
        // optional
    ...
    bool HandleWpLoadCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleWpReloadCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleWpUnLoadCommand(ChatHandler* handler, const char* /*args*/)
    {
    ...
    bool HandleWpEventCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleWpModifyCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        if (show == "move")
        {
            ...
            Map* map = chr->GetMap();
            {
                ...
                if (wpGuid != 0)
                {
                    ...
                    // re-create
                    Creature* wpCreature2 = Creature::Create(VISUAL_WAYPOINT, map, chr->GetPosition(), chr->GetPhaseMaskForSpawn());
                    if (!wpCreature2)
                    {
    ...
    bool HandleWpShowCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        if (show == "on")
        {
            ...
            do
            {
                ...
                Creature* wpCreature = Creature::Create(id, map, Position(x, y, z, o), chr->GetPhaseMaskForSpawn());
                if (!wpCreature)
                {
...
void AddSC_wp_commandscript()
{
    ...
    new LegacyCommandScript("cmd_wp_add",       HandleWpAddCommand,     false);
    new LegacyCommandScript("cmd_wp_event",     HandleWpEventCommand,   false);
    new LegacyCommandScript("cmd_wp_load",      HandleWpLoadCommand,    false);
    new LegacyCommandScript("cmd_wp_modify",    HandleWpModifyCommand,  false);
    new LegacyCommandScript("cmd_wp_unload",    HandleWpUnLoadCommand,  false);
    new LegacyCommandScript("cmd_wp_reload",    HandleWpReloadCommand,  false);
    new LegacyCommandScript("cmd_wp_show",      HandleWpShowCommand,    false);
}
