...
namespace chat { namespace command { namespace handler {
    ...
    //Edit Player HP
    bool HandleModifyHPCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Mana
    bool HandleModifyManaCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Energy
    bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Rage
    bool HandleModifyRageCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyRunicPowerCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Faction
    bool HandleModifyFactionCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Spell
    bool HandleModifySpellCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player TP
    bool HandleModifyTalentCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Aspeed
    bool HandleModifyASpeedCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Speed
    bool HandleModifySpeedCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifySwimCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyBWalkCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player Fly
    bool HandleModifyFlyCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyScaleCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Enable Player mount
    bool HandleModifyMountCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //Edit Player money
    bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        std::string name;
        Player* target = NULL;
        // check if charactername is given
        handler->extractOptFirstArg((char*)args, &nameStr, &moneyStr);
        // if name is given, then use player by name
        if (nameStr)
        {
            name = std::string(nameStr);
            normalizePlayerName(name);
            target = ObjectAccessor::FindConnectedPlayerByName(name);
        }
        // if name isn't given use the target
        ...
        int32 moneyToAdd = 0;
        // make G, S, C to g, s, c
        for (std::size_t i = 0; i < strlen(moneyStr); i++)
        {
            moneyStr[i] = std::tolower(moneyStr[i]);
        }
        if (strchr(moneyStr, 'g') || strchr(moneyStr, 's') || strchr(moneyStr, 'c'))
            moneyToAdd = MoneyStringToMoney(std::string(moneyStr));
        else
            moneyToAdd = atoi(moneyStr);

        if (!moneyToAdd)
            return false;

        // if target is found (player is online), modify money direct
        if (target)
        {
            // check online security
            if (handler->HasLowerSecurity(target, ObjectGuid::Empty))
            ...
            if (moneyToAdd < 0)
            {
                ...
                if (newmoney < 0) // player money can not be negativ
                {
                    handler->PSendSysMessage(LANG_CHAR_HAS_NOT_ENOUGH_GOLD, handler->GetNameLink(target).c_str(), moneyToMoneyString(targetMoney).c_str());
                    handler->SetSentErrorMessage(true);
                    return false;
                }
                else if (newmoney == 0) // take all money
                {
                    handler->PSendSysMessage(LANG_YOU_TAKE_ALL_MONEY, handler->GetNameLink(target).c_str(), moneyToMoneyString(targetMoney).c_str());
                    if (handler->needReportToTarget(target))
                        ChatHandler(target->GetSession()).PSendSysMessage(LANG_YOURS_ALL_MONEY_GONE, handler->GetNameLink().c_str());
                }
                else // all is okay, send info to player and GM
                {
                    handler->PSendSysMessage(LANG_YOU_TAKE_MONEY, moneyToMoneyString((int32)abs(moneyToAdd)).c_str(), handler->GetNameLink(target).c_str(), moneyToMoneyString(targetMoney).c_str(), moneyToMoneyString(newmoney).c_str());
                    if (handler->needReportToTarget(target))
            ...
            {
                if (newmoney >= MAX_MONEY_AMOUNT || newmoney < 0) // < 0 couse of int can be negativ after add to much money
                {
                    handler->PSendSysMessage(LANG_CHAR_CANT_GIVE_BECOUSE_TO_MUCH_MONEY, name.c_str(), moneyToMoneyString(targetMoney).c_str());
                    handler->SetSentErrorMessage(true);
                    return false;
                }

                handler->PSendSysMessage(LANG_YOU_GIVE_MONEY, moneyToMoneyString(moneyToAdd).c_str(), handler->GetNameLink(target).c_str(), moneyToMoneyString(targetMoney).c_str(), moneyToMoneyString(newmoney).c_str());
                if (handler->needReportToTarget(target))
                    ChatHandler(target->GetSession()).PSendSysMessage(LANG_YOURS_MONEY_GIVEN, handler->GetNameLink().c_str(), moneyToAdd);
            }
            target->ModifyMoney(moneyToAdd, ::RG::Logging::MoneyLogType::COMMAND_MOD, handler->GetSession() ? 0 : handler->GetSession()->GetPlayer()->GetGUID().GetCounter());

            TC_LOG_DEBUG("misc", handler->GetTrinityString(LANG_NEW_MONEY), targetMoney, moneyToAdd, target->GetMoney());
        }
        // if no target found modify money direct in database
        else if (nameStr)
        {
            uint32 targetGuid = player::PlayerCache::GetGUID(nameStr).GetCounter();
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_MONEY);
            stmt->setUInt32(0, targetGuid);
            PreparedQueryResult result = CharacterDatabase.Query(stmt);
            if (!result)
            {
                handler->PSendSysMessage(LANG_CHAR_NOT_FOUND, name.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            uint32 targetMoney = (*result)[0].GetUInt32();
            int32 newmoney = targetMoney + moneyToAdd;

            // money take
            if (moneyToAdd < 0)
            {
                int32 available = std::min<int32>(targetMoney, -moneyToAdd);

                TC_LOG_DEBUG("misc", handler->GetTrinityString(LANG_CURRENT_MONEY), targetMoney, moneyToAdd, newmoney);
                if (newmoney < 0) // player money can not be negativ
                {
                    handler->PSendSysMessage(LANG_CHAR_HAS_NOT_ENOUGH_GOLD, name.c_str(), moneyToMoneyString(targetMoney).c_str());
                    handler->SetSentErrorMessage(true);
                    return false;
                }
                else if (newmoney == 0) // take all money
                {
                    handler->PSendSysMessage(LANG_YOU_TAKE_ALL_MONEY, name.c_str(), moneyToMoneyString(targetMoney).c_str());
                }
                else // all is okay, send info to player
                {
                    handler->PSendSysMessage(LANG_YOU_TAKE_MONEY, moneyToMoneyString((int32)abs(moneyToAdd)).c_str(), name.c_str(), moneyToMoneyString(targetMoney).c_str(), moneyToMoneyString(newmoney).c_str());
                }
            }
            else // money give
            {
                ...
                handler->PSendSysMessage(LANG_YOU_GIVE_MONEY, moneyToMoneyString(moneyToAdd).c_str(), name.c_str(), moneyToMoneyString(targetMoney).c_str(), moneyToMoneyString(newmoney).c_str());
            }
            PreparedStatement* stmt2 = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_MONEY);
            stmt2->setUInt32(0, newmoney);
            stmt2->setUInt32(1, targetGuid);
            CharacterDatabase.Execute(stmt2);

            TC_LOG_DEBUG("misc", handler->GetTrinityString(LANG_NEW_MONEY), targetMoney, moneyToAdd, newmoney);
        }
    ...
    //Edit Unit field
    bool HandleModifyBitCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyHonorCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyDrunkCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyMorphCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyPhaseCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    //change standstate
    bool HandleModifyStandStateCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyArenaCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleModifyGenderCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    // mod xp command
    bool HandleModifyXPCommand(ChatHandler *handler, const char* args)
    {
        if (!*args)
            return false;
...
void AddSC_modify_commandscript()
{
    ...
    new LegacyCommandScript("cmd_modify_arenapoints",       HandleModifyArenaCommand,       false);
    new LegacyCommandScript("cmd_modify_bit",               HandleModifyBitCommand,         false);
    new LegacyCommandScript("cmd_modify_drunk",             HandleModifyDrunkCommand,       false);
    new LegacyCommandScript("cmd_modify_energy",            HandleModifyEnergyCommand,      false);
    new LegacyCommandScript("cmd_modify_faction",           HandleModifyFactionCommand,     false);
    new LegacyCommandScript("cmd_modify_gender",            HandleModifyGenderCommand,      false);
    new LegacyCommandScript("cmd_modify_honor",             HandleModifyHonorCommand,       false);
    new LegacyCommandScript("cmd_modify_hp",                HandleModifyHPCommand,          false);
    new LegacyCommandScript("cmd_modify_mana",              HandleModifyManaCommand,        false);
    new LegacyCommandScript("cmd_modify_money",             HandleModifyMoneyCommand,       false);
    new LegacyCommandScript("cmd_modify_mount",             HandleModifyMountCommand,       false);
    new LegacyCommandScript("cmd_modify_phase",             HandleModifyPhaseCommand,       false);
    new LegacyCommandScript("cmd_modify_rage",              HandleModifyRageCommand,        false);
    new LegacyCommandScript("cmd_modify_reputation",        HandleModifyRepCommand,         false);
    new LegacyCommandScript("cmd_modify_runicpower",        HandleModifyRunicPowerCommand,  false);
    new LegacyCommandScript("cmd_modify_scale",             HandleModifyScaleCommand,       false);
    new LegacyCommandScript("cmd_modify_spell",             HandleModifySpellCommand,       false);
    new LegacyCommandScript("cmd_modify_standstate",        HandleModifyStandStateCommand,  false);
    new LegacyCommandScript("cmd_modify_talentpoints",      HandleModifyTalentCommand,      false);
    new LegacyCommandScript("cmd_modify_xp",                HandleModifyXPCommand,          false);

    new LegacyCommandScript("cmd_modify_speed",             HandleModifyASpeedCommand,      false);
    new LegacyCommandScript("cmd_modify_speed_backwalk",    HandleModifyBWalkCommand,       false);
    new LegacyCommandScript("cmd_modify_speed_fly",         HandleModifyFlyCommand,         false);
    new LegacyCommandScript("cmd_modify_speed_walk",        HandleModifySpeedCommand,       false);
    new LegacyCommandScript("cmd_modify_speed_swim",        HandleModifySwimCommand,        false);

    new LegacyCommandScript("cmd_morph",                    HandleModifyMorphCommand,       false);
    new LegacyCommandScript("cmd_demorph",                  HandleDeMorphCommand,           false);
}
