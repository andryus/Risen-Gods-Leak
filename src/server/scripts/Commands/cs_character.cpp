...
namespace chat { namespace command { namespace handler {
    ...
    */
    static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo, ChatHandler* handler)
    {
        ...
        if (player::PlayerCache::Exists(delInfo.name))
        {
        ...
        player::PlayerCache::Entry entry;
        entry.guid = ObjectGuid(HighGuid::Player, delInfo.lowGuid);
        entry.name = delInfo.name;
        entry.account = delInfo.accountId;
        player::PlayerCache::Add(entry, true);
    }
    ...
    static void HandleCharacterLevel(Player* player, ObjectGuid playerGuid, uint32 oldLevel, uint32 newLevel, ChatHandler* handler)
    {
        ...
        else
        {
            ...
            player::PlayerCache::UpdateLevel(playerGuid, newLevel);
        }
    }

    bool HandleCharacterTitlesCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    //rename characters
    bool HandleCharacterRenameCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
        ...
        if (newNameStr)
        {
            ...
            else
            {
                ...
                playerOldName = player::PlayerCache::GetName(targetGuid);
            }
            ...
            player::PlayerCache::UpdateNameData(targetGuid, newName);
            ...
            if (WorldSession* session = handler->GetSession())
            {
                if (Player* player = session->GetPlayer())
                    sLog->outCommand(session->GetAccountId(), "GM %s (Account: %u) forced rename %s to player %s (Account: %u)", player->GetName().c_str(), session->GetAccountId(), newName.c_str(), playerOldName.c_str(), player::PlayerCache::GetAccountId(targetGuid));
            }
            else
        ...
        else
        {
            ...
            else
            {
                ...
                stmt->setUInt32(1, targetGuid.GetCounter());
                CharacterDatabase.Execute(stmt);
                player::PlayerCache::UpdateAtLogin(targetGuid, AT_LOGIN_RENAME, player::PlayerCache::FlagMode::ADD);
            }
        }
    ...
    bool HandleCharacterLevelCommand(ChatHandler* handler, char const* args)
    {
        char* nameStr;
        char* levelStr;
        ...
        int32 oldlevel = target ? target->getLevel() : player::PlayerCache::GetLevel(targetGuid);
    ...
    bool HandleCharacterCustomizeCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
        ObjectGuid targetGuid;
        std::string targetName;
        ...
        else
        {
            ...
            player::PlayerCache::UpdateAtLogin(targetGuid, AT_LOGIN_CUSTOMIZE, player::PlayerCache::FlagMode::ADD);
        }
        CharacterDatabase.Execute(stmt);
    ...
    bool HandleCharacterChangeFactionCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
        ObjectGuid targetGuid;
        std::string targetName;

        ObjectGuid handlerGuid = handler->GetSession()->GetPlayer()->GetGUID();

        if (!handler->extractPlayerTarget((char*)args, &target, &targetGuid, &targetName))
            return false;

        //Check if character has post, auctions or is guild/arenaleader
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHECK_FACTIONCHANGE);
        stmt->setUInt32(0, targetGuid.GetCounter());
        stmt->setUInt32(1, targetGuid.GetCounter());
        stmt->setUInt32(2, targetGuid.GetCounter());
        stmt->setUInt32(3, targetGuid.GetCounter());
 
        auto factionChange = [](Player* targetPlayer, ObjectGuid targetGuid, const std::string& targetName, ChatHandler& newHandler)
        {
            PreparedStatement* stm = CharacterDatabase.GetPreparedStatement(CHAR_UPD_ADD_AT_LOGIN_FLAG);
            stm->setUInt16(0, uint16(AT_LOGIN_CHANGE_FACTION));
            if (targetPlayer)
            {
                newHandler.PSendSysMessage(LANG_CUSTOMIZE_PLAYER, newHandler.GetNameLink(targetPlayer).c_str());
                targetPlayer->SetAtLoginFlag(AT_LOGIN_CHANGE_FACTION);
                stm->setUInt32(1, targetGuid.GetCounter());
            }
            else
            {
                std::string oldNameLink = newHandler.playerLink(targetName);
                newHandler.PSendSysMessage(LANG_CUSTOMIZE_PLAYER_GUID, oldNameLink.c_str(), targetGuid.GetCounter());
                stm->setUInt32(1, targetGuid.GetCounter());
                player::PlayerCache::UpdateAtLogin(targetGuid, AT_LOGIN_CHANGE_FACTION, player::PlayerCache::FlagMode::ADD);
            }
            CharacterDatabase.Execute(stm);
        };

        auto errorMessage = [](PreparedQueryResult result, ChatHandler& newHandler)
        {
            uint8 resultFlag = 0;
            do
            {
            ...
            {
                { 1, LANG_FACTION_CHANGE_AUCTION_ERROR },
                { 2, LANG_FACTION_CHANGE_POST_ERROR },
                { 4, LANG_FACTION_CHANGE_GUILD_ERROR },
                { 8, LANG_FACTION_CHANGE_ARENA_ERROR }
            };
            ...
            for (const auto[flag, msg] : resultMsg)
            {
                ...
                {
                    newHandler.PSendSysMessage(msg);
                    break;
                }
            }
        };
        ...
        CharacterDatabase.AsyncQuery(stmt)
                .via(handler->GetSession()->GetPlayer()->GetExecutor())
                .then([targetGuid, handlerGuid, factionChange, targetName, errorMessage](PreparedQueryResult result) {

                Player* targetPlayer = ObjectAccessor::FindPlayer(targetGuid);
                Player* handlerPlayer = ObjectAccessor::FindPlayer(handlerGuid);
                if (!targetPlayer || !handlerPlayer)
                    return;

                auto newHandler = ChatHandler(handlerPlayer->GetSession());

                if (!result)
                    factionChange(targetPlayer, targetGuid, targetName, newHandler);
                else
                    errorMessage(std::move(result), newHandler);
            });

        return true;
    }

    bool HandleCharacterChangeRaceCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
        ObjectGuid targetGuid;
        std::string targetName;
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_CUSTOMIZE_PLAYER_GUID, oldNameLink.c_str(), targetGuid.GetCounter());
            stmt->setUInt32(1, targetGuid.GetCounter());
            player::PlayerCache::UpdateAtLogin(targetGuid, AT_LOGIN_CHANGE_RACE, player::PlayerCache::FlagMode::ADD);
        }
        CharacterDatabase.Execute(stmt);
    ...
    bool HandleCharacterReputationCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
        if (!handler->extractPlayerTarget((char*)args, &target))
            return false;
    ...
    */
    bool HandleCharacterDeletedListCommand(ChatHandler* handler, char const* args)
    {
    ...
    */
    bool HandleCharacterDeletedRestoreCommand(ChatHandler* handler, char const* args)
    {
    ...
    * @param args the search string which either contains a player GUID or a part fo the character-name
    */
    bool HandleCharacterDeletedDeleteCommand(ChatHandler* handler, char const* args)
    {
        // It is required to submit at least one argument
        if (!*args)
            return false;
    ...
    * @param args the search string which either contains a player GUID or a part fo the character-name
    */
    bool HandleCharacterDeletedOldCommand(ChatHandler* /*handler*/, char const* args)
    {
    ...
    bool HandleCharacterEraseCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (!isNumeric(characterName_str))
        {
            if (!normalizePlayerName(characterName))
                return false;

            player = sObjectAccessor->FindPlayerByName(characterName);
            if (player)
                characterGuid = player->GetGUID().GetCounter();
            else
                characterGuid = player::PlayerCache::GetGUID(characterName).GetCounter();
        }
        else
        {
            characterGuid = atoi(characterName_str);
            player = ObjectAccessor::FindConnectedPlayer(ObjectGuid(HighGuid::Player, characterGuid));
        }

        if (player)
            player->GetSession()->KickPlayer();

        accountId = player::PlayerCache::GetAccountId(ObjectGuid(HighGuid::Player, characterGuid));

        if (!characterGuid || !accountId)
        {
        ...
        Player::DeleteFromDB(ObjectGuid(HighGuid::Player, characterGuid), accountId, true, true);
        handler->PSendSysMessage(LANG_CHARACTER_DELETED, characterName.c_str(), characterGuid, accountName.c_str(), accountId);

        return true;
    }

    bool HandleLevelUpCommand(ChatHandler* handler, char const* args)
    {
        char* nameStr;
        char* levelStr;
        handler->extractOptFirstArg((char*)args, &nameStr, &levelStr);
    ...
    bool HandlePDumpLoadCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (guidStr)
        {
            ...
            if (player::PlayerCache::Exists(ObjectGuid(HighGuid::Player, guid)))
            {
        ...
        switch (PlayerDumpReader().LoadDump(fileStr, accountId, name, guid, false))
        {
        case DUMP_SUCCESS:
            handler->PSendSysMessage(LANG_COMMAND_IMPORT_SUCCESS, guid);
            break;
    ...
    bool HandlePDumpLoadOldCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        switch (PlayerDumpReader().LoadDump(fileStr, accountId, name, guid, true))
        {
        case DUMP_SUCCESS:
    ...
    bool HandlePDumpWriteCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        else
        {
            ...
            guid = player::PlayerCache::GetGUID(name);
        }
...
void AddSC_character_commandscript()
{
    ...
    new LegacyCommandScript("cmd_character_customize",          HandleCharacterCustomizeCommand,        true);
    new LegacyCommandScript("cmd_character_changefaction",      HandleCharacterChangeFactionCommand,    true);
    new LegacyCommandScript("cmd_character_changerace",         HandleCharacterChangeRaceCommand,       true);
    new LegacyCommandScript("cmd_character_erase",              HandleCharacterEraseCommand,            true);
    new LegacyCommandScript("cmd_character_level",              HandleCharacterLevelCommand,            true);
    new LegacyCommandScript("cmd_character_rename",             HandleCharacterRenameCommand,           true);
    new LegacyCommandScript("cmd_character_reputation",         HandleCharacterReputationCommand,       true);
    new LegacyCommandScript("cmd_character_titles",             HandleCharacterTitlesCommand,           true);

    new LegacyCommandScript("cmd_character_deleted_delete",     HandleCharacterDeletedDeleteCommand,    true);
    new LegacyCommandScript("cmd_character_deleted_list",       HandleCharacterDeletedListCommand,      true);
    new LegacyCommandScript("cmd_character_deleted_restore",    HandleCharacterDeletedRestoreCommand,   true);
    new LegacyCommandScript("cmd_character_deleted_old",        HandleCharacterDeletedOldCommand,       true);

    new LegacyCommandScript("cmd_levelup",                      HandleLevelUpCommand,                   false);

    new LegacyCommandScript("cmd_pdump_load",               HandlePDumpLoadCommand,                 true);
    new LegacyCommandScript("cmd_pdump_loadold",            HandlePDumpLoadOldCommand,              true);
    new LegacyCommandScript("cmd_pdump_write",              HandlePDumpWriteCommand,                true);
}
