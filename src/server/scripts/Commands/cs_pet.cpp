...
namespace chat { namespace command { namespace handler {
    ...
    bool HandlePetCreateCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        Pet* pet = Pet::CreateFromCreature(*player, HUNTER_PET, *creatureTarget);
        if (!pet)
        {
    ...
    bool HandlePetLearnCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandlePetUnlearnCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
...
void AddSC_pet_commandscript()
{
    ...
    new LegacyCommandScript("cmd_pet_create",   HandlePetCreateCommand,     false);
    new LegacyCommandScript("cmd_pet_learn",    HandlePetLearnCommand,      false);
    new LegacyCommandScript("cmd_pet_unlearn",  HandlePetUnlearnCommand,    false);
}
