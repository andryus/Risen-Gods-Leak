...
namespace chat { namespace command { namespace handler {
    ...
    bool HandleLookupAreaCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupCreatureCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupEventCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupFactionCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupItemCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupItemSetCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupObjectCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupQuestCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupSkillCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupSpellCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupSpellIdCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupTaxiNodeCommand(ChatHandler* handler, const char * args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupTeleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
        ...
        auto teleMap = sObjectMgr->GetGameTeleMap();
        for (auto& [key, value] : teleMap)
        {
            if (value.wnameLow.find(wNamePart) == std::wstring::npos)
            {
                bool found = false;
                for (auto extraName : value.names)
                {
                    if (extraName.find(wNamePart) != std::wstring::npos)
                    {
                        found = true;
                        break;
                    }
            ...
            reply << "Teleport ID: " << key << "\n";
            if (handler->GetSession())
                reply << " -  |cffffffff|Htele:" << key << "|h[" << value.name << "]|h|r\n";
            else
            ...
            for (auto extraName : value.names)
            {
                WStrToUtf8(extraName, nameString);
                if (handler->GetSession())
                    reply << " -  |cffffffff|Htele:" << key << "|h[" << nameString << "]|h|r\n";
                else
                    reply << "  " << key << ' ' << nameString << "\n";
            }
    ...
    bool HandleLookupTitleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupMapCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool LookupPlayerSearchCommand(PreparedQueryResult result, int32 limit, ChatHandler* handler)
    {
        if (!result)
        {
    ...
    bool HandleLookupPlayerIpCommand(ChatHandler* handler, char const* args)
    {
        std::string ip;
        int32 limit;
        char* limitStr;
        ...
        return LookupPlayerSearchCommand(std::move(result), limit, handler);
    }

    bool HandleLookupPlayerAccountCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleLookupPlayerEmailCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
...
void AddSC_lookup_commandscript()
{
    ...
    new LegacyCommandScript("cmd_lookup_area",              HandleLookupAreaCommand,            true);
    new LegacyCommandScript("cmd_lookup_creature",          HandleLookupCreatureCommand,        true);
    new LegacyCommandScript("cmd_lookup_event",             HandleLookupEventCommand,           true);
    new LegacyCommandScript("cmd_lookup_faction",           HandleLookupFactionCommand,         true);
    new LegacyCommandScript("cmd_lookup_item",              HandleLookupItemCommand,            true);
    new LegacyCommandScript("cmd_lookup_itemset",           HandleLookupItemSetCommand,         true);
    new LegacyCommandScript("cmd_lookup_object",            HandleLookupObjectCommand,          true);
    new LegacyCommandScript("cmd_lookup_quest",             HandleLookupQuestCommand,           true);
    new LegacyCommandScript("cmd_lookup_skill",             HandleLookupSkillCommand,           true);
    new LegacyCommandScript("cmd_lookup_taxinode",          HandleLookupTaxiNodeCommand,        true);
    new LegacyCommandScript("cmd_lookup_teleport",          HandleLookupTeleCommand,            true);
    new LegacyCommandScript("cmd_lookup_title",             HandleLookupTitleCommand,           true);
    new LegacyCommandScript("cmd_lookup_map",               HandleLookupMapCommand,             true);

    new LegacyCommandScript("cmd_lookup_spell",             HandleLookupSpellCommand,           true);
    new LegacyCommandScript("cmd_lookup_spell_id",          HandleLookupSpellIdCommand,         true);

    new LegacyCommandScript("cmd_lookup_player_ip",         HandleLookupPlayerIpCommand,        true);
    new LegacyCommandScript("cmd_lookup_player_account",    HandleLookupPlayerAccountCommand,   true);
    new LegacyCommandScript("cmd_lookup_player_email",      HandleLookupPlayerEmailCommand,     true);
}
