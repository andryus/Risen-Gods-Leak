...
namespace chat { namespace command { namespace handler {
    ...
    */
    ...
    bool HandleGoCreatureCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (Creature* creature = ObjectAccessor::GetCreature(mapId, ObjectGuid(HighGuid::Unit, id, guid)))
        {
    ...
    bool HandleGoGraveyardCommand(ChatHandler* handler, char const* args)
    {
        Player* player = handler->GetSession()->GetPlayer();
    ...
    //teleport to grid
    bool HandleGoGridCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleGoObjectCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleGoTaxinodeCommand(ChatHandler* handler, char const* args)
    {
        Player* player = handler->GetSession()->GetPlayer();
    ...
    bool HandleGoTriggerCommand(ChatHandler* handler, char const* args)
    {
        Player* player = handler->GetSession()->GetPlayer();
    ...
    bool HandleGoZoneXYCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleGoXYZCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleGoTicketCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        const bool successful = support::sTicketMgr.withLock([&](support::TicketMgr& ticketMgr)
        {
...
void AddSC_go_commandscript()
{
    ...
    new LegacyCommandScript("cmd_go",               HandleGoXYZCommand,         false);
    new LegacyCommandScript("cmd_go_creature",      HandleGoCreatureCommand,    false);
    new LegacyCommandScript("cmd_go_graveyard",     HandleGoGraveyardCommand,   false);
    new LegacyCommandScript("cmd_go_grid",          HandleGoGridCommand,        false);
    new LegacyCommandScript("cmd_go_object",        HandleGoObjectCommand,      false);
    new LegacyCommandScript("cmd_go_taxinode",      HandleGoTaxinodeCommand,    false);
    new LegacyCommandScript("cmd_go_trigger",       HandleGoTriggerCommand,     false);
    new LegacyCommandScript("cmd_go_zonexy",        HandleGoZoneXYCommand,      false);
    new LegacyCommandScript("cmd_go_xyz",           HandleGoXYZCommand,         false);
    new LegacyCommandScript("cmd_go_ticket",        HandleGoTicketCommand,      false);
}
