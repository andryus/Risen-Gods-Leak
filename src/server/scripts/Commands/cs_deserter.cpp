...
namespace chat { namespace command { namespace handler {
    ...
    * @endcode
    */
    bool HandleDeserterAdd(ChatHandler* handler, char const* args, bool isInstance)
    {
        if (!*args)
            return false;
    ...
    * @endcode
    */
    bool HandleDeserterRemove(ChatHandler* handler, char const* /*args*/, bool isInstance)
    {
        Player* player = handler->getSelectedPlayer();
        if (!player)
        {
    ...
    bool HandleDeserterInstanceAdd(ChatHandler* handler, char const* args)
    {
        return HandleDeserterAdd(handler, args, true);
    }

    /// @sa HandleDeserterAdd()
    bool HandleDeserterBGAdd(ChatHandler* handler, char const* args)
    {
        return HandleDeserterAdd(handler, args, false);
    }

    /// @sa HandleDeserterRemove()
    bool HandleDeserterInstanceRemove(ChatHandler* handler, char const* args)
    {
        return HandleDeserterRemove(handler, args, true);
    }

    /// @sa HandleDeserterRemove()
    bool HandleDeserterBGRemove(ChatHandler* handler, char const* args)
    {
...
void AddSC_deserter_commandscript()
{
    ...
    new LegacyCommandScript("cmd_deserter_instance_add",        HandleDeserterInstanceAdd,      false);
    new LegacyCommandScript("cmd_deserter_instance_remove",     HandleDeserterInstanceRemove,   false);

    new LegacyCommandScript("cmd_deserter_bg_add",              HandleDeserterBGAdd,            false);
    new LegacyCommandScript("cmd_deserter_bg_remove",           HandleDeserterBGRemove,         false);
}
