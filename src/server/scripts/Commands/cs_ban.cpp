...
namespace chat { namespace command { namespace handler {
    ...
    bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        if (!*args)
            return false;
        ...
        BanResult result = BAN_SYNTAX_ERROR;
        {
            const auto duration = World::BanDuration{ TimeStringToSecs(durationStr) };

            switch (mode)
            {
                case BAN_IP:
                    result = sWorld->BanAccountByIp(nameOrIP, duration, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName().c_str() : "");
                    break;
                case BAN_ACCOUNT:
                    result = sWorld->BanAccountByName(nameOrIP, duration, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName().c_str() : "");
                    break;
                case BAN_CHARACTER:
                    result = sWorld->BanAccountByCharacterName(nameOrIP, duration, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName().c_str() : "");
                    break;
                default:
                    break;
            }
    ...
    bool HandleBanAccountCommand(ChatHandler* handler, char const* args)
    {
        return HandleBanHelper(BAN_ACCOUNT, args, handler);
    }

    bool HandleBanCharacterCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleBanAccountByCharCommand(ChatHandler* handler, char const* args)
    {
        return HandleBanHelper(BAN_CHARACTER, args, handler);
    }

    bool HandleBanIPCommand(ChatHandler* handler, char const* args)
    {
        return HandleBanHelper(BAN_IP, args, handler);
    }

    bool HandleBanInfoHelper(uint32 accountId, char const* accountName, ChatHandler* handler)
    {
    ...
    bool HandleBanInfoAccountCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleBanInfoCharacterCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleBanListHelper(PreparedQueryResult result, ChatHandler* handler)
    {
    ...
    bool HandleBanListAccountCommand(ChatHandler* handler, char const* args)
    {
    ...
    bool HandleBanListCharacterCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleBanListIPCommand(ChatHandler* handler, char const* args)
    {
        PreparedStatement* stmt = NULL;
    ...
    bool HandleUnBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        if (!*args)
            return false;
    ...
    bool HandleUnBanAccountCommand(ChatHandler* handler, char const* args)
    {
        return HandleUnBanHelper(BAN_ACCOUNT, args, handler);
    }

    bool HandleUnBanCharacterCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    bool HandleUnBanAccountByCharCommand(ChatHandler* handler, char const* args)
    {
        return HandleUnBanHelper(BAN_CHARACTER, args, handler);
    }

    bool HandleUnBanIPCommand(ChatHandler* handler, char const* args)
    {
...
void AddSC_ban_commandscript()
{
    ...
    new LegacyCommandScript("cmd_ban_account",          HandleBanAccountCommand,            true);
    new LegacyCommandScript("cmd_ban_character",        HandleBanCharacterCommand,          true);
    new LegacyCommandScript("cmd_ban_playeraccount",    HandleBanAccountByCharCommand,      true);
    new LegacyCommandScript("cmd_ban_ip",               HandleBanIPCommand,                 true);

    new LegacyCommandScript("cmd_baninfo_account",      HandleBanInfoAccountCommand,        true);
    new LegacyCommandScript("cmd_baninfo_character",    HandleBanInfoCharacterCommand,      true);
    new LegacyCommandScript("cmd_baninfo_ip",           HandleBanInfoIPCommand,             true);

    new LegacyCommandScript("cmd_banlist_account",      HandleBanListAccountCommand,        true);
    new LegacyCommandScript("cmd_banlist_character",    HandleBanListCharacterCommand,      true);
    new LegacyCommandScript("cmd_banlist_ip",           HandleBanListIPCommand,             true);

    new LegacyCommandScript("cmd_unban_account",        HandleUnBanAccountCommand,          true);
    new LegacyCommandScript("cmd_unban_character",      HandleUnBanCharacterCommand,        true);
    new LegacyCommandScript("cmd_unban_playeraccount",  HandleUnBanAccountByCharCommand,    true);
    new LegacyCommandScript("cmd_unban_ip",             HandleUnBanIPCommand,               true);
}
