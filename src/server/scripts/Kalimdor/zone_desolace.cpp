    ...
    struct npc_aged_dying_ancient_kodoAI : public ScriptedAI
    {
...

        {
            if (who->GetEntry() == NPC_SMEED && me->IsWithinDistInMap(who, 10.0f) && !me->HasAura(SPELL_KODO_KOMBO_GOSSIP))
            {
                ...
                DoCastSelf(SPELL_KODO_KOMBO_GOSSIP, true);
...
Position DemonSpiritSpawnPos = {-344.357f, 1764.762f, 138.371f, 3.65f};
    ...
    public:
        ...
        bool OnGossipHello(Player* player, GameObject* go) override
        {
            if (player->GetQuestStatus(QUEST_HAND_IRUXOS) == QUEST_STATUS_INCOMPLETE && !go->FindNearestCreature(NPC_DEMON_SPIRIT, 25.0f, true))
                player->SummonCreature(NPC_DEMON_SPIRIT, DemonSpiritSpawnPos, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000);
