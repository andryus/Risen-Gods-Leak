...
public:
    ...
    struct npc_ranshallaAI : public npc_escortAI, private DialogueHelper
    {
        ...
        void DoChannelTorchSpell(bool isAltarWaypoint = false)
        {
            ...
            if (isAltarWaypoint)
            {
                if (GameObject* go = me->FindNearestGameObject(GO_ELUNE_ALTAR, 10.0f))
                {
            ...
            else if (GameObject* go = me->FindNearestGameObject(GO_ELUNE_FIRE, 10.0f))
                go->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
        ...
        void WaypointReached(uint32 pointId) override
        {
            switch (pointId)
            {
                ...
                case 41:
                {
                    ...
                    me->GetGameObjectListWithEntryInGrid(eluneLights, GO_ELUNE_LIGHT, 20.0f);
        ...
        void JustDidDialogueStep(int32 entry) override
        {
            switch (entry)
            {
                ...
                case SAY_PRIESTESS_ALTAR_8:
                    ...
                    if (GameObject* gem = me->FindNearestGameObject(GO_ELUNE_GEM, 10.0f))
                    {
                        if (gem->isSpawned())
                            break;
...
public:
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        ...
        if (Creature* ranshalla = go->FindNearestCreature(NPC_RANSHALLA, 10.0f))
        {
