                ...
                if (Player* player = GetPlayerForEscort())
                {
                    switch (waypointId)
                    {
                        ...
                        case 24:
                            ...
                            if (GameObject* go = me->FindNearestGameObject(GO_NAGA_BRAZIER, INTERACTION_DISTANCE*2))
                            {
...
class go_naga_brazier : public GameObjectScript
{
    public:
        ...
        {
            if (Creature* creature = go->FindNearestCreature(NPC_MUGLASH, INTERACTION_DISTANCE*2))
            {
