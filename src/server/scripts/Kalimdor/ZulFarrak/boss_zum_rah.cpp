...
public:
    ...
    struct boss_zum_rahAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_ACTIVATE_GRAVE, 1000);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SHADOWBOLT_VOLLEY:
                        ...
                        break;
                    case EVENT_ACTIVATE_GRAVE:
                        if (GameObject* grave = me->FindNearestGameObject(GO_SHALLOW_GRAVE, 200.0f))
                        {
                            if (Creature* zombie = grave->SummonCreature(NPC_ZOMBIE, grave->GetPositionX(), grave->GetPositionY(), grave->GetPositionZ(), 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000))
                                zombie->AI()->AttackStart(zombie->FindNearestPlayer(100.0f));
                            grave->RemoveFromWorld(); // Never Respawns
                            Talk(SAY_CHILDREN);
                        }
                        events.ScheduleEvent(EVENT_ACTIVATE_GRAVE, 15000);
                        break;
                    default:
                        break;
                }
            ...
            if (!_ward80 && HealthBelowPct(80))
            {
                _ward80 = true;
                Talk(SAY_WARD);
                DoCastSelf(SPELL_WARD_OF_ZUM_RAH, true);
            }
