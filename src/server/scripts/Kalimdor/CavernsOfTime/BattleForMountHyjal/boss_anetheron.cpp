...
public:
    ...
    struct boss_anetheronAI : public hyjal_trashAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (AuraTimer <= diff)
            {
                DoCastSelf(SPELL_VAMPIRIC_AURA, true);
...
class spell_vampiric_auraAuraScript : public AuraScript
{
    PrepareAuraScript(spell_vampiric_auraAuraScript);
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& procInfo)
    {
        ...
        GetCaster()->CastCustomSpell(SPELL_VAMPIRIC_AURA_HEAL, SPELLVALUE_BASE_POINT0, 3 * procInfo.GetDamageInfo()->GetDamage(), GetCaster(), true);
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_vampiric_auraAuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
void AddSC_boss_anetheron()
{
    ...
    new SpellScriptLoaderEx<spell_vampiric_auraAuraScript>("spell_vampiric_aura");
}
