...
void hyjalAI::UpdateAI(uint32 diff)
{
    if (IsDummy)
    {
        ...
        {
            DoCastSelf(SPELL_MASS_TELEPORT, false);
            DoMassTeleport = false;
...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    auto creature_searcher = Trinity::createBaseObjectSearcher<Creature, Trinity::ContainerAction>(me, creatures, creature_check);
    me->GetMap()->Visit(x, y, me->GetGridActivationRange(), creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    auto creature_searcher = Trinity::createBaseObjectSearcher<Creature, Trinity::FunctionAction>(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        std::list<Creature*> creatures;
        Trinity::AllFriendlyCreaturesInGrid creature_check(me);
        me->VisitAnyNearbyObject<Creature, Trinity::ContainerAction>(me->GetGridActivationRange(), creatures, creature_check);
