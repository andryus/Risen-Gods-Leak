...
enum Gossipoptions
{
    GOSSIP_OPTION_HYJAL          = 60195,
    GOSSIP_ITEM_BEGIN_ALLY       = 0,
    GOSSIP_ITEM_ANETHERON        = 1,
    GOSSIP_ITEM_BEGIN_HORDE      = 2,
    GOSSIP_ITEM_AZGALOR          = 3,
    GOSSIP_ITEM_RETREAT          = 4,
    GOSSIP_ITEM_TYRANDE          = 5,
    GOSSIP_ITEM_GM1              = 6,
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (RageEncounter == NOT_STARTED)
            player->AddGossipItem(GOSSIP_OPTION_HYJAL, GOSSIP_ITEM_BEGIN_ALLY, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        else if (RageEncounter == DONE && AnetheronEncounter == NOT_STARTED)
            player->AddGossipItem(GOSSIP_OPTION_HYJAL, GOSSIP_ITEM_ANETHERON, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
        else if (RageEncounter == DONE && AnetheronEncounter == DONE)
            player->AddGossipItem(GOSSIP_OPTION_HYJAL, GOSSIP_ITEM_RETREAT, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);

        if (player->IsGameMaster())
            player->AddGossipItem(GOSSIP_OPTION_HYJAL, GOSSIP_ITEM_GM1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (AnetheronEvent == DONE && ai->GetInstanceData(DATA_ALLIANCE_RETREAT))
        {
            ...
            if (KazrogalEvent == NOT_STARTED)
                player->AddGossipItem(GOSSIP_OPTION_HYJAL, GOSSIP_ITEM_BEGIN_HORDE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            else if (KazrogalEvent == DONE && AzgalorEvent == NOT_STARTED)
                player->AddGossipItem(GOSSIP_OPTION_HYJAL, GOSSIP_ITEM_AZGALOR, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
...
public:
        ...
        if (action == GOSSIP_ACTION_INFO_DEF)
        {
                ...
                if (msg == EQUIP_ERR_OK)
                    if (Item* item = player->StoreNewItem(dest, ITEM_TEAR_OF_GODDESS, true))
                    {
                        ...
                        RG_LOG<ItemLogModule>(item, RG::Logging::ItemLogType::ADDED, 1);
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (AzgalorEvent == DONE && !player->HasItemCount(ITEM_TEAR_OF_GODDESS))
            player->AddGossipItem(GOSSIP_OPTION_HYJAL, GOSSIP_ITEM_TYRANDE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        player->SEND_GOSSIP_MENU(907, creature->GetGUID());
        return true;
    }
