...
enum Enums
{
    ...
    SAY_FAREWELL                  = 5,
    SAY_ATTACKED                  = 11,
    AGGRO_NPC_DEVIATE_GUARDIAN    = 12,
    AGGRO_DEVIATE_RAVAGER         = 13,
    AGGRO_NPC_ECTOPLASM           = 14,
    AGGRO_NPC_DRUID_OF_FANG       = 15,
    ...
    NPC_DEVIATE_GUARDIAN          = 3637,
    NPC_ECTOPLASM                 = 3640,
    NPC_DRUID_OF_FANG             = 3840
};
...
public:
    ...
    struct npc_disciple_of_naralexAI : public npc_escortAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            switch (who->GetEntry())
            {
                case NPC_DEVIATE_GUARDIAN:
                    Talk(AGGRO_NPC_DEVIATE_GUARDIAN);
                    break;
                case NPC_DEVIATE_RAVAGER:
                    Talk(AGGRO_DEVIATE_RAVAGER);
                    break;
                case NPC_ECTOPLASM:
                    Talk(AGGRO_NPC_ECTOPLASM);
                    break;
                case NPC_DRUID_OF_FANG:
                    Talk(AGGRO_NPC_DRUID_OF_FANG);
                    break;
                default:
                    break;
            }
        ...
        void JustSummoned(Creature* summoned) override
        {
            ...
            AddThreat(summoned, 1.0f);
            summoned->CombatStart(me);
            me->CombatStart(summoned);
            summoned->SetInCombatState(true, me);
            summoned->AI()->AttackStart(me);
            summoned->SetSpeedRate(MOVE_WALK, 0.3f);
            summoned->SetSpeedRate(MOVE_RUN, 0.3f);
        }

        void SummonedCreatureDies(Creature* c, Unit* /*attacker*/) override
        {
            ObjectGuid guid = c->GetGUID();

            std::set<ObjectGuid>::iterator itr = summons.find(guid);
            if (itr != summons.end())
                summons.erase(itr);
        }

        void SummonTempCreature(uint32 id, float x, float y, float z, float ang)
        {
            TempSummon *summon = me->SummonCreature(id, x, y, z, ang, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
            if (summon != nullptr)
                summons.insert(summon->GetGUID());
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(!summons.empty())  // there are alive summons, delay next event
                return;
                    ...
                    switch (currentEvent)
                    {
                        case TYPE_NARALEX_PART1:
                            if (eventProgress == 1)
                            {
                                ...
                                SummonTempCreature(NPC_DEVIATE_RAVAGER, -82.1763f, 227.874f, -93.3233f, 0);
                                SummonTempCreature(NPC_DEVIATE_RAVAGER, -72.9506f, 216.645f, -93.6756f, 0);
                            }
                        break;
                        case TYPE_NARALEX_PART2:
                            if (eventProgress == 1)
                            {
                                ...
                                eventTimer = 30000;
                                SummonTempCreature(NPC_DEVIATE_VIPER, -61.5261f, 273.676f, -92.8442f, 0);
                                SummonTempCreature(NPC_DEVIATE_VIPER, -58.4658f, 280.799f, -92.8393f, 0);
                                SummonTempCreature(NPC_DEVIATE_VIPER, -50.002f,  278.578f, -92.8442f, 0);
                            }
                            else
                            if (eventProgress == 2)
                            {
                        ...
                        case TYPE_NARALEX_PART3:
                            ...
                            if (eventProgress == 2)
                            {
                                ...
                                eventTimer = 15000;
                                //CAST_AI(npc_escort::npc_escortAI, me->AI())->SetCanDefend(false);
                                if (Creature* naralex = instance->instance->GetCreature(ObjectGuid(instance->GetGuidData(DATA_NARALEX))))
                            ...
                            if (eventProgress == 3)
                            {
                                ...
                                SummonTempCreature(NPC_DEVIATE_MOCCASIN, 135.943f, 199.701f, -103.529f, 0);
                                SummonTempCreature(NPC_DEVIATE_MOCCASIN, 151.08f,  221.13f,  -103.609f, 0);
                                SummonTempCreature(NPC_DEVIATE_MOCCASIN, 128.007f, 227.428f, -97.421f, 0);
                            }
                            else
                            if (eventProgress == 4)
                            {
                                ...
                                SummonTempCreature(NPC_NIGHTMARE_ECTOPLASM, 133.413f, 207.188f, -102.469f, 0);
                                SummonTempCreature(NPC_NIGHTMARE_ECTOPLASM, 142.857f, 218.645f, -102.905f, 0);
                                SummonTempCreature(NPC_NIGHTMARE_ECTOPLASM, 105.102f, 227.211f, -102.752f, 0);
                                SummonTempCreature(NPC_NIGHTMARE_ECTOPLASM, 153.372f, 235.149f, -102.826f, 0);
                                SummonTempCreature(NPC_NIGHTMARE_ECTOPLASM, 149.524f, 251.113f, -102.558f, 0);
                                SummonTempCreature(NPC_NIGHTMARE_ECTOPLASM, 136.208f, 266.466f, -102.977f, 0);
                                SummonTempCreature(NPC_NIGHTMARE_ECTOPLASM, 126.167f, 274.759f, -102.962f, 0);
                            }
                            else
                            if (eventProgress == 5)
                            {
                                ...
                                if (Creature* mutanus = me->FindNearestCreature(NPC_MUTANUS_THE_DEVOURER, 100.0f))
