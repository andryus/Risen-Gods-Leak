...
public:
    ...
    struct boss_ichoronAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            if (instance)
            {
                if (GameObject* pDoor = instance->instance->GetGameObject(ObjectGuid(instance->GetGuidData(DATA_ICHORON_CELL))))
            ...
            if (!bIsFrenzy)
            {
                if (uiBubbleCheckerTimer <= uiDiff)
                {
                    ...
                    else
                    {
                        ...
                        {
                            for (std::list<ObjectGuid>::const_iterator itr = m_waterElements.begin(); itr != m_waterElements.end(); ++itr)
...
public:
    ...
    struct npc_ichor_globuleAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            DoCastSelf(SPELL_WATER_GLOBULE);
        }
        ...
        void UpdateAI(uint32 uiDiff)
        {
            if (uiRangeCheck_Timer < uiDiff)
            {
                if (instance)
                {
                    if (Creature* pIchoron = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_ICHORON))))
                    {
