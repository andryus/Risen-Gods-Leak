...
enum define
{
    GOSSIP_ITEM_SINCLARI = 1623,
    OPTION_EVENT_START   = 1,
    OPTION_ONE           = 2,
    OPTION_WANT_IN       = 3
    /*
    ID 1: GOSSIP_START_EVENT "Get your people to safety, we'll keep the Blue Dragonflight's forces at bay."
    ID 2: GOSSIP_ITEM_1      "Activate the crystals when we get in trouble, right"
    ID 3: GOSSIP_I_WANT_IN   "I'm not fighting, so send me in now!"
    */
};
        ...
        if (InstanceScript* instance = creature->GetInstanceScript())
        {
            switch (instance->GetData(DATA_MAIN_EVENT_PHASE))
            {
                ...
                case FAIL: // Allow to start event if not started or wiped
                    player->AddGossipItem(GOSSIP_ITEM_SINCLARI, OPTION_EVENT_START, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                    player->AddGossipItem(GOSSIP_ITEM_SINCLARI, OPTION_ONE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
                    player->SEND_GOSSIP_MENU(13853, creature->GetGUID());                   
                    break;
                case IN_PROGRESS: // Allow to teleport inside if event is in progress
                    player->AddGossipItem(GOSSIP_ITEM_SINCLARI, OPTION_WANT_IN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
                    player->SEND_GOSSIP_MENU(13853, creature->GetGUID());
                    break;
                default:
...
public:
    ...
    struct npc_azure_saboteurAI : public npc_escortAI
    {
        ...
        {
            DoCastSelf(SABOTEUR_SHIELD_DISRUPTION, false);
            me->DisappearAndDie();
            Creature* pSaboPort = ObjectAccessor::GetCreature((*me), ObjectGuid(instance->GetGuidData(DATA_SABOTEUR_PORTAL)));
            if (pSaboPort)
...
struct violet_hold_trashAI : public npc_escortAI
{
    ...
    void UpdateAI(uint32)
    {
        ...
        if (!bHasGotMovingPoints)
        {
            ...
            //escort scripted creatures can't start waypoints movement, if they are in combat, will skiped in void npc_escortAI::Start() with an annoying error log out
            if (!UpdateVictim())
                Start(true, true);
        }
    }
    ...
    void JustDied(Unit* /*killer*/)
    {
        if (instance)
        {
            if (Creature* portal = ObjectAccessor::GetCreature((*me), ObjectGuid(instance->GetGuidData(DATA_TELEPORTATION_PORTAL))))
    ...
    struct npc_azure_stalkerAI : public violet_hold_trashAI
    {
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            else
            {
                ...
                {
                    Unit* target = SelectTarget(SELECT_TARGET_MINDISTANCE, 0, 10, true);
