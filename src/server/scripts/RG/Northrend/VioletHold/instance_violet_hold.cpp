...
public:
    ...
    struct instance_violet_hold_InstanceMapScript : public InstanceScript
    {
        ...
        std::set<ObjectGuid> trashMobs; // to kill with crystal
...
/*
        ...
        void StartBossEncounter(uint8 uiBoss, bool bForceRespawn = true)
        {
            ...
            switch (uiBoss)
            {
                ...
                case BOSS_EREKEM:
                    ...
                    {
                        if (bForceRespawn)
                            pGuard1->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        else
                            pGuard1->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        pGuard1->SetImmuneToPC(bForceRespawn);
                    ...
                    {
                        if (bForceRespawn)
                            pGuard2->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        else
                            pGuard2->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        pGuard2->SetImmuneToPC(bForceRespawn);
        ...
        std::string GetSaveData()
        {
            ...
            saveStream << "V H " << (uint32)m_auiEncounter[0]
                ...
                if (uiCyanigosaEventTimer <= diff)
                {
                    switch (uiCyanigosaEventPhase)
                    {
                        case 1:
                            ...
                            break;
                        case 2:
                            pCyanigosa->GetMotionMaster()->MoveJump(MiddleRoomLocation, 10.0f, 20.0f);
                            pCyanigosa->CastSpell(pCyanigosa, CYANIGOSA_BLUE_AURA, false);
                            uiCyanigosaEventTimer = 7*IN_MILLISECONDS;
                            ++uiCyanigosaEventPhase;
                            break;
                        case 3:
                            ...
                            pCyanigosa->SetImmuneToPC(false);
        ...
        void ActivateCrystal()
        {
            ...
            std::set<ObjectGuid> trashMobCopy(trashMobs);
            for (std::set<ObjectGuid>::const_iterator itr = trashMobCopy.begin(); itr != trashMobCopy.end(); ++itr)
            {
                Creature* creature = instance->GetCreature(*itr);
                if (creature && creature->IsAlive())
                    trigger->DealDamage(creature, MAX_DAMAGE, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
            }
        }
...
public:
    ...
    struct npc_violet_hold_arcane_sphereAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            DoCastSelf(SPELL_ARCANE_SPHERE_PASSIVE, true);
        }
