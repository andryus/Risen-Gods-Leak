...
class boss_king_dred : public CreatureScript
{
    public:
        ...
        {
            boss_king_dredAI(Creature* creature) : BossAI(creature, DATA_DRED) { }
...
class npc_drakkari_gutripper : public CreatureScript
{
    public:
        ...
        struct npc_drakkari_gutripperAI : public ScriptedAI
        {
            ...
            {
                if (Creature* Dred = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_DRED))))
...
class spell_grievous_bite : public SpellScriptLoader
{
    public:
        spell_grievous_bite() : SpellScriptLoader("spell_grievous_bite") { }

        class spell_grievous_biteAuraScript : public AuraScript
        {
            PrepareAuraScript(spell_grievous_biteAuraScript);

            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/) 
            {
                if (Unit* target = GetTarget())
                    if (target->GetHealthPct() == 100.0f)
                        target->RemoveAura(SPELL_GRIEVOUS_BITE);
            }

            void Register() override
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_grievous_biteAuraScript::HandleEffectPeriodic, EFFECT_1, SPELL_AURA_PERIODIC_DAMAGE);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_grievous_biteAuraScript();
        }
};
