    ...
    public:
        ...
        struct boss_trollgoreAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_CONSUME,                   15 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CRUSH,            urand(1, 5) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_INFECTED_WOUND, urand(10, 60) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CORPSE_EXPLODE,             3 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SPAWN,          urand(30, 40) * IN_MILLISECONDS);
            }
            ...
            void ExecuteEvent(uint32 eventId) override
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CRUSH:
                        ...
                        events.ScheduleEvent(EVENT_CRUSH, urand(1, 15) * IN_MILLISECONDS);
                        break;
                    case EVENT_INFECTED_WOUND:
                        DoCastVictim(SPELL_INFECTED_WOUND);
                        events.ScheduleEvent(EVENT_INFECTED_WOUND, urand(25, 35) * IN_MILLISECONDS);
                        break;
                    case EVENT_CORPSE_EXPLODE:
                        if (me->FindNearestCreature(NPC_DRAKKARI_INVADER_A, 10.0f, false)
                            || me->FindNearestCreature(NPC_DRAKKARI_INVADER_B, 10.0f, false)
                            || me->FindNearestCreature(NPC_DRAKKARI_INVADER_C, 10.0f, false))
                        {
                            Talk(SAY_EXPLODE);
                            DoCastAOE(SPELL_CORPSE_EXPLODE);
                        }
                        events.ScheduleEvent(EVENT_CORPSE_EXPLODE, urand(15, 19) * IN_MILLISECONDS);
                        break;
                    case EVENT_SPAWN:
                        for (uint8 i = 0; i < 3; ++i)
                            if (Creature* trigger = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_TROLLGORE_INVADER_SUMMONER_1 + i))))
            ...
            void HandleConsume(SpellEffIndex /*effIndex*/)
            {
                if (Unit* target = GetHitUnit())
                    if (InstanceScript* instance = target->GetInstanceScript())
                        if (Creature* trollgore = ObjectAccessor::GetCreature(*GetCaster(), ObjectGuid(instance->GetGuidData(DATA_TROLLGORE))))
                            if (trollgore->HasInArc(M_PI_F, target))
                                target->CastSpell(trollgore, SPELL_CONSUME_BUFF, true);
            }
