...
public:
    ...
    struct instance_utgarde_keep_InstanceMapScript : public InstanceScript
    {
        ...
        void SetData(uint32 type, uint32 data)
        {
            ...
            case DATA_FORGE_1:
                ...
                }else
                {
                    ...
                    if (Creature* c = instance->GetCreature(ForgeMaster[0]))
                        c->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_ATTACKABLE_1);
                }
                forge_event[0] = data;
                break;
            case DATA_FORGE_2:
                ...
                }else
                {
                    ...
                    if (Creature* c = instance->GetCreature(ForgeMaster[1]))
                        c->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_ATTACKABLE_1);
                }
                forge_event[1] = data;
                break;
            case DATA_FORGE_3:
                ...
                }else
                {
                    ...
                    //duble third forge master to prevent non attackable bug after server crash
                    if (Creature* c = instance->GetCreature(ForgeMaster[2]))
                        c->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_ATTACKABLE_1);
                }
                forge_event[2] = data;
                break;
        ...
        void SetGuidData(uint32 type, ObjectGuid value) override
        {
            switch (type)
            {
                case DATA_FORGE_MASTER_1:
                    ForgeMaster[0] = value;
                    break;
                case DATA_FORGE_MASTER_2:
                    ForgeMaster[1] = value;
                    break;
                case DATA_FORGE_MASTER_3:
                    ForgeMaster[2] = value;
                    break;
            }
        }
        ...
        uint32 GetData(uint32 type) const
        {
            switch (type)
            {
                ...
                case DATA_FORGE_1:                  return forge_event[0];
                case DATA_FORGE_2:                  return forge_event[1];
                case DATA_FORGE_3:                  return forge_event[2];
            }
