...
Position const SkeletonLoc[4] =
{
    { 154.013f, 257.246f, 42.8668f, 5.5403f }, 
    { 156.968f, 260.429f, 42.8668f, 5.5403f }, 
    { 154.784f, 265.174f, 42.8668f, 5.5403f }, 
    { 151.348f, 261.299f, 42.8668f, 5.5403f } 
};
...
public:
    ...
    struct npc_frost_tombAI : public ScriptedAI
    {
        ...
        {
            if (instance)
                if (Unit* boss = ObjectAccessor::GetUnit(*me, ObjectGuid(instance->GetGuidData(DATA_PRINCEKELESETH))))
...
public:
    ...
    struct boss_kelesethAI : public BossAI
    {
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FROST_TOMB:
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true, true, -SPELL_FROST_TOMB))
                    {
                        Talk(SAY_FROST_TOMB);
        ...
        void SummonSkeletons()
        {
            ...
            for (int i = 0; i < 4; ++i)
                me->SummonCreature(NPC_SKELETON, SkeletonLoc[i]);
        }
    private:
        bool onTheRocks;
    };
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_DECREPIFY:
                        DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true, true, -SPELL_DECREPIFY), SPELL_DECREPIFY);
                    ...
                    case EVENT_SHADOW_FISSURE:
                        DoCastSelf(SPELL_SHADOW_FISSURE, true);
