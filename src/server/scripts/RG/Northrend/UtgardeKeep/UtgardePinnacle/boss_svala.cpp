        ...
        struct boss_svalaAI : public BossAI
        {
            ...
            void MoveInLineOfSight(Unit* who) override
            {
                ...
                if (events.IsInPhase(IDLE) && me->IsValidAttackTarget(who) && me->IsWithinDistInMap(who, 40))
                {
                    ...
                    if (GameObject* mirror = me->FindNearestGameObject(OBJECT_UTGARDE_MIRROR, 100.0f))
            ...
            void SpellHitTarget(Unit* /*target*/, SpellInfo const* spellInfo) override
            {
                ...
                {
                    events.SetPhase(NORMAL);
                    events.ScheduleEvent(EVENT_SINISTER_STRIKE, Seconds(7), 0, NORMAL);
                    events.ScheduleEvent(EVENT_CALL_FLAMES, Seconds(urand(10, 20)), 0, NORMAL);
                    SetCombatMovement(true);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_INTRO_SVALA_TALK_0:
                            ...
                            events.ScheduleEvent(EVENT_INTRO_ARTHAS_TALK_0, Seconds(8), 0, INTRO);
                            break;
                        case EVENT_INTRO_ARTHAS_TALK_0:
                            ...
                            events.ScheduleEvent(EVENT_INTRO_TRANSFORM_0, Seconds(10), 0, INTRO);
                            break;
                        case EVENT_INTRO_TRANSFORM_0:
                        {
                            ...
                            me->GetCreatureListWithEntryInGrid(lspectatorList, NPC_SPECTATOR, 100.0f);
                            for (std::list<Creature*>::iterator itr = lspectatorList.begin(); itr != lspectatorList.end(); ++itr)
                            {
                                if ((*itr)->IsAlive())
                                {
                                    (*itr)->SetStandState(UNIT_STAND_STATE_STAND);
                                    (*itr)->GetMotionMaster()->MovePath(WAYPOINT_SPECTATOR, false);
                                }
                            }
                        ...
                        case EVENT_INTRO_TRANSFORM_2:
                            ...
                            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                            events.ScheduleEvent(EVENT_INTRO_SVALA_TALK_1, Seconds(10), 0, INTRO);
                            break;
                        case EVENT_INTRO_SVALA_TALK_1:
                            Talk(SAY_SVALA_INTRO_1);
                            events.ScheduleEvent(EVENT_INTRO_ARTHAS_TALK_1, Seconds(10), 0, INTRO);
                            break;
                        case EVENT_INTRO_ARTHAS_TALK_1:
                            if (Creature* arthas = ObjectAccessor::GetCreature(*me, _arthasGUID))
                                arthas->AI()->Talk(SAY_DIALOG_OF_ARTHAS_2);
                            events.ScheduleEvent(EVENT_INTRO_SVALA_TALK_2, Seconds(10), 0, INTRO);
                            break;
                        case EVENT_INTRO_SVALA_TALK_2:
                            ...
                            events.ScheduleEvent(EVENT_INTRO_RELOCATE_SVALA, Seconds(14), 0, INTRO);
                            break;
                        case EVENT_INTRO_RELOCATE_SVALA:
                        {
                            ...
                            me->SetHover(true);
                            events.ScheduleEvent(EVENT_INTRO_DESPAWN_ARTHAS, Seconds(3), 0, INTRO);
                            break;
                        }
                        ...
                        case EVENT_SINISTER_STRIKE:
                            ...
                            events.ScheduleEvent(EVENT_SINISTER_STRIKE, Seconds(urand(5, 9)), 0, NORMAL);
                            break;
                        ...
                        case EVENT_RITUAL_PREPARATION:
                            ...
                            events.ScheduleEvent(EVENT_SPAWN_RITUAL_CHANNELERS, Seconds(1), 0, SACRIFICING);
                            break;
                        case EVENT_SPAWN_RITUAL_CHANNELERS:
                            ...
                            events.ScheduleEvent(EVENT_RITUAL_STRIKE, Seconds(2), 0, SACRIFICING);
                            break;
    ...
    public:
        ...
        struct npc_ritual_channelerAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
                    if (Unit* victim = ObjectAccessor::GetUnit(*me, ObjectGuid(instance->GetGuidData(DATA_SACRIFICED_PLAYER))))
...
class npc_spectator : public CreatureScript
{
    public:
        ...
        struct npc_spectatorAI : public ScriptedAI
        {
            ...
            void Reset() override 
            {
                if (WaypointPath const* path = sWaypointMgr->GetPath(WAYPOINT_SPECTATOR))
                    _lastWP = path->size() - 1;
    ...
    public:
        ...
        bool operator() (WorldObject* obj) const
        {
            if (InstanceScript* instance = obj->GetInstanceScript())
                if (ObjectGuid(instance->GetGuidData(DATA_SACRIFICED_PLAYER)) == obj->GetGUID())
                    return false;
