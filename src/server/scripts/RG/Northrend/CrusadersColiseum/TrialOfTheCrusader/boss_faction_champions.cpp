    ...
    public:
        ...
        struct boss_toc_champion_controllerAI : public ScriptedAI
        {
            boss_toc_champion_controllerAI(Creature* creature) : ScriptedAI(creature), _summons(me)
            {
                ...
                if (_instance)
                    vChampionEntries = SelectChampions(Team(_instance->GetData(TYPE_TEAM_IN_INSTANCE)));
            }
                        ...
                        switch (uiData)
                        {
                            ...
                            case DONE:
                                ...
                                else if (_championsKilled >= _summons.size())
                                {
                                    ...
                                    DoCastSelf(SPELL_DEFEAT_FACTION_CHAMPIONS, true);
...
struct boss_faction_championsAI : public BossAI
{
    ...
    void Reset()
    {
        DoCastSelf(SPELL_ANTI_AOE, true);
    ...
    {
        if (instance)
            if (Creature* pChampionController = ObjectAccessor::GetCreature((*me), ObjectGuid(instance->GetGuidData(NPC_CHAMPIONS_CONTROLLER))))
    ...
    {
        float dist_mod = (_aiType == AI_MELEE || _aiType == AI_PET) ? 30.0f / (15.0f + distance) : 1.0f;
        float armor_mod = (_aiType == AI_MELEE || _aiType == AI_PET) ? armor / 16635.0f : 0.0f;
        float eh = (health + 1) * (1.0f + armor_mod);
        return dist_mod * frand(27000.0f, 32000.0f) / eh;
    }
        ...
        for (std::list<HostileReference*>::const_iterator itr = tList.begin(); itr != tList.end(); ++itr)
        {
            ...
            if (unit && me->GetThreatManager().getThreat(unit))
            {
                ...
                if (unit->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    me->GetThreatManager().doAddThreat(unit, threat*1000000.0f);
                }
            }
        }


        // prevent running creature into the wall
    ...
    bool DoSpellAttackIfReady(uint32 spell)
    {
        ...
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell))
        {
            if (me->IsWithinCombatRange(me->GetVictim(), spellInfo->GetMaxRange(false)))
            {
                ...
                DoStartMovement(me->GetVictim(), (float)RANGE_TO_PLAYER);
        }
    ...
    void KilledUnit(Unit* who)
    {
        if (who->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (!me->GetMap()->GetPlayers().empty())
                TeamInInstance = me->GetMap()->GetPlayers().begin()->GetTeam();

            if (instance)
            {
                if (TeamInInstance == ALLIANCE)
                {
                    if (Creature* temp = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(NPC_VARIAN))))
                        temp->AI()->Talk(SAY_KILL_PLAYER);
                }
                else
                    if (Creature* temp = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(NPC_GARROSH))))
                        temp->AI()->Talk(SAY_KILL_PLAYER);
            }
        }
    }
    ...
    void AttackStart(Unit* who)
    {
        ...
        if (me->Attack(who, true))
        {
            ...
            if (_aiType == AI_MELEE || _aiType == AI_PET)
                ...
                DoStartMovement(who, (float)RANGE_TO_PLAYER);
    ...
    struct DispelableAuraCheck
    {
        DispelableAuraCheck(Unit* caster, DispelType type) : _caster(caster) 
        {
        ...
        bool operator()(Unit* unit)
        {
            ...
            unit->GetDispellableAuraList(_caster, dispelMask, dispelList);
    ...
    Unit* SelectDispelableTarget(float rangef, float rangeh)
    {
        std::list<Unit*> units;
        Trinity::AnyFriendlyUnitInObjectRangeCheck f_check(me, me, rangef);
        me->VisitAnyNearbyObject<Unit, Trinity::ContainerAction>(rangef, units, f_check);

        Trinity::AnyUnfriendlyNoTotemUnitInObjectRangeCheck h_check(me, me, rangeh);
        me->VisitAnyNearbyObject<Unit, Trinity::ContainerAction>(rangeh, units, h_check);

        units.remove_if(DispelableAuraCheck(me, DISPEL_MAGIC));

        if (units.empty())
            return NULL;

        return Trinity::Containers::SelectRandomContainerElement(units);
    }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_HEAL_DISPEL:
                            if (Unit* target = SelectDispelableTarget(40.0f, 30.0f))
                        ...
                        case EVENT_SHATTERING_THROW:
                            {
                                ...
                                {
                                    if (player.HasAuraWithMechanic(1 << MECHANIC_IMMUNE_SHIELD) && player.GetDistance2d(me->GetPositionX(), me->GetPositionY()) <= 30.0f)
                                    {
                                        DoCast(&player, SPELL_SHATTERING_THROW);
    ...
    public:
        ...
        struct npc_toc_pet_warlockAI : public boss_faction_championsAI
        {
            ...
            void Reset()
            {
                ...
                me->SetMaxHealth(RAID_MODE(126000, 441000, 126000, 441000));
    ...
    public:
        ...
        struct npc_toc_pet_hunterAI : public boss_faction_championsAI
        {
            ...
            void Reset()
            {
                ...
                me->SetMaxHealth(RAID_MODE(53420, 186970, 53420, 186970));
                me->SetHealth(me->GetMaxHealth());
            }
