...
enum BossSpells
{
    ...
    SPELL_ACID_MANDIBLE_DOT     = 65775,

    // Burrower
    SPELL_SPIDER_FRENZY_PREIODIC= 66128,
    SPELL_SPIDER_FRENZY_AURA    = 66129,
...
#define SPELL_PERMAFROST_HELPER RAID_MODE<uint32>(SPELL_PERMAFROST, SPELL_PERMAFROST_N25, SPELL_PERMAFROST_H10, SPELL_PERMAFROST_H25)

#define GROUND_Z    142.26f
    ...
    public:
        ...
        struct boss_anubarak_trialAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                events.ScheduleEvent(EVENT_FREEZE_SLASH, urand(21, 24)*IN_MILLISECONDS, 0, PHASE_MELEE);
                events.ScheduleEvent(EVENT_PENETRATING_COLD, 25*IN_MILLISECONDS, PHASE_MELEE);
            ...
            void EnterCombat(Unit* who)
            {
                ...
                Talk(SAY_AGGRO);
                me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
                me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_PERSISTENT_AREA_AURA, false);
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUBMERGE:
                            ...
                            {
                                me->RemoveAllAuras();
                                DoCastSelf(SPELL_SUBMERGE_ANUBARAK);
                                ...
                                events.ScheduleEvent(EVENT_EMERGE, 65*IN_MILLISECONDS, 0, PHASE_SUBMERGED);
                            }
                            break;
                        ...
                        case EVENT_EMERGE:
                            ...
                            me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
                            DoCastSelf(SPELL_EMERGE_ANUBARAK);
                            Talk(SAY_EMERGE);
                            events.SetPhase(PHASE_MELEE);
                            events.ScheduleEvent(EVENT_FREEZE_SLASH, urand(21, 24)*IN_MILLISECONDS, 0, PHASE_MELEE);
                            events.ScheduleEvent(EVENT_PENETRATING_COLD, 20*IN_MILLISECONDS, PHASE_MELEE);
                            events.ScheduleEvent(EVENT_SUMMON_NERUBIAN, 5*IN_MILLISECONDS, 0, PHASE_MELEE);
                            events.ScheduleEvent(EVENT_SUBMERGE, 75*IN_MILLISECONDS, 0, PHASE_MELEE);
                            if (IsHeroic())
                                events.ScheduleEvent(EVENT_NERUBIAN_SHADOW_STRIKE, 30*IN_MILLISECONDS, 0, PHASE_MELEE);
                            return;
                ...
                if (!_reachedPhase3 && HealthBelowPct(30) && events.IsInPhase(PHASE_MELEE) && !me->HasAura(SPELL_EMERGE_ANUBARAK))
                {
                    _reachedPhase3 = true;
                    DoCastAOE(SPELL_LEECHING_SWARM);
                    Talk(EMOTE_LEECHING_SWARM);
                    Talk(SAY_LEECHING_SWARM);
                    events.RescheduleEvent(EVENT_PENETRATING_COLD, urand(15, 25), 0, PHASE_MELEE);
                }
    ...
    public:
        ...
        struct npc_swarm_scarabAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                DoCastSelf(SPELL_ACID_MANDIBLE);
                me->SetInCombatWithZone();
                if (me->IsInCombat())
                    if (Creature* Anubarak = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(NPC_ANUBARAK))))
    ...
    public:
        ...
        struct npc_nerubian_burrowerAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                DoCastSelf(SPELL_EXPOSE_WEAKNESS);
                DoCastSelf(SPELL_SPIDER_FRENZY_PREIODIC);
                DoCastSelf(SPELL_AWAKENED);
                me->SetInCombatWithZone();
                if (me->IsInCombat())
                    if (Creature* Anubarak = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(NPC_ANUBARAK))))
                        Anubarak->AI()->JustSummoned(me);
                ...
                if (HealthBelowPct(80))
                {
                    if (_submergeTimer <= diff)
                    {
                        if (me->HasAura(SPELL_SUBMERGE_EFFECT))
                        {
                            ...
                            DoCastSelf(SPELL_EMERGE_EFFECT);
                            DoCastSelf(SPELL_AWAKENED);
        ...
        struct npc_frost_sphereAI : public ScriptedAI
        {
            ...
            void DamageTaken(Unit* /*who*/, uint32& damage)
            {
                if (me->GetHealth() <= damage)
                {
                    ...
                    if (fabs(me->GetPositionZ() - floorZ) < 0.5f)
                    {
    ...
    public:
        ...
        struct npc_anubarak_spikeAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                {
                    if (!me->IsValidAttackTarget(target) && !target->HasAura(10278))
                    {
                        if (Unit* newTarget = SelectTarget(SELECT_TARGET_MINDISTANCE, 0.0f, 0.0f, true))
                        {
                            EnterCombat(newTarget);
                            return;
                        }
                    }
                    else
                    ...
                    if (_phaseSwitchTimer <= diff)
                    {
                        switch (_phase)
                        {
                            case PHASE_NO_MOVEMENT:
                                ...
                                DoCastSelf(SPELL_SPIKE_SPEED1);
                                DoCastSelf(SPELL_SPIKE_TRAIL);
                            ...
                            case PHASE_IMPALE_NORMAL:
                                DoCastSelf(SPELL_SPIKE_SPEED2);
                                _phase = PHASE_IMPALE_MIDDLE;
                                _phaseSwitchTimer = 7*IN_MILLISECONDS;
                                return;
                            case PHASE_IMPALE_MIDDLE:
                                DoCastSelf(SPELL_SPIKE_SPEED3);
            ...
            void MoveInLineOfSight(Unit* pWho)
            {
                ...
                if (me->IsWithinDist(pWho, 7.0f))
                {
                    ...
                    DoCastSelf(SPELL_SPIKE_FAIL, true);
            ...
            void StartChase(Unit* who)
            {
                ...
                me->SetInCombatWithZone();
                who->GetPosition(&pos);
                who->GetMotionMaster()->MovePoint(1, pos, false);
...
class spell_anubarak_leeching_swarm : public SpellScriptLoader
{
    public:
        ...
        class spell_anubarak_leeching_swarm_AuraScript : public AuraScript
        {
            ...
            {
                return sSpellMgr->GetSpellInfo(SPELL_LEECHING_SWARM_DMG);
            }
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
            {
                ...
                if (Unit* target = GetTarget())
                {
                    ...
                    lifeLeeched = std::max(lifeLeeched, 250);
                    // Damage
...
class spell_anubarak_leeching_swarm_damage : public SpellScriptLoader
{
    public:
        spell_anubarak_leeching_swarm_damage() : SpellScriptLoader("spell_anubarak_leeching_swarm_damage") { }

        class spell_anubarak_leeching_swarm_damage_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_anubarak_leeching_swarm_damage_SpellScript);

            bool Validate(SpellInfo const* /*spell*/)
            {
                return sSpellMgr->GetSpellInfo(SPELL_LEECHING_SWARM_HEAL);
            }

            void HandleAfterHit()
            {
                int32 damage = GetHitDamage();
                // Heal is increased by 35%
                ...
                GetCaster()->CastCustomSpell(GetCaster(), SPELL_LEECHING_SWARM_HEAL, &damage, 0, 0, false);
            }

            void Register()
            {
                AfterHit += SpellHitFn(spell_anubarak_leeching_swarm_damage_SpellScript::HandleAfterHit);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_anubarak_leeching_swarm_damage_SpellScript();
        }
};

class spell_anub_burrower_spier_frenzy : public SpellScriptLoader
{
    public:
        spell_anub_burrower_spier_frenzy() : SpellScriptLoader("spell_anub_burrower_spier_frenzy") { }

        class spell_anub_burrower_spier_frenzy_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_anub_burrower_spier_frenzy_AuraScript)

            bool Validate(SpellInfo const* spellEntry)
            {
                return sSpellMgr->GetSpellInfo(SPELL_SPIDER_FRENZY_AURA);
            };
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                std::list<Creature*> BurrowerList;
                target->GetCreatureListWithEntryInGrid(BurrowerList, NPC_BURROWER, 12.0f);
                BurrowerList.remove(target->ToCreature());

                for (std::list<Creature*>::iterator itr = BurrowerList.begin(); itr != BurrowerList.end(); ++itr)
                    if ((*itr)->IsAlive())
                        (*itr)->AddAura(SPELL_SPIDER_FRENZY_AURA, target);
            }

            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_anub_burrower_spier_frenzy_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_anub_burrower_spier_frenzy_AuraScript();
        }
};

class spell_scrab_dot : public SpellScriptLoader
{
    public:
        spell_scrab_dot() : SpellScriptLoader("spell_scrab_dot") { }

        class spell_scrab_dot_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_scrab_dot_AuraScript);

            void HandleEffectProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
            {
                if (Unit* target = eventInfo.GetProcTarget())
                    if (Unit* owner = GetUnitOwner())
                        if (Creature* trigger = owner->SummonCreature(NPC_SCRAB_DOT_CASTER, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 60*IN_MILLISECONDS))
                            trigger->AI()->DoCast(target, SPELL_ACID_MANDIBLE_DOT, true);
            ...
            void Register()
            {
                OnEffectProc += AuraEffectProcFn(spell_scrab_dot_AuraScript::HandleEffectProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_scrab_dot_AuraScript();
        }
};

class spell_anub_burrower_submerge : public SpellScriptLoader
{
public:
    spell_anub_burrower_submerge() : SpellScriptLoader("spell_anub_burrower_submerge") { }

    class spell_anub_burrower_submerge_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_anub_burrower_submerge_SpellScript);

        bool Validate(SpellInfo const* /*spell*/)
        {
            return sSpellMgr->GetSpellInfo(SPELL_SUBMERGE);
        }
        ...
        void HandleAfterHit()
        {
            ...
            {
            case RAID_DIFFICULTY_10MAN_NORMAL:
                aura = SPELL_PERMAFROST; 
                break;
            case RAID_DIFFICULTY_25MAN_NORMAL:
                aura = SPELL_PERMAFROST_N25;
                break;
            case RAID_DIFFICULTY_10MAN_HEROIC:
                aura = SPELL_PERMAFROST_H10;
                break;
            case RAID_DIFFICULTY_25MAN_HEROIC:
                aura = SPELL_PERMAFROST_H25;
                break;
            }

            if (aura && !GetCaster()->HasAura(aura))
            {
                GetCaster()->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                GetCaster()->CastSpell(GetCaster(), SPELL_SUBMERGE_EFFECT, false);
                GetCaster()->CastSpell(GetCaster(), SPELL_PERSISTENT_DIRT, true);
            }            
        }

        void Register()
        {
            AfterHit += SpellHitFn(spell_anub_burrower_submerge_SpellScript::HandleAfterHit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_anub_burrower_submerge_SpellScript();
    }
};
...
void AddSC_boss_anubarak_trial()
{
    ...
    new spell_anubarak_leeching_swarm_damage();

    new spell_anub_burrower_spier_frenzy();
    new spell_scrab_dot();
    new spell_anub_burrower_submerge();
}
