...
struct IsNotTankTargetSelector
{
    ...
    {
        return (target->GetTypeId() == TYPEID_PLAYER && !target->ToPlayer()->IsTank());
    }
};
...
Position const triggerEndPos = {4680.180f, 2769.150f, 392.5387f, 3.121708f};
    ...
    public:
        ...
        struct boss_blood_council_controllerAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who)
            {
                ...
                if (Creature* keleseth = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_PRINCE_KELESETH_GUID))))
                {
                    instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, keleseth);
                    DoZoneInCombat(keleseth);
                    keleseth->ClearUnitState(UNIT_STATE_EVADE);
                }

                if (Creature* taldaram = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID))))
                {
                    instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, taldaram);
                    DoZoneInCombat(taldaram);
                    taldaram->ClearUnitState(UNIT_STATE_EVADE);
                }

                if (Creature* valanar = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_PRINCE_VALANAR_GUID))))
                {
                    ...
                    DoCastSelf(SPELL_INVOCATION_OF_BLOOD_VALANAR);
                    valanar->ClearUnitState(UNIT_STATE_EVADE);
                }

                events.ScheduleEvent(EVENT_INVOCATION_OF_BLOOD, 46500);

                _invocationOrder[0] = InvocationData(ObjectGuid(instance->GetGuidData(DATA_PRINCE_VALANAR_GUID)), SPELL_INVOCATION_OF_BLOOD_VALANAR, EMOTE_VALANAR_INVOCATION, 71070);
                if (urand(0, 1))
                {
                    _invocationOrder[1] = InvocationData(ObjectGuid(instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID)), SPELL_INVOCATION_OF_BLOOD_TALDARAM, EMOTE_TALDARAM_INVOCATION, 71081);
                    _invocationOrder[2] = InvocationData(ObjectGuid(instance->GetGuidData(DATA_PRINCE_KELESETH_GUID)), SPELL_INVOCATION_OF_BLOOD_KELESETH, EMOTE_KELESETH_INVOCATION, 71080);
                }
                else
                {
                    _invocationOrder[1] = InvocationData(ObjectGuid(instance->GetGuidData(DATA_PRINCE_KELESETH_GUID)), SPELL_INVOCATION_OF_BLOOD_KELESETH, EMOTE_KELESETH_INVOCATION, 71080);
                    _invocationOrder[2] = InvocationData(ObjectGuid(instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID)), SPELL_INVOCATION_OF_BLOOD_TALDARAM, EMOTE_TALDARAM_INVOCATION, 71081);
                }
            }
...
class boss_prince_keleseth_icc : public CreatureScript
{
    public:
        ...
        struct boss_prince_kelesethAI : public BossAI
        {
            ...
            void EnterCombat(Unit* /*who*/)
            {
                if (Creature* controller = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_BLOOD_PRINCES_CONTROL))))
                    controller->SetInCombatWithZone();

                events.ScheduleEvent(EVENT_BERSERK, 600000);
                events.ScheduleEvent(EVENT_SHADOW_RESONANCE, urand(9, 12) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SHADOW_LANCE, 2000);

                if (IsHeroic())
                {
                    me->AddAura(SPELL_SHADOW_PRISON, me);
                    DoCastSelf(SPELL_SHADOW_PRISON_DUMMY, true);
                }
            }
            ...
            void JustSummoned(Creature* summon)
            {
                ...
                pos = me->MovePositionToFirstCollision2D(pos, maxRange, angle + me->GetOrientation());
            ...
            bool ChaseIfNecessary(uint32 spell)
            {
                if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell))
                {
                    if (me->IsWithinCombatRange(me->GetVictim(), spellInfo->GetMaxRange(false)))
                        ...
                        DoStartMovement(me->GetVictim(), 25.0f);
                }
                return true;
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SHADOW_LANCE:
                            if (_isEmpowered)
                            {
                                if (!ChaseIfNecessary(SPELL_EMPOWERED_SHADOW_LANCE))
                                    DoCastVictim(SPELL_EMPOWERED_SHADOW_LANCE);
                            }
                            else
                            {
                                if (!ChaseIfNecessary(SPELL_SHADOW_LANCE))
        ...
        struct boss_prince_taldaramAI : public BossAI
        {
            ...
            void JustSummoned(Creature* summon)
            {
                ...
                if (summon->GetEntry() == NPC_BALL_OF_INFERNO_FLAME || summon->GetEntry() == NPC_BALL_OF_FLAME)
                {
                    ...
                    target = CheckPlayersInRange(RAID_MODE(4, 9), 15.0f, 100.0f);  // try to select a target wich is farer than 15y 
                }

                if (!target)
                    target = SelectTarget(SELECT_TARGET_RANDOM, 0, IsNotTankTargetSelector()); // too bad for you raiders, its going to boom

                if (summon->GetEntry() == NPC_BALL_OF_INFERNO_FLAME && target)
                    Talk(EMOTE_TALDARAM_FLAME, target);

                if (summon->GetEntry() == NPC_BALL_OF_FLAME && target)
                    Talk(EMOTE_TALDARAM_FLAME, target);

                if (target)
                    summon->AI()->SetGuidData(target->GetGUID());
        ...
        struct boss_prince_valanarAI : public BossAI
        {
            ...
            void JustSummoned(Creature* summon)
            {
                switch (summon->GetEntry())
                {
                    ...
                    {
                        summon->SetSpeedRate(MOVE_WALK, IsHeroic() ? 0.3f : 0.15f);
                        summon->SetSpeedRate(MOVE_RUN, IsHeroic() ? 0.3f : 0.15f);
                        summon->SetSpeedRate(MOVE_FLIGHT, IsHeroic() ? 0.3f : 0.15f);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_KINETIC_BOMB:
                            ...
                            events.ScheduleEvent(EVENT_KINETIC_BOMB, Is25ManRaid() ? 20 * IN_MILLISECONDS : 30 * IN_MILLISECONDS);
                            break;
        ...
        struct npc_blood_queen_lana_thelAI : public ScriptedAI
        {
            ...
            void DoAction(int32 action)
            {
                ...
                case ACTION_START_BOSS_EVENT:
                    ...
                    if (Creature* summon = DoSummon(NPC_FLOATING_TRIGGER, triggerPos, 15000, TEMPSUMMON_TIMED_DESPAWN))
                    {
                        ...
                        summon->SetSpeedRate(MOVE_FLIGHT, 0.15f);
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                if (_events.ExecuteEvent() == EVENT_INTRO_1)
                {
                    ...
                    if (Creature* keleseth = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_PRINCE_KELESETH_GUID))))
                        keleseth->AI()->DoAction(ACTION_STAND_UP);

                    if (Creature* taldaram = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID))))
                        taldaram->AI()->DoAction(ACTION_STAND_UP);

                    if (Creature* valanar = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_PRINCE_VALANAR_GUID))))
                    {
...
public:
    ...
    struct npc_kinetic_bombAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_CHECK_AURA, 45*IN_MILLISECONDS);
            ...
            me->ApplySpellImmune(0, IMMUNITY_ID, 50796, true);  // Chaos Bolt
            me->ApplySpellImmune(0, IMMUNITY_ID, 59170, true);  // Chaos Bolt
            me->ApplySpellImmune(0, IMMUNITY_ID, 59171, true);  // Chaos Bolt
            me->ApplySpellImmune(0, IMMUNITY_ID, 59172, true);  // Chaos Bolt
        }
        ...
        void DoAction(int32 action)
        {
            switch(action)
            {
                ...
                case ACTION_KINETIC_BOMB_JUMP:
                {
                    ...
                    float positionZ = (me->GetPositionZ() + (IsHeroic() ? 12.0f : 7.0f)) > spawnZ ? spawnZ : (me->GetPositionZ() + (IsHeroic() ? 12.0f : 7.0f));
                    me->GetMotionMaster()->MovePoint(ACTION_KINETIC_BOMB_JUMP, me->GetPositionX(), me->GetPositionY(), positionZ + (IsHeroic() ? 5.0f : 0.0f));
                    break; 
                }
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            if (type == POINT_MOTION_TYPE && id == ACTION_KINETIC_BOMB_JUMP)
                events.ScheduleEvent(EVENT_CONTINUE_FALLING, 500);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CHECK_AURA:
                        ...
                        events.ScheduleEvent(EVENT_CHECK_AURA, 1*IN_MILLISECONDS);
                        break;
                    default:
                        break;
                }
    ...
    public:
        ...
        struct npc_dark_nucleusAI : public ScriptedAI
        {
            npc_dark_nucleusAI(Creature* creature) : ScriptedAI(creature)
            {
                _lockedTarget = false;
                _targetAuraCheck = 0;
                me->GetMotionMaster()->MovePoint(0, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 1);
                me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_DECREASE_SPEED, true);
                me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_CONFUSE, true); 
                // starfall
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                if (_targetAuraCheck <= diff)
                {
                    ...
                    {
                        if (me->GetDistance(victim) < 100.0f &&
...
class npc_shock_vortex : public CreatureScript
{
    ...
    struct npc_shock_vortexAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            events.ScheduleEvent(EVENT_SHOCK_VORTEX_AURA, 6*IN_MILLISECONDS);
        }
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            {
                case EVENT_SHOCK_VORTEX_AURA:
                    DoCastSelf(SPELL_SHOCK_VORTEX_PERIODIC, true);
                    break;
                default:
                    break;
            }
        ...
        class spell_flame_ball_visual_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (GetSpellInfo()->Id == SPELL_FLAME_SPHERE_SPAWN_EFFECT)
                {
                    ...
                    if (GetCaster()->GetEntry() == NPC_BALL_OF_INFERNO_FLAME)
                        GetCaster()->CastSpell(GetCaster(), SPELL_BALL_OF_FLAMES_PERIODIC, true);
                }
        ...
        class spell_taldaram_ball_of_inferno_flame_SpellScript : public SpellScript
        {
            ...
            void ModAuraStack()
            {
                if (Aura* aur = GetHitAura())
                    aur->SetStackAmount(uint32(GetCaster()->GetMap()->Is25ManRaid() ? 20 : 5));
            }
        ...
        class spell_valanar_kinetic_bomb_SpellScript : public SpellScript
        {
            ...
            void ChangeSummonPos(SpellEffIndex /*effIndex*/)
            {
                ...
                if (GetCaster()->GetMap()->IsHeroic())
                    offset = { 0.0f, 0.0f, 15.0f, 0.0f };
                else
                    offset = { 0.0f, 0.0f, 20.0f, 0.0f };
...
class at_blood_prince_start_event : public OnlyOnceAreaTriggerScript
{
public:
    at_blood_prince_start_event() : OnlyOnceAreaTriggerScript("at_blood_prince_start_event") { }

    bool _OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/)
    {
        InstanceScript* instance = player->GetInstanceScript();
        if (!instance || instance->GetBossState(DATA_BLOOD_PRINCE_COUNCIL) != NOT_STARTED)
            return true;

        if (Creature* creature = ObjectAccessor::GetCreature(*player, ObjectGuid(instance->GetGuidData(DATA_BLOOD_QUEEN_LANATHEL_TRIGGER))))
            creature->AI()->DoAction(ACTION_START_BOSS_EVENT);
...
void AddSC_boss_blood_prince_council()
{
    ...
    new at_blood_prince_start_event();
}
