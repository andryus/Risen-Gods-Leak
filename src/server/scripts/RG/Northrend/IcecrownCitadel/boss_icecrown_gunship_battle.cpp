...
Position const SkybreakerAddsSpawnPos = { 15.91131f, 0.0f, 20.4628f, static_cast<float>(M_PI) };
Position const OrgrimsHammerAddsSpawnPos = { 60.728395f, 0.0f, 38.93467f, static_cast<float>(M_PI) };
...
public:
    ...
    void SetTransport(MotionTransport* transport) { _transport = transport; }
...
private:
    ...
    {
        Position newPos;
        float angle = frand(-M_PI_F * 0.5f, M_PI_F * 0.5f);
...
public:
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        if (GameObject* go = ObjectAccessor::GetGameObject(*_caster, _otherTransport))
...
struct gunship_npc_AI : public ScriptedAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        if (pointId == EVENT_CHARGE_PREPATH && Slot)
        {
            ...
            DoCastSelf(SPELL_BATTLE_EXPERIENCE, true);
    ...
    public:
        ...
        struct npc_gunshipAI : public NullCreatureAI
        {
            ...
            void JustDied(Unit* /*killer*/) override
            {
                ...
                me->GetCreatureListWithEntryInGrid(creatures, NPC_MARTYR_STALKER_IGB_SAURFANG, SIZE_OF_GRIDS);
                ...
                creatures.clear();
                me->GetCreatureListWithEntryInGrid(creatures, NPC_GUNSHIP_HULL, 200.0f);
                for (std::list<Creature*>::iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                {
                    Creature* hull = *itr;
                ...
                creatures.clear();
                me->GetCreatureListWithEntryInGrid(creatures, _teamInInstance == HORDE ? NPC_HORDE_GUNSHIP_CANNON : NPC_ALLIANCE_GUNSHIP_CANNON, 200.0f);
                for (std::list<Creature*>::iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                {
                    Creature* cannon = *itr;
                    if (isVictory)
                ...
                if (isVictory)
                {
                    if (GameObject* go = ObjectAccessor::GetGameObject(*me, instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
                        if (MotionTransport* otherTransport = go->ToMotionTransport())
                            otherTransport->EnableMovement(true);

                    me->GetMotionTransport()->EnableMovement(true);
                    ...
                    creatures.clear();
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_SKYBREAKER_MARINE, 200.0f);
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_SKYBREAKER_SERGEANT, 200.0f);
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_KOR_KRON_REAVER, 200.0f);
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_KOR_KRON_SERGEANT, 200.0f);
                    for (std::list<Creature*>::iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                ...
                else
                {
                    ...
                    if (Unit* enemy = me->FindNearestCreature(_teamInInstance == HORDE ? NPC_THE_SKYBREAKER : NPC_ORGRIMS_HAMMER, 200.0f))
                        RG_LOG<EncounterLogModule>(enemy, EncounterLogModule::Type::WIPE);
                }
            }
            ...
            void SetGuidData(ObjectGuid guid, uint32 id/* = 0*/) override
            {
                ...
                auto itr = _shipVisits.find(guid);
    ...
    public:
        ...
        struct npc_high_overlord_saurfang_igbAI : public ScriptedAI
        {
                ...
                _instance(creature->GetInstanceScript())
            {
                ...
                _controller.SetTransport(creature->GetMotionTransport());
                me->setRegeneratingHealth(false);
            ...
            void EnterCombat(Unit* /*target*/) override
            {
                ...
                DoCastSelf(_instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE ? SPELL_FRIENDLY_BOSS_DAMAGE_MOD : SPELL_MELEE_TARGETING_ON_ORGRIMS_HAMMER, true);
                me->RemoveAurasDueToSpell(SPELL_BATTLE_FURY);
                if (!isOwnBoss)
                {
                    DoCastSelf(SPELL_BATTLE_FURY, true);
            ...
            void DoAction(int32 action) override
            {
                if (action == ACTION_ENEMY_GUNSHIP_TALK)
                {
                    ...
                    if (Creature* skybreaker = me->FindNearestCreature(NPC_THE_SKYBREAKER, 100.0f))
                    {
                        ...
                        RG_LOG<EncounterLogModule>(skybreaker, EncounterLogModule::Type::PULL);
                    }
            ...
            void sGossipSelect(Player* /*player*/, uint32 /*sender*/, uint32 /*action*/) override
            {
                ...
                me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, (((stats->BaseDamage[cinfo->expansion] + (stats->AttackPower / 14)) * cinfo->dmg_multiplier) * (cinfo->baseattacktime / 1000)) * 0.01f);
                me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, ((((stats->BaseDamage[cinfo->expansion] * 1.5) + (stats->AttackPower / 14)) * cinfo->dmg_multiplier) * (cinfo->baseattacktime / 1000)) * 0.01f);
                isOwnBoss = true;
            }
            ...
            void DamageTaken(Unit* , uint32& damage) override
            {
                if (!isOwnBoss && me->HealthBelowPctDamaged(65, damage) && !me->HasAura(SPELL_TASTE_OF_BLOOD))
                    DoCastSelf(SPELL_TASTE_OF_BLOOD, true);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_ADDS:
                            ...
                            if (MotionTransport* orgrimsHammer = me->GetMotionTransport())
                                orgrimsHammer->SummonPassenger(NPC_TELEPORT_PORTAL, OrgrimsHammerTeleportPortal, TEMPSUMMON_TIMED_DESPAWN, NULL, 21000);

                            if (GameObject* go = ObjectAccessor::GetGameObject(*me, _instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
...
class npc_muradin_bronzebeard_igb : public CreatureScript
{
    public:
        ...
        struct npc_muradin_bronzebeard_igbAI : public ScriptedAI
        {
            ...
            {
                _events.SetPhase(PHASE_COMBAT);
                DoCastSelf(_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE ? SPELL_FRIENDLY_BOSS_DAMAGE_MOD : SPELL_MELEE_TARGETING_ON_SKYBREAKER, true);
                me->RemoveAurasDueToSpell(SPELL_BATTLE_FURY);
                if (!isOwnBoss)
                {
                    DoCastSelf(SPELL_BATTLE_FURY, true);
            ...
            void DoAction(int32 action) override
            {
                if (action == ACTION_ENEMY_GUNSHIP_TALK)
                {
                    ...
                    if (Creature* orgrimsHammer = me->FindNearestCreature(NPC_ORGRIMS_HAMMER, 100.0f))
                    {
                        ...
                        RG_LOG<EncounterLogModule>(orgrimsHammer, EncounterLogModule::Type::PULL);
                    }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_ADDS:
                            ...
                            _controller.SummonCreatures(SLOT_MARINE_1, Is25ManRaid() ? SLOT_MARINE_4 : SLOT_MARINE_2);
                            _controller.SummonCreatures(SLOT_SERGEANT_1, Is25ManRaid() ? SLOT_SERGEANT_2 : SLOT_SERGEANT_1);
                            if (MotionTransport* skybreaker = me->GetMotionTransport())
...
struct npc_gunship_boarding_addAI : public gunship_npc_AI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        if (pointId == EVENT_CHARGE_PREPATH && Slot)
        {
            ...
            if (GameObject* go = ObjectAccessor::GetGameObject(*me, Instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
                ...
                {
                    myTransport->RemovePassenger(me);
                    destTransport->CalculatePassengerPosition(x, y, z, &o);
                    destTransport->AddPassenger(me);
                }
            ...
            me->m_Events.AddEvent(new BattleExperienceEvent(me), me->m_Events.CalculateTime(BattleExperienceEvent::ExperiencedTimes[0]));
            DoCastSelf(SPELL_BATTLE_EXPERIENCE, true);
            DoCastSelf(SPELL_TELEPORT_TO_ENEMY_SHIP, true);
            DoCastSelf(Instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE ? SPELL_MELEE_TARGETING_ON_ORGRIMS_HAMMER : SPELL_MELEE_TARGETING_ON_SKYBREAKER, true);
            ...
            me->VisitAnyNearbyObject<Player, Trinity::ContainerAction>(200.0f, players, check);
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage) override
    {
        ...
        DoCastSelf(SPELL_DESPERATE_RESOLVE, true);
    }
...
class npc_gunship_mage : public CreatureScript
{
    public:
        ...
        struct npc_gunship_mageAI : public gunship_npc_AI
        {
            ...
            void JustDied(Unit* killer) override
            {
                // @HACK: Movement Breaks Channeling. We add Aura 'Below Zero' manually to all Cannons and remove it once the casting Mage died
                if (IsCaster)
                {
                    std::list<Creature*> AddList;
                    me->GetCreatureListWithEntryInGrid(AddList, NPC_HORDE_GUNSHIP_CANNON, 1000.0f);
                    me->GetCreatureListWithEntryInGrid(AddList, NPC_ALLIANCE_GUNSHIP_CANNON, 1000.0f);
                    if (!AddList.empty())
                        for (std::list<Creature*>::iterator itr = AddList.begin(); itr != AddList.end(); itr++)
                            (*itr)->RemoveAurasDueToSpell(SPELL_BELOW_ZERO);
                }                

                gunship_npc_AI::JustDied(killer);
            }
            ...
            void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (GetSpellInfo()->Id == uint32(_teamInInstance == HORDE ? SPELL_ON_SKYBREAKER_DECK : SPELL_ON_ORGRIMS_HAMMER_DECK))
                    if (Creature* gunship = GetOwner()->FindNearestCreature(_teamInInstance == HORDE ? NPC_ORGRIMS_HAMMER : NPC_THE_SKYBREAKER, 200.0f))
                        gunship->AI()->SetGuidData(GetTarget()->GetGUID(), ACTION_SHIP_VISITS);
            }
        ...
        class spell_igb_gunship_fall_teleport_SpellScript : public SpellScript
        {
            ...
            void SelectTransport(WorldObject*& target)
            {
                if (InstanceScript* instance = target->GetInstanceScript())
                    target = ObjectAccessor::GetGameObject(*target, instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE));
            }
    ...
    public:
        ...
        class spell_igb_teleport_players_on_victory_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                {
                    return target->GetTransGUID() != ObjectGuid(instance->GetGuidData(DATA_ENEMY_GUNSHIP));
                });
            }
...
class achievement_im_on_a_boat : public AchievementCriteriaScript
{
    public:
        ...
        {
            uint8 count = 1;
            if (target->FindMap() && !target->GetMap()->Is25ManRaid())
                count = 2;

            return target->GetAI() && target->GetAI()->GetData(ACTION_SHIP_VISITS) <= count;
        }
};
