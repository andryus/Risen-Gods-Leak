...
// Remove this code if a more general Auraremove is given.
void DoRemoveAuraOnAllPlayerSaurfang(InstanceScript* instance, uint32 spellId)
{
...
struct IsNotTankTargetSelector
{
    ...
    {
        return (target->GetTypeId() == TYPEID_PLAYER && !target->ToPlayer()->IsTank() && !target->HasAura(SPELL_MARK_OF_THE_FALLEN_CHAMPION) && !target->IsTotem());
    }
};
    ...
    public:
        ...
        struct boss_deathbringer_saurfangAI : public BossAI
        {
            boss_deathbringer_saurfangAI(Creature* creature) : BossAI(creature, DATA_DEATHBRINGER_SAURFANG)
            {
                ...
                if (instance->GetData(DATA_SAURFANG_DONE) == 1)
                {
                    instance->HandleGameObject(ObjectGuid(instance->GetGuidData(GO_SAURFANG_S_DOOR)), true);
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                if (!_introDone)
                {
                    ...
                    if (Creature* creature = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_SAURFANG_EVENT_NPC))))
                ...
                events.ScheduleEvent(EVENT_BERSERK, IsHeroic() ? 360*IN_MILLISECONDS : 480*IN_MILLISECONDS, 0, PHASE_COMBAT);
                events.ScheduleEvent(EVENT_BOILING_BLOOD, 15*IN_MILLISECONDS, 0, PHASE_COMBAT);
                events.ScheduleEvent(EVENT_BLOOD_NOVA, 21*IN_MILLISECONDS, 0, PHASE_COMBAT);
            ...
            void DamageTaken(Unit* attacker, uint32& damage) override
            {
                ...
                if (!_dead && ((int32)me->GetHealth() - (int32)damage) < static_cast<int32>(FightWonValue))
                {
                    _dead = true;
            ...
            void JustSummoned(Creature* summon) override
            {
                ...
                summon->AddAura(SPELL_STUN, summon);
            ...
            void MovementInform(uint32 type, uint32 id) override
            {
                ...
                instance->HandleGameObject(ObjectGuid(instance->GetGuidData(GO_SAURFANG_S_DOOR)), false);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_BLOOD_BEAST:
                            ...
                            if (Is25ManRaid())
                                for (uint32 i25 = 0; i25 < uint32(me->GetMap()->IsHeroic() ? 3 : 2); ++i25)
                        ...
                        case EVENT_SCENT_OF_BLOOD:
                            if (!summons.empty())
                            {
                                ...
                                std::list<Creature*> beasts;
                                me->GetCreatureListWithEntryInGrid(beasts, NPC_BLOOD_BEAST, 150.0f);
                                for (std::list<Creature*>::iterator itr = beasts.begin(); itr != beasts.end(); ++itr)
                                    (*itr)->AI()->DoAction(ACTION_GAIN_SCENT_OF_BLOOD);
                            }
                            break;
                        default:
                            break;
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    ...
                    case PHASE_INTRO_H:
                    {
                        if (GameObject* teleporter = GameObject::GetGameObject(*me, ObjectGuid(instance->GetGuidData(GO_SCOURGE_TRANSPORTER_SAURFANG))))
                        {
                    ...
                    case ACTION_CONTINUE_INTRO:
                    {
                        ...
                        events.ScheduleEvent(EVENT_INTRO_FINISH, 13*IN_MILLISECONDS, 0, PHASE_INTRO_A);
                        ...
                        events.ScheduleEvent(EVENT_INTRO_FINISH, 46700+1000+8000+2000, 0, PHASE_INTRO_H);
                        break;
                    }
                    case ACTION_MARK_OF_THE_FALLEN_CHAMPION:
                    {
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, IsNotTankTargetSelector()))
                        {
...
class npc_high_overlord_saurfang_icc : public CreatureScript
{
    public:
        ...
        struct npc_high_overlord_saurfangAI : public ScriptedAI
        {
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    case ACTION_START_EVENT:
                    {
                        ...
                        me->GetCreatureListWithEntryInGrid(guardList, NPC_SE_KOR_KRON_REAVER, 20.0f);
                        ...
                        _instance->HandleGameObject(ObjectGuid(_instance->GetGuidData(GO_SAURFANG_S_DOOR)), true);
                        if (Creature* deathbringer = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_DEATHBRINGER_SAURFANG))))
                    ...
                    case ACTION_START_OUTRO:
                    {
                        ...
                        //if (Creature* eventTrigger = ObjectAccessor::GetCreature(*me, me->GetInstanceScript()->GetGuidData(NPC_MARTYR_STALKER_IGB_SAURFANG)))
                        //    eventTrigger->AI()->DoAction(ACTION_START_HORDE_EVENT);
            ...
            void MovementInform(uint32 type, uint32 id) override
            {
                ...
                else if (type == WAYPOINT_MOTION_TYPE && id == POINT_EXIT)
                {
                    ...
                    me->GetCreatureListWithEntryInGrid(guards, NPC_KOR_KRON_GENERAL, 50.0f);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INTRO_HORDE_8:
                            ...
                            me->GetMotionMaster()->MovePoint(0, chargePos[0].GetPositionX(), chargePos[0].GetPositionY(), chargePos[0].GetPositionZ());
                            break;
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_START_FIGHT, 0), 631, -ACTION_START_EVENT);
...
class npc_muradin_bronzebeard_icc : public CreatureScript
{
    public:
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    case ACTION_START_EVENT:
                    {
                        ...
                        std::list<Creature*> guardList;
                        me->GetCreatureListWithEntryInGrid(guardList, NPC_SE_SKYBREAKER_MARINE, 20.0f);
                        guardList.sort(Trinity::ObjectDistanceOrderPred(me));
                        uint32 x = 1;
                        for (std::list<Creature*>::iterator itr = guardList.begin(); itr != guardList.end(); ++x, ++itr)
                        {
                    ...
                    {                       
                        ...
                        for (ObjectGuid guid : guard_guids)
                        {
                            if (Creature* guard = sObjectAccessor->GetCreature(*me, guid))
                            {
                                ...
                                guard->ForbidInhabit(INHABIT_AIR);
        ...
        {
            InstanceScript* instance = creature->GetInstanceScript();
            if (instance && instance->GetData(DATA_SAURFANG_DONE) == 0 && creature->GetPositionZ() >= 520.711f)
            {
                player->ADD_GOSSIP_ITEM(0, player->GetOptionTextWithEntry(GOSSIP_START_FIGHT, 1), 631, -ACTION_START_EVENT + 1);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID()); 
            }
        ...
        struct npc_saurfang_eventAI : public ScriptedAI
        {
            ...
            void DoAction(int32 action) override
            {
                if (action == ACTION_CHARGE && _index)
                    me->GetMotionMaster()->MovePoint(0, chargePos[_index].GetPositionX(), chargePos[_index].GetPositionY(), chargePos[_index].GetPositionZ());
...
public:
    ...
    struct npc_blood_beastAI : public ScriptedAI
    {
        ...
        {
            AttackStartTimer = 3 * IN_MILLISECONDS;
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_GAIN_SCENT_OF_BLOOD)
                DoCastSelf(SPELL_SCENT_OF_BLOOD, false);
        }
...
class spell_deathbringer_boiling_blood : public SpellScriptLoader
{
    public:
        ...
        class spell_deathbringer_boiling_blood_AuraScript : public AuraScript
        {
            ...
            {
                return sSpellMgr->GetSpellInfo(SPELL_BLOOD_LINK_DUMMY);
            }
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                if (GetCaster()->GetPower(POWER_ENERGY) != GetCaster()->GetMaxPower(POWER_ENERGY))
                    GetOwner()->ToUnit()->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY, SPELLVALUE_BASE_POINT0, 1, GetCaster(), true);
            }

            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_deathbringer_boiling_blood_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
            }
        };
...
class spell_deathbringer_mark_of_the_fallen_champions : public SpellScriptLoader
{
public:
    spell_deathbringer_mark_of_the_fallen_champions() : SpellScriptLoader("spell_deathbringer_mark_of_the_fallen_champions") { }

    class spell_deathbringer_mark_of_the_fallen_champions_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_deathbringer_mark_of_the_fallen_champions_SpellScript);

        void RecalculateDamage()
        {
            if (Aura *aura = GetCaster()->GetAura(SPELL_BLOOD_POWER))
            {
                int32 dmg = GetHitDamage();
                AddPct(dmg, uint32(100 * GetCaster()->GetPower(GetCaster()->getPowerType()) / GetCaster()->GetMaxPower(GetCaster()->getPowerType())));
                SetHitDamage(dmg);
            }
        }
        ...
        void Register()
        {
            OnHit += SpellHitFn(spell_deathbringer_mark_of_the_fallen_champions_SpellScript::RecalculateDamage);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_deathbringer_mark_of_the_fallen_champions_SpellScript();
    }
};
...
void AddSC_boss_deathbringer_saurfang()
{
    ...
    new spell_deathbringer_mark_of_the_fallen_champions();
}
