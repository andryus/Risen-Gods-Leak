...
enum Sounds
{
    SOUND_INSTRUMENTAL_INTRO    = 17280
};
...
class boss_bronjahm : public CreatureScript
{
    public:
        ...
        struct boss_bronjahmAI : public BossAI
        {
            ...
            {
                DoCastSelf(SPELL_SOULSTORM_CHANNEL, true);
            }
            ...
            void Reset()
            {
                ...
                events.ScheduleEvent(EVENT_MAGIC_BANE, urand(8, 20) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CORRUPT_SOUL, urand(25, 35) * IN_MILLISECONDS, 0, PHASE_1);
                events.ScheduleEvent(EVENT_CHECK_HOME, IN_MILLISECONDS);
           ...
           void JustReachedHome()
           {
            ...
            {
                Talk(SAY_AGGRO);
                DoPlaySoundToSet(me, SOUND_INSTRUMENTAL_INTRO);
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/)
            {
                if (events.IsInPhase(PHASE_1) && !HealthAbovePct(30))
                {
                    ...
                    events.ScheduleEvent(EVENT_FEAR, urand(12, 16)* IN_MILLISECONDS, 0, PHASE_2);
                    events.ScheduleEvent(EVENT_SOULSTORM, 0.1 * IN_MILLISECONDS, 0, PHASE_2);
                }
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SOULSTORM:
                            ...
                            DoCastSelf(SPELL_SOULSTORM, false);
                            break;
                        case EVENT_FEAR:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true))
                                me->CastCustomSpell(SPELL_FEAR, SPELLVALUE_MAX_TARGETS, 1, target, false);
                            events.ScheduleEvent(EVENT_FEAR, urand(8, 12) * IN_MILLISECONDS, 0, PHASE_2);
                            break;
                        case EVENT_CHECK_HOME:
                            if (me->GetDistance(me->GetHomePosition()) > 22.0f)
                                EnterEvadeMode();
                            events.ScheduleEvent(EVENT_CHECK_HOME, IN_MILLISECONDS);
                            break;
                        default:
    ...
    public:
        ...
        struct npc_corrupted_soul_fragmentAI : public ScriptedAI
        {
            ...
            void InitializeAI()
            {
                if (instance)
                {
                    ObjectGuid BronjahmGUID = ObjectGuid(instance->GetGuidData(DATA_BRONJAHM));
