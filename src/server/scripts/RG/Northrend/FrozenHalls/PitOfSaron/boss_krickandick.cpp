...
enum Yells
{
    ...
    SAY_TYRANNUS_OUTRO_7                        = 7,
    SAY_TYRANNUS_OUTRO_9                        = 8
};
...
enum Events
{
    ...
    EVENT_SHADOW_BOLT,
    EVENT_TOXIC_WASTE,
    EVENT_SPECIAL, //special spell selection (one of event 5, 6 or 7)
    EVENT_PURSUIT,
    EVENT_POISON_NOVA,
...
class boss_ick : public CreatureScript
{
    public:
        ...
        struct boss_ickAI : public BossAI
        {
            ...
            Creature* GetKrick()
            {
                return ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_KRICK)));
            }
            ...
            void EnterCombat(Unit* who)
            {
                ...
                events.ScheduleEvent(EVENT_MIGHTY_KICK, Seconds(20));
                events.ScheduleEvent(EVENT_TOXIC_WASTE, Seconds(5));
                events.ScheduleEvent(EVENT_SHADOW_BOLT, Seconds(10));
                events.ScheduleEvent(EVENT_SPECIAL, Seconds(urand(30, 35)));
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SPECIAL:
                            ...
                            events.ScheduleEvent(EVENT_SPECIAL, Seconds(urand(23, 28)));
                            break;
...
class boss_krick : public CreatureScript
{
    public:
        ...
        struct boss_krickAI : public ScriptedAI
        {
            ...
            Creature* GetIck()
            {
                return ObjectAccessor::GetCreature(*me, ObjectGuid(_instanceScript->GetGuidData(DATA_ICK)));
            }
            ...
            void DoAction(int32 actionId)
            {
                ...
                {
                    Creature* tyrannusPtr = ObjectAccessor::GetCreature(*me, ObjectGuid(_instanceScript->GetGuidData(DATA_TYRANNUS_EVENT)));
                    if (tyrannusPtr)
                    ...
                    tyrannusPtr->setActive(true);
                    tyrannusPtr->SetVisible(true);
                }
            }
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                ...
                _phase = PHASE_OUTRO;
                _events.Reset();
                _events.ScheduleEvent(EVENT_OUTRO_1, Seconds(1));
            }
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_OUTRO_1:
                        {
                            if (Creature* temp = ObjectAccessor::GetCreature(*me, ObjectGuid(_instanceScript->GetGuidData(DATA_JAINA_SYLVANAS_1))))
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_2, Seconds(6));
                            break;
                        }
                        case EVENT_OUTRO_2:
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_3, Seconds(5));
                            break;
                        case EVENT_OUTRO_3:
                            Talk(SAY_KRICK_OUTRO_3);
                            _events.ScheduleEvent(EVENT_OUTRO_4, Seconds(18));
                            break;
                        case EVENT_OUTRO_4:
                            if (Creature* jainaOrSylvanas = ObjectAccessor::GetCreature(*me, _outroNpcGUID))
                            {
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_5, Seconds(5));
                            break;
                        case EVENT_OUTRO_5:
                            Talk(SAY_KRICK_OUTRO_5);
                            _events.ScheduleEvent(EVENT_OUTRO_6, Seconds(1));
                            break;
                        case EVENT_OUTRO_6:
                            if (Creature* tyrannus = ObjectAccessor::GetCreature(*me, ObjectGuid(_instanceScript->GetGuidData(DATA_TYRANNUS_EVENT))))
                            {
                                tyrannus->SetSpeedRate(MOVE_FLIGHT, 3.5f);
                                tyrannus->GetMotionMaster()->MovePoint(1, outroPos[4]);
                                _tyrannusGUID = tyrannus->GetGUID();
                            }
                            _events.ScheduleEvent(EVENT_OUTRO_7, Seconds(5));
                            break;
                        case EVENT_OUTRO_7:
                            if (Creature* tyrannus = ObjectAccessor::GetCreature(*me, _tyrannusGUID))
                                tyrannus->AI()->Talk(SAY_TYRANNUS_OUTRO_7);
                            _events.ScheduleEvent(EVENT_OUTRO_8, Seconds(5));
                            break;
                        case EVENT_OUTRO_8:
                            //! HACK: Creature's can't have MOVEMENTFLAG_FLYING
                            me->AddUnitMovementFlag(MOVEMENTFLAG_FLYING);
                            me->GetMotionMaster()->MovePoint(0, outroPos[5]);
                            DoCastSelf(SPELL_STRANGULATING);
                            _events.ScheduleEvent(EVENT_OUTRO_9, Seconds(2));
                            break;
                        case EVENT_OUTRO_9:
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_10, Seconds(1));
                            break;
                        case EVENT_OUTRO_10:
                            //! HACK: Creature's can't have MOVEMENTFLAG_FLYING
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_11, Seconds(2));
                            break;
                        case EVENT_OUTRO_11:
                            DoCastSelf(SPELL_KRICK_KILL_CREDIT); // don't really know if we need it
                            me->SetStandState(UNIT_STAND_STATE_DEAD);
                            me->SetHealth(0);
                            _events.ScheduleEvent(EVENT_OUTRO_12, Seconds(3));
                            break;
                        case EVENT_OUTRO_12:
                            if (Creature* tyrannus = ObjectAccessor::GetCreature(*me, _tyrannusGUID))
                                tyrannus->AI()->Talk(SAY_TYRANNUS_OUTRO_9);
                            _events.ScheduleEvent(EVENT_OUTRO_13, Seconds(2));
                            break;
                        case EVENT_OUTRO_13:
                            if (Creature* jainaOrSylvanas = ObjectAccessor::GetCreature(*me, _outroNpcGUID))
                            {
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_END, Seconds(3));
                            if (Creature* tyrannus = ObjectAccessor::GetCreature(*me, _tyrannusGUID))
                                tyrannus->GetMotionMaster()->MovePoint(0, outroPos[7]);
                            break;
                        case EVENT_OUTRO_END:
                            if (Creature* tyrannus = ObjectAccessor::GetCreature(*me, _tyrannusGUID))
                                tyrannus->SetVisible(false); // no despawn, triggernpc is needed for event near the gaunlet
        ...
        class spell_krick_explosive_barrage_AuraScript : public AuraScript
        {
            ...
            void HandlePeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                if (Unit* caster = GetCaster())
                    ...
                    {
                        for (Player& player : caster->GetMap()->GetPlayers())
