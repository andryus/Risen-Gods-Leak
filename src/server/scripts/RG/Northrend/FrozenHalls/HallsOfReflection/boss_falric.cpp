        ...
        struct boss_falricAI : public boss_horAI
        {
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& damage) override
            {
                    ...
                    || (_hopelessnessCount < 3 && me->HealthBelowPctDamaged(10, damage)))
                {
                    ...
                    DoCastSelf(HopelessnessHelper[_hopelessnessCount]);
                    ++_hopelessnessCount;
                }
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_DEFILING_HORROR:
                    {
                        ...
                        me->GetPlayerListInGrid(players, 60.f);
                        for (std::list<Player*>::const_iterator pitr = players.begin(); pitr != players.end(); ++pitr)
                        {
                            Player* player = *pitr;
                            player->AddAura((IsHeroic() ? SPELL_DEFILING_HORROR_H : SPELL_DEFILING_HORROR), player);
                        }
                        events.ScheduleEvent(EVENT_DEFILING_HORROR, urand(21000, 39000));
                        break;
                    }
