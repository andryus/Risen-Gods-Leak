...
const uint32 MAX_WAVE_COUNT = 10;
    ...
    public:
        ...
        struct instance_halls_of_reflection_InstanceMapScript : public InstanceScript
        {
            instance_halls_of_reflection_InstanceMapScript(Map* map) : InstanceScript(map)
            {
                ...
                JainaOrSylvanasIntroGUID .Clear();
                KorelnOrLoralenGUID      .Clear();
                TheLichkingIntroGUID     .Clear();
                FalricGUID               .Clear();
                MarwynGUID               .Clear();
                FrostmourneAltarBunnyGUID.Clear();
                FrostswornGeneralGUID    .Clear();
                JainaOrSylvanasEscapeGUID.Clear();
                TheLichKingEscapeGUID    .Clear();

                FrostmourneGUID          .Clear();
                EntranceDoorGUID         .Clear();
                ImpenetrableDoorGUID     .Clear();
                ShadowThroneDoorGUID     .Clear();
                CaveInGUID               .Clear();
                GunshipGUID              .Clear();
                CaptainsChestGUID        .Clear();
                CaptainGUID              .Clear();
                IcewallGUID              .Clear();
            ...
            bool SetBossState(uint32 type, EncounterState state) override
            {
                ...
                switch (type)
                {
                    case DATA_FALRIC:
                        if (state == DONE)
                        {
                            _waveCount = (_waveCount + 1) % (MAX_WAVE_COUNT + 1);
            ...
            void SpawnGunship()
            {
                ...
                if (MotionTransport* gunship = sTransportMgr->CreateTransport(_teamInInstance == HORDE ? GO_ORGRIMS_HAMMER : GO_THE_SKYBREAKER, 0, instance))
            ...
            void SetGuidData(uint32 type, ObjectGuid data) override
            {
                switch (type)
                {
                    case DATA_QUEL_DELAR_INVOKER:
                        QuelDelarInvokerGUID = ObjectGuid(data);
                        break;
                    default:
                        break;
                }
                ...
                switch (eventId)
                {
                    ...
                    case EVENT_ADD_WAVE:
                        ...
                        else
                        {
                            ...
                            else if (_waveCount != MAX_WAVE_COUNT)
                            {
                                _waveCount = (_waveCount + 1) % (MAX_WAVE_COUNT + 1);
                                events.ScheduleEvent(EVENT_NEXT_WAVE, 5000);
                            }
                    ...
                    case EVENT_GUNSHIP_ARRIVAL:
                        ...
                        if (MotionTransport* gunship = ObjectAccessor::GetMotionTransport(*instance, GunshipGUID))
        ...
        private:
            ...
            EventMap events;
            std::set<ObjectGuid> waveGuidList[8];
