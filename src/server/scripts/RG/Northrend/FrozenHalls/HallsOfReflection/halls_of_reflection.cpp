...
enum Misc
{
    ...
    SOUND_CHAMPION_KILL_GOTO_LK        = 17282
};
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch (_events.ExecuteEvent())
                {
                    case EVENT_WALK_INTRO1:
                        if (Creature* korelnOrLoralen = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_KORELN_LORALEN))))
                    ...
                    case EVENT_INTRO_A2_3:
                        DoCastSelf(SPELL_CAST_VISUAL, false);
                        DoCastSelf(SPELL_FROSTMOURNE_SOUNDS, true);
                        _instance->HandleGameObject(ObjectGuid(_instance->GetGuidData(DATA_FROSTMOURNE)), true);
                    ...
                    case EVENT_INTRO_LK_4:
                        ...
                        if (Creature* lichking = ObjectAccessor::GetCreature(*me, _lichkingGUID))
                        {
                            if (GameObject* frostmourne = ObjectAccessor::GetGameObject(*me, ObjectGuid(_instance->GetGuidData(DATA_FROSTMOURNE))))
                    ...
                    case EVENT_INTRO_LK_6:
                        ...
                        if (Creature* falric = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_FALRIC))))
                        {
                            falric->CastSpell(falric, SPELL_BOSS_SPAWN_AURA, true);
                            falric->SetVisible(true);
                        }
                        if (Creature* marwyn = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_MARWYN))))
                        {
                    ...
                    case EVENT_OPEN_IMPENETRABLE_DOOR:
                        _instance->HandleGameObject(ObjectGuid(_instance->GetGuidData(DATA_IMPENETRABLE_DOOR)), true);
                        break;
                    case EVENT_CLOSE_IMPENETRABLE_DOOR:
                        _instance->HandleGameObject(ObjectGuid(_instance->GetGuidData(DATA_IMPENETRABLE_DOOR)), false);
                        break;
...
class npc_jaina_or_sylvanas_escape_hor : public CreatureScript
{
    public:
        ...
        struct npc_jaina_or_sylvanas_escape_horAI : public ScriptedAI
        {
            ...
            {
                if (Creature* lichking = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_THE_LICH_KING_ESCAPE))))
            ...
            void DestroyIceWall()
            {
                ...
                _instance->HandleGameObject(ObjectGuid(_instance->GetGuidData(DATA_ICEWALL)), true);
                me->m_Events.AddEvent(new GameObjectDeleteDelayEvent(me, ObjectGuid(_instance->GetGuidData(DATA_ICEWALL))), me->m_Events.CalculateTime(5000));

                if (Creature* wallTarget = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_ICEWALL_TARGET))))
            ...
            void SummonIceWall()
            {
                if (_icewall < 4)
                {
                    if (Creature* lichking = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_THE_LICH_KING_ESCAPE))))
                    {
                        ...
                        {
                            lichking->GetMotionMaster()->MovementExpired();
                            lichking->GetMotionMaster()->Clear(true);
                            lichking->StopMoving();
            ...
            void AttackIceWall()
            {
                ...
                if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                    DoCastSelf(SPELL_JAINA_DESTROY_ICE_WALL, true);
                else
                    DoCastSelf(SPELL_SYLVANAS_DESTROY_ICE_WALL, true);
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 event = _events.ExecuteEvent())
                {
                    switch (event)
                    {
                        case EVENT_ESCAPE:
                            if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                                DoCastSelf(SPELL_JAINA_ICE_BARRIER);
                            else
                                DoCastSelf(SPELL_SYLVANAS_CLOAK_OF_DARKNESS);
                        ...
                        case EVENT_ESCAPE_2:
                            ...
                            if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                                DoCastSelf(SPELL_JAINA_ICE_PRISON, false);
                            else
                                DoCastSelf(SPELL_SYLVANAS_BLINDING_RETREAT, true);
                        ...
                        case EVENT_ESCAPE_4:
                            if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                                DoCastSelf(SPELL_CREDIT_FINDING_JAINA);
                            else
                                DoCastSelf(SPELL_CREDIT_FINDING_SYLVANAS);
                        ...
                        case EVENT_ESCAPE_16:
                            ...
                            if (MotionTransport* gunship = ObjectAccessor::GetMotionTransport(*me, _instance->GetGuidData(DATA_GUNSHIP)))
                        ...
                        case EVENT_ESCAPE_17:
                        {
                            ...
                            DoCastSelf(SPELL_CREDIT_ESCAPING_ARTHAS);
                            me->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP | UNIT_NPC_FLAG_QUESTGIVER);
                            // Reward Dungeon
                            Map* map = me->GetMap();
                            if (map && map->IsDungeon())
                            {
                                if (!map->GetPlayers().empty())
                                    if (Group* group = map->GetPlayers().begin()->GetGroup())
                                        if (group->isLFGGroup())
    ...
    public:
        ...
        struct npc_the_lich_king_escape_horAI : public ScriptedAI
        {
                ...
                if (type == POINT_MOTION_TYPE)
                {
                    switch (pointId)
                    {
                        ...
                        case 2:
                            ...
                            if (Creature* target = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_ESCAPE_LEADER))))
            ...
            void SummonedCreatureDies(Creature* /*summon*/, Unit* /*killer*/) override
            {
                ...
                {
                    if (Creature* jainaOrSylvanas = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_ESCAPE_LEADER))))
                ...
                while (uint32 event = _events.ExecuteEvent())
                {
                    switch (event)
                    {
                        case EVENT_REMORSELESS_WINTER:
                            ...
                            DoCastSelf(SPELL_REMORSELESS_WINTER);
                            break;
                        case EVENT_ESCAPE_SUMMON_GHOULS:
                            StopMoving();
                            Talk(SAY_LK_ESCAPE_GHOULS);
                            DoCastSelf(SPELL_RAISE_DEAD);
                            _events.ScheduleEvent(EVENT_RAISE_DEAD_END, 4 * IN_MILLISECONDS);
                            break;
                        case EVENT_ESCAPE_SUMMON_WITCH_DOCTOR:
                            StopMoving();
                            DoCastSelf(SPELL_SUMMON_RISEN_WITCH_DOCTOR);
                            _events.ScheduleEvent(EVENT_RAISE_DEAD_END, 2 * IN_MILLISECONDS);
                            break;
                        case EVENT_ESCAPE_SUMMON_LUMBERING_ABOMINATION:
                            StopMoving();
                            Talk(SAY_LK_ESCAPE_ABOMINATION);
                            DoCastSelf(SPELL_SUMMON_LUMBERING_ABOMINATION);
                            _events.ScheduleEvent(EVENT_RAISE_DEAD_END, 2 * IN_MILLISECONDS);
                            break;
                        case EVENT_RAISE_DEAD_END:
                            me->AddUnitState(UNIT_STATE_ALWAYS_REACH_TARGET);
                            if (Creature* target = me->FindNearestCreature(NPC_JAINA_ESCAPE, 1000.0f, true))
                                me->GetMotionMaster()->MoveChase(target);
                            else if (Creature* target = me->FindNearestCreature(NPC_SYLVANAS_ESCAPE, 1000.0f, true))
                                me->GetMotionMaster()->MoveChase(target);
                       //     if (Creature* target = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_ESCAPE_LEADER)))
                       //         me->GetMotionMaster()->MoveChase(target);
                            break;
...
struct npc_gauntlet_trash : public ScriptedAI
{
    ...
    void Reset() override
    {
        DoCastSelf(SPELL_WELL_OF_SOULS, true);
        _events.Reset();
    }
    ...
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/) override
        {
                ...
                player->GetQuestStatus(QUEST_HALLS_OF_REFLECTION_HORDE) == QUEST_STATUS_INCOMPLETE) && _instance->GetData(DATA_QUEL_DELAR_EVENT) == NOT_STARTED)
            {
                ...
                _instance->SetData(DATA_QUEL_DELAR_INVOKER, player->GetGUID().GetRawValue());
            }
    ...
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/) override
        {
            ...
            if (_instance->GetData(DATA_INTRO_EVENT) == DONE && _instance->GetBossState(DATA_MARWYN) != DONE)
            {
                ...
                if (Creature* falric = ObjectAccessor::GetCreature(*player, ObjectGuid(_instance->GetGuidData(DATA_FALRIC))))
                {
                    falric->CastSpell(falric, SPELL_BOSS_SPAWN_AURA, true);
                    falric->SetVisible(true);
                }
                if (Creature* marwyn = ObjectAccessor::GetCreature(*player, ObjectGuid(_instance->GetGuidData(DATA_MARWYN))))
                {
                    marwyn->CastSpell(marwyn, SPELL_BOSS_SPAWN_AURA, true);
                    marwyn->SetVisible(true);
                }
...
struct npc_escape_event_trash : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* /*summoner*/) override
    {
        ...
        if (Creature* leader = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_ESCAPE_LEADER))))
        {
...
class npc_raging_ghoul : public CreatureScript
{
    public:
        ...
        struct npc_raging_ghoulAI : public npc_escape_event_trash
        {
            ...
            void IsSummonedBy(Unit* summoner) override
            {
                DoCastSelf(SPELL_RAGING_GHOUL_SPAWN, true);
                me->SetReactState(REACT_PASSIVE);
...
class npc_risen_witch_doctor : public CreatureScript
{
    public:
        ...
        struct npc_risen_witch_doctorAI : public npc_escape_event_trash
        {
            ...
            void IsSummonedBy(Unit* summoner) override
            {
                DoCastSelf(SPELL_RISEN_WITCH_DOCTOR_SPAWN, true);
                me->SetReactState(REACT_PASSIVE);
                me->HandleEmoteCommand(EMOTE_ONESHOT_EMERGE);
                me->m_Events.AddEvent(new hor::StartMovementEvent(me), me->m_Events.CalculateTime(5000));
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_UTHER_1:
                    ...
                    break;
                case EVENT_UTHER_2:                   
                    if (Creature* bunny = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_FROSTMOURNE_ALTAR_BUNNY))))
                        if (Unit* target = ObjectAccessor::GetPlayer(*me, ObjectGuid(_instance->GetGuidData(DATA_QUEL_DELAR_INVOKER))))
                ...
                case EVENT_UTHER_10:
                    ...
                    break;
                case EVENT_UTHER_11:
                    DoCastSelf(SPELL_ESSENCE_OF_CAPTURED_1, true);
...
public:
    ...
    struct npc_quel_delar_swordAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (Creature* uther = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_UTHER_QUEL_DELAR))))
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/) override
    {
        ...
        if (_instance->GetData(DATA_QUEL_DELAR_EVENT) == IN_PROGRESS)
            if (Creature* uther = ObjectAccessor::GetCreature(*player, ObjectGuid(_instance->GetGuidData(DATA_UTHER_QUEL_DELAR))))
...
// 5752
class at_champion_kill_goto_lk_floor : public OnlyOnceAreaTriggerScript
{
public:
    at_champion_kill_goto_lk_floor() : OnlyOnceAreaTriggerScript("at_champion_kill_goto_lk_floor") { }

    bool _OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/)
    {
        InstanceScript* instance = player->GetInstanceScript();
        if (!instance || instance->GetData(DATA_FROSTSWORN_GENERAL) != DONE)
            return true;

        player->PlayDirectSound(SOUND_CHAMPION_KILL_GOTO_LK);
...
void AddSC_halls_of_reflection()
{
    ...
    new at_champion_kill_goto_lk_floor();
