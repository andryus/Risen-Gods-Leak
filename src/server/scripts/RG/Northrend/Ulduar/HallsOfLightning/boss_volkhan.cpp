...
public:
    ...
    struct boss_volkhanAI : public BossAI
    {
        ...
        void ShatterOrDespawnGolem(bool shatter)
        {
            ...
            for (ObjectGuid guid : m_lGolemGUIDList)
            {
                ...
                {
                    if (shatter) // Only shatter brittle golems
                    {
                    ...
                    else if (!shatter) // Despawn
                    {
                        if (temp->IsAlive())
                        {
                            temp->DespawnOrUnsummon(1 * IN_MILLISECONDS);
                        }
                    }
                }
            }
        ...
        void UpdateAI(uint32 diff) override
        {
                ...
                switch (eventId)
                {
                    ...
                    case EVENT_SHATTERING_STOMP:
                        if (!m_bHasTemper && m_uiHealthAmountModifier >= 3)
                        {
                            ...
                            DoCastSelf(SPELL_SHATTERING_STOMP);
            ...
            switch (m_uiSummonPhase)
            {
                ...
                case 3:
                    ...
                    if (Unit* target = me->FindNearestCreature(NPC_VOLKHAN_ANVIL, 1000.0f, true))
                    {
                        me->SetOrientation(2.29f);
                ...
                case 5:
                    // 5 - Spawn the Golems
                    if (Creature* creatureTarget = me->FindNearestCreature(NPC_VOLKHAN_ANVIL, 1000.0f, true))
