...
public:
    ...
    struct boss_bjarngrimAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (Creature* pLieutenant1 = ObjectAccessor::GetCreature(*me, instance ? ObjectGuid(instance->GetGuidData(DATA_STORMFORGED_LIEUTENANT_1)) : ObjectGuid::Empty))
            {
                if (!pLieutenant1->IsAlive())
                    pLieutenant1->Respawn();
                pLieutenant1->SetFullHealth();
            }
            if (Creature* pLieutenant2 = ObjectAccessor::GetCreature(*me, instance ? ObjectGuid(instance->GetGuidData(DATA_STORMFORGED_LIEUTENANT_2)) : ObjectGuid::Empty))
            {
                if (!pLieutenant2->IsAlive())
                    pLieutenant2->Respawn();
                pLieutenant2->SetFullHealth();
            }
        ...
        void UpdateAI(uint32 uiDiff)
        {
            ...
            if (m_uiChangeStance_Timer <= uiDiff)
            {
                ...
                switch (m_uiStance)
                {
                    ...
                    case STANCE_BERSERKER:
                        ...
                        DoCastSelf(SPELL_BERSEKER_STANCE);
                        SetEquipmentSlots(false, EQUIP_SWORD, EQUIP_SWORD, EQUIP_NO_CHANGE);
                        break;
                    case STANCE_BATTLE:
                        Talk(SAY_BATTLE_STANCE);
                        Talk(EMOTE_BATTLE_STANCE);
                        DoCastSelf(SPELL_BATTLE_STANCE);
            ...
            switch (m_uiStance)
            {
                ...
                case STANCE_BERSERKER:
                {
                    ...
                    {
                        if(!(me->IsNonMeleeSpellCast(false)) && me->GetDistance(me->GetVictim()) >= 8.0f)
                        {
                            //not much point is this, better random target and more often?
                            DoCastVictim(SPELL_INTERCEPT);
                            m_uiIntercept_Timer = urand(15000, 16000);
                        }
                    }
                    else
                        m_uiIntercept_Timer -= uiDiff;

                    if (m_uiWhirlwind_Timer <= uiDiff && me->GetDistance(me->GetVictim()) <= 5.0f)
                    {
...
public:
    ...
    struct npc_stormforged_lieutenantAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* who)
        {
            if (instance)
            {
                if (Creature* pBjarngrim = instance->instance->GetCreature(ObjectGuid(instance->GetGuidData(DATA_BJARNGRIM))))
                {
