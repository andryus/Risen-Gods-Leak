            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    case EVENT_WRAP:
                        ...
                        for (uint8 i = 0; i < RAID_MODE(1, 2); ++i)
                        {
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0U, NonTankTargetSelector(me, true)))
                            {
                                ...
                                me->AddAura(SPELL_WEB_WRAP, target);
                                uint8 pos = rand32()%MAX_POS_WRAP;
                                target->GetMotionMaster()->MoveJump(PosWrap[pos], 40, 20);
                                if (Creature* wrap = DoSummon(MOB_WEB_WRAP, PosWrap[pos], 0, TEMPSUMMON_CORPSE_DESPAWN))
                                    wrap->AI()->SetGuidData(target->GetGUID());
                            }
                        }
                    ...
                    case EVENT_POISON:
                        ...
                        break;
                    case EVENT_FRENZY:
                        DoCastSelf(RAID_MODE(SPELL_FRENZY_10, SPELL_FRENZY_25), true);
