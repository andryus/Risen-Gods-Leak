        ...
        struct boss_faerlinaAI : public BossAI
        {
            ...
            void SpellHit(Unit* caster, SpellInfo const* spell) override
            {
                if (spell->Id == SPELL_WIDOWS_EMBRACE || spell->Id == H_SPELL_WIDOWS_EMBRACE)
                {
                    ...
                    _delayFrenzy = true;
                    if (Is25ManRaid()) // in 10man mode the add already died
                        me->Kill(caster);
                }
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
                    _delayFrenzy = false;
                    DoCastSelf(RAID_MODE(SPELL_FRENZY, H_SPELL_FRENZY), true);
                }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_FRENZY:
                            Talk(EMOTE_FRENZY);
                            if (!me->HasAura(SPELL_WIDOWS_EMBRACE_HELPER))
                                DoCastSelf(RAID_MODE(SPELL_FRENZY, H_SPELL_FRENZY));
                            else
                                _delayFrenzy = true;
        ...
        struct npc_faerlina_addAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* /*killer*/) override
            {
                if (_instance && GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
                    if (Creature* faerlina = ObjectAccessor::GetCreature(*me, ObjectGuid(_instance->GetGuidData(DATA_FAERLINA))))
                        faerlina->CastSpell(faerlina, SPELL_WIDOWS_EMBRACE, true); // has 0 yards range
            }
