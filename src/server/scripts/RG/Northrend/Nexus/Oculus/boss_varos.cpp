    ...
    struct boss_varosAI : public BossAI
    {
        boss_varosAI(Creature* creature) : BossAI(creature, DATA_VAROS_EVENT)
        {
            if (instance->GetBossState(DATA_DRAKOS_EVENT) != DONE)
                DoCastSelf(SPELL_CENTRIFUGE_SHIELD);
        }

        void Reset()
        {
            BossAI::Reset();
            resetcheck_timer = 2 * IN_MILLISECONDS;
            events.ScheduleEvent(EVENT_AMPLIFY_MAGIC,         urand(20, 25) * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_ENERGIZE_CORES_VISUAL,             5 * IN_MILLISECONDS);
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            {
                uint32 tempz = uint32(me->GetPositionZ());
                if (tempz > 455 || tempz < 424)
                {
                    EnterEvadeMode();
                    return;
                }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_ENERGIZE_CORES:
                        DoCastSelf(SPELL_ENERGIZE_CORES);
                    ...
                    case EVENT_ENERGIZE_CORES_VISUAL:
                        ...
                        DoCastSelf(SPELL_ENERGIZE_CORES_VISUAL);
                        events.ScheduleEvent(EVENT_ENERGIZE_CORES_VISUAL, 5 * IN_MILLISECONDS);
                        events.ScheduleEvent(EVENT_ENERGIZE_CORES,        4 * IN_MILLISECONDS);
                        break;
                    case EVENT_CALL_AZURE:
                        // not sure how blizz handles this, i cant see any pattern between the differnt spells
                        DoCastSelf(SPELL_CALL_AZURE_RING_CAPTAIN);
                        Talk(SAY_AZURE);
            ...
            void DoAction(int32 action)
            {
                switch (action)
                {
                   case ACTION_CALL_DRAGON_EVENT:
                        if (instance)
                        {
                            if (Creature* varos = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_VAROS))))
                            {
        ...
        class spell_varos_centrifuge_shield_AuraScript : public AuraScript
        {
            ...
            void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    if (caster->IsImmuneToPC() || caster->IsImmuneToNPC() || caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_UNK_15|UNIT_FLAG_UNK_6))
                    {
                        caster->ToCreature()->SetReactState(REACT_PASSIVE);
                        caster->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_UNK_15|UNIT_FLAG_UNK_6);
                        caster->SetImmuneToAll(true);
                    }
                }
            }
