...
public:
    ...
    struct boss_eregosAI : public BossAI
    {
        ...
        void DoAction(int32 action)
        {
            ...
            events.ScheduleEvent(EVENT_ARCANE_BARRAGE, 2 * IN_MILLISECONDS, 0, PHASE_NORMAL);
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            bool inDist = me->IsWithinDist(me->GetVictim(), 40.f, true);
            if (inDist && IsCombatMovementAllowed())
            {
                SetCombatMovement(false);
                me->GetMotionMaster()->Clear();
                DoStartNoMovement(me->GetVictim());
            }
            else if (!inDist && !IsCombatMovementAllowed())
            {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SUMMON_LEY_WHELP:
                        ...
                        events.RescheduleEvent(EVENT_ARCANE_BARRAGE, 6 * IN_MILLISECONDS, 0, PHASE_NORMAL);
                        events.ScheduleEvent(EVENT_SUMMON_LEY_WHELP, urand(15, 30) * IN_MILLISECONDS, 0, PHASE_NORMAL);
                        break;
                }
            }
