                ...
                case PHASE_EYE_RED_BEAM:
                    if (DarkGlareTick < 35)
                    {
                        if (DarkGlareTickTimer <= diff)
                        {
                            ...
                            DoCastSelf(SPELL_DARK_GLARE, false);
...
public:
    ...
    struct cthunAI : public ScriptedAI
    {
        ...
        void SpawnEyeTentacle(float x, float y)
        {
            Creature* Spawned = Spawned = DoSpawnCreature(NPC_EYE_TENTACLE, x, y, 0, 0, TEMPSUMMON_CORPSE_DESPAWN, 500);
            if (Spawned && Spawned->AI())
            ...
            switch (currentPhase)
            {
                ...
                case PHASE_CTHUN_TRANSITION:
                    ...
                    if (PhaseTimer <= diff)
                    {
                        ...
                        me->InterruptNonMeleeSpells(false);
                        DoCastSelf(SPELL_TRANSFORM, false);
                        me->SetFullHealth();
