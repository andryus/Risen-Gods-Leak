...
struct boss_twinemperorsAI : public ScriptedAI
{
    ...
    {
        return ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(IAmVeklor() ? DATA_TWIN_VEKLINASH : DATA_TWIN_VEKLOR)));
    }
    ...
    void DamageTaken(Unit* /*done_by*/, uint32 &damage) override
    {
        ...
        if (pOtherBoss)
        {
            ...
            pOtherBoss->LowerPlayerDamageReq(odmg);
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (GameObject* door = me->FindNearestGameObject(GO_TWINS_GATE, 1000.0f))
    ...
    void TeleportToMyBrother()
    {
        ...
        {
            me->NearTeleportTo(pOtherBoss->GetPositionX(), pOtherBoss->GetPositionY(), pOtherBoss->GetPositionZ(), pOtherBoss->GetOrientation());
            pOtherBoss->NearTeleportTo(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());

            DoStopAttack();
            ResetThreatList();
            pOtherBoss->GetAI()->DoAction(ACTION_DO_RESET);

            DoCastSelf(SPELL_TWIN_TELEPORT_VISUAL);
            pOtherBoss->AI()->DoCast(pOtherBoss, SPELL_TWIN_TELEPORT_VISUAL);
    ...
    bool TryActivateAfterTTelep(uint32 diff)
    {
        if (AfterTeleport)
        {
            ...
            {
                me->ClearUnitState(UNIT_STATE_STUNNED);
                DoCastSelf(SPELL_TWIN_TELEPORT);
                me->AddUnitState(UNIT_STATE_STUNNED);
            }
...
public:
    ...
    struct boss_veklorAI : public boss_twinemperorsAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            {
                Unit* mvic;
                if ((mvic=SelectTarget(SELECT_TARGET_MINDISTANCE, 0, NOMINAL_MELEE_RANGE, true)) != NULL)
                {
