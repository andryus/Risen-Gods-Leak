    ...
    public:
        ...
        struct boss_ossirianAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.Reset();
                events.ScheduleEvent(EVENT_SILENCE, 30 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CYCLONE, 20 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_STOMP,   30 * IN_MILLISECONDS);
                ...
                data << uint32(WEATHER_STATE_LIGHT_SANDSTORM) << float(1) << uint8(0);
                map->SendToPlayers(std::move(data));
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SILENCE:
                            ...
                            events.ScheduleEvent(EVENT_SILENCE, urand(20, 30) * IN_MILLISECONDS);
                            break;
                        case EVENT_CYCLONE:
