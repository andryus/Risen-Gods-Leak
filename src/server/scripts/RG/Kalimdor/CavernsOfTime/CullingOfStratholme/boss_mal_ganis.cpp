...
public:
    ...
    struct boss_mal_ganisAI : public ScriptedAI
    {
        ...
        {
            if (instance && instance->GetData(DATA_EPOCH_EVENT) != DONE)
                EnterEvadeMode();
        ...
        void UpdateAI(uint32 diff) override
        {
            switch (Phase)
            {
                case COMBAT:
                    ...
                    if (Creature* pArthas = ObjectAccessor::GetCreature(*me, instance ? instance->GetGuidData(DATA_ARTHAS) : ObjectGuid::Empty))
                        if (pArthas->isDead())
                        {
                            EnterEvadeMode();
                            me->DisappearAndDie();
                    ...
                    if (uiOutroTimer < diff)
                    {
                        switch (uiOutroStep)
                        {
                            case 1:
                                ...
                                break;
                            case 2:
                                me->SetGuidValue(UNIT_FIELD_TARGET, instance ? instance->GetGuidData(DATA_ARTHAS) : ObjectGuid::Empty);
...
class spell_vampiric_touchAuraScript : public AuraScript
{
    PrepareAuraScript(spell_vampiric_touchAuraScript);

    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& procInfo)
    {
        if (!GetUnitOwner())
            return;
        GetUnitOwner()->CastCustomSpell(SPELL_VAMPIRIC_TOUCH_HEAL, SPELLVALUE_BASE_POINT0, procInfo.GetDamageInfo()->GetDamage() * 0.5, GetUnitOwner(), true);
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_vampiric_touchAuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
{
    new boss_mal_ganis();
    new SpellScriptLoaderEx<spell_vampiric_touchAuraScript>("spell_vampiric_touch");
}
