    ...
    public:
            ...
            switch (action)
            {
                case GOSSIP_ACTION_INFO_DEF + 1:
                    ...
                    if (auto ai = CAST_AI(npc_varian_wrynn::npc_varian_wrynnAI, creature->AI()))
                    {
                        ...
                        if (Creature* jaina = creature->FindNearestCreature(NPC_JAINA, 50.0f))
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if (player->GetQuestStatus(QUEST_BATTLE_A) == QUEST_STATUS_INCOMPLETE)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_WRYNN, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                ...
                switch (waveId)
                {
                    ...
                    case 6:
                        if (Unit* temp = me->SummonCreature(NPC_BLIGHTWORM, AllianceSpawn[7].x, AllianceSpawn[7].y, AllianceSpawn[7].z, TEMPSUMMON_MANUAL_DESPAWN))
                        {
                            ...
                            AddThreat(me, 100.0f, temp);
                            AddThreat(temp, 100.0f);
                            if (Creature* jaina = ObjectAccessor::GetCreature(*me, jainaGUID))
                            {
                                AddThreat(jaina, 100.0f, temp);
                                AddThreat(temp, 100.0f, jaina);
                            }
                        }
                   ...
                   switch (step)
                   {
                       ...
                       case 68:
                           ...
                           if (Creature* jaina = ObjectAccessor::GetCreature(*me, jainaGUID))
                           {
                               ...
                               jaina->SetImmuneToNPC(true);
                               jaina->SetImmuneToPC(true);
                           }
                           SetEscortPaused(false);
                           bStepping = false;
                           JumpToNextStep(0);
                       ...
                       case 73:
                           ...
                           if (Creature* thrall = ObjectAccessor::GetCreature(*me, thrallGUID))
                           {
                               ...
                               AddThreat(me, 100.0f, thrall);
                               AddThreat(thrall, 100.0f);
                           ...
                           if (Creature* sylvanas = ObjectAccessor::GetCreature(*me, sylvanasGUID))
                           {
                               ...
                               AddThreat(me, 100.0f, sylvanas);
                               sylvanas->AI()->AttackStart(me);
                               AddThreat(sylvanas, 100.0f);
                           }
                           for (uint8 i = 0; i < HORDE_FORCE_MAXCOUNT; ++i)
                           {
                               if (Creature* temp = ObjectAccessor::GetCreature(*me, hordeForcesGUID[i]))
                           ...
                           for (uint8 i = 0; i < ALLIANCE_FORCE_MAXCOUNT; ++i)
                           {
                               if (Creature* temp = ObjectAccessor::GetCreature(*me, allianceForcesGUID[i]))
                               {
                                   if (Creature* temp2 = ObjectAccessor::GetCreature(*me, hordeForcesGUID[i]))
                                   {
                                       ...
                                       AddThreat(temp2, 100.0f, temp);
                                       temp->AI()->AttackStart(temp2);
                                       AddThreat(temp, 100.0f, temp2);
                                   }
                               }
                           }
                           JumpToNextStep(6 * IN_MILLISECONDS);
           ...
           while (uint32 eventId = _events.ExecuteEvent())
           {
               switch (eventId)
               {
                   ...
                   case EVENT_AGGRO_JAINA:
                       if (me->GetVictim())
                           if (Creature* jaina = ObjectAccessor::GetCreature(*me, jainaGUID))
                               jaina->AI()->AttackStart(me->GetVictim()); DoCastSelf(SPELL_THUNDER);
                       _events.ScheduleEvent(EVENT_AGGRO_JAINA, 2 * IN_MILLISECONDS);
                       break;
    ...
    public:
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
        {
            ...
            switch (action)
            {
                case GOSSIP_ACTION_INFO_DEF + 1:
                {
                    ...
                    {
                        if (Creature* sylvannas = creature->FindNearestCreature(NPC_SYLVANAS, 50.0f))
                        {
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if (player->GetQuestStatus(QUEST_BATTLE_H) == QUEST_STATUS_INCOMPLETE)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetOptionTextWithEntry(GOSSIP_THRALL, 0), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            ...
            void JustSummoned(Creature* summoned) override
            {
                switch (summoned->GetEntry())
                {
                    ...
                    case NPC_CHEMIST_H:
                        ...
                        AddThreat(me, 100.0f, summoned);
                        AddThreat(summoned, 100.0f);
                        summoned->AI()->AttackStart(me);
                        break;
                    ...
                    if (phaseTimer <= diff)
                    {
                        switch (step)
                        {
                            ...
                            case 115:
                            {
                                ...
                                me->GetGameObjectListWithEntryInGrid(SaronitList, GO_BLOCKED_PASSAGE, 80.0f);
                            ...
                            case 137:
                                if (Creature* valimathras = ObjectAccessor::GetCreature(*me, ValimathrasGUID))
                                {
                                    ...
                                    AddThreat(me, 100.0f, valimathras);
                                    AddThreat(valimathras, 100.0f);
