...
public:
    ...
    struct boss_dathrohan_balnazzarAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            m_uiCrusadersHammer_Timer =  8 * IN_MILLISECONDS;
            m_uiCrusaderStrike_Timer  = 12 * IN_MILLISECONDS;
            m_uiMindBlast_Timer       =  6 * IN_MILLISECONDS;
            m_uiHolyStrike_Timer      = 18 * IN_MILLISECONDS;
            m_uiShadowShock_Timer     =  4 * IN_MILLISECONDS;
            m_uiPsychicScream_Timer   = 16 * IN_MILLISECONDS;
            m_uiDeepSleep_Timer       = 20 * IN_MILLISECONDS;
            m_uiMindControl_Timer     = 10 * IN_MILLISECONDS;
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            for (uint8 i = 0; i < uiCount; ++i)
                if (Creature* zombie = me->SummonCreature(NPC_ZOMBIE, m_aSummonPoint[i].m_fX, m_aSummonPoint[i].m_fY, m_aSummonPoint[i].m_fZ, m_aSummonPoint[i].m_fOrient, TEMPSUMMON_TIMED_DESPAWN, HOUR*IN_MILLISECONDS))
                {
                    zombie->SetReactState(REACT_AGGRESSIVE);
                    zombie->GetMotionMaster()->MovePath(NPC_ZOMBIE * 10, false);
                }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!m_bTransformed)
            {
                ...
                if (HealthBelowPct(40))
                {
                    ...
                    DoCastSelf(SPELL_BALNAZZARTRANSFORM);
            ...
            else
            {
                ...
                if (m_uiShadowShock_Timer <= diff)
                {
                    ...
                    m_uiShadowShock_Timer = 11 * IN_MILLISECONDS;
                ...
                if (m_uiPsychicScream_Timer <= diff)
                {
                    ...
                    m_uiPsychicScream_Timer = 20 * IN_MILLISECONDS;
