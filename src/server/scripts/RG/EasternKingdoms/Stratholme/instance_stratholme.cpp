...
Position const GuardLoc[5] =
{
    { 4031.838623f, -3391.313232f, 119.739624f, 4.682759f }, 
    { 4032.774658f, -3391.690918f, 119.739624f, 4.707959f }, 
    { 4033.474609f, -3391.694092f, 119.739624f, 4.707959f }, 
    { 4033.481201f, -3390.191162f, 119.739624f, 4.707959f },
    { 4032.186279f, -3390.185303f, 119.739624f, 4.707959f } 
};
...
class instance_stratholme : public InstanceMapScript
{
    public:
        ...
        struct instance_stratholme_InstanceMapScript : public InstanceScript
        {
            ...
            EventMap events;
            std::set<ObjectGuid> timmyConditionGUIDs;

            void spawnSkul()
            {
                uint8 ra= rand32() % 10 + 1; //Random 1-10
                Position skulPos;
                if(ra <= 3)
                {
                    skulPos.m_positionX = 3655.880859f;
                    skulPos.m_positionY = -3519.694092f;
                    skulPos.m_positionZ = 137.119781f;
                    skulPos.SetOrientation(2.298492f);
                    if(instance)
                    instance->SummonCreature(10393, skulPos);
                }
                else if(ra <= 6)
                {
                    skulPos.m_positionX = 3489.084473f;
                    skulPos.m_positionY = -3329.206055f;
                    skulPos.m_positionZ = 130.725861f;
                    skulPos.SetOrientation(0.694695f);
                    if(instance)
                    instance->SummonCreature(10393, skulPos);
                }
            }
            ...
            void OnCreatureCreate(Creature* creature) override
            {
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_CRIMSON_CONJURER:
                        if (creature->IsWithinDist2d(TimmyLocation[1].GetPositionX(), TimmyLocation[1].GetPositionY(), 40.0f))
                            timmyConditionGUIDs.insert(creature->GetGUID());
                        break;
                }
            }
            ...
            void OnUnitDeath(Unit* unit) override
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_CRIMSON_GUARDSMAN:
                    case NPC_CRIMSON_CONJURER:
                        if (timmyConditionGUIDs.find(creature->GetGUID()) != timmyConditionGUIDs.end())
                        {
                            timmyConditionGUIDs.erase(creature->GetGUID());

                            if (timmyConditionGUIDs.empty())
                                creature->SummonCreature(NPC_TIMMY_THE_CRUEL, TimmyLocation[0], TEMPSUMMON_DEAD_DESPAWN);
                        }
                        break;
                }
            }
                ...
                switch (type)
                {
                    ...
                    case TYPE_RAMSTEIN:
                        if (data == IN_PROGRESS)
                        {
                            ...
                            for (std::set<ObjectGuid>::const_iterator i = abomnationGUID.begin(); i != abomnationGUID.end(); ++i)
                            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SLAUGHTER_SQUARE:
                            ...
                            {
                                for (uint8 i = 0; i < 5; ++i)
                                    baron->SummonCreature(NPC_BLACK_GUARD, GuardLoc[i], TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 1800000);
