    ...
    public:
        ...
        struct boss_rend_blackhandAI : public BossAI
        {
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_WHIRLWIND,     urand (13, 15) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CLEAVE,        urand (15, 17) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_MORTAL_STRIKE, urand (17, 19) * IN_MILLISECONDS);
            }
            ...
            void JustSummoned(Creature* summoned) override
            {
                switch (summoned->GetEntry())
                {
                    ...
                    case NPC_BLACKHAND_DRAGON_HANDLER:
                        if (Unit* target = summoned->FindNearestPlayer(100.0f))
            ...
            void SetData(uint32 type, uint32 data) override
            {
                if (instance && type == AREATRIGGER && data == AREATRIGGER_BLACKROCK_STADIUM)
                {
                    if (!gythEvent)
                    {
                        ...
                        events.ScheduleEvent(EVENT_START_1, 1 * IN_MILLISECONDS);
                    }
                }
            }
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            case EVENT_START_1:
                                ...
                                events.ScheduleEvent(EVENT_START_2, 4 * IN_MILLISECONDS);
                                break;
                            case EVENT_START_2:
                                events.ScheduleEvent(EVENT_TURN_TO_PLAYER, 0);
                                if (Creature* victor = ObjectAccessor::GetCreature(*me, victorGUID))
                                    victor->HandleEmoteCommand(EMOTE_ONESHOT_POINT);
                                events.ScheduleEvent(EVENT_START_3, 4 * IN_MILLISECONDS);
                                break;
                            case EVENT_START_3:
                                if (Creature* victor = ObjectAccessor::GetCreature(*me, victorGUID))
                                    victor->AI()->Talk(SAY_NEFARIUS_1);
                                events.ScheduleEvent(EVENT_WAVE_1, 2000);
                                events.ScheduleEvent(EVENT_TURN_TO_REND, 4 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_TEXT_1, 20 * IN_MILLISECONDS);
                                break;
                            ...
                            case EVENT_TURN_TO_PLAYER:
                                if (Creature* victor = ObjectAccessor::GetCreature(*me, victorGUID))
                                    if (Unit* player = victor->FindNearestPlayer(60.0f))
                            ...
                            case EVENT_WAVES_TEXT_1:
                                ...
                                events.ScheduleEvent(EVENT_TURN_TO_FACING_1, 4 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_EMOTE_1,    5 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVE_2,           2 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_TEXT_2,    20 * IN_MILLISECONDS);
                                break;
                            case EVENT_WAVES_TEXT_2:
                                ...
                                events.ScheduleEvent(EVENT_TURN_TO_FACING_1, 4 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVE_3,           2 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_TEXT_3,    20 * IN_MILLISECONDS);
                                break;
                            case EVENT_WAVES_TEXT_3:
                                ...
                                events.ScheduleEvent(EVENT_TURN_TO_FACING_1, 4 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVE_4,           2 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_TEXT_4,    20 * IN_MILLISECONDS);
                                break;
                            case EVENT_WAVES_TEXT_4:
                                Talk(SAY_BLACKHAND_1);
                                events.ScheduleEvent(EVENT_WAVES_EMOTE_2,    4 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_TURN_TO_FACING_3, 8 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVE_5,           2 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_TEXT_5,    20 * IN_MILLISECONDS);
                                break;
                            case EVENT_WAVES_TEXT_5:
                                ...
                                events.ScheduleEvent(EVENT_TURN_TO_FACING_1,       4 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVE_6,                 2 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_COMPLETE_TEXT_1, 20 * IN_MILLISECONDS);
                                break;
                            case EVENT_WAVES_COMPLETE_TEXT_1:
                                ...
                                events.ScheduleEvent(EVENT_TURN_TO_FACING_1,       4 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_COMPLETE_TEXT_2, 13 * IN_MILLISECONDS);
                                break;
                            case EVENT_WAVES_COMPLETE_TEXT_2:
                                if (Creature* victor = ObjectAccessor::GetCreature(*me, victorGUID))
                                    victor->AI()->Talk(SAY_NEFARIUS_7);
                                Talk(SAY_BLACKHAND_2);
                                events.ScheduleEvent(EVENT_PATH_REND,             1 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_WAVES_COMPLETE_TEXT_3, 4 * IN_MILLISECONDS);
                                break;
                            case EVENT_WAVES_COMPLETE_TEXT_3:
                                if (Creature* victor = ObjectAccessor::GetCreature(*me, victorGUID))
                                    victor->AI()->Talk(SAY_NEFARIUS_8);
                                events.ScheduleEvent(EVENT_PATH_NEFARIUS, 1 * IN_MILLISECONDS);
                                events.ScheduleEvent(EVENT_PATH_REND,     1 * IN_MILLISECONDS);
                                break;
                            ...
                            case EVENT_TELEPORT_1:
                                ...
                                events.ScheduleEvent(EVENT_TELEPORT_2, 50 * IN_MILLISECONDS);
                                break;
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_WHIRLWIND:
                            ...
                            events.ScheduleEvent(EVENT_WHIRLWIND, urand(13, 18) * IN_MILLISECONDS);
                            break;
                        case EVENT_CLEAVE:
                            DoCastVictim(SPELL_CLEAVE);
                            events.ScheduleEvent(EVENT_CLEAVE, urand(10, 14) * IN_MILLISECONDS);
                            break;
                        case EVENT_MORTAL_STRIKE:
                            DoCastVictim(SPELL_MORTAL_STRIKE);
                            events.ScheduleEvent(EVENT_MORTAL_STRIKE, urand(14, 16) * IN_MILLISECONDS);
                            break;
                    }
                }
