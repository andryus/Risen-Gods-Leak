        ...
        struct boss_mothersmolderwebAI : public BossAI
        {
            ...
            void DamageTaken(Unit* /*done_by*/, uint32 &damage) override
            {
                if (me->GetHealth() <= damage)
                    DoCastSelf(SPELL_SUMMON_SPIRE_SPIDERLING, true);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CRYSTALIZE:
                            DoCastSelf(SPELL_CRYSTALIZE);
                            events.ScheduleEvent(EVENT_CRYSTALIZE, 15 * IN_MILLISECONDS);
                            break;
                        case EVENT_MOTHERS_MILK:
                            DoCastSelf(SPELL_MOTHERSMILK);
                            events.ScheduleEvent(EVENT_MOTHERS_MILK, urand(5000, 12500));
                            break;
                    }
                }
