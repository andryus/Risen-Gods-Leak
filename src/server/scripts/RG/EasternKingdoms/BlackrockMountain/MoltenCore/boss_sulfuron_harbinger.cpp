    ...
    public:
        ...
        struct boss_sulfuronAI : public BossAI
        {
            ...
            void EnterCombat(Unit* victim) override
            {
                ...
                events.ScheduleEvent(EVENT_DARK_STRIKE,        10 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_DEMORALIZING_SHOUT, 15 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_INSPIRE,            13 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_KNOCKDOWN,           6 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_FLAMESPEAR,          2 * IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_DARK_STRIKE:
                            ...
                            events.ScheduleEvent(EVENT_DARK_STRIKE, urand(15, 18) * IN_MILLISECONDS);
                            break;
                        case EVENT_DEMORALIZING_SHOUT:
                            DoCastVictim(SPELL_DEMORALIZING_SHOUT);
                            events.ScheduleEvent(EVENT_DEMORALIZING_SHOUT, urand(15, 20) * IN_MILLISECONDS);
                            break;
                        case EVENT_INSPIRE:
                        {
                            ...
                            events.ScheduleEvent(EVENT_INSPIRE, urand(20, 26) * IN_MILLISECONDS);
                            break;
                        }
                        case EVENT_KNOCKDOWN:
                            DoCastVictim(SPELL_KNOCKDOWN);
                            events.ScheduleEvent(EVENT_KNOCKDOWN, urand(12, 15) * IN_MILLISECONDS);
                            break;
                        case EVENT_FLAMESPEAR:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
                                DoCast(target, SPELL_FLAMESPEAR);
                            events.ScheduleEvent(EVENT_FLAMESPEAR, urand(12, 16) * IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
    ...
    public:
        ...
        struct npc_flamewaker_priestAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* victim) override
            {
                ...
                events.ScheduleEvent(EVENT_HEAL, urand(15, 30) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SHADOW_WORD_PAIN, 2 * IN_MILLISECONDS);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_HEAL:
                            ...
                            events.ScheduleEvent(EVENT_HEAL, urand(15, 20) * IN_MILLISECONDS);
                            break;
                        case EVENT_SHADOW_WORD_PAIN:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true, true, -SPELL_SHADOWWORDPAIN))
                                DoCast(target, SPELL_SHADOWWORDPAIN);
                            events.ScheduleEvent(EVENT_SHADOW_WORD_PAIN, urand(18, 26) * IN_MILLISECONDS);
                            break;
                        case EVENT_IMMOLATE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true, true, -SPELL_IMMOLATE))
                                DoCast(target, SPELL_IMMOLATE);
                            events.ScheduleEvent(EVENT_IMMOLATE, urand(15, 25) * IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
