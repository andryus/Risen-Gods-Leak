...
enum Events
{
    ...
    EVENT_SHADOW_BOLT_RANDOM = 3,
    EVENT_SHADOW_BOLT_TARGET = 4
};
    ...
    public:
        ...
        struct boss_gehennasAI : public BossAI
        {
            ...
            void EnterCombat(Unit* victim) override
            {
                ...
                events.ScheduleEvent(EVENT_GEHENNAS_CURSE,               12 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_RAIN_OF_FIRE,                 10 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SHADOW_BOLT_RANDOM, urand (3, 6) * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SHADOW_BOLT_TARGET,  urand(3, 6) * IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_GEHENNAS_CURSE:
                            ...
                            events.ScheduleEvent(EVENT_GEHENNAS_CURSE, urand(22, 30) * IN_MILLISECONDS);
                            break;
                        case EVENT_RAIN_OF_FIRE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                DoCast(target, SPELL_RAIN_OF_FIRE);
                            events.ScheduleEvent(EVENT_RAIN_OF_FIRE, urand(4, 12) * IN_MILLISECONDS);
                            break;
                        case EVENT_SHADOW_BOLT_RANDOM:
                            ...
                            events.ScheduleEvent(EVENT_SHADOW_BOLT_RANDOM, urand(3, 6) * IN_MILLISECONDS);
                            break;
                        case EVENT_SHADOW_BOLT_TARGET:
                            DoCastVictim(SPELL_SHADOW_BOLT_TARGET);
                            events.ScheduleEvent(EVENT_SHADOW_BOLT_TARGET, urand(3, 6) * IN_MILLISECONDS);
                            break;
                        default:
                            break;
