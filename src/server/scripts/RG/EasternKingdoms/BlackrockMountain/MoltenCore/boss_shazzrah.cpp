    ...
    public:
        ...
        struct boss_shazzrahAI : public BossAI
        {
            ...
            void EnterCombat(Unit* target) override
            {
                ...
                events.ScheduleEvent(EVENT_ARCANE_EXPLOSION, 6 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SHAZZRAH_CURSE,  10 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_MAGIC_GROUNDING, 24 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_COUNTERSPELL,    15 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_SHAZZRAH_GATE,   45 * IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_ARCANE_EXPLOSION:
                            ...
                            events.ScheduleEvent(EVENT_ARCANE_EXPLOSION, urand(4, 7) * IN_MILLISECONDS);
                            break;
                        ...
                        case EVENT_SHAZZRAH_CURSE:
                            ...
                            events.ScheduleEvent(EVENT_SHAZZRAH_CURSE, urand(25, 30) * IN_MILLISECONDS);
                            break;
                        case EVENT_MAGIC_GROUNDING:
                            DoCastSelf(SPELL_MAGIC_GROUNDING);
                            events.ScheduleEvent(EVENT_MAGIC_GROUNDING, 35 * IN_MILLISECONDS);
                            break;
                        case EVENT_COUNTERSPELL:
                            DoCastVictim(SPELL_COUNTERSPELL);
                            events.ScheduleEvent(EVENT_COUNTERSPELL, urand(16, 20) * IN_MILLISECONDS);
                            break;
                        case EVENT_SHAZZRAH_GATE:
                            ResetThreatList();
                            DoCastAOE(SPELL_SHAZZRAH_GATE_DUMMY);
                            events.ScheduleEvent(EVENT_ARCANE_EXPLOSION_TRIGGERED, 2 * IN_MILLISECONDS);
                            events.RescheduleEvent(EVENT_ARCANE_EXPLOSION, urand(3, 6) * IN_MILLISECONDS);
