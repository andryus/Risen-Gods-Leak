...
class boss_doomrel : public CreatureScript
{
    public:
        ...
        struct boss_doomrelAI : public ScriptedAI
        {
            ...
            {
                _events.ScheduleEvent(EVENT_SHADOW_BOLT_VOLLEY, 10 * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_IMMOLATE,           18 * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_CURSE_OF_WEAKNESS,   5 * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_DEMONARMOR,         16 * IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SHADOW_BOLT_VOLLEY:
                            ...
                            _events.ScheduleEvent(EVENT_SHADOW_BOLT_VOLLEY, 12 * IN_MILLISECONDS);
                            break;
                        case EVENT_IMMOLATE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true))
                                DoCast(target, SPELL_IMMOLATE);
                            _events.ScheduleEvent(EVENT_IMMOLATE, 25 * IN_MILLISECONDS);
                            break;
                        ...
                        case EVENT_DEMONARMOR:
                            ...
                            _events.ScheduleEvent(EVENT_DEMONARMOR, 300 * IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
