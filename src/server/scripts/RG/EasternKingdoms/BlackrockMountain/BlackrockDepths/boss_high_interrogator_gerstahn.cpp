...
class boss_high_interrogator_gerstahn : public CreatureScript
{
    public:
        ...
        struct boss_high_interrogator_gerstahnAI : public ScriptedAI
        {
            ...
            {
                _events.ScheduleEvent(EVENT_SHADOW_WORD_PAIN, 4 * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_MANABURN,        14 * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_PSYCHIC_SCREAM,  32 * IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_SHADOWSHIELD,     8 * IN_MILLISECONDS);
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SHADOW_WORD_PAIN:
                            ...
                            _events.ScheduleEvent(EVENT_SHADOW_WORD_PAIN, 7 * IN_MILLISECONDS);
                            break;
                        case EVENT_PSYCHIC_SCREAM:
                            DoCastVictim(SPELL_PSYCHICSCREAM);
                            _events.ScheduleEvent(EVENT_PSYCHIC_SCREAM, 30 * IN_MILLISECONDS);
                            break;
                        case EVENT_MANABURN:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true))
                                DoCast(target, SPELL_MANABURN);
                            _events.ScheduleEvent(EVENT_MANABURN, 10 * IN_MILLISECONDS);
                            break;
                        case EVENT_SHADOWSHIELD:
                            DoCastSelf(SPELL_SHADOWSHIELD);
                            _events.ScheduleEvent(EVENT_SHADOWSHIELD, 25 * IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
