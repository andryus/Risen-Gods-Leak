    ...
    public:
        ...
        class spell_love_is_in_the_air_romantic_picnic_AuraScript : public AuraScript
        {
            ...
            void OnPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                target->VisitAnyNearbyObject<Player, Trinity::ContainerAction>(INTERACTION_DISTANCE*2, playerList, checker);
                for (std::list<Player*>::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
                {
                    if ((*itr) != target && (*itr)->HasAura(GetId())) // && (*itr)->GetStandState() == UNIT_STAND_STATE_SIT)
                    {
                        if (caster)
                        {
                            ...
                            {
                                caster->AddAura(SPELL_ROMANTIC_PICNIC_ACHIEV, *itr);
                                caster->AddAura(SPELL_ROMANTIC_PICNIC_ACHIEV, target);
                            }                            
                        }
                        foundSomeone = true;
                        // break;
