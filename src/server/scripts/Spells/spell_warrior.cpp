...
class spell_warr_damage_shield : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_damage_shield_AuraScript : public AuraScript
        {
            ...
            {
                if (auto procced_by = eventInfo.GetSpellInfo())
                {
                    static const uint32 SPELL_DAMAGESHIELD_ICON_ID = 3214;
                    // prevent endless recursion when triggered by itself
                    if (procced_by->SpellIconID == SPELL_DAMAGESHIELD_ICON_ID
                        || procced_by->Id == SPELL_WARRIOR_DAMAGE_SHIELD_DAMAGE)
                        return false;
                }

                if (Player* player = GetTarget()->ToPlayer())
                    if (Item* offhand = player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND))
                        if (offhand->GetTemplate()->InventoryType == INVTYPE_SHIELD)
                            return true;
            ...
            void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                GetTarget()->CastCustomSpell(SPELL_WARRIOR_DAMAGE_SHIELD_DAMAGE, SPELLVALUE_BASE_POINT0, damage, eventInfo.GetProcTarget(), false, NULL, aurEff);
            }

            void Register() override
            {
                DoCheckProc += AuraCheckProcFn(spell_warr_damage_shield_AuraScript::CheckProc);
    ...
    public:
        ...
        class spell_warr_overpower_SpellScript : public SpellScript
        {
            ...
            void HandleEffect(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Player* target = GetHitPlayer())
                    if (target->FindCurrentSpellBySpellId(SPELL_MIND_CONTROLE))
...
class spell_warr_retaliation : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_retaliation_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_warr_retaliation_SpellScript);

            SpellCastResult CheckCast()
            {
                if (Unit* caster = GetCaster())
                    if (caster->HasAura(SPELL_WARRIOR_SHIELD_WALL) || caster->HasAura(SPELL_WARRIOR_RECKLESSNESS))
            ...
            void Register()
            {
                OnCheckCast += SpellCheckCastFn(spell_warr_retaliation_SpellScript::CheckCast);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_warr_retaliation_SpellScript();
        }
...
class spell_warr_victory_rushSpellScript : public SpellScript
{
    PrepareSpellScript(spell_warr_victory_rushSpellScript);
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_warr_victory_rushSpellScript::HandleHitTarget, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
void AddSC_warrior_spell_scripts()
{
    ...
    new spell_warr_rend();
    new SpellScriptLoaderEx<spell_warr_victory_rushSpellScript>("spell_warr_victory_rush");
}
