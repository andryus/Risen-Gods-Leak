...
enum PriestSpells
{
    ...
    SPELL_PRIEST_GLYPH_OF_SHADOWFIEND               = 58227,
    ...
    public:
        ...
        class spell_pri_divine_aegis_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                // Priest T9 Healing P4
                if (AuraEffect const* bonus = eventInfo.GetActor()->GetAuraEffect(SPELL_PRIEST_T9_HEALING_4P, EFFECT_0))
                    AddPct(absorb, bonus->GetAmount());
    ...
    public:
        ...
        class spell_pri_guardian_spirit_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* /*aurEff*/, DamageInfo & dmgInfo, uint32 & absorbAmount)
            {
                ...
                // check if target of Guardian Spirit is a Tank-Paladin so Ardent Defender can procc first
                // check if one Rank of Ardent Defender is skilled                              and                check if skill is on Cooldown
                // Ardent Defender Rank1       Ardent Defender Rank2     Ardent Defender Rank3                          Ardent Defender Cooldown
                if ((target->HasAura(31850) || target->HasAura(31851) || target->HasAura(31852)) && !target->GetSpellHistory()->HasCooldown(66235))
                    return;
        ...
        class spell_pri_pain_and_suffering_proc_SpellScript : public SpellScript
        {
            ...
            void HandleEffectScriptEffect(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* unitTarget = GetHitUnit())
                    ...
                    {
                        if (Unit* caster = GetCaster())
                            caster->CastSoftRefreshSpell(unitTarget, aur->GetBase()->GetSpellInfo()); //! Set max duration back to 18sec, refresh the snapshot.*/
                    }
            }
        ...
        class spell_pri_penance_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* unitTarget = GetHitUnit())
                {
                    ...
                    GetSpell()->RestoreUsedModAuras(); // the "Decision" spell of penance should not waste any spell mods 
                }
            }
            ...
            void HandleApplyEffect(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    if (AuraEffect const* empoweredRenewAurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, PRIEST_ICON_ID_EMPOWERED_RENEW_TALENT, EFFECT_1))
                    {
                        ...
                        // Priest T9 Healing P4
                        if (AuraEffect const* bonus = caster->GetAuraEffect(SPELL_PRIEST_T9_HEALING_4P, EFFECT_0))
                            AddPct(basepoints0, bonus->GetAmount());
    ...
    public:
        ...
        class spell_pri_shadow_word_death_SpellScript : public SpellScript
        {
            ...
            void HandleAfterHit()
            {
                ...
                int32 absorb = 0;
                //! deal no damage to caster if target will die
                if (Unit* target = GetHitUnit())
                {
                    Unit::AuraEffectList const& auraList = target->GetAuraEffectsByType(AuraType(SPELL_AURA_SCHOOL_ABSORB));
                    if (!auraList.empty())
                    {
                        for (Unit::AuraEffectList::const_iterator itr = auraList.begin(); itr != auraList.end(); ++itr)
                            absorb += (*itr)->GetAmount();
                    }
                    if ((damage - absorb) > (int32)target->GetHealth())
                        damage = 0;
                }
...
class spell_pri_mind_control_AuraScript : public AuraScript
{
    ...
    void BeforeDispel(DispelInfo* dispelInfo)
    {
        _dispeller = dispelInfo->GetDispeller();
        _dispellerSpellId = dispelInfo->GetDispellerSpellId();
    }

    void HandleOnEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (!_dispellerSpellId || !canBeDispelled)
            return;
        ...
        {
            Player* charmer = owner->ToPlayer();
            if (!charmer)
                return;

            // make copy of this set since we modify it inside
            auto controlled = charmer->m_Controlled;
            for (Unit* control : controlled)
                if (control->GetAura(GetId(), charmer->GetGUID()))
                    control->RemoveAurasDueToSpellByDispel(GetId(), _dispellerSpellId, charmer->GetGUID(), _dispeller);
        }
    }

    void Register() override
    {
        OnDispel += AuraDispelFn(spell_pri_mind_control_AuraScript::BeforeDispel);
        OnEffectRemove += AuraEffectRemoveFn(spell_pri_mind_control_AuraScript::HandleOnEffectRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }

private:
    Unit* _dispeller = nullptr;
    uint32 _dispellerSpellId = 0;
    bool canBeDispelled = true;
};

class spell_pri_shadowfiend_deathAuraScript : public AuraScript
{
    PrepareAuraScript(spell_pri_shadowfiend_deathAuraScript);

    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& procInfo)
    {
        if (Unit* owner = GetUnitOwner())
            owner->CastSpell(owner, SPELL_PRIEST_GLYPH_OF_SHADOWFIEND, true);
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_pri_shadowfiend_deathAuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};

class spell_pri_tier_6_trinketAuraScript : public AuraScript
{
    PrepareAuraScript(spell_pri_tier_6_trinketAuraScript);
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& procInfo)
    {
        ...
        if (procInfo.GetSpellInfo()->SpellFamilyFlags[0] & 0x8000)
            GetCaster()->CastSpell(GetCaster(), SPELL_PRIEST_DIVINE_WRATH, true);
        else if (procInfo.GetSpellInfo()->SpellFamilyFlags[0] & 0x40)
            GetCaster()->CastSpell(GetCaster(), SPELL_PRIEST_DIVINE_BLESSING, true);
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_pri_tier_6_trinketAuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};

class spell_pri_glyph_of_dispel_magicSpellScript : public SpellScript
{
    PrepareSpellScript(spell_pri_glyph_of_dispel_magicSpellScript);

    void HandleHealTarget(SpellEffIndex /*effIndex*/)
    {
        if (!GetHitUnit() || !GetSpellInfo() || !GetSpellInfo()->GetBasePoints(EFFECT_0))
            return;

        SetHitHeal(CalculatePct(GetHitUnit()->GetMaxHealth(), GetSpellInfo()->GetBasePoints(EFFECT_0)));
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_pri_glyph_of_dispel_magicSpellScript::HandleHealTarget, EFFECT_0, SPELL_EFFECT_HEAL);
    }
};
...
void AddSC_priest_spell_scripts()
{
    ...
    new SpellScriptLoaderEx<spell_pri_mind_control_AuraScript>("spell_pri_mind_control"); // 605
    new SpellScriptLoaderEx<spell_pri_shadowfiend_deathAuraScript>("spell_pri_shadowfiend_death");
    new SpellScriptLoaderEx<spell_pri_tier_6_trinketAuraScript>("spell_pri_tier_6_trinket");
    new SpellScriptLoaderEx<spell_pri_glyph_of_dispel_magicSpellScript>("spell_pri_glyph_of_dispel_magic");
}
