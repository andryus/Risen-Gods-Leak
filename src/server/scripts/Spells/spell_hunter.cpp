...
enum HunterSpells
{
    ...
    SPELL_HUNTER_ASPECT_OF_THE_VIPER_ITEM_BONI      = 60144,
        ...
        class spell_hun_ascpect_of_the_viper_AuraScript : public AuraScript
        {
            ...
            bool Check(ProcEventInfo& eventInfo)
            {
                ...
                if (SpellInfo const* spellInfo = eventInfo.GetSpellInfo())
                    if (spellInfo->SpellFamilyFlags[0] == 0x2000)
                        return false;
                return true;
            }
            ...
            void Register() override
            {
                DoCheckProc += AuraCheckProcFn(spell_hun_ascpect_of_the_viper_AuraScript::Check);
...
class spell_hun_disengage : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_disengage_SpellScript : public SpellScript
        {
            ...
            void HandleOnCast()
            {
                GetCaster()->ToPlayer()->SetFallInformation(0, GetCaster()->GetPositionZ());
            }

            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_hun_disengage_SpellScript::CheckCast);
                OnCast += SpellCastFn(spell_hun_disengage_SpellScript::HandleOnCast);
            }
        };
    ...
    public:
        ...
        class spell_hun_masters_call_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                ...
                if (Pet* pet = GetCaster()->ToPlayer()->GetPet())
                    if (pet->HasUnitState(UNIT_STATE_CONFUSED) || pet->HasUnitState(UNIT_STATE_FLEEING) || pet->HasUnitState(UNIT_STATE_STUNNED))
                        return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;
...
class spell_hun_readiness : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_readiness_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                // mark this spell that his cooldown will removed after his effects end
                if (GetCaster()->HasAura(SPELL_HUNTER_FEIGN_DEATH))
                    GetCaster()->AddAura(SPELL_READINESS_MARK_FOR_FEIGN_DEATH, GetCaster());
    ...
    public:
        ...
        class spell_hun_tame_beast_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                ...
                if (!caster->ToPlayer()->GetPetCache().GetBySlot(PET_SAVE_NOT_IN_SLOT))
                    return SPELL_CAST_OK;
                auto freeSlot = caster->ToPlayer()->GetPetCache().GetFirstFreeStableSlot();
                if (freeSlot != PET_SAVE_AS_CURRENT && freeSlot <= caster->ToPlayer()->m_stableSlots)
                    return SPELL_CAST_OK;
                else
                    return SPELL_FAILED_ALREADY_HAVE_SUMMON;
            }
