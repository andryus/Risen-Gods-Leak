    ...
    public:
        ...
        class spell_item_blessing_of_ancient_kings_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                if (AuraEffect* protEff = eventInfo.GetProcTarget()->GetAuraEffect(SPELL_PROTECTION_OF_ANCIENT_KINGS, EFFECT_0)) 
                {
...
class spell_item_shadowmourne : public SpellScriptLoader
{
    public:
        ...
        class spell_item_shadowmourne_AuraScript : public AuraScript
        {
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                if (GetTarget()->ToPlayer()->InArena()) // cant collect shards while in Arena
                    return false;
            ...
            void Register() override
            {
                ...
                OnEffectRemove += AuraEffectRemoveFn(spell_item_shadowmourne_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };
...
//Shadowfragment logic for spellscripts
void HandleShadowFragmentsInSpellScripts(Unit* caster, Unit* target)
{
    if (!caster->ToPlayer() || caster->ToPlayer()->GetWeaponForAttack(BASE_ATTACK)->GetEntry() != 49623 || caster->ToPlayer()->InArena() || caster->HasAura(SPELL_SHADOWMOURNE_CHAOS_BANE_BUFF))
        return;

    caster->CastSpell(caster, SPELL_SHADOWMOURNE_SOUL_FRAGMENT, true, NULL);
    ...
    if (Aura* soulFragments = caster->GetAura(SPELL_SHADOWMOURNE_SOUL_FRAGMENT))
        ...
        if (soulFragments->GetStackAmount() >= 10)
        {
            caster->CastSpell(target, SPELL_SHADOWMOURNE_CHAOS_BANE_DAMAGE, true, NULL);
            soulFragments->Remove();
        }
    }
}
...
// -772 - Rend
class spell_warr_rend_rg : public SpellScriptLoader
{
public:
    spell_warr_rend_rg() : SpellScriptLoader("spell_warr_rend_rg") { }

    class spell_warr_rend_rg_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warr_rend_rg_SpellScript);

        void HandleScriptEffect(SpellEffIndex)
        {
            HandleShadowFragmentsInSpellScripts(GetCaster(), GetHitUnit());
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_warr_rend_rg_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_warr_rend_rg_SpellScript();
    }
};
...
{
public:
    spell_warr_thunder_clap() : SpellScriptLoader("spell_warr_thunder_clap_rg") { }
    ...
    class spell_warr_thunder_clap_SpellScript : public SpellScript
    {
        ...
        void CountTargets(std::list<WorldObject*>& targetList)
        {
            if (targetList.size() > 0)
                HandleShadowFragmentsInSpellScripts(GetCaster(), targetList.front()->ToUnit());
        }

        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_warr_thunder_clap_SpellScript::CountTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
...
{
public:
    spell_warr_hamstring() : SpellScriptLoader("spell_warr_hamstring_rg") { }

    class spell_warr_hamstring_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warr_hamstring_SpellScript);
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_warr_hamstring_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_warr_hamstring_SpellScript();
    }
};
...
{
public:
    spell_dk_plague_strike() : SpellScriptLoader("spell_dk_plague_strike_rg") { }
    ...
    class spell_dk_plague_strike_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_plague_strike_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_NORMALIZED_WEAPON_DMG);
        }
    };
...
{
public:
    spell_dk_howling_blast() : SpellScriptLoader("spell_dk_howling_blast_rg") { }
    ...
    class spell_dk_howling_blast_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_howling_blast_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
...
// -45477 - Ice-Touch
class spell_dk_ice_touch : public SpellScriptLoader
{
public:
    spell_dk_ice_touch() : SpellScriptLoader("spell_dk_ice_touch_rg") { }

    class spell_dk_ice_touch_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_ice_touch_SpellScript);
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_ice_touch_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_dk_ice_touch_SpellScript();
    }
};
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                else
                {
                    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
                        Aura::TryRefreshStackOrCreate(spellInfo, MAX_EFFECT_MASK, caster, caster);
                }
            }
    ...
    public:
        ...
        class spell_item_gift_of_the_harvester_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckRequirement()
            {
                ...
                GetCaster()->GetAllMinionsByEntry(ghouls, NPC_ITEM_GHOUL);
...
class spell_item_nitro_boots : public SpellScriptLoader
{
    public:
        ...
        class spell_item_nitro_boots_SpellScript : public SpellScript
        {
            ...
            {
                Unit* caster = GetCaster();
                bool cast = roll_chance_i(95);
                if (MapEntry const* map = sMapStore.LookupEntry(caster->GetMapId()))
                    if (map->addon == 0 || map->IsBattlegroundOrArena() || map->IsDungeon() || map->IsRaid())
                    {
                        if (cast)
                            caster->CastSpell(caster, SPELL_NITRO_BOOTS_SUCCESS, true, GetCastItem());
                        return;
                    }
                        
                caster->CastSpell(caster, cast ? SPELL_NITRO_BOOTS_SUCCESS : SPELL_NITRO_BOOTS_BACKFIRE, true, GetCastItem());
            }
...
public:
    ...
    class mad_alchemists_potion_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            BeforeCast += SpellCastFn(mad_alchemists_potion_SpellScript::SecondaryEffect);
        }
...
public:
    ...
    class crazy_alchemists_potion_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            BeforeCast += SpellCastFn(crazy_alchemists_potion_SpellScript::SecondaryEffect);
        }
...
enum PurifiedShardOfTheScaleSetBonusSpells
{
    SPELL_CAUTERIZING_HEAL  = 69733,
    SPELL_SEARING_FLAMES    = 69729,
};

class spell_item_purified_shard_of_the_scale_2p_bonusAuraScript : public AuraScript
{
    PrepareAuraScript(spell_item_purified_shard_of_the_scale_2p_bonusAuraScript);
    ...
    void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& procInfo)
    {
        ...
        if (Unit* target = procInfo.GetActionTarget())
        {
            if (procInfo.GetDamageInfo())
                GetCaster()->CastSpell(target, SPELL_SEARING_FLAMES, true);
            if (procInfo.GetHealInfo())
                GetCaster()->CastSpell(target, SPELL_CAUTERIZING_HEAL, true);
        }
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_item_purified_shard_of_the_scale_2p_bonusAuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};

enum ShinyShardOfTheScaleSetBonusSpells
{
    SPELL_CAUTERIZING_HEAL_25   = 69734,
    SPELL_SEARING_FLAMES_25     = 69730,
};

class spell_item_shiny_shard_of_the_scale_2p_bonusAuraScript : public AuraScript
{
    PrepareAuraScript(spell_item_shiny_shard_of_the_scale_2p_bonusAuraScript);
    ...
    void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& procInfo)
    {
        ...
        {
            if (procInfo.GetDamageInfo())
                GetCaster()->CastSpell(target, SPELL_CAUTERIZING_HEAL_25, true);
            if (procInfo.GetHealInfo())
                GetCaster()->CastSpell(target, SPELL_SEARING_FLAMES_25, true);
        }
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_item_shiny_shard_of_the_scale_2p_bonusAuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};

enum NevermeltingIceCrystal
{
    SPELL_DEADLY_PRECISION = 71564
};

class spell_item_nevermelting_ice_crystal_AuraScript : public AuraScript
{
    PrepareAuraScript(spell_item_nevermelting_ice_crystal_AuraScript);

    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        Unit* target = GetTarget();
        target->CastSpell(target, SPELL_DEADLY_PRECISION, true, GetAura()->GetCastItem());
        if (Aura* newAura = target->GetAura(SPELL_DEADLY_PRECISION, target->GetGUID()))
            newAura->SetStackAmount(newAura->GetSpellInfo()->StackAmount);
    }

    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_item_nevermelting_ice_crystal_AuraScript::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};
...
void AddSC_item_spell_scripts()
{
    ...
    new SpellScriptLoaderEx<spell_item_purified_shard_of_the_scale_2p_bonusAuraScript>("spell_item_purified_shard_of_the_scale_2p_bonus");
    new SpellScriptLoaderEx<spell_item_shiny_shard_of_the_scale_2p_bonusAuraScript>("spell_item_shiny_shard_of_the_scale_2p_bonus");
    new SpellScriptLoaderEx<spell_item_nevermelting_ice_crystal_AuraScript>("spell_item_nevermelting_ice_crystal");
}
