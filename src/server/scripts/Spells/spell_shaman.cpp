    ...
    public:
        ...
        class spell_sha_earth_shield_AuraScript : public AuraScript
        {
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool & /*canBeRecalculated*/)
            {
                ...
                {
                    amount = caster->SpellHealingBonusDone(NULL, sSpellMgr->GetSpellInfo(SPELL_SHAMAN_EARTH_SHIELD_HEAL), amount, HEAL);
                    ...
                    if (AuraEffect* talentEarthShield = caster->GetAuraEffectOfRankedSpell(51561, EFFECT_1, caster->GetGUID()))
                        AddPct(amount, talentEarthShield->GetAmount());

                    //! Improved Shields
                    if (AuraEffect* improvedShields = caster->GetAuraEffectOfRankedSpell(16261, EFFECT_1, caster->GetGUID()))
                        AddPct(amount, improvedShields->GetAmount());
                }
            }
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
            {
                ...
                if (Unit* caster = GetCaster())
                    caster->CastCustomSpell(SPELL_SHAMAN_EARTH_SHIELD_HEAL, SPELLVALUE_BASE_POINT0, aurEff->GetAmount(), GetTarget(), true, NULL, aurEff, GetCasterGUID());
...
class spell_sha_elemntal_shields : public SpellScriptLoader
{
    public:
        spell_sha_elemntal_shields() : SpellScriptLoader("spell_sha_elemntal_shields") { }

        class spell_sha_elemntal_shields_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_sha_elemntal_shields_AuraScript);

            bool Check(ProcEventInfo& eventInfo)
            {
                if (SpellInfo const* spellInfo = eventInfo.GetSpellInfo())
                    // anub'arak leeching swarm
                    if (spellInfo->Id == 66240)
                        return false;
                return true;
            }
            ...
            void Register()
            {
                DoCheckProc += AuraCheckProcFn(spell_sha_elemntal_shields_AuraScript::Check);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_sha_elemntal_shields_AuraScript();
        }
};
...
void AddSC_shaman_spell_scripts()
{
    ...
    new spell_sha_elemntal_shields();
}
