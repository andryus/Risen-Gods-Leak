...
enum PaladinSpells
{
    ...
    SPELL_PALADIN_DIVINE_PURPOSE_R1              = 31871,
    SPELL_PALADIN_DIVINE_PURPOSE_R2              = 31872,
};
        ...
        class spell_pal_ardent_defender_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* aurEff, DamageInfo & dmgInfo, uint32 & absorbAmount)
            {
                ...
                if (remainingHealth <= 0 && !victim->GetSpellHistory()->HasCooldown(PAL_SPELL_ARDENT_DEFENDER_HEAL))
                {
                    ...
                    uint32 baseDef = victim->GetMaxSkillValueForLevel();
                    // No heal when defense skill is baseDef (level * 5)
                    // Max heal when defense skill denies critical hits from raid bosses (baseDef + 140)
                    float defenseFactor = std::min(1.0f, (defenseSkillValue - baseDef) / 140.0f);

                    int32 healAmount = int32(victim->CountPctFromMaxHealth(static_cast<uint32>(lroundf(healPct * defenseFactor))));
    ...
    public:
        ...
        class spell_pal_divine_sacrifice_AuraScript : public AuraScript
        {
            ...
            void Split(AuraEffect* /*aurEff*/, DamageInfo & /*dmgInfo*/, uint32 & splitAmount)
            {
                ...
                // it could break when absorbed everything , or if the casters hp drops below 20%
                if (Unit* caster = GetCaster())
            ...
            {
                if (Unit* caster = GetCaster())
                    if (caster->GetHealthPct() < minHpPct)
                        caster->RemoveAura(SPELL_PALADIN_DIVINE_SACRIFICE);
            }
            ...
            void Register() override
            {
                ...
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_pal_divine_sacrifice_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_SPLIT_DAMAGE_PCT);
            }
        };
    ...
    public:
        ...
        class spell_pal_hand_of_sacrifice_AuraScript : public AuraScript
        {
            ...
            void Split(AuraEffect* /*aurEff*/, DamageInfo & /*dmgInfo*/, uint32 & splitAmount)
            {
                ...
                {
                    GetCaster()->RemoveAura(SPELL_PALADIN_HAND_OF_SACRIFICE);
                    if (Unit* target = GetTarget())
                        target->RemoveAura(SPELL_PALADIN_HAND_OF_SACRIFICE);
                }
            }
            ...
            void Register() override
            {
                ...
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_pal_hand_of_sacrifice_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_SPLIT_DAMAGE_PCT);
            }
        };
    ...
    public:
        ...
        class spell_pal_light_s_beacon_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                // get total healing taken mods on healed target
                float TakenTotalMod = 1.0f;
                float minval = float(eventInfo.GetProcTarget()->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_HEALING_PCT));
                if (minval)
                    AddPct(TakenTotalMod, minval);
                int32 areaMaxAmount = 0;
                std::list<AuraEffect*> const& mHealingPct = eventInfo.GetProcTarget()->GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_PCT);
                for (std::list<AuraEffect*>::const_iterator i = mHealingPct.begin(); i != mHealingPct.end(); ++i)
                {
                    if ((*i)->GetAmount() > 0)
                    {
                ...
                // calculate amount without mods
                TakenTotalMod *= 100.0f;
                TakenTotalMod = 100.0f / TakenTotalMod;
                heal *= TakenTotalMod;
...
class spell_pal_hand_of_freedomSpellScript : public SpellScript
{
    PrepareSpellScript(spell_pal_hand_of_freedomSpellScript);
    ...
    void HandleEffectHit(SpellEffIndex /*effIndex*/)
    {
        ...
        if (GetCaster()->HasAura(SPELL_PALADIN_DIVINE_PURPOSE_R1))
            chance = 50;
        else if (GetCaster()->HasAura(SPELL_PALADIN_DIVINE_PURPOSE_R2))
            chance = 100;
        else
            return;

        if(roll_chance_i(chance))
            GetExplTargetUnit()->RemoveAurasWithMechanic(1 << MECHANIC_STUN, AURA_REMOVE_BY_ENEMY_SPELL);
    }

    void Register() override
    {
        OnEffectHit += SpellEffectFn(spell_pal_hand_of_freedomSpellScript::HandleEffectHit, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
    }
};
...
void AddSC_paladin_spell_scripts()
{
    ...
    new SpellScriptLoaderEx<spell_pal_hand_of_freedomSpellScript>("spell_pal_hand_of_freedom");
}
