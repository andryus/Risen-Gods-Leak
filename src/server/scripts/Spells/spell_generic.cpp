...
enum Bonked
{
    NPC_OLYMPIA_KILL_CREDIT = 1010751,
    SPELL_BONKED            = 62991,
        ...
        class spell_gen_bonked_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                if (Player* target = GetHitPlayer())
                {
                    ...
                    {
                        static std::unordered_map<ObjectGuid, std::unordered_set<ObjectGuid>*> victories;

                        std::unordered_set<ObjectGuid> *playerHit;
                        auto itr = victories.find(GetCaster()->GetGUID());
                        if (itr != victories.end())
                            playerHit = itr->second;
                        else
                        {
                            playerHit = new std::unordered_set<ObjectGuid>();
                            victories[GetCaster()->GetGUID()] = playerHit;
                        }
                        if (playerHit->find(target->GetGUID()) == playerHit->end())
                        {
                            playerHit->insert(target->GetGUID());
                            GetCaster()->ToPlayer()->KilledMonsterCredit(NPC_OLYMPIA_KILL_CREDIT);
                        }
                    }
    ...
    public:
        ...
        class spell_gen_cannibalize_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckIfCorpseNear()
            {
                ...
                WorldObject* result = caster->VisitSingleNearbyObject<WorldObject, true>(max_range, check);
                if (!result)
...
class spell_gen_count_pct_from_max_hp : public SpellScriptLoader
{
    public:
        ...
        public:
            ...
            {
                if (GetHitUnit()->GetEntry() == NPC_SKADI_RUTHLESS)
                    return;
        ...
        public:
            ...
            void HandleMount(SpellEffIndex effIndex)
            {
                ...
                if (Player* target = GetHitPlayer())
                {
                    ...
                    {
                        PreventHitAura();
                        target->GetSpellHistory()->ResetCooldown(mount, false);
            ...
            bool canFly(Player* player)
            {
                ...
                // Nothing to do with SpellInfo::CheckLocation, dont listen to Trinity
                switch (GetVirtualMapForMapAndZone(player->GetMapId(), zoneid))
                {
                case 530:
                    break;
                case 571:
                ...
                // Following checks are duplicate from Spell::CheckCast
                AreaTableEntry const* area = sAreaTableStore.LookupEntry(player->GetAreaId());
                if (!area || area->flags & AREA_FLAG_NO_FLY_ZONE)
                    return false;

                // Wintergrasp no-fly zone
...
class spell_gen_mounted_charge: public SpellScriptLoader
{
    public:
        ...
        class spell_gen_mounted_charge_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                if (GetSpellInfo()->Id == SPELL_CHARGE_TRIGGER_FACTION_MOUNTS
                    || GetSpellInfo()->Id == SPELL_CHARGE_TRIGGER_TRIAL_CHAMPION)
                {
                    if (Unit* target = GetExplTargetUnit())
                    {
                        ...
                        for (Unit::AuraApplicationMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                        {
                            if (Aura* aura = itr->second->GetBase())
                            {
                                ...
                                if (auraInfo && auraInfo->SpellIconID == 2007 && aura->HasEffectType(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN))
                                {
                                    ...
                                    if (Unit* rider = target->GetCharmer())
                                        if (Aura* defend = rider->GetAura(aura->GetId()))
                                            defend->ModStackAmount(GetCaster()->GetEntry() == 34127 ? -2 : -1, AURA_REMOVE_BY_ENEMY_SPELL);
                                    break;
                                }
                            }
                        }
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_gen_mounted_charge_SpellScript::CheckCast);
                SpellInfo const* spell = sSpellMgr->EnsureSpellInfo(m_scriptSpellId);
    ...
    public:
        ...
        class spell_gen_pet_summoned_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    if (Pet* newPet = Pet::Load(*player, player->GetLastPetNumber(), true, 0))
                    {
                        // revive the pet if it is dead
                        if (newPet->getDeathState() == CORPSE)
        ...
        class spell_gen_spirit_healer_res_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /* effIndex */)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    originalCaster->SendDirectMessage(std::move(data));
                }
            }
    ...
    public:
        ...
        class spell_gen_turkey_marker_AuraScript : public AuraScript
        {
            ...
            void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                _applyTimes.push_back(game::time::GetGameTimeMS());
                Unit* target = GetTarget();
            ...
            void OnPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                if (_applyTimes.front() + GetMaxDuration() < game::time::GetGameTimeMS())
...
class spell_gen_critter_bite : public SpellScriptLoader
{
public:
    spell_gen_critter_bite() : SpellScriptLoader("spell_gen_critter_bite") { }

    class spell_gen_critter_bite_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gen_critter_bite_AuraScript);

        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_CRITTER_BITE))
                return false;
            return true;
        }
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* target = GetTarget())
                if (Unit* caster = GetCaster())
                {
                    ...
                    target->GetMotionMaster()->MoveFollow(caster, 2.5f, -90.0f);
                }
        }
        ...
        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(spell_gen_critter_bite_AuraScript::OnApply, EFFECT_0, SPELL_AURA_AOE_CHARM, AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(spell_gen_critter_bite_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_AOE_CHARM, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_gen_critter_bite_AuraScript();
    }
};

class spell_gen_npc_jump_attack : public SpellScriptLoader
{
public:
    spell_gen_npc_jump_attack() : SpellScriptLoader("spell_gen_npc_jump_attack") { }

    class spell_gen_npc_jump_attack_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_npc_jump_attack_SpellScript);

        SpellCastResult CheckRequirement()
        {
            if (!GetCaster() || GetCaster()->HasUnitState(UNIT_STATE_ROOT))
                return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;
            return SPELL_CAST_OK;
        }
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(spell_gen_npc_jump_attack_SpellScript::CheckRequirement);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_gen_npc_jump_attack_SpellScript();
    }
};
...
void AddSC_generic_spell_scripts()
{
    ...
    new spell_gen_count_pct_from_max_hp("spell_gen_1pct_count_pct_from_max_hp", 1);
    ...
    new spell_gen_critter_bite();
    new spell_gen_npc_jump_attack();
}
