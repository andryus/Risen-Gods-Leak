...
_status(STATUS_CONNECTED), _build(0), _expversion(0), _db_busy(false),
...
void AuthSession::Start()
{
    ...
    LogStatus("connecting", "connect", "New connection established");
    ...
    LoginDatabase.AsyncQuery(stmt)
            .via(_executor).then([this](PreparedQueryResult result) { CheckIpCallback(std::move(result)); })
            .then([this]() { _db_busy = false; });
}
...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            ...
            LogStatus("connecting", "reject", "connection closed", "banned (ip)");
            return;
        }
    }
...
{
    if (IsOpen() && !packet.empty())
        QueuePacket(packet.Move());
}
...
bool AuthSession::HandleLogonChallenge()
{
    ...
    LogStatus("authentication", "check user", "Recieved new authentication challenge", "", login);
    ...
    if (_db_busy)
    {
        ...
        LogStatus("authentication", "reject", "Authentication challenge failed, retry too fast", "retry too early", login);
        return true;
    }
    ...
    _db_busy = true;
    LoginDatabase.AsyncQuery(stmt)
            .via(_executor).then([this](PreparedQueryResult result) { LogonChallengeCallback(std::move(result)); })
            .then([this]() { _db_busy = false; });
    return true;
}
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result)
{
    ...
    if (!result)
    {
        LogStatus("authentication", "reject", "Authentication challenge failed, unknown user", "unknown user");
    ...
    if (_accountInfo.IsLockedToIP)
    {
        ...
        {
            LogStatus("authentication", "reject", "Authentication challenge failed, ip lock check failed", "ip locked");
    ...
    else
    {
        ...
        else if (!_accountInfo.LockCountry.empty() && !_ipCountry.empty())
        {
            ...
            {
                LogStatus("authentication", "reject", "Authentication challenge failed, country lock check failed", "country locked");
    ...
    if (_accountInfo.IsBanned)
    {
        if (_accountInfo.IsPermanenetlyBanned)
        {
            ...
            LogStatus("authentication", "reject", "Authentication challenge failed, permanently banned", "banned (perm)");
            return;
        }
        else
        {
            ...
            LogStatus("authentication", "reject", "Authentication challenge failed, time banned", "banned (time)");
            return;
        }
    }
...
bool AuthSession::HandleLogonProof()
{
    ...
    if (_expversion == NO_VALID_EXP_FLAG)
    {
        ...
        LogStatus("authentication", "reject", "Authentication challenge failed, unsupported expansion", "unsupported expansion");
    ...
    if (!memcmp(M.AsByteArray(sha.GetLength()).get(), logonProof->M1, 20))
    {
        ...
        LogStatus("authentication", "success", "Authentication challenge succeeded");
        ...
        if ((logonProof->securityFlags & 0x04) || !_tokenKey.empty())
        {
            ...
            {
                LogStatus("authentication", "reject", "Authentication challenge failed, authenticator token mismatch", "token mismatch");
                ByteBuffer packet(8*4);
    ...
    else
    {
        ...
        LogStatus("authentication", "reject", "Authentication challenge failed, wrong password", "wrong password");
        ...
        if (MaxWrongPassCount > 0)
        {
            ...
            if (_accountInfo.FailedLogins >= MaxWrongPassCount)
            {
                ...
                LogStatus("authentication",
                          WrongPassBanType ? "flood ban (account)" : "flood ban (ip)",
                          "Account time banned due to too many failed authentication attemps");
            }
        }
    }
...
bool AuthSession::HandleReconnectChallenge()
{
    ...
    LogStatus("reconnect", "check session", "New reconnect challenge", "", login);
    ...
    if (_db_busy)
    {
        ...
        LogStatus("reconnect", "reject", "Reconnect challenge failed, retry too fast", "retry too early", login);
        return true;
    }
    ...
    _db_busy = true;
    LoginDatabase.AsyncQuery(stmt)
            .via(_executor).then([&](PreparedQueryResult result) { ReconnectChallengeCallback(std::move(result)); })
            .then([this]() { _db_busy = false; });
    return true;
}
...
void AuthSession::ReconnectChallengeCallback(PreparedQueryResult result)
{
    ...
    if (!result)
    {
        LogStatus("reconnect", "reject", "Reconnect challenge failed, unknown user", "unknown user");
        pkt << uint8(WOW_FAIL_UNKNOWN_ACCOUNT);
        SendPacket(std::move(pkt));
        return;
    }
...
bool AuthSession::HandleReconnectProof()
{
    ...
    {
        LogStatus("reconnect", "success", "Reconnect challenge succeeded");
    ...
    else
    {
        ...
        LogStatus("reconnect", "reject", "Reconnect challenge failed, session is invalid", "invalid session");
        return false;
    }
}
...
tcp::endpoint const GetAddressForClient(Realm const& realm, ip::address const& clientAddr)
{
    ...
    {
        if (Trinity::Net::IsInNetwork(clientAddr.to_v4(), realm.LocalSubnetMask.to_v4(), realm.LocalAddress.to_v4()))
            realmIp = realm.LocalAddress;
        else
...
//    LogStatus("realmlist", "query", "Sending realmlist");
    if (_db_busy)
    {
    ...
    _db_busy = true;
    LoginDatabase.AsyncQuery(stmt)
            .via(_executor).then([&](PreparedQueryResult result) { RealmListCallback(std::move(result)); })
            .then([this](){_db_busy = false; });
    return true;
}
...
void AuthSession::RealmListCallback(PreparedQueryResult result)
{
    ...
    ByteBuffer hdr(1 + 2 + pkt.size() + RealmListSizeBuffer.size());
    hdr << uint8(REALM_LIST);
    hdr << uint16(pkt.size() + RealmListSizeBuffer.size());
    hdr.append(RealmListSizeBuffer);                        // append RealmList's size buffer
    hdr.append(pkt);                                        // append realms in the realmlist
    SendPacket(std::move(hdr));
}
...
void AuthSession::LogStatus(const std::string& step,
                            const std::string& action,
                            const std::string& message,
                            const std::string& reason,
                            const std::string& login) const
{
    auto output = sGraylog->GetOutput();
    if (!output)
        return;
    ...
    builder(message);
    builder("authenticate_user_ip", GetRemoteIpAddress().to_string());
    builder("authenticate_user_step", step);
    builder("authenticate_user_action", action);
    if (!reason.empty())
        builder("authenticate_user_reason", reason);

    if (_accountInfo.Id)
    {
        builder("authenticate_user_id", _accountInfo.Id);
        builder("authenticate_user_account", _accountInfo.Login);
        builder("authenticate_user_security", uint32(_accountInfo.SecurityLevel));
        builder("authenticate_user_failed_logins", _accountInfo.FailedLogins);
        builder("authenticate_user_banned", _accountInfo.IsBanned ? (_accountInfo.IsPermanenetlyBanned ? "permanent"s : "time"s) : "free"s);
    }
    else if (!login.empty())
    {
        builder("authenticate_user_account", login);
    }
    output->Write(builder);
}
#else
void AuthSession::LogStatus(const std::string&,
                            const std::string&,
                            const std::string&,
                            const std::string&,
