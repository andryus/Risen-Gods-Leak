...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_INS_GAME_TELE, "INSERT INTO game_tele (position_x, position_y, position_z, orientation, map, name) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(WORLD_DEL_GAME_TELE, "DELETE FROM game_tele WHERE name = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_DEL_GAME_TELE_NAMES, "DELETE FROM game_tele_names WHERE id = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(WORLD_SEL_COMMANDS, "SELECT id, sorting_key, name, parent, permission, flags, script_name FROM command", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_COMMAND_HELP, "SELECT id, locale, syntax, description FROM command_help", CONNECTION_SYNCH);
    ...
    PrepareStatement(WORLD_LOAD_FAC_CHANGE_TITLES, "SELECT alliance_ingameid, horde_ingameid, action FROM player_factionchange_titles", CONNECTION_SYNCH);
    PrepareStatement(WORLD_LOAD_ITEMS_FOR_QUEST, "SELECT requiredraces, SourceItemId,  requireditemid1, requireditemid2, requireditemid3, requireditemid4, requireditemid5, requireditemid6 FROM quest_template where id = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_LOAD_FAC_CHANGE_HOMEBIND, "SELECT map, zone, position_x, position_y, position_z FROM player_factionchange_homebinds WHERE race = ?", CONNECTION_SYNCH);
}
