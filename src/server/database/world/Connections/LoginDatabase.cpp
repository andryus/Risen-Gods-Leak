...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_NAME, "SELECT a.id, a.sessionkey, a.last_ip, a.locked, a.lock_country, a.expansion, a.locale, a.recruiter, a.os, aa.gmLevel, "
    ...
    PrepareStatement(LOGIN_INS_REALM_CHARACTERS, "INSERT INTO realmcharacters (numchars, maxlevel, acctid, realmid) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_SEL_PINFO,
        "SELECT a.username, aa.gmlevel, a.email, a.reg_mail, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), TIME_TO_SEC(am.timeLeft), a.failed_logins, a.locked, a.OS, "
        "ab.unbandate, ab.bandate = ab.unbandate, ab.bannedby, ab.banreason "
        "FROM account a "
        "LEFT JOIN account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) "
        "LEFT JOIN account_banned ab ON (a.id = ab.id AND ab.active = 1) "
        "LEFT JOIN account_mute am ON a.id = am.account "
        "WHERE a.id = ? "
    ...
    PrepareStatement(LOGIN_SEL_MAXLEVEL, "SELECT MAX(maxlevel) FROM realmcharacters WHERE acctid = ?", CONNECTION_ASYNC);

    // UUACP logging
    PrepareStatement(LOGIN_INS_UUACP_LOG, "INSERT INTO uuacp_log (realmId, accountId, charGUID, ip, time, action) VALUES (?, ?, ?, ?, UNIX_TIMESTAMP(NOW()), ?)", CONNECTION_ASYNC);
    ...
    // Mute
    PrepareStatement(LOGIN_REP_MUTE, "REPLACE INTO account_mute (account, timeLeft) VALUES (?, SEC_TO_TIME(?))", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_INSUPD_MUTE, "INSERT INTO account_mute (account, timeLeft) VALUES (?, SEC_TO_TIME(?)) ON DUPLICATE KEY UPDATE timeLeft = GREATEST(timeLeft, SEC_TO_TIME(?))", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_DEL_MUTE, "DELETE FROM account_mute WHERE account = ?", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_SEL_MUTE, "SELECT TIME_TO_SEC(timeLeft) FROM account_mute WHERE account = ?", CONNECTION_ASYNC);
    
    PrepareStatement(LOGIN_API_SEL_ACCOUNT_INFO, 
        "SELECT a.username, aa.gmlevel, TIME_TO_SEC(am.timeLeft), ab.unbandate, ab.bandate = ab.unbandate, ab.bannedby, ab.banreason "
        "FROM account a "
        "LEFT JOIN account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) "
        "LEFT JOIN account_banned ab ON (a.id = ab.id AND ab.active = 1) "
        "LEFT JOIN account_mute am ON a.id = am.account "
...
#else
    PrepareStatement(LOGIN_SEL_CHECK_PASSWORD, "SELECT 1 FROM DUAL WHERE account_check_pass_hash(?, ?)", CONNECTION_SYNCH);
#endif
}
