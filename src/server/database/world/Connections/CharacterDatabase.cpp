...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_GUID_BY_NAME, "SELECT guid FROM characters WHERE name = ? AND (at_login & 0x1) = 0", CONNECTION_BOTH);
    PrepareStatement(CHAR_SEL_CHECK_NAME, "SELECT 1 FROM characters WHERE name = ? AND (at_login & 0x1) = 0", CONNECTION_BOTH);
    ...
    PrepareStatement(CHAR_SEL_MAXLEVEL, "SELECT MAX(level) FROM characters WHERE account = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHAR_CREATE_INFO, "SELECT race, class FROM characters WHERE account = ? LIMIT 0,?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_GUID_BY_NAME_FILTER, "SELECT guid, name FROM characters WHERE name LIKE CONCAT('%%', ?, '%%') AND (at_login & 0x1) = 0", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_ENUM_DECLINED_NAME, "SELECT c.guid, c.name, c.race, c.class, c.gender, c.playerBytes, c.playerBytes2, c.level, c.zone, c.map, "
                     ...
                     "LEFT JOIN character_banned AS cb ON c.guid = cb.guid AND cb.active = 1 WHERE c.account = ?  AND c.deleteInfos_Name IS NULL ORDER BY c.guid", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_FREE_NAME, "SELECT guid, name FROM characters WHERE guid = ? AND account = ? AND (at_login & ?) = ? AND NOT EXISTS (SELECT NULL FROM characters WHERE name = ? AND (at_login & 0x1) = 0)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_RESERVED_NAME, "INSERT INTO reserved_name (name) VALUES (?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_RESERVED_NAME, "DELETE FROM reserved_name WHERE name = (?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_AUCTIONS, "SELECT id, auctionHouseId, itemguid, itemEntry, count, itemowner, buyoutprice, time, buyguid, lastbid, startbid, deposit FROM auctionhouse ah INNER JOIN item_instance ii ON ii.guid = ah.itemguid", CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_AUCTION, "INSERT INTO auctionhouse (id, auctionHouseId, itemguid, itemowner, buyoutprice, time, buyguid, lastbid, startbid, deposit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_PENDING_SALE, "INSERT INTO auctionhouse_pending_sales (id, itemEntry, count, seller, buyer, bid, buyout, time, auctionHouseId, deposit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_PENDING_SALE, "DELETE FROM auctionhouse_pending_sales WHERE id = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_EMPTY_EXPIRED_MAIL, "DELETE FROM mail WHERE expire_time < ? AND has_items = 0 AND body = ''", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_NAME, "SELECT name, at_login FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_REALM_RECORD, "SELECT account, COUNT(guid), MAX(level) FROM characters WHERE account = ? GROUP BY account UNION SELECT ?, 0, 0", CONNECTION_ASYNC);
 //   PrepareStatement(CHAR_SEL_CHARACTER_COUNT, "SELECT account, COUNT(guid) FROM characters WHERE account = ? GROUP BY account", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_GUILD_BANK_ITEMS, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, "
        "guildid, TabId, SlotId, item_guid, itemEntry FROM guild_bank_item gbi INNER JOIN item_instance ii ON gbi.item_guid = ii.guid WHERE gbi.guildid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_ACCOUNT_COMPLAINTS, "SELECT sourceId, UNIX_TIMESTAMP(expirationTime) FROM account_complaints WHERE targetId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_ACCOUNT_COMPLAINTS, "REPLACE INTO account_complaints (sourceId, targetId, expirationTime) VALUES (?, ?, FROM_UNIXTIME(?))", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_INSTANCE_ENCOUNTER_SAVE, "INSERT INTO instance_encounter_state (id, encounter, tag, value) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_INSTANCE_ENCOUNTER_STATE, "SELECT tag, value FROM instance_encounter_state WHERE id = ? AND encounter = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_INSTANCE_ENCOUNTERS, "DELETE FROM instance_encounter_state WHERE id=?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_INS_INSTANCE_EVENT_SAVE, "INSERT INTO instance_event_state (id, event_name, data) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_INSTANCE_EVENT_STATE, "SELECT event_name, data FROM instance_event_state WHERE id = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_INSTANCE_EVENTS, "DELETE FROM instance_event_state WHERE id=?", CONNECTION_ASYNC);
    ...
    // GM Tickets
    PrepareStatement(CHAR_SEL_OPEN_TICKETS, "SELECT id, type, is_follow_up, map_id, pos_x, pos_y, pos_z, UNIX_TIMESTAMP(creation_time), author, viewed, completed, escalated, assignee, message, comment, response FROM ticket WHERE closer IS NULL", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MAX_TICKET_ID, "SELECT MAX(id) FROM ticket", CONNECTION_SYNCH);
    PrepareStatement(CHAR_REP_TICKET, "REPLACE INTO ticket (id, type, is_follow_up, map_id, pos_x, pos_y, pos_z, creation_time, author, author_name, viewed, completed, escalated, assignee, closer, message, comment, response) VALUES (?, ?, ?, ?, ?, ?, ?, FROM_UNIXTIME(?), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_TICKETS_BY_AUTHOR, "DELETE FROM ticket WHERE author = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_ALL_GM_TICKETS, "DELETE FROM ticket", CONNECTION_ASYNC);

    // GM Survey/subsurvey
    PrepareStatement(CHAR_SEL_MAX_SURVEY_ID, "SELECT MAX(surveyId) FROM ticket_survey", CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_TICKET_SURVEY, "INSERT INTO ticket_survey (guid, surveyId, mainSurvey, overallComment, createTime) VALUES (?, ?, ?, ?, UNIX_TIMESTAMP(NOW()))", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_TICKET_SUBSURVEY, "INSERT INTO ticket_subsurvey (surveyId, subsurveyId, rank, comment) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_EXPIRED_AUCTIONS, "SELECT id, auctionHouseId, itemguid, itemEntry, count, itemowner, buyoutprice, time, buyguid, lastbid, startbid, deposit FROM auctionhouse ah INNER JOIN item_instance ii ON ii.guid = ah.itemguid WHERE ah.time <= ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHAR_PINFO, "SELECT totaltime, level, money, account, race, class, map, zone, gender, health, playerFlags FROM characters WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_GUID_BY_NAME, "SELECT guid FROM characters WHERE name = ? AND (at_login & 0x1) = 0", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_INS_CHAR_ACHIEVEMENT_REALM_FIRST, "INSERT INTO character_achievement_realmfirst (guid, achievement, date) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INSUPD_CHAR_ACHIEVEMENT_PROGRESS, "INSERT INTO character_achievement_progress (guid, criteria, counter, date) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE counter = VALUES(counter), date = VALUES(date)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CHAR_ACHIEVEMENTS, "DELETE FROM character_achievement WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHAR_XP, "UPDATE characters SET level = ?, xp = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_XP_AND_MONEY, "UPDATE characters SET level = ?, xp = ?, money = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    //Custom statements for factionchange
    PrepareStatement(CHAR_SEL_CHECK_FACTIONCHANGE, "SELECT 1 FROM auctionhouse WHERE itemowner = ? UNION SELECT 2 FROM mail WHERE receiver = ? UNION SELECT 4 FROM guild WHERE leaderguid = ? UNION SELECT 8 FROM arena_team WHERE captainGUID = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CURR_TITLE, "UPDATE characters SET chosenTitle = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_TITLE_MASK, "UPDATE characters SET knowntitles = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_KNOWN_TITLES, "SELECT knownTitles FROM characters where guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_QUEST_STATUS, "SELECT quest FROM character_queststatus where guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_QUEST_ITEM_ITEM_INSTANCE, "SELECT guid FROM item_instance where itemEntry = ? AND owner_guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_QUEST_ITEM_FROM_INVENTORY, "DELETE FROM character_inventory where item = ? AND guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_QUEST_ITEM_FROM_ITEM_INSTANCE, "DELETE FROM item_instance where guid = ? AND owner_guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_SURCOAT, "DELETE FROM item_instance WHERE itemEntry = 5976 AND owner_guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_ARENA_STATS, "DELETE FROM character_arena_stats WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_LOAD_PLAYER_QUEST_REW, "SELECT quest FROM character_queststatus_rewarded WHERE guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_PLAYER_QUEST_REW, "DELETE FROM character_queststatus_rewarded WHERE guid = ? AND quest = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_PLAYER_QUEST_REW, "UPDATE character_queststatus_rewarded SET active = ? WHERE guid = ? AND quest = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_PLAYER_QUEST_REW, "INSERT INTO character_queststatus_rewarded (guid,quest,active) VALUES (?,?,?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_PLAYER_QUEST_REW, "SELECT quest FROM character_queststatus_rewarded WHERE guid = ? AND quest = ?", CONNECTION_SYNCH);

    // Custom statements for player custom flags
    PrepareStatement(CHAR_SEL_CUSTOM_FLAGS, "SELECT mask FROM rg_characters_data WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CUSTOM_FLAGS, "INSERT INTO rg_characters_data (guid, mask) VALUES (?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CUSTOM_FLAGS, "DELETE FROM rg_characters_data WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_PET_BY_OWNER, "SELECT id FROM character_pet WHERE owner = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_PVPSTATS_BATTLEGROUND, "INSERT INTO pvpstats_battlegrounds (winner_faction, bracket_id, type, date) VALUES (?, ?, ?, NOW())", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_PVPSTATS_BATTLEGROUND_WITH_ID, "INSERT INTO pvpstats_battlegrounds (id, winner_faction, bracket_id, type, date) VALUES (?, ?, ?, ?, NOW())", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CURRENT_EQUIP, "SELECT itemEntry, slot FROM character_inventory JOIN item_instance ON character_inventory.item = item_instance.guid WHERE character_inventory.guid = ? and slot between 0 and 18 and bag = 0", CONNECTION_SYNCH);

    PrepareStatement(CHAR_DEL_CREATURE_VENDOR_ITEM_COUNT, "DELETE FROM creature_vendor_item_count WHERE guid = ? AND itemEntry = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CREATURE_VENDOR_ITEM_COUNT, "REPLACE INTO `creature_vendor_item_count` (guid, itemEntry, count, lastIncrementTime) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);

    PrepareStatement(CHAR_SEL_TEMP_INSTANCE, "SELECT map, mode from tmp_instance where guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_TEMP_INSTANCE, "REPLACE INTO `tmp_instance` (guid, map, mode) VALUES (?, ?, ?)",CONNECTION_ASYNC);

    // Player Cache
    PrepareStatement(CHAR_SEL_PLAYER_CACHE, "SELECT c.guid, c.name, c.account, c.race, c.class, c.gender, c.level, c.at_login, gm.guildid, c.zone FROM characters c LEFT JOIN guild_member AS gm ON (c.guid = gm.guid) WHERE c.deleteDate IS NULL", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_PLAYER_CACHE_BY_GUID, "SELECT c.name, c.account, c.race, c.class, c.gender, c.level, c.at_login, gm.guildid, c.zone FROM characters c LEFT JOIN guild_member AS gm ON (c.guid = gm.guid) WHERE c.guid = ?", CONNECTION_SYNCH);

    // Mail Mgr
    PrepareStatement(CHAR_SEL_MAILS, "SELECT id, messageType, stationery, mailTemplateId, sender, receiver, subject, body, expire_time, deliver_time, money, cod, checked FROM mail", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MAIL_ITEMS, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, item_guid, itemEntry, owner_guid, mail_id FROM mail_items mi JOIN item_instance ii ON mi.item_guid = ii.guid", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MAIL_ITEMS_FOR_RETURN, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, item_guid, itemEntry, owner_guid FROM mail_items mi JOIN item_instance ii ON mi.item_guid = ii.guid WHERE mail_id = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_UPD_MAIL_MARK_AS_READ, "UPDATE mail SET checked = checked | 0x01 WHERE id = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_MAIL_MARK_AS_COPIED, "UPDATE mail SET checked = checked | 0x04 WHERE id = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_MAIL_TAKE_MONEY, "UPDATE mail SET money = 0 WHERE id = ?", CONNECTION_ASYNC);
    
    // PvP Rank System
    PrepareStatement(CHAR_SEL_PVP_RANK_ALLIANCE, "SELECT character_pvp.guid, rank, cp, rank_points, cp_last_week, rank_last_week, rating, rd, last_rating from character_pvp, characters WHERE character_pvp.guid=characters.guid AND race IN(1,3,4,7,11)", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_PVP_RANK_HORDE, "SELECT character_pvp.guid, rank, cp, rank_points, cp_last_week, rank_last_week, rating, rd, last_rating from character_pvp, characters WHERE character_pvp.guid=characters.guid AND race IN(2,5,6,8,10)", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_PVP_RANK_BY_GUID, "SELECT guid, rank, cp, rank_points, cp_last_week, rank_last_week, rating, rd, last_rating, arena_rating, arena_mmr, arena_rd, arena_last_match_time, seasonal_games, weekly_games from character_pvp where guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_PVP_RANK, "REPLACE INTO character_pvp (guid, rank, cp, rank_points, cp_last_week, rank_last_week, rating, rd, last_rating, arena_rating, arena_mmr, arena_rd, arena_last_match_time, seasonal_games, weekly_games) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_PVP_RANK, "DELETE FROM character_pvp where guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_PVP_ARENA_STATS, "UPDATE character_pvp SET arena_rating = ?, arena_mmr = ?, arena_rd = ?, arena_last_match_time = ?, seasonal_games = seasonal_games + 1, weekly_games = weekly_games + 1 WHERE guid = ?", CONNECTION_ASYNC);

    // Armory
    PrepareStatement(CHAR_SEL_ARMORY_ARENA_GAMES_MAXID, "SELECT MAX(gameid) FROM armory_game_chart", CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_ARMORY_ARENA_GAMES, "INSERT INTO armory_game_chart (gameid, teamid, guid, changeType, ratingChange, teamRating, damageDone, deaths, healingDone, damageTaken, healingTaken, killingBlows, mapId, start, end) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, (UNIX_TIMESTAMP() - ?), UNIX_TIMESTAMP())", CONNECTION_ASYNC);

    // Arena Replay System
    PrepareStatement(CHAR_INS_ARENAREPLAY_DATA, "INSERT INTO `arena_replay_data` (`id`, `map`, `team1`, `team2`, `type`, `winner`, `rated`, `time`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ARENAREPLAY_CHAR_DATA, "INSERT INTO `arena_replay_char_data` (`id`, `guid`, `team`, `name`) VALUES(?, ?, ?, ?)", CONNECTION_ASYNC);
}
