...
MySQLConnection::MySQLConnection(MySQLConnectionInfo& connInfo, const std::string& name) :
        ...
        m_connectionFlags(CONNECTION_SYNCH)
{
    ...
    monitoring_counter_ = RG::Monitoring::Database::Instance().GetCounters(name);
#endif
}

MySQLConnection::MySQLConnection(ProducerConsumerQueue<SQLOperation*>* queue, MySQLConnectionInfo& connInfo, const std::string& name) :
        m_reconnecting(false),
        m_prepareError(false),
        m_queue(queue),
        m_Mysql(NULL),
...
{
    m_worker = Trinity::make_unique<DatabaseWorker>(m_queue, this, name);
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
    monitoring_counter_ = RG::Monitoring::Database::Instance().GetCounters(name);
#endif
}
...
bool MySQLConnection::Execute(const char* sql)
{
    ...
    {
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
        auto timer = monitoring_counter_->ReportQuery(m_connectionFlags == CONNECTION_ASYNC, false, true);
#endif
    ...
    TC_LOG_DEBUG("sql.query", "SQL: %s", sql);
...
bool MySQLConnection::Execute(PreparedStatement* stmt)
{
    ...
    {
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
        auto timer = monitoring_counter_->ReportQuery(m_connectionFlags == CONNECTION_ASYNC, true, true);
#endif
        stmt->BindParameters();
    ...
    TC_LOG_DEBUG("sql.query", "SQL(p): %s", m_mStmt->getQueryString(m_queries[index].first).c_str());
...
bool MySQLConnection::_Query(PreparedStatement* stmt, MYSQL_RES **pResult, uint64* pRowCount, uint32* pFieldCount)
{
    ...
    {
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
        auto timer = monitoring_counter_->ReportQuery(m_connectionFlags == CONNECTION_ASYNC, true, false);
#endif
...
bool MySQLConnection::_Query(const char *sql, MYSQL_RES **pResult, MYSQL_FIELD **pFields, uint64* pRowCount, uint32* pFieldCount)
{
    ...
    {
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
        auto timer = monitoring_counter_->ReportQuery(m_connectionFlags == CONNECTION_ASYNC, false, false);
#endif
        ...
        *pFieldCount = mysql_field_count(m_Mysql);
        ...
        if (*pResult && *pRowCount)
            *pFields = mysql_fetch_fields(*pResult);
    }

    TC_LOG_DEBUG("sql.sql", "SQL: %s", sql);
...
{
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
    monitoring_counter_->ReportTransactionRollback();
#endif
...
{
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
    monitoring_counter_->ReportTransactionCommit();
#endif
...
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
    monitoring_counter_->ReportTransaction(queries.size());
#endif
...
{
#ifdef CONFIG_WITH_PROMETHEUS_MONITORING
    monitoring_counter_->ReportError(errNo);
#endif
    ...
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST:
        // for compatibility with versions of MariaDB library >= 10.2
