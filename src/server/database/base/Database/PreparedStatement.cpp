...
void PreparedStatement::setString(const uint8 index, std::string_view value)
{
    if (index >= statement_data.size())
        statement_data.resize(index + 1);
...
bool PreparedStatementTask::Execute()
{
    if (m_has_result)
    {
        ...
        {
            delete result;
            m_result->setValue(PreparedQueryResult(nullptr));
            return false;
        }
        m_result->setValue(PreparedQueryResult(result));
        return true;
    }
