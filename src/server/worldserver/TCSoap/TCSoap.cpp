...
{
    util::thread::nameing::SetName("SOAP");
     ...
     * The soap.send_timeout and soap.recv_timeout specify the timeout for non-blocking socket I/O operations.i
     * source: https://www.cs.fsu.edu/~engelen/soapdoc2.html#sec:timeout
     */
    soap.connect_timeout = 120;
    soap.recv_timeout = 120;
    soap.send_timeout = 120;
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    // normalize userid for logging
    std::string normUserId(soap->userid);
    Utf8ToUpperOnlyLatin(normUserId);
    ...
    if (AccountMgr::GetSecurity(accountId) < SEC_GAMEMASTER)
    {
    ...
    // commands are executed in the world thread. We have to wait for them to be completed
    {
        ...
        CliCommandHolder* cmd = new CliCommandHolder(&connection, command, &SOAPCommand::print, &SOAPCommand::commandFinished, accountId, normUserId);
