...
bool RASession::CheckPassword(const std::string& user, const std::string& pass)
{
    ...
    uint32 accountId = AccountMgr::GetId(safe_user);
    if (!accountId)
    {
        TC_LOG_INFO("commands.ra", "Client used invalid username '%s'", safe_user.c_str());
        return false;
    }

    if (!AccountMgr::CheckPassword(accountId, safe_pass))
    {
        TC_LOG_INFO("commands.ra", "Invalid password for account '%s'", safe_user.c_str());
        return false;
    }
