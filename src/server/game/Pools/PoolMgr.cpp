...
template<>
GAME_API bool ActivePoolData::IsActiveObject<Creature>(uint32 db_guid) const
{
...
template<>
GAME_API bool ActivePoolData::IsActiveObject<GameObject>(uint32 db_guid) const
{
...
template<>
GAME_API bool ActivePoolData::IsActiveObject<Pool>(uint32 sub_pool_id) const
{
...
template<>
GAME_API bool ActivePoolData::IsActiveObject<Quest>(uint32 quest_id) const
{
...
void PoolGroup<Creature>::Despawn1Object(uint32 guid)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(guid))
    {
        ...
        if (Creature* creature = ObjectAccessor::GetCreature(data->mapid, ObjectGuid(HighGuid::Unit, data->id, guid)))
...
void PoolGroup<GameObject>::Despawn1Object(uint32 guid)
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(guid))
    {
        ...
        if (GameObject* pGameobject = ObjectAccessor::GetGameObject(data->mapid, ObjectGuid(HighGuid::Gameobject, data->id, guid)))
...
void PoolGroup<Creature>::Spawn1Object(PoolObject* obj)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(obj->guid))
    {
        ...
        {
            Creature::LoadFromDB(obj->guid, map);
        }
    }
}
...
void PoolGroup<GameObject>::Spawn1Object(PoolObject* obj)
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(obj->guid))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
        {
            GameObject* gameObject = GameObject::LoadFromDB(obj->guid, *map, false);
            if (!gameObject)
                return;

            if (gameObject->isSpawnedByDefault())
                map->AddToMap(gameObject);
        }
    }
}
...
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(obj->guid))
        if (Creature* creature = ObjectAccessor::GetCreature(data->mapid, ObjectGuid(HighGuid::Unit, data->id, obj->guid)))
...
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(obj->guid))
        if (GameObject* pGameobject = ObjectAccessor::GetGameObject(data->mapid, ObjectGuid(HighGuid::Gameobject, data->id, obj->guid)))
...
void PoolMgr::Initialize()
{
    ...
    mPoolTemplate.clear();
    mPoolCreatureGroups.clear();
    mPoolGameobjectGroups.clear();
    mPoolPoolGroups.clear();
    mPoolQuestGroups.clear();
...
void PoolMgr::LoadFromDB()
{
    ...
    // Pool of pools
    if (sWorld->getBoolConfig(CONFIG_DEBUG_DISABLE_MOTHERPOOLS))
        TC_LOG_WARN("server.loading", "Skipped loading of Mother Pooling Data due to config.");
    else
    {
...
template<typename T>
bool check_if_exists(uint32 pool_id, const std::map<uint32, T>& map)
{
    auto itr = map.find(pool_id);
    if (itr == map.end())
        return true;

    return itr->second.CheckPool();
}
...
bool PoolMgr::CheckPool(uint32 pool_id) const
{
    return pool_id <= max_pool_id &&
        check_if_exists(pool_id, mPoolGameobjectGroups) &&
        check_if_exists(pool_id, mPoolCreatureGroups) &&
        check_if_exists(pool_id, mPoolPoolGroups) &&
        check_if_exists(pool_id, mPoolQuestGroups);
}
