...
AccountOpResult AccountMgr::DeleteAccount(uint32 accountId)
{
    ...
    support::sTicketMgr.withLock([&](support::TicketMgr& ticketMgr)
    {
        ...
        {
            if (ticket.IsAssigned() && *ticket.GetAssigneeId() == accountId)
                ticket.Unassign(0);
...
{
    return GetSecurity(accountId, realm.Id.Realm);
}
...
{
    return gmlevel < GetStaffSecurityLevel();
}

bool AccountMgr::IsStaffAccount(uint32 gmlevel)
{
    return gmlevel >= GetStaffSecurityLevel();
}
...
{
    return gmlevel >= GMLEVEL_T_LEAD && gmlevel <= GMLEVEL_SERVER;
}

bool AccountMgr::IsConsoleAccount(uint32 gmlevel)
{
    return gmlevel == GMLEVEL_SERVER;
}

GMLevel AccountMgr::GetStaffSecurityLevel()
{
    return static_cast<GMLevel>(sWorld->getIntConfig(CONFIG_STAFF_SECURITY_LEVEL));
}
