...
void ThreatManager::AddThreat(Unit* victim, float amount, SpellInfo const* spell, bool ignoreModifiers, bool ignoreRedirection)
{
    ...
    if (!victim->CanReceiveThreatFrom(*iOwner))
        return;

    if (!iOwner->CanReceiveThreatFrom(*victim))
        return;
        ...
        case UEV_THREAT_REF_ONLINE_STATUS:
            if (!hostilRef->isOnline())
            {
                if (hostilRef == getCurrentVictim())
                {
                    ...
                    // TODO: should be using this event, but is causing problems(deleting objects due to ref - delete ie.)
                }
