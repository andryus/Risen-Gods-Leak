...
MailSender::MailSender(Object* sender, MailStationery stationery) : m_stationery(stationery)
{
    switch (sender->GetTypeId())
    {
        ...
        default:
            ...
            TC_LOG_ERROR("misc", "MailSender::MailSender - Mail have unexpected sender typeid (%hhu)", sender->GetTypeId());
            break;
    }
}
...
MailReceiver::MailReceiver(Player* receiver) : m_receiver(receiver), m_receiver_guid(receiver->GetGUID()) { }


MailReceiver::MailReceiver(Player* receiver, ObjectGuid receiver_guid) : m_receiver(receiver), m_receiver_guid(receiver_guid)
{
    ASSERT(!receiver || receiver->GetGUID() == receiver_guid);
}
...
void MailDraft::deleteIncludedItems(SQLTransaction& trans, bool inDB /*= false*/ )
{
    for (MailItemMap::iterator mailItemIter = m_items.begin(); mailItemIter != m_items.end(); ++mailItemIter)
    {
        ...
        if (inDB)
        {
            ...
            item->SaveToDB(trans);          // already deletes item object
        } 
        else
            delete item;
    }
...
void MailDraft::SendReturnToSender(uint32 sender_acc, ObjectGuid sender_guid, ObjectGuid receiver_guid, SQLTransaction& trans)
{
    ...
    uint32 rc_account = 0;
    if (!receiver)
        rc_account = player::PlayerCache::GetAccountId(receiver_guid);
    ...
    SendMailTo(trans, MailReceiver(receiver, receiver_guid), MailSender(MAIL_NORMAL, sender_guid.GetCounter()), MAIL_CHECK_MASK_RETURNED, deliver_delay);
}
...
void MailDraft::SendMailTo(SQLTransaction& trans, MailReceiver const& receiver, MailSender const& sender, MailCheckMask checked, uint32 deliver_delay)
{
    ...
    auto pSender = ObjectAccessor::FindConnectedPlayer(ObjectGuid(HighGuid::Player, sender.GetSenderId()));
    ...
    stmt->setUInt32(++index, receiver.GetPlayerGUID().GetCounter());
    ...
    for (MailItemMap::const_iterator mailItemIter = m_items.begin(); mailItemIter != m_items.end(); ++mailItemIter)
    {
        ...
        RG_LOG<ItemLogModule>(pItem, RG::Logging::ItemLogType::SEND_MAIL, 0, sender.GetSenderId(), receiver.GetPlayerGUID().GetCounter());
    }

    RG_LOG<TradeLogModule>(*this, receiver, sender, m_items);
    ...
    m.receiver = receiver.GetPlayerGUID();
    m.expire_time = expire_time;
    m.deliver_time = deliver_time;
    m.checked = checked;

    MailMgr::Insert(std::move(m));
}
