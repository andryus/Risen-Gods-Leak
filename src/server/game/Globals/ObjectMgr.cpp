...
std::string GetScriptCommandName(ScriptCommands command)
{
    ...
    switch (command)
    {
        ...
        case SCRIPT_COMMAND_UNITSTATE: res = "SCRIPT_COMMAND_UNITSTATE"; break;
        default:
        {
            char sz[32];
...
constexpr LanguageDesc lang_descriptions[LANGUAGES_COUNT] =
{
    { LANG_ADDON,           0, 0 },
    { LANG_UNIVERSAL,       0, 0 },
...
constexpr auto makeLangTable()
{
    std::array<LanguageDesc, 255> langTable = { { LANG_ADDON, (uint32)-1, (uint32)-1, false } };

    for (const auto& desc : lang_descriptions)
    {
        langTable.data()[desc.lang_id + 1] = desc;
    }
...
LanguageDesc GetLanguageDescByID(uint32 lang)
{
    constexpr auto langTable = makeLangTable();

    return langTable[lang + 1];
}

LanguageDesc GetLanguageDescByIndex(uint32 index)
{
    return lang_descriptions[index];
}
...
void ObjectMgr::LoadCreatureLocales()
{
    ...
    _creatureLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.Name, data.SubName);
...
void ObjectMgr::LoadGossipMenuItemsLocales()
{
    ...
    _gossipMenuItemsLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.OptionText, data.BoxText);
    }
    while (result->NextRow());
...
void ObjectMgr::LoadPointOfInterestLocales()
{
    ...
    _pointOfInterestLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.IconName);
    } while (result->NextRow());
...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    //                                            79           80           81             82                83            84          85
                                             " questItem6, movementId, RegenHealth, mechanic_immune_mask, flags_extra, customFlags, ScriptName "
    ...
    // store difficulty entries to fixup script-entries
    std::vector<std::pair<const CreatureTemplate*, uint32>> difficulties;
    difficulties.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        {
            uint32 difficultyEntry = fields[1 + i].GetUInt32();
            creatureTemplate.DifficultyEntry[i] = difficultyEntry;
            if (difficultyEntry > 0)
                difficulties.emplace_back(&creatureTemplate, difficultyEntry);
        }
        ...
        creatureTemplate.customFlags        = fields[83].GetUInt32();
        creatureTemplate.ScriptName         = fields[84].GetString();
        creatureTemplate.ScriptID           = GetScriptId(creatureTemplate.ScriptName.c_str());
    ...
    for (const auto& [entry, creatureTemplate] : _creatureTemplateStore)
    {
        util::meta::Unreferenced(entry);

        CheckCreatureTemplate(&creatureTemplate);
    }

    for (const auto [baseTemplate, difficultyEntry] : difficulties)
    {
        ASSERT(baseTemplate);
        const auto itr = _creatureTemplateStore.find(difficultyEntry);

        if (itr != _creatureTemplateStore.end())
        {
            CreatureTemplate& difficultyTemplate = itr->second;

            difficultyTemplate.AIName = baseTemplate->AIName;
            difficultyTemplate.ScriptID = baseTemplate->ScriptID;
            difficultyTemplate.ScriptName = baseTemplate->ScriptName;
        }
    }
...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    //                                                0       1       2      3       4       5      6     7
    QueryResult result = WorldDatabase.Query("SELECT entry, path_id, mount, bytes1, bytes2, emote, auras, boundary FROM creature_template_addon");
    ...
    do
    {
        ...
        creatureAddon.boundary = fields[7].GetUInt16();
        ...
        for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Creature (Entry: %u) has duplicate aura (spell %u) in `auras` field in `creature_template_addon`.", entry, uint32(atol(*itr)));
                continue;
            }
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    //                                                0       1       2      3       4       5      6       7
    QueryResult result = WorldDatabase.Query("SELECT guid, path_id, mount, bytes1, bytes2, emote, auras, boundary FROM creature_addon");
    ...
    do
    {
        ...
        for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        {
            ...
            if (std::find(creatureAddon.auras.begin(), creatureAddon.auras.end(), atoul(*itr)) != creatureAddon.auras.end())
            {
                TC_LOG_ERROR("sql.sql", "Creature (GUID: %u) has duplicate aura (spell %u) in `auras` field in `creature_addon`.", guid, uint32(atol(*itr)));
                continue;
            }
...
void ObjectMgr::LoadCreatureVendorItemCount()
{
    ...
    CharacterDatabase.DirectExecute("DELETE FROM creature_vendor_item_count WHERE lastIncrementTime <= (UNIX_TIMESTAMP() - 604800)");
    //                                                0       1         2           3
    QueryResult result = CharacterDatabase.Query("SELECT guid, itemEntry, count, lastIncrementTime FROM creature_vendor_item_count");
    ...
    do
    {
        ...
        uint32 lastIncrementTime = fields[3].GetUInt32();

        VendorItemCount tmp(itemEntry, count, time_t(lastIncrementTime));
        _creatureVendorItemCountContainer[guid].push_back(tmp);

        ++count;
    }
    while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u creature vendor item counts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));

}

VendorItemCounts const* ObjectMgr::GetCreatureVendorItemCounts(uint32 lowguid)
{
    CreatureVendorItemCountContainer::const_iterator itr = _creatureVendorItemCountContainer.find(lowguid);
    if (itr != _creatureVendorItemCountContainer.end())
        return &itr->second;
...
void ObjectMgr::LoadGameObjectAddons()
{
    ...
    _gameObjectAddonStore.reserve(result->GetRowCount());
...
void ObjectMgr::LoadLinkedRespawn()
{
    ...
    do
    {
        ...
        bool error = false;
        ASSERT_DEV(guidLow != linkedGuidLow, "LinkedRespawn: guid: %u is the same as linkedGuid: %u. Values must be different!",
...
void ObjectMgr::LoadCreatures()
{
    ...
    /*std::vector<uint32> spawnMasks(sMapStore.GetNumRows());
    ...
    do
    {
        ...
        //if (!IsTransportMap(data.mapid) && data.spawnMask & ~spawnMasks.at(data.mapid))
...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            Grids::CellCoord cellCoord = Grids::CellCoord::FromPosition(data->posX, data->posY);
...
uint32 ObjectMgr::AddGOData(uint32 entry, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay, float rotation0, float rotation1, float rotation2, float rotation3)
{
    ...
    {
        GameObject* go = GameObject::LoadFromDB(guid, *map);
        if (!go)
        {
...
bool ObjectMgr::MoveCreData(uint32 guid, uint32 mapId, const Position& pos)
{
    ...
    if (Map* map = sMapMgr->CreateBaseMap(mapId))
    {
        ...
        {
            Creature* creature = Creature::LoadFromDB(guid, map);
            if (!creature)
            {
...
uint32 ObjectMgr::AddCreData(uint32 entry, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay /*= 0*/)
{
    ...
    uint32 guid = GenerateLowGuid(HighGuid::Unit);
    CreatureData& data = NewOrExistCreatureData(guid);
    data.id = entry;
    data.mapid = mapId;
    data.displayid = 0;
...
void ObjectMgr::LoadGameobjects()
{
    ...
    do
    {
        ...
        if (data.rotation.x == 0 && data.rotation.y == 0 && data.rotation.z == 0 && data.rotation.w == 0)
        {
            TC_LOG_ERROR("sql.sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with invalid rotation values (all equal to zero), skip", guid, data.id);
            continue;
        }
...
void ObjectMgr::LoadItemLocales()
{
    ...
    _itemLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.Name, data.Description);
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    do
    {
        ...
        // Since we are having some custom items, we already know, that they won't exist in the item.dbc
        // else
    ...
    for (uint32 i = 1; i < sCharStartOutfitStore.GetNumRows(); ++i)
    {
        ...
        for (int j = 0; j < MAX_OUTFIT_ITEMS; ++j)
        {
            ...
            const uint32 item_id = entry->ItemId[j];

            if (!GetItemTemplate(item_id))
                TC_LOG_ERROR("sql.sql", "Item (Entry: %u) does not exist in `item_template` but is referenced in `CharStartOutfit.dbc`", item_id);
        }
    }
...
void ObjectMgr::LoadItemSetNameLocales()
{
    ...
    _itemSetNameLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.Name);
    } while (result->NextRow());
...
void ObjectMgr::LoadQuests()
{
    ...
    do
    {
        ...
        _questTemplates.try_emplace(fields[0].GetUInt32(), fields);
    } while (result->NextRow());

    std::map<uint32, uint32> usedMailTemplates;

    // Post processing
    for (auto& [id, quest] : _questTemplates)
    {
        util::meta::Unreferenced(id);

//        // skip post-loading checks for disabled quests
//! skipping the checks cause crashes because GMs ignore disables and data for later processing aren't available
...
//            continue;
        ...
        if (qinfo->ExclusiveGroup)
            mExclusiveQuestGroups.emplace(qinfo->ExclusiveGroup, qinfo->GetQuestId());
        if (qinfo->LimitTime)
...
void ObjectMgr::LoadQuestLocales()
{
    ...
    _questLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.Title, data.Details, data.Objectives, data.OfferRewardText, data.RequestItemsText, data.EndText, data.CompletedText, data.ObjectiveText[0], data.ObjectiveText[1], data.ObjectiveText[2], data.ObjectiveText[3]);
...
void ObjectMgr::LoadSpellScriptNames()
{
    ...
    do
    {
        ...
        if (allRanks)
        {
            ...
            while (spellInfo)
            {
                const_cast<SpellInfo*>(spellInfo)->ScriptName = scriptName;
    ...
    for (SpellInfo* info : sSpellMgr->_getAllSpells())
    {
        if (!info)
            continue;

        const uint32 difficultyId = sSpellMgr->GetSpellDifficultyId(info->Id);
        if (const SpellInfo* baseInfo = sSpellMgr->GetSpellInfo(difficultyId))
            info->ScriptName = baseInfo->ScriptName;
    }
...
void ObjectMgr::LoadPageTextLocales()
{
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.Text);
...
void ObjectMgr::LoadInstanceTemplate()
{
    ...
    _instanceTemplateStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        InstanceTemplate& instanceTemplate = _instanceTemplateStore[mapID];

        instanceTemplate.AllowMount = fields[3].GetBool();
        instanceTemplate.Parent     = uint32(fields[1].GetUInt16());
        instanceTemplate.ScriptName = fields[2].GetCString();
        instanceTemplate.ScriptId   = sObjectMgr->GetScriptId(instanceTemplate.ScriptName.c_str());
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    //                                                 0         1            2                3                4
    QueryResult result = WorldDatabase.Query("SELECT entry, creditType, creditEntry, lastEncounterDungeon, ScriptName FROM instance_encounters");
    if (!result)
    {
    ...
    uint32 count = 0;
    std::unordered_map<uint32, DungeonEncounterEntry const*> dungeonLastBosses;

    std::unordered_map<uint32, std::string> normalEncounters;
    std::vector<std::tuple<uint32, DungeonEncounterVector*, uint8>> otherEncounters;
    do
    {
        Field* fields = result->Fetch();
        uint32 entry = fields[0].GetUInt32();
        ...
        DungeonEncounterVector& encounters = _dungeonEncounterStore[MAKE_PAIR32(dungeonEncounter->mapId, dungeonEncounter->difficulty)];

        if (creditType == ENCOUNTER_CREDIT_KILL_CREATURE)
        {
            const auto key = std::pair(creditEntry, (Difficulty)dungeonEncounter->difficulty);

            _encounterCreatureStore.emplace(key, entry);
        }

        if (dungeonEncounter->difficulty == DUNGEON_DIFFICULTY_NORMAL)
        {
            if (!ScriptName.empty())
                normalEncounters.emplace(creditEntry, ScriptName);
        }
        else
            otherEncounters.emplace_back(creditEntry, &encounters, encounters.size());

        encounters.emplace_back(dungeonEncounter, EncounterCreditType(creditType), creditEntry, lastEncounterDungeon, std::move(ScriptName));
        ++count;
    } while (result->NextRow());

    // fixup hc encounters
    for (const auto [entry, encounters, id] : otherEncounters)
    {
        auto itr = normalEncounters.find(entry);
        if (itr != normalEncounters.end())
            encounters->at(id).ScriptName = itr->second;
    }
...
{
    if (auto data = GetGameObjectTemplate(entry))
        return data->type == GAMEOBJECT_TYPE_TRANSPORT;
    return false;
}
...
void ObjectMgr::LoadNpcTextLocales()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT ID, "
        "Text0_0_loc1, Text0_0_loc2, Text0_0_loc3, Text0_0_loc4, Text0_0_loc5, Text0_0_loc6, Text0_0_loc7, Text0_0_loc8, Text0_1_loc1, Text0_1_loc2, Text0_1_loc3, Text0_1_loc4, Text0_1_loc5, Text0_1_loc6, Text0_1_loc7, Text0_1_loc8, "
        "Text1_0_loc1, Text1_0_loc2, Text1_0_loc3, Text1_0_loc4, Text1_0_loc5, Text1_0_loc6, Text1_0_loc7, Text1_0_loc8, Text1_1_loc1, Text1_1_loc2, Text1_1_loc3, Text1_1_loc4, Text1_1_loc5, Text1_1_loc6, Text1_1_loc7, Text1_1_loc8, "
        "Text2_0_loc1, Text2_0_loc2, Text2_0_loc3, Text2_0_loc4, Text2_0_loc5, Text2_0_loc6, Text2_0_loc7, Text2_0_loc8, Text2_1_loc1, Text2_1_loc2, Text2_1_loc3, Text2_1_loc4, Text2_1_loc5, Text2_1_loc6, Text2_1_loc7, Text2_1_loc8, "
        "Text3_0_loc1, Text3_0_loc2, Text3_0_loc3, Text3_0_loc4, Text3_0_loc5, Text3_0_loc6, Text3_0_loc7, Text3_0_loc8, Text3_1_loc1, Text3_1_loc2, Text3_1_loc3, Text3_1_loc4, Text3_1_loc5, Text3_1_loc6, Text3_1_loc7, Text3_1_loc8, "
        "Text4_0_loc1, Text4_0_loc2, Text4_0_loc3, Text4_0_loc4, Text4_0_loc5, Text4_0_loc6, Text4_0_loc7, Text4_0_loc8, Text4_1_loc1, Text4_1_loc2, Text4_1_loc3, Text4_1_loc4, Text4_1_loc5, Text4_1_loc6, Text4_1_loc7, Text4_1_loc8, "
        "Text5_0_loc1, Text5_0_loc2, Text5_0_loc3, Text5_0_loc4, Text5_0_loc5, Text5_0_loc6, Text5_0_loc7, Text5_0_loc8, Text5_1_loc1, Text5_1_loc2, Text5_1_loc3, Text5_1_loc4, Text5_1_loc5, Text5_1_loc6, Text5_1_loc7, Text5_1_loc8, "
        "Text6_0_loc1, Text6_0_loc2, Text6_0_loc3, Text6_0_loc4, Text6_0_loc5, Text6_0_loc6, Text6_0_loc7, Text6_0_loc8, Text6_1_loc1, Text6_1_loc2, Text6_1_loc3, Text6_1_loc4, Text6_1_loc5, Text6_1_loc6, Text6_1_loc7, Text6_1_loc8, "
        "Text7_0_loc1, Text7_0_loc2, Text7_0_loc3, Text7_0_loc4, Text7_0_loc5, Text7_0_loc6, Text7_0_loc7, Text7_0_loc8, Text7_1_loc1, Text7_1_loc2, Text7_1_loc3, Text7_1_loc4, Text7_1_loc5, Text7_1_loc6, Text7_1_loc7, Text7_1_loc8 "
    ...
    do
    {
        ...
        {
            FillLocaleContainer(fields, data.Text_0[j]);
            FillLocaleContainer(fields, data.Text_1[j]);
        }
...
{
    if (!serverUp) // not thread-safe
        MailMgr::ReturnOrDeleteOldMails();
}
...
void ObjectMgr::LoadAreaScripts()
{
    ...
    _areaScriptStore.clear();                            // need for reload case
    QueryResult result = WorldDatabase.Query("SELECT id, script FROM map_area_scripts");

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 map area scripts. DB table `map_area_scripts` is empty.");
        return;
    }

    uint32 count = 0;

    _areaScriptStore.reserve(result->GetFieldCount());
    ...
    do
    {
        ...
        const std::string_view scriptName = fields[1].GetCString();

        _areaScriptStore.emplace(areaId, scriptName);
    } while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u map area scripts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveYard(float x, float y, float z, uint32 MapId, uint32 team)
{
    ...
    if (!zoneId)
    {
        // Icecrown Citadel - Fallback Hackfix
        if (MapId == 631)
            if (WorldSafeLocsEntry const* entry = sWorldSafeLocsStore.LookupEntry(1682))
                return entry;
...
uint32 ObjectMgr::GenerateLowGuid(HighGuid guidhigh)
{
    ...
    {
        case HighGuid::Item:
            return _itemGuidGenerator.Generate();
        case HighGuid::Unit:
        {
...
void ObjectMgr::LoadGameObjectLocales()
{
    ...
    _gameObjectLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        FillLocaleContainer(fields, data.Name);
        FillLocaleContainer(fields, data.CastBarCaption);
...
void ObjectMgr::LoadGameObjectTemplate()
{
    ...
    do
    {
        ...
        got.ScriptName = fields[41].GetString();
        got.ScriptId = GetScriptId(got.ScriptName.c_str());
        ...
        switch (got.type)
        {
            ...
            {
                MMAP::MMapFactory::createOrGetMMapManager()->loadGameObject(got.displayId);
                break;
            }
...
void ObjectMgr::LoadCorpses()
{
    ...
    do
    {
        ...
        Corpse* corpse = Corpse::LoadFromDB(guid, fields, type);
        if (!corpse) // loading failed
            continue;
...
void ObjectMgr::LoadQuestStates()
{
    uint32 oldMSTime = getMSTime();

    _questStateStore.clear();                              // need for reload case

    uint32 count = 0;

    //                                                  0         1       2
    QueryResult result = WorldDatabase.Query("SELECT questId, ticketId, status FROM rg_custom_tickets");

    if (!result)
    {
        TC_LOG_ERROR("server.loading", ">> Loaded 0 quest state definitions. DB table `rg_custom_ticketstates` is empty.");
        return;
    }
    ...
    do
    {
        ...
        uint32 questId            = fields[0].GetUInt32();
        uint32 ticketId           = fields[1].GetUInt32();
        std::string state         = fields[2].GetString();

        TicketQuestState STATE(questId, ticketId, state);

        _questStateStore[questId] = STATE;

        ++count;
    } while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u ticket quest state definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool ObjectMgr::AddReservedName(const std::string& name)
{
    std::wstring wstr;
    if (!Utf8toWStr(name, wstr))
        return false;
    ...
    if (_reservedNamesStore.find(wstr) != _reservedNamesStore.end())
        return false;

    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_RESERVED_NAME);
...
bool ObjectMgr::DeleteReservedName(const std::string& name)
{
    std::wstring wstr;
    if (!Utf8toWStr(name, wstr))
        return false;
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_RESERVED_NAME);
    stmt->setString(0, name);
    CharacterDatabase.Execute(stmt);

    _reservedNamesStore.erase(wstr);
...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (!result)
    {
        if (min_value == MIN_TRINITY_STRING_ID)              // error only in case internal strings
            ...
            TC_LOG_INFO("server.loading", ">> Loaded 0 string templates. DB table `%s` is empty. You have imported an incorrect database for more info search for TCE00003 on forum.", table);
    ...
    _trinityStringLocaleStore.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        data.Content.reserve(9);
        ++count;

        AddLocaleString(data.Content, 0, fields);
        FillLocaleContainer(fields, data.Content);
    } while (result->NextRow());
...
std::string_view ObjectMgr::GetAreaScript(uint32 ID) const
{
    auto itr = _areaScriptStore.find(ID);
    if (itr != _areaScriptStore.end())
        return itr->second;
...
void ObjectMgr::LoadGameTele()
{
    ...
    QueryResult nameResult = WorldDatabase.Query("SELECT id, name FROM game_tele_names");
    if (nameResult)
    {
        do
        {
            Field* names = nameResult->Fetch();
            uint32 id = names[0].GetUInt32();
            std::string extraName = names[1].GetString();
            std::wstring wExtraName;

            if (_gameTeleStore.find(id) == _gameTeleStore.end())
            {
                TC_LOG_ERROR("sql.sql", "No existing entry in `game_tele` for id %u of `game_tele_names`, ignoring.", id);
                continue;
            }

            if (!Utf8toWStr(extraName, wExtraName))
            {
                TC_LOG_ERROR("sql.sql", "Wrong UTF8 name %s for id %u in `game_tele_names` table, ignoring.", extraName.c_str(), id);
                continue;
            }

            wstrToLower(wExtraName);

            _gameTeleStore[id].names.push_back(wExtraName);
        }
...
GameTele const* ObjectMgr::GetGameTele(const std::string& name) const
{
    ...
    for (auto& [key, value] : _gameTeleStore)
    {
        ::util::meta::Unreferenced(key);
        if (value.wnameLow == wname)
            return &value;
        else if (alt == NULL && value.wnameLow.find(wname) != std::wstring::npos)
            alt = &value;

        for (auto extraName : value.names)
        {
            if (extraName == wname)
                return &value;
            else if (alt == NULL && extraName.find(wname) != std::wstring::npos)
                alt = &value;
        }
    }
...
bool ObjectMgr::AddGameTele(GameTele& tele)
{
    ...
    stmt->setFloat(0, tele.position_x);
    stmt->setFloat(1, tele.position_y);
    stmt->setFloat(2, tele.position_z);
    stmt->setFloat(3, tele.orientation);
    stmt->setUInt16(4, uint16(tele.mapId));
    stmt->setString(5, tele.name);
...
bool ObjectMgr::DeleteGameTele(const std::string& name)
{
    ...
    for (auto&[key, value] : _gameTeleStore)
    {
        if (value.wnameLow == wname)
        {
            ...
            stmt->setString(0, value.name);
            trans->Append(stmt);

            PreparedStatement* stmt2 = WorldDatabase.GetPreparedStatement(WORLD_DEL_GAME_TELE_NAMES);
            stmt2->setUInt32(0, key);
            trans->Append(stmt2);

            WorldDatabase.CommitTransaction(trans);

            _gameTeleStore.erase(key);
            return true;
        }
    }
...
void ObjectMgr::LoadMailLevelRewardsExtended()
{
    uint32 oldMSTime = getMSTime();

    _mailLevelRewardExtendedStore.clear();                           // for reload case

    //                                                 0        1       2      3       4       5
    QueryResult result = WorldDatabase.Query("SELECT level, raceMask, flags, sender, subject, body FROM mail_level_reward_extended");

    if (!result)
    {
        TC_LOG_ERROR("server.loading", ">> Loaded 0 level dependent mail rewards. DB table `mail_level_reward_extended` is empty.");
        return;
    }
    ...
    do
    {
        ...
        std::string sender    = fields[3].GetString();
        std::string subject   = fields[4].GetString();
        std::string body      = fields[5].GetString();

        if (level > DEFAULT_MAX_LEVEL || level < MIN_LEVEL)
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_level_reward_extended` have data for level %u that more or less supported by client (%u), ignoring.", level, MAX_LEVEL);
            continue;
        }
        ...
        if (!(raceMask & RACEMASK_ALL_PLAYABLE))
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_level_reward_extended` have raceMask (%u) for level %u that not include any player races, ignoring.", raceMask, level);
            continue;
        }
        
        if (!sender.length())
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_level_reward_extended` have invalid have no sender by mail for level %d. This is absolute needed for sending mail, ignoring.", level);
            continue;
        }

        if (!subject.length())
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_level_reward_extended` have invalid have no subject by mail for level %d. This is absolute needed for sending mail, ignoring.", level);
            continue;
        }

        if (!body.length())
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_level_reward_extended` have no body for level %u with subject \"%s\", ignoring.", level, subject.c_str());
            continue;
        }

        _mailLevelRewardExtendedStore[level].push_back(MailLevelRewardExtended(level, raceMask, flags, sender, subject, body));

        ++count;
    }
    while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u level dependent mail rewards extended in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadScriptNames()
{
    ...
    QueryResult result = WorldDatabase.Query(
      ...
      " UNION "
      "SELECT DISTINCT(script_name) FROM command WHERE script_name <> ''"
    );
    ...
    _scriptIndexStore.reserve(result->GetRowCount());

    _scriptNamesStore.push_back("");
    do
    {
        auto& string = _scriptNamesStore.emplace_back((*result)[0].GetString());
        _scriptIndexStore.emplace(string, count);
        ++count;
    }
    while (result->NextRow());
...
uint32 ObjectMgr::GetScriptId(const char* name)
{
    ...
    auto itr = _scriptIndexStore.find(name);
    if (itr == _scriptIndexStore.end())
        return 0;
    else
        return itr->second;
}

void ObjectMgr::CheckScripts(ScriptsType type, std::unordered_set<int32>& ids)
{
    ScriptMapMap* scripts = GetScriptsMapByType(type);
    if (!scripts)
        return;
    ...
    {
        for (const auto& [key, value] : script.second)
        {
            switch (value.command)
            {
                case SCRIPT_COMMAND_TALK:
                {
                    if (!GetTrinityStringLocale (value.Talk.TextID))
                        TC_LOG_ERROR("sql.sql", "Table `%s` references invalid text id %u from `db_script_string`, script id: %u.", GetScriptsTableNameByType(type).c_str(), value.Talk.TextID, key);

                    auto itr = ids.find(value.Talk.TextID);
                    if (itr != ids.end())
                        ids.erase(itr);
                }
                default:
...
void ObjectMgr::LoadBroadcastTexts()
{
    ...
    do
    {
        ...
        _broadcastTextStore.emplace(bct.Id, std::move(bct));
...
void ObjectMgr::LoadBroadcastTextLocales()
{
    ...
    do
    {
        ...
        FillLocaleContainer(fields, bct->second.MaleText);
        FillLocaleContainer(fields, bct->second.FemaleText);
...
void ObjectMgr::LoadDbScriptStrings()
{
    ...
    std::unordered_set<int32> ids;
    ids.reserve(MAX_DB_SCRIPT_STRING_ID - MIN_DB_SCRIPT_STRING_ID);
...
void ObjectMgr::LoadFactionChangeItems()
{
    ...
    do
    {
        ...
        else
        {
            ...
            for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
            {
                auto itemA = GetItemTemplate(alliance);
                auto itemH = GetItemTemplate(horde);
                _Spell const& spellDataA = itemA->Spells[i];
                _Spell const& spellDataH = itemH->Spells[i];

                if (!spellDataA.SpellId || !spellDataH.SpellId || 
                    spellDataA.SpellTrigger != ITEM_SPELLTRIGGER_ON_USE ||
                    spellDataH.SpellTrigger != ITEM_SPELLTRIGGER_ON_USE ||
                    (itemA->InventoryType != INVTYPE_TRINKET && itemA->Class != ITEM_CLASS_CONSUMABLE))
                    continue;

                FactionChangeCfbgSpells[spellDataA.SpellId] = spellDataH.SpellId;
                FactionChangeCfbgSpells[spellDataH.SpellId] = spellDataA.SpellId;
            }
...
void ObjectMgr::LoadFactionChangeReputations()
{
    ...
    do
    {
        ...
        else
        {
            ...
            FactionChangeFullReputationMap[alliance] = horde;
            FactionChangeFullReputationMap[horde] = alliance;
        }
...
void ObjectMgr::LoadFactionChangeSpells()
{
    ...
    do
    {
        ...
        else
        {
            ...
            auto spellInfo = sSpellMgr->GetSpellInfo(alliance);
            if (spellInfo->HasAura(SPELL_AURA_MOUNTED)) 
            {
                FactionChangeCfbgSpells[alliance] = horde;
                FactionChangeCfbgSpells[horde] = alliance;
            }
        }
...
void ObjectMgr::LoadFactionChangeTitles()
{
    ...
    do
    {
        ...
        uint16 alliance = fields[0].GetUInt16();
        uint16 horde = fields[1].GetUInt16();
        ...
        else
        {
            ...
            FactionChangeFullTitleMap[sCharTitlesStore.LookupEntry(alliance)->bit_index] = sCharTitlesStore.LookupEntry(horde)->bit_index;
            FactionChangeFullTitleMap[sCharTitlesStore.LookupEntry(horde)->bit_index] = sCharTitlesStore.LookupEntry(alliance)->bit_index;
        }
...
DungeonEncounterSpan ObjectMgr::GetDungeonEncounterList(uint32 mapId, Difficulty difficulty) const
{
...
uint32 ObjectMgr::GetEncounterForCreature(uint32 entry, Difficulty difficulty) const
{
    auto itr = _encounterCreatureStore.find({entry, difficulty});
    if (itr != _encounterCreatureStore.end())
        return itr->second;
...

{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded %u anticheat config definitions", count);
        TC_LOG_ERROR("server.loading", "Error loading `anticheat_config` table or table is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("server.loading", "Wrong check type id %u in `anticheat_config` table, ignoring.",AntiCheatConfigEntry.checkType);
            continue;
        }
...
void ObjectMgr::LoadPolygonAssignments()
{
    uint32 oldMSTime = getMSTime();

    QueryResult result = WorldDatabase.Query("SELECT `map`, `tileX`, `tileY`, `polyIndex`, COUNT(*) FROM `gameobject_polygons` GROUP BY `map`, `tileX`, `tileY`, `polyIndex`");
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 polygon assignments. DB table `gameobject_polygons` is empty!");
        return;
    }
    do
    {
        ...
        uint16 mapID = fields[0].GetUInt16();
        int32 tileX = fields[1].GetInt32();
        int32 tileY = fields[2].GetInt32();
        uint32 polyIndex = fields[3].GetUInt32();
        size_t count = (size_t) fields[4].GetUInt64();
        PolyIdentifier id = std::make_tuple(mapID, tileX, tileY, polyIndex);
        polygonAssignments.insert(PolygonMap::value_type(id, std::vector<uint32>()));
        polygonAssignments[id].reserve(count);
    } while (result->NextRow());

    result = WorldDatabase.Query("SELECT `map`, `tileX`, `tileY`, `polyIndex`, `guid` FROM `gameobject_polygons`");
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 polygon assignments. DB table `gameobject_polygons` is empty!");
        return;
    ...
    do
    {
        ...
        int32 tileY = fields[2].GetInt32();
        uint32 polyIndex = fields[3].GetUInt32();
        uint32 guidLow = fields[4].GetUInt32();

        polygonAssignments[std::make_tuple(mapID, tileX, tileY, polyIndex)].push_back(guidLow);
        ++count;
    } while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u polygon assignments in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}

void ObjectMgr::PrepareLocaleContainer(StringVector& data)
{
    data.reserve(TOTAL_LOCALES);
    data.resize(1);
}

void ObjectMgr::AddLocaleString(StringVector& data, uint8 index, Field*& fields)
{
    auto string = fields->GetString();
    if(!string.empty())
    {
        data.resize(index);
        data.emplace_back(std::move(string));
    }
