...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING))
    {
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            // Ghoul energy never go below thirty when autocasting
...
bool PetAI::CanAttack(Unit* target)
{
    ...
    // check if CharmInfo is not null
    if (CharmInfo* charmInfo = me->GetCharmInfo()) {
        // Passive - passive pets can attack if told to
        if (me->HasReactState(REACT_PASSIVE))
            return charmInfo->IsCommandAttack();
        ...
        if (charmInfo->IsReturning())
            return !charmInfo->IsCommandFollow();

        // Stay - can attack if target is within range or commanded to
        if (charmInfo->HasCommandState(COMMAND_STAY))
            return (me->IsWithinMeleeRange(target) || charmInfo->IsCommandAttack());
