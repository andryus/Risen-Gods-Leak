    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_THREAT_SINGLE_PCT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    me->GetThreatManager().ModifyThreatByPercent((*itr)->ToUnit(), std::max<int32>(-100, int32(e.action.threatPCT.threatINC) - int32(e.action.threatPCT.threatDEC)));
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(e.action.cast.spell))
                {
                    if (me)
                    {
                        ...
                        if (((spellInfo->IsChanneled() && (spellInfo->AttributesCu & SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING) != 0)
                             || spellInfo->CalcCastTime())
                            && me->isMoving()
                            && (spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT) != 0)
                            me->StopMoving();
                    ...
                    TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CAST:: %s: %u casts spell %u on target %u with castflags %u",
                        (me ? me->GetGUID() : go->GetGUID()).GetTypeName().data(), me ? me->GetGUID().GetCounter() : go->GetGUID().GetCounter(), e.action.cast.spell, (*itr)->GetGUID().GetCounter(), e.action.cast.flags);
                }
                else
                    TC_LOG_DEBUG("scripts.ai", "Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: %s Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*itr)->GetGUID().ToString().c_str(), (*itr)->GetEntry(), uint32((*itr)->GetTypeId()));
            }
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST:
        {
            ...
            if (!me->HasAuraType(SPELL_AURA_MOD_STUN) && !me->HasAuraType(SPELL_AURA_MOD_ROOT))
        ...
        case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
        {
            ...
            else if (Player* player = unit->GetCharmerOrOwnerPlayerOrPlayerItself())
            {
                player->GroupEventHappens(e.action.quest.quest, GetBaseObject());
                TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_GROUPEVENTHAPPENS: Player %u, group credit for quest %u",
                    unit->GetGUID().GetCounter(), e.action.quest.quest);
                    ...
                    else if (IsUnit(*itr)) // Special handling for vehicles
                    {
                        if (Vehicle* vehicle = (*itr)->ToUnit()->GetVehicleKit())
                        {
                            for (auto seat : vehicle->Seats)
                                if (Player* player = ObjectAccessor::GetPlayer(*(*itr), seat.second.Passenger.Guid))
        ...
        case SMART_ACTION_SET_DATA:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr) && (*itr)->ToCreature()->AI())
                    (*itr)->ToCreature()->AI()->SetData(e.action.setData.field, e.action.setData.data);
                else if (IsGameObject(*itr) && (*itr)->ToGameObject()->AI())
        ...
        case SMART_ACTION_SUMMON_CREATURE:
        {
            ...
            if (targets)
            {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    if (Creature* summon = GetBaseObject()->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
                        if (e.action.summonCreature.attackInvoker && summon->AI())
            ...
            if (Creature* summon = GetBaseObject()->SummonCreature(e.action.summonCreature.creature, e.target.x, e.target.y, e.target.z, e.target.o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
                if (unit && e.action.summonCreature.attackInvoker && summon->AI())
            ...
            for (ObjectList::const_iterator itr = casters->begin(); itr != casters->end(); ++itr)
            {
                ...
                for (ObjectList::const_iterator it = targets->begin(); it != targets->end(); ++it)
                {
                    ...
                    if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*it)->ToUnit()->HasAura(e.action.cast.spell))
                    {
                        ...
                        TC_LOG_DEBUG("scripts.ai", "Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: %s Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*it)->GetGUID().ToString().c_str(), (*it)->GetEntry(), uint32((*it)->GetTypeId()));
                }
            }
        ...
        case SMART_ACTION_SUMMON_CREATURE_GROUP:
        {
            ...
            for (std::list<TempSummon*>::const_iterator itr = summonList.begin(); itr != summonList.end(); ++itr)
                if (unit && e.action.creatureGroup.attackInvoker && (*itr)->AI())
        ...
        case SMART_ACTION_MOVE_HOME_POS:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_MOVE_HOME_POS: Creature %u MoveTargetedHome", me->GetGUID().GetCounter());
            break;
        }
        ...
        case SMART_ACTION_FLEE:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    (*itr)->ToCreature()->GetMotionMaster()->MoveFleeing(me, e.action.flee.fleeTime, e.action.flee.run);
                    if (e.action.flee.withEmote)
                    {
                        BroadcastTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, BROADCAST_TEXT_FLEE_FOR_ASSIST, NULL, (*itr)->ToCreature()->getGender());
        ...
        case SMART_ACTION_ADD_THREAT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsUnit(*itr))
                    me->GetThreatManager().AddThreat((*itr)->ToUnit(), float(e.action.threatPCT.threatINC) - float(e.action.threatPCT.threatDEC), nullptr, true, true);
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= NULL*/)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_CREATURE_GUID:
        {
            ...
            target = FindCreature(scriptTrigger ? scriptTrigger : baseObject, e.target.unitGUID.dbGuid);
        ...
        case SMART_TARGET_GAMEOBJECT_GUID:
        {
            ...
            target = FindGameObject(scriptTrigger ? scriptTrigger : baseObject, e.target.goGUID.dbGuid);
        ...
        {
            Creature* target = baseObject->FindNearestCreature(e.target.closest.entry, (float)(e.target.closest.dist ? e.target.closest.dist : 100), !e.target.closest.dead);
            if (target)
                l->push_back(target);
            break;
        }
        ...
        {
            GameObject* target = baseObject->FindNearestGameObject(e.target.closest.entry, (float)(e.target.closest.dist ? e.target.closest.dist : 100));
            if (target)
                l->push_back(target);
            break;
        }
        case SMART_TARGET_CLOSEST_PLAYER:
        {
            if (WorldObject* obj = GetBaseObject())
                if (Player* target = obj->FindNearestPlayer(float(e.target.playerDistance.dist)))
                    l->push_back(target);
            break;
        }
...
ObjectList* SmartScript::GetWorldObjectsInDist(float dist)
{
    ...
    if (obj)
    {
        ...
        auto searcher = Trinity::createBaseObjectSearcher<WorldObject, Trinity::ContainerAction>(obj, *targets, u_check);
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_DISTANCE_CREATURE:
        {
            ...
            {
                creature = FindCreature(me, e.event.distance.guid);
                if (!creature)
                    return;
        ...
        case SMART_EVENT_DISTANCE_GAMEOBJECT:
        {
            ...
            if (e.event.distance.guid != 0)
            {
                gameobject = FindGameObject(me, e.event.distance.guid);
                if (!gameobject)
                    return;
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    auto searcher = Trinity::createBaseObjectSearcher<Creature, Trinity::ContainerAction>(me, _list, u_check);
