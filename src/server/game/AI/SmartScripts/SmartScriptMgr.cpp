...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    do
    {
        ...
        const int32 entry = fields[0].GetInt32();
        ...
        auto& sourceMap = mEventMap[source_type];

        // creature entry / guid not found in storage, create empty event list for it and increase counters
        if (!sourceMap.count(temp.entryOrGuid))
        {
            ++count;
            sourceMap.try_emplace(temp.entryOrGuid);
        }

        // store the new event
        sourceMap[entry].push_back(temp);
    }
    while (result->NextRow());

    // TO-DO: Find better way
    for (auto& map : mEventMap)
    {
        for (auto [event, unused] : map)
        {
            util::meta::Unreferenced(unused);
