...
class GAME_API SmartScript
{
    public:
        ...
        static GameObject* FindGameObject(WorldObject* searchObject, uint32 guid)
        {
            auto gameObject = searchObject->GetMap()->GetGameObjectBySpawnId(guid);
            
            return gameObject.get_ptr();
        }

        static Creature* FindCreature(WorldObject* searchObject, uint32 guid)
        {
            auto creature = searchObject->GetMap()->GetCreatureBySpawnId(guid);

            return creature.get_ptr();
        }
        ...
        {
            WorldObject* lookupRoot = me ? static_cast<WorldObject*>(me) : static_cast<WorldObject*>(go);
            if (lookupRoot)
            {
                if (meOrigGUID)
                {
                    if (Creature* creature = ObjectAccessor::GetCreature(*lookupRoot, meOrigGUID))
                    {
                        me = creature;
                        go = nullptr;
                    }
                ...
                if (goOrigGUID)
                {
                    if (GameObject* obj = ObjectAccessor::GetGameObject(*lookupRoot, goOrigGUID))
                    {
                        me = nullptr;
                        go = obj;
                    }
