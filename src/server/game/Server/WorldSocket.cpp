...
{
public:
    EncryptablePacket() = default;
    EncryptablePacket(WorldPacket&& packet, bool encrypt) : WorldPacket(std::move(packet)), _encrypt(encrypt) { }
    EncryptablePacket(EncryptablePacket&& packet) = default;

    EncryptablePacket& operator=(EncryptablePacket&&) = default;
...
bool WorldSocket::Update()
{
    ...
    while (const auto queued = _bufferQueue.Dequeue())
    {
        // buffers can skip data on the front
        // todo: should have its own mechanism, outside of rpos
        const uint32 startPos = queued->rpos();
        const uint32 queuedSize = queued->size() - startPos;
        ServerPktHeader header(queuedSize + 2, queued->GetOpcode());
        if (queued->NeedsEncryption())
            _authCrypt.EncryptSend(header.header, header.getHeaderLength());

        if (buffer.GetRemainingSpace() < queuedSize + header.getHeaderLength())
        {
        ...
        if (buffer.GetRemainingSpace() >= queuedSize + header.getHeaderLength())
        {
            ...
            if (queuedSize)
                buffer.Write(queued->contents() + startPos, queuedSize);
        }
        else    // single packet larger than 4096 bytes
        {
            MessageBuffer packetBuffer(queuedSize + header.getHeaderLength());
            packetBuffer.Write(header.header, header.getHeaderLength());
            if (!queued->empty())
                packetBuffer.Write(queued->contents() + startPos, queuedSize);
...
void WorldSocket::HandleSendAuthSession()
{
    ...
    SendPacketAndLogOpcode(std::move(packet));
}
...
WorldSocket::ReadDataHandlerResult WorldSocket::ReadDataHandler()
{
    ...
    switch (opcode)
    {
        ...
        default:
        {
            ...
            // Our Idle timer will reset on any non PING and non CMSG_WARDEN_DATA opcodes.
...
void WorldSocket::SendPacket(WorldPacket&& packet)
{
    ...
    _bufferQueue.Enqueue(std::move(packet), _authCrypt.IsInitialized());
}

void WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    std::unique_ptr<AuthSession> authSession = std::make_unique<AuthSession>();
    ...
    recvPacket.extract(authSession->Digest, 20);
    authSession->AddonInfo.append(recvPacket);
    ...
    LoginDatabase.AsyncQuery(stmt)
        .via(_executor).then([this, authSession = std::move(authSession)](PreparedQueryResult result) mutable
        {
            HandleAuthSessionCallback(std::move(authSession), std::move(result));
        });
}

void WorldSocket::HandleAuthSessionCallback(std::unique_ptr<AuthSession> authSession, PreparedQueryResult result)
{
    ...
    */
    ...
    _worldSession->LoadPermissionsAsync()
            .via(_executor).then([this](PreparedQueryResult result) { LoadSessionPermissionsCallback(std::move(result)); });
    AsyncRead();
}
...
bool WorldSocket::HandlePing(WorldPacket& recvPacket)
{
    ...
    if (_LastPingTime == std::chrono::steady_clock::time_point())
    {
