...
namespace RG { namespace Suspicious
{
class Monitor;
}}
...
enum AuctionhouseQueryIndex
{
    AUCTIONHOUSE_QUERY_NORMAL = 0,
    AUCTIONHOUSE_QUERY_FULL   = 1,
    MAX_AUCTIONHOUSE_QUERY
};

enum ChangeDynamicDifficultyResult
{
    ERR_DIFFICULTY_CHANGE_COOLDOWN_S                            = 1,
    ERR_DIFFICULTY_CHANGE_WORLDSTATE                            = 2,
    ERR_DIFFICULTY_CHANGE_ENCOUNTER                             = 3,
    ERR_DIFFICULTY_CHANGE_COMBAT                                = 4,
    ERR_DIFFICULTY_CHANGE_PLAYER_BUSY                           = 5,
    ERR_DIFFICULTY_CHANGE_SUCCESS                               = 6,
    ERR_DIFFICULTY_CHANGE_ALREADY_STARTED                       = 7
};
...
//process only map-safe packets in Map::Update()
...
class GAME_API WorldSession : public ExecutionContext<WorldSession>
{
    public:
        ...
        void InitializeSessionCallback(QueryResultHolder realmHolder);
        ...
        void HandlePackets(PacketFilter& updater);
        ...
        WorldPacket BuildPartyMemberStatsChangedPacket(Player* player);
        ...
        const RG::Suspicious::Monitor* GetMonitor() const { return &_monitor; }
    ...
    public:                                                 // opcodes handlers
        ...
        void HandleCharEnum(SharedPreparedQueryResult result);
        ...
        uint32 MovePetToStableSlot(uint32 petNumber, uint8 slot);
        ...
        void HandleWrappedItemCallback(PreparedQueryResult result, ObjectGuid guid);
    ...
    private:
        void BeginLoadPlayer(ObjectGuid playerGuid);
        std::vector<QueryResultHolder> _petLoadCallback;
        void HandlePetListOpcodeCallBack(ObjectGuid playerGuid, PreparedQueryResult result);
    private:
     ...
     // std::string m_LAddress;                             // Last Attempted Remote Adress - we can not set attempted ip for a non-existing session!
        ...
        uint32 recruiterId;
        bool isRecruiter;
        LockedQueue<WorldPacket, boost::container::deque<WorldPacket>> _recvQueue; // std::deque won't work with incomplete types
        time_t timeLastWhoCommand;
        rbac::RBACData* _RBACData;
        uint32 expireTime;
        bool forceExit;
        ObjectGuid m_currentBankerGUID;
        uint32 _lastAuctionhouseQueries[MAX_AUCTIONHOUSE_QUERY];

    private:
        bool userRequestedLogout;
        bool _dropIncoming;
        RG::Suspicious::Monitor _monitor;

    // ####################################
    // ####  Mute
    // ####################################
    public:
        const MuteMgr& GetMuteMgr() const;
        MuteMgr& GetMuteMgr();
    ...
    public:
        bool IsCommandExecuting() { return commandExecuting; }
        void SetCommandExecuting(bool commandExecuting) { this->commandExecuting = commandExecuting; }

    private:
        bool commandExecuting = false;

    public:
        api::addon::Endpoint& GetAddonAPIEndpoint() { return addonAPIEndpoint; }

    private:
        api::addon::Endpoint addonAPIEndpoint{ *this };
};
#endif
/// @}
