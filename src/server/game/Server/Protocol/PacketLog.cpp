...
void PacketLog::LogPacket(WorldPacket const& packet, Direction direction)
{
    ...
    // todo: rework, see comment in WorldSocket.cpp
    const uint32 startPos = packet.rpos();
    const uint32 packetSize = packet.size() - packet.rpos();
    ...
    header.OptionalDataSize = 0;
    header.Length = packetSize + 4;
    header.Opcode = packet.GetOpcode();

    fwrite(&header, sizeof(header), 1, _file);
    if (packetSize)
        fwrite(packet.contents() + startPos, 1, packetSize, _file);
