...
Corpse* Corpse::CreateFor(Player& player, bool isPvPDeath)
{
    Corpse* corpse = new Corpse(isPvPDeath ? CORPSE_RESURRECTABLE_PVP : CORPSE_RESURRECTABLE_PVE);

    if (!corpse->Create(sObjectMgr->GenerateLowGuid(HighGuid::Corpse), &player))
    {
        TC_LOG_ERROR("entities.corpse", "Failed to create corpse for player (%s).", player.GetGUID().ToString().c_str());
        delete corpse;
        return nullptr;
    }
...
Corpse* Corpse::LoadFromDB(uint32 guid, Field* fields, CorpseType type)
{
    Corpse* corpse = new Corpse(type);

    if (!corpse->LoadCorpseFromDB(guid, fields))
    {
        TC_LOG_ERROR("entities.corpse", "Failed to load corpse (GUID: %u).", guid);
        delete corpse;
        return nullptr;
    }
...
Corpse* Corpse::CreateBones(uint32 guidLow, Map& map)
{
    Corpse* bones = new Corpse(CORPSE_BONES);
    bones->Create(guidLow, &map);
    return bones;
}
