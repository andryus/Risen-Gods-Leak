...
std::unordered_map<ObjectGuid, std::unordered_set<Unit*>> debugMap;
...
namespace
{
    static bool isAttackedFromBehind(const Unit& victim, const Unit& attacker)
    {
        return !victim.HasInArc(float(M_PI), &attacker) && !victim.HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION);
    }
}
...
DamageInfo::DamageInfo(Unit* _attacker, Unit* _victim, uint32 _damage, uint32 _absorb, uint32 _resist, uint32 _block, SpellInfo const* _spellInfo, SpellSchoolMask _schoolMask, DamageEffectType _damageType)
    : m_attacker(_attacker), m_victim(_victim), m_damage(_damage), m_absorb(_absorb),m_resist(_resist),m_block(_block), m_spellInfo(_spellInfo), m_schoolMask(_schoolMask),
    m_damageType(_damageType), m_attackType(BASE_ATTACK)
{}
...
ProcEventInfo::ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget,
                             ...
                             Spell* spell, SpellInfo const* spellInfo,
    ...
    _spellInfo(spellInfo), _damageInfo(damageInfo), _healInfo(healInfo)
{ }
...
#endif
Unit::Unit() :
    WorldObject(), m_movedPlayer(NULL), m_lastSanctuaryTime(0),
    IsAIEnabled(false), NeedChangeAI(false), LastCharmerGUID(), isInRelocationList(false),
    ...
    m_removedAurasCount(0), motionMaster(this), m_regenTimer(0), m_ThreatManager(this),
...
#pragma warning(default:4355)
    ...
    _attackTimerOverlap[BASE_ATTACK] = 0;
    _attackTimerOverlap[OFF_ATTACK] = 0;
    _attackTimerOverlap[RANGED_ATTACK] = 0;
    m_modAttackSpeedPct[BASE_ATTACK] = 1.0f;
    m_modAttackSpeedPct[OFF_ATTACK] = 1.0f;
    m_modAttackSpeedPct[RANGED_ATTACK] = 1.0f;
    m_autoAttackTimerPause.Reset(0);
...
void Unit::Update(uint32 p_time)
{
    ...
    uint32 diff_size[UNIT_DIFF_VALUE_COUNT] = {0};
    uint32 diffTime = 0;
    uint32 startTime = 0;
    if (lagReportActive)
    {
    ...
    if (lagReportActive)
    {
        diff_size[UNIT_DIFF_EVENT_UPDATE] = GetMSTimeDiffToNow(diffTime);
        diffTime = getMSTime();
    }

    *this <<= Fire::ProcessEvents.Bind(DiffParam(p_time));

    _UpdateSpells(p_time);

    if (lagReportActive)
    {
        diff_size[UNIT_DIFF_SPELL_UPDATE] = GetMSTimeDiffToNow(diffTime);
    ...
    if (m_autoAttackTimerPause.Passed())
        UpdateAttackTimer(p_time);
    else
        m_autoAttackTimerPause.Update(p_time);
    ...
    UpdateVisibilityGroup(p_time);

    if (lagReportActive)
    {
        diff_size[UNIT_DIFF_MISC_UPDATE] = GetMSTimeDiffToNow(diffTime);

        diffTime = getMSTime();
    }

    UpdateSplineMovement(p_time);
    motionMaster.UpdateMotion(p_time);

    if (lagReportActive)
        diff_size[UNIT_DIFF_MOTION_UPDATE] = GetMSTimeDiffToNow(diffTime);

    if (lagReportActive &&
        GetMSTimeDiffToNow(startTime) > sWorld->getIntConfig(CONFIG_LAG_REPORT_MAX_DIFF_UNITS))
        sLog->outMessage("rg.lagreport", LOG_LEVEL_WARN, "[Unit Lag Report] ID: %u (%i) SUM: %u EVENT_UPDATE: %u SPELL_UPDATE: %u MISC_UPDATE: %u MOTION_UPDATE: %u",
            GetTypeId() == TYPEID_PLAYER ? GetGUID().GetCounter() : GetEntry(),
            GetTypeId() == TYPEID_PLAYER,
            GetMSTimeDiffToNow(startTime),
            diff_size[UNIT_DIFF_EVENT_UPDATE],
            diff_size[UNIT_DIFF_SPELL_UPDATE],
            diff_size[UNIT_DIFF_MISC_UPDATE],
            diff_size[UNIT_DIFF_MOTION_UPDATE]);
}
...
void Unit::UpdateAttackTimer(uint32 diff)
{
    constexpr WeaponAttackType attacks[] = { BASE_ATTACK, OFF_ATTACK, RANGED_ATTACK };

    for (const WeaponAttackType attackType : attacks)
    {
        if (uint32 base_att = getAttackTimer(attackType))
        {
        	setAttackTimer(attackType, (diff >= base_att ? 0 : base_att - diff), (diff > base_att ? diff - base_att : 0));

	    	if (isAttackReady(attackType))
	    		*this <<= Fire::AttackTimer((AttackType)attackType);
	    }
	}
}
...
{
    uint32 tmp = uint32(GetAttackTime(type) * m_modAttackSpeedPct[type]);

    if (IsOfType<Creature>()) // TODO: hack
        setAttackTimerOverlap(type, 0);

    if (_attackTimerOverlap[type] > 50)
        setAttackTimerOverlap(type, 0); // Reset overlap, since it should only reduce lag between real hit and server delay and should not increase timer beyond it

    m_attackTimer[type] = _attackTimerOverlap[type] > tmp ? 0 : tmp - _attackTimerOverlap[type];
    setAttackTimerOverlap(type, 0);
}
...
bool Unit::CanReceiveThreatFrom(const Unit& target) const
{
    if (!HasScript()) // @todo: for all scripts
        return true;
        
    if (const Creature* creature = To<Creature>())
        if (creature->IsTrigger())
            return true;

    if (target.IsOfType<Player>())
        return !IsImmuneToPC();
    else
        return !IsImmuneToNPC();
}
...
void Unit::DealDamageMods(Unit* victim, uint32 &damage, uint32* absorb)
{
    ...
    /**
    * This is a hack: the Eye is charmed an has no AI enabled.
    * Therefore we added this hack here to prevent abuse of damage exploits.
    * @TODO: Find a more general solution for Pets.
    **/
    if (GetEntry() == 4277) // Eye of Kilrogg
    {
        if (damage > 400)
            damage = frand(6.0f, 7.0f) * getLevel();
        else
            damage = frand(2.0f, 3.5f) * getLevel();
    }
}

uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    if (victim->isUnkillable)
        damage = std::min(damage, victim->GetHealth() - 1);

    damage *= (*victim <<= Query::DamageTakenFromMultiplier.Bind(this));
    damage *= (*this <<= Query::DamageDealtToMultiplier.Bind(victim));

    *victim <<= Fire::DamageTaken.Bind({*this, damage});
    ...
    {
        if (victim && victim->ToCreature()->HasFlagExtra(CREATURE_FLAG_EXTRA_NO_CREATURE_DAMAGE) && this->GetTypeId() == TYPEID_UNIT && !this->IsPet() && !this->IsCharmedOwnedByPlayerOrPlayer())
            return 0;
    }
    ...
    if (damagetype != NODAMAGE)
    {
       ...
       if (!damage && damagetype != DOT && cleanDamage && cleanDamage->absorbed_damage)
        ...
        for (AuraEffectList::iterator i = vCopyDamageCopy.begin(); i != vCopyDamageCopy.end(); ++i)
        {
            ...
            uint32 sharedDamage = CalculatePct(damage, (*i)->GetAmount());

            SpellNonMeleeDamage damageInfo(victim, shareDamageTarget, spell->Id, schoolMask);
            damageInfo.damage = sharedDamage;
            damageInfo.physicalLog = true;
            victim->CalcAbsorbResist(shareDamageTarget, schoolMask, SPELL_DIRECT_DAMAGE, sharedDamage, sharedDamage, &damageInfo.absorb, &damageInfo.resist, spell);
            damageInfo.damage -= damageInfo.absorb;
            victim->DealDamageMods(damageInfo.target, damageInfo.damage, &damageInfo.absorb);
            victim->SendSpellNonMeleeDamageLog(&damageInfo);
            // Damage should be done by initial damage dealer to avoid that in group encounters, player
            // deal not enough damage to get the loot (PlayerDamageReq is called in deal damage)
            DealDamage(shareDamageTarget, damageInfo.damage, NULL, NODAMAGE, spell->GetSchoolMask(), spell, false);
        }
    }
    ...
    TC_LOG_DEBUG("entities.unit", "Unit %s dealt %u damage to unit %s", GetGUID().ToString().c_str(), damage, victim->GetGUID().ToString().c_str());
    ...
    else if (GetMap() && GetMap()->IsBattleground() && GetTypeId() == TYPEID_UNIT && GetOwner() && GetOwner()->GetTypeId() == TYPEID_PLAYER)
    {
...
void Unit::DoDelayed(std::chrono::milliseconds const& delay, std::function<void(Unit&)> fn)
{
    class DoDelayedEvent : public BasicEvent
    {
    public:
        DoDelayedEvent(Unit& owner, std::function<void(Unit&)> fn) : _owner(owner), _fn(std::move(fn)), BasicEvent() { }
    ...
    private:
        Unit& _owner;
        std::function<void(Unit&)> _fn;
    };

    m_Events.AddEvent(new DoDelayedEvent(*this, std::move(fn)), m_Events.CalculateTime(delay.count()));
}

void Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster, bool softRefresh)
{
    if (!spellInfo)
    {
    ...
    spell->SetSoftRefresh(softRefresh);
...
/**
* @brief casts the given spell (by its SpellInfo) on the victim to only softRefresh existing auras. Should be used if, as 
* a side effect of a spellcast, another aura on the target should get softrefreshed (meaning that the effect of dots/hots 
* should not get completely recalculated). However this does NOT check if the target does have the aura that should get softrefreshed.
* If it doesn't have it, it will get created, so in most cases you want to first assure the target has the aura (see example)
* The advantage of using this over directly modifiying the aura is, that also proccs, that should be done on reapplying
* the aura, are created. And the duration should be correct because for some reason the Aura itself does not
* take into account caster haste this is only done during casting the spell.
* @param victim the target of the spell
* @param spellInfo the SpellInfo of the Aura that should get recasted to softrefresh it
* @code
* //Refresh Shadow Word: Pain on target during mindfly (Pain and Suffering Proc)
* if (Unit* unitTarget = GetHitUnit()) 
*    if (AuraEffect* aur = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST, 0x8000, 0, 0, GetCaster()->GetGUID())) //Check that the unit actually has SW:Pain
*    {
*        if (Unit* caster = GetCaster())
*            caster->CastSoftRefreshSpell(unitTarget, aur->GetBase()->GetSpellInfo());  //Then cast the soft refresh spell
*    }
* @endcode         
*/
void Unit::CastSoftRefreshSpell(Unit* victim, SpellInfo const* spellInfo)
{
    SpellCastTargets targets;
    targets.SetUnitTarget(victim);
    CastSpell(targets, spellInfo, NULL, TRIGGERED_FULL_MASK, NULL, NULL, ObjectGuid::Empty, true);
}
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit)
{
        ...
        switch (spellInfo->DmgClass)
        {
            ...
            case SPELL_DAMAGE_CLASS_MELEE:
            {
                ...
                if (attackType != RANGED_ATTACK)
                {
                    ...
                    ApplyResilience(victim, NULL, &noncritDamage, false, CR_CRIT_TAKEN_MELEE);
                }
                else
                {
                    ApplyResilience(victim, NULL, &damage, crit, CR_CRIT_TAKEN_RANGED);
                    ApplyResilience(victim, NULL, &noncritDamage, false, CR_CRIT_TAKEN_RANGED);
                }
                break;
            }
                // Magical Attacks
            ...
            case SPELL_DAMAGE_CLASS_MAGIC:
            {
                ...
                ApplyResilience(victim, NULL, &noncritDamage, false, CR_CRIT_TAKEN_SPELL);
                break;
            }
            default:
                break;
    ...
    if (damage > 0)
    {
        CalcAbsorbResist(victim, damageSchoolMask, SPELL_DIRECT_DAMAGE, damage, noncritDamage, &damageInfo->absorb, &damageInfo->resist, spellInfo);
...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss)
{
    ...
    if (IsPet() && damageInfo->HitInfo & SPELL_HIT_TYPE_CRIT)
        ToPet()->SetDoneSpellCrit();
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    if (int32(damageInfo->damage) > 0)
    {
        ...
        CalcAbsorbResist(damageInfo->target, SpellSchoolMask(damageInfo->damageSchoolMask), DIRECT_DAMAGE, damageInfo->damage, damageInfo->damage, &damageInfo->absorb, &damageInfo->resist);
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->TargetState == VICTIMSTATE_PARRY)
    {
        if (victim->GetTypeId() != TYPEID_UNIT ||
            !(((Creature*)victim)->GetCreatureTemplate()->flags_extra &
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->CastItemCombatSpell(victim, damageInfo->attackType, NULL, damageInfo->procAttacker, damageInfo->procVictim, damageInfo->procEx);
    ...
    if (damageInfo->damage)
    {
        ...
        for (AuraEffectList::const_iterator dmgShieldItr = vDamageShieldsCopy.begin(); dmgShieldItr != vDamageShieldsCopy.end(); ++dmgShieldItr)
        {
            ...
            constexpr auto damageType = SELF_DAMAGE;

            if (Unit* caster = (*dmgShieldItr)->GetCaster())
            {
                damage = caster->SpellDamageBonusDone(this, i_spellProto, damage, damageType);
                damage = this->SpellDamageBonusTaken(caster, i_spellProto, damage, damageType);
            }
            ...
            victim->DealDamage(this, damage, 0, damageType, i_spellProto->GetSchoolMask(), i_spellProto, true);
        }
    }
}
...
bool Unit::IsDamageReducedByArmor(SpellSchoolMask schoolMask, SpellInfo const* spellInfo, uint8 effIndex)
{
    ...
    if (spellInfo)
    {
        ...
        else // if no spezific effect provided, search for any direct damge effect and check for bleed mechanic
        {
            for (uint8 eff = EFFECT_0; eff < MAX_SPELL_EFFECTS; ++eff)
            {
                if (spellInfo->Effects[eff].Effect == SPELL_EFFECT_SCHOOL_DAMAGE)
                    if (spellInfo->GetEffectMechanicMask(eff) & (1<<MECHANIC_BLEED))
                        return false;
            }
        }
    }
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        float maxArmorPen = 0;
        if (getLevel() < 60)
            maxArmorPen = float(400 + 85 * getLevel());
        else
            maxArmorPen = 400 + 85 * getLevel() + 4.5f * 85 * (getLevel() - 59);
...
uint32 Unit::GetSpellPenetration() const
{
    ...
    if (this->ToPlayer())
        Unit const* source = ToPlayer();
    else if (this->GetTypeId() == TYPEID_UNIT && this->GetOwner() && this->GetOwner()->GetTypeId() == TYPEID_PLAYER)
        Unit const* source = GetOwner()->ToPlayer();
...
uint32 Unit::CalcSpellResistance(Unit* victim, SpellSchoolMask schoolMask, SpellInfo const* spellInfo) const
{
    ...
    if (spellInfo && spellInfo->AttributesEx3 & SPELL_ATTR3_IGNORE_HIT_RESULT && (!spellInfo || spellInfo->HasAttribute(SPELL_ATTR4_IGNORE_RESISTANCES)))
        return 0;

    // Chaos Bolt and Envenom should not be resisted
    if (spellInfo && (spellInfo->SpellIconID == 3178 || spellInfo->SpellIconID == 2237))
        return 0;
    ...
    // Get spell penetration, for pets take the spell penetration from their owner
    uint32 spellPenetration = GetSpellPenetration();
    ...
    victimResistance += (levelDiff * 8); // Level diff resistance cannot be pierced
    ...
    if (spellInfo && spellInfo->IsBinarySpell() &&
        (GetTypeId() == TYPEID_PLAYER ||
        (GetEntry() == WORLD_TRIGGER && GetOwner() && GetOwner()->GetTypeId() == TYPEID_PLAYER)))
    {
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 const nonCritDamage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo /*= NULL*/)
{
    ...
    uint32 spellResistance = (spellInfo && spellInfo->IsBinarySpell() && GetTypeId() == TYPEID_PLAYER) ? 0 : CalcSpellResistance(victim, schoolMask, spellInfo);
    dmgInfo.ResistDamage(CalculatePct(nonCritDamage, spellResistance));

    ApplyAbsorbResist(dmgInfo);

    // split damage auras - only when not damaging self
    if (victim != this)
    {
        // generic function for all types of split-damage
        const auto applyAuras = [this, &dmgInfo, victim, schoolMask](AuraType type, auto functor)
        {            
            const AuraEffectList& vSplitDamage = victim->GetAuraEffectsByType(type);

            // We're going to call functions which can modify content of the list during iteration over it's elements
            // Let's copy the list so we can prevent iterator invalidation (using vector for performance)
            std::vector<AuraEffect*> vSplitDamageCopy;
            vSplitDamageCopy.reserve(vSplitDamage.size());

            for(AuraEffect* effect : vSplitDamage)
            {
                vSplitDamageCopy.push_back(effect);
            }

            for (AuraEffect* effect : vSplitDamageCopy)
            {
                AuraApplication const* aurApp = effect->GetBase()->GetApplicationOfTarget(victim->GetGUID());
                if (!aurApp)
                    continue;
                // check damage school mask
                if (!(effect->GetMiscValue() & schoolMask))
                    return;

                // Damage can be splitted only if aura has an alive caster
                Unit* caster = effect->GetCaster();
                if (!caster || (caster == victim) || !caster->IsInWorld() || !caster->IsAlive())
                    continue;

                // calculate basic split-damage depending on type
                int32 splitDamage = functor(effect, aurApp);
                ...
                {
                    victim->SendSpellMiss(caster, effect->GetSpellInfo()->Id, SPELL_MISS_IMMUNE);
                    return;
                }
                ...
                DamageInfo splitDmgInfo(this, caster, splitted, dmgInfo.GetSpellInfo(), dmgInfo.GetSchoolMask(), dmgInfo.GetDamageType());
                caster->ApplyAbsorbResist(splitDmgInfo);
                splitted = splitDmgInfo.GetDamage();
                splitted_absorb += splitDmgInfo.GetAbsorb();

                // source of damage should show up as victim of the original attack
                victim->SendSpellNonMeleeDamageLog(caster, effect->GetSpellInfo()->Id, splitted + splitDmgInfo.GetAbsorb(), schoolMask, splitted_absorb, 0, false, 0, false);

                CleanDamage cleanDamage = CleanDamage(splitted, 0, BASE_ATTACK, MELEE_HIT_NORMAL);
                DealDamage(caster, splitted, &cleanDamage, DIRECT_DAMAGE, schoolMask, effect->GetSpellInfo(), false);
            }
        };

        // flat damage
        const auto flatSplitDamage = [](AuraEffect* effect, AuraApplication const*)
        {
            return effect->GetAmount();
        };

        applyAuras(SPELL_AURA_SPLIT_DAMAGE_FLAT, flatSplitDamage);

        // percentdamage
        const auto pctSplitDamage = [&dmgInfo](AuraEffect* effect, AuraApplication const* aurApp)
        {
            uint32 splitDamage = CalculatePct(dmgInfo.GetDamage(), effect->GetAmount());

            effect->GetBase()->CallScriptEffectSplitHandlers(effect, aurApp, dmgInfo, splitDamage);

            return (int32)splitDamage;
        };

        applyAuras(SPELL_AURA_SPLIT_DAMAGE_PCT, pctSplitDamage);
...
void Unit::ApplyAbsorbResist(DamageInfo& dmgInfo)
{
    const SpellInfo* spellInfo = dmgInfo.GetSpellInfo();
    Unit* victim = dmgInfo.GetVictim();
    const SpellSchoolMask schoolMask = dmgInfo.GetSchoolMask();
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra, bool ignoreLOS)
{
    ...
    if ((attType == BASE_ATTACK || attType == OFF_ATTACK) && !IsWithinLOSInMap(victim) && !ignoreLOS)
        return;
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            TC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (Player) %u attacked %u (TypeId: %hhu) for %u dmg, absorbed %u, blocked %u, resisted %u.",
            ...
            TC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (NPC)    %u attacked %u (TypeId: %hhu) for %u dmg, absorbed %u, blocked %u, resisted %u.",
                GetGUID().GetCounter(), victim->GetGUID().GetCounter(), victim->GetTypeId(), damageInfo.damage, damageInfo.absorb, damageInfo.blocked_amount, damageInfo.resist);
    }
}
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType) const
{
    ...
    if (victim->GetTypeId() == TYPEID_UNIT)
    {
        if (victim->ToCreature()->IsInEvadeMode() || victim->ToCreature()->IsEvadingAttacks())
            return MELEE_HIT_EVADE;

        if (victim->ToCreature()->IsTotem())
            return MELEE_HIT_NORMAL;
    }
    // sitting player should always be hit for 100% damage
    // while it should show as crit, it is not known whether it shoudl use the mechanic
    else if(victim->IsSitState()) 
        return MELEE_HIT_CRIT;

    constexpr combat::TableFormatEntry<MeleeHitOutcome> tableFormat[] =
    {
        { combat::AttackTableEntry::Miss, MELEE_HIT_MISS },
        { combat::AttackTableEntry::Dodge , MELEE_HIT_DODGE },
        { combat::AttackTableEntry::Parry , MELEE_HIT_PARRY },
        { combat::AttackTableEntry::Glancing , MELEE_HIT_GLANCING },
        { combat::AttackTableEntry::Block , MELEE_HIT_BLOCK },
        { combat::AttackTableEntry::Crit , MELEE_HIT_CRIT },
        { combat::AttackTableEntry::Crushing , MELEE_HIT_CRUSHING },
    };

    const auto table = CreateAttackTable(*victim, attType, nullptr);
    const auto attackTable = combat::AttackTableHandler::BuildAttackTable(table, tableFormat);

    return combat::AttackTableHandler::RollAttackTable(attackTable, MELEE_HIT_NORMAL);
}

uint32 Unit::CalculateDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, bool average)
{
    ...
    return average ? (uint32)((minDamage+maxDamage)/2.0f) : urand(uint32(minDamage), uint32(maxDamage));
}
...
bool Unit::isSpellBlocked(Unit* victim, SpellInfo const* spellProto, WeaponAttackType attackType)
{
    ...
    if (!isAttackedFromBehind(*victim, *this))
    {
        ...
        // cannot block while casting or while stunned
...
SpellMissInfo Unit::MeleeSpellHitResult(const Unit& victim, const SpellInfo& spellInfo) const
{
    ...
    auto table = CreateAttackTable(victim, BASE_ATTACK, &spellInfo);

    bool& canDodge = table.checks[combat::AttackTableEntry::Dodge];
    bool& canParry = table.checks[combat::AttackTableEntry::Parry];
    bool& canBlock = table.checks[combat::AttackTableEntry::Block];

    const bool canDefendAgainst = !spellInfo.HasAttribute(SPELL_ATTR0_IMPOSSIBLE_DODGE_PARRY_BLOCK);

    // check spell reqs for evasion
    canParry &= canDefendAgainst;
    canDodge &= canDefendAgainst;
    canBlock &= canDefendAgainst & spellInfo.HasAttribute(SPELL_ATTR3_BLOCKABLE_SPELL);


    // Check for attack from behind
    if (!isAttackedFromBehind(victim, *this) && spellInfo.HasAttribute(SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET))
        canParry = false;

    // Ignore combat result aura
    const Unit::AuraEffectList& ignore = GetAuraEffectsByType(SPELL_AURA_IGNORE_COMBAT_RESULT);
    for (const AuraEffect* effect : ignore)
    {
        ...
        case MELEE_HIT_PARRY:
            ...
            break;
        default:
            TC_LOG_DEBUG("entities.unit", "Spell %u SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state %d", effect->GetId(), effect->GetMiscValue());
            break;
        }
    }

    constexpr combat::TableFormatEntry<SpellMissInfo> tableFormat[] =
    {
        { combat::AttackTableEntry::Miss, SPELL_MISS_MISS },
        { combat::AttackTableEntry::Resist, SPELL_MISS_RESIST },
        { combat::AttackTableEntry::Dodge , SPELL_MISS_DODGE },
        { combat::AttackTableEntry::Parry , SPELL_MISS_PARRY },
        { combat::AttackTableEntry::Block , SPELL_MISS_BLOCK },
    };

    const auto specialAttackTable = combat::AttackTableHandler::BuildAttackTable(table, tableFormat);

    return combat::AttackTableHandler::RollAttackTable(specialAttackTable, SPELL_MISS_NONE);
}

SpellMissInfo Unit::RangedSpellHitResult(const Unit& victim, const SpellInfo& spellInfo) const
{
    const auto baseTable = CreateAttackTable(victim, RANGED_ATTACK, &spellInfo);

    constexpr combat::TableFormatEntry<SpellMissInfo> tableFormat[] =
    {
        { combat::AttackTableEntry::Miss, SPELL_MISS_MISS },
        { combat::AttackTableEntry::Resist, SPELL_MISS_RESIST },
        { combat::AttackTableEntry::Deflect , SPELL_MISS_DEFLECT }
    };

    const auto specialAttackTable = combat::AttackTableHandler::BuildAttackTable(baseTable, tableFormat);
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    // Only Max Positive because Faerie Fire and Misery should not stack
    modHitChance += victim->GetMaxPositiveAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_SPELL_HIT_CHANCE, schoolMask);
    modHitChance += victim->GetTotalNegativeAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_SPELL_HIT_CHANCE, schoolMask);
    ...
    if (spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK && spellInfo->SpellIconID == 3178) // chaosblolt ignore deflect chances
        return SPELL_MISS_NONE;
    ...
    if (!victim->HasUnitState(UNIT_STATE_STUNNED) && !isAttackedFromBehind(*victim, *this))
    {
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spellInfo, bool CanReflect)
{
    ...
    if (victim->GetTypeId() == TYPEID_UNIT && (victim->ToCreature()->IsInEvadeMode() || victim->ToCreature()->IsEvadingAttacks()))
    ...
    if (CanReflect)
    {
        ...
        if (reflectchance > 0 && roll_chance_i(reflectchance))
        {
            ...
            ProcDamageAndSpell(victim, PROC_FLAG_NONE, PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, 1, 0, BASE_ATTACK, spellInfo);
    ...
    {
        case SPELL_DAMAGE_CLASS_RANGED:
            return RangedSpellHitResult(*victim, *spellInfo);
        case SPELL_DAMAGE_CLASS_MELEE:
...
uint32 Unit::GetShieldBlockValue(uint32 soft_cap, uint32 dmg_cap, uint32 hard_cap) const
{
    ...
    {
        float bv_range = hard_cap - soft_cap;
        float dmg_range = dmg_cap - soft_cap;

        float class_const = pow(bv_range / dmg_range, 2.f) * dmg_range;

        value = dmg_cap - pow(static_cast<float>(value) - static_cast<float>(hard_cap), 2.f) / class_const;
    }
...
uint32 Unit::GetDefenseSkillValue(const Unit* target, bool levelCapped) const
{
    uint32 value = 0;
    if (const Player* player = To<Player>())
    {
        // in PvP use full skill instead current skill value
        value  = (target && target->IsOfType<Player>())
            ? player->GetMaxSkillValue(SKILL_DEFENSE)
            : player->GetSkillValue(SKILL_DEFENSE);

        if (!levelCapped)
            value += uint32(player->GetRatingBonusValue(CR_DEFENSE_SKILL));
    }
    else
...
/**
* @name GetUnitCriticalChance (for meele/ranged attacks)
* @brief returns the chance that an attack of this unit lands 
* critcally on the victim
* @param attackType the attacktype (base,off,ranged) of this attack
* @param victim the unit who gets hit by the attack
* @return the critchance of the attack against in percent or zero
* if the attack can't crit or would have a negative critchance
* or if the victim is null
*/
float Unit::GetUnitCriticalChance(WeaponAttackType attackType, const Unit* victim) const
{
    ...
    float crit = UnitCriticalChanceDone(attackType) + victim->UnitCriticalChanceBonusTaken(attackType, this);
...
/**
* @name UnitCriticalChanceBonusTaken (for meele/ranged attacks)
* @brief returns the critchance bonus (or malus) attacks against this unit will have
* based on defensive skills, resilience and buffs/debuffs
* To get the critchance against a specific attack, you should add
* the UnitCriticalChanceDone of the attacker to this, or directly use
* GetUnitCriticalChance for the attacker instead of this two methods
* @param attackType the attacktype (base,off,ranged) of this attack
* @param attacker the unit who wants to critcally hit this target
* @return the critchance bonus of the attack against in percent
* can be negative if this unit has many incoming crit reducing buffs and skills
*/
float Unit::UnitCriticalChanceBonusTaken(WeaponAttackType attackType, const Unit* attacker) const
{
    float crit = 0.0f;
    // flat aura mods
    if (attackType == RANGED_ATTACK)
        crit += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_CHANCE);
    else
        crit += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_MELEE_CRIT_CHANCE);

    crit += GetMaxPositiveAuraModifier(SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE);
    crit += GetTotalNegativeAuraModifier(SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE);

    AuraEffectList const& critChanceForCaster = GetAuraEffectsByType(SPELL_AURA_MOD_CRIT_CHANCE_FOR_CASTER);
    ...
    if (attackType != RANGED_ATTACK)
    {
        ApplyResilience(this, &crit, NULL, false, CR_CRIT_TAKEN_MELEE);
        // Glyph of barkskin
        if (HasAura(63057) && HasAura(22812))
            crit -= 25.0f;
    }
    else
        ApplyResilience(this, &crit, NULL, false, CR_CRIT_TAKEN_RANGED);

    return crit;
}

/**
* @name UnitCriticalChanceDone (for meele/rangeds attacks as units can only crit with those)
* @brief returns the critchance this unit has with the given attacktype based
* on his critchance and buffs
* To get the critchance against a specific target, you should add 
* the UnitCriticalChanceBonusTaken of the target, or directly use
* GetUnitCriticalChance instead of this two methods
* @param attackType the attacktype (base,off,ranged) of this attack
* @return the critchance of the attack in percent
* can be negative if this unit has many negative crit buffs
*/
float Unit::UnitCriticalChanceDone(WeaponAttackType attackType) const
{
    float crit = 0.0f;
    if (IsOfType<Player>())
    {
...
void Unit::_UpdateSpells(uint32 time)
{
    ...
    if (lastChannelTarget && !m_currentSpells[CURRENT_CHANNELED_SPELL])
        lastChannelTarget = ObjectGuid::Empty;
}
...
void Unit::_UpdateAutoRepeatSpell()
{
    ...
    // delay Autoshot while channeling volley
    if(m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id == 75 && m_currentSpells[CURRENT_CHANNELED_SPELL])
        setAttackTimer(RANGED_ATTACK, 500);
...
{
    bool ground = false; bool water = false; bool air = false;
    ...
    else
    {
        ...
        if ((HasUnitMovementFlag(MOVEMENTFLAG_FALLING) || HasUnitMovementFlag(MOVEMENTFLAG_SWIMMING)) && GetBaseMap()->IsInWater(GetPositionX(), GetPositionY(), GetPositionZ() - 2.f))
            water = true;

        if (!HasUnitMovementFlag(MOVEMENTFLAG_FLYING) || (c->GetMeleeRange(this) >=
            GetPositionZ() - GetMap()->GetHeight(GetPhaseMask(), GetPositionX(), GetPositionY(), GetPositionZ() + 2.0f, true)))
            ground = true;
    }

    return (ground && c->CanWalk()) || (water && c->CanSwim()) || (air && (c->CanFly() || c->IsDungeonBoss()));
}
...
/**
* @brief tries to either stack or refresh an existing aura of the same type (same SpellInfo) on this unit. 
* @param newAura the SpellInfo for the aura that should be stacked or refreshed (should not be null)
...
* @param softRefresh if true the found aura will only be softrefreshed.
* @return the aura that was refreshed or stacked. Returns null if there is no aura with the given SpellInfo on the unit
* or if it couldn't be stacked or refreshed (e.g if the Aura is a multi slot aura).
*/
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, ObjectGuid casterGUID /*= 0*/, bool softRefresh /*=false*/)
{
    ...
    if (!newAura->IsMultiSlotAura())
    {
        ...
        if (Aura* foundAura = GetOwnedAura(newAura->Id, casterGUID, newAura->HasAttribute(SPELL_ATTR0_CU_ENCHANT_PROC) ? castItemGUID : ObjectGuid::Empty, 0))
        {
            ...
            foundAura->ModStackAmount(1, AURA_REMOVE_BY_DEFAULT, softRefresh);
            return foundAura;
        }
    }
...
void Unit::_AddAura(UnitAura* aura, Unit* caster)
{
    ...
    if (aura->IsSingleTarget())
    {
        ...
        Unit::SingleTargetAuraVector& scAuras = caster->GetSingleCastAuras();
        for (Unit::SingleTargetAuraVector::iterator itr = scAuras.begin(); itr != scAuras.end();)
        {
            Unit* target = ObjectAccessor::GetUnit(*this, itr->first);
            if (!target)
            {
                itr = scAuras.erase(itr);
                continue;
            }
            Aura* itrAura = target->GetAura(itr->second, caster->GetGUID());
            if (!itrAura)
            {
                itr = scAuras.erase(itr);
                continue;
            ...
            if (itrAura != aura &&
                itrAura->IsSingleTargetWith(aura))
            {
                itrAura->Remove();
                itr = scAuras.begin();
            }
        ...
        caster->GetSingleCastAuras().push_back(std::make_pair(GetGUID(), aura->GetId()));
    }
}
...
void Unit::_ApplyAura(AuraApplication * aurApp, uint8 effMask)
{
    ...
    if (AuraStateType aState = aura->GetSpellInfo()->GetAuraState())
        DoDelayed(400ms, [aState, spellInfo = aurApp->GetBase()->GetSpellInfo()](Unit& owner)
        {
            if (owner.HasAura(spellInfo->Id))
                owner.ModifyAuraState(aState, true);
        });
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    if (aura->GetId() == 5229 && !HasAura(70726) && GetTypeId() == TYPEID_PLAYER) // Enrage Reduction Armor with Check T1
    {
        if (GetShapeshiftForm() == FORM_BEAR)
            HandleStatModifier(UNIT_MOD_ARMOR, BASE_PCT, -27.0f, false);
        else if (GetShapeshiftForm() == FORM_DIREBEAR)
            HandleStatModifier(UNIT_MOD_ARMOR, BASE_PCT, -16.0f, false);
    }
...
void Unit::RemoveNotOwnSingleTargetAuras(uint32 newPhase)
{
    ...
    SingleTargetAuraVector& scAuras = GetSingleCastAuras();
    for (SingleTargetAuraVector::iterator iter = scAuras.begin(); iter != scAuras.end();)
    {
        Unit* target = ObjectAccessor::GetUnit(*this, iter->first);
        if (!target)
        {
            iter = scAuras.erase(iter);
            continue;
        }
        Aura* aura = target->GetAura(iter->second, GetGUID());
        if (!aura)
        {
            iter = scAuras.erase(iter);
            continue;
        }
        if (aura->GetUnitOwner() != this && (newPhase == 0 || !aura->GetUnitOwner()->InSamePhase(newPhase)))
        {
            aura->Remove();
            iter = scAuras.begin();
        }
...
void Unit::RemoveAllGroupBuffsFromOthers()
{
    for (AuraMap::iterator iter = m_ownedAuras.begin(); iter != m_ownedAuras.end();)
    {
        Aura* aura = iter->second;
        if (aura->GetCasterGUID() != GetGUID() && aura->GetSpellInfo()->IsGroupBuff())
        {
            RemoveOwnedAura(iter);
            continue;
        }
...
bool Unit::HasNegativeAuraWithAttributeEx(uint32 flag, ObjectGuid guid) const
{
    for (AuraApplicationMap::const_iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end(); ++iter)
    {
        Aura const* aura = iter->second->GetBase();
...
int32 Unit::GetTotalNegativeAuraModifier(AuraType auratype) const
{
    int32 modifier = 0;
...
int32 Unit::GetTotalNegativeAuraModifierByMiscMask(AuraType auratype, uint32 miscMask) const
{
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue()& miscMask && (*i)->GetAmount() < 0)
            modifier += (*i)->GetAmount();
    }
...
int32 Unit::GetMaxPositiveAuraModifierByMiscValue(AuraType auratype, int32 misc_value, bool debuffResist) const
{
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == misc_value && (*i)->GetAmount() > modifier && (debuffResist || (!debuffResist && (*i)->GetId() != 53659)))
            modifier = (*i)->GetAmount();
    }
...
float Unit::GetTotalNegativeAuraMultiplierByMiscMask(AuraType auratype, uint32 misc_mask) const
{
    float multiplier = 1.0f;
...
void Unit::OnSummon(WorldObject& summoned)
{
    ApplyEncounterTo(summoned);
    summoned <<= Fire::SummonedBy.Bind(this);
}
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    Item* castItem = triggeredByAura->GetBase()->GetCastItem();
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 71519: // Deathbringer's Will Normal
                {
                    ...
                    if (GetSpellHistory()->HasCooldown(71519))
                        return false;
                ...
                case 71562: // Deathbringer's Will Heroic
                {
                    ...
                    if (GetSpellHistory()->HasCooldown(71562))
                        return false;
                ...
                case 62337: // Freya - Elder Stonebark - Petrified Bark (10man)
                case 62933: // Freya - Elder Stonebark - Petrified Bark (25man)
                {
                    basepoints0 = damage;
                    victim->CastCustomSpell(62379, SPELLVALUE_BASE_POINT0, basepoints0);
                    return true;
                }
                case 63305: // Yogg Saron - Influence Tantacle - Grim Reprisal
                {
                    basepoints0 = damage * 0.6;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 70770:
                    ...
                    if (procSpell->SpellFamilyFlags[0] & 0x800)
                    {
                        ...
                        if (!blessHealing)
                            return false;
                        int32 amount = int32(CalculatePct(damage, triggerAmount) / (blessHealing->GetMaxDuration() / blessHealing->Effects[0].Amplitude));
                        basepoints0 += amount + target->GetRemainingPeriodicAmount(GetGUID(), triggered_spell_id, SPELL_AURA_PERIODIC_HEAL);
                    }
                    break;
            }
            break;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                // Seal of Corruption, fallthrough to Seal of Vengeance 
                case 53736:
                {
                ...
                case 31801:
                {
                    ...
                    bool stacker = !procSpell || procSpell->Id == 53595 || (procSpell->SpellIconID == 243 && procSpell->SpellVisual[0] == 39);
                    ...
                    if (!stacker && !damager)
                        return false;
                    if (!triggered_spell_id) //set trigger spell only if corruption case didn't do that
                        triggered_spell_id = 31803;

                    // On target with 5 stacks of BloodCorruption // Holy Vengeance  direct damage is done
                    if (Aura* aur = victim->GetAura(triggered_spell_id, GetGUID()))
                    {
                        uint8 stackAmount = aur->GetStackAmount();
                        if (stackAmount >= 1)
                        {
                            // select damage spell: 42463 for seal of vengeance, 53739 else for seal of corruption
                            uint32 sealProccSpellId = (triggered_spell_id == 31803 ? 42463 : 53739);
                            // seal procc does x% of weapon damage, depending on the stackamount
                            const int32 percentWeaponDamage = (int32)(6.6f * stackAmount);
                            CastCustomSpell(victim, sealProccSpellId, &percentWeaponDamage, NULL, NULL, true, castItem, triggeredByAura, originalCaster);
                ...
                case 64890:
                {
                    ...
                    basepoints0 = CalculatePct(int32(damage), triggerAmount) / 3 /* Ticks */;
                    break;
                }
                // Glyph of Holy Light
                case 54937:
                ...
                case 71545: // Tiny Abomination in a Jar (Heroic)
                {
                    if (!victim || !victim->IsAlive())
                        return false;
                                                              // Righteous Vengeance    // Holy Vengeance         // Blood Corruption
                    if (procFlag & PROC_FLAG_DONE_PERIODIC && procSpell->Id != 61840 && procSpell->Id != 31803 && procSpell->Id != 53742)
                        return false;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    if (player->HasAura(60764)) // Totem of Splintering
                        basepoints0 = int32((extra_attack_power + 212) / 14.0f * GetAttackTime(attType) / 1000);
                ...
                case 70811:
                {
                    ...
                    if ((procSpell->SpellFamilyFlags[0] & 0x3) && !(procSpell->HasAttribute(SPELL_ATTR4_CAN_CAST_WHILE_CASTING)))
                    {
                        if (GetSpellHistory()->HasCooldown(16166) && !GetSpellHistory()->HasCooldown(70811))
                        {
                            uint32 newCooldownDelay = GetSpellHistory()->GetRemainingCooldown(16166);
                            if (newCooldownDelay < 3 * IN_MILLISECONDS)
                                newCooldownDelay = 0;
                            else
                                newCooldownDelay -= 2 * IN_MILLISECONDS;
                            GetSpellHistory()->AddCooldown(16166, 0, std::chrono::milliseconds(newCooldownDelay));
                            //hack: prevent that cooldown is reduced multiple times due to chain lightning
                            GetSpellHistory()->AddCooldown(70811, 0, std::chrono::seconds(1));

                            WorldPacket data(SMSG_MODIFY_COOLDOWN, 4+8+4);
                            data << uint32(16166);                  // Spell ID
                            data << uint64(GetGUID());              // Player GUID
                            data << int32(-2000);                   // Cooldown mod in milliseconds
                            ToPlayer()->SendDirectMessage(std::move(data));
                            return true;
                        }
                    }
                    return false;
                ...
                case 70817:
                {
                    ...
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_SHAMAN, 0x10000000, 0x0, 0x0, GetGUID()))
                    {
                        ...
                        int32 newDuration = flameShock->GetDuration() + 6 * IN_MILLISECONDS;
            ...
            if (dummySpell->SpellIconID == 2018)            // only this spell has SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
                ...
                switch (procSpell->Id)
                {
                    ...
                    // Lightning Bolt - Procc on Procc
                    case 45284: spellId = 45284; break;     // Rank  1
                    case 45286: spellId = 45286; break;     // Rank  2
                    case 45287: spellId = 45287; break;     // Rank  3
                    case 45288: spellId = 45288; break;     // Rank  4
                    case 45289: spellId = 45289; break;     // Rank  5
                    case 45290: spellId = 45290; break;     // Rank  6
                    case 45291: spellId = 45291; break;     // Rank  7
                    case 45292: spellId = 45292; break;     // Rank  8
                    case 45293: spellId = 45293; break;     // Rank  9
                    case 45294: spellId = 45294; break;     // Rank 10
                    case 45295: spellId = 45295; break;     // Rank 11
                    case 45296: spellId = 45296; break;     // Rank 12
                    case 49239: spellId = 49239; break;     // Rank 13
                    case 49240: spellId = 49240; break;     // Rank 14
                    // Chain Lightning
                    ...
                    // Chain Lightning - Procc on Procc
                    case 45297: spellId = 45297; break;     // Rank  1
                    case 45298: spellId = 45298; break;     // Rank  2
                    case 45299: spellId = 45299; break;     // Rank  3
                    case 45300: spellId = 45300; break;     // Rank  4
                    case 45301: spellId = 45301; break;     // Rank  5
                    case 45302: spellId = 45302; break;     // Rank  6
                    case 49268: spellId = 49268; break;     // Rank  7
                    case 49269: spellId = 49269; break;     // Rank  8
                    default:
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            {
                // only consume proc on use of rune strike
            ...
            if (dummySpell->Id == 49028)
            {
                ...
                if (pPet && procSpell)
                {
                    ...
                    if (pPet->GetVictim() != victim) 
                        pPet->Attack(victim, false);
                        
                    if (damage && !procSpell->IsPositive() && pPet->GetVictim() && procSpell->Id != 48982 /*Rune Tap*/)
                    {
                    ...
                    {
                        if (!procSpell->IsPositive() && pPet->GetVictim())
        ...
        case SPELLFAMILY_GENERIC:
            switch (dummySpell->Id)
            {
                ...
                case 72784:
                    if (procSpell)
                    {
                        // Spell: Flames (triggered on Ball of Flame explosion, but shouldn't lower stack count)
                        if (   procSpell->Id == 71393 
                            || procSpell->Id == 72789 
                            || procSpell->Id == 72790 
                            || procSpell->Id == 72791)
                            break;
                ...
                *handled = true;
                ...
                if (Player* player = ToPlayer())
                {
                    ...
                    for (uint8 i = 0; i < MAX_RUNES && runesLeft; ++i)
                    {
                        ...
                        --runesLeft;
                        // Mark aura as used
                        player->ConvertRuneByAuraEffect(i, RUNE_DEATH, aurEff);
                    }
                    return true;
                }
                return false;
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    Unit*  target = NULL;
    int32  basepoints0 = 0;
    bool checkCD = true; // Ignoring auraSpellInfo->Id CD if necessary
            ...
            case SPELLFAMILY_PALADIN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 67758: // Item - Coliseum 25 Heroic Caster Trinket
                    {
                        ...
                        // cooldown
                        if (GetSpellHistory()->HasCooldown(auraSpellInfo->Id))
                            return false;
                        GetSpellHistory()->AddCooldown(auraSpellInfo->Id, 0, std::chrono::seconds(cooldown));
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 12867:
        {
            ...
            WeaponAttackType attackType = procFlags & PROC_FLAG_DONE_OFFHAND_ATTACK ? OFF_ATTACK : BASE_ATTACK;
            uint32 averageMeeleDamage = CalculateDamage(attackType, false, true, true);
            basepoints0 = MeleeDamageBonusDone(victim, averageMeeleDamage, attackType);
            break;
        }
    ...
    switch (trigger_spell_id)
    {
        ...
        case 52916:
        {
            ...
            if (cooldown && target->GetSpellHistory()->HasCooldown(trigger_spell_id))
                return false;
            ...
            if (cooldown && target->GetTypeId() == TYPEID_PLAYER)
                target->GetSpellHistory()->AddCooldown(trigger_spell_id, 0, std::chrono::seconds(cooldown));
            return true;
        }
        ...
        case 71169:
        {
            ...
            if (GetTypeId() != TYPEID_PLAYER || !victim || victim->GetTypeId() != TYPEID_UNIT || victim->GetCreatureType() == CREATURE_TYPE_CRITTER || victim->GetEntry() == 37782) 
                return false;
    ...
    if (cooldown && (GetSpellHistory()->HasCooldown(trigger_spell_id) || (GetSpellHistory()->HasCooldown(auraSpellInfo->Id) && checkCD)))
        return false;
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        if (victim->ToPlayer()->IsGameMaster() || victim->ToPlayer()->isDeveloper())
            return false;
    }
    else
    {
...
void Unit::SetOwnerGUID(ObjectGuid owner)
{
    ...
    WorldPacket packet = CreateValuesUpdateBlockForPlayer(player);
...
void Unit::SetMinion(Minion *minion, bool apply)
{
    ...
    if (apply)
    {
        ...
        if (GetTransport())
        {
            float x, y, z, o;
            GetPosition(x, y, z, o);
            GetTransport()->CalculatePassengerOffset(x, y, z, &o);
            GetTransport()->AddPassenger(minion);
            minion->m_movementInfo.transport.pos.Relocate(x, y, z, o);
            minion->NearTeleportTo(GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
        }
    }
    else
    {
...
/**
* @name SpellDamageBonusDone
* @brief returns the damage this unit does with the given spell against the given target
* takes into account abilities from the caster increasing the damage (Done)
* For player also SpellMods are taken into acoount
* Effects on the target must be added for the complete damage (like debuffs) (taken)
* (taken is implemented in Unit::SpellDamageBonusTaken)
* @return the damage done with the spell or 0 if it would be negative
*/
...
{
    int32 damage;
    SpellDirectDamageBonus(victim, spellProto, pdamage, damagetype, stack, damage);
    // apply spellmod to Done damage (flat and pct)
    if (Player* modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(spellProto->Id, damagetype == DOT ? SPELLMOD_DOT : SPELLMOD_DAMAGE, damage);

    return std::max(damage,0);
}

/**
* @name SpellDirectDamageBonus
* @brief returns the damage this unit does with the given spell against the given target
* takes into account abilities from the caster increasing the damage (Done)
* but not effects on the target (like debuffs) (taken) 
* (taken is implemented in Unit::SpellDamageBonusTaken)
* Also does not take into account player SpellMods. (Use ApplySpellMods())
* @param victim the Unit getting hit by the spell
* @param spellProto the spellInfo of the spell (not null)
* @param damageType the DamageEffectType of the spell
* @param pdamage the base damage amount which the spell should have
* @param stack the number of stacks of the spell on the target (mostly for dots)
* @param damageWithBonus the damage the caster does with the spell gets assigned to this parameter, might get negative in rare cases
* @return the intermediate results constisting of the flat damage (dependent on spellpower/ap, the pdamage etc)
* and the percentage Modifiers (from talents, auras etc) bundled together in (flatdamage is first, 
* the percentage modifier is second) The following is valid after the method: flatDamage * percentage Modifiers
* == damageWithBonus. These two can be used to later recalculate this result or to store one or another 
* for reusing later (used by e.g. PeriodicEffectSnapShot). Percantage modifier starts at 1, flatdamage at pdamage.
*/
Unit::FlatAndPercentModifiers Unit::SpellDirectDamageBonus(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, 
                                                           uint32 stack, int32& damageWithBonus) const
{
    damageWithBonus = pdamage;

    if (!spellProto || !victim || damagetype == DIRECT_DAMAGE)
        return FlatAndPercentModifiers(pdamage, 1.0f);

    // Some spells don't benefit from done mods
    if (!spellProto->IsAffectedBySpellMods())
        return FlatAndPercentModifiers(pdamage, 1.0f);;
    ...
    if (GetTypeId() == TYPEID_UNIT && IsTotem())
        if (Unit* owner = GetOwner())
        {
            return owner->SpellDirectDamageBonus(victim, spellProto, pdamage, damagetype, stack, damageWithBonus);
        }
    ...
    if (!(spellProto->AttributesEx6 & SPELL_ATTR6_NO_DONE_PCT_DAMAGE_MODS))
    {
        ...
        for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
        {
            ...
            // Raid wide auras should not be able to stack
            if ((*i)->GetSpellInfo()->Effects[(*i)->GetEffIndex()].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID)
            {
                if ((*i)->GetAmount() > areaMaxAmount)
                    areaMaxAmount = (*i)->GetAmount();
        ...
        if (areaMaxAmount)
            AddPct(DoneTotalMod, areaMaxAmount);
    }
    ...
    int32 DoneAdvertisedBenefit  = SpellBaseDamageBonusDone(spellProto->GetSchoolMask()) + SpellPowerBonus;
    //int32 TakenAdvertisedBenefit = victim->SpellBaseDamageBonusTaken(spellProto->GetSchoolMask()); //not used
    ...
    switch (spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x200000)
                if (AuraEffect* aurEff = GetAuraEffect(64950, EFFECT_0, GetGUID()))
    ...
    if (DoneAdvertisedBenefit)
    {
        ...
        if (Player* modOwner = GetSpellModOwner())
        {
            //! don't add any SPELLMOD_BONUS_MULTIPLIER for mage fireball dot
            bool canNotScale = false;
            canNotScale = spellProto->SpellFamilyName == SPELLFAMILY_MAGE && spellProto->SpellFamilyFlags[2] & 0x8 && damagetype == DOT;
            if (!canNotScale)
            {
                coeff *= 100.0f;
    ...
    int32 flatDamage = (int32(pdamage) + DoneTotal);
    float percentageModifiers = DoneTotalMod;
    damageWithBonus = (int32(pdamage) + DoneTotal) * DoneTotalMod;
    return FlatAndPercentModifiers(flatDamage, percentageModifiers);
}
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    {
        // store the highest modifier for disease damage, apply only the strongest one
        int32 maxPosDiseaseDamageMod = 0;
        ...
        for (AuraEffectList::const_iterator i = mDamageDoneMechanic.begin(); i != mDamageDoneMechanic.end(); ++i)
        {
            ...
            // apply only modifiers by auras which effect this spell
            if (mechanicMask & uint32(1 << ((*i)->GetMiscValue())))
            {
                // handle all modifiers but DISEASE modifiers with POSITIVE value
                if (((*i)->GetMiscValue() != MECHANIC_INFECTED) || ((*i)->GetAmount() < 0))
                    AddPct(TakenTotalMod, (*i)->GetAmount());

                // when we find a DISEASE MODIFIER with a higher value, store it
                if (((*i)->GetMiscValue() == MECHANIC_INFECTED) && ((*i)->GetAmount() > maxPosDiseaseDamageMod))
                    maxPosDiseaseDamageMod = (*i)->GetAmount();
            }
        }

        if (maxPosDiseaseDamageMod) // apply highest disease mod (only nonzero if this is a disease and there is a modifier)
            AddPct(TakenTotalMod, maxPosDiseaseDamageMod);
    }
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && (schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0)
    {
        // Base value
...
{
    float crit_chance = SpellCritChance(victim, spellProto, schoolMask, attackType);
...
* @brief returns the critchance for the given attack on the given victim.
* Takes into account effects of the caster that increases critchance (Done)
* aswell as effects on the target (like debuffs) (taken)
* @param victim the Unit getting hit by the attack
* @param spellProto the spellInfo of the spell (not null)
* @param schoolMask the SpellSchoolMask of the spell
* @return the critchance in percent from 0 to 100
*/
float Unit::SpellCritChance(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    float crit_chance = SpellCritChanceDone(victim, spellProto, schoolMask, attackType) + victim->SpellCritChanceBonusTaken(victim, spellProto, schoolMask, attackType);
    return std::max(crit_chance, 0.0f);
}

/**
* @name SpellCritChanceDone
* @brief returns the critchance coming from the casters stats/skills for the given attack. 
* Takes into account effects of the caster that increases critchance (Done)
* but not effects on the target (like debuffs) (taken) (see below at todo)
* @param victim the Unit getting hit by the attack 
* @param spellProto the spellInfo of the spell (not null)
* @param schoolMask the SpellSchoolMask of the spell 
* @return the critchance in percent, can be negative if this unit 
* has critchance debuffs
* 
* @todo: the seperation of the crit bonus done from taken might currently not be perfect 
* The seperation is needed because some effects calculate the crit chance bonus from the caster
* before taking into account any debuff on the target 
* for example PeriodicEffectSnapShot needs the seperation, as the
* critchance for the caster is directly taken when applying the dot but the effect 
* of the debuffs should be reviewed every time the dot ticks (deals damage))
*/
float Unit::SpellCritChanceDone(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    //! Mobs can't crit with spells. Player Totems can
    //! Fire Elemental (from totem) can too - but this part is a hack and needs more research
    //! Ebon Gargoyle - should crit with gargoyle strike
    //! Val'kyr Guardian - should crit with Smite
    if (GetGUID().IsCreature() && !(IsTotem() && GetOwnerGUID().IsPlayer())
        && (GetEntry() != 15438 && GetEntry() != 27829 && GetEntry() != 38391 && GetEntry() != 38392))
        return CRIT_CHANCE_NEVER;

    if (!IsControlledByPlayer() && GetGUID().IsVehicle())
        return CRIT_CHANCE_NEVER;
    ...
    case SPELL_DAMAGE_CLASS_NONE:
    {
        ...
        {
            SpellSchools firstSchoolInMask = GetFirstSchoolInMask(schoolMask);
            // None damage "physical" heal, like e.g. potions, can crit but PLAYER_SPELL_CRIT_PERCENTAGE1 for 
            // school Normal is always 0 so we take the base-attack player crit chance instead
            if (firstSchoolInMask == SPELL_SCHOOL_NORMAL)
                crit_chance = GetFloatValue(PLAYER_CRIT_PERCENTAGE);
            else
                crit_chance = GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1 + firstSchoolInMask);
            break;
        }
        return CRIT_CHANCE_NEVER;
    }
    ...
    case SPELL_DAMAGE_CLASS_MAGIC:
    {
        ...
        {
            //physical magic heal like healthstones can crit
            if (spellProto->Effects[0].Effect == SPELL_EFFECT_HEAL)
                crit_chance = GetFloatValue(PLAYER_CRIT_PERCENTAGE);
            else
                crit_chance = CRIT_CHANCE_NEVER;
        }
        // For other schools
        ...
        if (victim)
        {
            ...
            switch (spellProto->SpellFamilyName)
            {
                // Improved Flash Heal
            case SPELLFAMILY_PRIEST:
                if (spellProto->SpellFamilyFlags[0] & 0x00000800)
                    if (victim->GetHealthPct() <= 50)
                        if (AuraEffect* eff = GetDummyAuraEffect(SPELLFAMILY_PRIEST, 2542, EFFECT_0))
    ...
    case SPELL_DAMAGE_CLASS_RANGED:
    {
        ...
        if (spellProto->SpellFamilyName == SPELLFAMILY_HUNTER && spellProto->SpellIconID == 1629)
            if (AuraEffect const* aurEff = GetAuraEffect(56842, EFFECT_0))
                crit_chance += aurEff->GetAmount();

        crit_chance += UnitCriticalChanceDone(attackType);
        crit_chance += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_SPELL_CRIT_CHANCE_SCHOOL, schoolMask);

        // Autoshot must crit if target is sitting down and crit chance not 0
        if ((spellProto->AttributesEx4 & SPELL_ATTR4_UNK24) && victim->GetTypeId() == TYPEID_PLAYER && crit_chance > 0 && !victim->IsStandState())
            return CRIT_CHANCE_ALWAYS;
...
/**
* @name SpellCritChanceBonusTaken
* @brief returns the critchance bonus/malus through this units skills/gear and buffs/debuffs for the spell
* @param attacker the Unit the attack comes from
* @param spellProto the spellInfo of the spell (not null)
* @param schoolMask the SpellSchoolMask of the spell
* @return the critchance in percent, can be negative if this unit
* has a lot of defensive skills and buffs
*/
float Unit::SpellCritChanceBonusTaken(Unit* attacker, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    float crit_chance = 0.0f;
    switch (spellProto->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_NONE: //fallthrough intentional
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            if (!spellProto->IsPositive())
            {
                // Modify critical chance by SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE
                crit_chance += GetMaxPositiveAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE, schoolMask);
                crit_chance += GetTotalNegativeAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE, schoolMask);
                // Modify critical chance by SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE
                crit_chance += GetMaxPositiveAuraModifier(SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE);
                crit_chance += GetTotalNegativeAuraModifier(SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE);
                ApplyResilience(this, &crit_chance, NULL, false, CR_CRIT_TAKEN_SPELL);
            }
            break;
        }
        case SPELL_DAMAGE_CLASS_MELEE: //fallthrough intentional
        case SPELL_DAMAGE_CLASS_RANGED:
        {
            crit_chance += UnitCriticalChanceBonusTaken(attackType, attacker);
            break;
        }
        default:
            break;
...
uint32 Unit::SpellCriticalDamageBonus(SpellInfo const* spellProto, uint32 damage, Unit* victim)
{
    ...
    if (Player* modOwner = GetSpellModOwner())
    {
        ...
        // special case for runic focus because it is the only critdamagebonus that should stack multiplicative
        if ((damageClass == SPELL_DAMAGE_CLASS_MAGIC || damageClass == SPELL_DAMAGE_CLASS_NONE) && modOwner->HasAura(61455))
            crit_bonus *= 2;
    }
...
/**
* @name SpellHealingBonusDone
* @brief returns the healing this unit does with the given spell on the given target
* takes into account abilities from the caster increasing the heal (Done)
* For player also SpellMods are taken into acoount
* Effects on the target must be added for the complete damage (like debuffs) (taken)
* (taken is implemented in Unit::SpellHealingBonusTaken)
* @return the healing done with the spell or 0 if it would be negative
*/
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack) const
{
    ...
    SpellDirectHealingBonus(victim, spellProto, healamount, damagetype, stack, heal);
    // apply spellmod to Done amount
    if (Player* modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(spellProto->Id, damagetype == DOT ? SPELLMOD_DOT : SPELLMOD_DAMAGE, heal);

    return uint32(std::max(heal, 0));
}

/**
* @name SpellDirectHealingBonus
* @brief returns the healing this unit does with the given spell on the given target
* takes into account abilities from the caster increasing the heal (Done)
* but not effects on the target (like additional buffs) (taken)
* (taken is implemented in Unit::SpellHealingBonusTaken)
* Also does not take into account player SpellMods. (Use ApplySpellMods())
* @param victim the Unit getting hit by the spell
* @param spellProto the spellInfo of the spell (not null)
* @param damageType the DamageEffectType of the spell
* @param healamount the base heal amount which the spell should have
* @param stack the number of stacks of the spell on the target (mostly for hots)
* @param healWithBonus the healing the caster does with the spell gets assigned to this parameter, might get negative in rare cases
* @return the intermediate results constisting of the flat heal (dependent on spellpower, the healamount etc)
* and the percentage Modifiers (from talents, auras etc) bundled together (flatHeal is first,
* the percentage modifier is second) The following is valid after the method: flatHeal * percentage Modifiers
* == healWithBonus. These two can be used to later recalculate this result or to store one or another
* for reusing later (used by e.g. PeriodicEffectSnapShot) Percantage modifier starts at 1, flatheal at healamount.
*
*/
Unit::FlatAndPercentModifiers Unit::SpellDirectHealingBonus(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, 
                                                            uint32 stack, int32& healWithBonus) const
{
    healWithBonus = healamount;
    // For totems get healing bonus from owner (statue isn't totem in fact)
    if (GetTypeId() == TYPEID_UNIT && IsTotem())
        if (Unit* owner = GetOwner())
            return owner->SpellDirectHealingBonus(victim, spellProto, healamount, damagetype, stack, healWithBonus);

    // No bonus healing for potion spells
    if (spellProto->SpellFamilyName == SPELLFAMILY_POTION)
        return FlatAndPercentModifiers(healamount,1.0f);
    ...
    // No bonus healing for these spells
    if (spellProto->AttributesEx6 & SPELL_ATTR6_NO_HEALING_BONUS)
        return FlatAndPercentModifiers(0, 1.0f);
    ...
    //int32 TakenAdvertisedBenefit = victim->SpellBaseHealingBonusTaken(spellProto->GetSchoolMask()); //not used
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_DRUID:
            ...
            if (spellProto->SpellFamilyFlags[1] & 0x2000000)
                if (AuraEffect* aurEff = GetAuraEffect(64949, EFFECT_0, GetGUID()))
                    DoneAdvertisedBenefit += aurEff->GetAmount();
            // Lifebloom (final heal)
            if (spellProto->Id == 33778)
            {
                // bonus from Empowered Rejuvenation
                if (AuraEffect* effect = GetAuraEffectOfRankedSpell(33886, EFFECT_0, GetGUID()))
                    AddPct(DoneAdvertisedBenefit, effect->GetAmount());
                // bonus from pvp idol
                if (AuraEffect* effect = GetAuraEffect(60738, EFFECT_0, GetGUID()))
                    DoneAdvertisedBenefit += effect->GetAmount();
                // bonus from pvp idol
                else if (AuraEffect* effect = GetAuraEffect(60739, EFFECT_0, GetGUID()))
                    DoneAdvertisedBenefit += effect->GetAmount();
                // bonus from pvp idol
                else if (AuraEffect* effect = GetAuraEffect(60740, EFFECT_0, GetGUID()))
                    DoneAdvertisedBenefit += effect->GetAmount();
                // bonus from pvp idol
                else if (AuraEffect* effect = GetAuraEffect(60741, EFFECT_0, GetGUID()))
                    DoneAdvertisedBenefit += effect->GetAmount();
            }
            break;
        case SPELLFAMILY_PALADIN:
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x80000000)
                if (AuraEffect* aurEff = GetAuraEffect(64956, EFFECT_0, GetGUID()))
                    DoneAdvertisedBenefit += aurEff->GetAmount();
            break;
        default:
            break;
    ...
    healWithBonus = (int32(healamount) + DoneTotal) * DoneTotalMod;
    return Unit::FlatAndPercentModifiers(int32(healamount) + DoneTotal, DoneTotalMod);
}
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack) const
{
    ...
    // Healing taken percent - positive mods
    int32 areaMaxAmount = 0;
    AuraEffectList const& mHealingPct = GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_PCT);
    for (AuraEffectList::const_iterator i = mHealingPct.begin(); i != mHealingPct.end(); ++i)
    {   
        if ((*i)->GetAmount() > 0)
        {
            // Raid wide auras should not be able to stack
    ...
    AddPct(TakenTotalMod, areaMaxAmount);
    ...
    // Spells with this attributes don't profit from modifications, but can be reduced through them
    if (spellProto->AttributesEx6 & SPELL_ATTR6_NO_HEALING_BONUS)
    {
        TakenTotal = TakenTotal > 0 ? 0 : TakenTotal;
        TakenTotalMod = TakenTotalMod > 1.0f ? 1.0f : TakenTotalMod;
    }
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
    ...
    if (spellInfo->Effects[index].Effect == SPELL_EFFECT_ENERGIZE || spellInfo->Effects[index].ApplyAuraName == SPELL_AURA_PERIODIC_ENERGIZE)
    {
        if (spellInfo->Effects[index].MiscValue == 0)  // Mana
        {
            if(spellInfo->Attributes & SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY)
                return false;
        }
        else
            if (HasAura(62692)) // Aura of Despair
                return false;
    }
    else if (spellInfo->Effects[index].Effect == SPELL_EFFECT_ATTACK_ME && GetTypeId() == TYPEID_UNIT && 
        (ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_TAUNT_DIMINISH))
    {
        auto itr = std::find_if(m_Diminishing.cbegin(), m_Diminishing.cend(), 
            [](const DiminishingReturn d){ return d.DRGroup == DIMINISHING_TAUNT; });
        if (itr != m_Diminishing.cend() && itr->hitCount >= DIMINISHING_LEVEL_TAUNT_IMMUNE
            && (itr->stack != 0 || getMSTimeDiff(itr->hitTime, getMSTime()) < 15000))
            return true;
    }

    // RG-Custom to prevent Buguse - Workaround
    // Following Spells should not be resistable while having an Immunity Effect
    // Trying Flag SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY in SpellMgr causes other bugs, so this workaround must be in Unit.cpp
    switch (spellInfo->Id)
    {
        // DK - Anti Magic Shell should not block following SpellIDs
        case 25771: // Forbearance Debuff from Paladins (Vorahnung)
        case 6788: // Weakened Soul Debuff from Discipline Priests
        case 70127: // Mystic Buffet
        case 72528: // Mystic Buffet
        case 72529: // Mystic Buffet
        case 72530: // Mystic Buffet
    ...
    if (uint32 aura = spellInfo->Effects[index].ApplyAuraName)
    {
        ...
        // Check for immune to application of harmful effects
        AuraEffectList const& immuneAuraApply = GetAuraEffectsByType(SPELL_AURA_MOD_IMMUNE_AURA_APPLY_SCHOOL);
        for (AuraEffectList::const_iterator iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
                if (((*iter)->GetMiscValue() & spellInfo->GetSchoolMask()) &&  // Check school                      // Check school
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    // Some spells don't benefit from pct done mods
    if (spellProto)
        if (!spellProto->HasAttribute(SPELL_ATTR6_NO_DONE_PCT_DAMAGE_MODS) && !spellProto->IsRankOf(sSpellMgr->GetSpellInfo(12162)))
        {
            int32 areaMaxAmount = 0;
            AuraEffectList const& mModDamagePercentDone = GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
            for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
    ...
    if (Player* player = ToPlayer())
    {
        ...
        // unsummon pet and guardian (mage only, water-elemental)
        if (getClass() == CLASS_MAGE && !HasAura(70937))
            if (Guardian* guard = player->GetGuardianPet())
                guard->UnSummon();
        ...
        // if we have charmed npc, stun him also
        ...
        data << GetPackGUID();
        data << uint32(game::time::GetGameTime());   // Packet counter
...
void Unit::Dismount()
{
    ...
    if (Player* thisPlayer = ToPlayer())
    {
        ...
        thisPlayer->SendDirectMessage(std::move(data));
    }
...
void Unit::SetInCombatWith(Unit* enemy)
{
 ...
 void Unit::EngageWithTarget(Unit* who) 
 {
     ...
     // creatures should not set each other infight upon engagement
     // otherwise, the target would be infight, but doesn't have any threat/victim, and would evade next tick
     // it is also blizlike that ie. player-controller minions only set creatures infight upon dealing damage
     if (!IsOfType<Creature>() || !who->IsOfType<Creature>())
...
void Unit::CombatStart(Unit* target, bool initialAggro)
{
    ...
    {
        if (!me->IsPvP() || !me->pvpInfo.EndTimer)
            me->UpdatePvP(true);
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (m_currentSpells[CURRENT_GENERIC_SPELL] && m_currentSpells[CURRENT_GENERIC_SPELL]->getState() != SPELL_STATE_FINISHED) // karn
        if (m_currentSpells[CURRENT_GENERIC_SPELL]->m_spellInfo->Attributes & SPELL_ATTR0_CANT_USED_IN_COMBAT) // karn
            InterruptSpell(CURRENT_GENERIC_SPELL); // karn

    *this <<= Fire::EnterCombat.Bind(enemy);
    ...
    if (Creature* creature = ToCreature())
    {
        ...
        if (enemy)
        {
            if (IsAIEnabled)
            {
                creature->OnEnterCombat(*enemy);
				creature->SetImmuneToPC(false); // unit has engaged in combat, remove immunity so players can fight back

                if (creature->IsDungeonBoss())
                    RG_LOG<EncounterLogModule>(this, EncounterLogModule::Type::PULL);
            }
            ...
            if ((creature->GetCreatureTemplate()->unit_flags & UNIT_FLAG_PVP))
            {
                if (enemy->GetTypeId() == TYPEID_UNIT)
                    if (Player* player = enemy->GetCharmerOrOwnerPlayerOrPlayerItself())
                    {
                ...
                {
                    enemy->ToPlayer()->SetContestedPvP();
...
{
    if (IsInWorld() && GetTypeId() == TYPEID_PLAYER && !ToPlayer()->IsGameMaster())
        if (InstanceMap* instance = GetInstance())
            if (instance->IsEncounterInProgress())
            {
                ToPlayer()->SetDelayedCombatStop();
                return;
            }
    ...
    if (Creature* creature = ToCreature())
    {
        // new script system resets itself
		if (!HasScript() && creature->GetCreatureTemplate() && creature->GetCreatureTemplate()->unit_flags & UNIT_FLAG_IMMUNE_TO_PC)
			creature->SetImmuneToPC(true); // set immunity state to the one from db on evade
...
bool Unit::isTargetableForAttack(bool checkFakeDeath) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && (ToPlayer()->IsGameMaster() || ToPlayer()->isDeveloper()))
        return false;
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if (target->HasUnitState(UNIT_STATE_UNATTACKABLE)
        || (target->GetTypeId() == TYPEID_PLAYER && (target->ToPlayer()->IsGameMaster() || target->ToPlayer()->isDeveloper())))
        return false;
    ...
    if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_TAXI_FLIGHT | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_UNK_16)
        ...
        || (GetEntry() != WORLD_TRIGGER && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && IsImmuneToPC()))
        return false;
    ...
    if (playerAffectingAttacker && playerAffectingTarget)
    {
        if (!IsPvP() && !IsFFAPvP() && bySpell && bySpell->IsTargetingArea())
            return false;
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    // We should not cancel assistance bla
    bool skipCheck = false;
    if (bySpell && bySpell->Id == 3286
        && GetReactionTo(target) >= REP_NEUTRAL
        && target->GetReactionTo(this) >= REP_NEUTRAL)
        skipCheck = true;
...
{
    return static_cast<uint32>(m_serverSideVisibility.GetValue(SERVERSIDE_VISIBILITY_GM)) <= SEC_PLAYER;
}
...
void Unit::UpdateSpeed(UnitMoveType mtype)
{
    ...
    switch (mtype)
    {
        ...
        case MOVE_FLIGHT:
        {
            ...
            if (GetTypeId() == TYPEID_UNIT)
                speed *= IsPet() ? 1.15f : ToCreature()->GetCreatureTemplate()->speed_run;    // at this point, MOVE_WALK is never reached
...
void Unit::SetSpeedRate(UnitMoveType mtype, float rate)
{
    ...
    Player* playerMover = m_movedPlayer;
    // Update player even if the owner isn't on control
    if (!playerMover && IsOfType<Player>())
        playerMover = this->ToPlayer();
    ...
    if (playerMover)  // unit controlled by a player.
    {
        ...
        WorldPacket self(moveTypeToOpcode[mtype][1], mtype != MOVE_RUN ? 8 + 4 + 4 : 8 + 4 + 1 + 4);
        self << GetPackGUID();
        ...
        if (IsOfType<Player>() && playerMover != this)
            ToPlayer()->SendDirectMessage(self);
        playerMover->SendDirectMessage(std::move(self));
        ...
        WorldPacket data = BuildMovementPacket(moveTypeToOpcode[mtype][2]);
    ...
    else // unit controlled by AI.
    {
        ...
        WorldPacket data(moveTypeToOpcode[mtype][0], 8 + 4);
        data << GetPackGUID();
        data << float(GetSpeed(mtype));
        SendMessageToSet(data, false);
    }
...
void Unit::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        ...
        if (GetTypeId() != TYPEID_UNIT) // Creature case will be handeled by deathDelayTimer
            SetHealth(0);
...
void Unit::TauntApply(Unit* taunter)
{
    ...
    if (!taunter || (taunter->GetTypeId() == TYPEID_PLAYER && (taunter->ToPlayer()->IsGameMaster() || taunter->ToPlayer()->isDeveloper())))
        return;
...
void Unit::TauntFadeOut(Unit* taunter)
{
    ...
    {
        if (creature->IsAIEnabled)
            creature->OnEvade(CreatureAI::EVADE_REASON_NO_HOSTILES);
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (!(spellProto->HasAttribute(SPELL_ATTR0_ABILITY) || spellProto->HasAttribute(SPELL_ATTR0_TRADESPELL) || spellProto->HasAttribute(SPELL_ATTR3_NO_DONE_BONUS)) &&
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group)
{
    for (Diminishing::iterator i = m_Diminishing.begin(); i != m_Diminishing.end(); ++i)
    {
        ...
        if (i->stack == 0 && getMSTimeDiff(i->hitTime, game::time::GetGameTimeMS()) > 15000)
        {
...
void Unit::IncrDiminishing(DiminishingGroup group)
{
    ...
    m_Diminishing.push_back(DiminishingReturn(group, game::time::GetGameTimeMS(), DIMINISHING_LEVEL_2));
}
...
float Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32 &duration, Unit* caster, DiminishingLevels Level, int32 limitduration)
{
    ...
    Unit const* casterOwner = caster->GetTypeId() == TYPEID_PLAYER && caster->IsCharmed() ? caster : caster->GetCharmerOrOwner();
...
void Unit::ApplyDiminishingAura(DiminishingGroup group, bool apply)
{
    ...
    for (Diminishing::iterator i = m_Diminishing.begin(); i != m_Diminishing.end(); ++i)
    {
        ...
        else if (i->stack)
        {
            ...
            if (i->stack == 0)
                i->hitTime = game::time::GetGameTimeMS();
        }
        break;
    }
}
...
########                         ########
#######################################*/
inline bool IsUnitModMultiplicative(UnitMods unitMod, const SpellInfo* spell)
{
    if (unitMod == UNIT_MOD_ARMOR) // Armor mods are always multiplicative
        return true;
    return unitMod == UNIT_MOD_DAMAGE_MAINHAND || unitMod == UNIT_MOD_DAMAGE_OFFHAND || unitMod <= UNIT_MOD_STAT_SPIRIT;
}

bool Unit::HandleStatModifier(UnitMods unitMod, UnitModifierType modifierType, float amount, bool apply, const SpellInfo* spell)
{
    ...
    switch (modifierType)
    {
        ...
        case BASE_PCT:
        case TOTAL_PCT:
            if (IsUnitModMultiplicative(unitMod, spell))
            {
                if (apply)
                    m_auraModifiersGroup[unitMod][modifierType] *= 1.0f + (amount / 100.0f);
                else
                {
                    if (m_auraModifiersGroup[unitMod][modifierType] == 0)
                        m_auraModifiersGroup[unitMod][modifierType] = (amount / 100.0f) * -1.0f;
                    else
                        m_auraModifiersGroup[unitMod][modifierType] /= 1.0f + (amount / 100.0f);
                }
            }
            else
                m_auraModifiersGroup[unitMod][modifierType] += (apply ? amount : -amount) / 100;
            break;
        default:
            break;
    }
...
/**
 * Checks Stats for Buffstacking rules in database
 * @brief Unit::GetStatValueToRemove
 * @author Pointerx
 * @param stat
 * @return value to remove
 */
int32 Unit::GetStatValueToRemove(Stats stat)
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    int32 modifier = 0;
    int32 totalMod = 0;

    AuraEffectList const& mTotalAuraList = GetAuraEffectsByType(SPELL_AURA_MOD_STAT);
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
       if ((*i)->GetMiscValue() == stat || (*i)->GetMiscValue() < 0)
       {
           totalMod += (*i)->GetAmount();
           if (!sSpellMgr->AddSameEffectStackRuleSpellGroups((*i)->GetSpellInfo(), (*i)->GetAmount(), SameEffectSpellGroup))
               modifier += (*i)->GetAmount();
       }
    ...
    return (totalMod - modifier);
}

/**
 * This method has its purposes to check for buffstacking rules in database with type 3
 * Please note that this method does NOT return the total value of the specified unitMod.
 * Moreover, it returns the total value which has to be deleted in order to fit buffstacking rules.
 * @brief Unit::GetUnitModValueToRemove
 * @author Pointerx
 * @param unitMod
 * @param modifierType
 * @return value to remove
 */
float Unit::GetUnitModValueToRemove(UnitMods unitMod, UnitModifierType modifierType) const
{
    AuraType auraType;
    switch(unitMod)
    {
        case UNIT_MOD_ATTACK_POWER:
            auraType = modifierType == TOTAL_VALUE ? SPELL_AURA_MOD_ATTACK_POWER : SPELL_AURA_NONE;
            break;
        case UNIT_MOD_ATTACK_POWER_RANGED:
            auraType = modifierType == TOTAL_VALUE ? SPELL_AURA_MOD_RANGED_ATTACK_POWER : SPELL_AURA_NONE;
            break;
        case UNIT_MOD_ARMOR:
            auraType = modifierType == TOTAL_PCT ? SPELL_AURA_MOD_RESISTANCE_PCT : SPELL_AURA_NONE;
            break;
        default:
...
float Unit::GetModifierValue(UnitMods unitMod, UnitModifierType modifierType) const
{
    ...
    int32 minusModifier = 0;
    if (modifierType == TOTAL_VALUE)
    {
        minusModifier = GetUnitModValueToRemove(unitMod, TOTAL_VALUE);
    }

    return m_auraModifiersGroup[unitMod][modifierType] - minusModifier;
}
...
float Unit::GetTotalAuraModValue(UnitMods unitMod) const
{
    ...
    value *= m_auraModifiersGroup[unitMod][BASE_PCT];
    value += m_auraModifiersGroup[unitMod][TOTAL_VALUE];
    float pct = m_auraModifiersGroup[unitMod][TOTAL_PCT];
    float amount = GetUnitModValueToRemove(unitMod, TOTAL_PCT);
    if (amount != 0)
    {
        //TC_LOG_DEBUG(LOG_FILTER_VEHICLES, "StatPct: percentage examined was %f for amount %f", pct, amount);

        if (amount >= 0)
            pct *= 1.0f - std::abs(amount) / 100.0f;
        else
            pct *= 1.0f + std::abs(amount) / 100.0f;

        //TC_LOG_DEBUG(LOG_FILTER_VEHICLES, "StatPct: percentage is examined %f for amount %f", pct, amount);
    }
    value *= pct;
...
bool Unit::CanFreeMoveNoRoot() const
{
    return !HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_FLEEING | UNIT_STATE_IN_FLIGHT |
        UNIT_STATE_STUNNED | UNIT_STATE_DISTRACTED) && GetOwnerGUID().IsEmpty();
}
...
void Unit::SetLevel(uint8 lvl)
{
    ...
    if (Player* player = ToPlayer())
    {
        ...
        player::PlayerCache::UpdateLevel(GetGUID(), lvl);
    }
}
...
void Unit::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        GetMap()->RemoveUnitFromRelocateList(*this);
...
{
    for (const auto petAction : PetActionBar)
        data << uint32(petAction.packedData);
}
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorb, SpellInfo const* procAura)
{
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && target)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            {
                if (target->GetTypeId() != TYPEID_PLAYER && target->GetCreatureType() != CREATURE_TYPE_CRITTER && !target->IsPet())
    ...
    DamageInfo damageInfo = DamageInfo(actor, actionTarget, damage, absorb, 0, 0, procSpell, procSpell ? SpellSchoolMask(procSpell->SchoolMask) : SPELL_SCHOOL_MASK_NORMAL, SPELL_DIRECT_DAMAGE);
    ...
    ProcEventInfo eventInfo = ProcEventInfo(actor, actionTarget, target, procFlag, 0, 0, procExtra, nullptr, procSpell, &damageInfo, &healInfo);
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        SpellInfo const* spellProto = itr->second->GetBase()->GetSpellInfo();
        // only auras that trigger a spell or stealth should proc of fully absorbed damage
        if (procExtra & PROC_EX_ABSORB && isVictim)
            if (spellProto->Effects[0].TriggerSpell || spellProto->Effects[1].TriggerSpell || spellProto->Effects[2].TriggerSpell ||
                spellProto->Effects[0].ApplyAuraName == SPELL_AURA_MOD_SHAPESHIFT) // can we use SPELL_ATTR1_UNK4 "stealth and whirlwind + some other"
                active = true;
        ...
        if (!IsAuraTriggeredAtSpellProcEvent(target, triggerData.aura, procSpell, procFlag, procExtra, attType, isVictim, active, triggerData.spellProcEvent))
            continue;
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        if (!aurApp) 
        {
            TC_LOG_ERROR("spells", "ProcDamageAndSpell: procced spell %u does not have an aura application (triggered with value by %s aura of spell %u)", spellInfo->Id, (isVictim ? "a victim's" : "an attacker's"), Id);
            continue;
        }
...
void Unit::TriggerAurasProcOnEvent(std::list<AuraApplication*>* myProcAuras, std::list<AuraApplication*>* targetProcAuras, Unit* actionTarget, uint32 typeMaskActor, uint32 typeMaskActionTarget, uint32 spellTypeMask, uint32 spellPhaseMask, uint32 hitMask, Spell* spell, DamageInfo* damageInfo, HealInfo* healInfo)
{
    ...
    ProcEventInfo myProcEventInfo = ProcEventInfo(this, actionTarget, actionTarget, typeMaskActor, spellTypeMask, spellPhaseMask, hitMask, spell, spell->GetSpellInfo(), damageInfo, healInfo);
    std::list<AuraApplication*> myAurasTriggeringProc;
    GetProcAurasTriggeredOnEvent(myAurasTriggeringProc, myProcAuras, myProcEventInfo);

    // prepare data for target trigger
    ProcEventInfo targetProcEventInfo = ProcEventInfo(this, actionTarget, this, typeMaskActionTarget, spellTypeMask, spellPhaseMask, hitMask, spell, spell->GetSpellInfo(), damageInfo, healInfo);
...
void Unit::SendPetActionFeedback(uint8 msg)
{
    ...
    data << uint8(msg);
    owner->ToPlayer()->SendDirectMessage(std::move(data));
}
...
void Unit::StopMoving(bool force /* = false */)
{
    ...
    if (!force && (!IsInWorld() || (!isMoving() && movespline->Finalized())))
        return;
    ...
    {
        float x, y, z;
        CalculateExpectedPosition(100).GetPosition(x, y, z);
        Relocate(x, y, z);
    }
...
void Unit::SendMovementFlagUpdate(bool self /* = false */)
{
    ...
    SendMessageToSet(std::move(data), self);
}

WorldPacket Unit::BuildHeartBeatMsg(void) const
{
    return BuildMovementPacket(MSG_MOVE_HEARTBEAT);
}
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
    ...
    VisitAnyNearbyObject<Unit, Trinity::ContainerAction>(dist, targets, u_check);
...
{
    return getMSTimeDiff(m_lastManaUse, game::time::GetGameTimeMS()) < 5000;
}
...
Pet* Unit::CreateTamedPetFrom(Creature* creatureTarget, uint32 spell_id)
{
    ...
    Pet* pet = Pet::CreateFromCreature(*ToPlayer(), HUNTER_PET, *creatureTarget);
    if (!pet)
        return NULL;

    if (auto currentPet = ToPlayer()->GetPetCache().GetBySlot(PET_SAVE_NOT_IN_SLOT))
    {
        auto stableSlot = ToPlayer()->GetPetCache().GetFirstFreeStableSlot();

        if (stableSlot == PET_SAVE_AS_CURRENT)
        {
            delete pet;
            return NULL;
        }

        ToPlayer()->GetPetCache().SwapStableSlots(PET_SAVE_NOT_IN_SLOT, stableSlot);
    }
...
Pet* Unit::CreateTamedPetFrom(uint32 creatureEntry, uint32 spell_id)
{
    ...
    Pet* pet = Pet::CreateFromCreatureInfo(*ToPlayer(), HUNTER_PET, creatureInfo);
    if (!pet)
        return nullptr;
...
bool Unit::IsAuraTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
	SpellInfo const* spellReceivingProc = aura->GetSpellInfo();
	return IsTriggeredAtSpellProcEvent(victim, spellReceivingProc, procSpell, procFlag, procExtra, attType, isVictim, active, spellProcEvent);
}

bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, SpellInfo const* spellReceivingProc, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    // let the aura be handled by new proc system if it has new entry
    if (sSpellMgr->GetSpellProcEntry(spellReceivingProc->Id))
        return false;

    // Get proc Event Entry
    spellProcEvent = sSpellMgr->GetSpellProcEvent(spellReceivingProc->Id);
    ...
    if (spellProcEvent && spellProcEvent->procFlags) // if exist get custom spellProcEvent->procFlags
        ...
        EventProcFlag = spellReceivingProc->ProcFlags;       // else get from spell proto
    ...
    {
        if (!spellReceivingProc->HasAttribute(SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED))
            return false;
    }
    ...
    if (EventProcFlag & PROC_FLAG_KILL && GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (spellReceivingProc->Id == 32409)
            allow = true;
        if (!allow)
            return false;
    }
    ...
    if (procSpell && procSpell->Id == spellReceivingProc->Id
        && !(spellReceivingProc->ProcFlags&(PROC_FLAG_TAKEN_PERIODIC | PROC_FLAG_KILL)))
        return false;
    ...
    {
        Player* player = ToPlayer();
        if (spellReceivingProc->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            Item* item = NULL;
            ...
            if (!item || item->IsBroken() || item->GetTemplate()->Class != ITEM_CLASS_WEAPON || !((1<<item->GetTemplate()->SubClass) & spellReceivingProc->EquippedItemSubClassMask))
                return false;
        }
        else if (spellReceivingProc->EquippedItemClass == ITEM_CLASS_ARMOR)
        {
            // Check if player is wearing shield
            Item* item = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
            if (!item || item->IsBroken() || item->GetTemplate()->Class != ITEM_CLASS_ARMOR || !((1<<item->GetTemplate()->SubClass) & spellReceivingProc->EquippedItemSubClassMask))
                return false;
        }
    }
    // Get chance from spell
    float chance = float(spellReceivingProc->ProcChance);
    ...
    if (spellProcEvent && spellProcEvent->ppmRate != 0)
    {
        if (!isVictim)
        {
            ...
            chance = GetPPMProcChance(weaponSpeed, spellProcEvent->ppmRate, spellReceivingProc);
        }
        else if (victim)
        {
            uint32 weaponSpeed = victim->GetAttackTime(attType);
            chance = victim->GetPPMProcChance(weaponSpeed, spellProcEvent->ppmRate, spellReceivingProc);
        }
    }
    // Apply chance modifer aura
    if (Player* modOwner = GetSpellModOwner())
    {
        modOwner->ApplySpellMod(spellReceivingProc->Id, SPELLMOD_CHANCE_OF_SUCCESS, chance);
    }
    return roll_chance_f(chance);
}

void Unit::SetCustomFlag(UnitCustomFlagMask flags)
{
    customFlags |= flags;
}

void Unit::RemoveCustomFlag(UnitCustomFlagMask flags)
{
    customFlags &= ~flags;
}

bool Unit::HasCustomFlag(UnitCustomFlagMask flag) const
{
...
bool Unit::HandleAuraRaidProcFromCharge(AuraEffect* triggeredByAura)
{
    ...
    if (jumps > 0)
    {
        if (Unit* caster = triggeredByAura->GetCaster())
        {
            ...
            if (Unit* target= GetNextRandomRaidMemberOrPet(radius))
            {
                caster->CastSpell(target, spellProto, true, NULL, triggeredByAura, caster_guid);
                if (Aura* aura = target->GetAura(spellProto->Id, caster_guid))
                    aura->SetCharges(jumps);
            }
        }
    }
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (victim->GetTypeId() == TYPEID_UNIT)
        victim->ToCreature()->HandleDelayedDeath(CREATURE_DEATH_DELAY);
    else
        victim->SetHealth(0);
        ...
        if (group)
        {
            ...
            if (creature)
            {
                ...
                if (group->GetLooterGuid())
                {
                    Player* masterLooter;
                    if (
                        group->GetLootMethod() == MASTER_LOOT && creature->IsDungeonBoss() &&
                        (masterLooter = ObjectAccessor::GetPlayer(*creature, group->GetMasterLooterGuid())) &&
                        masterLooter->IsAtGroupRewardDistance(creature)
                    )
                        looter = masterLooter;
                    else
                        looter = ObjectAccessor::GetPlayer(*this, group->GetLooterGuid());
    ...
    if (victim->GetCreatureType() != CREATURE_TYPE_CRITTER && this != victim)
    ...
    if (player)
        player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE_TYPE, 1, 0, victim);
    ...
    *this <<= Fire::Kill.Bind(victim);
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        if ((durabilityLoss && !player && !victim->ToPlayer()->InBattleground()) || (player && sWorld->getBoolConfig(CONFIG_DURABILITY_LOSS_IN_PVP)))
        {
            ...
            plrVictim->SendDirectMessage(std::move(data));
        }

        // Call KilledUnit for creatures
        if (GetTypeId() == TYPEID_UNIT && IsAIEnabled)
            ToCreature()->OnKill(*victim);
                ...
                if (instanceMap->IsRaidOrHeroicDungeon())
                {
                    if (creature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_INSTANCE_BIND)
                    {
                        ...
                        if (!creditedPlayer && instanceMap->HavePlayers())
                            creditedPlayer = &(*instanceMap->GetPlayers().begin());
    ...
    if (creature && creature->IsDungeonBoss())
        RG_LOG<EncounterLogModule>(victim, EncounterLogModule::Type::KILL);
}
...
void Unit::SetFeared(bool apply)
{
    if (apply)
    {
        ...
        GetMotionMaster()->MoveFleeing(caster, fearAuras.empty() ? sWorld->getIntConfig(CONFIG_CREATURE_FAMILY_FLEE_DELAY) : 0, true);             // caster == NULL processed in MoveFleeing
    }
    else
    {
        if (IsAlive())
...
//        ABORT();
    ...
    if (Creature* creature = ToCreature())
    {
        ...
        if (charmer && type != CHARM_TYPE_VEHICLE)
                ...
                if (GetTypeId() == TYPEID_UNIT && charmer->getClass() == CLASS_WARLOCK)
                {
                    ...
                    if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                    {
                        ...
                        if (GetCharmInfo())
                            ...
                            TC_LOG_ERROR("entities.unit", "Aura::HandleModCharm: target=%s has a charm aura but no charm info!", GetGUID().ToString().c_str());
                    }
                }
                break;
            case CHARM_TYPE_CONVERT:
...
{
    return m_vehicle && vehicle && m_vehicle == vehicle->GetVehicleKit();
}
...
bool Unit::IsInPartyWith(Unit const* unit) const
{
    ...
    else if (u1->GetTypeId() == TYPEID_UNIT && u2->GetTypeId() == TYPEID_UNIT && u1->GetFactionTemplateEntry() && u2->GetFactionTemplateEntry())
        return u1->GetFactionTemplateEntry()->IsFriendlyTo(*u2->GetFactionTemplateEntry());
    else
        return false;
}
...
float Unit::MeleeSpellHitChance(WeaponAttackType attType, uint32 spellId) const
{
    //calculate hit chance
    float hitChance = 0.0f;

    if ((!spellId && haveOffhandWeapon() && !m_currentSpells[CURRENT_MELEE_SPELL]) || (IsOfType<Creature>() && attType == OFF_ATTACK))
        hitChance -= 19;
...
void Unit::UpdateObjectVisibility(bool forced)
{
    ...
    {
        if (IsInWorld())
            GetMap()->AddUnitToRelocateList(*this);
    }
    else
    {
        WorldObject::UpdateObjectVisibility(true);
        // call MoveInLineOfSight for nearby creatures
        auto notifier = Trinity::createRelocationNotifier<Unit>(*this);
...
combat::BaseTable Unit::CreateAttackTable(const Unit& victim, WeaponAttackType attType, const SpellInfo* spellInfo) const
{
    // attacker params

    combat::AttackerParams attackerParams = { *this };
    attackerParams.isPlayer = IsOfType<Player>();
    attackerParams.isRanged = attType == RANGED_ATTACK;
    if (spellInfo && spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED && !spellInfo->IsRangedWeaponSpell())
        attackerParams.weaponSkill = getLevel() * 5;
    else
        attackerParams.weaponSkill = (int32)GetWeaponSkillValue(attType, &victim);

    attackerParams.hit = MeleeSpellHitChance(attType, spellInfo ? spellInfo->Id : 0);
    attackerParams.hit += GetTotalAuraModifier(SPELL_AURA_MOD_HIT_CHANCE);

    attackerParams.crit = GetUnitCriticalChance(attType, &victim);
    attackerParams.expertise = AquireExpertise(attType);

    // victim params 
    combat::VictimParams victimParams = { victim };
    victimParams.isPlayer = victim.IsOfType<Player>();
    victimParams.level = victim.getLevel();
    victimParams.defenseSkill = victim.GetDefenseSkillValue(this, false);

    victimParams.miss = victim.GetUnitMissChance(attType);
    victimParams.parry = victim.GetUnitParryChance();
    victimParams.dodge = victim.GetUnitDodgeChance();
    victimParams.block = victim.GetUnitBlockChance();

    if (spellInfo)
        victimParams.resist = victim.GetMechanicResistChance(spellInfo);
    else
        victimParams.resist = 0.0f;
    victimParams.deflect = victim.GetTotalAuraModifier(SPELL_AURA_DEFLECT_SPELLS);

    victimParams.isAttackedFromBehind = isAttackedFromBehind(victim, *this);
    victimParams.cantDefend = IsNonMeleeSpellCast(false, false, true) || HasUnitState(UNIT_STATE_CONTROLLED);

    // apply modifiers to base chance
    victimParams.dodge += GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_COMBAT_RESULT_CHANCE, VICTIMSTATE_DODGE);

    // create base table

    auto baseTable = combat::Formulas::CreateBaseTable(attackerParams, victimParams);

    // modifier pre-calculation
    baseTable.mods[combat::AttackTableEntry::Dodge] *= GetTotalAuraMultiplier(SPELL_AURA_MOD_ENEMY_DODGE);
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
    ...
    if (result)
        this <<= Fire::SpellClick.Bind(clicker);
...
WorldPacket Unit::BuildMovementPacket(Opcodes opcode, ObjectGuid guid) const
{
...
void Unit::BuildMovementPacket(ByteBuffer& packet) const
{
    packet << uint32(GetUnitMovementFlags());            // movement flags
    packet << uint16(GetExtraUnitMovementFlags());       // 2.3.0
    packet << uint32(game::time::GetGameTimeMS());                       // time / counter
    packet << GetPositionX();
    packet << GetPositionY();
    packet << GetPositionZMinusOffset();
    ...
    {
        if (m_vehicle)
            packet << m_vehicle->GetBase()->GetPackGUID();
        else if (GetTransport())
            packet << GetTransport()->GetPackGUID();
        else
            packet << (uint8)0;

        packet << float(GetTransOffsetX());
        packet << float(GetTransOffsetY());
        packet << float(GetTransOffsetZ());
        packet << float(GetTransOffsetO());
        packet << uint32(GetTransTime());
        packet << uint8(GetTransSeat());

        if (GetExtraUnitMovementFlags() & MOVEMENTFLAG2_INTERPOLATED_MOVEMENT)
            packet << uint32(m_movementInfo.transport.time2);
    }

    // 0x02200000
    if ((GetUnitMovementFlags() & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING))
        || (m_movementInfo.flags2 & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING))
        packet << (float)m_movementInfo.pitch;

    packet << (uint32)m_movementInfo.fallTime;

    // 0x00001000
    if (GetUnitMovementFlags() & MOVEMENTFLAG_FALLING)
    {
        packet << (float)m_movementInfo.jump.zspeed;
        packet << (float)m_movementInfo.jump.sinAngle;
        packet << (float)m_movementInfo.jump.cosAngle;
        packet << (float)m_movementInfo.jump.xyspeed;
    }

    // 0x04000000
    if (GetUnitMovementFlags() & MOVEMENTFLAG_SPLINE_ELEVATION)
        packet << (float)m_movementInfo.splineElevation;
}
...
void Unit::SendTeleportPacket(Position& pos)
{
    ...
    WorldPacket data = BuildMovementPacket(MSG_MOVE_TELEPORT);
    if (GetTypeId() == TYPEID_UNIT)
        Relocate(&oldPos);
    if (GetTypeId() == TYPEID_PLAYER)
        Relocate(&pos);
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport, bool passenger)
{
...
void Unit::RewardRage(uint32 damage, uint32 weaponSpeedHitFactor, bool attacker)
{
    ...
    float rageconversion = ((0.0091107836f * getLevelForTarget(this) * getLevelForTarget(this)) + 3.225598133f * getLevelForTarget(this)) + 4.2652911f;

    // Unknown if correct, but lineary adjust rage conversion above level 70
    if (getLevelForTarget(this) > 70)
        rageconversion += 13.27f * (getLevelForTarget(this) - 70);
...
void Unit::OutDebugInfo() const
{
    ...
    TC_LOG_DEBUG("entities.unit", "GUID " UI64FMTD ", entry %u, type %u, name %s", GetGUID().GetRawValue(), GetEntry(), (uint32)GetTypeId(), GetName().c_str());
    TC_LOG_DEBUG("entities.unit", "OwnerGUID " UI64FMTD ", MinionGUID " UI64FMTD ", CharmerGUID " UI64FMTD ", CharmedGUID " UI64FMTD, GetOwnerGUID().GetRawValue(), GetMinionGUID().GetRawValue(), GetCharmerGUID().GetRawValue(), GetCharmGUID().GetRawValue());
...
void Unit::UpdateMovementFlags(uint32 flags)
{
    SetWalk(flags & MOVEMENTFLAG_WALKING);
    SetDisableGravity(flags & MOVEMENTFLAG_DISABLE_GRAVITY);
    SetFall(flags & MOVEMENTFLAG_FALLING);
    SetSwim(flags & MOVEMENTFLAG_SWIMMING);
    SetCanFly(flags & MOVEMENTFLAG_CAN_FLY);
...
void Unit::BuildValuesUpdate(ObjectUpdateType updateType, ByteBuffer& data, Player* target) const
{
    const auto startPos = data.wpos();
    UpdateMask updateMask(m_valuesCount, 0);
    data << updateMask;
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
            ...
            (index == UNIT_FIELD_AURASTATE && HasFlag(UNIT_FIELD_AURASTATE, PER_CASTER_AURA_STATE_MASK)))
        {
            ...
            if (index == UNIT_NPC_FLAGS)
            {
                ...
                if (creature)
                    if (!target || !target->CanSeeSpellClickOn(creature))
                        appendValue &= ~UNIT_NPC_FLAG_SPELLCLICK;

                data << uint32(appendValue);
            }
            else if (index == UNIT_FIELD_AURASTATE && target)
            {
            ...
            else if (index == UNIT_FIELD_FLAGS)
            {
                ...
                if (target && (target->IsGameMaster() || target->isDeveloper()))
            ...
            else if (index == UNIT_FIELD_DISPLAYID)
            {
                ...
                if (auto plr = ToPlayer())
                    if (!plr->IsPlayingNative() && plr->GetNativeDisplayId() == displayId && plr == target)
                        displayId = plr->GetOriginalDisplayId();

                data << uint32(displayId);
            }
            else if (index == UNIT_FIELD_NATIVEDISPLAYID)
            {
                uint32 displayId = m_uint32Values[UNIT_FIELD_NATIVEDISPLAYID];

                if (auto plr = ToPlayer())
                    if (!plr->IsPlayingNative() && plr == target)
                        displayId = plr->GetOriginalDisplayId();
            ...
            else if (index == UNIT_DYNAMIC_FLAGS)
            {
                ...
                if (creature)
                {
                    if (creature->hasLootRecipient())
                    {
                        ...
                        if (target && creature->isTappedBy(target))
                            dynamicFlags |= UNIT_DYNFLAG_TAPPED_BY_PLAYER;
                    }

                    if (!target || !target->isAllowedToLoot(creature))
                        dynamicFlags &= ~UNIT_DYNFLAG_LOOTABLE;
                }

                // unit UNIT_DYNFLAG_TRACK_UNIT should only be sent to caster of SPELL_AURA_MOD_STALKED auras
                if (dynamicFlags & UNIT_DYNFLAG_TRACK_UNIT)
                    if (!target || !HasAuraTypeWithCaster(SPELL_AURA_MOD_STALKED, target->GetGUID()))
            ...
            {
                if (target && IsControlledByPlayer() && target != this && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) && IsInRaidWith(target))
                {
            ...
            else if (index == PLAYER_CHOSEN_TITLE)
            {
                uint32 swap = 0;
                if (auto plr = ToPlayer())
                if (!plr->IsPlayingNative())
                {
                    auto itr = sObjectMgr->FactionChangeFullTitleMap.find(m_uint32Values[index]);
                    if (itr != sObjectMgr->FactionChangeFullTitleMap.end())
                        swap = itr->second;
                }

                data << (swap ? swap : m_uint32Values[index]);
            }
            else if (index == UNIT_FIELD_BYTES_0)
            {
                auto val = m_uint32Values[index];
                if (auto plr = ToPlayer())
                if (!plr->IsPlayingNative() && this != target)
                    val = plr->getRace() + (val & 0xFFFFFF00);
                data << val;
            }
            else
            {
    ...
    data.replace(startPos, updateMask);
}

std::vector<WorldPacket> Unit::BuildInitialVisiblePackets()
{
    std::vector<WorldPacket> result;

    if (!GetVisibleAuras()->empty())
    {
    ...
    if (IsAlive() && HasUnitState(UNIT_STATE_MELEE_ATTACKING) && GetVictim())
    {
        WorldPacket data(SMSG_ATTACKSTART, 8 + 8);
        data << uint64(GetGUID());
        data << uint64(GetVictim()->GetGUID());
        result.push_back(std::move(data));
    }
...
int32 Unit::GetHighestExclusiveAuraDiff(Aura const* aura, uint8 index, bool checkMiscValue, int32 miscValue) const
{
    AuraEffect const *aurEff = aura->GetEffect(index);
    // absolute value only
    int32 baseAmmount = abs(aurEff->GetAmount());
    int32 val = baseAmmount;

    AuraEffectList const& auras = GetAuraEffectsByType(AuraType(aura->GetSpellInfo()->Effects[index].ApplyAuraName));
    for (Unit::AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end();)
    {
        ...
        if (sSpellMgr->CheckSpellGroupStackRules(aura->GetSpellInfo(), existingAurEff->GetSpellInfo())
            == SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT && (!checkMiscValue || existingAurEff->GetMiscValue() & miscValue))
        {
            int32 diff = baseAmmount - abs(existingAurEff->GetAmount());
            if (diff < val)
                val = diff; 
            if (val < 0)
                return 0; // If we are not the most powerful aura, return null.
        }
    }
    // return the difference to the second most powerful effect 
    // or our value if there are no other auras with the same effect
    return val;
}
...
Position Unit::CalculateExpectedPosition(uint32 time) const
{
    if (!m_movementInfo.HasMovementFlag(MOVEMENTFLAG_MASK_MOVING) || GetTransport() != nullptr)
        return Position(*this);

    if (auto playerMover = ToPlayer())
        time += std::min(GetMSTimeDiffToNow(playerMover->GetLastMovementPacketTime()), 500u);

    float timeSec = float(time) / 1000.f;

    if (m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING))
    {
        float x, y, z;
        const MovementInfo::JumpInfo jump = m_movementInfo.jump;
        GetPosition(x, y, z);
        x += (jump.xyspeed * timeSec) * jump.sinAngle;
        y += (jump.xyspeed * timeSec) * jump.cosAngle;
        z = GetPositionZ() + 2.f;
        UpdateAllowedPositionZ(x, y, z);
        return Position(x, y, z);
    }

    if (HasUnitMovementFlag(MOVEMENTFLAG_SPLINE_ENABLED | MOVEMENTFLAG_FORWARD))
    {
        if (!movespline->Initialized())
            return Position(*this);
        auto const loc = movespline->ComputePosition(time);
        return Position(loc.x, loc.y, loc.z, loc.orientation);
    }
    
    float speedXY = GetSpeed(Movement::SelectSpeedType(m_movementInfo.flags));

    Position origin(*this);

    if (m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FLYING | MOVEMENTFLAG_SWIMMING))
    {
        const float sin = std::sin(m_movementInfo.pitch);
        const float cos = std::cos(m_movementInfo.pitch);
        const float speedZ = sin * speedXY;
        speedXY *= cos;
        origin.m_positionZ += speedZ * timeSec;
    }

    int32 turnDirection = (m_movementInfo.HasMovementFlag(MOVEMENTFLAG_LEFT)) ? 1 : (m_movementInfo.HasMovementFlag(MOVEMENTFLAG_RIGHT)) ? -1 : 0;
    
    float orientation = 0.f;
    switch (m_movementInfo.flags & (MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_STRAFE_RIGHT | MOVEMENTFLAG_STRAFE_LEFT | MOVEMENTFLAG_BACKWARD))
    {
        case MOVEMENTFLAG_FORWARD: break;
        case MOVEMENTFLAG_STRAFE_RIGHT: orientation -= M_PI_F / 2.0f; break;
        case MOVEMENTFLAG_STRAFE_LEFT: orientation += M_PI_F / 2.0f; break;
        case MOVEMENTFLAG_STRAFE_RIGHT + MOVEMENTFLAG_FORWARD: orientation -= M_PI_F / 4.0f; break;
        case MOVEMENTFLAG_STRAFE_LEFT + MOVEMENTFLAG_FORWARD: orientation += M_PI_F / 4.0f; break;
        case MOVEMENTFLAG_BACKWARD: orientation += M_PI_F; break;
        case MOVEMENTFLAG_STRAFE_RIGHT + MOVEMENTFLAG_BACKWARD: orientation -= (3.0f * M_PI_F) / 4.0f; break;
        case MOVEMENTFLAG_STRAFE_LEFT + MOVEMENTFLAG_BACKWARD: orientation += (3.0f * M_PI_F) / 4.0f; break;
    }

    Position result;

    if (!turnDirection)
        result = MovePositionToFirstCollision2D(origin, speedXY * timeSec, orientation + GetOrientation());
    else
    {
        orientation += (M_PI / 2.f) * turnDirection;
        // Turn-Around-Time with MOVE_TURN_RATE == 1.f => 2.666666 sec.
        float turnaroundtime = 2.666666f / GetSpeedRate(MOVE_TURN_RATE);
        float radius = (speedXY * turnaroundtime) / (2.f * M_PI);
        // While turning we are running in a circle. First we calculate our circle centre here.
        GetNearPosition(result, radius, orientation);
        // Now calculate our angle within the circle after timeSec passed.
        float angle = 2 * M_PI * timeSec / turnaroundtime;
        angle = orientation + M_PI + (angle * turnDirection);
        MovePosition(result, radius, angle);
        result = MovePositionToFirstCollision2D(origin, GetExactDist2d(&result), GetAngle(&result));
    }

    return result;
}

void Unit::GetPetFollowPosition(WorldObject const* searcher, float &x, float &y, float &z, float dist, float angle) const
{
    if (!IsInWorld())
    {
        GetPosition(x, y, z);
    ...
    if (m_movementInfo.HasMovementFlag(MOVEMENTFLAG_MASK_MOVING))
        CalculateExpectedPosition(500).GetPosition(tarX, tarY, tarZ);
    else
        GetPosition(tarX, tarY, tarZ);

    angle += GetOrientation();
    x = tarX + (GetObjectSize() + 1.f + dist) * cos(angle);
    y = tarY + (GetObjectSize() + 1.f + dist) * sin(angle);
    float groundZ = GetMap()->GetHeight(GetPhaseMask(), x, y, MAX_HEIGHT);
    float floorZ = GetMap()->GetHeight(GetPhaseMask(), x, y, tarZ);
    z = (floorZ <= INVALID_HEIGHT && groundZ > INVALID_HEIGHT && groundZ > tarZ) ? groundZ : tarZ;
    searcher->UpdateAllowedPositionZ(x, y, z);

    if (fabs(tarZ - GetPositionZ()) < 5.f && fabs(tarZ - z) < 5.f && IsWithinLOS(x, y, z) &&
        GetMap()->isInLineOfSight(tarX, tarY, tarZ + 2.f, x, y, z + 2.f, GetPhaseMask()))
    {
        return;
    }

    GetClosePoint(x, y, z, 1.f, dist, GetAngle(searcher) - GetOrientation());
}

Position Unit::GetFollowPosition(Position const& target, float preferedOrientation, float maxDistance, float preferedMinDistance) const
{
    ASSERT(maxDistance >= preferedMinDistance);

    const float minDistSq = preferedMinDistance * preferedMinDistance;

    const std::array<float, 8> SEARCH_ANGLES = { 0.f, float(M_PI_4), float(2 * M_PI - M_PI_4), float(2 * M_PI_4), 
        float(2 * (M_PI - M_PI_4)), float(3 * M_PI_4), float(2 * M_PI - 3 * M_PI_4), float(M_PI) };

    for (float angle : SEARCH_ANGLES)
    {
        Position result = MovePositionToFirstCollision3D(target, maxDistance, preferedOrientation + angle);
        
        if (target.GetExactDistSq(&result) >= minDistSq)
        {
            if (GetMap()->isInLineOfSight(target.GetPositionX(), target.GetPositionY(), target.GetPositionZ() + 2.f,
                result.GetPositionX(), result.GetPositionY(), result.GetPositionZ() + 2.f, GetPhaseMask()))
                return result;
        }
    }
...
void RelocateAttackerToAngle(Unit* origin, const AttackerData& attacker, float angle)
{
    ...
    origin->GetCombatPosition(attacker.unit, x, y, z, angle, false, attacker.dist);
...
void Unit::UpdateChaserCombatPosition()
{
    ...
    for (auto attacker : m_attackers)
    {
        ...
        {
            AttackerData data = { GetAngle(attacker), GetExactDist(attacker), attacker, nextIdx++ };
            float dist = data.dist / DEFAULT_COMBAT_REACH;
            if (dist < 1.0)
                continue;
    ...
    for (uint32 layer = 0; layer < MAX_LAYERS; layer++)
    {
        ...
        if (std::abs(lastItr->angle - 2 * M_PI_F - attackers.begin()->angle) >= max_angle)
        {
            ProcessAttackerGroup(this, attackers, groupItr, lastItr, lastItr - groupItr + 1, max_angle);
