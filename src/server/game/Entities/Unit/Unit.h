...
// Crit Chance Always is arbitrarily choosen: Should be higher than any reachable crit reduction, 
// except effects that make you immune to crit
// these effects reduce the crit chance by -10000 (see Roar of Sacrifice: Id 53480 for example)
#define CRIT_CHANCE_ALWAYS 1000.0f
#define CRIT_CHANCE_NEVER -10000.0f
...
#define MAX_AGGRO_RESET_TIME 12 // in seconds
...
constexpr float DEFAULT_COMBAT_REACH  = 1.5f;
constexpr float MIN_MELEE_REACH       = 2.0f;
constexpr float NOMINAL_MELEE_RANGE   = 5.0f;
constexpr float MELEE_RANGE           = (NOMINAL_MELEE_RANGE - MIN_MELEE_REACH * 2); //center to center for players
...
enum UnitState
{
    ...
    UNIT_STATE_ALWAYS_REACH_TARGET = 0x80000000,
    UNIT_STATE_ALL_STATE_SUPPORTED = UNIT_STATE_DIED | UNIT_STATE_MELEE_ATTACKING | UNIT_STATE_STUNNED | UNIT_STATE_ROAMING | UNIT_STATE_CHASE
                                   ...
                                   | UNIT_STATE_CHASE_MOVE | UNIT_STATE_FOLLOW_MOVE | UNIT_STATE_IGNORE_PATHFINDING | UNIT_STATE_ALWAYS_REACH_TARGET,
...
private:
    ...
    uint32 m_block;
public:
    explicit DamageInfo(Unit* _attacker, Unit* _victim, uint32 _damage, uint32 _absorb, uint32 _resist, uint32 _block, SpellInfo const* _spellInfo, SpellSchoolMask _schoolMask, DamageEffectType _damageType);
...
public:
    ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget, uint32 typeMask,
                  ...
                  Spell* spell, SpellInfo const* _spellInfo, DamageInfo* damageInfo, HealInfo* healInfo);
...
struct GAME_API CalcDamageInfo
{
...
#define ATTACK_DISPLAY_DELAY 50
#define MAX_PLAYER_STEALTH_DETECT_RANGE 30.0f               // max distance for detection targets by player

struct SpellProcEventEntry;                                 // used only privately

class GAME_API Unit : public WorldObject, protected combat::ICombatant
{
    public:
        ...
        using SingleTargetAuraVector = std::vector<std::pair<ObjectGuid, uint32>>;

        typedef std::map<uint8, AuraApplication*> VisibleAuraMap;

        // A flat value (int32, first) and a percentual modifier (float, second)
        // which belong together as part of a intermediate result of a
        // method (calculation)
        // Needed because in some cases, for example for the snapshot mechanic for dots,
        // we are interested in the intermediate results of a damage calculation
        // because after a refreshing of the dot we only recalculate the flat values
        typedef std::pair<int32, float> FlatAndPercentModifiers;
        ...
        void UpdateAttackTimer(uint32 diff);
        void setAttackTimer(WeaponAttackType type, uint32 time, uint32 overlap = 0)
        {
            m_attackTimer[type] = time;
            setAttackTimerOverlap(type, _attackTimerOverlap[type] > 0 && overlap == 0 ? _attackTimerOverlap[type] : _attackTimerOverlap[type] + overlap);
        }
        void setAttackTimerOverlap(WeaponAttackType type, uint32 overlap) { _attackTimerOverlap[type] = overlap; }
        void SetAttackTimerPause(std::chrono::milliseconds const& duration) { m_autoAttackTimerPause.Reset(duration); }
        ...
        uint32 GetSpellPenetration() const;
        ...
        void SetUnkillable(bool isUnkillable) { this->isUnkillable = isUnkillable; }

        void ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorb, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL);
        void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorb, SpellInfo const* procAura = NULL);
        ...
        void AttackerStateUpdate (Unit* victim, WeaponAttackType attType = BASE_ATTACK, bool extra = false, bool ignoreLOS = false);
        ...
        float MeleeSpellHitChance(WeaponAttackType attType, uint32 spellId) const;
        SpellMissInfo MeleeSpellHitResult(const Unit& victim, const SpellInfo& spellInfo) const;
        SpellMissInfo RangedSpellHitResult(const Unit& victim, const SpellInfo& spellInfo)const;
        ...
        float UnitCriticalChanceDone(WeaponAttackType attackType) const;
        float UnitCriticalChanceBonusTaken(WeaponAttackType attackType, const Unit* attacker) const;
        int32 GetMechanicResistChance(SpellInfo const* spellInfo) const;
        bool CanUseAttackType(uint8 attacktype) const;

        virtual uint32 GetShieldBlockValue() const =0;
        uint32 GetShieldBlockValue(uint32 soft_cap, uint32 dmg_cap, uint32 hard_cap) const;
        uint32 GetUnitMeleeSkill(Unit const* target = nullptr) const;
        uint32 GetDefenseSkillValue(Unit const* target = nullptr, bool levelCapped = false) const;
        ...
        void DoDelayed(std::chrono::milliseconds const& delay, std::function<void(Unit&)> fn);

        void CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags = TRIGGERED_NONE, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, ObjectGuid originalCaster = ObjectGuid::Empty, bool softRefresh = false);
        ...
        void CastSoftRefreshSpell(Unit* victim, SpellInfo const* spellInfo);
        ...
        virtual bool UpdatePosition(float x, float y, float z, float ang, bool teleport = false, bool passenger = false);
        ...
        void UpdateMovementFlags(uint32 flags);
        ...
        /**
         * Calculates the position of this unit after a given number of milliseconds using the current Movementflags
         * and direction.
         * @param time time difference in milliseconds
         * @return the calculated position
         * @bug This function doesn't consider yet an active movement spline or 3d flying movement.
         */
        Position CalculateExpectedPosition(uint32 time) const;
        /**
        * Calculates a position that is in line of sight to the target position and within maxDist to the target. This
        * method attemps find a resulting position with minimal angle distance to a given preferred angle. The 
        * resulting position is also above the ground and at a location that is accessible by this unit.
        * @param target the resulting position fulfills all constraints to this target location
        * @param preferedOrientation optimal angle relative from target location
        * @param maxDistance maxium allowed distance for the resulting position (using 3d distance)
        * @param preferedMinDistance algorthm will first select angles where this minimum can be fulfilled
        * @return a position which fulfills all contrains to the target position
        */
        Position GetFollowPosition(Position const& target, float preferedOrientation, float maxDistance, float preferedMinDistance) const;
        void GetPetFollowPosition(WorldObject const* searcher, float &x, float &y, float &z, float dist, float angle) const;
        ...
        WorldPacket BuildHeartBeatMsg() const;
        ...
        Aura* _TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount = NULL, Item* castItem = NULL, ObjectGuid casterGUID = ObjectGuid::Empty, bool softRefresh = false);
        ...
        void RemoveAllGroupBuffsFromOthers();
        ...
        SingleTargetAuraVector      & GetSingleCastAuras()       { return m_scAuras; }
        SingleTargetAuraVector const& GetSingleCastAuras() const { return m_scAuras; }
        ...
        bool HasNegativeAuraWithAttributeEx(uint32 flag, ObjectGuid guid = ObjectGuid::Empty) const;
        ...
        int32 GetTotalNegativeAuraModifier(AuraType auratype) const;
        ...
        int32 GetTotalNegativeAuraModifierByMiscMask(AuraType auratype, uint32 misc_mask) const;
        ...
        float GetTotalNegativeAuraMultiplierByMiscMask(AuraType auratype, uint32 misc_mask) const;
        ...
        // stat system
        bool HandleStatModifier(UnitMods unitMod, UnitModifierType modifierType, float amount, bool apply, const SpellInfo* spell = NULL);
        void SetModifierValue(UnitMods unitMod, UnitModifierType modifierType, float value) { m_auraModifiersGroup[unitMod][modifierType] = value; }
        int32 GetStatValueToRemove(Stats stat);
        float GetUnitModValueToRemove(UnitMods unitMod, UnitModifierType modifierType) const;
        float GetModifierValue(UnitMods unitMod, UnitModifierType modifierType) const;
        float GetTotalStatValue(Stats stat);
        ...
        uint32 CalculateDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, bool average = false);
        ...
        std::vector<WorldPacket> BuildInitialVisiblePackets();
        ...
        bool CanReceiveThreatFrom(const Unit& target) const;
        ...
        FlatAndPercentModifiers SpellDirectDamageBonus(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, int32& damage) const;
        ...
        FlatAndPercentModifiers SpellDirectHealingBonus(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack, int32& heal) const;
        ...
        float  SpellCritChance(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType = BASE_ATTACK) const;
        float  SpellCritChanceBonusTaken(Unit* attacker, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType = BASE_ATTACK) const;
        float  SpellCritChanceDone(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType = BASE_ATTACK) const;
        ...
        void CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 const nonCritDamage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo = NULL);
        void ApplyAbsorbResist(DamageInfo& info);
        ...
        MotionMaster* GetMotionMaster() { return &motionMaster; }
        const MotionMaster* GetMotionMaster() const { return &motionMaster; }
        ...
        WorldPacket BuildMovementPacket(Opcodes opcode, ObjectGuid guid = ObjectGuid::Empty) const;
        void BuildMovementPacket(ByteBuffer& packet) const;
        ...
        int32 GetHighestExclusiveAuraDiff(Aura const* aura, uint8 index, bool checkMiscValue = false, int32 miscValue = 0) const;
        bool IsHighestExclusiveAura(Aura const* aura, bool removeOtherAuraApplications = false);

        ObjectGuid lastChannelTarget;

        void SetInRelocationList(bool value) { isInRelocationList = value; }
        bool IsInRelocationList() const { return isInRelocationList; }

        void OnSummon(WorldObject& summoned);

        bool HasCustomFlag(UnitCustomFlagMask flag) const;
    protected:
        explicit Unit();
        ...
        uint32 _attackTimerOverlap[MAX_ATTACK];
        TimeTrackerSmall m_autoAttackTimerPause;
        ...
        SingleTargetAuraVector m_scAuras;                        // cast singlecast auras
        ...
        void DisableSpline();
		bool IsAuraTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent);
		bool IsTriggeredAtSpellProcEvent(Unit* victim, SpellInfo const* spellReceivingProc, SpellInfo const* spellTriggeringProc, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent);

        void SetCustomFlag(UnitCustomFlagMask flags);
        void RemoveCustomFlag(UnitCustomFlagMask flags);

        virtual float AquireExpertise(WeaponAttackType attType) const = 0;
    ...
    private:	
        ...
        combat::BaseTable CreateAttackTable(const Unit& victim, WeaponAttackType attType, const SpellInfo* spellInfo) const;
        ...
        UnitCustomFlagMask customFlags;
