...
void Player::ApplySpellPowerBonus(int32 amount, bool apply)
{
    ...
    if (Pet* pet = GetPet())
        pet->IsHunterPet() ? pet->UpdateAttackPowerAndDamage(true) : pet->UpdateAttackPowerAndDamage(false);
}
...
bool Guardian::UpdateStats(Stats stat)
{
    ...
    Unit::AuraEffectList const& auraList = GetAuraEffectsByType(AuraType(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE));
    if (!auraList.empty())
    {
        for (Unit::AuraEffectList::const_iterator itr = auraList.begin(); itr != auraList.end(); ++itr)
        {
            if (stat == (*itr)->GetMiscValue() || (*itr)->GetMiscValue() == -1)
                addBonus += (*itr)->GetAmount();
        }
    }
    ...
    else if (stat == STAT_STAMINA)
    {
        ...
        else
        {
            ...
            if (IsPet())
            {
                if (AuraEffect* aurEff = GetDummyAuraEffect(SPELLFAMILY_PET, 3748, 0)) // Wild Hunt
...
void Guardian::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (owner && owner->GetTypeId() == TYPEID_PLAYER)
    {
        if (IsHunterPet())                      //hunter pets benefit from owner's attack power
        {
            ...
            if (AuraEffect* aurEff = GetDummyAuraEffect(SPELLFAMILY_PET, 3748, 1)) // Wild Hunt
                AddPct(mod, aurEff->GetAmount());
        ...
        else if (IsPet())
        {
            ...
            if (GetEntry() == NPC_FELGUARD && owner->HasAura(56246))
            {
        ...
        else if (GetEntry() == NPC_WATER_ELEMENTAL || GetEntry() == NPC_WATER_ELEMENTAL_P)
        {
        ...
        else if (IsRisenAlly()) //ghouls benefit from deathknight's attack power (may be summon pet or not)
        {
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    float damagePerAttackPower = 0.f;
    ...
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        switch (GetEntry())
        {
            ...
            case NPC_FELGUARD: damagePerAttackPower = 0.187f; break;
            case NPC_FELHUNTER: damagePerAttackPower = 0.12f; break;
            case NPC_SUCCUBUS: damagePerAttackPower = 0.157f; break;
            case NPC_VOIDWALKER: damagePerAttackPower = 0.13f; break;
        }
    }
    ...
    float base_value  = GetModifierValue(unitMod, BASE_VALUE) + bonusDamage + GetTotalAttackPowerValue(attType)
        * (damagePerAttackPower ? damagePerAttackPower : (1.f / 14.0f * att_speed));
    ...
    for (Unit::AuraEffectList::const_iterator itr = mDummy.begin(); itr != mDummy.end(); ++itr)
    {
        switch ((*itr)->GetSpellInfo()->Id)
        {
            case 61682:
            case 61683:
                // Attackspeed change is 15/30 % but damage reduction only 7.5/15 %
