...
#include "RG/Logging/modules/MoneyLogTypes.hpp"
...
constexpr float INSPECT_DISTANCE  = 28.0f;
...
{
    SpellModifier(Aura* _ownerAura = NULL) : op(SPELLMOD_DAMAGE), type(SPELLMOD_FLAT), value(0), mask(), spellId(0), ownerAura(_ownerAura) { }
    SpellModOp   op   : 8;
    SpellModType type : 8;
    int32 value;
    flag96 mask;
...
enum PlayerExtraFlags
{
    ...
    PLAYER_EXTRA_GM_VISIBLE_TO_OTHER_GMS    = 0x0080,               // RG Custom: show to other GMs while invisible
...
{
    AT_LOGIN_NONE              = 0x00,
    AT_LOGIN_RENAME            = 0x01,  ///< dependency in character db prepared statements and World::LoadCharacterNameData
...
typedef std::map<uint32, uint32> QuestItemDropPercentage;
...
enum TeleportToOptions
{
    ...
    TELE_TO_SPELL               = 0x10,
    // RG - Custom
    TELE_TO_ARENA_REPLAY        = 0x20,
    TELE_FORCE_TELEPORT_FAR     = 0x80
};
...
enum PlayerLoginQueryIndex
{
    ...
    PLAYER_LOGIN_QUERY_LOAD_SOCIAL_LIST             = 10,
    PLAYER_LOGIN_QUERY_LOAD_HOME_BIND               = 11,
    PLAYER_LOGIN_QUERY_LOAD_SPELL_COOLDOWNS         = 12,
    PLAYER_LOGIN_QUERY_LOAD_DECLINED_NAMES          = 13,
    PLAYER_LOGIN_QUERY_LOAD_GUILD                   = 14,
    PLAYER_LOGIN_QUERY_LOAD_ARENA_INFO              = 15,
    PLAYER_LOGIN_QUERY_LOAD_ACHIEVEMENTS            = 16,
    PLAYER_LOGIN_QUERY_LOAD_CRITERIA_PROGRESS       = 17,
    PLAYER_LOGIN_QUERY_LOAD_EQUIPMENT_SETS          = 18,
    PLAYER_LOGIN_QUERY_LOAD_BG_DATA                 = 19,
    PLAYER_LOGIN_QUERY_LOAD_GLYPHS                  = 20,
    PLAYER_LOGIN_QUERY_LOAD_TALENTS                 = 21,
    PLAYER_LOGIN_QUERY_LOAD_ACCOUNT_DATA            = 22,
    PLAYER_LOGIN_QUERY_LOAD_SKILLS                  = 23,
    PLAYER_LOGIN_QUERY_LOAD_WEEKLY_QUEST_STATUS     = 24,
    PLAYER_LOGIN_QUERY_LOAD_RANDOM_BG               = 25,
    PLAYER_LOGIN_QUERY_LOAD_BANNED                  = 26,
    PLAYER_LOGIN_QUERY_LOAD_QUEST_STATUS_REW        = 27,
    PLAYER_LOGIN_QUERY_LOAD_INSTANCE_LOCK_TIMES     = 28,
    PLAYER_LOGIN_QUERY_LOAD_SEASONAL_QUEST_STATUS   = 29,
    PLAYER_LOGIN_QUERY_LOAD_MONTHLY_QUEST_STATUS    = 30,
    PLAYER_LOGIN_QUERY_LOAD_ACCOUNT_COMPLAINTS      = 31,
    PLAYER_LOGIN_QUERY_LOAD_PVP_RANK                = 32,
    PLAYER_LOGIN_QUERY_LOAD_CUSTOM_FLAGS            = 33,
...
enum PlayerDelayedOperations
{
    ...
    DELAYED_ARENA_REPLAY_START  = 0x40,
    DELAYED_LFG_GROUP_CLEANUP   = 0x80,
    DELAYED_END
};
...
class GAME_API Player : public Unit, public Grids::GridObject<Player>, public ExecutionContext<Player>, public std::enable_shared_from_this<Player>
{
    ...
    public:
        static std::shared_ptr<Player> Create(WorldSession& session, CharacterCreateInfo& createInfo);
        static std::shared_ptr<Player> LoadFromDB(ObjectGuid const& guid, WorldSession& session, SQLQueryHolder& holder);
    ...
    public:
        explicit Player(private_tag, WorldSession* session); //< Use static factory methods (::Create or ::LoadFromDB) instead
    ...
    private:
        ...
        void InvalidatePlayer(ObjectGuid playerGUID);
    public:
        void SendChatMessage(const char *format, ...);
        void FitPlayerInTeam(bool action, Battleground* pBattleGround);
        ...
        uint32 GetOriginalDisplayId() const { return originalDisplayId; }
        uint8 getFRace() const { return m_FakeRace; }
        void MorphFit(bool value);
        bool IsPlayingNative() const;
        ...
        TeamId GetOTeamId() const { return GetOTeam() == ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE; }
        bool SendRealNameQuery() const;
        ...
        WorldLocation GetBGEntryPoint(){ return m_bgData.joinPos; }
        ...
        static bool BuildEnumData(SharedPreparedQueryResult result, WorldPacket* data);
        ...
        bool isGMVisibleToOtherGMs() const { return m_ExtraFlags & PLAYER_EXTRA_GM_VISIBLE_TO_OTHER_GMS; }
        void SetGMVisibleToOtherGMs(bool on);
        ...
        void SetDelayedCombatStop() { m_delayedCombatStop = 1; }
        ...
        void Whisper(std::string const& text, const uint32 language, Player* receiver);

        void SendMailLevelRewardExtended(uint8 level, uint32 raceMask);
        ...
        InventoryResult CanUseItem(ItemTemplate const* pItem, bool checkLevel) const;
        InventoryResult CanUseItem(ItemTemplate const* pItem) const { return CanUseItem(pItem, true); }
        InventoryResult CanUseAmmo(uint32 item) const;
        InventoryResult CanRollForItemInLFG(LootItem const* item, WorldObject const* lootedObject) const;
        ...
        QuestItemDropPercentage m_QuesItemDropPercentage;
        uint32 GetQuestItemDropStep(uint32 questId);
        void IncreaseOrAddQuestItemDropStep(uint32 questId);
        void ResetQuestItemDropStep(uint32 questId);
        ...
        void RemoveAurasDueToItem(ObjectGuid castItemGuid);
        ...
        void SetQuestAsRewarded(uint32 questId);
        ...
        uint32 GetQuestIdOfItemId(uint32 entry) const;
        void KilledMonster(CreatureTemplate const* cInfo, ObjectGuid guid);
        void KilledMonsterCredit(uint32 entry, ObjectGuid guid = ObjectGuid::Empty);
        bool GetKilledCreatureCountForQuest(uint32 quest_entry, uint8 mobcount, uint32 amount) const;
        ...
        ObjectGuid GetQuestSharer() const { return m_questSharer; }
        void SetQuestSharer(ObjectGuid guid) { m_questSharer = guid; }
        ...
        void SaveToDB(bool create = false, bool logout = false);
        void SaveInventoryAndCurrenciesToDB(SQLTransaction& trans); // fast save function for item/money/arena/honor cheating preventing
        void SaveInventoryAndGoldToDB(SQLTransaction& trans);       // fast save function for item/money cheating preventing
        void SaveGoldToDB(SQLTransaction& trans);
        void SaveQuestStatusAndReward();

        static void SetUInt32ValueInArray(Tokenizer& data, uint16 index, uint32 value);
        static void SetFloatValueInArray(Tokenizer& data, uint16 index, float value);
        static void Customize(SQLTransaction& trans, ObjectGuid guid, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair);
        static void SavePositionInDB(SQLTransaction& trans, uint32 mapid, float x, float y, float z, float o, uint32 zone, ObjectGuid guid);
        ...
        bool ModifyMoney(int32 amount, ::RG::Logging::MoneyLogType type = ::RG::Logging::MoneyLogType::UNKNOWN, uint32 data = 0);
        ...
        void SendSpellModifier(SpellModifier* mod, bool apply, int32 value, uint8 eff);
        ...
        Unit::FlatAndPercentModifiers ApplySpellMod(uint32 spellId, SpellModOp op, float &basevalue, Spell* spell = NULL, bool useCachedMods = true);
        Unit::FlatAndPercentModifiers ApplySpellMod(uint32 spellId, SpellModOp op, uint32 &basevalue, Spell* spell = NULL, bool useCachedMods = true);
        Unit::FlatAndPercentModifiers ApplySpellMod(uint32 spellId, SpellModOp op, int32 &basevalue, Spell* spell = NULL, bool useCachedMods= true);
        ...
        util::synchronized_ptr<Guild, std::unique_lock<std::mutex>> GetGuild();
        ...
        void FlushUpdateData();
        void BuildUpdateBlockForPlayer(UpdateData& data, Player* target) const override;
        void ApplyValuesUpdateBlock(const Object& object, bool flush);
        ...
        void SendDynamicDifficultyResult(uint32 result, uint32 data = 0);

        virtual bool UpdatePosition(float x, float y, float z, float orientation, bool teleport = false, bool passenger = false);
        bool UpdatePosition(const Position &pos, bool teleport = false) { return UpdatePosition(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation(), teleport, false); }
        void UpdateUnderwaterState(Map* m, float x, float y, float z);

        void SendMessageToSet(const WorldPacket& data, bool self) override;
        void SendMessageToSetInRange(const WorldPacket& data, float fist, bool self) override;
        void SendMessageToSetInRange(const WorldPacket& data, float dist, bool self, bool own_team_only);
        void SendMessageToSet(const WorldPacket& data, Player const* skipped_rcvr);
        ...
        {
            if (m_ghoulResurrectPlayerGUID)
                return m_ghoulResurrectPlayerGUID == guid;
            else
                return false;
        }
        void GhoulResurrect();
        ...
        bool IsBeingTeleportedToReplay() const { return (m_DelayedOperations & DELAYED_ARENA_REPLAY_START) == DELAYED_ARENA_REPLAY_START; }
        ...
        bool IsAtGroupLootDistance(WorldObject const* pRewardSource) const;
        bool IsAtRecruitAFriendDistance(WorldObject const* pOther) const;
        void RewardPlayerAndGroupAtKill(Unit* victim, bool isBattleGround);
        void RewardPlayerAndGroupAtEvent(uint32 creature_id, WorldObject* pRewardSource, bool overrideId = true);
        ...
        void RewardHonorCreatureKill(Creature* victim, int32 honor);
        ...
        uint8 GetTalentPointsInTab(uint8 Tab) const;
        bool IsHealer();
        bool IsTank();
        ...
        bool CanTitanGripCurrently() const;
        bool IsTitanGripTwoHandWeapon(uint32 weaponSubtype) const;
        ...
        void CastItemCombatSpell(Unit* target, WeaponAttackType attType, SpellInfo const * procSpell, uint32 procAttacker, uint32 procVictim, uint32 procEx);
        void CastItemUseSpell(Item* item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex);
        void CastItemCombatSpell(Unit* target, WeaponAttackType attType, SpellInfo const * procSpell, uint32 procAttacker, uint32 procVictim, uint32 procEx, Item* item, ItemTemplate const* proto);
        ...
        void SendDirectMessage(const WorldPacket& data) const;
        ...
        void JoinArenaSoloQueue(bool rated);
        ...
        bool CanUseOpenWorldPvPObject(GameObject* gameobject);
        bool isTotalImmune();
        ...
        VisibilityGroupSet visibleGroups;
        ...
        bool AttendsWorldBossFight() const;
        /*********************************************************/
        /***                 INSTANCE SYSTEM                   ***/
        /*********************************************************/
        ...
        InstancePlayerBind* GetBoundInstance(uint32 mapid, Difficulty difficulty, bool checkLFGState = true);
        ...
        void ConvertRuneByAuraEffect(uint8 index, RuneType newType, AuraEffect const* aura);
        void RemoveRunesByAuraEffect(AuraEffect const* aura);
        void RestoreBaseRune(uint8 index);
        std::pair<uint8, bool> GetFirstRuneWithCD(RuneType rune) const;
        void ActivateRune(uint8 index, bool notify = true);
        ...
        void RemoveAchievement(uint32 achievementId);
        ...
        float GetAverageItemLevel(bool includingQuality = false);
        float GetMaximumItemLevel();
        float GetPvPItemLevel();
        ...
        void SetLastMovementPacketTime(uint32 time) { m_lastMovementPacketTime = time; }
        uint32 GetLastMovementPacketTime() const { return m_lastMovementPacketTime; }
        ...
        enum PlayerCustomFlags
        {
            ...
            XP_NORMAL                   =   0x00001000,
            CROSSFACTION_DISABLED       =   0x00002000,

            // 0x00004000 - reuse

        };

        std::shared_ptr<Player> GetSharedPtr() { return shared_from_this(); }
        std::weak_ptr<Player> GetWeakPtr() { return std::weak_ptr<Player>(shared_from_this()); }

        bool HasCustomFlag(uint32 const flag) const { return m_customFlags & flag; }
        void SetCustomFlag(uint32 const flag) { m_customFlags |= flag; }
        void RemoveCustomFlag(uint32 const flag)  { m_customFlags &= ~flag; }
        AntiCheat* GetAntiCheat() const { return m_anticheat; }

        void UpdatePersonalArenaRating();

        //RG Custom "Verstecktes Anmelden"
        bool IsSecretQueuingActive() { return GetMapId() == 35 || (InArena() && GetBGEntryPoint().GetMapId() == 35); }

        //RG Custom ArenaReplay
    ...
    protected:
        ...
        */
        ...
        void _LoadAccountComplaints(PreparedQueryResult result);
        ...
        void _SaveHonorAndArenaPoints(SQLTransaction& trans);
        void _SaveMaxLevel();
        ...
        VisibilityGroup zoneVisibilityGroup;
        ...
        float  m_summon_y;
        float  m_summon_z;
        uint32 m_summon_count; // ignore summon responses in HandleSummonResponseOpcode except last summon (double port lets client automaticly send false for first port)
    ...
    private:
        ...
        //Helpers for CanEquipItem()
        uint8 FindIgnorableSlotForUniqueItem(Item* pItem, uint8 equipSlot, bool swap) const;
        InventoryResult CanEquipItemsCurrently(Item* pItem) const;
        InventoryResult CanFindEquipSlot(Item* itemToEquip, uint32 preferredEquipSlot, bool swap, uint8& targetSlot, bool not_loading) const;
        InventoryResult CanEquipQuiver(Item* itemToEquip, uint8 preferredSlot, uint8* slots) const;

        float AquireExpertise(WeaponAttackType attType) const override;
        void EvaluateAttackerTable(combat::AttackCheckTable& checks, const combat::AttackerParams& attacker) const override final;
        void EvaluateVictimTable(combat::AttackCheckTable& checks, const combat::VictimParams& victim) const override final;


        std::unique_ptr<UpdateData> accumulatedUpdate;
        ...
        uint32 m_lastMovementPacketTime;
        ...
        float m_fishingSkillAccurate; // See Player::UpdateFishingSkill()

//#######################################################
//## Pet Cache
//#######################################################
public:
    const PetCache& GetPetCache() const { return _petCache; }
    PetCache& GetPetCache() { return _petCache; }
...
//#######################################################
public:
    const player::complaint::ComplaintMgr& GetComplaintMgr() const;
    player::complaint::ComplaintMgr& GetComplaintMgr();

private:
    player::complaint::ComplaintMgr complaintMgr;

public:
    static void LockCharacter(ObjectGuid guid) { lockedCharacters_.insert(guid); }
    static void UnlockCharacter(ObjectGuid guid) { lockedCharacters_.erase(guid); }
    static bool IsLocked(ObjectGuid guid) { return lockedCharacters_.find(guid) != lockedCharacters_.end(); };
private:
    static std::unordered_set<ObjectGuid> lockedCharacters_;
};
