...
void SocialMgr::GetFriendInfo(Player* player, ObjectGuid friendGUID, FriendInfo &friendInfo)
{
    ...
    if (!player->GetSession()->HasPermission(rbac::RBAC_PERM_WHO_SEE_ALL_SEC_LEVELS) &&
        target->GetSession()->GetSecurity() > sWorld->getIntConfig(CONFIG_GM_LEVEL_IN_WHO_LIST))
        return;
    ...
    if (!player->IsGameMaster() && target->IsSecretQueuingActive())
        return;
    ...
    if (target->IsVisibleGloballyFor(player))
    {
        ...
        //! RG Custom - show old position at friendlist for arena except of gms
        if (target->InArena() && !player->IsGameMaster())
        {
            WorldLocation loc = target->GetBGEntryPoint(); 
            friendInfo.Area = sMapMgr->GetAreaId(loc.GetMapId(), loc.GetPositionX(), loc.GetPositionY(), loc.GetPositionZ());
        }
...
WorldPacket SocialMgr::MakeFriendStatusPacket(FriendsResult result, ObjectGuid guid)
{
...
void SocialMgr::SendFriendStatus(Player* player, FriendsResult result, ObjectGuid friendGuid, bool broadcast)
{
    ...
    WorldPacket data = MakeFriendStatusPacket(result, friendGuid);
...
void SocialMgr::BroadcastToFriendListers(Player* player, const WorldPacket& packet)
{
    if (!player)
        return;
    ...
    for (SocialMap::const_iterator itr = m_socialMap.begin(); itr != m_socialMap.end(); ++itr)
    {
        ...
        if (itr2 != itr->second.m_playerSocialMap.end() && (itr2->second.Flags & SOCIAL_FLAG_FRIEND))
        {
            ...
            //RG Custom "Verstecktes Anmelden"
            if (!target->IsGameMaster() && player->IsSecretQueuingActive() && !player->IsBeingTeleportedFar())
                return;
