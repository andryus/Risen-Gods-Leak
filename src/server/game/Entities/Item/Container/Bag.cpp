...
void Bag::BuildUpdateBlockForPlayer(UpdateData& data, Player* target) const
{
    Item::BuildUpdateBlockForPlayer(data, target);

    for (uint32 i = 0; i < GetBagSize(); ++i)
        if (m_bagslot[i])
            m_bagslot[i]->BuildUpdateBlockForPlayer(data, target);
}
