...
void Item::UpdateDuration(Player* owner, uint32 diff)
{
    ...
    if (GetUInt32Value(ITEM_FIELD_DURATION) <= diff)
    {
        ...
        RG_LOG<ItemLogModule>(this, RG::Logging::ItemLogType::DESTROYED_BY_TIME_EXPIRE);
...
bool Item::LoadFromDB(uint32 guid, ObjectGuid owner_guid, Field* fields, uint32 entry)
{
    ...
    // temp: no bind removal on gems or recipes to restrict trade of FM vendor items
    if (IsSoulBound() && proto->Bonding == NO_BIND
        && !(proto->Class == ITEM_CLASS_GEM || (proto->Class == ITEM_CLASS_CONSUMABLE && proto->SubClass == ITEM_SUBCLASS_ITEM_ENHANCEMENT)))
    {
...
Item* Item::CreateItem(uint32 itemEntry, uint32 count, Player const* player)
{
    ...
    if (proto)
    {
        ...
        const uint32 guid = sObjectMgr->GenerateLowGuid(HighGuid::Item);
...
void Item::BroadcastValuesUpdate()
{
    if (Player* owner = GetOwner())
    {
...
void Item::ScheduleObjectUpdate()
{
    if (auto owner = GetOwner())
        if (auto map = owner->FindMap())
            map->AddToUpdateObjectList(this);
}

void Item::AbortObjectUpdate() 
{
    if (auto owner = GetOwner())
        if (auto map = owner->FindMap())
            map->RemoveFromUpdateObjectList(this);
}

void Item::SetBinding(bool val)
{
    if (val == HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_SOULBOUND))
        return;

    ApplyModFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_SOULBOUND, val);

    // log bind on equip/use items
    if (val
        && GetTemplate()->Bonding != NO_BIND
        && GetTemplate()->Bonding != BIND_WHEN_PICKED_UP)
        RG_LOG<ItemLogModule>(this, RG::Logging::ItemLogType::BOUND);
}
