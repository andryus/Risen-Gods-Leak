...
DynamicObject* DynamicObject::CreateAreaSpell(Unit& caster, uint32 spellId, Position const& pos, float radius)
{
    DynamicObject* dynObj = new DynamicObject;
    bool const successful = dynObj->CreateDynamicObject(
        sObjectMgr->GenerateLowGuid(HighGuid::DynamicObject),
        &caster,
        spellId,
        pos,
        radius,
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.dynamicobject", "Failed to create dynamic object for area spell (Caster: %s, Spell: %u).", caster.GetGUID().ToString().c_str(), spellId);
        delete dynObj;
        return nullptr;
    }
...
DynamicObject* DynamicObject::CreateFarsightFocus(Unit& caster, uint32 spellId, Position const& pos, float radius)
{
    DynamicObject* dynObj = new DynamicObject;
    bool const successful = dynObj->CreateDynamicObject(
        sObjectMgr->GenerateLowGuid(HighGuid::DynamicObject),
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.dynamicobject", "Failed to create farsight focus (Caster: %s, Spell: %u).", caster.GetGUID().ToString().c_str(), spellId);
        delete dynObj;
        return nullptr;
    }
...
bool DynamicObject::CreateDynamicObject(uint32 guidlow, Unit* caster, uint32 spellId, Position const& pos, float radius, DynamicObjectType type)
{
    ...
    SetUInt32Value(DYNAMICOBJECT_CASTTIME, game::time::GetGameTimeMS());
