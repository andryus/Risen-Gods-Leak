...
void Vehicle::ApplyAllImmunities()
{
    ...
    // Player vehicles are not immune to knock back
    ...
    switch (GetVehicleInfo()->m_ID)
    {
        ...
        case 336: // Salvaged Siege Engine
            _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_SCHOOL_ABSORB, false); // Shieldgenerator
        case 335: // Salvaged Chopper
        case 338: // Salvaged Demolisher
            _me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, false); // Battering Ram
            _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, false); // Battering Ram
            break;
        case 222: // Hyldsmeet Warbear
            _me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, false); // Battering Ram
            break;
        case 257: // Lithe Stalker
            _me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, false); 
            _me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK_DEST, false);
            break;
        case 158: // Strand of the Ancients
            _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_RESISTANCE_EXCLUSIVE, true);
            _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_RANGED_ATTACK_POWER_PCT, true);
            _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_CRIT_DAMAGE_BONUS, true);
            _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_CRIT_PCT, true);
            _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_SPELL_CRIT_CHANCE, true);
            break;
        default:
            break;
    }
...
void Vehicle::RemoveAllPassengers()
{
    ...
    /// This will properly "reset" the pending join process for the passenger.
    {
        ...
        while (!_pendingJoinEvents.empty())
        {
            ...
            e->ScheduleAbort();
            e->TargetGuid = eventVehicle ? eventVehicle->GetBase()->GetGUID() : ObjectGuid::Empty;
...
Vehicle* Vehicle::RemovePassenger(Unit* unit)
{
    ...
    // Passenger should be attackable again
    // was VEHICLE_SEAT_FLAG_PASSENGER_NOT_ATTACKABLE
    if (seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_SHOULD_USE_VEH_SEAT_EXIT_ANIM_ON_FORCED_EXIT)
    ...
    if (unit->IsFlying() && !unit->ToCreature())
...
void Vehicle::RelocatePassengers()
{
    ...
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
    {
        if (Unit* passenger = ObjectAccessor::GetUnit(*GetBase(), itr->second.Passenger.Guid))
        {
            ...
            passenger->SendMovementFlagUpdate();
        }
    }
...
{
    if (Unit* TargetUnit = sObjectAccessor->GetUnit(*Passenger, TargetGuid))
        if (Vehicle* Target = TargetUnit->GetVehicleKit())
...
{
    Unit* TargetUnit = sObjectAccessor->GetUnit(*Passenger, TargetGuid);
    if (!TargetUnit)
        return true;

    Vehicle* Target = Target = TargetUnit->GetVehicleKit();
...
void VehicleJoinEvent::Abort(uint64)
{
    ...
    if (TargetUnit)
    {
        if (Vehicle* Target = Target = TargetUnit->GetVehicleKit())
        {
        ...
        TC_LOG_ERROR("entities.vehicle", "VehicleJoinEvent::Abort vehicle not found VehicleGuid: %s, GuidLow: %u, Entry: %u cancelled",
            TargetGuid.ToString().c_str(), Passenger->GetGUID().GetCounter(), Passenger->GetEntry());
