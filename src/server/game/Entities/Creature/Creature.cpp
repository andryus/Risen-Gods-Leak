...
#include "ScriptMgr.h"
#include "DBCStores.h"
#include "Combat/AttackTableHandler.h"
...
m_respawnDelay(300), m_corpseDelay(60), m_respawnradius(0.0f), m_combatPulseTime(1), m_combatPulseDelay(1), m_reactState(REACT_AGGRESSIVE), deathDelayTimer(0),
m_defaultMovementType(IDLE_MOTION_TYPE), spawnId(0), m_equipmentId(0), m_originalEquipmentId(0), m_AlreadyCallAssistance(false),
m_AlreadySearchedAssistance(false), m_regenHealth(true), m_AI_locked(false), m_meleeDamageSchoolMask(SPELL_SCHOOL_MASK_NORMAL),
m_originalEntry(0), m_homePosition(), m_transportHomePosition(), m_creatureInfo(NULL), m_creatureData(NULL), m_waypointID(0), m_path_id(0), m_formation(NULL), m_respawnAtHome(false),
_mmap_disabled(false), evadeTimer(0), m_inhabitType(InhabitTypeValues::INHABIT_GROUND)
{
...
Creature* Creature::Create(uint32 entry, Map* map, Position const& pos, uint32 phaseMask)
{
    Creature* creature = new Creature();
    bool const successful = creature->Create(
        sObjectMgr->GenerateLowGuid(HighGuid::Unit),
        map,
        phaseMask,
        entry,
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.creature", "Failed to create creature (Entry: %u).", entry);
        delete creature;
        return nullptr;
    }
...
Creature* Creature::CreateVehicle(uint32 entry, Map* map, Position const& pos, uint32 phaseMask, uint32 vehId)
{
    Creature* vehicle = new Creature();
    bool const successful = vehicle->Create(
        sObjectMgr->GenerateLowGuid(HighGuid::Vehicle),
        map,
        phaseMask,
        entry,
        pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation(),
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.creature", "Failed to create vehicle (Entry: %u).", entry);
        delete vehicle;
        return nullptr;
    }
...
Creature* Creature::LoadFromDB(uint32 spawnId, Map* map, bool addToMap)
{
    Creature* creature = new Creature();
    bool const successful = creature->LoadCreatureFromDB(spawnId, map, addToMap);

    if (!successful)
    {
        TC_LOG_ERROR("entities.unit", "Failed to load creature (SpawnId: %u).", spawnId);
        delete creature;
        return nullptr;
    }
...
void Creature::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        Unit::AddToWorld();
        SearchFormation();
        InitScript(m_creatureInfo->ScriptName);
        AIM_Initialize();
        if (IsVehicle())
            GetVehicleKit()->Install();

        if (IsVendor() && GetSpawnId())
            if (VendorItemCounts const* itemCounts = sObjectMgr->GetCreatureVendorItemCounts(GetSpawnId()))
                m_vendorItemCounts = *itemCounts;

        //Arena RV totem workaround (will be removed asap)
        if (IsTotem() && GetMap()->GetId() == 618)
        {
            Unit* summoner = ToTempSummon()->GetSummoner();
            if (summoner && summoner->GetTypeId() == TYPEID_PLAYER)
            {
                if (Battleground* bg = summoner->ToPlayer()->GetBattleground())
                {
                    bg->DoAction(1, GetGUID());
                }
            }
        }

        FindAttackableTargetOnSpawnAndRespawn();
    }
}
...
bool Creature::InitEntry(uint32 entry, CreatureData const* data /*= nullptr*/)
{
    ...
    ForbidInhabit(INHABIT_ANYWHERE);
    AllowInhabit(InhabitTypeValues(cinfo->InhabitType));
...
bool Creature::UpdateEntry(uint32 entry, CreatureData const* data /*= nullptr*/)
{
    ...
    if (cInfo->flags_extra & CREATURE_FLAG_EXTRA_IMMUNITY_TURN_EVIL)
        ApplySpellImmune(0, IMMUNITY_ID, 10326, true); // Turn Evil

    SetCustomFlag(UnitCustomFlagMask(cInfo->customFlags));

    UpdateMovementFlags();
    LoadCreaturesAddon();

    if (m_inhabitType & INHABIT_WATER && HasCustomFlag(UnitCustomFlag::WaterGroundTransitAnimation))
...
void Creature::Update(uint32 diff)
{
    ...
    uint32 diff_size[CREATURE_DIFF_VALUE_COUNT] = {0};
    uint32 diffTime = 0;
    uint32 startTime = 0;
    if (lagReportActive)
        startTime = getMSTime();
    ...
    if (deathDelayTimer)
    {
        if (deathDelayTimer <= diff)
        {
            deathDelayTimer = 0;
            SetHealth(0);
        }
        else
            deathDelayTimer -= diff;
    }

    if (lagReportActive)
        diffTime = getMSTime();

    *this <<= Fire::CheckThreat;
    ...
    switch (m_deathState)
    {
        ...
        case DEAD:
        {
            ...
            if (m_respawnTime <= now)
            {
                ...
                ObjectGuid dbtableHighGuid(HighGuid::Unit, GetEntry(), spawnId);
            ...
            if (lagReportActive)
            {
                diff_size[CREATURE_DIFF_CASE_UPDATE] = GetMSTimeDiffToNow(diffTime);
                diff_size[CREATURE_DIFF_UNIT_UPDATE] = 0;
                diff_size[CREATURE_DIFF_AI_UPDATE] = 0;
            }
        ...
        case CORPSE:
        {
            ...
            if (lagReportActive)
            {
                diff_size[CREATURE_DIFF_UNIT_UPDATE] = GetMSTimeDiffToNow(diffTime);
                diffTime = getMSTime();
            }
        ...
        case ALIVE:
        {
            ...
            if (m_combatPulseDelay > 0 && IsEngaged() && GetMap()->IsDungeon() && GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_BOSS_ENCOUNTER)
            {
            ...
            if (IsInCombat() && !IsInEvadeMode() && !CanReachTarget())
            {
                evadeTimer += diff;
                if (evadeTimer >= 13 * IN_MILLISECONDS)
                {
                    if (GetMap()->Instanceable())
                    {
                        if (Unit* victim = GetVictim())
                        {
                            TC_LOG_WARN("entities.unit.evade", "Creature %s teleported to victim %s", GetGUID().ToString().c_str(), victim->GetGUID().ToString().c_str());
                            NearTeleportTo(*victim);
                        }
                        else
                        {
                            TC_LOG_WARN("entities.unit.evade", "Creature %s tried to port to victim but no victim found", GetGUID().ToString().c_str());
                ...
                evadeTimer = diff / 2 < evadeTimer ? evadeTimer - diff / 2 : 0;
            ...
            if (lagReportActive)
            {
                diff_size[CREATURE_DIFF_AI_UPDATE] = GetMSTimeDiffToNow(diffTime);
                diffTime = getMSTime();
            }
            ...
            bool bInCombat = IsInCombat() && (!GetVictim() ||                                        // if IsInCombat() is true and this has no victim
                             ...
                             !EnsureVictim()->GetCharmerOrOwnerPlayerOrPlayerItself()->IsGameMaster() ||
                             !EnsureVictim()->GetCharmerOrOwnerPlayerOrPlayerItself()->isDeveloper()); // or the victim/owner/charmer is not a GameMaster
            ...
            if (lagReportActive)
                diff_size[CREATURE_DIFF_CASE_UPDATE] = uint32(GetMSTimeDiffToNow(diffTime));
    ...
    if (lagReportActive &&
        GetMSTimeDiffToNow(startTime) > sWorld->getIntConfig(CONFIG_LAG_REPORT_MAX_DIFF_CREATURE))
        sLog->outMessage("rg.lagreport", LOG_LEVEL_WARN, "[Creature Lag Report] GUID: %u (%u) SUM: %u MOVEMENT_FLAGS: %u STATE_DEPENDENT: %u UNIT_UPDATE: %u AI_UPDATE: %u",
            GetGUID().GetCounter(),
            GetEntry(),
            GetMSTimeDiffToNow(startTime),
            diff_size[CREATURE_DIFF_FLAG_UPDATE],
            diff_size[CREATURE_DIFF_CASE_UPDATE],
            diff_size[CREATURE_DIFF_UNIT_UPDATE],
            diff_size[CREATURE_DIFF_AI_UPDATE]);
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius >0)
    {
        ...
        Creature* creature = VisitSingleNearbyObject<Creature>(radius, u_check);
        ...
        if (!creature)
            ...
            /// @todo use 31365
            GetMotionMaster()->MoveFleeing(GetVictim(), sWorld->getIntConfig(CONFIG_CREATURE_FAMILY_FLEE_DELAY), false);
        else
...
bool Creature::Create(uint32 guidlow, Map* map, uint32 phaseMask, uint32 entry, float x, float y, float z, float ang, CreatureData const* data /*= nullptr*/, uint32 vehId /*= 0*/)
{
    ...
    _mmap_disabled = DisableMgr::IsDisabledFor(DisableType::DISABLE_TYPE_MMAP, entry, this, MMAP_DISABLE_FOR_CREATURE);

    return true;
}

void Creature::AllowInhabit(InhabitTypeValues inhabitType)
{
    if ((m_inhabitType & inhabitType) == inhabitType)
        return;

    m_inhabitType = InhabitTypeValues(m_inhabitType | inhabitType);

    // play swim animation in water
    if (inhabitType & INHABIT_WATER && HasCustomFlag(UnitCustomFlag::WaterGroundTransitAnimation))
        SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_UNK_15);

    UpdateMovementFlags();
}

void Creature::ForbidInhabit(InhabitTypeValues inhabitType)
{
    if ((m_inhabitType & inhabitType) == 0)
        return;
...
Player* Creature::GetLootRecipient() const
{
    if (!m_lootRecipient)
        return nullptr;
    return ObjectAccessor::GetPlayer(*this, m_lootRecipient);
}
...
bool Creature::LoadCreatureFromDB(uint32 spawnId, Map* map, bool addToMap)
{
    ...
    // check if there is already a creature in map with our spawn id
    if (map->GetInstanceId() == 0 && map->GetCreature(ObjectGuid(HighGuid::Unit, data->id, spawnId)))
        return false;

    uint32 const lowGuid = (map->GetInstanceId() == 0) ? spawnId : sObjectMgr->GenerateLowGuid(HighGuid::Unit);

    if (!Create(lowGuid, map, data->phaseMask, data->id, data->posX, data->posY, data->posZ, data->orientation, data))
        return false;
    ...
    m_respawnTime  = GetMap()->GetCreatureRespawnTime(spawnId);
...
void Creature::DeleteFromDB()
{
    ...
    GetMap()->RemoveCreatureRespawnTime(spawnId);
...
bool Creature::OnInitScript(std::string_view scriptName)
{
    if (scriptName.empty())
    {
        if (m_creatureInfo->type == CreatureType::CREATURE_TYPE_TOTEM)
            scriptName = "totem_base";
    }

    return loadScriptType(*this, scriptName);
}

float Creature::AquireExpertise(WeaponAttackType attType) const
{
    return GetTotalAuraModifier(SPELL_AURA_MOD_EXPERTISE);
}

void Creature::OnEnterCombat(Unit& enemy)
{
    AI()->EnterCombat(&enemy);
}

void Creature::OnDeath(Unit& killer)
{
    if (IsAIEnabled)
        AI()->JustDied(&killer);
}

void Creature::OnKill(Unit& victim)
{
    if (IsAIEnabled) // there should be no use in checking this, outside of backwards-compatibility => should always have empty creatureScript
...
{
    if (IsAIEnabled)
        AI()->EnterEvadeMode((CreatureAI::EvadeReason)reason);
...
float Creature::GetAttackDistance(Unit const* player) const
{
	...
	bool creatureinstance = player->GetInstanceId() != 0;
    ...
    // Aggro Radius in Instances Rate.Creature.Instance.Aggro in worldserver.conf
    if (creatureinstance)
    {
        float aggroInstanceRate = sWorld->getRate(RATE_CREATURE_INSTANCE_AGGRO);
        return (RetDistance*aggroInstanceRate);
    }
...
void Creature::Respawn(bool force)
{
    ...
    if (getDeathState() == DEAD)
    {
        ...
        TC_LOG_DEBUG("entities.unit", "Respawning creature %s (%s Entry: %u)",
            GetName().c_str(), GetGUID().ToString().c_str(), GetEntry());
        m_respawnTime = 0;
...
using FlagExtraEntry = std::pair<combat::AttackTableEntry, CreatureFlagsExtra>;

void applyCreatureFlagExtra(const Creature& creature, combat::AttackCheckTable& table, span<FlagExtraEntry> entries)
{
    for (const auto [entry, flag] : entries)
        table[entry] &= !creature.GetCreatureTemplate()->HasFlagExtra(flag);
}

void Creature::EvaluateAttackerTable(combat::AttackCheckTable& checks, const combat::AttackerParams& attacker) const
{
    constexpr FlagExtraEntry flagsToCheck[] =
    {
        { combat::AttackTableEntry::Crit, CREATURE_FLAG_EXTRA_NO_CRIT },
        { combat::AttackTableEntry::Crushing, CREATURE_FLAG_EXTRA_NO_CRUSH },
    };

    applyCreatureFlagExtra(*this, checks, flagsToCheck);

    checks[combat::AttackTableEntry::Glancing] &= IsPet() || IsGuardian();
}

void Creature::EvaluateVictimTable(combat::AttackCheckTable& checks, const combat::VictimParams& victim) const
{
    constexpr FlagExtraEntry flagsToCheck[] =
    {
        { combat::AttackTableEntry::Parry, CREATURE_FLAG_EXTRA_NO_PARRY },
        { combat::AttackTableEntry::Block, CREATURE_FLAG_EXTRA_NO_BLOCK },
    };

    applyCreatureFlagExtra(*this, checks, flagsToCheck);

    checks[combat::AttackTableEntry::Glancing] &= !IsPet();
}
...
bool Creature::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
    ...
    switch (spellInfo->Effects[index].Effect)
    {
        case SPELL_EFFECT_HEAL:
            if (GetCreatureTemplate()->type == CREATURE_TYPE_MECHANICAL && GetEntry() != 34183)
                return true;
            break;
...
Unit* Creature::SelectNearestTarget(float dist, bool playerOnly /* = false */) const
{
...

    {
        ...
        target = VisitSingleNearbyObject<Unit>(dist, u_check);
    }
...
void Creature::CallAssistance()
{
    if (!m_AlreadyCallAssistance && GetVictim() && !IsPet() && !IsCharmed())
    {
        ...
        if (radius > 0)
        {
            AssistDelayEvent* e = nullptr;

            const auto addAssistant = [this, &e](Creature* creature)
            {
                if(!e)
                    e = new AssistDelayEvent(EnsureVictim()->GetGUID(), *this);

                e->AddAssistant(creature->GetGUID());
            };

            {
                Trinity::AnyAssistCreatureInRangeCheck u_check(this, GetVictim(), radius);

                VisitAnyNearbyObject<Creature, Trinity::FunctionAction>(radius, addAssistant, u_check);
            }
...
void Creature::CallForHelp(float radius)
{
    ...
    VisitAnyNearbyObject<Creature, Trinity::FunctionAction>(radius, u_do);
}
...
bool Creature::_IsTargetAcceptable(const Unit* target) const
{
    ...
    if (Unit* owner = GetOwner(); owner && owner->IsEngagedBy(target))
        return true;
...
{
    if (IsSummon() || !spawnId || (m_creatureData && !m_creatureData->dbData))
        return;

    GetMap()->SaveCreatureRespawnTime(spawnId, m_respawnTime);
}
...
CreatureAddon const* Creature::GetCreatureAddon(bool useCreatureTemplateEntry /* = true */) const
{
    if (spawnId)
    {
    ...
    return sObjectMgr->GetCreatureTemplateAddon(useCreatureTemplateEntry ? GetCreatureTemplate()->Entry : GetEntry());
}

bool Creature::CanReachTarget() const
{
    if (GetMotionMaster()->GetCurrentMovementGeneratorType() == CHASE_MOTION_TYPE)
        return GetMotionMaster()->top()->CanReachTarget();
    return true;
}

bool Creature::IsEvadingAttacks() const
{
    return !CanReachTarget() && evadeTimer >= 1000 && !IsControlledByPlayer() && !GetMap()->Instanceable();
}

void Creature::ResetOnReachedHome()
{
    evadeTimer = 0;
}
...
void Creature::ClearInCombatWithZone()
{
    Map* map = GetMap();

    if (!map->IsDungeon())
    {
        TC_LOG_ERROR("entities.unit", "Creature entry %u call ClearInCombatWithZone for map (id: %u) that isn't an instance.", GetEntry(), map->GetId());
        return;
    }
    ...
    Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, 150.0f);

    VisitAnyNearbyObject<Creature, Trinity::ContainerAction>(150.0f, creatureList, u_check);
    ...
    for (std::list<Creature*>::const_iterator itr = creatureList.begin(); itr != creatureList.end(); ++itr)
    {
        ...
        if (c->IsAlive() && c->IsInCombat())
        {
            ...
            c->OnEvade(CreatureAI::EVADE_REASON_NO_HOSTILES);
        }
    }
}
...
void Creature::GetRespawnPosition(float &x, float &y, float &z, float* ori, float* dist) const
{
    if (RespawnAtHome())
    {
        x = GetHomePosition().GetPositionX();
        y = GetHomePosition().GetPositionY();
        z = GetHomePosition().GetPositionZ();

        if (ori)
            *ori = GetHomePosition().GetOrientation();
        if (dist)
            *dist = 0;
...
void Creature::UpdateCreatureEntry(uint32 entry)
{
    SetEntry(entry);

    UpdateEntry(entry, m_creatureData);
}
...
uint32 Creature::GetVendorItemCurrentCount(VendorItem const* vItem)
{
    ...
    if (time_t(vCount->lastIncrementTime + vItem->incrtime) <= ptime)
        if (ItemTemplate const* pProto = sObjectMgr->GetItemTemplate(vItem->item))
        {
            ...
            {
                UpdateVendorItemCurrentCountInDB(false, vCount->itemId, vCount->count, vCount->lastIncrementTime);
            ...
            UpdateVendorItemCurrentCountInDB(true, vCount->itemId, vCount->count, vCount->lastIncrementTime);
        }
...
uint32 Creature::UpdateVendorItemCurrentCount(VendorItem const* vItem, uint32 used_count)
{
    ...
    if (itr == m_vendorItemCounts.end())
    {
        ...
        m_vendorItemCounts.push_back(VendorItemCount(vItem->item, new_count, time(NULL)));

        UpdateVendorItemCurrentCountInDB(true, vItem->item, new_count, time(NULL));
    ...
    UpdateVendorItemCurrentCountInDB(true, vItem->item, vCount->count, vCount->lastIncrementTime);

    return vCount->count;
}

void Creature::UpdateVendorItemCurrentCountInDB(bool insert, uint32 itemId, uint32 count, uint32 lastIncrementTime) const
{
    if (insert)
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CREATURE_VENDOR_ITEM_COUNT);
        stmt->setInt32(0, GetSpawnId());
        stmt->setInt32(1, itemId);
        stmt->setInt32(2, count);
        stmt->setInt32(3, lastIncrementTime);
        CharacterDatabase.Execute(stmt);
    }
    else
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CREATURE_VENDOR_ITEM_COUNT);
        stmt->setInt32(0, GetSpawnId());
        stmt->setInt32(1, itemId);
        CharacterDatabase.Execute(stmt);
    }
...
std::string_view Creature::GetNameForLocaleIdx(LocaleConstant loc_idx) const
{
...
Unit* Creature::SelectNearestHostileUnitInAggroRange(bool useLOS) const
{
    ...
    return VisitSingleNearbyObject<Unit>(MAX_AGGRO_RADIUS, u_check);
}
...
void Creature::UpdateMovementFlags()
{
    ...
    if (IsInhabitting(INHABIT_AIR) && isInAir && !IsFalling())
    {
        if (IsInhabitting(INHABIT_GROUND))
            SetCanFly(true);

        SetDisableGravity(true);
    }
    else if (IsInhabitting(INHABIT_WATER) && IsInWater())
    {
        SetCanFly(false);
        SetSwim(true);
    }
    else
    {
        SetCanFly(false);
        SetSwim(false);
        SetDisableGravity(HasUnitMovementFlag(MOVEMENTFLAG_HOVER));
    }
...
/// @todo Please unhack me!
void Creature::FindAttackableTargetOnSpawnAndRespawn()
{
    if (GetMap()->GetEntry()->IsContinent())
    {
        if (!IsInCombat() && !IsTrigger() && IsAIEnabled && !IsVehicle())
        {
            if (!HasUnitState(UNIT_STATE_CANNOT_AUTOATTACK | UNIT_STATE_DIED | UNIT_STATE_LOST_CONTROL | UNIT_STATE_EVADE
                | UNIT_STATE_FLEEING | UNIT_STATE_FLEEING_MOVE | UNIT_STATE_FOLLOW | UNIT_STATE_FOLLOW_MOVE
                | UNIT_STATE_NOT_MOVE | UNIT_STATE_POSSESSED | UNIT_STATE_ROOT | UNIT_STATE_STUNNED))
            {
                if (Unit* target = SelectNearbyTarget(NULL, 45.0f))
                {
                    if (target->GetTypeId() == TYPEID_UNIT && !target->ToPlayer() && !target->ToPet() && !target->IsGuardian() && !target->HasUnitState(UNIT_STATE_DIED | UNIT_STATE_EVADE | UNIT_STATE_UNATTACKABLE) && !target->IsVehicle())
                    {
                        float aggroRadius = 15; // guestimate, it's probably more

                                                // Aggro Radius varies with level difference at a rate of roughly 1 yard/level
                        int32 levelDiff = int32(target->getLevel()) - int32(getLevel());
                        aggroRadius -= (float)levelDiff;
                        ...
                        if (GetDistance(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ()) <= aggroRadius) // is our target in aggro range?
                            if (target->IsHostileTo(this))
                                if (CanCreatureAttack(target, false))
                                    AI()->MoveInLineOfSight_Safe(target);
                    }
                }
            }
        }
