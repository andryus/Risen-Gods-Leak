...
bool FormationMgr::AddCreatureToGroup(uint32 groupId, Creature* member)
{
    ...
    if (itr != map->CreatureGroupHolder.end())
    {
        ...
        return itr->second->AddMember(member);
    }
    //Create new group
    else
    {
...
bool CreatureGroup::AddMember(Creature* member)
{
    ...
    auto itr = sFormationMgr->CreatureGroupMap.find(member->GetSpawnId());
    if (itr == sFormationMgr->CreatureGroupMap.end())
    {
        TC_LOG_ERROR("entities.unit", "Unit GUID: %u failed to be added to formation group (has no leader).", member->GetGUID().GetCounter());
...
void CreatureGroup::LeaderMoveTo(float x, float y, float z)
{
    ...
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ...
        if (member == m_leader || !member->IsAlive() || member->GetVictim() || member->IsInEvadeMode())
            continue;

        if (itr->second->point_1)
            if (m_leader->GetCurrentWaypointID() == static_cast<uint32>(itr->second->point_1 - 1)
                || m_leader->GetCurrentWaypointID() == static_cast<uint32>(itr->second->point_2 - 1))
