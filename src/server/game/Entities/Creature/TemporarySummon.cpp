...
TempSummon* TempSummon::Create(SummonPropertiesEntry const* properties, Unit* owner, uint32 entry, Map* map, Position const& pos, uint32 phaseMask, CreatureData const* data, uint32 vehId)
{
    TempSummon* summon = new TempSummon(properties, owner);
    bool const successful = summon->Creature::Create(
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.tempsummon", "Failed to create temp summon (Entry: %u).", entry);
        delete summon;
        return nullptr;
    }
...
void TempSummon::Update(uint32 diff)
{
    ...
    switch (m_type)
    {
        ...
        case TEMPSUMMON_TIMED_DESPAWN:
        {
            if (m_timer <= diff)
            {
                if (m_Properties && m_Properties->Type == SUMMON_TYPE_GUARDIAN)
                {
                    KillSelf();
                    UnSummon(10 * IN_MILLISECONDS);
                    m_timer = 20 * IN_MILLISECONDS;
                    return;
                }
                UnSummon();
                return;
...
void TempSummon::InitSummon()
{
    ...
    if (owner)
    {
        *owner <<= Fire::SummonCreature(GetEntry()).Bind(this);
...
void TempSummon::UnSummon(uint32 msTime)
{
    ...
    static_cast<Creature*>(this) <<= Fire::UnSummon;
...
Minion* Minion::Create(SummonPropertiesEntry const* properties, Unit* owner, uint32 entry, Map* map, Position const& pos, uint32 phaseMask, CreatureData const* data, uint32 vehId)
{
    Minion* minion = new Minion(properties, owner);
    bool const successful = minion->Creature::Create(
        sObjectMgr->GenerateLowGuid(HighGuid::Unit),
        map,
        phaseMask,
        entry,
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.minion", "Failed to create minion (Entry: %u).", entry);
        delete minion;
        return nullptr;
    }
...
Guardian* Guardian::Create(SummonPropertiesEntry const* properties, Unit* owner, uint32 entry, Map* map, Position const& pos, uint32 phaseMask, CreatureData const* data, uint32 vehId)
{
    Guardian* guardian = new Guardian(properties, owner);
    bool const successful = guardian->Creature::Create(
        sObjectMgr->GenerateLowGuid(HighGuid::Unit),
        map,
        phaseMask,
        entry,
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.guardian", "Failed to create guardian (Entry: %u).", entry);
        delete guardian;
        return nullptr;
    }
...
Puppet* Puppet::Create(SummonPropertiesEntry const* properties, Unit* owner, uint32 entry, Map* map, Position const& pos, uint32 phaseMask, CreatureData const* data, uint32 vehId)
{
    Puppet* puppet = new Puppet(properties, owner);
    bool const successful = puppet->Creature::Create(
        sObjectMgr->GenerateLowGuid(HighGuid::Unit),
        map,
        phaseMask,
        entry,
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.puppet", "Failed to create puppet (Entry: %u).", entry);
        delete puppet;
        return nullptr;
    }
