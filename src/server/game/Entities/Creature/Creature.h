...
enum CreatureFlagsExtra
{
    ...
    CREATURE_FLAG_EXTRA_CIVILIAN           = 0x00000002,       // creature does not aggro (ignore faction/reputation hostility)
    CREATURE_FLAG_EXTRA_NO_PARRY           = 0x00000004,       // creature can't parry
    CREATURE_FLAG_EXTRA_NO_PARRY_HASTEN    = 0x00000008,       // creature can't counter-attack at parry
    CREATURE_FLAG_EXTRA_NO_BLOCK           = 0x00000010,       // creature can't block
    CREATURE_FLAG_EXTRA_NO_CRUSH           = 0x00000020,       // creature can't do crush attacks
    CREATURE_FLAG_EXTRA_NO_XP_AT_KILL      = 0x00000040,       // creature kill not provide XP
    CREATURE_FLAG_EXTRA_TRIGGER            = 0x00000080,       // creature is trigger
    ...
    CREATURE_FLAG_EXTRA_IMMUNITY_TURN_EVIL = 0x00000800,       // creature is immunue to 'Turn Evil' effects
    CREATURE_FLAG_EXTRA_WORLDEVENT         = 0x00004000,       // custom flag for world event creatures (left room for merging)
    CREATURE_FLAG_EXTRA_GUARD              = 0x00008000,       // creature is guard
    CREATURE_FLAG_EXTRA_BOSS_ENCOUNTER     = 0x00010000,       // creature is instanceboss
    CREATURE_FLAG_EXTRA_NO_CRIT            = 0x00020000,       // creature can't do critical strikes
    CREATURE_FLAG_EXTRA_NO_SKILLGAIN       = 0x00040000,       // creature won't increase weapon skills
    CREATURE_FLAG_EXTRA_TAUNT_DIMINISH     = 0x00080000,       // Taunt is a subject to diminishing returns on this creautre
    CREATURE_FLAG_EXTRA_ALL_DIMINISH       = 0x00100000,       // creature is subject to all diminishing returns as player are
    CREATURE_FLAG_EXTRA_DUNGEON_BOSS       = 0x10000000,       // creature is a dungeon boss (SET DYNAMICALLY, DO NOT ADD IN DB)
    CREATURE_FLAG_EXTRA_DAMAGE_LIKE_PLAYER = 0x20000000,       // creature damage is dealed like player damage
    CREATURE_FLAG_EXTRA_IGNORE_PATHFINDING = 0x40000000,       // creature ignore pathfinding
    CREATURE_FLAG_EXTRA_IMMUNITY_KNOCKBACK = 0x80000000        // creature is immune to knockback effects
};
...
#define CREATURE_FLAG_EXTRA_DB_ALLOWED (CREATURE_FLAG_EXTRA_INSTANCE_BIND | CREATURE_FLAG_EXTRA_CIVILIAN | \
    ...
    CREATURE_FLAG_EXTRA_GUARD | CREATURE_FLAG_EXTRA_DAMAGE_LIKE_PLAYER | CREATURE_FLAG_EXTRA_IGNORE_PATHFINDING | \
    CREATURE_FLAG_EXTRA_NO_PULL_BY_GRIP | CREATURE_FLAG_EXTRA_IMMUNITY_KNOCKBACK | CREATURE_FLAG_EXTRA_NO_CREATURE_DAMAGE | \
    CREATURE_FLAG_EXTRA_IMMUNITY_TURN_EVIL)
...
struct CreatureTemplate
{
    ...
    bool HasFlagExtra(uint32 flagExtra) const { return (flags_extra & flagExtra) != 0; }
};
...
{
    explicit VendorItemCount(uint32 _item, uint32 _count, time_t _lastIncrementTime)
...
typedef std::map<uint64, VendorItemCounts> CreatureVendorItemCountContainer;
...
#define CREATURE_DEATH_DELAY 100
...
class GAME_API Creature : public Unit, public Grids::GridObject<Creature>
{
    public:
        static Creature* Create(uint32 entry, Map* map, Position const& pos, uint32 phaseMask);
        static Creature* CreateVehicle(uint32 entry, Map* map, Position const& pos, uint32 phaseMask, uint32 vehId);
        static Creature* LoadFromDB(uint32 spawnId, Map* map, bool addToMap = true);
    ...
    public:
        ...
        bool HasFlagExtra(uint32 flag) const { return GetCreatureTemplate()->flags_extra & flag; }
        bool IsDungeonBoss() const { return (GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_DUNGEON_BOSS) != 0; }

        bool CanWalk() const { return IsInhabitting(INHABIT_GROUND); }
        bool CanSwim() const { return IsInhabitting(INHABIT_WATER) || IsPet(); }
        bool CanFly()  const override { return IsInhabitting(INHABIT_AIR); }

        void AllowInhabit(InhabitTypeValues inhabitType);
        void ForbidInhabit(InhabitTypeValues inhabitType);
        bool IsInhabitting(InhabitTypeValues inhabitType) const { return (static_cast<uint32>(m_inhabitType) & static_cast<uint32>(inhabitType)) != 0; }
        ...
        void UpdateVendorItemCurrentCountInDB(bool insert /*false is delete*/, uint32 itemId, uint32 count, uint32 lastIncrementTime = 0) const;
        ...
        /**
        * \brief get creature_addon for creatures spawned via db
        *        get creature_template_addon for tempsummons
        * \param useCreatureTemplateEntry try to find creature_template_addon for "original entry". meaning not to search creature_template_addon for the difficulty_entry_x
        */
        CreatureAddon const* GetCreatureAddon(bool useCreatureTemplateEntry = true) const;

        std::string GetAIName() const;
        std::string GetScriptName() const;
        uint32 GetScriptId() const;
        void UpdateCreatureEntry(uint32 entry);

        // override WorldObject function for proper name localization
        std::string_view GetNameForLocaleIdx(LocaleConstant locale_idx) const;
        ...
        void SetRespawnAtHome(bool val) { m_respawnAtHome = val; }
        bool RespawnAtHome() const { return m_respawnAtHome; }
        ...
        void FindAttackableTargetOnSpawnAndRespawn();


        // script inteface functions
        void OnEnterCombat(Unit& initiator);
    ...
    protected:
        ...
        uint32 evadeTimer;                                  // (msec) how long the creature cannot reach its target
        ...
        uint32 spawnId;                               ///< For new or temporary creatures is 0 for saved it is lowguid
        uint8 m_equipmentId;
        ...
        void CallEvent(typename Event::params params)
        {
            return CallEvent(*this, params);
        }
    ...
    private:
        ...
        G3D::Point3 lastMovementStateUpdatePos;
        bool _mmap_disabled;
