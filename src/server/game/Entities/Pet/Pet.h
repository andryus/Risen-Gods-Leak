...
{
    friend class PetCache;
    public:
		static Pet* Create(Player& owner, PetType type, Map* map, uint32 phaseMask, uint32 entry, uint32 petNumber);
		static Pet* CreateFromCreature(Player& owner, PetType type, Creature& creature);
		static Pet* CreateFromCreatureInfo(Player& owner, PetType type, CreatureTemplate const* cinfo);
        static Pet* Load(Player& owner, PetCache::Entry const& petCacheEntry);
		static Pet* Load(Player& owner, uint32 petnumber, bool current, uint32 petentry);
    ...
    public:
        ...
        void SetDoneSpellCrit();
        bool DoneSpellCrit() { return m_spellCrit; }
        void SetDoneSpellProc();
        bool DoneSpellProc() { return m_spellProc; }
        ...
        */
        ...
        bool CanHaveAura(uint32 auraId) const;
        ...
        void _LoadSpellCooldowns(PetCache::Entry const* petData);
        void _LoadAuras(PetCache::Entry const* petData, uint32 timediff);
        void _SaveAuras(SQLTransaction& trans);
        void _LoadSpells(PetCache::Entry const* petData);
    ...
    protected:
        ...
        uint32  m_recastStatAuraTimer;
        uint64  m_auraRaidUpdateMask;
        bool    m_loading;
        uint32  m_regenTimer;
        uint32  m_spellCritTimer;
        bool    m_spellCrit;
        uint32  m_spellProcTimer;
        bool    m_spellProc;
