...
struct GameObjectTemplate
{
    ...
    uint32 GetMinGold() const
    {
        switch (type)
        {
        case GAMEOBJECT_TYPE_CHEST:       return chest.minGold;
        default: return 0;
        }
    }

    uint32 GetMaxGold() const
    {
        switch (type)
        {
        case GAMEOBJECT_TYPE_CHEST:       return chest.maxGold;
        default: return 0;
        }
    }
...
class GAME_API GameObject : public WorldObject, public Grids::GridObject<GameObject>
{
    public:
        static GameObject* Create(uint32 entry, Map& map, uint32 phaseMask, Position const& pos, G3D::Quat const& rotation, uint32 animprogress, GOState go_state = GO_STATE_READY, uint32 lowGuid = 0);
        static GameObject* LoadFromDB(uint32 spawnId, Map& map, bool addToMap = true);
    ...
    public:
        ...
        std::string_view GetNameForLocaleIdx(LocaleConstant locale_idx) const override;
        ...
        /*
         * @name: DespawnOrUnsummon
         * @auth: Erimioa
         * @brief: despawn GO after specific time
         * GOs can't be "unsummon" but this function have so the same name as the creature function for a back detection
         * @parm: despawnTimeInMS in uint32, standard 0, is the time between the call of function an the despawn
        */
        void DespawnOrUnsummon(uint32 despawnTimeInMS = 0)
        {
            m_calledForDespawnOrUnsummon = true;
            m_despawnOrUnsummonTime = despawnTimeInMS;
        };
        ...
        bool UseDoorOrButton(uint32 time_to_restore = 0, bool alternative = false, Unit* user = NULL);
        ...
        std::pair<float, bool> GetModelCollisionHeight(float localX, float localY, float localZ) const;
        void UpdateModelPosition();

        bool UsedByPlayer(Player* player, bool reportUsed);
    ...
    protected:
        ...
        uint32      m_despawnOrUnsummonTime;                // (secs) if 0 and m_calledForDespawnOrUnsummon is true, go will despawn
        bool        m_calledForDespawnOrUnsummon;        
        LootState   m_lootState;
        ObjectGuid  m_lootStateUnitGUID;                    // GUID of the unit passed with SetLootState(LootState, Unit*)
        bool        m_spawnedByDefault;
        time_t      m_cooldownTime;                         // used as internal reaction delay time store (not state change reaction).
        TimeTrackerSmall m_trapTimer;                       // used to calculate trap activation times (seconds are not accurate enought)
        ...
        uint32 spawnId;                               ///< For new or temporary gameobjects is 0 for saved it is lowguid
    ...
    private:
        ...
        bool _IsWithinDist(WorldObject const* obj, float dist2compare, bool /*is3D*/) const override
        {
            ...
            return IsInRange(obj->GetPositionX(), obj->GetPositionY(), obj->GetPositionZ(), dist2compare + obj->GetObjectSize());
        }
        GameObjectAI* m_AI;
};
#endif
