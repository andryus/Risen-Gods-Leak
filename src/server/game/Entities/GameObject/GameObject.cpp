...
constexpr float INTERACTION_DISTANCE_GO = 5.2f;

GameObject* GameObject::Create(uint32 entry, Map& map, uint32 phaseMask, Position const& pos, G3D::Quat const& rotation,
    uint32 animprogress, GOState go_state, uint32 lowGuid)
{
    bool const isStaticTransport = sObjectMgr->IsGameObjectStaticTransport(entry);

    GameObject* obj = isStaticTransport ? new StaticTransport() : new GameObject();
    bool const successful = obj->Create(
        lowGuid != 0 ? lowGuid : sObjectMgr->GenerateLowGuid(HighGuid::Gameobject),
        entry,
        &map,
        phaseMask,
    ...
    if (!successful)
    {
        TC_LOG_ERROR("entities.gameobject", "Failed to create gameobject (Entry: %u).", entry);
        delete obj;
        return nullptr;
    }
...
GameObject* GameObject::LoadFromDB(uint32 spawnId, Map& map, bool addToMap)
{
    GameObjectData const* data = sObjectMgr->GetGOData(spawnId);
    bool const isStaticTransport = data ? sObjectMgr->IsGameObjectStaticTransport(data->id) : false;

    GameObject* obj = isStaticTransport ? new StaticTransport() : new GameObject();
    bool const successful = obj->LoadGameObjectFromDB(spawnId, &map, addToMap);

    if (!successful)
    {
        TC_LOG_ERROR("entities.gameobject", "Failed to load gameobject (SpawnId: %u).", spawnId);
        delete obj;
        return nullptr;
    }
...
GameObject::GameObject() : WorldObject(), m_model(nullptr), m_goValue(), m_AI(nullptr), m_trapTimer(0)
{
    ...
    m_despawnOrUnsummonTime = 0;
    m_calledForDespawnOrUnsummon = false;
...
void GameObject::RemoveFromOwner()
{
    ...
    if (Unit* owner = ObjectAccessor::GetUnit(*this, ownerGUID))
    {
        ...
        if (GetOwnerGUID())
        {
            TC_LOG_FATAL("misc", "GameObject::RemoveFromOwner(): FATAL! After remove GO, always set an owner to this [%s]", GetOwnerGUID().ToString().c_str());
...
void GameObject::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        InitScript(m_goInfo->ScriptName);
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, G3D::Quat const& rotation, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    HighGuid high = HighGuid::Gameobject;

    if (goinfo->type == GAMEOBJECT_TYPE_TRANSPORT)
    {
        m_updateFlag = (m_updateFlag | UPDATEFLAG_TRANSPORT) & ~UPDATEFLAG_POSITION;
        high = HighGuid::Transport;
    }

    Object::_Create(guidlow, goinfo->entry, high);
    ...
    if (!DisableMgr::IsDisabledFor(DISABLE_TYPE_GO_LOS, goinfo->entry, NULL))
        m_model = CreateModel();
    else
        m_model = nullptr;
...
void GameObject::Update(uint32 diff)
{
    *this <<= Fire::ProcessEvents.Bind(DiffParam(diff));
    ...
    switch (m_lootState)
    {
            ...
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    if (goInfo->trap.type == 2)
                        ...
                        m_trapTimer.Reset(10s);
                    else if (Unit* owner = GetOwner())
                        m_trapTimer.Reset(Seconds(goInfo->trap.startDelay));

                    SetLootState(GO_READY);
                    break;
                }
                case GAMEOBJECT_TYPE_TRANSPORT:
                    break; // handled in StaticTransport::Update
                case GAMEOBJECT_TYPE_FISHINGNODE:
                {
                    ...
                    if (time(NULL) > m_respawnTime - FISHING_BOBBER_READY_TIME)
                    {
                        ...
                        {
                            if (Player* owner = caster->To<Player>())
                            {
                                SetGoState(GO_STATE_ACTIVE);
                                SetUInt32Value(GAMEOBJECT_FLAGS, GO_FLAG_NODESPAWN);

                                WorldPacket packet = CreateValuesUpdateBlockForPlayer(owner);
                                owner->SendDirectMessage(std::move(packet));
        ...
        case GO_READY:
        {
            if (m_calledForDespawnOrUnsummon)
            {
                if (m_despawnOrUnsummonTime <= diff)
                {
                    m_lootState = GO_JUST_DEACTIVATED;
                    m_calledForDespawnOrUnsummon = false;

                    //we doesn't want a instant respawn
                    return;
                }
                else m_despawnOrUnsummonTime -= diff;
            }
            ...
            if (m_respawnTime > 0)                          // timer on
            {
                ...
                {
                    ObjectGuid dbtableHighGuid(HighGuid::Gameobject, GetEntry(), spawnId);
                    ...
                    switch (GetGoType())
                    {
                        case GAMEOBJECT_TYPE_FISHINGNODE:   //  can't fish now
                        {
                            ...
                            if (caster && caster->GetTypeId() == TYPEID_PLAYER)
                            {
                                ...
                                caster->ToPlayer()->SendDirectMessage(std::move(data));
                            }
                            // can be delete
                            m_lootState = GO_JUST_DEACTIVATED;
                            return;
            ...
            if (isSpawned())
            {
                ...
                {
                    if (!m_trapTimer.Passed())
                    {
                        m_trapTimer.Update(diff);
                        break;
                    }
                    ...
                    if (Unit* owner = GetOwner())
                    {
                        ...
                        Trinity::NearestTrapTargetInObjectRangeCheck checker(this, owner, radius);
                        target = VisitSingleNearbyObject<Unit>(radius, checker);
                    }
                    else
                    {
                        // Environmental trap: Any player
                        Player* player = FindNearestPlayer(radius);
                        target = player;
                    }

                    // Toy Train Set
                    if (goInfo->entry == 193963)
                        target = GetOwner();
        ...
        case GO_ACTIVATED:
        {
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_DOOR:
                case GAMEOBJECT_TYPE_BUTTON:
                    if (m_cooldownTime && game::time::GetGameTimeMS() >= m_cooldownTime)
                        ResetDoorOrButton();
                    break;
                case GAMEOBJECT_TYPE_GOOBER:
                    if (game::time::GetGameTimeMS() >= m_cooldownTime)
                    {
                ...
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    else if (Unit* target = ObjectAccessor::GetUnit(*this, m_lootStateUnitGUID))
                    {
                        ...
                        {
                            if (Unit* owner = GetOwner())
                                if (!owner->_IsValidAttackTarget(target, spellInfo))
                                    return;

                            uint32 _spellId = goInfo->trap.spellId;
                            sScriptMgr->OnGameObjectTriggerTrap(this, target, _spellId);
                            CastSpell(target, _spellId);
                        }

                        // Template value or 4 seconds
                        m_trapTimer.Reset(goInfo->trap.cooldown ? Seconds(goInfo->trap.cooldown) : 4s);
...
bool GameObject::LoadGameObjectFromDB(uint32 spawnId, Map* map, bool addToMap)
{
    ...
    uint32 const lowGuid = (map->GetInstanceId() == 0) ? spawnId : sObjectMgr->GenerateLowGuid(HighGuid::Gameobject);

    if (!Create(lowGuid, entry, map, phaseMask, x, y, z, ang, data->rotation, animprogress, go_state, artKit))
        return false;
    ...
    if (data->spawntimesecs >= 0)
    {
        ...
        else
        {
            ...
            m_respawnTime = GetMap()->GetGORespawnTime(spawnId);

            // ready to respawn
            if (m_respawnTime && m_respawnTime <= time(NULL))
            {
                m_respawnTime = 0;
                GetMap()->RemoveGORespawnTime(spawnId);
            }
        }
    }
    else
...
void GameObject::SaveRespawnTime()
{
    if (m_goData && m_goData->dbData && m_respawnTime > time(NULL) && m_spawnedByDefault)
        GetMap()->SaveGORespawnTime(spawnId, m_respawnTime);
}
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    // found correct GO
    if (GameObject* trapGO = VisitSingleNearbyObject<GameObject>(range, go_check))
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    return VisitSingleNearbyObject<GameObject, true>(range, u_check);
}
...
bool GameObject::UseDoorOrButton(uint32 time_to_restore, bool alternative /* = false */, Unit* user /*=nullptr*/)
{
    ...
    m_cooldownTime = time_to_restore ? (game::time::GetGameTimeMS() + time_to_restore) : 0;
...
void GameObject::SetGoArtKit(uint8 kit)
{
    ...
    GameObjectData* data = const_cast<GameObjectData*>(sObjectMgr->GetGOData(spawnId));
    if (data)
        data->artKit = kit;
}
...
void GameObject::Use(Unit* user)
{
    ...
    if (Player* playerUser = user->ToPlayer())
    {
        ...
        if (UsedByPlayer(playerUser, false))
            return;
    }
    ...
    if (uint32 cooldown = GetGOInfo()->GetCooldown())
    {
        ...
        m_cooldownTime = game::time::GetGameTimeMS() + cooldown * IN_MILLISECONDS;
    }
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_TRAP:                          //6
        {
            ...
            m_trapTimer.Reset(goInfo->trap.cooldown ? Seconds(goInfo->trap.cooldown) : 4s); // template or 4 seconds
        ...
        case GAMEOBJECT_TYPE_CHAIR:                         //7
        {
            ...
            if (found_free_slot)
            {
                ...
                if (itr != ChairListSlots.end())
                {
                    ...
                    if (Player* player = user->To<Player>())
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            m_cooldownTime = game::time::GetGameTimeMS() + info->GetAutoCloseTime();
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    // There is a chance for skill increase regardless of what is catched
                    player->UpdateFishingSkill();

                    /// @todo find reasonable value for fishing hole search
                    if (GameObject* go = LookupFishingHoleAround(20.0f + CONTACT_DISTANCE))
                    {
                        go->Use(player); // There is no junk in fishing holes
                        SetLootState(GO_JUST_DEACTIVATED);
                        break;
                    }
        ...
        case GAMEOBJECT_TYPE_SUMMONING_RITUAL:              //18
        {
            ...
            if (m_ritualOwnerGUID)
                m_ritualOwner = ObjectAccessor::GetPlayer(*GetMap(), m_ritualOwnerGUID);
        ...
        case GAMEOBJECT_TYPE_SPELLCASTER:                   //22
        {
            ...
            {
                if (GetGOInfo()->flags & GO_FLAG_IS_PORTAL)
                {
                    Unit* caster = GetOwner();
                    if (!caster || caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                ...
                else
                {
                    ...
                    if (Group* group = user->ToPlayer()->GetGroup())
                        if (!group->IsMember(GetOwnerGUID()))
                            return;
                }                
            }
            ...
            //        if (spellInfo->Attributes & SPELL_ATTR0_NOT_SHAPESHIFT && user->HasAuraType(SPELL_AURA_MOD_SHAPESHIFT))
            //            return;
        ...
        default:
            if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
                TC_LOG_ERROR("misc", "GameObject::Use(): unit (type: %hhu, guid: %u, name: %s) tries to use object (guid: %u, entry: %u, name: %s) of unknown type (%u)",
...
std::string_view GameObject::GetNameForLocaleIdx(LocaleConstant loc_idx) const
{
...
void GameObject::BuildValuesUpdate(ObjectUpdateType updateType, ByteBuffer& data, Player* target) const
{
    bool forcedFlags = GetGoType() == GAMEOBJECT_TYPE_CHEST && GetGOInfo()->chest.groupLootRules && HasLootRecipient();
    bool targetIsGM = target && target->IsGameMaster();

    const size_t startPos = data.wpos();
    UpdateMask updateMask(m_valuesCount, 0);
    data << updateMask;

    uint32* flags = GameObjectUpdateFieldFlags;
    uint32 visibleFlag = UF_FLAG_PUBLIC;
    if (target && GetOwnerGUID() == target->GetGUID())
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
            ...
            (index == GAMEOBJECT_FLAGS && forcedFlags))
        {
            ...
            if (index == GAMEOBJECT_DYNAMIC)
            {
                ...
                {
                    case GAMEOBJECT_TYPE_QUESTGIVER:
                        if (target && ActivateToQuest(target))
            ...
            else if (index == GAMEOBJECT_FLAGS)
            {
                ...
                if (GetGoType() == GAMEOBJECT_TYPE_CHEST)
                    if (target && GetGOInfo()->chest.groupLootRules && !IsLootAllowedFor(target))
...
float GameObject::GetInteractionDistance()
{
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_FISHINGHOLE:
        case GAMEOBJECT_TYPE_FISHINGNODE:
            return 20.0f + 1.f + CONTACT_DISTANCE; // max spell range (20 + 0.5) + extra 1m to avoid bugs
            // (custom) fix problems with big GOs, like Hodir's Helm
        case GAMEOBJECT_TYPE_QUESTGIVER:
            return INTERACTION_DISTANCE + GetFloatValue(OBJECT_FIELD_SCALE_X) + (GetGOInfo()->flags & GO_FLAG_HUGE_SIZE ? 40.0f : 0.0f); 
        case GAMEOBJECT_TYPE_CHEST:
            return INTERACTION_DISTANCE + (GetGOInfo()->flags & GO_FLAG_HUGE_SIZE ? 20.0f : 0.0f);
        default:
            return INTERACTION_DISTANCE_GO;
    }
}

std::pair<float, bool> GameObject::GetModelCollisionHeight(float localX, float localY, float localZ) const
{
    if (m_model)
    {
        const G3D::Vector3 dir = G3D::Vector3(0, 0, -1);
        const G3D::Ray ray(G3D::Point3(localX, localY, localZ + 2.f), dir);
        float maxDist = DEFAULT_HEIGHT_SEARCH;
        if (m_model->intersectRay(ray, maxDist, true, GetPhaseMask()))
            return std::make_pair(ray.origin().z - maxDist, true);
    }
    return std::make_pair(localZ, false);
}
...
bool GameObject::UsedByPlayer(Player* player, bool reportUsed)
{
    *this <<= Fire::UsedByPlayer.Bind(player);

    if (GameObjectAI* ai = AI())
        return ai->GossipHello(player, reportUsed);
...
bool GameObject::OnInitScript(std::string_view scriptName)
{
