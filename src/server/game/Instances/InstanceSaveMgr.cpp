...
InstanceSave* InstanceSaveManager::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, time_t resetTime, bool canReset, bool load)
{
    ...
    difficulty = GetSaveDifficultyForMap(difficulty, mapId);
...
Difficulty InstanceSaveManager::GetSaveDifficultyForMap(Difficulty difficulty, uint32 mapId)
{
...
void InstanceSaveManager::DeleteInstanceFromDB(uint32 instanceid)
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_INSTANCE_ENCOUNTERS);
    stmt->setUInt32(0, instanceid);
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_INSTANCE_EVENTS);
    stmt->setUInt32(0, instanceid);
    trans->Append(stmt);
...
void InstanceSave::SaveToDB()
{
    ...
    if (InstanceMap* map = static_cast<InstanceMap*>(sMapMgr->FindMap(GetMapId(), m_instanceid)))
    {
...
Difficulty InstanceSave::GetDifficultyFor(Player* player) const
{
    if (auto mapEntry = GetMapEntry())
        ...
        switch (m_difficulty)
        {
            ...
            case RAID_DIFFICULTY_25MAN_NORMAL:
                if (auto group = player->GetGroup())
                    return group->GetDifficulty(true);
                return player->GetDifficulty(true);
            default: break;
        }
    }
