...
#include "Unit.h"               //@todo remove this heavy include
...
enum SpellCustomAttributes
{
    ...
    SPELL_ATTR0_CU_SAME_STACK                    = 0x00040000,
    SPELL_ATTR0_CU_BINARY                        = 0x00080000,
    SPELL_ATTR0_CU_ALLOW_INFLIGHT_TARGET         = 0x00200000,
    SPELL_ATTR0_CU_NEEDS_AMMO_DATA               = 0x00400000,
    SPELL_ATTR0_CU_ALWAYS_BREAK_STEALTH          = 0x00800000,
    SPELL_ATTR0_CU_FORCE_REFLECT                 = 0x01000000,
	SPELL_ATTR0_CU_KEEP_SPELLMOD_WHILE_CASTING   = 0x02000000,
    SPELL_ATTR0_CU_USABLE_FOR_PLAYER_AI          = 0x04000000,
...
public:
    ...
    bool IsTargetingArea() const;
    bool EffectIsTargetingArea(uint32 eff) const;
    ...
    bool TriggeredEffectNeedsLosChecks(uint32 eff) const;
    ...
    //return the value of BasePoints of 'effect' +1, also the virtual number
    int32 GetBasePoints(SpellEffIndex effect) const;
    ...
    bool IsAffectedBySpellDelay(Unit* caster, Unit* target = NULL) const;
    bool IsAffectedByBinaryResistance(Unit* caster, Unit* target) const;
    bool IsBinarySpell() const;

    // Infos for the procc system
    // Really ugly, @todo find better solution
    void GetProccFlags(uint32& procAttacker, uint32& procVictim, uint32& procEx, TriggerCastFlags triggeredCastFlags = TRIGGERED_FULL_MASK,
                       WeaponAttackType attackType = BASE_ATTACK, Unit* caster = NULL, Unit* originalCaster = NULL, 
                       int32 damage = 0, int32 healing = 0) const;
    void GetBaseProccFlags(uint32& procAttacker, uint32& procVictim, uint32& procEx, TriggerCastFlags triggeredCastFlags = TRIGGERED_FULL_MASK,
                           WeaponAttackType attackType = BASE_ATTACK, Unit* caster = NULL, Unit* originalCaster = NULL) const;
    void GetPosititvityProccFlags(uint32& procAttacker, uint32& procVicitim, int32 damage = 0, int32 healing = 0) const;
