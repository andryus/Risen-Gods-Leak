...
class GAME_API AuraEffect
{
    ...
    friend Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount, Item* castItem, ObjectGuid casterGUID, bool softRefresh);
    friend Aura::~Aura();
    private:
        ~AuraEffect();
    ...
    public:
        ...
        void Refresh(bool resetTimer, bool softRefresh);
        void RecieveSnapShot(const PeriodicEffectSnapShot* recievedSnapShot);
        const PeriodicEffectSnapShot* GetSnapShot() { return snapShot; }
        void CalculatePeriodic(Unit* caster, bool resetTimer = true, bool load = false);
    ...
    private:
        ...
        PeriodicEffectSnapShot* snapShot;
    ...
    public:
        ...
        void HandleModCastingSpeedNotStack(AuraApplication const* aurApp, uint8 mode, bool apply) const;
...
class UnitGUIDComparator
{
public:
    bool operator()(const Unit* x, const Unit* y) const { return (x->GetGUID() < y->GetGUID()); }
};

// Modifies a (Periodic)AuraEffect by storing the current casters stats and applying them on the effect by
// recalling them later to determine the periodic results (Damage of Dot-,Hotticks etc.) of the AuraEffect
// instead of always recalculating all values
// This delievers the calculated damage (like from Unit::SpellDamage(Healing)BonusDone)
// and the critchance for periodic ticks
class PeriodicEffectSnapShot
{
private:
    AuraEffect* baseEffect;
    // we need to know that to decide how we calculate the "baseDamage" (/heal)
    bool isHealEffect;

    // only important for effects caused by players
    int32 playerSpellModFlatModifier = 0;
    float playerSpellModPercentageModifier = 1.0f;
    
    // all these values do not only depend on the caster
    // e.g some classes do more dmg vs undeads
    struct TargetDependetValues
    {
        uint32 baseDamage; // or heal in case of heal spells
        float percentageModifier;
        float critChance;
    };

    // One AuraEffect can technically have multiple targets
    // (although no dot/hot with multiple targets comes to my mind... is there any?)
    typedef std::map<Unit*, TargetDependetValues,UnitGUIDComparator> TargetDependentValuesMap;
    TargetDependentValuesMap targetMap;
    void calculateAllForTarget(Unit* target, TargetDependetValues& targetValues, Unit* caster);
    void recalculateBaseDamageForTarget(Unit* target, TargetDependetValues& targetValues, Unit* caster);
    void calculatePlayerSpellModValues(Unit* caster);
    Unit::FlatAndPercentModifiers directFlatAndPercentModifiers(Unit* target, Unit* caster);
public: // all accept null values in each parameter, except constructors 
    uint32 GetDamage(Unit* target);
    float GetCritChance(Unit* target);
    TargetDependentValuesMap GetTargetValuesMap() const;
    TargetDependetValues GetExampleTargetValues() const;
    void AddTargetDependentValues(Unit* target, Unit* caster);
    void RemoveTargetDependentValues(Unit* target);
    void Refresh(Unit* caster, bool softRefresh);
    int32 GetSpellModFlatModifier() const { return playerSpellModFlatModifier; }
    float GetSpellModPercentageModifier() const { return playerSpellModPercentageModifier; }
    explicit PeriodicEffectSnapShot(AuraEffect* baseEffect, Unit* caster);
    explicit PeriodicEffectSnapShot(AuraEffect* baseEffect, const PeriodicEffectSnapShot* snapShotToCopy);
