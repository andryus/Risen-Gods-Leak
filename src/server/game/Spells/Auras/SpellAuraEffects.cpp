...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         // 54 SPELL_AURA_MOD_HIT_CHANCE implemented in Unit::CreateAttackTable
    ...
    &AuraEffect::HandleModCastingSpeedNotStack,                   // 65 SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK
...
m_spellmod(NULL), snapShot(NULL), m_periodicTimer(0), m_tickNumber(0), m_effIndex(effIndex),
m_canBeRecalculated(true), m_isPeriodic(false)
{
    CalculatePeriodic(caster, true, false);
    m_amount = CalculateAmount(caster);
    if (UsesSnapShot())
        snapShot = new PeriodicEffectSnapShot(this, caster);
    CalculateSpellMod();
}
...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_MANA_SHIELD:
        {
            if (caster)
            {
                if (SpellBonusEntry const* bonus = sSpellMgr->GetSpellBonusData(GetSpellInfo()->Id))
                {
                    float coeff = bonus->direct_damage;
                    float directBonus = 0.f;

                    // Priest - Power Word: Shield
                    if (GetSpellInfo()->SpellIconID == 566)
                        // Borrowed Time
                        if (AuraEffect const* borrowedTime = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 2899, EFFECT_1))
                            AddPct(coeff, borrowedTime->GetAmount());

                    if (GetSpellInfo()->Mechanic == MECHANIC_SHIELD)
                        directBonus = caster->SpellBaseHealingBonusDone(GetSpellInfo()->GetSchoolMask()) * coeff;
                    else
                        directBonus = caster->SpellBaseDamageBonusDone(GetSpellInfo()->GetSchoolMask()) * coeff;

                    directBonus = caster->ApplyEffectModifiers(GetSpellInfo(), GetEffIndex(), directBonus);

                    directBonus *= caster->CalculateLevelPenalty(GetSpellInfo());

                    amount += static_cast<int32>(directBonus);
                    amount *= caster->GetTotalAuraMultiplier(SPELL_AURA_MOD_HEALING_DONE_PERCENT);

                    // RG CUSTOM: Arena heal reduce over time
                    const uint32 HEAL_REDUCE_SPELL = 38770;
                    if (auto healReduceAura = caster->GetAura(HEAL_REDUCE_SPELL))
                        AddPct(amount, healReduceAura->GetEffect(EFFECT_1)->CalculateAmount(caster));
                }
            }
        ...
        case SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED:
            switch (GetId())
            {
                case 68769: // Little Ivory Raptor
                case 68768: // Little White Stallion
                {
                    uint16 ridingSkill = GetBase()->GetUnitOwner()->ToPlayer()->GetBaseSkillValue(SKILL_RIDING);
                    if (ridingSkill > 75)
                        amount = 100;
                    else if (ridingSkill > 0)
                        amount = 60;
                    break;
                }
            }
    ...
    // Mixology effect increase
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_POTION && caster && caster->HasAura(53042))
    {
        ...
        {
            if(caster->HasSpell(m_spellInfo->Effects[0].TriggerSpell)) // Has to have the elixir learned
            {
                switch (m_spellInfo->Id)
                {
                    case 7178: // (Elixir of) Water Breathing (Rank 1)
                    case 11319: // (Elixir of) Water Walking
                    case 11389: // (Elixir of) Detect Undead - No spell trigger anyway
                    case 11403: // (Elixir of) Dream Vision (Summon) - No spell trigger anyway
                    case 11407: // (Elixir of) Detect Demon - No spell trigger anyway
                    case 22807: // (Elixir of) Greater Water Breathing // Potion of Water Breathing
                    case 67016: //  >
                    case 67017: //   |-> Flask of the North - No spell trigger anyway
                    case 67018: //  >
                        // No effect
                        break;
                    case 6512: // (Elixir of) Detect Lesser Invisibility
                    case 11371: // Gift of Arthas (Rank 1)
                    case 12608: // Stealth Detection (Catseye Elixir)
                    case 28489: // (Elixir of) Camouflage
                    case 28496: // (Elixir of) Greater Stealth Detection
                    case 28514: // (Elixir of) Empowerment - only one with negativ effect basepoints
                        // Unknown - Better safe than sorry
                        break;
                    case 11348: // (Elixir of Superior Defense) Greater Armor
                        amount += 140; // Armor (WoWHead)
                        break;
                    case 11405: // Elixir of the Gients
                    ...
                    case 53764: // (Elixir of Mighty Mageblood) Mighty Mana Regeneration
                    ...
                    case 17629: // (Flask of) Chromatic Resistance
                        ...
                        break;
                    // - Classic Elixirs
                    case 673: // (Elixir of) Minor Defense // Armor
                    case 2378: // (Elixir of) Minor Fortitude // HP
                        amount += 15;
                        break;
                    case 2367: // (Elixir of) Lion's Strength // STR
                    case 2374: // (Elixir of) Minor Agility // AGI
                    case 3219: // Weak Troll's Blood (Elixir) // HP/5
                        amount += 2;
                        break;
                    ...
                    case 11390: // Arcane Elixir
                        amount += 6; // INT + CR /* CHECKME */
                        break;
                    case 3160: // (Elixir of) Lesser Agility // AGI
                    case 3166: // (Elixir of) Wisdom // INT
                    case 8212: // (Elixir of Giant Growth) Enlarge // STR + Size - Not sure about the size
                        amount += 5;
                        break;
                    ...
                    case 17627: // (Flask of) Distilled Wisdom
                        ...
                        break;
                    // - TBC Elixirs
                    case 17539: // Greater Arcane Elixir // SP + CR
                    case 28491: // Healing Power // SP + SPI - Incorrect actually +9/+8
                    case 39627: // Elixir of Draenic Wisdom (diff in 4.1) // INT + SPI
                        amount += 8;
                        break;
                    ...
                    case 53746: // Wrath Elixir
                        amount += 40; // AP
                        break;
                    // - Individual effects scaling different
                    ...
                    case 60340: // (Elixir of) Accuracy - CHECKME
                    case 60341: // (Elixir of) Deadly Strikes
                    case 60344: // (Elixir of) Expertise
                    case 60346: // (Elixir of) Lightning Speed - CHECKME
                    case 60347: // (Elixir of) Mighty Thoughts (diff in 4.1)
                    case 60345: // (Elixir of) Armor Piercing - CHECKME
                        amount += 20;
                        break;
                    default:
                        TC_LOG_ERROR("spells", "SpellAuraEffect::CalculateAmount: unhandled mixology effect for spell %u", m_spellInfo->Id);
                        break;
                }
            }
        }
...
/**
* @name Refresh
* @brief refreshs the effect by recalculating the time between periodic ticks
* and refreshing the snapShot of the casters values (if there is one)
* @param resetTimer if true, the period timer will be set back to 0, which
* means it takes again the full period time (amplitude) until the next tick happens
* @param softRefresh if true, the PeriodicEffectSnapShot of this effect will only get softRefreshed
* meaning not all values will be recalculated for dot- or hot-effects
*/
void AuraEffect::Refresh(bool resetTimer, bool softRefresh)
{
    Unit* caster = GetCaster();
    // should always be called because it calls some scripthandler even if not periodic
    CalculatePeriodic(caster, resetTimer, false);
    if (snapShot)
        snapShot->Refresh(caster, softRefresh);
}


void AuraEffect::CalculatePeriodic(Unit* caster, bool resetTimer, bool load)
{
    ...
    {
        m_tickNumber = 0;
        // reset periodic timer on aura create or on reapply
...
{   
    ...
    // if needed, add target to snapShot for later calculating dot/hot ticks 
    if (snapShot && (mode & AURA_EFFECT_HANDLE_REAL))
    {
        if (apply)
            if(Aura* baseAura = aurApp->GetBase())
                snapShot->AddTargetDependentValues(aurApp->GetTarget(), baseAura->GetCaster());
        if(!apply)
            snapShot->RemoveTargetDependentValues(aurApp->GetTarget());
    }
                ...
                case SPELLFAMILY_GENERIC:
                    switch (GetId())
                    {
                        ...
                        case 66823: case 67618: case 67619: case 67620: // Paralytic Toxin
                            ...
                            case 66020: // Chains of Ice (ToC - Faction Champions)
                            {
                                // Get 0 effect aura
                                if (AuraEffect* slow = GetBase()->GetEffect(0))
                                {
...
{
    ASSERT(caster);
    // we will have to add the bonus from debuffs on the target, because they are taken into account live (every tick) and are therfore not included in the snapshot
    float targetCritBonus = target->SpellCritChanceBonusTaken(target, GetSpellInfo(), GetSpellInfo()->GetSchoolMask());
    ...
    {
        if ((*itr)->IsAffectedOnSpell(m_spellInfo) && roll_chance_f(snapShot->GetCritChance(target) + targetCritBonus))
            return true;
    }

    // Rupture - since 3.3.3 can crit
    if (m_spellInfo->SpellIconID == 500 && m_spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE)
        return roll_chance_f(snapShot->GetCritChance(target) + targetCritBonus);
...
void AuraEffect::PeriodicTick(AuraApplication * aurApp, Unit* caster) const
{
    ...
    *m_base <<= Fire::EffectPeriodic(m_effIndex);
...
/**
* @name UsesSnapShot
* @brief true if the Auratype needs a snapShot to be handled
*/
bool AuraEffect::UsesSnapShot() const
{
    switch (GetAuraType())
    {
        case SPELL_AURA_PERIODIC_DAMAGE:
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        if (target->HasAura(5229) && !target->HasAura(70726)) // Enrage Reduction Armor with Check T10
            target->RemoveAurasDueToSpell(5229);
...
void AuraEffect::HandleAuraCloneCaster(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (auto map = target->FindMap())
            if (auto replay = map->GetReplayData())
                replay->AddMirrorImage(target->GetGUID(), caster->GetGUID());
    }
    else
    {
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        const auto interruptSpell = [](Unit* target)
        {
            if (!target->HasUnitState(UNIT_STATE_CASTING))
                return;

            if (target->ToCreature() && target->ToCreature()->IsDungeonBoss())
                return;

            for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
            {
                if (target->GetCurrentSpell(i)
                    && target->GetCurrentSpell(i)->m_targets.GetUnitTargetGUID() == target->GetGUID())
                {
        ...
        target->VisitAnyNearbyObject<Unit, Trinity::FunctionAction>(target->GetMap()->GetVisibilityRange(), interruptSpell, u_check);
...
void AuraEffect::HandleAuraModDisarm(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER && GetAuraType() == SPELL_AURA_MOD_DISARM_RANGED)
        if (Item* offhand = target->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED))
            if (offhand->GetTemplate()->InventoryType == INVTYPE_RELIC)
                return;
...
void AuraEffect::HandleAuraMounted(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (auto player = target->ToPlayer())
        {
            ...
            {
                auto itr = sObjectMgr->FactionChangeCfbgSpells.find(GetSpellInfo()->Id);
                if (itr != sObjectMgr->FactionChangeCfbgSpells.end())
                {
                    auto spellInfo = sSpellMgr->GetSpellInfo(itr->second);
                    if (spellInfo)
                    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                        if (spellInfo->Effects[i].Effect == SPELL_EFFECT_APPLY_AURA && 
                            spellInfo->Effects[i].ApplyAuraName == SPELL_AURA_MOUNTED) 
                        {
                            creatureEntry = spellInfo->Effects[i].MiscValue;
                            break;
                        }
                }
            }
...
void AuraEffect::HandleAuraControlVehicle(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        if (caster->IsInWorld() && !caster->GetMap()->IsDungeon())
            caster->RemoveAurasByType(SPELL_AURA_MOD_SHAPESHIFT, [](const AuraApplication* app) { return app->GetBase()->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_DRUID; });
        ...
        caster->RemoveAura(54861); // Nitro Boost
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (GetId())
    {
        ...
        case 46924: // Bladestorm
            ...
            //target->ApplySpellImmune(0, IMMUNITY_ID, 605, apply); // Mind Control ** should be handled in MECHANIC_CHARM, if not remove the comment **
            break;
...
void AuraEffect::HandleAuraModSchoolImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (!apply && target->HasAura(aurApp->GetBase()->GetSpellInfo()->Id))
        return;
...
void AuraEffect::HandleAuraModResistance(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    bool applyResMod = true;
    float amount = GetAmount();
    Unit::AuraEffectList const& resMods = target->GetAuraEffectsByType(SPELL_AURA_MOD_RESISTANCE);
    for (Unit::AuraEffectList::const_iterator i = resMods.begin(); i != resMods.end(); ++i)
    {
        //! Auras have same stat modifier or all as mod type (< 0)
        if (((*i)->GetMiscValue() == GetMiscValue() || GetMiscValue() < 0) && aurApp->GetBase())
    {
        SpellGroupStackRule stackRule = sSpellMgr->CheckSpellGroupStackRules((*i)->GetSpellInfo(), aurApp->GetBase()->GetSpellInfo());
        //! Only stacks of this type should not be added
        if (stackRule == SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT)
        {
           //! Amount check is always absolute value checking
           if (std::abs((*i)->GetAmount()) >= std::abs(amount))
           {
               applyResMod = false;
               break; //! When it should not be added it should not be added
           }
        }
    }
    }
...
void AuraEffect::HandleAuraModBaseResistancePCT(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() != TYPEID_PLAYER)
    {
        ...
        if (target->IsPet() && (GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL))
            target->HandleStatModifier(UNIT_MOD_ARMOR, BASE_PCT, float(GetAmount()), apply, GetSpellInfo());
    }
    else
    {
        for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++)
...
void AuraEffect::HandleAuraModIncreaseHealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 ammount = GetAmount();
    if (ammount > 0)
        target->GetHighestExclusiveAuraDiff(GetBase(), GetEffIndex());

    if (apply)
    {
        target->HandleStatModifier(UNIT_MOD_HEALTH, TOTAL_VALUE, float(ammount), apply);
        target->ModifyHealth(ammount);
    }
    else
    {
...
void AuraEffect::HandleAuraModIncreaseMaxHealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetHealth() > 0)
    {
        uint32 newHealth = std::max<uint32>(target->CountPctFromMaxHealth(percent), 1);
...
void AuraEffect::HandleModCastingSpeed(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // Boss creatures shouldn't be affected
    if (Unit* caster = GetCaster())
        if (caster->GetTypeId() == TYPEID_PLAYER && target->ToCreature() && target->ToCreature()->isWorldBoss())
            return;

    target->ApplyCastTimePercentMod((float)GetAmount(), apply);
}

void AuraEffect::HandleModCastingSpeedNotStack(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    if (!(mode & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    Unit::AuraEffectList hasteList = target->GetAuraEffectsByType(SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK);
    if (hasteList.size())
        for (Unit::AuraEffectList::const_iterator itr = hasteList.begin(); itr != hasteList.end(); ++itr)
        {
            if (sSpellMgr->CheckSpellGroupStackRules(aurApp->GetBase()->GetSpellInfo(), (*itr)->GetSpellInfo()) == SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT)
            {
                if (GetAmount() > (*itr)->GetAmount())
                    target->ApplyCastTimePercentMod((float) (*itr)->GetAmount(), !apply); // remove old mod
                else
                    return; // stronger buff already there
            }
        }
    target->ApplyCastTimePercentMod((float)GetAmount(), apply);
}
...
void AuraEffect::HandleModCombatSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        int32 MaxVal = 0;
        Unit::AuraEffectList HasteBuffs = target->GetAuraEffectsByType(SPELL_AURA_MELEE_SLOW);
        if (HasteBuffs.size())
        {
            for (Unit::AuraEffectList::const_iterator itr = HasteBuffs.begin(); itr != HasteBuffs.end(); ++itr)
            {

                if (!(*itr) || (*itr) == this)
                    continue;

                // stronger debuff found - return
                if ((*itr)->GetAmount() <= GetAmount())
                    return;

                // look for strongest debuff already there
                if ((*itr)->GetAmount() < MaxVal)
                    MaxVal = (*itr)->GetAmount();

            }
        }
        // no stronger buff found - "remove" strongest buff already there
        target->ApplyCastTimePercentMod(float(MaxVal), !apply);
        target->ApplyAttackTimePercentMod(BASE_ATTACK, float(MaxVal), !apply);
        target->ApplyAttackTimePercentMod(OFF_ATTACK, float(MaxVal), !apply);
        target->ApplyAttackTimePercentMod(RANGED_ATTACK, float(MaxVal), !apply);
    }
...
void AuraEffect::HandleAuraDummy(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (mode & (AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY))
    {
        ...
        {
            if ((GetSpellInfo()->IsQuestTame()) && caster && caster->IsAlive() && target->IsAlive() && aurApp->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE)
            {
...
void AuraEffect::HandleChannelDeathItem(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    RG_LOG<ItemLogModule>(newitem, RG::Logging::ItemLogType::ADDED, count);
    if (!newitem)
    {
...
void AuraEffect::HandleAuraConvertRune(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    const uint8 runes = m_amount;
    ASSERT(runes == 1); // only known instance of this effect uses 1 rune (bloodtap)

    // convert number of runes specified in aura amount of rune type in miscvalue to runetype in miscvalueb
    if (apply)
    {
        const auto [index, hasCD] = player->GetFirstRuneWithCD(RuneType(GetMiscValue()));
        util::meta::Unreferenced(hasCD);

        player->ConvertRuneByAuraEffect(index, RuneType(GetMiscValueB()), this);
    }
    else
    ...
    switch (GetSpellInfo()->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch (GetId())
            {
                case 67039: // argent squire mount
                    ...
                    {
                        if (caster->GetOwner()->IsMounted() && !caster->IsMounted())
                        {
                            caster->SetSpeedRate(MOVE_RUN, 2.0f);
                            caster->Mount(29736);
                        }
                        else if (caster->IsMounted() && !caster->GetOwner()->IsMounted())
                        {
                            caster->SetSpeedRate(MOVE_RUN, 1.0f);
                            caster->Dismount();
                        }
                    }
                    break;
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (GetSpellInfo()->Id)
            {
                // Master of Subtlety
                case 31666:
                    if (!target->HasAuraType(SPELL_AURA_MOD_STEALTH) || !target->HasAuraType(SPELL_AURA_MOD_SHAPESHIFT))
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
            ...
            case SPELLFAMILY_GENERIC:
            {
                switch (auraId)
                {
                    ...
                    case 46736:
                        ...
                        break;
                    // Food - Well Fed trigger
                    case 24869:
                        triggerSpellId = 19711;
                        break;
                    // Food - Noble Garden Bufffood
                    case 61874:
                        triggerSpellId = 24870;
                        break;
                    // Time Bomb (Mage-Lord Urom - Occulus)
                    case 51121: // nhc
                    case 59376: // hc
                        target->CastCustomSpell(51132, SPELLVALUE_BASE_POINT0, int32(target->GetMaxHealth()-target->GetHealth()), target, true, NULL, this);
                        return;
                }
                break;
            }
    ...
    else
    {
        ...
        switch (auraId)
        {
            ...
            case 58882: // Rank 2
            {
                if (int32 mana = target->GetMaxPower(POWER_MANA) * GetAmount() / 100)
...
{
    if (!caster || !target->IsAlive() || !snapShot)
        return;
 
    if (GetSpellInfo()->Effects[GetEffIndex()].Effect == SPELL_EFFECT_PERSISTENT_AREA_AURA)
        caster->CombatStart(target, !m_spellInfo->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO));
    ...
    uint32 damage = std::max(m_amount, 0); 
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        ...
        if (m_spellInfo->SpellIconID == 937) // Mage - Ignite
            apply_direct_bonus = false;
        else if (m_spellInfo->SpellIconID == 1494) // Unholy Blight
            apply_direct_bonus = false;

        if (m_spellInfo->SpellIconID == 2111) // LK - Infest
            no_snapshot = true;

        if (apply_direct_bonus)
        {
            if (!no_snapshot)
                damage = snapShot->GetDamage(target);
        ...
        if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_GENERIC)
        {
            switch (GetId())
            {
                ...
                case 72856: // Unbound Plague
                    damage = uint32(damage * std::pow(1.25f, int32(m_tickNumber)));
                    break;
                default:
                    break;
            }
    ...
    if (!GetSpellInfo()->HasAttribute(SPELL_ATTR4_FIXED_DAMAGE))
    {
        ...
        caster->ApplyResilience(target, NULL, &noncritDamage, crit, CR_CRIT_TAKEN_SPELL);
    }
    damage = dmg;

    caster->CalcAbsorbResist(target, GetSpellInfo()->GetSchoolMask(), DOT, damage, damage, &absorb, &resist, GetSpellInfo());

    TC_LOG_DEBUG("spells.periodic", "PeriodicTick: %s attacked %s for %u dmg inflicted by %u abs is %u",
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    uint32 damage = snapShot->GetDamage(target);
    ...
    caster->CalcAbsorbResist(target, GetSpellInfo()->GetSchoolMask(), DOT, damage, damage, &absorb, &resist, m_spellInfo);
...
* @brief Replaces the current PeriodicEffectSnapShot of this AuraEffect with a copy of the recievedSnapShot
* To make sense, the snapshot should come from a similar spell
* @param recievedSnapShot the snapshot that should get copied and then used for this effect
* This method does nothing if recievedSnapShot is null
*/
void AuraEffect::RecieveSnapShot(const PeriodicEffectSnapShot* recievedSnapShot)
{
    if (!recievedSnapShot)
        return;
    delete snapShot;
    snapShot = new PeriodicEffectSnapShot(this, recievedSnapShot);
} 

/**
* @name PeriodicEffectSnapShot (Constructor)
* @brief If the caster of the baseEffect is a player this "Snapshots" (stores) the effects
* of the current player SpellMods on the periodic effect in this instance. 
* Also decides if the effect is a healeffect.
* To be useful, each target of the periodic effect should be added as soon as available
* to fully calculate the values for the snapshot.
* @param baseEffect the AuraEffect (not null) that will use this for calculating the effect of his dots etc
* @param caster the unit which casts the base-auraeffect, could probably be null (e.g. in case of loading a spell from an unknown caster)
*/
PeriodicEffectSnapShot::PeriodicEffectSnapShot(AuraEffect* baseEffect, Unit* caster) :
baseEffect(baseEffect)
{
    ASSERT(baseEffect);
    AuraType type = baseEffect->GetAuraType();
    isHealEffect = ((type == SPELL_AURA_PERIODIC_HEAL) || (type == SPELL_AURA_OBS_MOD_HEALTH)) ? true : false;
    if (caster)
    {
        calculatePlayerSpellModValues(caster);
    }
}

/**
* @name PeriodicEffectSnapShot (Constructor for mimicking another snapshot)
* @brief Uses the values of the caster from the snapShotToCopy to construct a new SnapShot
* which will get added as targets all current targets of the aura of the baseeffect.
* This constructor should be used if this effect should use the same caster-dependent snapshot values as another effect does at the moment
* @param baseEffect the AuraEffect (not null) that will use this for calculating the effect of his dots etc
* @param snapShotToCopy the PeriodicEffectSnapShot (not null) which values this snapshot should use
* The snapShotToCopy should come from a similar spell otherwise it makes not much sense to copy its values
*/
PeriodicEffectSnapShot::PeriodicEffectSnapShot(AuraEffect* baseEffect, const PeriodicEffectSnapShot* snapShotToCopy) :
baseEffect(baseEffect)
{
    ASSERT(baseEffect);
    ASSERT(snapShotToCopy);
    AuraType type = baseEffect->GetAuraType();
    isHealEffect = ((type == SPELL_AURA_PERIODIC_HEAL) || (type == SPELL_AURA_OBS_MOD_HEALTH)) ? true : false;
    // Copy the target indepent values (caster values)
    playerSpellModFlatModifier = snapShotToCopy->GetSpellModFlatModifier();
    playerSpellModPercentageModifier = snapShotToCopy->GetSpellModPercentageModifier();

    // The seperation between target indepent caster values and target dependent values isn't very good at the moment
    // (like % Modifiers (except spell mods) which should apply for all targets: they
    // are not seperated from target dependet values, they are both calculated in SpellDirectDamageBonus)
    // so calculating the target dependent values for each known target now would lead to wrong
    // results (e.g we wouldn't copy the %modifiers from the recieved snapshot, although we want exactly that)

    // So to go around this problem we just take a random (here the first) target of the other snapshot
    TargetDependetValues exampleValuesForTarget = snapShotToCopy->GetExampleTargetValues();

    std::list<Unit*> targetslist;
    baseEffect->GetTargetList(targetslist);
    // and pretend each target of this snapshot (or auraEffect) has exactly the same values
    for (std::list<Unit*>::iterator itr = targetslist.begin(); itr != targetslist.end(); ++itr)
    {
        targetMap.emplace(*itr, exampleValuesForTarget);
    }
}

/**
* @brief returns the TargetDependentValues of the first target of this spell
* This is to help copying this snapshot into another auraEffect (RecieveSnapShot()), because the target indepent values
* (currently only the player spell mods) don't include everything that should be
* copied
*/
PeriodicEffectSnapShot::TargetDependetValues PeriodicEffectSnapShot::GetExampleTargetValues() const
{
    return targetMap.begin()->second;
}

/**
* @name AddTargetDependentValues
* @brief adds a target to the SnapShot to enable the snapshot to fully calculate
* the future periodic effect (on this specific target)
* (needed because some modifiers are target dependent (e.g some classes
* do more damage on low life enemys))
* @param target one of the Units the baseEffect is applied on
* @param caster the unit which casts the base-auraeffect, and you want to use to calculate the values
* (does nothing if target or caster is null)
*/
void PeriodicEffectSnapShot::AddTargetDependentValues(Unit* target, Unit* caster)
{
    if (target && caster)
    {
        TargetDependetValues newTargetValues;
        calculateAllForTarget(target, newTargetValues, caster);
        targetMap.emplace(target, newTargetValues);
    }
}

/**
* @name RemoveTargetDependentValues
* @brief remove a targetfrom the SnapShot such that Refresh() no longer
* unnecessarily updates the values for that target
* @param target one of the Units the baseEffect is removed from
*/
void PeriodicEffectSnapShot::RemoveTargetDependentValues(Unit* target)
{
...
/**
* @name Refresh
* @brief recalculates the values of a SnapShot that should be recalculated
* when the AuraEffect gets refreshed.
* When softrefreshed only the flat damage of the spell is recalculated (from spellpower and base amount etc)
* else everything will be redone, including player spell mods
* @param caster the unit which casts the base-auraeffect, and you want to use to calculate the values
* does nothing if the caster is null
* @param softRefresh if true only same parts of the snapshot gets recalulated (only the basedamage), else everything
*/
void PeriodicEffectSnapShot::Refresh(Unit* caster, bool softRefresh)
{
    if (!caster)
        return;
    for (TargetDependentValuesMap::iterator itr = targetMap.begin(); itr != targetMap.end(); ++itr)
    {
        Unit* target = itr->first;
        TargetDependetValues& values = (itr->second);
        if (softRefresh)
            recalculateBaseDamageForTarget(target, values, caster);
        else
            calculateAllForTarget(target, values, caster);
    }
    if (!softRefresh)
    {
        calculatePlayerSpellModValues(caster);
...
/**
* @name GetDamage
* @brief returns the damage/heal which the baseEffect should do, without taking
* into account what damage bonus the target takes from debuffs etc.
* (this is comparable to what Unit::SpellDamageBonusDone does, and should be
* treated as if the values comes from there)
* @param target the Unit which the baseEffect should do something to (heal, deal damage)
* @return the damage/heal the periodicEffect will deal, or 0 if the target wasn't added
* to the snapshot or the amount would be negative
*/
uint32 PeriodicEffectSnapShot::GetDamage(Unit* target)
{
    if (!targetMap.count(target))
        return 0;
    int32 damage = (((int32)targetMap[target].baseDamage) * (targetMap[target].percentageModifier)
        + playerSpellModFlatModifier) * playerSpellModPercentageModifier;
...
/**
* @name GetCritChance
* @brief returns the critchance which the baseEffect should have against this target without 
* taking into account the bonus chance the target gets from debuffs etc
* (this is comparable to what Unit::SpellCritChanceDone does, and should be
* treated as if the values comes from there)
* @param target the Unit which the baseEffect should do something to (heal, deal damage)
*/
float PeriodicEffectSnapShot::GetCritChance(Unit* target) {
    if (!targetMap.count(target))
        return 0.0f;
    return targetMap[target].critChance;
}

// private method to calculate all 3 target dependent values (crit, flat and percent-modifiers)
// caster should NOT be null if you call this
void PeriodicEffectSnapShot::calculateAllForTarget(Unit* target, TargetDependetValues& targetValues, Unit* caster){
    const SpellInfo* spellInfo = baseEffect->GetSpellInfo();
    targetValues.critChance = baseEffect->GetCaster()->SpellCritChanceDone(target, spellInfo, spellInfo->GetSchoolMask());
    Unit::FlatAndPercentModifiers modifiers = directFlatAndPercentModifiers(target, caster);
    targetValues.baseDamage = modifiers.first;
    targetValues.percentageModifier = modifiers.second;
}

// private method to calculate all playerSpellMods
// caster should NOT be null if you call this
void PeriodicEffectSnapShot::calculatePlayerSpellModValues(Unit* caster)
{
    if (Player* modOwner = caster->GetSpellModOwner())
    {
        int32 damage = baseEffect->GetAmount()+1; //should not be zero otherwise apply spell mods doesn't apply any percent mods
        Unit::FlatAndPercentModifiers modifiers = modOwner->ApplySpellMod(baseEffect->GetBase()->GetSpellInfo()->Id, SPELLMOD_DOT, damage);
        playerSpellModFlatModifier = modifiers.first;
        playerSpellModPercentageModifier = modifiers.second;
    }
}

// private method to calculate all the target dependent values that change when the effect gets soft-refreshed
// (only the flat base damage, without the percent modifier)
// caster should NOT be null if you call this
void PeriodicEffectSnapShot::recalculateBaseDamageForTarget(Unit* target, TargetDependetValues& targetValues, Unit* caster){

    Unit::FlatAndPercentModifiers modifiers = directFlatAndPercentModifiers(target, caster);
    targetValues.baseDamage = modifiers.first;
}

// helper method for calculateAll and recalculateBaseDamage
// caster should NOT be null if you call this
Unit::FlatAndPercentModifiers PeriodicEffectSnapShot::directFlatAndPercentModifiers(Unit* target, Unit* caster)
{
    int32 damage = baseEffect->GetAmount();
    if (isHealEffect)
        return caster->SpellDirectHealingBonus(target, baseEffect->GetSpellInfo(), baseEffect->GetAmount(),
        DOT, baseEffect->GetBase()->GetStackAmount(), damage);
    else
        return caster->SpellDirectDamageBonus(target, baseEffect->GetSpellInfo(), baseEffect->GetAmount(),
        DOT, baseEffect->GetBase()->GetStackAmount(), damage);
}
