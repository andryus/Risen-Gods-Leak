...
class GAME_API Aura : public script::Scriptable
{
    ...
    public:
        ...
        static Aura* TryRefreshStackOrCreate(SpellInfo const* spellproto, uint8 tryEffMask, WorldObject* owner, Unit* caster, int32* baseAmount = NULL, Item* castItem = NULL, ObjectGuid casterGUID = ObjectGuid::Empty, bool* refresh = NULL, bool softRefresh = false);
        ...
        void Refresh(bool resetTimer = true, bool softRefresh = false);
        void SoftRefresh(); // = Refresh(false, true)
        void RecieveSnapShots(Aura* snapShotGivingAura);
        ...
        bool ModStackAmount(int32 num, AuraRemoveMode removeMode = AURA_REMOVE_BY_DEFAULT, bool softRefresh = false);
    ...
    private:
        ...
        // Spells have a list of (SpellMod-)Auras they used
        // To ensure the validity of the aura pointers in the spells list, we make a list of "reverse" pointers
        // from this aura to the spells that use this aura. 
        // In case this aura fades / gets removed we inform the spells
        // so they delete the now invalid aura pointer
        friend void Spell::RemoveUsedModAura(Aura* aura);
        friend void Spell::AddUsedModAura(Aura* aura);
        std::list<Spell*> spellUsingModsFromThisAura;
