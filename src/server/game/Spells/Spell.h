...
class ByteBuffer;
class PathGenerator;
namespace RG { namespace Logging { class SpellCastLogEntry; }}
...
class GAME_API Spell : public script::Scriptable
{
    ...
    public:
        ...
        void EffectModifyThreatByPercent(SpellEffIndex effIndex);
        ...
        // Guess: Some Casts seem call themself after their Success. If 'strict' ist true the 
        // cast ist called first time, if false it was called by itself
        ...
        void ResetAttackTimerAfterCastIfNeeded();
        void SetSoftRefresh(bool softRefresh) { m_softRefresh = softRefresh; }
        ...
        bool FindCachedSpellMods(std::pair<int32, float>& modValues, SpellModOp op, uint32 modTakingSpellId);
        Unit::FlatAndPercentModifiers GetCachedSpellModsForOp(SpellModOp i);
		// @brief returns true if the spell has cached mods, which can only happen for delayed spells after their cast() phase
        bool HasCachedMods() { return !m_spellModCache.empty(); }
        
        
        void AddAppliedMod(SpellModifier* mod);
        const UsedSpellMods GetUsedModAuras() { return m_appliedMods; }
        void DropUsedModAurasCharges(bool finishedCasting);
        void RestoreUsedModAuras(uint32 ownerAuraId = 0, Aura* auraToRestore = NULL);

        // Helper for keeping all pointers in appliedMods valid 
        // Generate/deletes the aura pointers in this list aswell as removing the spell counterpointer in the aura
        void AddUsedModAura(Aura* usedAura);
        void RemoveUsedModAura(Aura* usedAura);
    ...
    protected:
        ...
        void CacheSpellModsForDelayedHit();
        std::map<uint8, std::pair<int32, float>> m_spellModCache;

        // A list of all auras which spellmods were used to enhance this spell
        // The pointer in this list are kept valid by counterpointer in the auras
        // in case the aura fades, the pointer in this list gets removed
        ...
        bool m_autoRepeat;
        uint8 m_runesState;
        bool m_softRefresh;                                 // should the spell try to softrefresh his aura if the spell is already on the target?                                         
        ...
        bool delayedDuelCheck;                              // check for friendly target when missile arrives
        ...
        struct HitTriggerSpell
        {
            ...
            ObjectGuid triggeredByItemGUID;
        ...
        std::unique_ptr<PathGenerator> preGeneratedPath;
        ...
        std::unique_ptr<RG::Logging::SpellCastLogEntry> cast_timing_log_;

    private:
        // we need to increase our grid selecting range by the maximum hitbox
        static constexpr float MAX_HITBOX_RANGE = 32.0f;

        std::string OnQueryOwnerName() const override final;
        std::string OnQueryFullName() const override final;
        bool OnInitScript(std::string_view scriptName) override final;
};
...
namespace Trinity
{
    ...
    struct GAME_API WorldObjectSpellTrajTargetCheck : public WorldObjectSpellAreaTargetCheck
    {
