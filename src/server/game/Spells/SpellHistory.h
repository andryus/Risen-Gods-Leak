...
public:
    ...
    void AddCooldown(uint32 spellId, uint32 itemId, std::chrono::duration<Type, Period> cooldownDuration)
    {
        ...
        AddCooldown(spellId, itemId, now + std::chrono::duration_cast<Clock::duration>(cooldownDuration));
    }
    ...
    void ForAll(std::function<void(CooldownStorageType::const_iterator)> predicate)
    {
