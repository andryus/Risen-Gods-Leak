...
bool SpellInfo::EffectIsTargetingArea(uint32 eff) const
{
    return (Effects[eff].IsEffect() && (Effects[eff].IsTargetingArea() || Effects[eff].IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA)));
}
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player) const
{
    ...
    switch (Id)
    {
        ...
        case 22012:                                         // Spirit Heal
        {
            ...
            return zone_id == 4197 || mapEntry->IsBattleground() ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        }
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_FLY:
            {
                if (HasAttribute(SPELL_ATTR7_IGNORE_COLD_WEATHER_FLYING))
                    break;
                if (GetVirtualMapForMapAndZone(map_id, zone_id) == 571 && !(player && player->HasSpell(54197))) // 54197 = Cold Weather Flying
                    return SPELL_FAILED_INCORRECT_AREA;
            }
        }
    }
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    if (!HasAttribute(SPELL_ATTR6_CAN_TARGET_INVISIBLE) && !caster->CanSeeOrDetect(target, implicit) && !(caster->GetTypeId() == TYPEID_UNIT && caster->ToUnit()->GetEntry() == WORLD_TRIGGER))
        return SPELL_FAILED_BAD_TARGETS;
    ...
    if (unitTarget)
    {
        ...
        if (HasAttribute(SPELL_ATTR3_ONLY_TARGET_GHOSTS) != (unitTarget->HasAuraType(SPELL_AURA_GHOST) || unitTarget->IsSpiritService()))
        {
        ...
        if (caster != unitTarget)
        {
            if (Unit* owner = caster->GetOwner())
                if (!IsPositive() && !(caster->IsControlledByPlayer() ? caster : owner)->_IsValidAttackTarget(unitTarget, this))
                    return unitTarget->GetTypeId() == TYPEID_PLAYER ? (unitTarget->ToPlayer()->IsGameMaster() ? SPELL_FAILED_BM_OR_INVISGOD : SPELL_FAILED_BAD_TARGETS) : SPELL_FAILED_BAD_TARGETS;
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (Effects[i].TargetA.GetTarget() == TARGET_UNIT_TARGET_MINIPET)
                if (!unitTarget->GetOwner() || unitTarget->GetGUID() != unitTarget->GetOwner()->GetCritterGUID())
                    return SPELL_FAILED_BAD_TARGETS;
        }
    }
...
SpellCastResult SpellInfo::CheckExplicitTarget(Unit const* caster, WorldObject const* target, Item const* itemTarget) const
{
    ...
    if (Unit const* unitTarget = target->ToUnit())
    {
        ...
        {
            if (unitTarget->GetVehicleBase() && unitTarget->GetVehicleBase()->GetTypeId() == TYPEID_PLAYER) // Gormok snobold vassal on player
                return SPELL_CAST_OK;
...
bool SpellInfo::_IsPositiveEffect(uint8 effIndex, bool deep) const
{
    ...
    switch (Effects[effIndex].Effect)
    {
        ...
        case SPELL_EFFECT_SCHOOL_DAMAGE:
        {
            // Spells with only a damage effect should be negative
            // Checks if there is no other effect
            if (!Effects[(effIndex + 1) % 3].IsEffect() && !Effects[(effIndex + 2) % 3].IsEffect())
                return false;
            break;
        }
...
bool SpellInfo::IsAffectedBySpellDelay(Unit* caster, Unit* target) const
{
    if (!Delay)
        return false;

    if (CastTimeEntry->ID != 1)
        return false;
...
bool SpellInfo::TriggeredEffectNeedsLosChecks(uint32 eff) const
{
    // Positive triggered spells don't require any LoS checks
    if (IsPositive())
        return false;

    switch (Effects[eff].TargetA.GetTarget())
    {
...
bool SpellInfo::IsAffectedByBinaryResistance(Unit* caster, Unit* target) const
{
    if (!(AttributesCu & SPELL_ATTR0_CU_BINARY)) //! only magic spells, defined in spellmgr
        return false;

    if (caster == target) //! no heals, or other spells on self
        return false;

    if (caster->IsFriendlyTo(target)) //! spells like health funnel
        return false;

    if (caster->GetTypeId() != TYPEID_PLAYER) //! only for spells casted by player
        return false;

    if ((target->GetTypeId() != TYPEID_PLAYER) && (!target->IsPet()) && (!target->IsHunterPet())) //! only for player or pet targets
        return false;
...
{
    return HasAttribute(SPELL_ATTR0_CU_BINARY);
}

int32 SpellInfo::GetBasePoints(SpellEffIndex effect) const
{
    return Effects[effect].BasePoints+1;
}

/**
* @name GetBaseProccFlags
* @brief fills in the procc flags based whatever the spell is positive or not. To know that, we need to know how much damage/healing 
* a specific casted spell with this SpellInfo did
* @param procAttacker the flag for procc infos of the attacker, the caster of the spell, will be changed by this method
* @param procVictim the flag for procc infos about the victim of the spell, will be changed by this method
* @param damage the amount of damage the spell did, if > 0 the spell is considered negative for deciding the procc flags
* @param healing the amount of healing the spell does, if 0 the spell is considered positive or not based on IsPositive()
* of this SpellInfo
*/
void SpellInfo::GetPosititvityProccFlags(uint32& procAttacker, uint32& procVictim, int32 damage, int32 healing) const
{
    bool positive = true;
    if (damage > 0)
        positive = false;
...
/**
* @name GetBaseProccFlags
* @brief fills in the procc flags purely based on SpellInfo and the way of casting/excecuting a specific spell with this SpellInfo
* @param procAttacker the flag for procc infos of the attacker, the caster of the spell, will be changed by this method 
* @param procVictim the flag for procc infos about the victim of the spell, will be changed by this method
* @param procEx the flag for procc infos about the excecution of the procc, will be changed by this method
* @param triggeredCastFlags the TriggerCastFlags of the specific casting of this spell, contaiing details if the spell should
* trigger proccs etc.
* @param attackType the WeaponAttackType of the specific casting/execution of this spell
* @param caster the Unit casting the spell, can be null
* @param originalCaster the Unit which counts as the original caster of the spell, for some spells not the same
* as the caster, can be null (e.g. for totem spells, the original caster is the unit controlling the totems)
*/
void SpellInfo::GetBaseProccFlags(uint32& procAttacker, uint32& procVictim, uint32& procEx, TriggerCastFlags triggeredCastFlags,
                                 WeaponAttackType attackType, Unit* caster, Unit* originalCaster) const //terrible long parameter list....
{
    switch (DmgClass)
    {
    ...
    case SPELL_DAMAGE_CLASS_RANGED:
        // Auto attack
        if (AttributesEx2 & SPELL_ATTR2_AUTOREPEAT_FLAG)
        {
        ...
        break;
    default:
        if (EquippedItemClass == ITEM_CLASS_WEAPON &&
            EquippedItemSubClassMask & (1 << ITEM_SUBCLASS_WEAPON_WAND)
            && AttributesEx2 & SPELL_ATTR2_AUTOREPEAT_FLAG) // Wands auto attack
        {
            procAttacker = PROC_FLAG_DONE_RANGED_AUTO_ATTACK;
            procVictim = PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK;
        }
    ...
    if (SpellFamilyName == SPELLFAMILY_WARLOCK && SpellFamilyFlags[0] & 0x00000040)
    {
    ...
    if (!(procAttacker & PROC_FLAG_DONE_RANGED_AUTO_ATTACK))
    {
        if (triggeredCastFlags & TRIGGERED_DISALLOW_PROC_EVENTS &&
            (AttributesEx2 & SPELL_ATTR2_TRIGGERED_CAN_TRIGGER_PROC ||
            AttributesEx3 & SPELL_ATTR3_TRIGGERED_CAN_TRIGGER_PROC_2))
            procEx |= PROC_EX_INTERNAL_CANT_PROC;
        else if (triggeredCastFlags & TRIGGERED_DISALLOW_PROC_EVENTS)
            procEx |= PROC_EX_INTERNAL_TRIGGERED;
    }
    // Totem casts require spellfamilymask defined in spell_proc_event to proc
    if (originalCaster && caster && caster != originalCaster && caster->GetTypeId() == TYPEID_UNIT && caster->ToCreature()->IsTotem() && caster->IsControlledByPlayer())
        procEx |= PROC_EX_INTERNAL_REQ_FAMILY;
}

/**
* @name GetProccFlags
* @brief fills in the procc flags based on the SpellInfo, the way of casting/excecuting the spell and the positivity of a specific spell
* with this SpellInfo
* @param procAttacker the flag for procc infos of the attacker, the caster of the spell, will be changed by this method
* @param procVictim the flag for procc infos about the victim of the spell, will be changed by this method
* @param procEx the flag for procc infos about the excecution of the procc, will be changed by this method
...
* of this SpellInfo
*/
void SpellInfo::GetProccFlags(uint32& procAttacker, uint32& procVictim, uint32& procEx, TriggerCastFlags triggeredCastFlags,
    WeaponAttackType attackType, Unit* caster, Unit* originalCaster,
    int32 damage, int32 healing) const
{
    GetBaseProccFlags(procAttacker, procVictim, procEx, triggeredCastFlags, attackType, caster, originalCaster);
    GetPosititvityProccFlags(procAttacker, procVictim, damage, healing);
}
