...
{
    Clock::time_point now = game::time::GetGameTimeSystemPoint();
...
void SpellHistory::StartCooldown(SpellInfo const* spellInfo, uint32 itemId, Spell* spell /*= nullptr*/, bool onHold /*= false*/)
{
    ...
    if (itemId)
    {
        if (auto player = GetPlayerOwner())
            if (!player->IsPlayingNative() && spell && spellInfo->Id == spell->GetSpellInfo()->Id)
            {
                auto replacedSpell = sObjectMgr->FactionChangeCfbgSpells.find(spellInfo->Id);
                if (replacedSpell != sObjectMgr->FactionChangeCfbgSpells.end())
                {
                    auto otherSpell = sSpellMgr->GetSpellInfo(replacedSpell->second);
                    if (otherSpell && otherSpell != spellInfo)
                    {
                        StartCooldown(otherSpell, itemId, spell, onHold);
                        return;
                    }
                }
            }
    ...
    Clock::time_point curTime = game::time::GetGameTimeSystemPoint();
    ...
    if (recTime != curTime)
    {
        ...
        if (needsCooldownPacket)
        {
            if (Player* playerOwner = GetPlayerOwner())
            {
                ...
                playerOwner->SendDirectMessage(std::move(spellCooldown));
            }
        }
    }
...
void SpellHistory::ModifyCooldown(uint32 spellId, int32 cooldownModMs)
{
    ...
    if (Player* playerOwner = GetPlayerOwner())
    {
        ...
        playerOwner->SendDirectMessage(std::move(modifyCooldown));
    }
}
...
void SpellHistory::ResetCooldown(CooldownStorageType::iterator& itr, bool update /*= false*/)
{
    if (update)
    {
        if (Player* playerOwner = GetPlayerOwner())
        {
            ...
            data << uint64(_owner->GetGUID());
            playerOwner->SendDirectMessage(std::move(data));
        }
    }
...
void SpellHistory::LockSpellSchool(SpellSchoolMask schoolMask, uint32 lockoutTime)
{
    ...
    if (Player* player = GetPlayerOwner())
    {
        if (!cooldowns.empty())
        {
            ...
            player->SendDirectMessage(std::move(spellCooldowns));
        }
    }
}
...
bool SpellHistory::HasGlobalCooldown(SpellInfo const* spellInfo) const
{
    ...
    return itr != _globalCooldowns.end() && itr->second > game::time::GetGameTimeSystemPoint();
}

void SpellHistory::AddGlobalCooldown(SpellInfo const* spellInfo, uint32 duration)
{
    _globalCooldowns[spellInfo->StartRecoveryCategory] = game::time::GetGameTimeSystemPoint() + std::chrono::duration_cast<Clock::duration>(std::chrono::milliseconds(duration));
}
