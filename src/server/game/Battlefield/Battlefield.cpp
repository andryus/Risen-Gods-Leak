...
void Battlefield::InvitePlayersInZoneToQueue()
{
    for (uint8 team = 0; team < 2; ++team)
        for (GuidSet::const_iterator itr = m_players[team].begin(); itr != m_players[team].end(); ++itr)
            if (Player* player = ObjectAccessor::GetPlayer(*m_Map, *itr))
                if (!player->IsGameMaster())
                    InvitePlayerToQueue(player);
}
...
{
    if (Player* player = ObjectAccessor::GetPlayer(*m_Map, guid))
...
void Battlefield::EndBattle(bool endByTimer)
{
    ...
    bool autoChangeFaction = false;
    uint8 winner;

    if (endByTimer)
    {
        winner = GetDefenderTeam();
        uint64 factionWins = sWorld->getWorldState(WS_BATTLEFIELD_WG_FACTIN_WINS);
        if (factionWins >= sWorld->getIntConfig(CONFIG_WINTERGRASP_MAX_WINS_IN_ROW))
        {
            autoChangeFaction = true;
            sWorld->setWorldState(WS_BATTLEFIELD_WG_FACTIN_WINS, 0);
        }
        else
            sWorld->setWorldState(WS_BATTLEFIELD_WG_FACTIN_WINS, ++factionWins);
    }
    else
    {
        winner = GetAttackerTeam();
        SetDefenderTeam(GetAttackerTeam());
        sWorld->setWorldState(WS_BATTLEFIELD_WG_FACTIN_WINS, 1);
    }
    ...
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_PVPSTATS_BATTLEGROUND);
        stmt->setUInt8(0, winner);
        stmt->setUInt8(1, 8); // Always 80
        stmt->setUInt8(2, 42); // Wintergrasp: 42
        CharacterDatabase.Execute(stmt);
    }

    // Reset battlefield timer
    m_Timer = CalculateStartTime();

    OnBattleEnd(endByTimer, autoChangeFaction);
}

uint32 Battlefield::CalculateStartTime(bool crashRestart, bool setup, uint32 setupTimer) const
{
    if (crashRestart) // server crashed during the battle
        return m_RestartAfterCrash;

    uint32 nextBattleTimer = setup ? setupTimer : m_NoWarBattleTime;

    {
        // check sleep time
        const int32 sleepTimeStart = sWorld->getIntConfig(CONFIG_WINTERGRASP_SLEEP_TIME_START);
        const int32 sleepTimeEnd   = sWorld->getIntConfig(CONFIG_WINTERGRASP_SLEEP_TIME_END);
        const bool  isOverlapping  = sleepTimeStart > sleepTimeEnd;

        const time_t now = time(NULL);
        const time_t nextBattleTime = now + nextBattleTimer / IN_MILLISECONDS;
        tm ta = *localtime(&nextBattleTime);

        if (
            ( isOverlapping && (ta.tm_hour >= sleepTimeStart || ta.tm_hour < sleepTimeEnd)) ||
            (!isOverlapping && (ta.tm_hour >= sleepTimeStart && ta.tm_hour < sleepTimeEnd))
        ) {
            // next battle would take place within sleep time -> delay to the end of sleep time
            ta.tm_hour = sleepTimeEnd;
            ta.tm_min = 0;
            ta.tm_sec = 0;

            int timer = mktime(&ta) - now; // if the sleep period is overlapping and tm_hours is in the first part (tm_hours > start && tm_hours < 24), this value is negative

            if (timer < 0)
                timer += 1 * DAY;

            nextBattleTimer = static_cast<uint32>(timer) * IN_MILLISECONDS;
        }
    }
    
    return nextBattleTimer;
}
...
void Battlefield::AskToLeave(Battlefield* bf, Player* player)
{
    // Remove player from queue
    bf->AskToLeaveQueue(player);
    player->TeleportTo(KickPosition);
}
...
void Battlefield::BroadcastPacketToZone(const WorldPacket& data) const
{
    for (uint8 team = 0; team < BG_TEAMS_COUNT; ++team)
        for (GuidSet::const_iterator itr = m_players[team].begin(); itr != m_players[team].end(); ++itr)
            if (Player* player = ObjectAccessor::GetPlayer(*m_Map, *itr))
...
void Battlefield::BroadcastPacketToQueue(const WorldPacket& data) const
{
    for (uint8 team = 0; team < BG_TEAMS_COUNT; ++team)
        for (GuidSet::const_iterator itr = m_PlayersInQueue[team].begin(); itr != m_PlayersInQueue[team].end(); ++itr)
            if (Player* player = ObjectAccessor::GetPlayer(*m_Map, *itr))
...
void Battlefield::BroadcastPacketToWar(const WorldPacket& data) const
{
    for (uint8 team = 0; team < BG_TEAMS_COUNT; ++team)
        for (GuidSet::const_iterator itr = m_PlayersInWar[team].begin(); itr != m_PlayersInWar[team].end(); ++itr)
            if (Player* player = ObjectAccessor::GetPlayer(*m_Map, *itr))
...
void Battlefield::AddPlayerToResurrectQueue(ObjectGuid playerGuid)
{
...
void Battlefield::SetLastResurrectTime(ObjectGuid guid)
{
    m_LastResurrectTime[guid] = 0;
}
...
BfGraveyard::BfGraveyard(Battlefield* battlefield)
{
    ...
    m_SpiritGuideGUID[0].Clear();
    m_SpiritGuideGUID[1].Clear();
}
...
void BfGraveyard::SetSpirit(Creature* spirit, TeamId team)
{
    ...
    m_SpiritGuideGUID[team] = spirit->GetGUID();
...
void BfGraveyard::GiveControlTo(TeamId team)
{
    ...
    TeamId oldTeam = m_ControlTeam;
    m_ControlTeam = team;
    // Teleport to other graveyard, player witch were on this graveyard
    if (oldTeam == TEAM_ALLIANCE || oldTeam == TEAM_HORDE)
    {
        RelocateDeadPlayers(oldTeam);
        if (Creature* spirit = m_Bf->GetCreature(m_SpiritGuideGUID[oldTeam]))
            spirit->CastStop();
    }
}

void BfGraveyard::RelocateDeadPlayers(TeamId teamId)
{
    WorldSafeLocsEntry const* closestGrave = NULL;
    if (Creature* spirit = m_Bf->GetCreature(m_SpiritGuideGUID[teamId]))
    {
        for (GuidSet::const_iterator itr = m_Bf->GetPlayers(teamId).cbegin(); itr != m_Bf->GetPlayers(teamId).cend(); ++itr)
        {
            if (Player* player = ObjectAccessor::GetPlayer(*spirit, *itr))
            {
                if (player->HasAuraType(SPELL_AURA_GHOST))
                {
                    if (player->IsBeingTeleported())
                    {
                        if (spirit->GetDistance(player->GetTeleportDest()) > 20)
                            continue;
                    }
                    else if (!player->IsInRange(spirit, 0, 20))
                        continue;
...
{
    if (!m_SpiritGuideGUID[0] || !m_SpiritGuideGUID[1])
        return false;

    if (!m_Bf->GetCreature(m_SpiritGuideGUID[0]) ||
        !m_Bf->GetCreature(m_SpiritGuideGUID[1]))
        return false;

    return (m_SpiritGuideGUID[0] == guid || m_SpiritGuideGUID[1] == guid);
}
...
Creature* Battlefield::SpawnCreature(uint32 entry, float x, float y, float z, float o, TeamId /*team*/)
{
    ...
    creature->SetRespawnAtHome(true);
...
boost::optional<Creature&> Battlefield::GetCreatureBySpawnId(uint32 spawnId)
{
    if (!m_Map)
        return boost::none;
...
boost::optional<GameObject&> Battlefield::GetGameObjectBySpawnId(uint32 spawnId)
{
    if (!m_Map)
        return boost::none;
    return m_Map->GetGameObjectBySpawnId(spawnId);
}
...
void BfCapturePoint::SendUpdateWorldState(uint32 field, uint32 value)
{
    for (uint8 team = 0; team < 2; ++team)
        for (GuidSet::iterator itr = m_activePlayers[team].begin(); itr != m_activePlayers[team].end(); ++itr)  // send to all players present in the area
            if (Player* player = ObjectAccessor::GetPlayer(*m_Bf->GetMap(), *itr))
                player->SendUpdateWorldState(field, value);
}
