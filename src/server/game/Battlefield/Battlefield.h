...
enum BattlefieldAutoChangeFactionData
{
    WS_BATTLEFIELD_WG_FACTIN_WINS   = 20100, // Custom worldstate (if one faction wins 5games in a row the faction is automaticly changed)
    BATTLEFIELD_WG_MAX_WINS_IN_ROW  = 5
};
...
class BfGraveyard
{
    public:
        ...
        void RelocateDeadPlayers(TeamId teamId);
    ...
    protected:
        ...
        ObjectGuid m_SpiritGuideGUID[BG_TEAMS_COUNT];
        Battlefield* m_Bf;
};
...
class GAME_API Battlefield : public ZoneScript
{
    ...
    public:
        ...
        GuidSet const& GetPlayers(TeamId teamId) const { return m_players[teamId]; }

        // Battlefield - player count get functions
        uint32 GetPlayersInZoneCount(TeamId teamId) { return m_players[teamId].size(); }
        uint32 GetPlayersInQueueCount(TeamId teamId) { return m_PlayersInQueue[teamId].size(); }
        uint32 GetPlayersInWarCount(TeamId teamId) { return m_PlayersInWar[teamId].size(); }
        ...
        virtual void AddPlayerToResurrectQueue(ObjectGuid player_guid);
        void RemovePlayerFromResurrectQueue(ObjectGuid player_guid);
        void SetGraveyardNumber(uint32 number) { m_GraveyardList.resize(number); }
        BfGraveyard* GetGraveyardById(uint32 id) const;
        void SetLastResurrectTime(ObjectGuid guid);
        ...
        boost::optional<Creature&> GetCreatureBySpawnId(uint32 spawnId);
        boost::optional<GameObject&> GetGameObjectBySpawnId(uint32 spawnId);
        ...
        virtual void OnBattleEnd(bool /*endByTimer*/, bool /*autoChangeFaction*/) { }
        ...
        void AskToLeave(Battlefield* bf, Player* player);
    ...
    protected:
        ...
        void BroadcastPacketToZone(const WorldPacket& data) const;
        void BroadcastPacketToQueue(const WorldPacket& data) const;
        void BroadcastPacketToWar(const WorldPacket& data) const;
        ...
        uint32 CalculateStartTime(bool crashRestart = false, bool setup = false, uint32 timer = 0) const;
};
