...
bool BattlefieldWG::SetupBattlefield()
{
    ...
    {
        m_isActive = false;
        m_Timer = CalculateStartTime(true);
    }
    else
        m_Timer = CalculateStartTime(false, true, sWorld->getWorldState(ClockWorldState[0]));

    return true;
}

void BattlefieldWG::DelayedInitialization()
{
    for (uint8 i = 0; i < BATTLEFIELD_WG_GRAVEYARD_MAX; i++)
    {
        ...
        for (uint32 spawnId : WGSpiritGuideSpawnIds)
        {
            if (auto creature = GetCreatureBySpawnId(spawnId))
            {
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_TAUNKA_SPIRIT_GUIDE:
                    {
                        ...
                        if (m_GraveyardList[graveyardId])
                            m_GraveyardList[graveyardId]->SetSpirit(creature.get_ptr(), teamId);
                        SetLastResurrectTime(creature->GetGUID());
                        break;
                    }
                }
            }
    ...
    for (uint8 i = 0; i < WG_MAX_WORKSHOP; i++)
    {
        ...
        switch (i)
        {
            case BATTLEFIELD_WG_WORKSHOP_NE:
                spawnId = GO_WINTERGRASP_FACTORY_BANNER_NE_SI;
                break;
            case BATTLEFIELD_WG_WORKSHOP_NW:
                spawnId = GO_WINTERGRASP_FACTORY_BANNER_NW_SI;
                break;
            case BATTLEFIELD_WG_WORKSHOP_SE:
                spawnId = GO_WINTERGRASP_FACTORY_BANNER_SE_SI;
                break;
            case BATTLEFIELD_WG_WORKSHOP_SW:
                spawnId = GO_WINTERGRASP_FACTORY_BANNER_SW_SI;
                break;
            default:
                continue;
        }

        if (auto go = GetGameObjectBySpawnId(spawnId))
        {
            WintergraspCapturePoint* capturePoint = new WintergraspCapturePoint(this, GetAttackerTeam());

            capturePoint->SetCapturePointData(go.get_ptr());
            capturePoint->LinkToWorkshop(workshop);
            workshop->capturePoint = capturePoint;
            AddCapturePoint(capturePoint);
        }
        else
            TC_LOG_ERROR("bg.battlefield", "BattlefieldWG::SetupBattleground: Cannot find Capturepoint Gameobject Spawn Id: %u", spawnId);
    ...
    // Register all gameobjects
    for (uint8 i = 0; i < WG_MAX_OBJ; i++)
    {
        if (auto go = GetGameObjectBySpawnId(WGGameObjectBuilding[i].spawnId))
        {
            BfWGGameObjectBuilding* b = new BfWGGameObjectBuilding(this);
            b->Init(go.get_ptr(), WGGameObjectBuilding[i].type, WGGameObjectBuilding[i].WorldState, WGGameObjectBuilding[i].nameId);
    ...
    // Fixup worldstates - not sure if there is a better way...
    for (GameObjectBuilding::const_iterator itr = BuildingsInZone.begin(); itr != BuildingsInZone.end(); ++itr)
        if (*itr)
            (*itr)->SetWorldstateAndFaction();
...
bool BattlefieldWG::Update(uint32 diff)
{
    ...
    if (!IsWarTime() && m_announceToWorld)
    {
        uint32 tmpTimer = m_Timer / IN_MILLISECONDS;
        if (tmpTimer < 3600 && tmpTimer > 3550)
            sWorld->SendWorldText(BATTLEFIELD_WG_TEXT_ANNOUNCE_START, 60);
        else if (tmpTimer < 1800 && tmpTimer > 1750)
            sWorld->SendWorldText(BATTLEFIELD_WG_TEXT_ANNOUNCE_START, 30);
        else if (tmpTimer < 900 && tmpTimer > 850)
            sWorld->SendWorldText(BATTLEFIELD_WG_TEXT_ANNOUNCE_START, 15);
        else if (tmpTimer < 600 && tmpTimer > 550)
            sWorld->SendWorldText(BATTLEFIELD_WG_TEXT_ANNOUNCE_START, 10);
        else if (tmpTimer < 300 && tmpTimer > 250)
            sWorld->SendWorldText(BATTLEFIELD_WG_TEXT_ANNOUNCE_START, 5);
        else if (tmpTimer < 60 && tmpTimer > 5)
            sWorld->SendWorldText(BATTLEFIELD_WG_TEXT_ANNOUNCE_START, 1);

        m_announceToWorld = false;
    }
...
void BattlefieldWG::OnBattleStart()
{
    ...
    for (Workshop::const_iterator itr = WorkshopsList.begin(); itr != WorkshopsList.end(); ++itr)
        if (*itr)
            (*itr)->UpdateGraveyardAndWorkshop(true);

    for (uint8 team = 0; team < 2; ++team)
        for (GuidSet::const_iterator itr = m_players[team].begin(); itr != m_players[team].end(); ++itr)
        {
            // Kick player in orb room, TODO: offline player ?
            if (Player* player = sObjectAccessor->GetPlayer(*m_Map, *itr))
            {
                ...
                else
                {
                    if (player->GetTeamId() == TEAM_ALLIANCE)
                        player->TeleportTo(WgAttackerTeleportLoc[0]);
                    else
                        player->TeleportTo(WgAttackerTeleportLoc[1]);
                }
    ...
    //SendWarning(BATTLEFIELD_WG_TEXT_START); TODO fix wintergrasp texts
}
...
void BattlefieldWG::OnBattleEnd(bool endByTimer, bool autoChangeFaction)
{
    m_announceToWorld = true;
    ...
    RewardAllPlayer(endByTimer, autoChangeFaction);

    if (autoChangeFaction)
    ...
    for (GuidSet::const_iterator itr = CanonList.begin(); itr != CanonList.end(); ++itr)
    {
        if (Creature* creature = GetCreature(*itr))
        {
            if (!endByTimer || autoChangeFaction)
                creature->setFaction(WintergraspFaction[GetDefenderTeam()]);
            HideNpc(creature);
        }
    }

    if (!endByTimer || autoChangeFaction) // One player triggered the relic
    {
    ...
    for (Workshop::const_iterator itr = WorkshopsList.begin(); itr != WorkshopsList.end(); ++itr)
        if (*itr)
            (*itr)->UpdateGraveyardAndWorkshop(false);
    ...
    if (!endByTimer || autoChangeFaction) // Captured (Now defending team is victorious)
    ...
    /// @todo Merge with log update code above, also these texts seem to be missing!!
    // TODO fix wintergrasp texts
    /*if (!endByTimer || autoChangeFaction) // win alli/horde
...
void BattlefieldWG::RewardAllPlayer(bool endByTimer, bool autoChangeFaction)
{
    for (GuidSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin(); itr != m_PlayersInWar[GetDefenderTeam()].end(); ++itr)
    {
        if (Player* player = sObjectAccessor->GetPlayer(*m_Map, *itr))
        {
            ...
            if (player->GetQuestStatus(player->GetTeamId() == TEAM_HORDE ? QUEST_WIN_WINTERGRASP_H : QUEST_WIN_WINTERGRASP_A) == QUEST_STATUS_INCOMPLETE)
                player->CompleteQuest(player->GetTeamId() == TEAM_HORDE ? QUEST_WIN_WINTERGRASP_H : QUEST_WIN_WINTERGRASP_A);
            // Send Wintergrasp victory achievement
            DoCompleteOrIncrementAchievement(ACHIEVEMENTS_WIN_WG, player);
            // Award achievement for succeeding in Wintergrasp in 10 minutes or less
            if (!endByTimer && GetTimer() <= 10000 && player->GetTeamId() == GetAttackerTeam())
    ...
    for (uint8 team = 0; team < 2; ++team)
    {
        uint8 towerDamaged = 0;         // for both teams
        uint8 towerDestroyed = 0;       // for both teams
        uint8 damagedBuildingSaved = 0; // only for defender team
        uint8 damagedTowerSaved = 0;    // for both teams
            ...
            if (team == GetDefenderTeam())
            {
                switch ((*itr)->m_Type)
                {
                    ...
                    case BATTLEFIELD_WG_OBJECTTYPE_DOOR_LAST:
                        if ((*itr)->GetGoState() == BATTLEFIELD_WG_GO_STATE_DAMAGED)
                            ++damagedBuildingSaved;
                        break;
                    ...
                    case BATTLEFIELD_WG_OBJECTTYPE_TOWER:
                        if ((*itr)->GetGoState() == BATTLEFIELD_WG_GO_STATE_DAMAGED)
                            ++towerDamaged;
                        else if ((*itr)->GetGoState() == BATTLEFIELD_WG_GO_STATE_DESTOYED)
                            ++towerDestroyed;
                        break;
                    default:
                        break;
        ...
        for (GuidSet::const_iterator itr = m_PlayersInWar[team].begin(); itr != m_PlayersInWar[team].end(); ++itr)
        {
            if (Player* player = sObjectAccessor->GetPlayer(*m_Map, *itr))
            {
                if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_DAMAGED_TOWER))
                {
                    // reward player for every damaged tower. Maximum 750 honor
                    int32 bp = (spellInfo->GetBasePoints(EFFECT_0) / (team == GetDefenderTeam() ? 3 : 4));
                    bp *= towerDamaged;
                    bp = std::min(bp, 750);
                    player->CastCustomSpell(player, spellInfo->Id, &bp, NULL, NULL, true);
                }

                if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_DESTROYED_TOWER))
                {
                    // reward player for every destroyed tower. Maximum 750 honor
                    int32 bp = (spellInfo->GetBasePoints(EFFECT_0) / (team == GetDefenderTeam() ? 3 : 4));
                    bp *= towerDestroyed;
                    bp = std::min(bp, 750);
                    player->CastCustomSpell(player, spellInfo->Id, &bp, NULL, NULL, true);
                ...
                if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_INTACT_TOWER))
                {
                    // reward player for every saved damaged tower. Maximum 750 honor
                    int32 bp = (spellInfo->GetBasePoints(EFFECT_0) / (team == GetDefenderTeam() ? 3 : 4));
                    bp *= towerDestroyed;
                    bp = std::min(bp, 750);
                    player->CastCustomSpell(player, spellInfo->Id, &bp, NULL, NULL, true);
                ...
                if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_INTACT_BUILDING))
                {
                    // reward 18 Honor for retaining control of a damaged building. Maximum 1500
                    int32 bp = 18;
                    bp *= damagedBuildingSaved;
                    bp = std::min(bp, 1500);
                    player->CastCustomSpell(player, spellInfo->Id, &bp, NULL, NULL, true);
                }
            }
...
{
    //SendWarning(BATTLEFIELD_WG_TEXT_WILL_START); TODO fix wintergrasp texts
}
...
// Creatures may be created before SetupBattlefield is called since the grid is loaded first
    ...
    if (IsWarTime())
    {
        switch (creature->GetEntry())
        {
            ...
            case NPC_WINTERGRASP_DEMOLISHER:
            {
                ...
                Player* creator = sObjectAccessor->GetPlayer(*m_Map, creature->ToTempSummon()->GetSummonerGUID());
...
// Objects may be created before SetupBattlefield is called since the grid is loaded first
...
void BattlefieldWG::HandleKill(Player* killer, Unit* victim)
{
    ...
    {
        killer->KilledMonsterCredit(NPC_PVP_KILL_ALLIANCE);
        killer->KilledMonsterCredit(NPC_PVP_KILL_HORDE);
        ...
        if (Creature* creature = GetCreature(*itr))
        {
            if (victim->GetEntry() == creature->GetEntry() && !again)
            {
                ...
                for (GuidSet::const_iterator iter = m_PlayersInWar[killerTeam].begin(); iter != m_PlayersInWar[killerTeam].end(); ++iter)
                    if (Player* player = sObjectAccessor->GetPlayer(*m_Map, *iter))
...
void BattlefieldWG::PromotePlayer(Player* killer)
{
    ...
    if (Aura* auraRecruit = killer->GetAura(SPELL_RECRUIT))
    {
        if (auraRecruit->GetStackAmount() >= 5)
        {
            ...
            //SendWarning(BATTLEFIELD_WG_TEXT_FIRSTRANK, killer); TODO fix wintergrasp texts
        }
        else
    ...
    else if (Aura* auraCorporal = killer->GetAura(SPELL_CORPORAL))
    {
        if (auraCorporal->GetStackAmount() >= 5)
        {
            ...
            //SendWarning(BATTLEFIELD_WG_TEXT_SECONDRANK, killer); TODO fix wintergrasp texts
        }
        else
            killer->CastSpell(killer, SPELL_CORPORAL, true);
    }
...
void BattlefieldWG::OnPlayerEnterZone(Player* player)
{
    if (!m_isActive)
        RemoveAurasFromPlayer(player);
    if (player->IsFlying() && player->IsAlive() && !player->IsGameMaster())
...
{
    WorldPacket data(SMSG_INIT_WORLD_STATES, (4 + 4 + 4 + 2 + (BuildingsInZone.size() * 8) + (WorkshopsList.size() * 8) + 4 * 2 * 4));

    data << uint32(m_MapId);
    data << uint32(m_ZoneId);
    data << uint32(0);
    data << uint16(10 + BuildingsInZone.size() + WorkshopsList.size() + 4 * 2 * 4); // Number of fields
    ...
    if (team == GetAttackerTeam())
    {
        ...
        for (GuidSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin(); itr != m_PlayersInWar[GetDefenderTeam()].end(); ++itr)
            if (Player* player = sObjectAccessor->GetPlayer(*m_Map, *itr))
            {
                ...
                player->KilledMonsterCredit(NPC_TOWER_DESTOY_CREDIT);
            }
...
void BattlefieldWG::ProcessEvent(WorldObject* obj, uint32 eventId)
{
    ...
    for (GameObjectBuilding::const_iterator itr = BuildingsInZone.begin(); itr != BuildingsInZone.end(); ++itr)
    {
        if (auto build = GetGameObjectBySpawnId((*itr)->m_SpawnId))
        {
...
void BattlefieldWG::UpdateTenacity()
{
    ...
    if (hordePlayers > alliancePlayers) // more horde than alliance
    {
        newStack = hordePlayers - alliancePlayers;
        team = TEAM_ALLIANCE;
    }
    else if (alliancePlayers > hordePlayers) // more alliance than horde
    {
        newStack = alliancePlayers - hordePlayers;
        team = TEAM_HORDE;
    }

    newStack *= 0.1471f; // was 0.1071f
    ...
    if (team != TEAM_NEUTRAL)
    {
        ...
        // remove old honor buff
        for (GuidSet::const_iterator itr = m_vehicles[team].begin(); itr != m_vehicles[team].end(); ++itr)
            if (Creature* creature = GetCreature(*itr))
                creature->RemoveAurasDueToSpell(m_tenacityHonorSpell);
    }

    // Dont try to apply buff if stacks are below 1 or no team gets the buff
    if (newStack < 1 || team == TEAM_NEUTRAL)
        return;    

    for (GuidSet::const_iterator itr = m_PlayersInWar[team].begin(); itr != m_PlayersInWar[team].end(); ++itr)
        if (Player* player = sObjectAccessor->GetPlayer(*m_Map, *itr))
            if (player->getLevel() >= m_MinLevel)
                player->SetAuraStack(SPELL_TENACITY, player, uint32(newStack));

    for (GuidSet::const_iterator itr = m_vehicles[team].begin(); itr != m_vehicles[team].end(); ++itr)
        if (Creature* creature = GetCreature(*itr))
            creature->SetAuraStack(SPELL_TENACITY_VEHICLE, creature, uint32(newStack));

    // bonus honor if player have some tenacity stacks
    uint32 tenacityHonorSpell = SPELL_GREATEST_HONOR;
    if (newStack < 10)
        tenacityHonorSpell = SPELL_GREATER_HONOR;
    if (newStack < 4)
        tenacityHonorSpell = SPELL_GREAT_HONOR;
    if (newStack < 1.5f)
        tenacityHonorSpell = 0;

    m_tenacityHonorSpell = tenacityHonorSpell;

    if (tenacityHonorSpell != 0)
    {
        for (GuidSet::const_iterator itr = m_PlayersInWar[team].begin(); itr != m_PlayersInWar[team].end(); ++itr)
            if (Player* player = sObjectAccessor->GetPlayer(*m_Map, *itr))
                player->CastSpell(player, tenacityHonorSpell, true);
    }
}
