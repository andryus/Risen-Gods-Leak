...
bool ChatHandler::HasLowerSecurity(Player* target, ObjectGuid guid, bool strong)
{
    ...
    if (target)
        ...
        target_account = player::PlayerCache::GetAccountId(guid);
...
void ChatHandler::SendSysMessage(const char *str) const
{
    ...
    while (char* line = LineFromMessage(pos))
    {
        ...
        m_session->SendPacket(std::move(data));
    }
...
void ChatHandler::SendGlobalGMSysMessage(const char *str) const
{
    // Chat output
    WorldPacket data(0);
...
void ChatHandler::SendSysMessage(int32 entry) const
{
    SendSysMessage(GetTrinityString(entry));
}

bool ChatHandler::isValidChatMessage(char const* message) const
{
/*
Valid examples:
...
Creature* ChatHandler::getSelectedCreature() const
{
    ...
    return ObjectAccessor::GetCreature(*m_session->GetPlayer(), m_session->GetPlayer()->GetTarget());
}
...
Player* ChatHandler::getSelectedPlayerOrSelf() const
{
    ...
    Player* targetPlayer = ObjectAccessor::GetPlayer(*m_session->GetPlayer(), selected);
...
char* ChatHandler::extractKeyFromLink(char* text, char const* linkType, char** something1) const
{
    // skip empty
    if (!text)
        return NULL;
...
char* ChatHandler::extractKeyFromLink(char* text, char const* const* linkTypes, int* found_idx, char** something1) const
{
    // skip empty
    if (!text)
        return NULL;
...
GameObject* ChatHandler::GetNearbyGameObject() const
{
    ...
    auto searcher = Trinity::createBaseObjectSearcher<GameObject, Trinity::LastAction>(pl, obj, check);
    pl->GetMap()->Visit(pl->GetPositionX(), pl->GetPositionY(), SIZE_OF_GRIDS, searcher);
    return obj;
}

GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry) const
{
    if (!m_session)
        return NULL;
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        obj = pl->VisitSingleNearbyObject<GameObject, true>(pl->GetGridActivationRange(), go_check);
    }
...
uint32 ChatHandler::extractSpellIdFromLink(char* text) const
{
...
GameTele const* ChatHandler::extractGameTeleFromLink(char* text, bool exactName /* = false */) const
{
    ...
    if (exactName)
        return sObjectMgr->GetGameTeleExactName(cId);
    else
...
ObjectGuid ChatHandler::extractGuidFromLink(char* text) const
{
    int type = 0;
...
std::string ChatHandler::extractPlayerNameFromLink(char* text) const
{
    // |color|Hplayer:name|h[name]|h|r
...
bool ChatHandler::extractPlayerTarget(char* args, Player** player, ObjectGuid* player_guid /*=NULL*/, std::string* player_name /*= NULL*/)
{
    if (args && *args)
    {
        ...
        if (player_guid)
        {
            *player_guid = pCacheEntry.guid;
            valid = *player_guid != ObjectGuid::Empty;
        }
        if (player_name)
        {
            *player_name = pCacheEntry.name;
            valid = true;
        }
        
        if (player)
        {
            *player = ObjectAccessor::FindPlayer(pCacheEntry.guid);
            if (*player != nullptr)
                valid = true;
        }
...
bool CliHandler::HasPermission(uint32 permission) const {
    // command line
    if (!m_accountId)
        return true;

    // SOAP
    uint8 secLevel = AccountMgr::GetSecurity(m_accountId);

    rbac::RBACData* _RBACData = new rbac::RBACData(m_accountId, m_accountName, realm.Id.Realm, secLevel);
    _RBACData->LoadFromDB();

    return _RBACData->HasPermission(permission);
}

void CliHandler::SendSysMessage(const char* str) const
{
