...
void Guild::RankInfo::CreateMissingTabsIfNeeded(SQLTransaction& trans, uint8 tabs, bool logOnCreate /* = false */)
{
...
void Guild::RankInfo::SetName(SQLTransaction& trans, std::string const& name)
{
    if (m_name == name)
        return;
...
void Guild::RankInfo::SetRights(SQLTransaction& trans, uint32 rights)
{
    if (m_rankId == GR_GUILDMASTER)                     // Prevent loss of leader rights
        rights = GR_RIGHT_ALL;
...
void Guild::RankInfo::SetBankMoneyPerDay(SQLTransaction& trans, uint32 money)
{
    if (m_rankId == GR_GUILDMASTER)                     // Prevent loss of leader rights
...
void Guild::RankInfo::SetBankTabSlotsAndRights(SQLTransaction& trans, GuildBankRightsAndSlots rightsAndSlots, bool saveToDB)
{
    if (m_rankId == GR_GUILDMASTER)                     // Prevent loss of leader rights
        rightsAndSlots.SetGuildMasterValues();
...
void Guild::BankTab::SetInfo(SQLTransaction& trans, std::string const& name, std::string const& icon)
{
...
void Guild::BankTab::SetText(SQLTransaction& trans, std::string const& text)
{
    if (m_text == text)
        return;
...
// Member
void Guild::Member::SetPublicNote(SQLTransaction& trans, std::string const& publicNote)
{
...
void Guild::Member::SetOfficerNote(SQLTransaction& trans, std::string const& officerNote)
{
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    m_logoutTime = fields[12].GetUInt32();                  // characters.logout_time
...
{
    Player* target = ObjectAccessor::FindPlayer(m_guid);

    if (target)
    {
    data << uint64(m_guid)
         << uint8((target->IsSecretQueuingActive()) ? static_cast<uint8>(GUILDMEMBER_STATUS_NONE) : m_flags) //RG Custom "Verstecktes Anmelden"
         << target->GetName()
         << uint32(m_rankId)
         << uint8(target->getLevel())
         << uint8(target->getClass())
         << uint8(0);

    //RG Custom "Verstecktes Anmelden"
    if ((!target->InArena() && !target->IsSecretQueuingActive()))
        data << uint32(target->GetZoneId());
    else
    {
        if (!target->IsSecretQueuingActive())
        {
            //! RG Custom - show old position at guildmemberlist for arena
            WorldLocation loc = target->GetBGEntryPoint();
            data << sMapMgr->GetZoneId(loc.GetMapId(), loc.GetPositionX(), loc.GetPositionY(), loc.GetPositionZ());
        }
        else
            data << uint32(4395); //Dalaran
    }

    if (!m_flags || (target->IsSecretQueuingActive()))
        data << float(float(::time(nullptr) - m_logoutTime) / DAY);
}
    else
    {
        auto entry = player::PlayerCache::Get(m_guid);
        if (entry)
        {
            data << uint64(m_guid)
                 << uint8(m_flags) //RG Custom "Verstecktes Anmelden"
                 << entry.name
                 << uint32(m_rankId)
                 << uint8(entry.level)
                 << uint8(entry.class_)
                 << uint8(0)
                 << uint32(entry.zoneId)
                 << float(float(::time(nullptr) - m_logoutTime) / DAY);
        }
        else
        {
            data << uint64(m_guid)
                 ...
                 << uint8(0)
                 << uint32(0)
                 << float(float(::time(NULL) - m_logoutTime) / DAY);
        }
    }
    data << m_publicNote;
...
void EmblemInfo::SaveToDB(SQLTransaction& trans, uint32 guildId) const
{
...
Item* Guild::PlayerMoveItemData::StoreItem(SQLTransaction& trans, Item* pItem)
{
    ...
    // RG-Custom Item logging
    // Item can be invalidated during moving process
    RG_LOG<ItemLogModule>(pItem, RG::Logging::ItemLogType::FROM_GUILDBANK, 0, m_pPlayer->GetGUID().GetRawValue(), m_pGuild->GetId());
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    if (!pFrom->IsBank() && m_pPlayer->GetSession()->HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE)) /// @todo Move this to scripts
    {
        sLog->outCommand(m_pPlayer->GetSession()->GetAccountId(),
            ...
            m_pGuild->m_name.c_str(), m_pGuild->GetId());
    }
}
...
// Guild
Guild::Guild(uint32 id, ObjectGuid leaderGuid, std::string const& name)
    :
    m_id(id),
...
bool Guild::Create(SQLTransaction& trans, Player* pLeader)
{
    m_info = "";
    ...
    TC_LOG_DEBUG("guild", "GUILD: creating guild [%s] for leader %s (%u)",
        m_name.c_str(), pLeader->GetName().c_str(), m_leaderGuid.GetCounter());
...
void Guild::Disband(SQLTransaction& trans)
{
...
bool Guild::SetName(SQLTransaction& trans, std::string const& name)
{
...
void Guild::HandleRoster(WorldSession* session) const
{
    uint32 membersize = std::min(uint32(m_members.size()), sWorld->getIntConfig(CONFIG_GUILD_MAX_PLAYER_SHOWN));

     // Guess size
     WorldPacket data(SMSG_GUILD_ROSTER, (4 + m_motd.length() + 1 + m_info.length() + 1 + 4 + _GetRanksSize() * (4 + 4 + GUILD_BANK_MAX_TABS * (4 + 4)) + membersize * 50));
     ...
     // first show player which are Online, max player from config
     for (Members::const_iterator itr = m_members.begin(); itr != m_members.end() && membercount < sWorld->getIntConfig(CONFIG_GUILD_MAX_PLAYER_SHOWN); ++itr)
         if (itr->second->IsOnline())
         {
             itr->second->WritePacket(data, _HasRankRight(session->GetPlayer(), GR_RIGHT_VIEWOFFNOTE));
             membercount++;
         }
     // show offline players if not max reached
     for (Members::const_iterator itr = m_members.begin(); itr != m_members.end() && membercount < sWorld->getIntConfig(CONFIG_GUILD_MAX_PLAYER_SHOWN); ++itr)
         if (!itr->second->IsOnline())
         {
             itr->second->WritePacket(data, _HasRankRight(session->GetPlayer(), GR_RIGHT_VIEWOFFNOTE));
...
void Guild::HandleQuery(WorldSession* session) const
{
...
{
    Player* mutator = session->GetPlayer();
    assert(mutator);
    ...
    if (!_HasRankRight(mutator, GR_RIGHT_SETMOTD))
...
void Guild::HandleSetInfo(WorldSession* session, std::string const& info)
{
    ...
    if (_HasRankRight(mutator, GR_RIGHT_MODIFY_GUILD_INFO))
    {
        m_info = info;
...
void Guild::HandleSetEmblem(WorldSession* session, const EmblemInfo& emblemInfo)
{
    ...
    else
    {
        ...
        m_emblemInfo.SaveToDB(trans, m_id);
...
void Guild::HandleSetLeader(WorldSession* session, std::string const& name)
{
    ...
    else if (Member* pOldLeader = GetMember(player->GetGUID()))
    {
        ...
        {
            SQLTransaction trans = CharacterDatabase.BeginTransaction();
            _SetLeaderGUID(trans, pNewLeader);
...
void Guild::HandleSetMemberNote(WorldSession* session, std::string const& name, std::string const& note, bool officer)
{
    ...
    if (!_HasRankRight(player, officer ? GR_RIGHT_EOFFNOTE : GR_RIGHT_EPNOTE))
        SendCommandResult(session, GUILD_COMMAND_PUBLIC_NOTE, ERR_GUILD_PERMISSIONS);
    else if (Member* member = GetMember(name))
    {
        auto trans = CharacterDatabase.BeginTransaction();
        if (officer)
            member->SetOfficerNote(trans, note);
        else
            member->SetPublicNote(trans, note);
...
void Guild::HandleSetRankInfo(WorldSession* session, uint8 rankId, std::string const& name, uint32 rights, uint32 moneyPerDay, const GuildBankRightsAndSlotsVec& rightsAndSlots)
{
    ...
    else if (RankInfo* rankInfo = GetRankInfo(rankId))
    {
        ...
        auto trans = CharacterDatabase.BeginTransaction();
        rankInfo->SetName(trans, name);
        rankInfo->SetRights(trans, rights);
        _SetRankBankMoneyPerDay(trans, rankId, moneyPerDay);

        for (GuildBankRightsAndSlotsVec::const_iterator itr = rightsAndSlots.begin(); itr != rightsAndSlots.end(); ++itr)
            _SetRankBankTabRightsAndSlots(trans, rankId, *itr);
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    //RG Custom "Verstecktes Anmelden"
    if (pInvitee->IsSecretQueuingActive())
    {
        SendCommandResult(session, GUILD_COMMAND_INVITE, ERR_GUILD_PLAYER_NOT_FOUND_S, name);
        return;
    }
    ...
    SQLTransaction trans = CharacterDatabase.BeginTransaction();
    _LogEvent(trans, GUILD_EVENT_LOG_INVITE_PLAYER, player->GetGUID().GetCounter(), pInvitee->GetGUID().GetCounter());
    CharacterDatabase.CommitTransaction(trans);

    WorldPacket data(SMSG_GUILD_INVITE, 8 + 10);              // Guess size
    data << player->GetName();
    data << m_name;
    pInvitee->SendDirectMessage(std::move(data));
...
void Guild::HandleAcceptMember(WorldSession* session)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
        player->GetTeam() != player::PlayerCache::GetTeam(GetLeaderGUID()))
        return;
...
void Guild::HandleLeaveMember(WorldSession* session)
{
    ...
    {
        DeleteMember(trans, player->GetGUID(), false);

        _LogEvent(trans, GUILD_EVENT_LOG_LEAVE_GUILD, player->GetGUID().GetCounter());
    ...
    else if (Member* member = GetMember(name))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                DeleteMember(trans, guid, false);
                _LogEvent(trans, GUILD_EVENT_LOG_UNINVITE_PLAYER, player->GetGUID().GetCounter(), guid.GetCounter());
...
void Guild::HandleUpdateMemberRank(WorldSession* session, std::string const& name, bool demote)
{
    ...
    else if (Member* member = GetMember(name))
    {
        ...
        _LogEvent(trans, demote ? GUILD_EVENT_LOG_DEMOTE_PLAYER : GUILD_EVENT_LOG_PROMOTE_PLAYER, player->GetGUID().GetCounter(), member->GetGUID().GetCounter(), newRankId);
...
{
    RemoveRank(session, _GetLowestRankId());
}

void Guild::RemoveRank(WorldSession* session, uint8 rankId)
{
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    player->ModifyMoney(-int32(amount), ::RG::Logging::MoneyLogType::GUILD_DEPOSIT, m_id);
    player->SaveGoldToDB(trans);
    ...
    if (session->HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
    {
        sLog->outCommand(session->GetAccountId(),
            ...
            player->GetName().c_str(), session->GetAccountId(), amount, m_id);
    }
}
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint32 amount, bool repair)
{
   ...
   if (uint32(_GetMemberRemainingMoney(member)) < amount)   // Check if we have enough slot/money today
    ...
    if (!repair)
    {
        if (!player->ModifyMoney(amount, ::RG::Logging::MoneyLogType::GUILD_WITHDRAW, m_id))
            return false;
...
void Guild::SendInfo(WorldSession* session) const
{
    ...
    data << m_name;
    data << asPackedTime(m_createdDate);           // 3.x (prev. year + month + day)
...
{
    if (m_bankState != BankState::LOADED)
        return;
...
{
    if (m_bankState == BankState::NOT_LOADED)
    {
        m_bankState = BankState::LOADING;
        m_playerToSendBank[session->GetPlayer()->GetGUID()] = sendAllSlots;

        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILD_BANK_ITEMS);
        stmt->setUInt32(0, m_id);

        CharacterDatabase.AsyncQuery(stmt)
                .via(sWorld->GetExecutor())
                .then([guildId = m_id](PreparedQueryResult result) { Guild::LoadBankItemsFromDB(guildId, std::move(result)); })
                .then([guildId = m_id]() {
                    if (auto guild = sGuildMgr->GetGuildById(guildId))
                        for (const auto& [guid, allSlots] : guild->m_playerToSendBank)
                            if (auto player = ObjectAccessor::FindPlayer(guid))
                                guild->SendBankTabsInfo(player->GetSession(), allSlots);
                });
    }
    else if (m_bankState == BankState::LOADING)
    {
        m_playerToSendBank[session->GetPlayer()->GetGUID()] = sendAllSlots;
    }
    else /* if (m_bankState == BankState::LOADED) */
    {
...
bool Guild::LoadMemberFromDB(SQLTransaction& trans, Field* fields, ObjectGuid guid)
{
    Member *member = new Member(m_id, guid, fields[2].GetUInt8());
    if (!player::PlayerCache::Exists(guid) || !member->LoadFromDB(fields))
    {
        _DeleteMemberFromDB(trans, guid.GetCounter());
        delete member;
        return false;
    }
    m_members[guid.GetCounter()] = member;
    return true;
}

void Guild::LoadBankRightFromDB(SQLTransaction& trans, Field* fields)
{
                                           // tabId              rights                slots
    GuildBankRightsAndSlots rightsAndSlots(fields[1].GetUInt8(), fields[3].GetUInt8(), fields[4].GetUInt32());
                                         // rankId
    _SetRankBankTabRightsAndSlots(trans, fields[2].GetUInt8(), rightsAndSlots, false);
}
...
void Guild::LoadBankItemsFromDB(uint32 guildId, PreparedQueryResult result)
{
    ...
    guild->m_bankState = BankState::LOADED;
}
...
bool Guild::Validate()
{
    ...
    else
    {
        for (uint8 rankId = 0; rankId < ranks; ++rankId)
        {
            ...
            {
                rankInfo->CreateMissingTabsIfNeeded(trans, _GetPurchasedTabsSize(), true);
            }
        }
    }
...
void Guild::BroadcastPacketToRank(const WorldPacket& packet, uint8 rankId) const
{
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
...
void Guild::BroadcastPacket(const WorldPacket& packet) const
{
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    ...
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ...
        if (count >= CALENDAR_MAX_INVITES)
        {
            if (Player* player = session->GetPlayer())
                CalendarMgr::SendCalendarCommandResult(*player, CALENDAR_ERROR_INVITES_EXCEEDED);
            return;
        }

        Member* member = itr->second;
        uint32 level = player::PlayerCache::GetLevel(member->GetGUID());
...
bool Guild::AddMember(SQLTransaction& trans, ObjectGuid guid, uint8 rankId)
{
    ...
    else if (player::PlayerCache::GetGuildId(guid) != 0)
        return false;
    ...
    _UpdateAccountsNumber();
    _LogEvent(trans, GUILD_EVENT_LOG_JOIN_GUILD, lowguid);
    _BroadcastEvent(GE_JOINED, guid, name.c_str());

    // Call scripts if member was succesfully added (and stored to database)
    sScriptMgr->OnGuildAddMember(this, player, rankId);

    player::PlayerCache::UpdateGuild(guid, GetId());

    return true;
}

void Guild::DeleteMember(SQLTransaction& trans, ObjectGuid guid, bool isDisbanding, bool isKicked)
{
    uint32 lowguid = guid.GetCounter();
    Player* player = ObjectAccessor::FindConnectedPlayer(guid);
    ...
    if (m_leaderGuid == guid && !isDisbanding)
    {
        ...
        _SetLeaderGUID(trans, newLeader);
        ...
        if (oldLeader)
        {
            std::string oldLeaderName = player::PlayerCache::GetName(guid);
            std::string newLeaderName = player::PlayerCache::GetName(newLeader->GetGUID());

            _BroadcastEvent(GE_LEADER_CHANGED, ObjectGuid::Empty, oldLeaderName.c_str(), newLeaderName.c_str());
            _BroadcastEvent(GE_LEFT, guid, oldLeaderName.c_str());
        }
    }
    // Call script on remove before member is actually removed from guild (and database)
    sScriptMgr->OnGuildRemoveMember(this, player, isDisbanding, isKicked);
    player::PlayerCache::UpdateGuild(guid, 0);
...
void Guild::_CreateNewBankTab(SQLTransaction& trans)
{
    ...
    for (Ranks::iterator itr = m_ranks.begin(); itr != m_ranks.end(); ++itr)
        (*itr).CreateMissingTabsIfNeeded(trans, tabId, false);
}
...
bool Guild::_CreateRank(SQLTransaction& trans, std::string const& name, uint32 rights)
{
    ...
    info.CreateMissingTabsIfNeeded(trans, _GetPurchasedTabsSize());
    info.SaveToDB(trans);
...
void Guild::_UpdateAccountsNumber()
{
    ...
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
        accountsIdSet.insert(player::PlayerCache::GetAccountId(itr->second->GetGUID()));
...
void Guild::_SetLeaderGUID(SQLTransaction& trans, Member* pLeader)
{
    if (!pLeader)
        return;
...
void Guild::_SetRankBankMoneyPerDay(SQLTransaction& trans, uint8 rankId, uint32 moneyPerDay)
{
    if (RankInfo* rankInfo = GetRankInfo(rankId))
        rankInfo->SetBankMoneyPerDay(trans, moneyPerDay);
}

void Guild::_SetRankBankTabRightsAndSlots(SQLTransaction& trans, uint8 rankId, GuildBankRightsAndSlots rightsAndSlots, bool saveToDB)
{
    if (rightsAndSlots.GetTabId() >= _GetPurchasedTabsSize())
        return;

    if (RankInfo* rankInfo = GetRankInfo(rankId))
        rankInfo->SetBankTabSlotsAndRights(trans, rightsAndSlots, saveToDB);
}
...
inline void Guild::_LogEvent(SQLTransaction& trans, GuildEventLogTypes eventType, uint32 playerGuid1, uint32 playerGuid2, uint8 newRank)
{
...
bool Guild::_DoItemsMove(MoveItemData* pSrc, MoveItemData* pDest, bool sendError, uint32 splitedAmount)
{
    ...
    // RG-Custom Item logging, only log if player -> Bank, not Bank -> Bank
    if (!pSrc->IsBank())
        RG_LOG<ItemLogModule>(pSrc->GetItem(), RG::Logging::ItemLogType::TO_GUILDBANK, splitedAmount, 0, GetId());
...
/*
@auth Erimioa
Return the Member with the highest time be loged out. 
Important! The guild lead is never returned, because he is not a normal Member
*/
ObjectGuid Guild::GetMaxDaysOfflineMember()
{
    ObjectGuid maxDaysOfflineMember;
    float maxDaysOffline = 0.0f;

    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ObjectGuid delPlayer = itr->second->GetGUID();
        float daysOffline = float(float(time(NULL) - itr->second->GetLogoutTime()) / DAY);

        if (daysOffline > maxDaysOffline && itr->second->GetRankId() > 0)
        {
            maxDaysOffline = daysOffline;
            maxDaysOfflineMember = delPlayer;
        }
    }

    return maxDaysOfflineMember;
}
...
If (tabId == GUILD_BANK_MAX_TABS) decrease money amount.
Otherwise decrease remaining items amount for specified tab.
If (amount == 0) amount = rest of possible BankWithdrawValue
*/
void Guild::UpdateBankWithdrawValueForMember(SQLTransaction& trans, ObjectGuid playerGuid, uint8 tabId, uint32 amount)
{
    if (!playerGuid)
        return;
    ...
    if (Member* member = GetMember(playerGuid))
    {
        ...
        {
            if (tabId == GUILD_BANK_MAX_TABS)
                amount = _GetRankBankMoneyPerDay(rankId);
            else 
                amount = _GetRankBankTabSlotsPerDay(rankId, tabId);
        }

        member->UpdateBankWithdrawValue(trans, tabId, amount);
    }
}
