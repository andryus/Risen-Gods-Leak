...
public:
    ...
    void SaveToDB(SQLTransaction& trans, uint32 guildId) const;
...
private:
    ...
    public:
        ...
        void SetPublicNote(SQLTransaction& trans, std::string const& publicNote);
        void SetOfficerNote(SQLTransaction& trans, std::string const& officerNote);
    ...
    public:
        ...
        void SetName(SQLTransaction& trans, std::string const& name);

        uint32 GetRights() const { return m_rights; }
        void SetRights(SQLTransaction& trans, uint32 rights);

        int32 GetBankMoneyPerDay() const { return m_bankMoneyPerDay; }

        void SetBankMoneyPerDay(SQLTransaction& trans, uint32 money);
        ...
        void SetBankTabSlotsAndRights(SQLTransaction& trans, GuildBankRightsAndSlots rightsAndSlots, bool saveToDB);
        void CreateMissingTabsIfNeeded(SQLTransaction& trans, uint8 ranks, bool logOnCreate = false);
    ...
    public:
        ...
        void SetInfo(SQLTransaction& trans, std::string const& name, std::string const& icon);
        void SetText(SQLTransaction& trans, std::string const& text);
...
public:
    using GuildPtr = util::synchronized_ptr<Guild, Lock>;
    ...
    Guild(uint32 id, ObjectGuid leaderGuid, std::string const& name);
    ~Guild();
    ...
    bool SetName(SQLTransaction& trans, std::string const& name);

    // Handle client commands
    void HandleRoster(WorldSession* session) const;
    void HandleQuery(WorldSession* session) const;
    ...
    void UpdateMemberData(ObjectGuid guid, uint8 dataid, uint32 value);
    ...
    void BroadcastPacketToRank(const WorldPacket& packet, uint8 rankId) const;
    void BroadcastPacket(const WorldPacket& packet) const;
    ...
    void DeleteMember(SQLTransaction& trans, ObjectGuid guid, bool isDisbanding = false, bool isKicked = false);
    bool ChangeMemberRank(SQLTransaction& trans, ObjectGuid guid, uint8 newRank);
    ObjectGuid GetMaxDaysOfflineMember();
    ...
    void UpdateBankWithdrawValueForMember(SQLTransaction& trans, ObjectGuid playerGuid, uint8 tabId, uint32 amount = 0);
...
private:
    bool Create(SQLTransaction& trans, Player* pLeader);

    // Load from DB
    bool LoadFromDB(Field* fields);
    void LoadRankFromDB(Field* fields);
    bool LoadMemberFromDB(SQLTransaction& trans, Field* fields, ObjectGuid guid);
    bool LoadEventLogFromDB(Field* fields);
    void LoadBankRightFromDB(SQLTransaction& trans, Field* fields);
    void LoadBankTabFromDB(Field* fields);
    bool LoadBankEventLogFromDB(Field* fields);
    static void LoadBankItemsFromDB(uint32 guildId, PreparedQueryResult result);
    bool LoadBankItemFromDB(Field* fields);
    bool Validate();

    void RemoveRank(WorldSession* session, uint8 rankId);
    ...
    {
        return GetMember(player::PlayerCache::GetGUID(name));
    }

    inline void _DeleteMemberFromDB(SQLTransaction& trans, uint32 lowguid) const
    {
    ...
    void _SetLeaderGUID(SQLTransaction& trans, Member* pLeader);

    void _SetRankBankMoneyPerDay(SQLTransaction& trans, uint8 rankId, uint32 moneyPerDay);
    void _SetRankBankTabRightsAndSlots(SQLTransaction& trans, uint8 rankId, GuildBankRightsAndSlots rightsAndSlots, bool saveToDB = true);
    ...
    void _LogEvent(SQLTransaction& trans, GuildEventLogTypes eventType, uint32 playerGuid1, uint32 playerGuid2 = 0, uint8 newRank = 0);
    ...
    enum class BankState : uint8
    {
        NOT_LOADED,
        LOADING,
        LOADED,
    };
    BankState m_bankState = BankState::NOT_LOADED;
    std::unordered_map<ObjectGuid, bool> m_playerToSendBank;
};
#endif
