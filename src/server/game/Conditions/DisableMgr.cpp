...
{
    template<>
    struct hash<DisableType>
    {
        typedef DisableType argument_type;
        typedef std::size_t result_type;
        result_type operator()(argument_type const& s) const
        {
            return std::hash<uint32>()(static_cast<uint32>(s));
        }
    };
}
...
namespace
{
    ...
    typedef std::unordered_map<DisableType, DisableTypeMap> DisableMap;
        ...
        switch (type)
        {
            ...
            case DISABLE_TYPE_MMAP:
            {
                switch (flags)
                {
                    ...
                    case MMAP_DISABLE_FOR_CREATURE:
                        {
                            ...
                            TC_LOG_INFO("misc", "Pathfinding disabled for creature with entry %u.", entry);
                            break;
                        }
                    default:
                        TC_LOG_ERROR("sql.sql", "Disable flags specified for MMap %u, undefined data.", entry);
                        break;
                }
                break;
            }
            case DISABLE_TYPE_GO_LOS:
            {
                ...
                {
                    TC_LOG_ERROR("sql.sql", "Gameobject entry %u from `disables` doesn't exist, skipped.", entry);
                    continue;
                }
                if (flags)
                    TC_LOG_ERROR("sql.sql", "Disable flags specified for Gameobject %u, useless data.", entry);
            }
            default:
                break;
        }
...
std::pair<bool, uint8> GetDisableFlags(DisableType type, uint32 entry)
{
    ASSERT(type < MAX_DISABLE_TYPES);
    if (m_DisableMap[type].empty())
        return{ false, 0 };
    ...
    switch (type)
    {
    case DisableType::DISABLE_TYPE_VMAP:
        return{ true, itr->second.flags };
    default:
        return{ false, 0 };
    }
}
