...
Quest::Quest(Field* questRecord)
{
    ...
    RewardArenaPoints = questRecord[42].GetInt16();
...
uint32 Quest::XPValue(Player* player) const
{
    if (player)
    {
        ...
        /* RG CUSTOM: Double XP if Classic Quest AND player level between 20, 60 
        and the player haven't the flag, that he don't want extra XP*/
        if (player->getLevel() >= 20 && player->getLevel() <= 60 && !(this->Flags & QUEST_FLAGS_TBC) && !player->HasCustomFlag(Player::XP_NORMAL))
            xp = 2*xp;
