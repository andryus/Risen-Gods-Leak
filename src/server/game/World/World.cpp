...
#include "RG/Rebase/ItemRebase.hpp"
...
constexpr float DEFAULT_VISIBILITY_INSTANCE = 170.0f; // default visible distance in instances, 170 yards
constexpr float DEFAULT_VISIBILITY_BGARENAS = 533.0f; // default visible distance in BG/Arenas, roughly 533 yards
...
bool   World::_afActive     = false;
uint32 World::_afMinActive  = 10;
uint32 World::_afMaxActive  = 1000;
uint32 World::_afMaxProcess = 100;

/// World constructor
World::World() :
    normalThreadPool(std::min((unsigned int)1, std::thread::hardware_concurrency())),
    lowPrioThreadPool(1), httpRequestThreadPool(1), diffTimer(UpdateTimer::WORLD_DECAY)
{
    m_playerLimit = 0;
    ...
    _executor = std::make_shared<folly::ManualExecutor>();
}
...
void World::SetStartguildName(TeamId team, std::string name)
{
    ASSERT(team == TEAM_ALLIANCE || team == TEAM_HORDE);
    _startguild_names[team] = std::move(name);
}

std::string const& World::GetStartguildName(TeamId team) const
{
    ASSERT(team == TEAM_ALLIANCE || team == TEAM_HORDE);
    return _startguild_names[team];
}
...
void World::LoadConfigSettings(bool reload)
{
    ...
    ///- Read the startguild names from the config file
    SetStartguildName(TEAM_ALLIANCE, sConfigMgr->GetStringDefault("RG.StartguildAllianz", ""));
    SetStartguildName(TEAM_HORDE, sConfigMgr->GetStringDefault("RG.StartguildHorde", ""));

    //- Read the account verify secret
    SetAccountVerifySecret(sConfigMgr->GetStringDefault("RG.AccountVerifySecret", "youShallNotPass"));
    ...
    rate_values[RATE_CREATURE_INSTANCE_AGGRO]        = sConfigMgr->GetFloatDefault("Rate.Creature.Instance.Aggro", 1.0f); // RG-Custom, if Rate.Creature.Aggro != 1;
    ...
    m_bool_configs[CONFIG_UPDATE_ALL_CELLS] = sConfigMgr->GetBoolDefault("UpdateAllCells", false);
    ...
    m_int_configs[CONFIG_INTERVAL_MAPUPDATE] = sConfigMgr->GetIntDefault("MapUpdateInterval", 500);
    ...
    m_float_configs[CONFIG_GROUP_LOOT_DISTANCE] = sConfigMgr->GetFloatDefault("MaxGroupLootDistance", 110.0f);
    ...
    m_int_configs[CONFIG_ARENA_MAX_RATING_DIFFERENCE_2V2]            = sConfigMgr->GetIntDefault ("Arena.MaxRatingDifference2v2", 150);
    m_int_configs[CONFIG_ARENA_MAX_RATING_DIFFERENCE_3V3]            = sConfigMgr->GetIntDefault ("Arena.MaxRatingDifference3v3", 150);
    m_int_configs[CONFIG_ARENA_MAX_RATING_DIFFERENCE_5V5]            = sConfigMgr->GetIntDefault ("Arena.MaxRatingDifference5v5", 150);
    m_int_configs[CONFIG_ARENA_RATING_DISCARD_TIMER_2V2]             = sConfigMgr->GetIntDefault ("Arena.RatingDiscardTimer2v2", 10 * MINUTE * IN_MILLISECONDS);
    m_int_configs[CONFIG_ARENA_RATING_DISCARD_TIMER_3V3]             = sConfigMgr->GetIntDefault ("Arena.RatingDiscardTimer3v3", 10 * MINUTE * IN_MILLISECONDS);
    m_int_configs[CONFIG_ARENA_RATING_DISCARD_TIMER_5V5]             = sConfigMgr->GetIntDefault ("Arena.RatingDiscardTimer5v5", 10 * MINUTE * IN_MILLISECONDS);
    m_int_configs[CONFIG_ARENA_WIDENING_TIMER_2V2]                   = sConfigMgr->GetIntDefault ("Arena.RatingWideningTimer2v2", 30 * IN_MILLISECONDS);
    m_int_configs[CONFIG_ARENA_WIDENING_TIMER_3V3]                   = sConfigMgr->GetIntDefault ("Arena.RatingWideningTimer3v3", 30 * IN_MILLISECONDS);
    m_int_configs[CONFIG_ARENA_WIDENING_TIMER_5V5]                   = sConfigMgr->GetIntDefault ("Arena.RatingWideningTimer5v5", 30 * IN_MILLISECONDS);
    m_int_configs[CONFIG_ARENA_WIDENING_DELTA_2V2]                   = sConfigMgr->GetIntDefault ("Arena.RatingWideningDelta2v2", 100);
    m_int_configs[CONFIG_ARENA_WIDENING_DELTA_3V3]                   = sConfigMgr->GetIntDefault ("Arena.RatingWideningDelta3v3", 100);
    m_int_configs[CONFIG_ARENA_WIDENING_DELTA_5V5]                   = sConfigMgr->GetIntDefault ("Arena.RatingWideningDelta5v5", 100);
    ...
    m_int_configs[CONFIG_ARENA_START_RATING_2V2]                     = sConfigMgr->GetIntDefault ("Arena.ArenaStartRating2v2", 0);
    m_int_configs[CONFIG_ARENA_START_RATING_3V3]                     = sConfigMgr->GetIntDefault ("Arena.ArenaStartRating3v3", 0);
    m_int_configs[CONFIG_ARENA_START_RATING_5V5]                     = sConfigMgr->GetIntDefault ("Arena.ArenaStartRating5v5", 0);
    m_bool_configs[CONFIG_ARENA_SEASON_IN_PROGRESS]                  = sConfigMgr->GetBoolDefault("Arena.ArenaSeason.InProgress", true);
    m_int_configs[CONFIG_ARENA_MMR_COMPOSITE_TYPE]                   = sConfigMgr->GetIntDefault ("Arena.MMRCompositeType", 1);
    ...
    m_float_configs[CONFIG_BG_MATCHMAKING_PLAYER_COUNT]              = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.PlayerCount", 2.f);
    m_float_configs[CONFIG_BG_MATCHMAKING_PLAYER_DIFF]               = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.PlayerDiff", 3.f);
    m_float_configs[CONFIG_BG_MATCHMAKING_HEALER_DIFF]               = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.HealerDiff", 0.1f);
    m_float_configs[CONFIG_BG_MATCHMAKING_PREMADE]                   = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.Premade", 1.f);
    m_float_configs[CONFIG_BG_MATCHMAKING_SKILL]                     = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.Skill", 1.f);
    m_float_configs[CONFIG_BG_MATCHMAKING_MAX_SKILL]                 = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.MaxSkill", 500.f);
    m_float_configs[CONFIG_BG_MATCHMAKING_ITEMLVL]                   = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.ItemLvl", 0.1f);
    m_float_configs[CONFIG_BG_MATCHMAKING_MAX_ITEMLVL]               = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.MaxItemLvl", 80.f);
    m_float_configs[CONFIG_BG_MATCHMAKING_FACTION]                   = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.Faction", 0.5f);
    m_float_configs[CONFIG_BG_MATCHMAKING_WAIT]                      = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.Wait", 1.0f);
    m_float_configs[CONFIG_BG_MATCHMAKING_MAX_WAIT]                  = sConfigMgr->GetFloatDefault("Battleground.Matchmaking.MaxWait", 60.f * 30.f * 20.f);

    m_bool_configs[CONFIG_ARENA_SOLOQUEUE_ENABLED] = sConfigMgr->GetBoolDefault("Arena.Soloqueue", true);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_PLAYER_COUNT_WEIGHT] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.PlayerCount", 2.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_PREMADE_WEIGHT] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Premade", 0.5f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_MAX_PREMADE_DIFF] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Premade.Max", 1.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_DIFF_WEIGHT] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Diff", 0.5f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_DIFF_MIN] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Diff.Min", 100.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_DIFF_MAX] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Diff.Max", 500.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_DIFF_TIME] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Diff.Time", 900.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_RANGE_WEIGHT] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Range", 0.5f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_RANGE_MIN] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Range.Min", 400.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_RANGE_MAX] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Range.Max", 1000.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_SKILL_RANGE_TIME] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.Skill.Range.Time", 900.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_2V2_TIME] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.2v2.Time", 360.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_3V3_TIME] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.3v3.Time", 120.f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_RECENT_OPPONENT_PENALTY] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.RecentOpponent.Multiplier", 0.75f);
    m_float_configs[CONFIG_ARENA_MATCHMAKING_RECENT_OPPONENT_TIME] = sConfigMgr->GetFloatDefault("Arena.Matchmaking.RecentOpponent.Time", 900.f);
    ...
    m_float_configs[CONFIG_DYNAMIC_VISIBILITY_RATE] = sConfigMgr->GetFloatDefault("Visibility.Dynamic.Rate", 1.0001f);
    m_float_configs[CONFIG_DYNAMIC_VISIBILITY_MIN] = sConfigMgr->GetFloatDefault("Visibility.Dynamic.Min", 1.f);
    m_float_configs[CONFIG_DYNAMIC_VISIBILITY_MAX] = sConfigMgr->GetFloatDefault("Visibility.Dynamic.Max", 1.f);
    m_float_configs[CONFIG_DYNAMIC_VISIBILITY_LARGE_OBJECT_THRESHOLD] = sConfigMgr->GetFloatDefault("Visibility.Dynamic.LargeThreshold", 3.f);
    m_float_configs[CONFIG_DYNAMIC_VISIBILITY_MAX_OBJECT_VISIBILITY] = sConfigMgr->GetFloatDefault("Visibility.Dynamic.MaxObjectMultiplier", 3.f);
    rate_values[RATE_VISIBILITY_GAME_OBJECTS] = sConfigMgr->GetFloatDefault("Visibility.GameObject.Rate", 2.f);
    if (rate_values[RATE_VISIBILITY_GAME_OBJECTS] < 1.f)
    {
        TC_LOG_ERROR("server.loading", "Visibility.GameObject.Rate can't be less than 1.");
        rate_values[RATE_VISIBILITY_GAME_OBJECTS] = 1.f;
    }
    m_int_configs[CONFIG_DYNAMIC_VISIBILITY_MAX_THRESHOLD] = sConfigMgr->GetIntDefault("Visibility.Dynamic.Max.Threshold", 100);
    m_int_configs[CONFIG_DYNAMIC_VISIBILITY_MIN_THRESHOLD] = sConfigMgr->GetIntDefault("Visibility.Dynamic.Min.Threshold", 50);
    ...
    m_int_configs[CONFIG_WINTERGRASP_MAX_WINS_IN_ROW] = sConfigMgr->GetIntDefault("Wintergrasp.MaxWinsInRow", 3);
    m_int_configs[CONFIG_WINTERGRASP_SLEEP_TIME_START] = sConfigMgr->GetIntDefault("Wintergrasp.SleepTimeStart", 0);
    m_int_configs[CONFIG_WINTERGRASP_SLEEP_TIME_END] = sConfigMgr->GetIntDefault("Wintergrasp.SleepTimeEnd", 14);
    ...
    m_float_configs[CONFIG_PERFORMANCE_PATHFINDING_REUSE] = std::clamp(sConfigMgr->GetFloatDefault("Performance.Pathfinding.Reuse", 0.8f), 0.f, 1.f);
    m_float_configs[CONFIG_PERFORMANCE_PATHFINDING_SHORTCUT_STEP_SIZE] = std::max(sConfigMgr->GetFloatDefault("Performance.Pathfinding.ShortcutStepSize", 2.f), 0.f);
    m_float_configs[CONFIG_PERFORMANCE_PATHFINDING_SMOOTH_PATH_STEP_SIZE] = std::max(sConfigMgr->GetFloatDefault("Performance.Pathfinding.SmoothPathStepSize", .5f), 0.f);

    // ####### RG ########
    m_bool_configs[CONFIG_RG_DATABASE_AVAILABLE] = sConfigMgr->GetBoolDefault("RG.Database", false);

    // Antiflood
    _afActive     = sConfigMgr->GetBoolDefault("Antiflood.Active",      false);
    _afMinActive  = sConfigMgr->GetIntDefault ("Antiflood.Active.Min",    10);
    _afMaxActive  = sConfigMgr->GetIntDefault ("Antiflood.Active.Max",  1000);
    _afMaxProcess = sConfigMgr->GetIntDefault ("Antiflood.Process.Max",  100);

    // AntiCheat
    m_bool_configs[CONFIG_ANTICHEAT_ENABLE] = sConfigMgr->GetBoolDefault("Anticheat.Enable", false);
    m_bool_configs[CONFIG_ANTICHEAT_DEBUG] = sConfigMgr->GetBoolDefault("Anticheat.Debug", false);
    m_int_configs[CONFIG_ANTICHEAT_ACTION_DELAY] = sConfigMgr->GetIntDefault("Anticheat.DelayAfterAction", 30);
    m_int_configs[CONFIG_ANTICHEAT_MAX_REPORT_LEVEL] = sConfigMgr->GetIntDefault("Anticheat.MaxReportLevel", 0);
    m_bool_configs[CONFIG_ANTICHEAT_WARDEN] = sConfigMgr->GetBoolDefault("Anticheat.WardenIngame", false);
    m_int_configs[CONFIG_ANTICHEAT_PLAYER_LIMIT] = sConfigMgr->GetIntDefault("AntiCheat.PlayerLimit", 1700);
    if (reload) // normally loaded at startup
        sObjectMgr->LoadAntiCheatConfig();

    // Lag Report
    m_bool_configs[CONFIG_LAG_REPORT_ENABLED] = sConfigMgr->GetBoolDefault("LagReports.Enabled", false);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_SUM] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.Sum", 400);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_OPCODES] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.Opcodes", 200);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_PLAYERS] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.Players", 200);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_UNITS] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.Units", 100);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_MAPS] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.Maps", 200);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_OBJECT_CELL] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.ObjectCells", 100);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_PLAYER_CELL] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.PlayerCells", 100);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_RELOCATION_GRID] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.RelocationGrids", 100);
    m_int_configs[CONFIG_LAG_REPORT_MAX_DIFF_CREATURE] = sConfigMgr->GetIntDefault("LagReports.MaxDiffSize.Creatures", 100);

    m_int_configs[CONFIG_ICC_RELEASE_STAGE] = sConfigMgr->GetIntDefault("RG.ICC.ReleaseStage", 50);

    m_int_configs[CONFIG_STARTGUILD_MAX_PLAYER] = sConfigMgr->GetIntDefault("RG.StartguildMaxPlayer", 750);
    m_int_configs[CONFIG_GUILD_MAX_PLAYER_SHOWN] = sConfigMgr->GetIntDefault("RG.GuildMaxPlayerShown", 750);

    m_int_configs[CONFIG_ACCOUNT_VERIFY_LENGTH] = sConfigMgr->GetIntDefault("RG.AccountVerifyCodeLength", 10);

    m_int_configs[CONFIG_PLAYER_COMPLAINT_TIME] = sConfigMgr->GetIntDefault("PlayerComplaint.Time", 30 * MINUTE);
    m_int_configs[CONFIG_PLAYER_COMPLAINT_GM_NOTIFY] = sConfigMgr->GetIntDefault("PlayerComplaint.Notify", 5);
    m_int_configs[CONFIG_PLAYER_COMPLAINT_MUTE] = sConfigMgr->GetIntDefault("PlayerComplaint.MuteCount", 10);
    m_int_configs[CONFIG_PLAYER_COMPLAINT_MUTE_TIME] = sConfigMgr->GetIntDefault("PlayerComplaint.MuteTime", 30 * MINUTE);
    m_int_configs[CONFIG_PLAYER_COMPLAINT_BAN] = sConfigMgr->GetIntDefault("PlayerComplaint.BanCount", 0);
    m_int_configs[CONFIG_PLAYER_COMPLAINT_BAN_TIME] = sConfigMgr->GetIntDefault("PlayerComplaint.BanTime", DAY);

    m_int_configs[CONFIG_BIRTHDAY_TIME] = sConfigMgr->GetIntDefault("BirthdayTime", 1222964635);

    m_int_configs[CONFIG_LFG_LEVEL_THRESHOLD] = sConfigMgr->GetIntDefault("DungeonFinder.LevelThreshold", 1);
    m_int_configs[CONFIG_LFG_WAITTIME_PER_DIFF] = sConfigMgr->GetIntDefault("DungeonFinder.WaittimePerLevelDiff", 3 * MINUTE);

    m_bool_configs[CONFIG_PVP_REWARD_TITLE] = sConfigMgr->GetBoolDefault("RewardPvPTitle", true);
    rate_values[RATE_PVP_CP_HK] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.HK", 1.0f);
    rate_values[RATE_PVP_CP_BONUS] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.Bonus", 0.0f);
    rate_values[RATE_PVP_CP_BG_WIN] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.BG.Win", 15.0f);
    rate_values[RATE_PVP_CP_BG_LOSE] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.BG.Lose", 3.0f);
    rate_values[RATE_PVP_CP_BG_BONUS] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.BG.Bonus", 1.0f);
    rate_values[RATE_PVP_CP_ARENA_WIN] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.Arena.Win", 5.0f);
    rate_values[RATE_PVP_CP_ARENA_LOSE] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.Arena.Lose", 1.0f);
    rate_values[RATE_PVP_CP_ARENA_BONUS] = sConfigMgr->GetFloatDefault("RG.ClassicTitle.Arena.Bonus", 250.0f);
    m_int_configs[CONFIG_PVP_CP_ARENA_BONUS_MATCHES] = sConfigMgr->GetIntDefault("RG.ClassicTitle.Arena.Matches", 50);
    m_int_configs[CONFIG_PVP_CP_MINIMUM] = sConfigMgr->GetIntDefault("RG.ClassicTitle.MinCp", 1000);
    ...
    m_int_configs[CONFIG_AUCTIONHOUSE_QUERY_DELAY_NORMAL] = sConfigMgr->GetIntDefault("Auctionhouse.Query.Delay.Normal", 300);
    m_int_configs[CONFIG_AUCTIONHOUSE_QUERY_DELAY_FULL] = sConfigMgr->GetIntDefault("Auctionhouse.Query.Delay.Full", 15*MINUTE*IN_MILLISECONDS);

    m_int_configs[RG_CONFIG_INSTANCE_UNLOAD_COOLDOWN] = sConfigMgr->GetIntDefault("Command.InstanceUnload.Cooldown", 5 * MINUTE) * IN_MILLISECONDS;

    m_bool_configs[CONFIG_PREMIUM_NPC_ACTIVE] = sConfigMgr->GetBoolDefault("RG.Premium.Npc.Active", true);

    if (!reload)
    {
        m_bool_configs[CONFIG_ARENA_REPLAY_ENABLED] = sConfigMgr->GetBoolDefault("ArenaReplay.Enabled", false);
        m_bool_configs[CONFIG_ARENA_REPLAY_RATED_ONLY] = sConfigMgr->GetBoolDefault("ArenaReplay.Replays.RatedMatchesOnly", true);
    }

    m_int_configs[CONFIG_ARENA_REPLAY_DELAY] = sConfigMgr->GetIntDefault("ArenaReplay.Delay", 0);
    
    m_int_configs[CONFIG_STAFF_SECURITY_LEVEL] = sConfigMgr->GetIntDefault("Account.StaffSecurityLevel", GMLEVEL_RG_TEAM);

    // debug
    m_bool_configs[CONFIG_DEBUG_DISABLE_BATTLEFIELD] = sConfigMgr->GetBoolDefault("RG.Debug.DisableBattlefield", false);
    m_bool_configs[CONFIG_DEBUG_DISABLE_MOTHERPOOLS] = sConfigMgr->GetBoolDefault("RG.Debug.DisableMotherpools", false);
    m_bool_configs[CONFIG_DEBUG_DISABLE_LOCALES] = sConfigMgr->GetBoolDefault("RG.Debug.DisableLocales", false);

    normalThreadPool.setNumThreads(sConfigMgr->GetIntDefault("ThreadPool.NormalWorkerThreads", normalThreadPool.numThreads()));
    lowPrioThreadPool.setNumThreads(sConfigMgr->GetIntDefault("ThreadPool.LowPrioWorkerThreads", lowPrioThreadPool.numThreads()));
    httpRequestThreadPool.setNumThreads(sConfigMgr->GetIntDefault("ThreadPool.HttpRequestWorkerThreads", httpRequestThreadPool.numThreads()));

    m_int_configs[CONFIG_ARENA_REPLAY_LIST_NEWEST_COUNT] = sConfigMgr->GetIntDefault("ArenaReplay.Replays.ListNewestCount", 30);

    rate_values[RATE_ARENA_POINTS_DISTRIBUTION] = sConfigMgr->GetFloatDefault("Rate.ArenaPoints.Distribution", 1.0f);

    sMapMgr->GetMapUpdater()->SetThreadCount(sConfigMgr->GetIntDefault("MapUpdate.Threads", 2));
...
void World::SetInitialWorldSettings()
{
    ...
    RG::Rebase::ItemRebase::Instance()->Initialize();
    ...
    TC_LOG_INFO("server.loading", "Registering additional monitoring modules");
    RG::Monitoring::RegisterModule<RG::Monitoring::Auctionhouse>();
    RG::Monitoring::RegisterModule<RG::Monitoring::Maps>();
    RG::Monitoring::RegisterModule<RG::Monitoring::WorldMonitoring>();
#endif
    ...
    VisibilityGroupBase::LoadVisibilityInfo();

    TC_LOG_INFO("server.loading", "Loading Boundary store...");
    boundary::BoundaryMgr::LoadFromDB();
    ...
    if (sWorld->getBoolConfig(CONFIG_DEBUG_DISABLE_LOCALES))
        TC_LOG_WARN("server.loading", "Skipped loading locales due to config.");
    else
    {
    ...
    TC_LOG_INFO("server.loading", "Loading Spell Delay definitions...");
    ...
    TC_LOG_INFO("server.loading", "Loading Creature Vendor Item Count ...");
    ...
    TC_LOG_INFO("server.loading", "Loading MapArea script names...");
    ...
    TC_LOG_INFO("server.loading", "Loading Player level dependdent mail rewads extended...");
    ...
    sAchievementMgr->LoadRealmFirstAchievements();

    RG::Logging::Manager::Initialize();

    player::PlayerCache::Init();                                    // must be initialized before loading groups

    // rebase auctionhouse, guildbanks and (unowned) mail items
    RG::Rebase::ItemRebase::Instance()->RebaseAuctionhouse();
    RG::Rebase::ItemRebase::Instance()->RebaseGuildBankAndMails();
    ...
    support::sTicketMgr.lock()->Initialize();
    ...
    TC_LOG_INFO("server.loading", "Loading polygon assignments...");
    sObjectMgr->LoadPolygonAssignments();
    ...
    LoginDatabase.PExecute("INSERT INTO uptime (realmid, starttime, uptime, revision) VALUES(%u, %u, 0, '%s')",
                            realm.Id.Realm, uint32(game::time::GetStartTime()), GitRevision::GetFullVersion());       // One-time query
    ...
    tm localTm;
    time_t gameTime = game::time::GetGameTime();
    ...
    if (sWorld->getBoolConfig(CONFIG_DEBUG_DISABLE_BATTLEFIELD))
        TC_LOG_WARN("server.loading", "Skipped start of Battlefield System due to config.");
    else
    {
    ...
    TC_LOG_INFO("server.loading", "Loading AnitCheat config...");
    sObjectMgr->LoadAntiCheatConfig();
    ...
    TC_LOG_INFO("server.loading", "Deleting expired complains...");
    CharacterDatabase.Execute("DELETE FROM account_complaints WHERE expirationTime <= NOW()");
    ...
    TC_LOG_INFO("server.loading", "Loading Arena Replay data...");
    sArenaReplayMgr->Initialize();

    TC_LOG_INFO("server.loading", "Loading cells...");
    if (m_bool_configs[CONFIG_BASEMAP_LOAD_GRIDS])
        sMapMgr->LoadAllCells();
    else
    {
        if (sWorld->getBoolConfig(CONFIG_DEBUG_DISABLE_BATTLEFIELD))
            TC_LOG_WARN("server.loading", "Skipped loading of Wintergrasp cells due to config.");
        else
            sMapMgr->LoadSpecificCells();
    }

    ///@todo remove this hack
    if (BattlefieldWG* wgScript = dynamic_cast<BattlefieldWG*>(sBattlefieldMgr->GetZoneScript(4197 /*BATTLEFIELD_WG_ZONEID*/)))
        wgScript->DelayedInitialization();
...
void World::Update(uint32 diff)
{
    ...
    sLFGMgr->UpdateStart(); // Allows LFGMgr to run asynchronous calculations
    ...
    sLFGMgr->UpdateFinish(diff);
    RecordTimeDiff("UpdateLFGMgr");

    if (lagReportActive)
        diff_size[DIFF_MAPS] = GetMSTimeDiffToNow(diffTime);

    auto update_latency = diffTimer.StartTimer();
    ...
    if (lagReportActive)
        diff_size[DIFF_SESSIONS] = GetMSTimeDiffToNow(diffTime);
    ...
    {
        uint32 tmpDiff = game::time::GetUptime();
        ...
        stmt->setUInt32(3, uint32(game::time::GetStartTime()));
    ...
    if (lagReportActive)
        diff_size[DIFF_RESULT_QUEUE] = GetMSTimeDiffToNow(getMSTime() - diffTime);
    ...
    if (lagReportActive &&
        GetMSTimeDiffToNow(startTime) > getIntConfig(CONFIG_LAG_REPORT_MAX_DIFF_SUM))
    {
        sLog->outMessage("rg.lagreport", LOG_LEVEL_WARN, "[Lag Report] SUM: %u SESSIONS: %u MAPS: %u RESULT_QUEUE: %u",
            GetMSTimeDiffToNow(startTime),
            diff_size[DIFF_SESSIONS],
            diff_size[DIFF_MAPS],
            diff_size[DIFF_RESULT_QUEUE]);
    }

    auto update_duration = diffTimer.StopTimer();
    MONITOR_WORLD(ReportUpdate(update_latency, update_duration));
}

void World::ForceGameEventUpdate()
{
    /// Schedule game events update for next tick
    m_timers[WUPDATE_EVENTS].SetInterval(1);
}

/// Send a packet to all players (except self if mentioned)
void World::SendGlobalMessage(const WorldPacket& packet, WorldSession* self, uint32 team)
{
    SessionMap::const_iterator itr;
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
...
void World::SendGlobalGMMessage(const WorldPacket& packet, WorldSession* self, uint32 team)
{
...
bool World::SendZoneMessage(uint32 zone, const WorldPacket& packet, WorldSession* self, uint32 team)
{
...
BanResult World::BanAccountByIp(std::string const& ip, BanDuration duration, std::string const& reason, std::string const& author) const
{
    PreparedStatement* stmt = nullptr;

    // insert ip ban into the db
    stmt = LoginDatabase.GetPreparedStatement(LOGIN_INS_IP_BANNED);
    stmt->setString(0, ip);
    stmt->setUInt32(1, duration.count());
    stmt->setString(2, author);
    stmt->setString(3, reason);
    LoginDatabase.Execute(stmt);

    // query affected accounts for kicking
    stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_BY_IP);
    stmt->setString(0, ip);
    PreparedQueryResult affectedAccounts = LoginDatabase.Query(stmt);

    if (affectedAccounts)
    {
        // Disconnect all affected players (can be several)
        do
        {
            Field* fields = affectedAccounts->Fetch();
            uint32 accountId = fields[0].GetUInt32();

            if (WorldSession* session = FindSession(accountId))
                session->KickPlayer();
        } while (affectedAccounts->NextRow());
    }

    return BAN_SUCCESS;
}

BanResult World::BanAccountById(uint32 accountId, BanDuration duration, std::string const& reason, std::string const& author) const
{
...
BanResult World::BanAccountByName(std::string const& accountName, BanDuration duration, std::string const& reason, std::string const& author) const
{
    // find the id of the account with the given name
    PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_ID_BY_NAME);
    stmt->setString(0, accountName);
    PreparedQueryResult affectedAccount = LoginDatabase.Query(stmt);

    if (!affectedAccount)
        return BAN_NOTFOUND;

    Field* fieldsAccount = affectedAccount->Fetch();
    uint32 accountId = fieldsAccount[0].GetUInt32();
    return BanAccountById(accountId, duration, reason, author);
}

BanResult World::BanAccountByCharacterName(std::string const& characterName, BanDuration duration, std::string const& reason, std::string const& author) const
{
    // find the account the character belongs to
    if (!player::PlayerCache::Exists(characterName))
        return BAN_NOTFOUND;

    uint32 accountId = player::PlayerCache::GetAccountId(characterName);
    return BanAccountById(accountId, duration, reason, author);
}
...
bool World::RemoveBanAccount(BanMode mode, std::string const& nameOrIP)
{
    ...
    else
    {
        ...
        if (mode == BAN_ACCOUNT)
            ...
            account = player::PlayerCache::GetAccountId(nameOrIP);
...
BanResult World::BanCharacter(std::string const& name, std::string const& duration, std::string const& reason, std::string const& author)
{
...
{
    ///- update the time
    time_t lastGameTime = game::time::GetGameTime();
    game::time::UpdateGameTimers();

    uint32 elapsed = uint32(game::time::GetGameTime() - lastGameTime);
...
void World::ShutdownMsg(bool show, Player* player, const std::string& reason)
{
        ...
        (m_ShutdownTimer > 12 * HOUR && (m_ShutdownTimer % (12 * HOUR)) == 0)) // > 12 h ; every 12 h
    {
        ...
        if (player && show && m_ShutdownReason != "")
            ChatHandler(player->GetSession()).PSendSysMessage("%s", m_ShutdownReason.c_str());
    }
}
...
void World::UpdateSessions(uint32 diff)
{
    ...
    for (SessionMap::iterator itr = m_sessions.begin(), next; itr != m_sessions.end(); itr = next)
    {
        ...
        else 
        {
            ...
            pSession->HandlePackets(updater);
        }
    }
}
...
void World::ProcessCliCommands()
{
    ...
    while (cliCmdQueue.next(command))
    {
        ...
        accountId = command->m_accountId;
        CliHandler handler(callbackArg, zprint, accountId);
        chat::command::CommandMgr::HandleConsole(command->m_command, handler);
...
void World::UpdateRealmCharRecord(uint32 accountId)
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_REALM_RECORD);
    stmt->setUInt32(0, accountId);
    stmt->setUInt32(1, accountId);
    CharacterDatabase.AsyncQuery(stmt)
            .via(_executor).then([this](PreparedQueryResult result) { _UpdateRealmCharRecord(std::move(result)); });
}

void World::_UpdateRealmCharRecord(PreparedQueryResult resultRealmChar)
{
    if (resultRealmChar)
    {
        Field* fields = resultRealmChar->Fetch();
        uint32 accountId = fields[0].GetUInt32();
        uint8 charCount = uint8(fields[1].GetUInt64());
        uint8 maxLevel   = fields[2].GetUInt8();
...
void World::SetPlayerSecurityLimit(uint32 _sec)
{
    uint32 sec = _sec <= GMLEVEL_SERVER ? _sec : static_cast<uint32>(GMLEVEL_PLAYER);
