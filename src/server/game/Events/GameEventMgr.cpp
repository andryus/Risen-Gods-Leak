...
void GameEventMgr::UnApplyEvent(uint16 event_id)
{
    ...
    FireFScriptTriggers(event_id, false);
...
void GameEventMgr::ApplyNewEvent(uint16 event_id)
{
    ...
    FireFScriptTriggers(event_id, true);
    //! Run SAI scripts with SMART_EVENT_GAME_EVENT_END
...
void GameEventMgr::UpdateEventNPCFlags(uint16 event_id)
{
    ...
    for (NPCFlagList::iterator itr = mGameEventNPCFlags[event_id].begin(); itr != mGameEventNPCFlags[event_id].end(); ++itr)
    {
        ...
        {
            Creature* cr = ObjectAccessor::GetCreature(data->mapid, ObjectGuid(HighGuid::Unit, data->id, itr->first));
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    for (GuidList::iterator itr = mGameEventCreatureGuids[internal_event_id].begin(); itr != mGameEventCreatureGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr))
        {
            ...
            {
                Creature::LoadFromDB(*itr, map); // automatically adds the creature to the map
            }
        }
    }
    ...
    for (GuidList::iterator itr = mGameEventGameobjectGuids[internal_event_id].begin(); itr != mGameEventGameobjectGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (GameObjectData const* data = sObjectMgr->GetGOData(*itr))
        {
            ...
            if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
            {
                ...
                GameObject* obj = GameObject::LoadFromDB(*itr, *map, false);
                if (!obj)
                    continue;
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    for (GuidList::iterator itr = mGameEventCreatureGuids[internal_event_id].begin(); itr != mGameEventCreatureGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr))
        {
            ...
            if (Creature* creature = ObjectAccessor::GetCreature(data->mapid, ObjectGuid(HighGuid::Unit, data->id, *itr)))
    ...
    for (GuidList::iterator itr = mGameEventGameobjectGuids[internal_event_id].begin(); itr != mGameEventGameobjectGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (GameObjectData const* data = sObjectMgr->GetGOData(*itr))
        {
            ...
            if (GameObject* pGameobject = ObjectAccessor::GetGameObject(data->mapid, ObjectGuid(HighGuid::Gameobject, data->id, *itr)))
...
void GameEventMgr::ChangeEquipOrModel(int16 event_id, bool activate)
{
    for (ModelEquipList::iterator itr = mGameEventModelEquip[event_id].begin(); itr != mGameEventModelEquip[event_id].end(); ++itr)
    {
        ...
        // Update if spawned
        Creature* creature = ObjectAccessor::GetCreature(data->mapid, ObjectGuid(HighGuid::Unit, data->id, itr->first));
        if (creature)
        {
            if (activate)
            {
...
void GameEventMgr::UpdateWorldStates(uint16 event_id, bool Activate)
{
    ...
    if (event.holiday_id != HOLIDAY_NONE)
    {
        ...
        if (bgTypeId != BATTLEGROUND_TYPE_NONE)
        {
            ...
            {
                WorldPacket data = sBattlegroundMgr->BuildUpdateWorldStatePacket(bl->HolidayWorldStateId, Activate ? 1 : 0);
...
void GameEventMgr::FireFScriptTriggers(uint16 event_id, bool activate)
{
    sMapMgr->ExecuteForEach([event_id, activate](Map& map)
    {
        if (activate)
            map <<= Fire::GameEventActivate(event_id);
        else
            map <<= Fire::GameEventDeactivate(event_id);
    });
}
...
void GameEventMgr::RunSmartAIScripts(uint16 event_id, bool activate)
{
    ...
    sMapMgr->ExecuteForEach([=](Map& map)
    {
        for (Creature& c : map.GetCreatures())
            if (c.AI())
                c.AI()->sOnGameEvent(activate, event_id);

        for (GameObject& g : map.GetGameObjects())
            if (g.AI())
                g.AI()->OnGameEvent(activate, event_id);
    });
}
