...
struct PlayerQueueInfo                                      // stores information for players in queue
{
    ...
    double  Rating;                                         // rating for unranked battleground
    float   AvgItemLevel;                                   // pvp item level
    bool    IsHeal;                                         // has most points in heal talent tree
...
struct GroupQueueInfo                                       // stores information about the group in queue (also used when joined as solo!)
{
    ...
    bool AllowCrossfaction;                                 // allow playing for opposing team

    bool CanJoinBGTeam(uint32 team);
};
...
class BattlegroundQueue
{
    public:
        ...
        bool BattlegroundMatchmaking(BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id);
        void StartProposal(Battleground* bg, const Matchmaking::Proposal &proposal);
        void ArenaMatchmaking(BattlegroundBracketId bracket_id);
        bool BattlegroundMatchmaking();

        void FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id);
        bool CheckPremadeMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam);
        bool CheckNormalMatch(Battleground* bg_template, BattlegroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers);
        bool CheckSkirmishForSameFaction(BattlegroundBracketId bracket_id, uint32 minPlayersPerTeam);
        GroupQueueInfo* AddGroup(Player* leader, Group* group, BattlegroundTypeId bgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 ArenaTeamId = 0, bool isSoloQueue = false);
