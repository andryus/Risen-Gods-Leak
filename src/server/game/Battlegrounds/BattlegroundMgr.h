...
class GAME_API BattlegroundMgr
{
    ...
    public:
        ...
        /* Packet Building */
        WorldPacket BuildPlayerJoinedBattlegroundPacket(Player* player);
        WorldPacket BuildPlayerLeftBattlegroundPacket(ObjectGuid guid);
        WorldPacket BuildBattlegroundListPacket(ObjectGuid guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere);
        WorldPacket BuildGroupJoinedBattlegroundPacket(GroupJoinBattlegroundResult result);
        WorldPacket BuildUpdateWorldStatePacket(uint32 field, uint32 value);
        WorldPacket BuildPvpLogDataPacket(Battleground* bg);
        WorldPacket BuildBattlegroundStatusPacket(Battleground* bg, uint8 queueSlot, uint8 statusId, uint32 time1, uint32 time2, uint8 arenaType, uint32 arenaFaction);
        ...
        static BattlegroundQueueTypeId BGQueueTypeId(BattlegroundTypeId bgTypeId, uint8 arenaType, ObjectGuid soloQueuePlayer = ObjectGuid::Empty);
        ...
        uint32 GetMaxRatingDifference(ArenaTeamTypes arenaType) const;
        uint32 GetRatingDiscardTimer(ArenaTeamTypes arenaType)  const;
        uint32 GetRatingWideningTimer(ArenaTeamTypes arenaType) const;
        uint32 GetRatingWideningDelta(ArenaTeamTypes arenaType) const;
        ...
        uint32 GetPlayerCountInQueue(BattlegroundQueueTypeId queueTypeId, BattlegroundBracketId bracketId, BattlegroundQueueGroupTypes queueGroupType);
