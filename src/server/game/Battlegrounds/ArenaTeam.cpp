...
bool ArenaTeam::Create(ObjectGuid captainGuid, uint8 type, std::string const& teamName, uint32 backgroundColor, uint8 emblemStyle, uint32 emblemColor, uint8 borderStyle, uint32 borderColor)
{
    ...
    switch (ArenaTeamTypes(Type))
    {
        case ARENA_TEAM_2v2:
            Stats.Rating = sWorld->getIntConfig(CONFIG_ARENA_START_RATING_2V2);
            break;
        case ARENA_TEAM_3v3:
            Stats.Rating = sWorld->getIntConfig(CONFIG_ARENA_START_RATING_3V3);
            break;
        case ARENA_TEAM_5v5:
            Stats.Rating = sWorld->getIntConfig(CONFIG_ARENA_START_RATING_5V5);
            break;
        default:
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    {
        auto entry = player::PlayerCache::Get(playerGuid);
        if (!entry)
            return false;

        playerName = entry.name;
        playerClass = entry.class_;
    }
    ...
    TC_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] joined arena team type: %u [Id: %u, Name: %s].", playerName.c_str(), playerGuid.GetCounter(), GetType(), GetId(), GetName().c_str());

    return true;
}

bool ArenaTeam::LoadArenaTeamFromDB(SharedQueryResult result)
{
    if (!result)
        return false;
...
bool ArenaTeam::LoadMembersFromDB(SharedQueryResult result)
{
    if (!result)
        return false;
...
void ArenaTeam::BroadcastPacket(const WorldPacket& packet)
{
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end(); ++itr)
        if (Player* player = ObjectAccessor::FindConnectedPlayer(itr->Guid))
...
uint32 ArenaTeam::GetPoints(uint32 memberRating)
{
    ...
    points *= sWorld->getRate(RATE_ARENA_POINTS_DISTRIBUTION);
...
uint32 ArenaTeam::GetMaxMMR(Group *group) const
{
    if (!group)
        return 0;

    return std::accumulate(Members.begin(), Members.end(), 0, [group](uint32 value, ArenaTeamMember const &member) {
        // Skip if player is not online
        ...
        return std::max(value, static_cast<uint32>(member.MatchMakerRating));
    });
}

uint32 ArenaTeam::GetMinMMR(Group *group) const
{
    if (!group)
        return 0;
    ...
    return std::accumulate(Members.begin(), Members.end(), 0, [group](uint32 value, ArenaTeamMember const &member) {
        ...
        return std::min(value, static_cast<uint32>(member.MatchMakerRating));
    });
}
...
int32 ArenaTeam::GetRatingMod(uint32 ownRating, uint32 opponentRating, bool won /*, float confidence_factor*/)
{
    ...
    // CUSTOM RG: no points if change is less than 3 for ratings over 2300
    mod = (int32)ceil(mod);
...
void ArenaTeam::MemberLost(Player* player, uint32 againstMatchmakerRating, int32 MatchmakerRatingChange)
{
    ...
    for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        if (itr->Guid == player->GetGUID())
        {
            ...
            if (itr->SeasonGames >= 3)
                player->pvpData.RewardArenaBonusCP(itr->WeekGames);
            return;
        }
    }
}
...
void ArenaTeam::SaveToDB(SQLTransaction trans)
{
...
void ArenaTeam::SetRating(int16 rating)
{
    Stats.Rating = rating;
}

void ArenaTeamMember::SetPersonalRating(int16 rating)
{
