...
void BattlegroundMgr::Update(uint32 diff)
{
        ...
        for (uint8 i = 0; i < scheduled.size(); i++)
        {
            ...
            switch (bgTypeId)
            {
                ...
                case BATTLEGROUND_IC:
                    m_BattlegroundQueues[bgQueueTypeId].BattlegroundMatchmaking(bgTypeId, bracket_id);
                    break;
    ...
    {
        m_BattlegroundQueues[BATTLEGROUND_QUEUE_SOLO_ARENA].ArenaMatchmaking(BG_BRACKET_ID_LVL_80_SOLOQUEUE);
...
WorldPacket BattlegroundMgr::BuildBattlegroundStatusPacket(Battleground* bg, uint8 QueueSlot, uint8 StatusID, uint32 Time1, uint32 Time2, uint8 arenatype, uint32 arenaFaction)
{
    ...
    WorldPacket data(SMSG_BATTLEFIELD_STATUS, (4+8+1+1+4+1+4+4+4));
    ...
    switch (StatusID)
    {
        ...
        case STATUS_IN_PROGRESS:                                // status_in_progress
            ...
            if (arenaFaction == ALLIANCE)
                arenaFaction = 1;
            else if (arenaFaction == HORDE)
                arenaFaction = 0;
            else
                arenaFaction = 2;
            data << uint8(arenaFaction);   // arenafaction (0 for horde, 1 for alliance, 2 for replay)
            break;
        default:
...
WorldPacket BattlegroundMgr::BuildPvpLogDataPacket(Battleground* bg)
{
    const uint8 type = (bg->isArena() ? 1 : 0);
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        {
            TC_LOG_ERROR("bg.battleground", "Player %s has scoreboard entry for battleground %u but is not in battleground!", itr->first.ToString().c_str(), bg->GetTypeID(true));
            continue;
        }
        ...
        {
            Player* player = ObjectAccessor::GetPlayer(*bg->GetBgMap(), itr2->first);
...
WorldPacket BattlegroundMgr::BuildGroupJoinedBattlegroundPacket(GroupJoinBattlegroundResult result)
{
...
WorldPacket BattlegroundMgr::BuildPlayerLeftBattlegroundPacket(ObjectGuid guid)
{
...
WorldPacket BattlegroundMgr::BuildPlayerJoinedBattlegroundPacket(Player* player)
{
...
WorldPacket BattlegroundMgr::BuildBattlegroundListPacket(ObjectGuid guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
    if (!player)
        return WorldPacket(0, 0);
    ...
    {
        const size_t count_pos = data.wpos();
...
BattlegroundQueueTypeId BattlegroundMgr::BGQueueTypeId(BattlegroundTypeId bgTypeId, uint8 arenaType, ObjectGuid soloQueuePlayer)
{
    switch (bgTypeId)
    {
        case BATTLEGROUND_AB:
        ...
        case BATTLEGROUND_RV:
            switch (arenaType)
            {
                ...
                case ARENA_TYPE_3v3:
                    if (soloQueuePlayer && sWorld->getBoolConfig(CONFIG_ARENA_SOLOQUEUE_ENABLED))
                    {
                        BattlegroundQueue& bgQueue = sBattlegroundMgr->GetBattlegroundQueue(BATTLEGROUND_QUEUE_3v3);
                        GroupQueueInfo ginfo;
                        if (!bgQueue.GetPlayerGroupInfoData(soloQueuePlayer, &ginfo))
                            return BATTLEGROUND_QUEUE_SOLO_ARENA;
                    }
...
uint32 BattlegroundMgr::GetMaxRatingDifference(ArenaTeamTypes arenaType) const
{
    switch (arenaType)
    {
        case ARENA_TEAM_2v2:
            return sWorld->getIntConfig(CONFIG_ARENA_MAX_RATING_DIFFERENCE_2V2);
        case ARENA_TEAM_3v3:
            return sWorld->getIntConfig(CONFIG_ARENA_MAX_RATING_DIFFERENCE_3V3);
        case ARENA_TEAM_5v5:
            return sWorld->getIntConfig(CONFIG_ARENA_MAX_RATING_DIFFERENCE_5V5);
        default:
...
uint32 BattlegroundMgr::GetRatingDiscardTimer(ArenaTeamTypes arenaType) const
{
    switch (arenaType)
    {
        case ARENA_TEAM_2v2:
            return sWorld->getIntConfig(CONFIG_ARENA_RATING_DISCARD_TIMER_2V2);
        case ARENA_TEAM_3v3:
            return sWorld->getIntConfig(CONFIG_ARENA_RATING_DISCARD_TIMER_3V3);
        case ARENA_TEAM_5v5:
            return sWorld->getIntConfig(CONFIG_ARENA_RATING_DISCARD_TIMER_5V5);
        default:
            // this should never happen
            return 1;
    }
...
uint32 BattlegroundMgr::GetRatingWideningTimer(ArenaTeamTypes arenaType) const
{
    switch (arenaType)
    {
    case ARENA_TEAM_2v2:
        return sWorld->getIntConfig(CONFIG_ARENA_WIDENING_TIMER_2V2);
    case ARENA_TEAM_3v3:
        return sWorld->getIntConfig(CONFIG_ARENA_WIDENING_TIMER_3V3);
    case ARENA_TEAM_5v5:
        return sWorld->getIntConfig(CONFIG_ARENA_WIDENING_TIMER_5V5);
    default:
        // this should never happen
        return 1;
    }
...
uint32 BattlegroundMgr::GetRatingWideningDelta(ArenaTeamTypes arenaType) const
{
    switch (arenaType)
    {
    case ARENA_TEAM_2v2:
        return sWorld->getIntConfig(CONFIG_ARENA_WIDENING_DELTA_2V2);
    case ARENA_TEAM_3v3:
        return sWorld->getIntConfig(CONFIG_ARENA_WIDENING_DELTA_3V3);
    case ARENA_TEAM_5v5:
        return sWorld->getIntConfig(CONFIG_ARENA_WIDENING_DELTA_5V5);
    default:
        // this should never happen
        return 1;
    }
...
BattlegroundTypeId BattlegroundMgr::GetRandomBG(BattlegroundTypeId bgTypeId)
{
    ...
    {
        // CUSTOM RG: some bgs are closed when wintergrasp is closed
        int32 sleepTimerStart = sWorld->getIntConfig(CONFIG_WINTERGRASP_SLEEP_TIME_START);
        int32 sleepTimerEnd = sWorld->getIntConfig(CONFIG_WINTERGRASP_SLEEP_TIME_END);
        time_t cur = time(NULL);
        tm* ta = localtime(&cur);
        bool closed = false;

        if (sleepTimerStart != sleepTimerEnd && ta->tm_hour >= sleepTimerStart && ta->tm_hour < sleepTimerEnd)
            closed = true;

        for (BattlegroundSelectionWeightMap::const_iterator it = m_BGSelectionWeights.begin(); it != m_BGSelectionWeights.end(); ++it)
        {
            if (closed && (it->first == BATTLEGROUND_AV || it->first == BATTLEGROUND_IC))
                continue;
...
uint32 BattlegroundMgr::GetPlayerCountInQueue(BattlegroundQueueTypeId queueTypeId, BattlegroundBracketId bracketId, BattlegroundQueueGroupTypes queueGroupType)
{
    uint32 players = 0;
    ...
    switch (queueTypeId)
    {
        ...
        case BATTLEGROUND_QUEUE_SA:
        case BATTLEGROUND_QUEUE_IC:
            battleground = (queueGroupType == BG_QUEUE_NORMAL_ALLIANCE || queueGroupType == BG_QUEUE_NORMAL_HORDE);
            break;
        default:
            break;
    }
    ...
    if (battleground)
    {
        const auto& groupsQueueType = m_BattlegroundQueues[queueTypeId].m_QueuedGroups[bracketId][BG_QUEUE_MIXED];
        for (auto itr = groupsQueueType.begin(); itr != groupsQueueType.end(); ++itr)
            if (!(*itr)->IsInvitedToBGInstanceGUID && (*itr)->Team == (queueGroupType == BG_QUEUE_NORMAL_HORDE ? HORDE : ALLIANCE))
                players += (*itr)->Players.size();
    }
    else
    {
        const auto& groupsQueueType = m_BattlegroundQueues[queueTypeId].m_QueuedGroups[bracketId][queueGroupType];
        for (auto itr = groupsQueueType.begin(); itr != groupsQueueType.end(); ++itr)
