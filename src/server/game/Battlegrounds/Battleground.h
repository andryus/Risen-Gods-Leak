...
// Arena ground positions
// Used for falling under map handling
#define ARENA_BE_GROUND 0.5f
#define ARENA_DS_GROUND 3.0f
#define ARENA_NA_GROUND 11.0f
#define ARENA_RL_GROUND 31.0f
#define ARENA_RV_GROUND 28.27f
...
enum ArenaQuests
{
    QUEST_ARENA_2V2                 = 110156,
    QUEST_ARENA_3V3                 = 110155,
    EVENT_QUEST_3V3                 = 108
};
...
enum BattlegroundSpells
{
    ...
    SPELL_PET_SUMMONED              = 6962,                 // Pet Summoned
    ...
    COSTUM_SOULWELL                 = 58888,                // Arena If warlock is available. this spell will be casted
    COSTUM_TABLE                    = 58661                 // Arena If mage is available, this spell will be casted
};
...
enum BattlegroundTimeIntervals
{
    ...
    MAX_OFFLINE_TIME_IN_BG          = 120,                  // secs
    MAX_OFFLINE_TIME_IN_ARENA       = 300,                  // secs
...
enum BattlegroundBuffObjects
{
    ...
    ARENA_MAGE_TABLE                = 193061,
    ARENA_WARLOCK_SOULWELL          = 193169,
    ARENA_WARLOCK_SOULWELL_TWO      = 193170,
    ARENA_WARLOCK_SOULWELL_THREE    = 193171,
...
class GAME_API Battleground
{
    public:
        ...
        void SetLastResurrectTime(ObjectGuid guid) { m_LastResurrectTime[guid] = 0; }
        void SetMaxPlayers(uint32 MaxPlayers) { m_MaxPlayers = MaxPlayers; mmData.SetConfig(MaxPlayers); }
        ...
        void AddPlayerToResurrectQueue(ObjectGuid player_guid);
        ...
        void SendPacketToTeam(uint32 TeamID, const WorldPacket& packet, Player* sender = NULL, bool self = true);
        ...
        void RewardExperienceToTeam(float multiplier, uint32 TeamID);
        ...
        uint32 GetPlayerScore(Player* Source, ScoreType type);
        ...
        Matchmaking::BattlegroundMatchmakingData& GetMatchmakingData() { return mmData; }

        void AddArenaMMRData(ObjectGuid guid, uint32 team, SkillValue data, uint32 rating);
        void CalculateArenaMMRData();
    ...
    protected:
        ...
        // Spirit Guide guid + last resurrect time
        std::map<ObjectGuid, uint32>  m_LastResurrectTime;
    ...
    private:
        ...
        std::set<ObjectGuid> m_ReadyMarkerList;                 // Player GUID
        ...
        uint32 arenaHealReduceLevel;
        void UpdateHealReduceLevel();
        ...
        // RG Custom
        float bgTeamRating[BG_TEAMS_COUNT];
        float bgTeamRD[BG_TEAMS_COUNT];
        std::map<ObjectGuid, uint32> playerSamples;
        std::map<ObjectGuid, uint32> playerTeam;
        uint32 ratingSampleSize;
        uint32 ratingSampleTimer;

        Matchmaking::BattlegroundMatchmakingData mmData;

        using PlayerRatingMap = std::unordered_map<ObjectGuid, std::pair<SkillValue, uint32>>;

        std::array<PlayerRatingMap, BG_TEAMS_COUNT> ratedArenaData;
        std::array<std::array<PlayerRatingMap, BG_TEAMS_COUNT>, BG_TEAMS_COUNT> ratedArenaResultData;
        std::array<std::array<int32, BG_TEAMS_COUNT>, BG_TEAMS_COUNT> ratedArenaRatingChange;
        void UpdateArenaRating(uint32 winner);

        void UpdateRatingTimer(uint32 diff);
        void EndRatingUpdate();
};
