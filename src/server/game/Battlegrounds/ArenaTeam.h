...
struct GAME_API ArenaTeamMember
{
    ...
    void SetPersonalRating(int16 rating);
};
...
class GAME_API ArenaTeam
{
    public:
        ...
        uint32 GetMaxMMR(Group* group) const;
        uint32 GetMinMMR(Group* group) const;
        ...
        bool LoadArenaTeamFromDB(SharedQueryResult arenaTeamDataResult);
        bool LoadMembersFromDB(SharedQueryResult arenaTeamMembersResult);
        void SaveToDB(SQLTransaction trans = SQLTransaction(nullptr));

        void BroadcastPacket(const WorldPacket& packet);
