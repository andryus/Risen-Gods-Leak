...
bool GroupQueueInfo::CanJoinBGTeam(uint32 joinTeam)
{
    if (joinTeam == TEAM_ALLIANCE)
        return Team == ALLIANCE || AllowCrossfaction;
    else if (joinTeam == TEAM_HORDE)
        return Team == HORDE || AllowCrossfaction;
    return false;
}

// add group or player (grp == NULL) to bg queue with the given leader and bg specifications
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid, bool isSoloQueue)
{
    ...
    ginfo->JoinTime                  = game::time::GetGameTimeMS();
    ...
    ginfo->AllowCrossfaction         = sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED) && !leader->HasCustomFlag(Player::PlayerCustomFlags::CROSSFACTION_DISABLED);
    ...
    uint32 lastOnlineTime = game::time::GetGameTimeMS();
    ...
    if (grp)
    {
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            pl_info.IsHeal           = member->IsHealer();
            pl_info.AvgItemLevel     = member->GetPvPItemLevel();
            if (isRated)
                pl_info.Rating = member->pvpData.GetArenaSkillValues().rating;
            else
                pl_info.Rating = member->pvpData.GetRating().rating;
    ...
    else
    {
        ...
        pl_info.LastOnlineTime   = lastOnlineTime;
        pl_info.GroupInfo        = ginfo;
        pl_info.IsHeal           = leader->IsHealer();
        pl_info.AvgItemLevel     = leader->GetPvPItemLevel();
        if (isRated)
            pl_info.Rating = leader->pvpData.GetArenaSkillValues().rating;
        else
            pl_info.Rating = leader->pvpData.GetRating().rating;
                ...
                if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED))
                {
                    ...
                    if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY))
                    {
                        ChatHandler(leader->GetSession()).PSendSysMessage(
                            ...
                            q_max_level,
                            qPlayers,
                            minPlayers > qPlayers ? minPlayers - qPlayers : uint32(0)
                        );
                    }
                    else
                    {
                        sWorld->SendWorldText(
                            LANG_BG_QUEUE_ANNOUNCE_CROSSFACTION_WORLD,
                            bgName,
                            q_min_level,
                            q_max_level,
                            qPlayers,
...
{
    uint32 timeInQueue = getMSTimeDiff(ginfo->JoinTime, game::time::GetGameTimeMS());
...
void BattlegroundQueue::RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount)
{
    ...
    if (decreaseInvitedCount && group->IsInvitedToBGInstanceGUID)
        if (Battleground* bg = sBattlegroundMgr->GetBattleground(group->IsInvitedToBGInstanceGUID, group->BgTypeId))
        {
            ...
            bg->GetMatchmakingData().RemovePlayerData(guid);
        }
    ...
    if (!group->IsInvitedToBGInstanceGUID && group->IsRated)
    {
        ...
        if (Player* plr2 = ObjectAccessor::FindConnectedPlayer(group->Players.begin()->first))
        {
            ...
            plr2->SendDirectMessage(std::move(data));
        }
...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, uint32 side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        ginfo->RemoveInviteTime = game::time::GetGameTimeMS() + INVITE_ACCEPT_WAIT_TIME;
        ...
        for (std::map<ObjectGuid, PlayerQueueInfo*>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
            ...
            if (!player || player->GetSession()->PlayerLogout() || (!player->IsInWorld() && !player->IsBeingTeleportedFar()))
                continue;
            ...
            bg->GetMatchmakingData().AddPlayerData(player, ginfo);
            bg->AddArenaMMRData(player->GetGUID(), side, player->pvpData.GetArenaSkillValues(), player->pvpData.GetArenaRating());
            ...
            // send status packet
            WorldPacket data = sBattlegroundMgr->BuildBattlegroundStatusPacket(bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME, 0, ginfo->ArenaType, 0);
...
bool BattlegroundQueue::CheckPremadeMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam)
{
    ...
    uint32 time_before = game::time::GetGameTimeMS() - sWorld->getIntConfig(CONFIG_BATTLEGROUND_PREMADE_GROUP_WAIT_FOR_MATCH);
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    static const auto get_player_count = [](const GroupsQueueType& queue)
    {
        return std::accumulate(queue.begin(), queue.end(), uint32(0),
            [](uint32 init, const GroupQueueInfo* info) -> uint32
        {
            if (info->IsInvitedToBGInstanceGUID)
                return init;
            else
                return init + info->Players.size();
        });
    };

    const auto log_queue_status_on_invite = [this, bracket_id](const Battleground* bg)
    {
        TC_LOG_INFO("pvp.bg.queue", "Queue status (before invite) for '%s' (%u) (%u: %u-%u): alliance: normal: %u, premade: %u; horde: normal: %u, premade: %u",
                    bg->GetName(), bg->GetTypeID(), bg->GetBracketId(), bg->GetMinLevel(), bg->GetMaxLevel(),
                    get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE]),
                    get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE]),
                    get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE]),
                    get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE]));
    };

    if (Battleground* bg_template = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId))
    {
        TC_LOG_INFO(
            "pvp.bg.queue",
            "Queue status (update) (%u: %u-%u): alliance: normal: %u, premade: %u; horde: normal: %u, premade: %u",
            bracket_id, bg_template->GetMinLevel(), bg_template->GetMaxLevel(),
            get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE]),
            get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE]),
            get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE]),
            get_player_count(m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE])
    ...
    ASSERT(bg_template->isArena() || isRated);
    ...
    if (bg_template->isBattleground())
    {
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            log_queue_status_on_invite(bg2);
    ...
    else if (bg_template->isArena())
    {
        ...
        const auto currentTime = static_cast<int32>(game::time::GetGameTimeMS());
        const auto discardTime = currentTime - static_cast<int32>(sBattlegroundMgr->GetRatingDiscardTimer(ArenaTeamTypes(arenaType)));

        auto const maxRatingDelta = sBattlegroundMgr->GetMaxRatingDifference(ArenaTeamTypes(arenaType));

        const auto wideningTimer = sBattlegroundMgr->GetRatingWideningTimer(ArenaTeamTypes(arenaType));
        const auto wideningDelta = sBattlegroundMgr->GetRatingWideningDelta(ArenaTeamTypes(arenaType));
        const auto isRatingWidening = wideningTimer > 0 && wideningDelta > 0;
        ...
        auto teamPredicate = [&](GroupQueueInfo const* queue_info) {
            if (queue_info->IsInvitedToBGInstanceGUID)
                return false;

            const auto joinTime = static_cast<int32>(queue_info->JoinTime);
            if ((!isRatingWidening && joinTime < discardTime))
                return true;

            const auto teamMMR = queue_info->ArenaMatchmakerRating;
            const auto teamMMRDelta = maxRatingDelta + (isRatingWidening ? ((currentTime - joinTime) / wideningTimer) * wideningDelta : 0);

            if (teamMMR <= (arenaRating + teamMMRDelta) && arenaRating <= (teamMMR + teamMMRDelta))
                return true;

            return false;
        };

        // try to find a alliance and horde team matching the criterias
        // this selection is unfair: it favors opposite faction matches
        for (uint8 i : { BG_QUEUE_PREMADE_ALLIANCE, BG_QUEUE_PREMADE_HORDE })
        {
            auto& queue = m_QueuedGroups[bracket_id][i];
            auto teamIterator = std::find_if(std::begin(queue), std::end(queue), teamPredicate);

            if (teamIterator != std::end(queue))
            {
                itr_teams[found++] = teamIterator;
                team = i;
                // no break here because we want to match the opposite faction
            }
        }
        ...
        // we didn't find a match for the opposite faction, looking for a match with the same faction
        if (found == 1)
        {
            auto& queue = m_QueuedGroups[bracket_id][team];
            auto begin = itr_teams[0];
            auto teamIterator = std::find_if(++begin, std::end(queue), teamPredicate);
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue or in battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            WorldPacket data = sBattlegroundMgr->BuildBattlegroundStatusPacket(bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME - INVITATION_REMIND_TIME, 0, m_ArenaType, 0);
            player->SendDirectMessage(std::move(data));
        }
    }
