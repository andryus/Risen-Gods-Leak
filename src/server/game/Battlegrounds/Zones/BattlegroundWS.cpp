...
void BattlegroundWS::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (_bothFlagsKept)
        {
            ...
            {
                if (Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), m_FlagKeepers[0]))
                    player->CastSpell(player, BG_WS_SPELL_FOCUSED_ASSAULT, true);
                if (Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), m_FlagKeepers[1]))
                    player->CastSpell(player, BG_WS_SPELL_FOCUSED_ASSAULT, true);
                _flagDebuffState = 1;
            }
...
void BattlegroundWS::StartingEventOpenDoors()
{
    ...
    for (uint32 i = BG_WS_OBJECT_DOOR_A_5; i <= BG_WS_OBJECT_DOOR_A_6; ++i)
        DoorDelete(i);
    for (uint32 i = BG_WS_OBJECT_DOOR_H_3; i <= BG_WS_OBJECT_DOOR_H_4; ++i)
        DoorDelete(i);
