...
void BattlegroundEY::CheckSomeoneJoinedPoint()
{
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        if (GameObject* obj = ObjectAccessor::GetGameObject(*GetBgMap(), BgObjects[BG_EY_OBJECT_TOWER_CAP_FEL_REAVER + i]))
        {
            uint8 j = 0;
            while (j < m_PlayersNearPoint[EY_POINTS_MAX].size())
            {
                Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), m_PlayersNearPoint[EY_POINTS_MAX][j]);
                if (!player)
                {
...
void BattlegroundEY::CheckSomeoneLeftPoint()
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        if (GameObject* obj = ObjectAccessor::GetGameObject(*GetBgMap(), BgObjects[BG_EY_OBJECT_TOWER_CAP_FEL_REAVER + i]))
        {
            ...
            {
                Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), m_PlayersNearPoint[i][j]);
                if (!player)
                {
...
void BattlegroundEY::UpdatePointStatuses()
{
    for (uint8 point = 0; point < EY_POINTS_MAX; ++point)
    {
        ...
        {
            Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), m_PlayersNearPoint[point][i]);
            if (player)
            {
...
void BattlegroundEY::RespawnFlagAfterDrop()
{
    ...
    if (GameObject* obj = ObjectAccessor::GetGameObject(*GetBgMap(), GetDroppedFlagGUID()))
        obj->Delete();
    else
