...
void BattlegroundBE::StartingEventOpenDoors()
{
    ...
    DelObject(BG_BE_READY_MARKER_H);
    DelObject(BG_BE_READY_MARKER_A);
}
...
bool BattlegroundBE::SetupBattleground()
{
    ...
    if (!AddObject(BG_BE_READY_MARKER_H, GO_READY_MARKER, xH+1.8f, yH+2.2f, zH, oH, 0, 0, 0, 0, RESPAWN_IMMEDIATELY)
        || !AddObject(BG_BE_READY_MARKER_A, GO_READY_MARKER, xA+1.8f, yA+2.2f, zA, oA, 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        TC_LOG_ERROR("sql.sql", "BatteGroundBE: Failed to spawn ready-marker!");
        return false;
    }
