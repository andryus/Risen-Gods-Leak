...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN])
    {
        ...
        ChangeMineOwner(AV_NORTH_MINE, killer->GetTeam(), false, unit);
        ...
        ChangeMineOwner(AV_SOUTH_MINE, killer->GetTeam(), false, unit);
}
...
Creature* BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type)
{
    ...
    else if (creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_N_MINE_A_4]
    || creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_S_MINE_A_1]
    || creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_S_MINE_A_3]
    || creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_S_MINE_A_4])
        creature->setFaction(1216);
...
void BattlegroundAV::EndBattleground(uint32 winner)
{
    ...
    for (int i = TEAM_ALLIANCE; i <= TEAM_HORDE; ++i)
    {
        ...
        if (rep[i] != 0)
            RewardReputationToTeam(730, 729, 10, i == ALLIANCE ? ALLIANCE : HORDE);
        if (kills[i] != 0)
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial, Creature* source)
{
    ...
    //Reward group (Quest: Capture A Mine)
    if (team != AV_NEUTRAL_TEAM &&  m_Mine_PrevOwner[mine] != AV_NEUTRAL_TEAM)
    {
        if (Group* group = GetBgRaid(team))
            group->RewardGroupAtEvent(BG_AV_MineCapturedQuestReward, source, false);
    }
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    if (m_Nodes[node].TotalOwner != AV_NEUTRAL_TEAM)
    {
        ...
        if (IsTower(node))
        {
            //Reward group (AV Towers/Bunkers Quests)
            if (Group* group = GetBgRaid(team))
            {
                WorldObject* rewardSource = GetBGObject(object);
                group->RewardGroupAtEvent(BG_AV_TowerCapturedQuestReward, rewardSource);
            }
        ...
        {
            //Reward group (AV Graveyards Quests)
            if (Group* group = GetBgRaid(team))
            {
                WorldObject* rewardSource = GetBGObject(object);
                group->RewardGroupAtEvent(BG_AV_GraveyardsCapturedQuestReward, rewardSource);
            }
