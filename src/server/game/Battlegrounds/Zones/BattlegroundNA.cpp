...
void BattlegroundNA::StartingEventOpenDoors()
{
    ...
    DelObject(BG_NA_READY_MARKER_H);
    DelObject(BG_NA_READY_MARKER_A);
}
...
bool BattlegroundNA::SetupBattleground()
{
    ...
    if (!AddObject(BG_NA_READY_MARKER_H, GO_READY_MARKER, xH+1.8f, yH+2.2f, zH, oH, 0, 0, 0, 0, RESPAWN_IMMEDIATELY)
        || !AddObject(BG_NA_READY_MARKER_A, GO_READY_MARKER, xA+1.8f, yA+2.2f, zA, oA, 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        TC_LOG_ERROR("sql.sql", "BatteGroundNA: Failed to spawn ready-marker!");
        return false;
    }
