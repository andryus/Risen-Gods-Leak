...
void BattlegroundSA::PostUpdateImpl(uint32 diff)
{
    ...
    else if (Status == BG_SA_SECOND_WARMUP)
    {
        ...
        if (TotalTime >= 60000)
        {
            ...
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                if (Player* p = ObjectAccessor::GetPlayer(*GetBgMap(), itr->first))
    ...
    else if (GetStatus() == STATUS_IN_PROGRESS)
    {
        if (Status == BG_SA_ROUND_ONE)
        {
            ...
            {
                // Achievement Defense of the Ancients - 2200, criteriaId - 7740
                    ...
                    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), itr->first))
                            if (player->GetTeamId() != Attackers)
                            {
                                ...
                                {
                                    if (AchievementEntry const *AchievRespectYourElders = sAchievementStore.LookupEntry(1757)) // Ach Alliance
                                        player->CompletedAchievement(AchievRespectYourElders);
                                }   
                                else // if Horde
                                {
                                    if (AchievementEntry const *AchievRespectYourElders = sAchievementStore.LookupEntry(2200)) // Ach Horde
                                        player->CompletedAchievement(AchievRespectYourElders);
                                }
                            }
                    }
...
void BattlegroundSA::TeleportPlayers()
{
    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
    {
        if (Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), itr->first))
        {
            ...
            player->RemoveAurasWithMechanic((1 << MECHANIC_DISORIENTED) | (1 << MECHANIC_FEAR));
                ...
                else if (eventId == go->GetGOInfo()->building.destroyedEvent)
                {
                    ...
                    case BG_SA_GREEN_GATE:
                        if (GateStatus[BG_SA_BLUE_GATE] == BG_SA_GATE_DESTROYED)
                            rewardHonor = false;
                        SendWarningToAll(LANG_BG_SA_BLUE_GATE_DESTROYED, go->GetGOInfo()->name.c_str());
                        break;
                    case BG_SA_BLUE_GATE:
                        if (GateStatus[BG_SA_GREEN_GATE] == BG_SA_GATE_DESTROYED)
                            rewardHonor = false;
                        SendWarningToAll(LANG_BG_SA_GREEN_GATE_DESTROYED, go->GetGOInfo()->name.c_str());
                        break;
                    case BG_SA_RED_GATE:
                        if (GateStatus[BG_SA_PURPLE_GATE] == BG_SA_GATE_DESTROYED)
                            rewardHonor = false;
                        SendWarningToAll(LANG_BG_SA_PURPLE_GATE_DESTROYED, go->GetGOInfo()->name.c_str());
                        break;
                    case BG_SA_PURPLE_GATE:
                        if (GateStatus[BG_SA_RED_GATE] == BG_SA_GATE_DESTROYED)
                            rewardHonor = false;
                        SendWarningToAll(LANG_BG_SA_RED_GATE_DESTROYED, go->GetGOInfo()->name.c_str());
                        break;
                    case BG_SA_YELLOW_GATE:
                        SendWarningToAll(LANG_BG_SA_YELLOW_GATE_DESTROYED, go->GetGOInfo()->name.c_str());
                        break;
                    case BG_SA_ANCIENT_GATE:
                        SendWarningToAll(LANG_BG_SA_ANCIENT_GATE_DESTROYED, go->GetGOInfo()->name.c_str());
                        break;
                    default:
                        break;
                    }
...
void BattlegroundSA::HandleKillPlayer(Player* victim, Player* killer)
{
    Battleground::HandleKillPlayer(victim, killer);

    if (victim->HasItemCount(39213, 1, false))
        victim->DestroyItemCount(39213, 1, true);
}
...
void BattlegroundSA::DemolisherStartState(bool start)
{
    ...
    for (uint8 i = BG_SA_DEMOLISHER_1; i <= BG_SA_DEMOLISHER_4; i++)
    {
        if (Creature* dem = GetBGCreature(i))
        {
            ...
            dem->SetCorpseDelay(10); // 10 seconds
        }
    }
}
        ...
        if (BgCreatures[i])
        {
            if (Creature* Demolisher = GetBGCreature(i))
            {
                if (Demolisher->isDead())
                {
                    ...
                    {
                        DemoliserRespawnList[i] = game::time::GetGameTimeMS()+30000;
                    }
                    else
                    {
                        if (DemoliserRespawnList[i] < game::time::GetGameTimeMS())
                        {
                ...
                Demolisher->SetCorpseDelay(10); // 10 seconds
            }
        }
    }
}
