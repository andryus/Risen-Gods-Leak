        ...
        for (int node = 0; node < BG_AB_DYNAMIC_NODES_COUNT; ++node)
        {
            ...
            if (m_NodeTimers[node])
            {
                ...
                else
                {
                    ...
                    //Reward group (Quest: The Battle for Arathi Basin!)
                    if (Group* group = GetBgRaid((teamIndex == 0) ? ALLIANCE : HORDE))
                    {
                        GameObject* banner = GetBGObject(node * 8 + teamIndex);
                        group->RewardGroupAtEvent(BG_AB_NodeCapturedQuestReward[node], banner);
                    }
