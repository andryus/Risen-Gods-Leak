...
BattlegroundIC::BattlegroundIC()
{
    ...
    BgCreatures.resize(MAX_NORMAL_NPCS_SPAWNS + MAX_WORKSHOP_SPAWNS + MAX_DOCKS_SPAWNS + MAX_SPIRIT_GUIDES_SPAWNS + MAX_TELEPORT_TRIGGER_SPAWNS + MAX_HONORABLE_DEFENDER_TRIGGER);
...
void BattlegroundIC::DoAction(uint32 action, ObjectGuid var)
{
    ...
    Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), var);
    if (!player)
        return;

    (player->GetTeamId() == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde)->AddPassenger(player);

    if (nodePoint[NODE_TYPE_HANGAR].controllingFaction != player->GetTeamId())
        return;
            ...
            if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A || nodePoint[i].nodeState == NODE_STATE_CONTROLLED_H)
            {
                if (siegeEngineWorkshopTimer <= diff)
                {
                    ...
                    if (Creature* siege = GetBGCreature(siegeType)) // this always should be true
                    {
                        if (siege->IsAlive())
                        {
                            if (siege->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE) || siege->IsImmuneToPC())
                            {
        ...
        if (nodePoint[i].needChange)
        {
            if (nodePoint[i].timer <= diff)
            {
              ...
              //nodePoint[i].faction = the faction should be the same one...
                //! set controllingFaction later HandleCapturedNodes needs the old faction
                nodePoint[i].controllingFaction = nodePoint[i].faction;
...
void BattlegroundIC::EventPlayerLoggedIn(Player* player)
{
...
void BattlegroundIC::AddPlayer(Player* player)
{
    ...
    player->ResetAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, 245, 628);
    player->ResetAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, 246, 628);
    player->LearnSpell(SPELL_LEARN_DISARM_IC, false);
...
bool BattlegroundIC::SetupBattleground()
{
    ...
    for (uint8 i = 0; i < MAX_TELEPORT_TRIGGER_SPAWNS; ++i)
    {
        if (!AddCreature(BG_IC_TriggerNpcSpawnLoc[i].entry, BG_IC_TriggerNpcSpawnLoc[i].type,
            BG_IC_TriggerNpcSpawnLoc[i].x, BG_IC_TriggerNpcSpawnLoc[i].y,
            BG_IC_TriggerNpcSpawnLoc[i].z, BG_IC_TriggerNpcSpawnLoc[i].o,
            BG_IC_TriggerNpcSpawnLoc[i].team, RESPAWN_ONE_DAY))
        {
            TC_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning teleport trigger %u", BG_IC_NpcSpawnlocs[i].entry);
            return false;
        }
    }
    ...
    // setting correct factions and respawntimes for Keep Cannons
    for (uint8 i = BG_IC_NPC_KEEP_CANNON_1; i < BG_IC_NPC_KEEP_CANNON_13; ++i)
    {
        if (Creature* cannon = GetBGCreature(i))
        {
            cannon->setFaction(BG_IC_Factions[0]);
            cannon->SetRespawnTime(RESPAWN_3_MINUTES);
            cannon->SetRespawnDelay(RESPAWN_3_MINUTES);
            cannon->SetCorpseDelay(CORSPE_DESPAWN_5_SECONDS);
            cannon->SetControlled(true, UNIT_STATE_ROOT);
        }
    }
    for (uint8 i = BG_IC_NPC_KEEP_CANNON_13; i < BG_IC_NPC_KEEP_CANNON_24; ++i)
    {
        if (Creature* cannon = GetBGCreature(i))
        {
            cannon->setFaction(BG_IC_Factions[1]);
            cannon->SetRespawnTime(RESPAWN_3_MINUTES);
            cannon->SetRespawnDelay(RESPAWN_3_MINUTES);
            cannon->SetCorpseDelay(CORSPE_DESPAWN_5_SECONDS);
            cannon->SetControlled(true, UNIT_STATE_ROOT);
    ...
    AddHonorableDefenderTrigger(&nodePoint[NODE_TYPE_ALLIANCE_KEEP]);
    AddHonorableDefenderTrigger(&nodePoint[NODE_TYPE_HORDE_KEEP]);
...
void BattlegroundIC::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    // Achievementcriteria - kill a player
    AchievementCriteriaEntry const* achievement = sAchievementCriteriaStore.LookupEntry(11491);
    killer->CompletedCriteria(achievement);
}
...
void BattlegroundIC::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
    {
        if (nodePoint[i].gameobject_entry == target_obj->GetEntry())
        {
            ...
            // special case, we must set last_enrty to the opposite faction contested banner
            if (IsBannerNeutral(nodePoint[i].last_entry))
                nodePoint[i].last_entry = player->GetTeamId() == TEAM_HORDE ? nodePoint[i].banners[BANNER_A_CONTESTED] : nodePoint[i].banners[BANNER_H_CONTESTED];
            ...
            if (nextBanner ==  nodePoint[i].banners[BANNER_A_CONTESTED] || nextBanner ==  nodePoint[i].banners[BANNER_H_CONTESTED])
            {
                ...
                nodePoint[i].controllingFaction = TEAM_NEUTRAL; // we must set controlling faction to neutral, nodody controlls this node. it is contested
                ...
                player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, 245, 628);
            ...
            else if (nextBanner == nodePoint[i].banners[BANNER_A_CONTROLLED] || nextBanner == nodePoint[i].banners[BANNER_H_CONTROLLED])
            {
                nodePoint[i].timer = BANNER_STATE_CHANGE_TIME;
                nodePoint[i].needChange = false;
                nodePoint[i].controllingFaction = player->GetTeamId(); // make sure correct faction is set after recapture
                ...
                player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, 246, 628);
            }
...
void BattlegroundIC::UpdateNodeWorldState(ICNodePoint* nodePoint)
{
    ...
    // with this we are sure we don't bug the client
    for (uint8 i = 0; i < 5; ++i)
    {
...
void BattlegroundIC::HandleContestedNodes(ICNodePoint* nodePoint)
{
    if (nodePoint->nodeType == NODE_TYPE_HANGAR)
    {
        ...
        for (uint8 u = BG_IC_GO_HANGAR_TELEPORTER_1; u < BG_IC_GO_HANGAR_TELEPORTER_3+1; ++u)
            DelObject(u);
    }

    RemoveHonorableDefenderTrigger(nodePoint);
}
...
void BattlegroundIC::HandleCapturedNodes(ICNodePoint* nodePoint, bool recapture)
{
    ...
    switch (nodePoint->gameobject_type)
    {
        ...
        case BG_IC_GO_DOCKS_BANNER:
            ...
            //! if team was neutral it's not possible that any vehicles were spawned, check prevents error spamming
            if (nodePoint->controllingFaction != TEAM_NEUTRAL)
            {
                // we must del opposing faction vehicles when the node is captured (unused ones)
                for (uint8 i = (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_1_H : BG_IC_NPC_GLAIVE_THROWER_1_A); i < (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_2_H+1 : BG_IC_NPC_GLAIVE_THROWER_2_A+1); ++i)
                {
                ...
                for (uint8 i = (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_CATAPULT_1_H : BG_IC_NPC_CATAPULT_1_A); i < (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_CATAPULT_4_H+1  : BG_IC_NPC_CATAPULT_4_A+1); ++i)
                {
                    if (Creature* catapult = GetBGCreature(i))
                    {
                        if (Vehicle* vehicleCatapult = catapult->GetVehicleKit())
                        {
                            if (!vehicleCatapult->GetPassenger(0))
                                DelCreature(i);
                        }
                    }
                }
        ...
        case BG_IC_GO_WORKSHOP_BANNER:
            {
                ...
                if (!recapture)
                {
                    ...
                    {
                        // we must del opposing faction vehicles when the node is captured (unused ones)
                        for (uint8 i = (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_DEMOLISHER_1_H : BG_IC_NPC_DEMOLISHER_1_A); i < (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_DEMOLISHER_4_H+1 : BG_IC_NPC_DEMOLISHER_4_A+1); ++i)
                        {
                    ...
                    if (nodePoint->controllingFaction != TEAM_NEUTRAL)
                    {
                        if (Creature* siegeEngine = GetBGCreature(enemySiege))
                            //! we can't check with IsVehicleInUse(), because siege engine has always passenger (the small tower). so we check distance to home position
                            if (siegeEngine->GetDistance(siegeEngine->GetHomePosition()) <= 5.0f)
    ...
    AddHonorableDefenderTrigger(nodePoint);
}
...
void BattlegroundIC::DestroyGate(Player* player, GameObject* go)
{
    ...
    if (player->GetTeamId() == TEAM_ALLIANCE)
        ...
        allianceBackdoor = false;

    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
    {
        if (Player* player = ObjectAccessor::GetPlayer(*GetBgMap(), itr->first))
            go->DestroyForPlayer(player, true);
    }    
...
WorldSafeLocsEntry const* BattlegroundIC::GetClosestGraveYard(Player* player)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
        if (nodePoint[i].controllingFaction == player->GetTeamId())
            nodes.push_back(i);
...
bool BattlegroundIC::IsBannerNeutral(uint32 entry) const
{
    if (entry == GO_REFINERY_BANNER || entry == GO_QUARRY_BANNER || entry == GO_DOCKS_BANNER ||
        entry == GO_HANGAR_BANNER || entry == GO_WORKSHOP_BANNER)
        return true;
    return false;
}

void BattlegroundIC::AddHonorableDefenderTrigger(ICNodePoint* nodePoint)
{
    Creature* trigger = BgCreatures[nodePoint->honorableDefenderTrigger] ? GetBGCreature(nodePoint->honorableDefenderTrigger) : NULL;
    GameObject* obj = GetBGObject(nodePoint->gameobject_type);
    if (obj) {
        if (!trigger)
            trigger = AddCreature(WORLD_TRIGGER, nodePoint->honorableDefenderTrigger, obj->GetPositionX(),
                obj->GetPositionY(), obj->GetPositionZ(), obj->GetOrientation());

        //add bonus honor aura trigger creature when node is accupied
        //cast bonus aura (+50% honor in 60yards)
        //aura should only apply to players who have accupied the node, set correct faction for trigger
        if (trigger)
        {
            trigger->setFaction(BG_IC_Factions[(nodePoint->faction == TEAM_ALLIANCE ? 0 : 1)]);
            trigger->CastSpell(trigger, SPELL_HONORABLE_DEFENDER_60Y, false);
        }
    }
}

void BattlegroundIC::RemoveHonorableDefenderTrigger(ICNodePoint* nodePoint)
{
    if (BgCreatures[nodePoint->honorableDefenderTrigger])
        DelCreature(nodePoint->honorableDefenderTrigger);
}
