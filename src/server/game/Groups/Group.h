...
class GAME_API Group
{
    public:
        ...
        typedef std::unordered_map< uint32 /*achievement*/, bool /*completed*/> AchievementAccessRequirementsMap;
    protected:
    ...
    public:
        ...
        void   LoadGroupAchievementsFromDB(Field* field);
        ...
        void BroadcastPacket(const WorldPacket& packet, bool ignorePlayersInBGRaid, int group = -1, ObjectGuid ignoredPlayer = ObjectGuid::Empty);
        void BroadcastReadyCheck(const WorldPacket& packet);
        ...
        void UpdateRequiredAchievements(Player* leader);
        bool HasRequiredAchievement(uint32 achievement) const;

        void ResetLfgBootVoteTime() { lfgBootVoteTime = time(nullptr); }
        uint32 GetLfgBootVoteTime() const;

        void ResetLastRaidDifficultyChange() { lastRaidDifficultyChange = time(nullptr); }
        uint32 GetLastRaidDifficultyChange() const { return lastRaidDifficultyChange; };

        // FG: evil hacks
        void BroadcastGroupUpdate(void);

        void RewardGroupAtEvent(uint32 creature_id, WorldObject* pRewardSource, bool overrideId = true);
    ...
    protected:
        ...
        AchievementAccessRequirementsMap m_achievementAccessRequirementsMap;
        uint8*              m_subGroupsCounts;
        ObjectGuid          m_guid;
        uint32              m_counter;                      // used only in SMSG_GROUP_LIST
        uint32              m_maxEnchantingLevel;
        uint32              m_dbStoreId;                    // Represents the ID used in database (Can be reused by other groups if group was disbanded)
        time_t              lfgBootVoteTime;
        uint32              lastRaidDifficultyChange;
};
#endif
