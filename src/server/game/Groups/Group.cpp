...
lfgBootVoteTime(0), lastRaidDifficultyChange(0)
{
...
void Group::LoadGroupFromDB(Field* fields)
{
    ...
    if (player::PlayerCache::Exists(m_leaderGuid))
        m_leaderName = player::PlayerCache::GetName(m_leaderGuid);
    else
        return;
...
void Group::LoadGroupAchievementsFromDB(Field* field)
{
    m_achievementAccessRequirementsMap[field[1].GetUInt16()] = true;
}
...
void Group::LoadMemberFromDB(uint32 guidLow, uint8 memberFlags, uint8 subgroup, uint8 roles)
{
    ...
    if (player::PlayerCache::Exists(member.guid))
        member.name = player::PlayerCache::GetName(member.guid);
    else
    {
...
void Group::ConvertToRaid()
{
    ...
    if (Player* leader = ObjectAccessor::FindPlayer(m_leaderGuid))
        UpdateRequiredAchievements(leader);
}
...

    {
        ...
        for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            if (Player* member = itr->GetSource())
            {
                ...
                {
                    WorldPacket packet = player->CreateValuesUpdateBlockForPlayer(member);
                    if (!packet.empty())
                        member->SendDirectMessage(std::move(packet));
                }
            }
        }

        if (groupData.HasData())
            player->SendDirectMessage(groupData.BuildPacket());
...
bool Group::RemoveMember(ObjectGuid guid, const RemoveMethod& method /*= GROUP_REMOVEMETHOD_DEFAULT*/, ObjectGuid kicker /*= 0*/, const char* reason /*= NULL*/)
{
    ...
    // Remove all group buffs from player
    if (player)
    {
        player->RemoveAllGroupBuffsFromOthers();
        if (auto* pet = player->GetPet())
            pet->RemoveAllGroupBuffsFromOthers();
    }

    // Remove group buffs casted by removed player on each group member
    for (auto* itr = GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        if (Player* groupMember = itr->GetSource())
        {
            ...
            if (auto* pet = groupMember->GetPet())
                pet->RemoveAllGroupBuffsFromCaster(guid);
        }
    }
    ...
    if (GetMembersCount() > ((isBGGroup() || isLFGGroup() || isBFGroup()) ? 1u : 2u))
    {
        ...
        if (GetMasterLooterGuid() == guid)
            SetMasterLooterGuid(GetLeaderGUID());
...
void Group::ChangeLeader(ObjectGuid newLeaderGuid)
{
    ...
    UpdateRequiredAchievements(newLeader);
...
void Group::SendLootStartRollToPlayer(uint32 countDown, uint32 mapId, Player* p, bool canNeed, Roll const& r)
{
    ...
    p->SendDirectMessage(std::move(data));
}
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        {
            ObjectGuid newitemGUID = ObjectGuid(HighGuid::Item, sObjectMgr->GenerateLowGuid(HighGuid::Item));
            ...
            if (r->totalPlayersRolling > 0)
            {
                ...
                for (Roll::PlayerVote::const_iterator itr = r->playerVote.begin(); itr != r->playerVote.end(); ++itr)
                {
                    ...
                    if (itr->second == PASS)
                        ...
                        SendLootStartRollToPlayer(60000, lootedObject->GetMapId(), p, p->CanRollForItemInLFG(&*i, lootedObject) == EQUIP_ERR_OK, *r);
                }
    ...
    if (roll->totalNeed > 0)
    {
        if (!roll->playerVote.empty())
        {
            ...
            if (player && player->GetSession())
            {
                ...
                {
                    item->is_looted = true;
                    roll->getLoot()->NotifyItemRemoved(roll->itemSlot, *player);
                    roll->getLoot()->unlootedCount--;
                    Item* pItem = player->StoreNewItem(dest, roll->itemid, true, item->randomPropertyId, item->GetAllowedLooters());
                    // RG-Custom ItemLogging
                    RG_LOG<ItemLogModule>(pItem, RG::Logging::ItemLogType::ADDED_BY_GREED, static_cast<uint32>(item->count));
                }
                else
                {
            ...
            if (player && player->GetSession())
            {
                ...
                if (rollvote == GREED)
                {
                    ...
                    if (msg == EQUIP_ERR_OK)
                    {
                        ...
                        Item* pItem = player->StoreNewItem(dest, roll->itemid, true, item->randomPropertyId, item->GetAllowedLooters());
                        // RG-Custom ItemLogging
                        RG_LOG<ItemLogModule>(pItem, RG::Logging::ItemLogType::ADDED_BY_NEED, static_cast<uint32>(item->count));
                    }
                    else
                    {
                        item->is_blocked = false;
...
void Group::SendUpdateToPlayer(ObjectGuid playerGUID, MemberSlot* slot)
{
    ...
    if (isLFGGroup())
    {
        data << uint8(sLFGMgr->GetState(m_guid) == lfg::LFG_STATE_FINISHED_DUNGEON ? 2 : 1); // FIXME - Dungeon save status? 2 = done
...
void Group::UpdatePlayerOutOfRange(Player* player)
{
    ...
    WorldPacket data = player->GetSession()->BuildPartyMemberStatsChangedPacket(player);
...
void Group::BroadcastPacket(const WorldPacket& packet, bool ignorePlayersInBGRaid, int group /*= -1*/, ObjectGuid ignoredPlayer /*= ObjectGuid::Empty*/)
{
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
...
void Group::BroadcastReadyCheck(const WorldPacket& packet)
{
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        Player* player = itr->GetSource();
...
void Group::UpdateLooterGuid(WorldObject* pLootedObject, bool ifneed)
{
    ...
    if (guid_itr != m_memberSlots.end())
    {
        if (ifneed)
        {
            ...
            Player* looter = ObjectAccessor::GetPlayer(*pLootedObject, guid_itr->guid);
    ...
    {
        if (Player* player = ObjectAccessor::GetPlayer(*pLootedObject, itr->guid))
...
InstanceGroupBind* Group::GetBoundInstance(Difficulty difficulty, uint32 mapId)
{
    ...
    difficulty = InstanceSaveManager::GetSaveDifficultyForMap(difficulty, mapId);
...
void Group::SetLfgRoles(ObjectGuid guid, uint8 roles)
{
    ...
    slot->roles = roles;
    m_memberSlots.sort([](const MemberSlot &a, const MemberSlot&b) { return a.roles < b.roles; });
    SendUpdate();
}
...
void Group::UpdateRequiredAchievements(Player* leader)
{
    enum AchievemtsToUpdate
    {
        ACHIEVEMT_FROST_THORNE_10 = 4530,
        ACHIEVEMT_FROST_THORNE_25 = 4597
    };
    for (uint32 id : { ACHIEVEMT_FROST_THORNE_10, ACHIEVEMT_FROST_THORNE_25 })
        m_achievementAccessRequirementsMap[id] = leader->HasAchieved(id);
}

bool Group::HasRequiredAchievement(uint32 achievement) const
{
    auto itr = m_achievementAccessRequirementsMap.find(achievement);
    if (itr == m_achievementAccessRequirementsMap.end())
        return false;
    return itr->second;
}

uint32 Group::GetLfgBootVoteTime() const
{
    time_t now = time(nullptr);
    if (now - lfgBootVoteTime >= 15 * MINUTE)
        return 0;
    else
        return 15 * MINUTE - (now - lfgBootVoteTime);
}
...
void Group::RewardGroupAtEvent(uint32 creature_id, WorldObject* pRewardSource, bool overrideId)
{
    if (!pRewardSource)
        return;

    ObjectGuid creature_guid = (pRewardSource->GetTypeId() == TYPEID_UNIT && overrideId) ? pRewardSource->GetGUID() : ObjectGuid::Empty;
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        Player* player = itr->GetSource();
        if (!player)
