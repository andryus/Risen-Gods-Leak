...
void GroupMgr::LoadGroups()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Group achievement requirements...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                      0          1
        QueryResult result = CharacterDatabase.Query("SELECT g.guid, ca.achievement FROM groups g LEFT JOIN character_achievement ca ON g.leaderGuid = ca.guid WHERE g.groupType & 2 AND achievement IN(4530, 4597)");
        ...
        if (result)
        {
            do
            {
                ...
                if (Group* group = GetGroupByDbStoreId(field[0].GetUInt32()))
                    group->LoadGroupAchievementsFromDB(field);

            } while (result->NextRow());

            TC_LOG_INFO("server.loading", ">> Loaded " UI64FMTD " group achievement requirements in %u ms", result->GetRowCount(), GetMSTimeDiffToNow(oldMSTime));
        }
        else
            TC_LOG_INFO("server.loading", ">> Loaded 0 group achievement requirements.");
    }
