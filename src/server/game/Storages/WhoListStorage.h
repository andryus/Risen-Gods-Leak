...
{
public:
    WhoListPlayerInfo(ObjectGuid guid, uint32 team, uint32 security, uint8 level, uint8 clss, uint8 race, uint32 zoneid, uint8 gender, bool visible, std::wstring const& widePlayerName,
        std::wstring const& wideGuildName, std::string const& playerName, std::string const& guildName, bool secretQueuing, uint32 orgZoneId) :
        ...
        _widePlayerName(widePlayerName), _wideGuildName(wideGuildName), _playerName(playerName), _guildName(guildName), _secretQueuing(secretQueuing),
        _orgZoneId(orgZoneId) {}
    ...
    bool GetSecretQueuing() const { return _secretQueuing; }
    uint32 GetOrgZoneId() const { return _orgZoneId; }
