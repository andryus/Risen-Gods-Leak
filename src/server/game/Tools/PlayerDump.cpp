...
uint32 registerNewGuid(uint32 oldGuid, std::map<uint32, uint32> &guidMap, ItemIdGenerator& idGenerator)
{
    std::map<uint32, uint32>::const_iterator itr = guidMap.find(oldGuid);
    if (itr != guidMap.end())
        return itr->second;

    uint32 newguid = idGenerator.Generate();
    guidMap[oldGuid] = newguid;
    return newguid;
}
...
bool changeGuid(std::string &str, int n, std::map<uint32, uint32> &guidMap, ItemIdGenerator& idGenerator, bool nonzero = false)
{
    char chritem[20];
    uint32 oldGuid = atoll(getnth(str, n).c_str());
    if (nonzero && oldGuid == 0)
    ...
    uint32 newGuid = registerNewGuid(oldGuid, guidMap, idGenerator);
    snprintf(chritem, 20, "%u", newGuid);

    return changenth(str, n, chritem, false, nonzero);
}

std::string CreateDumpString(char const* tableName, SharedQueryResult result)
{
...
void StoreGUID(SharedQueryResult result, uint32 field, std::set<uint32>& guids)
{
    Field* fields = result->Fetch();
...
void StoreGUID(SharedQueryResult result, uint32 data, uint32 field, std::set<uint32>& guids)
{
    Field* fields = result->Fetch();
...
bool PlayerDumpWriter::DumpTable(std::string& dump, uint32 guid, char const*tableFrom, char const*tableTo, DumpTableType type)
{
    ...
    do
    {
        ...
        SharedQueryResult result = CharacterDatabase.PQuery("SELECT * FROM %s WHERE %s", tableFrom, wherestr.c_str());
        if (!result)
            return true;
...
DumpReturn PlayerDumpReader::LoadDump(std::string const& file, uint32 account, std::string name, uint32& guid, bool oldDbStructure)
{
    ...
    char newguid[20], chraccount[20], newpetid[20], currpetid[20], lastpetid[20], at_login[4];
        ...
        */
        ...
        switch (type)
        {
            case DTT_CHARACTER:
            {
                ...
                at_login_flags = uint16(atol(getnth(line, 37).c_str()));
                zoneId = uint32(atol(getnth(line, 38).c_str()));
                ...
                if (name.empty())
                {
                    ...
                    {
                        at_login_flags |= 0x1;
                        snprintf(at_login, 4, "%u", at_login_flags);
                        if (!changenth(line, 37, at_login))       // characters.at_login set to "rename on login"
                            ROLLBACK(DUMP_FILE_BROKEN);
                    }
                }
                ...
                {
                    const char null[5] = "NULL";
                    const char grantableNull[10] = "0', 'NULL";
                    if (!changenth(line, 68, grantableNull))    // characters.grantableLevels + characters.deleteInfos_Account
                         ROLLBACK(DUMP_FILE_BROKEN);
                    if (!changenth(line, 70, null))             // characters.deleteInfos_Name
                        ROLLBACK(DUMP_FILE_BROKEN);
                    if (!changenth(line, 71, null))             // characters.deleteDate
            ...
            case DTT_INVENTORY:
            {
                ...
                if (!changeGuid(line, 2, items, sObjectMgr->_itemGuidGenerator, true))
                    ROLLBACK(DUMP_FILE_BROKEN);             // character_inventory.bag update
                if (!changeGuid(line, 4, items, sObjectMgr->_itemGuidGenerator))
            ...
            case DTT_MAIL_ITEM:                             // mail_items
            {
                ...
                if (!changeGuid(line, 2, items, sObjectMgr->_itemGuidGenerator))
            ...
            case DTT_ITEM:
            {
                ...
                if (!changeGuid(line, 1, items, sObjectMgr->_itemGuidGenerator))
            ...
            case DTT_PET:
            {
                ...
                std::map<uint32, uint32> ::const_iterator petids_iter = petids.find(atoll(currpetid));

                if (petids_iter == petids.end())
                {
                    petids.insert(PetIdsPair(atoll(currpetid), atoll(newpetid)));
                }
    ...
    entry.guid = ObjectGuid(HighGuid::Player, guid);
    entry.name = name;
    entry.account = account;
    entry.gender = gender;
    entry.race = race;
    entry.class_ = playerClass;
    entry.level = level;
    entry.at_login = at_login_flags;
    entry.zoneId = zoneId;
    player::PlayerCache::Add(entry, false);
