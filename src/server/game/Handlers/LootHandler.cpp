...
void WorldSession::HandleAutostoreLootItemOpcode(WorldPacket& recvData)
{
    ...
    if (lguid.IsGameObject())
    {
        ...
        if (!go || ((go->GetOwnerGUID() != GetPlayer()->GetGUID() && go->GetGoType() != GAMEOBJECT_TYPE_FISHINGHOLE) && !go->IsWithinDistInMap(GetPlayer(), INTERACTION_DISTANCE)))
        {
    ...
    else
    {
        ...
        if (!lootAllowed || !creature->IsWithinDistInMap(GetPlayer(), INTERACTION_DISTANCE))
        {
...
void WorldSession::HandleLootMoneyOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (loot)
    {
        loot->NotifyMoneyRemoved(*player);
        if (shareMoney && player->GetGroup())      //item, pickpocket and players can be looted only single player
        {
            ...
            {
                (*i)->ModifyMoney(goldPerPlayer, ::RG::Logging::MoneyLogType::LOOTED);
                ...
                (*i)->SendDirectMessage(std::move(data));
            }
        }
        else
        {
            player->ModifyMoney(loot->gold, ::RG::Logging::MoneyLogType::LOOTED);
...
void WorldSession::DoLootRelease(ObjectGuid lguid)
{
    ...
    else if (lguid.IsItem())
    {
        ...
        if (proto->Flags & (ITEM_PROTO_FLAG_PROSPECTABLE | ITEM_PROTO_FLAG_MILLABLE))
        {
            ...
            RG_LOG<ItemLogModule>(pItem, RG::Logging::ItemLogType::DESTROYED, count);
        ...
        else
        {
            ...
            {
                // RG-Custom ItemLogging
                RG_LOG<ItemLogModule>(pItem, RG::Logging::ItemLogType::DESTROYED);
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket& recvData)
{
    ...
    if (!GetPlayer()->GetGroup() || GetPlayer()->GetGroup()->GetMasterLooterGuid() != GetPlayer()->GetGUID() || GetPlayer()->GetGroup()->GetLootMethod() != MASTER_LOOT)
    {
        GetPlayer()->SendLootError(lootguid, LOOT_ERROR_DIDNT_KILL);
        return;
    }

    Player* target = ObjectAccessor::GetPlayer(*GetPlayer(), target_playerguid);
    if (!target)
    {
    ...
    if (!GetPlayer()->IsInRaidWith(target))
    {
        GetPlayer()->SendLootError(lootguid, LOOT_ERROR_MASTER_OTHER);
    ...
    if (msg != EQUIP_ERR_OK)
    {
        if (msg == EQUIP_ERR_CANT_CARRY_MORE_OF_THIS)
            GetPlayer()->SendLootError(lootguid, LOOT_ERROR_MASTER_UNIQUE_ITEM);
        else if (msg == EQUIP_ERR_INVENTORY_FULL)
            GetPlayer()->SendLootError(lootguid, LOOT_ERROR_MASTER_INV_FULL);
        else
            GetPlayer()->SendLootError(lootguid, LOOT_ERROR_MASTER_OTHER);
    ...
    // RG-Custom ItemLogging
    RG_LOG<ItemLogModule>(newitem, RG::Logging::ItemLogType::ADDED_BY_PM, uint32(item.count));
    ...
    loot->NotifyItemRemoved(slotid, *GetPlayer());
    --loot->unlootedCount;
}
