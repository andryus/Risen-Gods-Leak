...
void WorldSession::HandleDismissCritter(WorldPacket& recvData)
{
    ...
    Creature* pet = ObjectAccessor::GetCreature(*GetPlayer(), guid);

    if (!pet)
    {
        TC_LOG_DEBUG("network", "Vanitypet (%s) does not exist - player '%s' (guid: %u / account: %u) attempted to dismiss it (possibly logged out)",
            guid.ToString().c_str(), GetPlayer()->GetName().c_str(), GetPlayer()->GetGUID().GetCounter(), GetAccountId());
        return;
    }

    if (GetPlayer()->GetCritterGUID() == pet->GetGUID() && pet->IsSummon())
...
void WorldSession::HandlePetAction(WorldPacket& recvData)
{
    ...
    Unit* pet = ObjectAccessor::GetUnit(*GetPlayer(), guid1);
...
void WorldSession::HandlePetActionHelper(Unit* pet, ObjectGuid guid1, uint32 spellid, uint16 flag, ObjectGuid guid2)
{
    ...
    if (!charmInfo)
    {
        TC_LOG_DEBUG("network", "WorldSession::HandlePetAction(petGuid: %s, tagGuid: %s, spellId: %u, flag: %u): object (GUID: %u Entry: %u TypeId: %hhu) is considered pet-like but doesn't have a charminfo!",
    ...
    switch (flag)
    {
        case ACT_COMMAND:                                   //0x07
            switch (spellid)
            {
                ...
                case COMMAND_FOLLOW:                        //spellid=1792  //FOLLOW
                    ...
                    pet->GetMotionMaster()->MoveFollow(GetPlayer(), PET_FOLLOW_DIST, pet->GetFollowAngle());
                    if (pet->ToPet())
                        pet->ToPet()->ClearCastWhenWillAvailable();
                    ...
                    if (!pet->IsInCombat())
                        pet->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PET_IN_COMBAT);
                    break;
                case COMMAND_ATTACK:                        //spellid=1792  //ATTACK
                {
                    // only place where pet can be player
                    Unit* TargetUnit = ObjectAccessor::GetUnit(*GetPlayer(), guid2);
                    if (!TargetUnit)
                        return;
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            if (guid2)
                unit_target = ObjectAccessor::GetUnit(*GetPlayer(), guid2);
...
{
    Creature* pet = ObjectAccessor::GetCreature(*GetPlayer(), petguid);
    if (!pet)
    {
...
void WorldSession::HandlePetSetAction(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_ERROR("network", "HandlePetSetAction: Unknown %s or owner (%s)", petguid.ToString().c_str(), GetPlayer()->GetGUID().ToString().c_str());
        return;
    }
    ...
    if (!charmInfo)
    {
        TC_LOG_ERROR("network", "WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %hhu) is considered pet-like but doesn't have a charminfo!", pet->GetGUID().GetCounter(), pet->GetTypeId());
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        TC_LOG_DEBUG("network", "Player %s has changed pet spell action. Position: %u, Spell: %u, State: 0x%X",
            GetPlayer()->GetName().c_str(), position[i], spell_id, uint32(act_state));
...
void WorldSession::HandlePetRename(WorldPacket& recvData)
{
    ...
    if (!pet || pet->getPetType()!= HUNTER_PET || !pet->HasByteFlag(UNIT_FIELD_BYTES_2, 2, UNIT_CAN_BE_RENAMED) ||
        pet->GetOwnerGUID() != GetPlayer()->GetGUID() || !pet->GetCharmInfo())
        return;
...
void WorldSession::HandlePetAbandon(WorldPacket& recvData)
{
    ...
    // pet/charmed
    if (Creature* pet = ObjectAccessor::GetCreature(*GetPlayer(), guid))
    {
        if (pet->IsPet())
        {
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPacket& recvPacket)
{
    ...
    if (!GetPlayer()->GetGuardianPet() && !GetPlayer()->GetCharm())
        return;

    Creature* pet = ObjectAccessor::GetCreature(*GetPlayer(), guid);

    if (!pet || (pet != GetPlayer()->GetGuardianPet() && pet != GetPlayer()->GetCharm()))
    {
    ...
    if (!charmInfo)
    {
        TC_LOG_ERROR("network", "WorldSession::HandlePetSpellAutocastOpcod: object (GUID: %u TypeId: %hhu) is considered pet-like but doesn't have a charminfo!", pet->GetGUID().GetCounter(), pet->GetTypeId());
        return;
    }
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    if (!caster || (caster != GetPlayer()->GetGuardianPet() && caster != GetPlayer()->GetCharm()))
    {
    ...
    if (result == SPELL_CAST_OK)
    {
        ...
        if (spell->GetCastResult() == SPELL_CAST_OK)
            RG_LOG<GMLogModule>(GetPlayer(), spell, caster, targets);
    }
    else
    {
        spell->SendPetCastResult(result);
...
void WorldSession::HandlePetLearnTalent(WorldPacket& recvData)
{
    ...
    GetPlayer()->LearnPetTalent(guid, talentId, requestedRank);
