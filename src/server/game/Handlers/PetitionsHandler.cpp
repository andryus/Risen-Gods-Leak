...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket& recvData)
{
    ...
    else
    {
        ...
        if (!ArenaTeamMgr::HasPlayerAllowedArenaLevel(GetPlayer()->getLevel()))
        {
            SendNotification(LANG_ARENA_ONE_TOOLOW, sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL));
            return;
        }

        if (!ArenaTeamMgr::IsAllowedArenaType(clientIndex))
        {
            SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, name, "", ERR_ARENA_TEAMS_LOCKED);
            return;
        }
    ...
    {                                                       //player hasn't got enough money
        GetPlayer()->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, creature, charterid, 0);
        return;
    }
    ...
    RG_LOG<ItemLogModule>(charter, RG::Logging::ItemLogType::ADDED, pProto->BuyCount);
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& recvData)
{
    ...
    if (type == GUILD_CHARTER_TYPE && GetPlayer()->GetGuildId())
        return;
    ...
    data << uint64(GetPlayer()->GetGUID());                     // owner guid
    ...
    TC_LOG_DEBUG("network", "SMSG_PETITION_SHOW_SIGNATURES Data: Guid: " UI64FMTD " Owner: " UI64FMTD " Guild: %u Signs: %u %s", petitionguid.GetRawValue(), GetPlayer()->GetGUID().GetRawValue(), petitionGuidLow, signs, dbg.str().c_str());
...
void WorldSession::SendPetitionQueryOpcode(ObjectGuid petitionguid)
{
    ...
    data << uint32(petitionguid.GetCounter());              // guild/team guid (in Trinity always same as petition guid.GetCounter()
    ...
    TC_LOG_DEBUG("network", "SMSG_PETITION_QUERY_RESPONSE Data: Guid " UI64FMTD " (%u) Owner " UI64FMTD "Name %s Type %u Ident %u", petitionguid.GetRawValue(), petitionguid.GetCounter(), ownerguid.GetRawValue(), name.c_str(), type, uint32(type != GUILD_CHARTER_TYPE));
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& recvData)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && GetPlayer()->GetTeam() != player::PlayerCache::GetTeam(ownerGuid))
    {
        if (type != GUILD_CHARTER_TYPE)
    ...
    if (type != GUILD_CHARTER_TYPE)
    {
        ...
        {
            SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", GetPlayer()->GetName().c_str(), ERR_ARENA_TEAM_TARGET_TOO_LOW_S);
            return;
        }
        ...
        {
            SendArenaTeamCommandResult(ERR_ARENA_TEAM_INVITE_SS, "", GetPlayer()->GetName().c_str(), ERR_ALREADY_IN_ARENA_TEAM_S);
            return;
        }

        if (GetPlayer()->GetArenaTeamIdInvited())
        {
            SendArenaTeamCommandResult(ERR_ARENA_TEAM_INVITE_SS, "", GetPlayer()->GetName().c_str(), ERR_ALREADY_INVITED_TO_ARENA_TEAM_S);
            return;
        }
    }
    else
    {
        if (GetPlayer()->GetGuildId())
        {
            Guild::SendCommandResult(this, GUILD_COMMAND_INVITE, ERR_ALREADY_IN_GUILD_S, GetPlayer()->GetName());
            return;
        }
        if (GetPlayer()->GetGuildIdInvited())
        {
            Guild::SendCommandResult(this, GUILD_COMMAND_INVITE, ERR_ALREADY_INVITED_TO_GUILD_S, GetPlayer()->GetName());
            return;
        }
    }
    ...
    TC_LOG_DEBUG("network", "PETITION SIGN: %s by player: %s (GUID: %u Account: %u)", petitionGuid.ToString().c_str(), GetPlayer()->GetName().c_str(), playerGuid, GetAccountId());
...
void WorldSession::HandlePetitionDeclineOpcode(WorldPacket& recvData)
{
    ...
    ObjectGuid petitionguid;
    recvData >> petitionguid;                              // petition guid
    TC_LOG_DEBUG("network", "Petition %s declined by %u", petitionguid.ToString().c_str(), GetPlayer()->GetGUID().GetCounter());
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket& recvData)
{
    ...
    player = ObjectAccessor::GetPlayer(*GetPlayer(), plguid);
    if (!player)
        return;
    ...
    if (type != GUILD_CHARTER_TYPE)
    {
        ...
        // check if the players have the same level
        if (player->getLevel() < this->GetPlayer()->getLevel())
        {
            SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", player->GetName().c_str(), ERR_ARENA_TEAM_TARGET_TOO_LOW_S);
            return;
        }
        else if (player->getLevel() > this->GetPlayer()->getLevel())
        {
            SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", player->GetName().c_str(), ERR_ARENA_TEAM_TARGET_TOO_HIGH_S);
            return;
        }
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_DEBUG("network", "Petition %s turned in by %u", petitionGuid.ToString().c_str(), GetPlayer()->GetGUID().GetCounter());
    ...
    {
        TC_LOG_ERROR("network", "Player %s (guid: %u) tried to turn in petition (%s) that is not present in the database", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUID().GetCounter(), petitionGuid.ToString().c_str());
        return;
    }

    // Only the petition owner can turn in the petition
    if (GetPlayer()->GetGUID().GetCounter() != ownerguidlo)
        return;
    ...
    SQLTransaction trans = CharacterDatabase.BeginTransaction();
    // RG-Custom ItemLogging
    RG_LOG<ItemLogModule>(item, RG::Logging::ItemLogType::DESTROYED_BY_TURN_IN);
    ...
    {
        // Create guild
        auto guild = sGuildMgr->CreateGuild(trans, GetPlayer(), name);
        if (!guild)
            return;
    ...
    else
    {
        ...
        if (!arenaTeam->Create(GetPlayer()->GetGUID(), type, name, background, icon, iconcolor, border, bordercolor))
        {
            delete arenaTeam;
            return;
        }
    ...
    // created
    TC_LOG_DEBUG("network", "Player %s (%s) turning in petition %s", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUID().ToString().c_str(), petitionGuid.ToString().c_str());
