...
{
    if (operation == PARTY_OP_UNINVITE && res == ERR_PARTY_LFG_BOOT_NOT_ELIGIBLE_S)
        if (Group const* grp = GetPlayer()->GetGroup())
            val = grp->GetLfgBootVoteTime();
...
void WorldSession::HandleGroupInviteOpcode(WorldPacket& recvData)
{
    ...
    if (!GetPlayer()->IsGameMaster() && invitedPlayer->IsSecretQueuingActive())
    {
        SendPartyResult(PARTY_OP_INVITE, membername, ERR_BAD_PLAYER_NAME_S);
        return;
    }
    ...
    if (group2 || invitedPlayer->GetGroupInvite())
    {
        ...
        if (group2)
        {
            ...
            data << uint8(0);                                       // count
            data << uint32(0);                                      // unk
            invitedPlayer->SendDirectMessage(std::move(data));
        }
...
void WorldSession::HandleGroupDeclineOpcode(WorldPacket & /*recvData*/)
{
    ...
    leader->SendDirectMessage(std::move(data));
}
...
void WorldSession::HandleRaidTargetUpdateOpcode(WorldPacket& recvData)
{
    ...
    else                                                    // target icon update
    {
        ...
        if (guid.IsPlayer())
        {
            Player* target = ObjectAccessor::GetPlayer(*GetPlayer(), guid);

            if (!target || target->IsHostileTo(GetPlayer()))
                return;
        }

        group->SetTargetIcon(x, GetPlayer()->GetGUID(), guid);
    }
}
...
void WorldSession::HandleGroupChangeSubGroupOpcode(WorldPacket& recvData)
{
    ...
    ObjectGuid guid = player::PlayerCache::GetGUID(name);
...
WorldPacket WorldSession::BuildPartyMemberStatsChangedPacket(Player* player)
{
