...
void WorldSession::HandleArenaTeamInviteOpcode(WorldPacket& recvData)
{
    ...
    if (player->IsSecretQueuingActive())
    {
        SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", invitedName, ERR_ARENA_TEAM_PLAYER_NOT_FOUND_S);
        return;
    }
...
void WorldSession::HandleArenaTeamAcceptOpcode(WorldPacket & /*recvData*/)
{
    ...
    ArenaTeam* arenaTeam = sArenaTeamMgr->GetArenaTeamById(GetPlayer()->GetArenaTeamIdInvited());
    if (!arenaTeam)
        return;

    // Check if player is already in another team of the same size
    if (GetPlayer()->GetArenaTeamId(arenaTeam->GetSlot()))
    {
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && GetPlayer()->GetTeam() != player::PlayerCache::GetTeam(arenaTeam->GetCaptain()))
    {
    ...
    // Add player to team
    if (!arenaTeam->AddMember(GetPlayer()->GetGUID()))
    {
    ...
    // Broadcast event
    arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_JOIN_SS, GetPlayer()->GetGUID(), 2, GetPlayer()->GetName(), arenaTeam->GetName(), "");
}
...
void WorldSession::HandleArenaTeamDeclineOpcode(WorldPacket & /*recvData*/)
{
    ...
    GetPlayer()->SetArenaTeamIdInvited(0);
}
...
void WorldSession::HandleArenaTeamLeaveOpcode(WorldPacket& recvData)
{
    ...
    if (GetPlayer()->GetGUID() == arenaTeam->GetCaptain() && arenaTeam->GetMembersSize() > 1)
    {
    ...
    if (GetPlayer()->GetGUID() == arenaTeam->GetCaptain())
    {
        ...
        arenaTeam->DelMember(GetPlayer()->GetGUID(), true);

    // Broadcast event
    arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_LEAVE_SS, GetPlayer()->GetGUID(), 2, GetPlayer()->GetName(), arenaTeam->GetName(), "");
...
void WorldSession::HandleArenaTeamDisbandOpcode(WorldPacket& recvData)
{
    ...
    if (ArenaTeam* arenaTeam = sArenaTeamMgr->GetArenaTeamById(arenaTeamId))
    {
        ...
        if (arenaTeam->GetCaptain() != GetPlayer()->GetGUID())
            return;
...
void WorldSession::HandleArenaTeamRemoveOpcode(WorldPacket& recvData)
{
    ...
    // Broadcast event
    arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_REMOVE_SSS, ObjectGuid::Empty, 3, name, arenaTeam->GetName(), GetPlayer()->GetName());
}
...
void WorldSession::HandleArenaTeamLeaderOpcode(WorldPacket& recvData)
{
    ...
    // Broadcast event
    arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_LEADER_CHANGED_SSS, ObjectGuid::Empty, 3, GetPlayer()->GetName(), name, arenaTeam->GetName());
}
