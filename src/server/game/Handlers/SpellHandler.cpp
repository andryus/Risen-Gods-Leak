...
void WorldSession::HandleOpenItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED))// wrapped?
    {
        ...
        CharacterDatabase.AsyncQuery(stmt)
                .via(GetPlayer()->GetExecutor()).then([this, guid](PreparedQueryResult result) { HandleWrappedItemCallback(std::move(result), guid); });
    }
    else
        pUser->SendLoot(item->GetGUID(), LOOT_CORPSE);
}

void WorldSession::HandleWrappedItemCallback(PreparedQueryResult result, ObjectGuid guid)
{
    ...
    else
    {
        ...
        RG_LOG<ItemLogModule>(item, RG::Logging::ItemLogType::DESTROYED);
...
void WorldSession::HandleGameobjectReportUse(WorldPacket& recvPacket)
{
    ...
    if (go->UsedByPlayer(GetPlayer(), true))
        return;

    GetPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_USE_GAMEOBJECT, go->GetEntry());
}
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    if (mover != GetPlayer() && mover->IsVehicle())
    {
    ...
    if (caster->GetTypeId() == TYPEID_UNIT && !caster->ToCreature()->HasSpell(spellId))
    {
        ...
        if (!GetPlayer()->IsOnVehicle(caster) || spellInfo->CheckVehicle(GetPlayer()) != SPELL_CAST_OK)
        {
            recvPacket.rfinish(); // prevent spam at ignore packet
            return;
        }
...
void WorldSession::HandleCancelCastOpcode(WorldPacket& recvPacket)
{
    ...
    if (GetPlayer()->IsNonMeleeSpellCast(false))
        GetPlayer()->InterruptNonMeleeSpells(false, spellId, false);
}
...
void WorldSession::HandleCancelAuraOpcode(WorldPacket& recvPacket)
{
    ...
    GetPlayer()->RemoveOwnedAura(spellId, ObjectGuid::Empty, 0, AURA_REMOVE_BY_CANCEL);
    ...
    {
        Unit::AuraEffectList const& auraEffects = GetPlayer()->GetAuraEffectsByType(SPELL_AURA_TRACK_RESOURCES);
        if (!auraEffects.empty())
        {
            ...
            for (std::list<uint32>::iterator it = spellIDs.begin(); it != spellIDs.end(); ++it)
                GetPlayer()->RemoveOwnedAura(*it, ObjectGuid::Empty, 0, AURA_REMOVE_BY_CANCEL);
        }
    }
}
...
void WorldSession::HandleTotemDestroyed(WorldPacket& recvPacket)
{
    ...
    if (!GetPlayer()->m_SummonSlot[slotId])
        return;

    Creature* totem = GetPlayer()->GetMap()->GetCreature(GetPlayer()->m_SummonSlot[slotId]);
...
void WorldSession::HandleSelfResOpcode(WorldPacket & /*recvData*/)
{
    ...
    {
        SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(GetPlayer()->GetUInt32Value(PLAYER_SELF_RES_SPELL));
        if (spellInfo)
            GetPlayer()->CastSpell(GetPlayer(), spellInfo, false, nullptr);
...
void WorldSession::HandleSpellClick(WorldPacket& recvData)
{
    ...
    creature->HandleSpellClick(GetPlayer());
}
...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    if (auto replayMap = GetPlayer()->GetMap()->ToArenaReplayMap())
    {
        if (auto replayData = replayMap->GetReplayData())
            if (auto mirrorImageData = replayData->GetMirrorImageData(guid))
            {
                WorldPacket data(SMSG_MIRRORIMAGE_DATA, 68);

                data << uint64(guid);
                data.append(*mirrorImageData);
...
void WorldSession::HandleUpdateProjectilePosition(WorldPacket& recvPacket)
{
    ...
    Unit* caster = ObjectAccessor::GetUnit(*GetPlayer(), casterGuid);
    if (!caster)
        return;
