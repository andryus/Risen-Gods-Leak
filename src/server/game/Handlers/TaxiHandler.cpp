...
bool WorldSession::SendLearnNewTaxiNode(Creature* unit)
{
    ...
    if (GetPlayer()->m_taxi.SetTaximaskNode(curloc))
    {
        ...
        SendPacket(std::move(msg));

        WorldPacket update(SMSG_TAXINODE_STATUS, 9);
        update << uint64(unit->GetGUID());
        update << uint8(1);
        SendPacket(std::move(update));
...
void WorldSession::HandleMoveSplineDoneOpcode(WorldPacket& recvData)
{
    ...
    // required transport position update in case an spline with enter vehicle flag ends 
    if (GetPlayer()->m_movementInfo.transport.guid)
        GetPlayer()->m_movementInfo.transport.pos = movementInfo.transport.pos;
