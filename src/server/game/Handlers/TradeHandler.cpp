...
constexpr float TRADE_DISTANCE = 11.11f;
...
{
    TC_LOG_DEBUG("network", "WORLD: Ignore Trade %u", GetPlayer()->GetGUID().GetCounter());
    // recvPacket.print_storage();
}

void WorldSession::HandleBusyTradeOpcode(WorldPacket& /*recvPacket*/)
{
    TC_LOG_DEBUG("network", "WORLD: Busy Trade %u", GetPlayer()->GetGUID().GetCounter());
    // recvPacket.print_storage();
}
...
{
    TradeData* trade = GetPlayer()->GetTradeData();
    if (!trade)
        return;
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (myItems[i])
            {
                ...
                if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
                {
                    sLog->outCommand(GetPlayer()->GetSession()->GetAccountId(), "GM %s (Account: %u) trade: %s (Entry: %d Count: %u) to player: %s (Account: %u)",
                        GetPlayer()->GetName().c_str(), GetPlayer()->GetSession()->GetAccountId(),
                ...
                RG_LOG<ItemLogModule>(myItems[i], RG::Logging::ItemLogType::TRADED, 0, 0, GetPlayer()->GetGUID().GetCounter());
            ...
            if (hisItems[i])
            {
                ...
                // RG-Custom ItemLogging
                RG_LOG<ItemLogModule>(hisItems[i], RG::Logging::ItemLogType::TRADED, 0, 0, GetPlayer()->GetGUID().GetCounter());
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (!GetPlayer()->IsWithinDistInMap(trader, TRADE_DISTANCE, false))
    {
    ...
    if (GetPlayer()->GetMoney() >= uint32(MAX_MONEY_AMOUNT) - his_trade->GetMoney())
    {
        info.Status = TRADE_STATUS_CLOSE_WINDOW;
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (Item* item = his_trade->GetItem(TradeSlots(i)))
        {
            ...
            //if (item->IsBindedNotWith(GetPlayer()))   // dont mark as invalid when his item isnt good (not exploitable because if item is invalid trade will fail anyway later on the same check)
            //{
    ...
    if (his_trade->IsAccepted())
    {
        ...
        if (uint32 my_spell_id = my_trade->GetSpell())
        {
            ...
            my_spell = new Spell(GetPlayer(), spellEntry, TRIGGERED_FULL_MASK);
        ...
        myCanCompleteInfo.Result = GetPlayer()->CanStoreItems(hisItems, TRADE_SLOT_TRADED_COUNT, &myCanCompleteInfo.ItemLimitCategoryId);
        ...
        RG_LOG<TradeLogModule>(GetPlayer(), my_trade, trader, his_trade);
        ...
        for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
        {
            if (myItems[i])
            {
                myItems[i]->SetGuidValue(ITEM_FIELD_GIFTCREATOR, GetPlayer()->GetGUID());
        ...
        // update money
        GetPlayer()->ModifyMoney(-int32(my_trade->GetMoney()), ::RG::Logging::MoneyLogType::TRADE_GIVE, trader->GetGUID().GetCounter());
        GetPlayer()->ModifyMoney(his_trade->GetMoney(), ::RG::Logging::MoneyLogType::TRADE_GET, trader->GetGUID().GetCounter());
        trader->ModifyMoney(-int32(his_trade->GetMoney()), ::RG::Logging::MoneyLogType::TRADE_GIVE, GetPlayer()->GetGUID().GetCounter());
        trader->ModifyMoney(my_trade->GetMoney(), ::RG::Logging::MoneyLogType::TRADE_GET, GetPlayer()->GetGUID().GetCounter());
        ...
        GetPlayer()->SaveInventoryAndCurrenciesToDB(trans);
        trader->SaveInventoryAndCurrenciesToDB(trans);
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    Player* pOther = ObjectAccessor::GetPlayer(*GetPlayer(), ID);
    ...
    info.TraderGuid = GetPlayer()->GetGUID();
