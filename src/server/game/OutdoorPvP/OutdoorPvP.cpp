...
class DefenseMessageBuilder
{
    public:
        ...
        void operator()(WorldPacket& data, LocaleConstant locale) const
        {
            ...
            data.write<uint32>(_zoneId);
            data.write<uint32>(text.length());
            data << text;
        }
...
OPvPCapturePoint::OPvPCapturePoint(OutdoorPvP* pvp):
    m_capturePoint(nullptr), m_maxValue(0.0f), m_minValue(0.0f), m_maxSpeed(0),
...
bool OPvPCapturePoint::DelCapturePoint()
{
    if (m_capturePoint)
    {
        sObjectMgr->DeleteGOData(m_capturePoint->GetGUID().GetCounter());
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    for (uint32 team = 0; team < 2; ++team)
    {
        for (GuidSet::iterator itr = m_activePlayers[team].begin(); itr != m_activePlayers[team].end();)
        {
            ...
            if (Player* player = ObjectAccessor::GetPlayer(*m_capturePoint, playerGuid))
    ...
    m_capturePoint->GetPlayerListInGrid(players, radius);
...
void OPvPCapturePoint::SendUpdateWorldState(uint32 field, uint32 value)
{
    for (uint32 team = 0; team < 2; ++team)
    {
        ...
        for (GuidSet::iterator itr = m_activePlayers[team].begin(); itr != m_activePlayers[team].end(); ++itr)
            if (Player* const player = ObjectAccessor::GetPlayer(*m_capturePoint, *itr))
                player->SendUpdateWorldState(field, value);
    }
}
