...
enum LFGMgrEnum
{
    ...
    LFG_QUEUEUPDATE_INTERVAL                     = 2 * IN_MILLISECONDS,
...
typedef std::map<uint32, time_t> TimedDungeonSet;
...
{
    LfgQueueStatusData(int32 waitTimeAvg, uint32 queuedTime, uint8 tanks, uint8 healers, uint8 dps) : // Everything else has no known effect for client (dungeonId must be != 0)
        dungeonId(1), waitTime(-1), waitTimeAvg(waitTimeAvg), waitTimeTank(-1), waitTimeHealer(-1), waitTimeDps(-1),
        queuedTime(queuedTime), tanks(tanks), healers(healers), dps(dps) {}
...
struct LfgProposal
{
    LfgProposal(uint32 dungeon = 0): id(0), dungeonId(dungeon), state(LFG_PROPOSAL_INITIATING),
        group(), leader(), cancelTime(0), encounters(0), isNew(true), needSendUpdate(true)
        { }
    ...
    bool needSendUpdate;                                   ///< Whether or not we need to send an update
    ...
    LfgDungeonSet commonDungeons; // To store queue times for
...
struct LfgRoleCheck
{
    ...
    uint8 minlevel;                                        ///< Lowest player level
    uint8 maxlevel;                                        ///< Highest player level
...
struct LFGDungeonData
{
    ...
    LFGDungeonData(LFGDungeonEntry const* dbc): id(dbc->ID), name(dbc->name[0]), map(dbc->map),
        ...
        minlevel(dbc->minlevel), maxlevel(dbc->maxlevel), recMinlevel(dbc->recminlevel),
        recMaxlevel(dbc->recmaxlevel), difficulty(Difficulty(dbc->difficulty)),
...
class GAME_API LFGMgr
{
    ...
    public:
        ...
        // LFGHandler
        /// Get locked dungeons
        LfgLockMap const GetLockedDungeons(ObjectGuid guid, bool ignoreBindings = false);
        ...
        void TeleportPlayer(Player* player, bool out, bool teleportToEntry = false, bool forceTeleport = false);
    ...
    private:
        ...
        std::future<void> currentUpdate; // Used for asynchronous calculations (see StartUpdate)
        ...
        void GetCompatibleDungeons(LfgDungeonSet& dungeons, GuidSet const& players, LfgLockPartyMap& lockMap, bool ignoreBindings);
        void GetDungeonTimes(LfgDungeonSet const& dungeons, TimedDungeonSet& timedDungeons, uint32 minlevel, uint32 maxlevel, bool isRandom);
        uint32 GetDungeonRecommendedLevelDiff(uint32 dungeon, uint32 minlevel, uint32 maxlevel);
