...
// If needed, asynchronously updates queue data for later use in UpdateFinish
void LFGMgr::UpdateStart()
{
    for (LfgQueueContainer::iterator it = QueuesStore.begin(); it != QueuesStore.end(); ++it)
        if (it->second.NeedsUpdate())
        {
            for (auto& queue : QueuesStore)
            {
                queue.second.SetDebugUpdating(true);
            }

            currentUpdate = thread_pool.schedule(&LFGMgr::UpdateQueues, this);
...
{
    for (LfgQueueContainer::iterator it = QueuesStore.begin(); it != QueuesStore.end(); ++it) {
        it->second.FindGroups();
        it->second.RefreshBestArrangements();
        it->second.RefreshWaitTimes();
        it->second.SetDebugUpdating(false);
    }
}

void LFGMgr::UpdateFinish(uint32 diff)
{
    if (!isOptionEnabled(LFG_OPTION_ENABLE_DUNGEON_FINDER | LFG_OPTION_ENABLE_RAID_BROWSER))
        return;

    if (currentUpdate.valid()) // If updates were queued wait and invalidate
    {
        uint32 time = getMSTime();
        currentUpdate.get();
        TC_LOG_INFO("lfg.queue", "LfgMgr update overtime: %u.", GetMSTimeDiffToNow(time));
    }
    ...
    for (LfgProposalContainer::const_iterator itProposal = ProposalsStore.find(m_lfgProposalId); itProposal != ProposalsStore.end(); ++itProposal)
    {
        ...
        if (!proposal.needSendUpdate)
            continue;

        proposal.needSendUpdate = false;
        // update encounter mask
        if (!proposal.isNew)
        if (Group* grp = sGroupMgr->GetGroupByGUID(proposal.group.GetCounter()))
        {
            if (LFGDungeonData const* dungeon = GetLFGDungeon(proposal.dungeonId))
            for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                if (Player* plrg = itr->GetSource())
                {
                    if (auto map = sMapMgr->CreateMap(uint32(dungeon->map), plrg))
                        if (auto instance = map->ToInstanceMap())
                            proposal.encounters = instance->GetCompletedEncounterMask();
    ...
    if (m_QueueTimer > LFG_QUEUEUPDATE_INTERVAL)
    {
        m_QueueTimer = 0;
        for (LfgQueueContainer::iterator it = QueuesStore.begin(); it != QueuesStore.end(); ++it)
            it->second.SendQueueData(currTime);
    }
    else
        m_QueueTimer += diff;
}
...
void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons, const std::string& comment)
{
    ...
    if (state == LFG_STATE_QUEUED)
    {
        ...
        TC_LOG_DEBUG("lfg.queue", "RemoveFromSystem called from JoinLfg for %s.", guid.ToString().c_str());
        queue.RemoveFromSystem(gguid);
    }
    ...
    else if (player->HasAura(LFG_SPELL_DUNGEON_COOLDOWN) && (!player->GetGroup() || !player->GetGroup()->isLFGGroup()))
    ...
    else if (grp)
    {
        ...
        else if (isContinue && grp->GetMembersCount() == MAXGROUPSIZE)
            joinData.result = LFG_JOIN_GROUPFULL;
        else
        {
            uint8 memberCount = 0;
            for (GroupReference* itr = grp->GetFirstMember(); itr != NULL && joinData.result == LFG_JOIN_OK; itr = itr->next())
            {
                if (Player* plrg = itr->GetSource())
                {
                    ...
                    else if (plrg->HasAura(LFG_SPELL_DUNGEON_COOLDOWN) && (!plrg->GetGroup() || !plrg->GetGroup()->isLFGGroup()))
    ...
    if (joinData.result == LFG_JOIN_OK && !isContinue && rDungeonId)
    {
        ...
        else if (grp)
        {
            for (GroupReference* itr = grp->GetFirstMember(); itr != NULL && joinData.result == LFG_JOIN_OK; itr = itr->next())
                if (Player* plrg = itr->GetSource())
                    if (plrg->HasAura(LFG_SPELL_DUNGEON_COOLDOWN))
                        joinData.result = LFG_JOIN_PARTY_RANDOM_COOLDOWN;
        }
    }
    ...
    if (joinData.result == LFG_JOIN_OK)
    {
        ...
        if (joinData.result == LFG_JOIN_OK)
        {
            ...
            GetCompatibleDungeons(dungeons, players, joinData.lockmap, rDungeonId != 0 || isContinue);
            if (dungeons.empty())
    ...
    if (grp)                                               // Begin rolecheck
    {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            if (Player* plrg = itr->GetSource())
            {
                ...
                roleCheck.minlevel = std::min(roleCheck.minlevel, plrg->getLevel());
                roleCheck.maxlevel = std::max(roleCheck.maxlevel, plrg->getLevel());
            }
        }
        // Update leader role
    ...
    else                                                   // Add player to queue
    {
        ...
        TimedDungeonSet dungeonsTimes;
        GetDungeonTimes(dungeons, dungeonsTimes, player->getLevel(), player->getLevel(), rDungeonId != 0);
        queue.AddNewToQueue(guid, time(NULL), dungeonsTimes, rolesMap, isContinue);
...
void LFGMgr::LeaveLfg(ObjectGuid guid)
{
    ...
    switch (state)
    {
        case LFG_STATE_QUEUED:
            TC_LOG_DEBUG("lfg.queue", "RemoveFromSystem called from LeaveLfg for %s.", guid.ToString().c_str());
            if (gguid)
            {
                LFGQueue& queue = GetQueue(gguid);
                queue.RemoveFromSystem(gguid);
                RestoreState(gguid, "Group leaves lfg queue");
                GuidSet const& players = GetPlayers(gguid);
                for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
                {
                    RestoreState(*it, "Group leaves lfg queue");
...
{
    ObjectGuid pguid = ObjectGuid::Empty, gguid = ObjectGuid::Empty;
    if (guid.IsGroup())
        gguid = guid;
    ...
    if (pguid)
        ...
        {
            TC_LOG_DEBUG("lfg.queue", "RemoveFromSystem called from LeaveAllLfgQueues for %s", guid.ToString().c_str());
            itr->second.RemoveFromSystem(pguid);
        }
    if (gguid)
        for (lfg::LfgQueueContainer::iterator itr = QueuesStore.begin(); itr != QueuesStore.end(); ++itr)
        {
            TC_LOG_DEBUG("lfg.queue", "RemoveFromSystem called from LeaveAllLfgQueues for %s", guid.ToString().c_str());
            itr->second.RemoveFromSystem(gguid);
        }
...
void LFGMgr::UpdateRoleCheck(ObjectGuid gguid, ObjectGuid guid /* = ObjectGuid::Empty */, uint8 roles /* = PLAYER_ROLE_NONE */)
{
    ...
    if (roleCheck.state == LFG_ROLECHECK_FINISHED)
    {
        SetState(gguid, LFG_STATE_QUEUED);
        LFGQueue& queue = GetQueue(gguid);
        TimedDungeonSet dungeons;
        GetDungeonTimes(roleCheck.dungeons, dungeons, roleCheck.minlevel, roleCheck.maxlevel, roleCheck.rDungeonId != 0);
        queue.AddNewToQueue(gguid, time_t(time(NULL)), dungeons, roleCheck.roles, IsLfgGroup(gguid));
        RoleChecksStore.erase(itRoleCheck);
    }
...
uint32 LFGMgr::GetDungeonRecommendedLevelDiff(uint32 dungeon, uint32 minlevel, uint32 maxlevel)
{
    auto dungeonEntry = GetLFGDungeon(dungeon);
    if (!dungeonEntry)
        return 0;

    uint32 lvldiff = 0;
    if (minlevel < dungeonEntry->recMinlevel)
        lvldiff = dungeonEntry->recMinlevel - minlevel;
    if (maxlevel > dungeonEntry->recMaxlevel)
        lvldiff = std::max(lvldiff, maxlevel - dungeonEntry->recMaxlevel);

    return lvldiff;
}

void LFGMgr::GetDungeonTimes(LfgDungeonSet const& dungeons, TimedDungeonSet& timedDungeons, uint32 minlevel, uint32 maxlevel, bool isRandom)
{
    if (isRandom)
    {
        uint32 minDiff = MAX_LEVEL;
        std::for_each(dungeons.begin(), dungeons.end(), [&](uint32 dungeon)
            { minDiff = std::min(minDiff, GetDungeonRecommendedLevelDiff(dungeon, minlevel, maxlevel)); });

        auto now = time(NULL);

        std::for_each(dungeons.begin(), dungeons.end(), [&](uint32 dungeon)
        {
            uint32 lvldiff = GetDungeonRecommendedLevelDiff(dungeon, minlevel, maxlevel) - minDiff;
            if (lvldiff > sWorld->getIntConfig(CONFIG_LFG_LEVEL_THRESHOLD))
                timedDungeons[dungeon] = now + lvldiff * sWorld->getIntConfig(CONFIG_LFG_WAITTIME_PER_DIFF);
            else
                timedDungeons[dungeon] = 0;
        });
    }
    else
        std::for_each(dungeons.begin(), dungeons.end(), [&](uint32 dungeon) { timedDungeons[dungeon] = 0; });
}
...
/**
   ...
   @param[in]     ignoreBindings If true player instance ids will be ignored
*/
void LFGMgr::GetCompatibleDungeons(LfgDungeonSet& dungeons, GuidSet const& players, LfgLockPartyMap& lockMap, bool ignoreBindings)
{
    lockMap.clear();
    for (GuidSet::const_iterator it = players.begin(); it != players.end() && !dungeons.empty(); ++it)
    {
        ObjectGuid guid = (*it);
        LfgLockMap const& cachedLockMap = GetLockedDungeons(guid, ignoreBindings);
...
void LFGMgr::MakeNewGroup(LfgProposal const& proposal)
{
    ...
    if (proposal.isNew)
        grp->UnbindInstance(dungeon->map, dungeon->difficulty, true);
    ...
    for (GuidList::const_iterator it = playersToTeleport.begin(); it != playersToTeleport.end(); ++it)
        ...
        {
            if (player->GetGroup() != grp) // could not add because group was full (some shitness happened)
                continue;
            // Add the cooldown spell if queued for a random dungeon
            if (selectedRandomLfgDungeon(player->GetGUID()) && !player->HasAura(LFG_SPELL_DUNGEON_COOLDOWN))
                player->AddAura(LFG_SPELL_DUNGEON_COOLDOWN, player);
            TeleportPlayer(player, false, proposal.isNew, inLfgDungeonMap(player->GetGUID(), player->GetMapId(), player->GetMap()->GetDifficulty()));
        }
    
    TC_LOG_INFO("lfg.proposal", "Proposal successful. Create new group: players: %s dungeon: %i (pool: %s) isNew: %i", 
        std::accumulate(players.begin(), players.end(), std::string(), [](std::string a, ObjectGuid b) -> std::string
        { return a + (a.empty() ? "" : ", ")+ std::to_string(b.GetCounter()); }).c_str(), proposal.dungeonId, ConcatenateDungeons(proposal.commonDungeons).c_str(), proposal.isNew);
...
void LFGMgr::UpdateProposal(uint32 proposalId, ObjectGuid guid, bool accept)
{
    ...
    for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin(); it != proposal.players.end(); ++it)
    {
        ...
        if (gguid)
        {
            TC_LOG_DEBUG("lfg.queue", "GetJoinTime called from UpdateProposal (gguid) for %s.", guid.ToString().c_str());
            waitTime = int32((joinTime - queue.GetJoinTime(gguid)));
            SendLfgUpdateParty(pguid, updateData);
        }
        else
        {
            TC_LOG_DEBUG("lfg.queue", "GetJoinTime called from UpdateProposal for %s.", guid.ToString().c_str());
        ...
        waitTimes.insert(std::make_pair((LfgRoles)(it->second.role & ~PLAYER_ROLE_LEADER), waitTime));

        SetState(pguid, LFG_STATE_DUNGEON);
    }

    if (proposal.queues.size() > MAXGROUPSIZE / 2) // Groups with few premades should not be regarded for average queue times
        queue.UpdateWaitTime(proposal.commonDungeons, waitTimes);

    // Remove players/groups from Queue
    for (GuidList::const_iterator it = proposal.queues.begin(); it != proposal.queues.end(); ++it)
    {
        TC_LOG_DEBUG("lfg.queue", "RemoveFromSystem called from UpdateProposal for %s.", guid.ToString().c_str());
        queue.RemoveFromSystem(*it);
    }
...
void LFGMgr::RemoveProposal(LfgProposalContainer::iterator itProposal, LfgUpdateType type)
{
    ...
    // Just a workaround for a bug where queue data accidently gets deleted!
    for (GuidList::const_iterator it = proposal.queues.begin(); it != proposal.queues.end(); ++it)
        if (!GetQueue(*it).HasData(*it))
            toRemove.insert(*it);
    ...
    {
        ObjectGuid guid = *it;
        TC_LOG_DEBUG("lfg.queue", "RemoveFromSystem called from RemoveProposal for %s.", guid.ToString().c_str());
        queue.RemoveFromSystem(guid);
    ...
    {
        ObjectGuid guid = *it;
        TC_LOG_DEBUG("lfg.queue", "AddOldToQueue called from RemoveProposal for %s.", guid.ToString().c_str());
        queue.AddOldToQueue(guid);
    }
...
/**
   ...
   @param[in]     teleportToEntry Teleport to dungeon entry or group member position? (Default false)
   @param[in]     forceTeleport Always teleport player even within dungeons (Default false)
*/
void LFGMgr::TeleportPlayer(Player* player, bool out, bool teleportToEntry /*= false*/, bool forceTeleport /*= false*/)
{
    ...
    else if (player->GetMapId() != uint32(dungeon->map) || forceTeleport)  // Do not teleport players in dungeon to the entrance
    {
        ...
        if (!teleportToEntry && !forceTeleport)
        {
        ...
        if (!player->TeleportTo(mapid, x, y, z, orientation, forceTeleport ? uint32(TELE_FORCE_TELEPORT_FAR) : 0U))
            error = LFG_TELEPORTERROR_INVALID_LOCATION;
    }
    else
        error = LFG_TELEPORTERROR_INVALID_LOCATION;
    ...
    for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        else
        {
            ...
            if (quest)
            {
                ...
                player->RewardQuest(quest, 0, NULL, false, true);
            }
            else
                done = true;
        }
        ...
        // send reward notification only if we have received a reward
        if (!done)
        {
...
LfgState LFGMgr::GetState(ObjectGuid guid)
{
    ...
    if (guid.IsGroup())
    {
        ...
        TC_LOG_TRACE("lfg.data.group.state.get", "Group: %u, State: %s", guid.GetCounter(), GetStateString(state).c_str());
    }
    else
    {
        state = PlayersStore[guid].GetState();
        TC_LOG_TRACE("lfg.data.player.state.get", "Player: %u, State: %s", guid.GetCounter(), GetStateString(state).c_str());
    }
...
bool LFGMgr::IsVoteKickActive(ObjectGuid gguid)
{
    ...
    TC_LOG_TRACE("lfg.data.group.votekick.get", "Group: %u, Active: %d", gguid.GetCounter(), active);
...
LfgLockMap const LFGMgr::GetLockedDungeons(ObjectGuid guid, bool ignoreBindings)
{
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
    {
        ...
        if (denyJoin)
            ...
            lockData = LFG_LOCKSTATUS_RAID_LOCKED;
        else if (!ignoreBindings && dungeon->difficulty > DUNGEON_DIFFICULTY_NORMAL && player->GetBoundInstance(dungeon->map, Difficulty(dungeon->difficulty), false))
            lockData = LFG_LOCKSTATUS_RAID_LOCKED;
...
void LFGMgr::SetVoteKick(ObjectGuid gguid, bool active)
{
    ...
    TC_LOG_TRACE("lfg.data.group.votekick.set", "Group: %u, New state: %d, Previous: %d",
        gguid.GetCounter(), active, data.IsVoteKickActive());
