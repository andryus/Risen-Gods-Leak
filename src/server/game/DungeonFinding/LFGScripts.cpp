...
void LFGPlayerScript::OnLogin(Player* player, bool /*loginFirst*/)
{
    ...
    sLFGMgr->SetTeam(player->GetGUID(), player->GetOTeam());
...
void LFGGroupScript::OnAddMember(Group* group, ObjectGuid guid)
{
    ...
    if (!group->isLFGGroup())
    {
        sLFGMgr->LeaveAllLfgQueues(leader, true, gguid); // invited, queued, party formed, neither party nor new member are queued, but leader is in queue solo!
