...
constexpr Rates qualityToRate[MAX_ITEM_QUALITY] =
{
...
GAME_API LootStore LootTemplates_Creature("creature_loot_template", "creature entry", true);
GAME_API LootStore LootTemplates_Disenchant("disenchant_loot_template", "item disenchant id", true);
GAME_API LootStore LootTemplates_Fishing("fishing_loot_template", "area id", true);
GAME_API LootStore LootTemplates_Gameobject("gameobject_loot_template", "gameobject entry", true);
GAME_API LootStore LootTemplates_Item("item_loot_template", "item entry", true);
GAME_API LootStore LootTemplates_Mail("mail_loot_template", "mail template id", false);
GAME_API LootStore LootTemplates_Milling("milling_loot_template", "item entry (herb)", true);
GAME_API LootStore LootTemplates_Pickpocketing("pickpocketing_loot_template", "creature pickpocket lootid", true);
GAME_API LootStore LootTemplates_Prospecting("prospecting_loot_template", "item entry (ore)", true);
GAME_API LootStore LootTemplates_Reference("reference_loot_template", "reference id", false);
GAME_API LootStore LootTemplates_Skinning("skinning_loot_template", "creature skinning id", true);
GAME_API LootStore LootTemplates_Spell("spell_loot_template",                 "spell id (random item creating)", false);
    ...
    bool operator()(LootStoreItem* item) const
    {
        ...
        for (std::vector<LootItem>::const_iterator itr = _loot.items.begin(); itr != _loot.items.end(); ++itr)
            if (itr->itemid == item->itemid)
                // some tokens don't count for max duplicates
                if (++foundDuplicates == ((sObjectMgr->GetItemTemplate(item->itemid)->DisplayInfoID == 41438
                                        || sObjectMgr->GetItemTemplate(item->itemid)->DisplayInfoID == 64877) ? 4 : _loot.maxDuplicates))
                    return true;
...
bool LootStoreItem::Roll(bool rate, float OverrideChance) const
{
    if (chance >= 100.0f)
        return true;

    float rollChance = OverrideChance ? OverrideChance : chance;

    if (reference > 0)                                   // reference case
        return roll_chance_f(rollChance* (rate ? sWorld->getRate(RATE_DROP_ITEM_REFERENCED) : 1.0f));

    ItemTemplate const* pProto = sObjectMgr->GetItemTemplate(itemid);

    float qualityModifier = pProto && rate ? sWorld->getRate(qualityToRate[pProto->Quality]) : 1.0f;

    return roll_chance_f(rollChance*qualityModifier);
}
...
bool LootItem::AllowedForTeam(Player const* player) const
{
    ItemTemplate const* pProto = sObjectMgr->GetItemTemplate(itemid);
    if (!pProto)
        return false;
...
void packet::BufferIO<LootItem>::write(ByteBuffer& b, const LootItem& li)
{
    b << uint32(li.itemid);
    ...
    if (!personal && group)
    {
        ...
        for (Group::MemberSlotList::const_iterator itr = members.begin(); itr != members.end(); ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(itr->guid))
                FillNotNormalLootFor(player, player->IsAtGroupLootDistance(lootOwner));
            else
                FillNotNormalLootForOffline(itr->guid.GetCounter());
...
void Loot::FillNotNormalLootForOffline(uint32 guid)
{
    QuestItemMap::const_iterator qmapitr = PlayerQuestItems.find(guid);
    if (qmapitr == PlayerQuestItems.end())
        FillQuestLootOffline(guid);

    qmapitr = PlayerFFAItems.find(guid);
    if (qmapitr == PlayerFFAItems.end())
        FillFFALootOffline(guid);

    qmapitr = PlayerNonQuestNonFFAConditionalItems.find(guid);
    if (qmapitr == PlayerNonQuestNonFFAConditionalItems.end())
        FillNonQuestNonFFAConditionalLootOffline(guid);
}
...
QuestItemList* Loot::FillFFALootOffline(uint32 guid)
{
    ...
    for (uint8 i = 0; i < items.size(); ++i)
    {
        ...
        {
            ql->push_back(QuestItem(i, false, true));
            ++unlootedCount;
        }
    }
    if (ql->empty())
...
QuestItemList* Loot::FillQuestLoot(Player* player)
{
    ...
    for (uint8 i = 0; i < quest_items.size(); ++i)
    {
        ...
        if (!item.is_looted && (item.AllowedForPlayer(player) || (item.follow_loot_rules && item.AllowedForTeam(player) && player->GetGroup() && ((player->GetGroup()->GetLootMethod() == MASTER_LOOT && player->GetGroup()->GetMasterLooterGuid() == player->GetGUID()) || player->GetGroup()->GetLootMethod() != MASTER_LOOT))))
        {
            ql->push_back(QuestItem(i));
...
QuestItemList* Loot::FillQuestLootOffline(uint32 guid)
{
...
QuestItemList* Loot::FillNonQuestNonFFAConditionalLoot(Player* player, bool presentAtLooting)
{
    ...
    {
        LootItem &item = items[i];
        if (!item.is_looted && !item.freeforall && (item.AllowedForPlayer(player) || (item.follow_loot_rules && item.AllowedForTeam(player) && player->GetGroup() && ((player->GetGroup()->GetLootMethod() == MASTER_LOOT && player->GetGroup()->GetMasterLooterGuid() == player->GetGUID()) || player->GetGroup()->GetLootMethod() != MASTER_LOOT))))
        {
            if (presentAtLooting)
...
QuestItemList* Loot::FillNonQuestNonFFAConditionalLootOffline(uint32 guid)
{
    QuestItemList *ql = new QuestItemList();
    ...
    PlayerNonQuestNonFFAConditionalItems[guid] = ql;
    return ql;
}

//===================================================

void Loot::NotifyItemRemoved(uint8 lootIndex, Player const& remover)
{
    ...
    for (GuidSet::iterator i = PlayersLooting.begin(); i != PlayersLooting.end(); i = i_next)
    {
        i_next = i;
        ++i_next;
        if (Player* player = ObjectAccessor::GetPlayer(remover, *i))
...
void Loot::NotifyMoneyRemoved(Player const& remover)
{
...
void Loot::NotifyQuestItemRemoved(uint8 questIndex, Player const& remover)
{
...
LootItem* Loot::LootItemInSlot(uint32 lootSlot, Player* player, QuestItem* *qitem, QuestItem* *ffaitem, QuestItem* *conditem)
{
    ...
    if (lootSlot >= items.size())
    {
        ...
        if (itr != PlayerQuestItems.end() && questSlot < itr->second->size())
        {
            ...
            if (!qitem2->check_at_loot || quest_items[qitem2->index].AllowedForPlayer(player))
            {
                if (qitem)
                    *qitem = qitem2;
            ...
            if (itr != PlayerFFAItems.end())
            {
                for (QuestItemList::const_iterator iter=itr->second->begin(); iter!= itr->second->end(); ++iter)
                    if (iter->index == lootSlot)
                    {
                        ...
                        if (!ffaitem2->check_at_loot || item->AllowedForPlayer(player))
                        {
                            if (ffaitem)
                                *ffaitem = ffaitem2;
            ...
            if (itr != PlayerNonQuestNonFFAConditionalItems.end())
            {
                for (QuestItemList::const_iterator iter=itr->second->begin(); iter!= itr->second->end(); ++iter)
                {
                    if (iter->index == lootSlot)
                    {
                        ...
                        if (!conditem2->check_at_loot || item->AllowedForPlayer(player))
                        {
                            if (conditem)
                                *conditem = conditem2;
...
void packet::BufferIO<LootView>::write(ByteBuffer& b, LootView lv)
{
    ...
    if (q_itr != lootPlayerQuestItems.end())
    {
        ...
        for (QuestItemList::const_iterator qi = q_list->begin(); qi != q_list->end(); ++qi)
        {
            ...
            if (!qi->is_looted && !item.is_looted)
            {
                ...
                b << item;
                if (item.follow_loot_rules && item.AllowedForTeam(lv.viewer))
                {
                    switch (lv.permission)
                    {
                    case MASTER_PERMISSION:
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId) const
{
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        uint32 questId = 0;
        uint32 step = QUEST_ITEM_DROP_STEP_INVALID;
        float overrideChance = 0;
        bool isItemNeededForQuest = false;

        // item is a quest item AND player is solo
        if (item->needs_quest && item->chance >= 10.0f && !loot.lootOwnerPlayer->GetGroup())
        {
            // check if player needs this item for a quest
            questId = loot.lootOwnerPlayer->GetQuestIdOfItemId(item->itemid);
            if (questId)
            {
                // try to get the step
                step = loot.lootOwnerPlayer->GetQuestItemDropStep(questId);

                // if step was found calculate highter drop chance
                if (step != QUEST_ITEM_DROP_STEP_INVALID)
                    overrideChance = CalculateQuestDropChance(step, item->chance);
                else
                    // if step is not found create step 0
                    loot.lootOwnerPlayer->IncreaseOrAddQuestItemDropStep(questId);

                isItemNeededForQuest = true;
            }
        }

        if (!item->Roll(rate, overrideChance))
        {
            // if the item is a quest item AND the player needs the item increase the step
            if (isItemNeededForQuest)
                loot.lootOwnerPlayer->IncreaseOrAddQuestItemDropStep(questId);

            continue;                                           // Bad luck for the entry
        }
        // only try to reset if the player needs the item
        else if (isItemNeededForQuest)
            loot.lootOwnerPlayer->ResetQuestItemDropStep(questId);
...
float LootTemplate::CalculateQuestDropChance(uint32 step, float dbPct) const
{
    if (step == QUEST_ITEM_DROP_STEP_3)
        return 100.0f;
    else if (step == QUEST_ITEM_DROP_STEP_0)
        return dbPct;

    float newPct = 100 - dbPct;
    newPct /= 3;
    return dbPct + (newPct * step);
}
