...
void CreatureTextMgr::LoadCreatureTexts()
{
    ...
    mTextMap.reserve(result->GetRowCount());
    ...
    do
    {
        ...
        mTextMap[std::make_pair(temp.entry, temp.group)].push_back(temp);
    }
    while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %lu creature texts in %u ms", mTextMap.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void CreatureTextMgr::LoadCreatureTextLocales()
{
    ...
    do
    {
        ...
        fields += 3;
        ObjectMgr::FillLocaleContainer(fields, loc.Text);
...
uint32 CreatureTextMgr::SendChat(Creature* source, uint8 textGroup, WorldObject const* whisperTarget /*= nullptr*/, ChatMsg msgType /*= CHAT_MSG_ADDON*/, Language language /*= LANG_ADDON*/, CreatureTextRange range /*= TEXT_RANGE_NORMAL*/, uint32 sound /*= 0*/, Team team /*= TEAM_OTHER*/, bool gmOnly /*= false*/, Player* srcPlr /*= nullptr*/)
{
    ...
    CreatureTextMap::const_iterator itr = mTextMap.find(std::make_pair(source->GetEntry(), textGroup));
    if (itr == mTextMap.end())
    {
        TC_LOG_ERROR("sql.sql", "CreatureTextMgr: Could not find Text in group %u for Creature %s (Entry %u, GUID %u) in 'creature_text' table. Ignoring.", uint32(textGroup), source->GetName().c_str(), source->GetEntry(), source->GetGUID().GetCounter());
        return 0;
    }
...
void CreatureTextMgr::SendSound(Creature* source, uint32 sound, ChatMsg msgType, WorldObject const* whisperTarget, CreatureTextRange range, Team team, bool gmOnly)
{
    ...
    data << uint32(sound);
    SendNonChatPacket(source, std::move(data), msgType, whisperTarget, range, team, gmOnly);
}
...
void CreatureTextMgr::SendNonChatPacket(WorldObject* source, WorldPacket&& data, ChatMsg msgType, WorldObject const* whisperTarget, CreatureTextRange range, Team team, bool gmOnly) const
{
    switch (msgType)
    {
        case CHAT_MSG_MONSTER_PARTY:
        {
            ...
            if (Player const* whisperPlayer = whisperTarget->ToPlayer())
            {
                if (Group const* group = whisperPlayer->GetGroup())
                    group->BroadcastWorker([data = std::move(data)](Player* player) { player->SendDirectMessage(data); });
            }
            return;
        }
        ...
        case CHAT_MSG_RAID_BOSS_WHISPER:
        {
            if (range == TEXT_RANGE_NORMAL) // ignores team and gmOnly
            {
                ...
                whisperTarget->ToPlayer()->SendDirectMessage(std::move(data));
                return;
            }
            break;
        }
    ...
    switch (range)
    {
        case TEXT_RANGE_AREA:
        {
            ...
            for (Player& player : source->GetMap()->GetPlayers())
...
void CreatureTextMgr::SetRepeatId(Creature* source, uint8 textGroup, uint8 id)
{
    ...
    if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
        ...
        TC_LOG_ERROR("sql.sql", "CreatureTextMgr: TextGroup %u for Creature(%s) GuidLow %u Entry %u, id %u already added", uint32(textGroup), source->GetName().c_str(), source->GetGUID().GetCounter(), source->GetEntry(), uint32(id));
}
...
bool CreatureTextMgr::TextExist(uint32 sourceEntry, uint8 textGroup)
{
    ...
    CreatureTextMap::const_iterator sList = mTextMap.find({sourceEntry, textGroup});
    if (sList == mTextMap.end())
    {
        TC_LOG_DEBUG("entities.unit", "CreatureTextMgr::TextExist: Could not find Text in group %u for Creature (entry %u) in 'creature_text' table.", uint32(textGroup), sourceEntry);
        return false;
    }
...
{
    CreatureTextMap::const_iterator holderItr = mTextMap.find({entry, textGroup });
    if (holderItr == mTextMap.end())
        return "";
