...
typedef std::unordered_map<std::pair<uint32, uint8>, CreatureTextGroup> CreatureTextMap;     // all creatures by entry
...
void CreatureTextMgr::SendChatPacket(WorldObject* source, Builder const& builder, ChatMsg msgType, WorldObject const* whisperTarget /*= nullptr*/, CreatureTextRange range /*= TEXT_RANGE_NORMAL*/, Team team /*= TEAM_OTHER*/, bool gmOnly /*= false*/) const
{
    ...
    switch (range)
    {
        case TEXT_RANGE_AREA:
        {
            ...
            Map::PlayerList players = source->GetMap()->GetPlayers();
            for (Player& itr : players)
                if (itr.GetAreaId() == areaId && (!team || Team(itr.GetTeam()) == team) && (!gmOnly || itr.IsGameMaster()))
                    localizer(&itr);
            return;
        }
        case TEXT_RANGE_ZONE:
        {
            ...
            Map::PlayerList players = source->GetMap()->GetPlayers();
            for (Player& itr : players)
                if (itr.GetZoneId() == zoneId && (!team || Team(itr.GetTeam()) == team) && (!gmOnly || itr.IsGameMaster()))
                    localizer(&itr);
            return;
        }
        case TEXT_RANGE_MAP:
        {
            Map::PlayerList players = source->GetMap()->GetPlayers();
            for (Player& itr : players)
                if ((!team || Team(itr.GetTeam()) == team) && (!gmOnly || itr.IsGameMaster()))
                    localizer(&itr);
            return;
        }
    ...
    const auto range_checker = [&](Player* target)
    {
        if (source->GetDistance(target) > dist)
            return;
    ...
    source->VisitAnyNearbyObject<Player, Trinity::FunctionAction>(dist, range_checker);
}
