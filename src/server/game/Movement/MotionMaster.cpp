...
GAME_API bool isStatic(MovementGenerator *mv)
{
...
// This will pause active movement, without clearing underlaying idle movement slot
void MotionMaster::PauseActiveMovement() 
{
    if (_owner->GetTypeId() == TYPEID_UNIT) 
    {
        TC_LOG_DEBUG("misc", "Creature (GUID: %u) paused active movement", _owner->GetGUID().GetCounter());
        Mutate(&si_idleMovement, MOTION_SLOT_ACTIVE);
    }
}
...
void MotionMaster::MoveCloserAndStop(uint32 id, Unit* target, float distance, float height)
{
    ...
    if (distanceToTravel > 0.0f)
    {
        ...
        float destz = target->GetPositionZ() + height;
...
void MotionMaster::MoveJump(Position const & pos, float speedXY, float speedZ, uint32 id, bool hasOrientation)
{
...
void MotionMaster::MovePullTo(float x, float y, float z, float speedXY, float speedZ, uint32 id)
{
    TC_LOG_DEBUG("misc", "Unit (GUID: %u) jump to point (X: %f Y: %f Z: %f)", _owner->GetGUID().GetCounter(), x, y, z);
    if (speedXY <= 0.1f)
        return;
...
void MotionMaster::MoveFall(uint32 id /*=0*/)
{
    ...
    const float waterHeight = _owner->GetMap()->GetWaterLevel(_owner->GetPositionX(), _owner->GetPositionY()) - _owner->GetCombatReach();
    if (_owner->GetPositionZ() < waterHeight)
        return;
...
void MotionMaster::MoveFleeing(Unit* enemy, uint32 time, bool run)
{
    if (!enemy)
        return;
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        Mutate(new FleeingMovementGenerator<Player>(enemy->GetGUID(), run), MOTION_SLOT_CONTROLLED);
    }
    else
    {
        ...
        if (time)
            Mutate(new TimedFleeingMovementGenerator(enemy->GetGUID(), time, run), MOTION_SLOT_CONTROLLED);
        else
            Mutate(new FleeingMovementGenerator<Creature>(enemy->GetGUID(), run), MOTION_SLOT_CONTROLLED);
    }
}
...
void MotionMaster::MoveScriptPath(uint32 path_id, bool repeatable, bool updatehomepos)
{
    if (!path_id)
        return;

    Mutate(new WaypointMovementGenerator<Creature>(path_id, repeatable, updatehomepos), MOTION_SLOT_ACTIVE);

    TC_LOG_DEBUG("entities.unit", "%s start moving over script path(repeatable: %s, updating homepos: %s)",
        _owner->GetGUID().ToString().c_str(), repeatable ? "YES" : "NO", updatehomepos ? "YES" : "NO");
}

void MotionMaster::MovePath(WaypointPath const* path, bool repeatable)
{
    if (!path)
        return;
    ...
    TC_LOG_DEBUG("entities.unit", "%s start moving over dynamic path(repeatable: %s)",
        _owner->GetGUID().ToString().c_str(), repeatable ? "YES" : "NO");
}

void MotionMaster::MoveFlightPath(WaypointPath const* path, bool repeatable)
{
    if (!path)
        return;

    Mutate(new CreatureFlightPathMovementGenerator(path, repeatable), MOTION_SLOT_IDLE);
