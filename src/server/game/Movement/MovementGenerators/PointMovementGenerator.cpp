...
template <> void PointMovementGenerator<Creature>::MovementInform(Creature* unit)
{
    *unit <<= Fire::PointReached;
    ...
    if (TempSummon* tmpSum = unit->ToTempSummon())
        if (Unit* summoner = tmpSum->GetSummoner())
            if (Creature* csummoner = summoner->ToCreature(); csummoner && csummoner->AI())
                csummoner->AI()->SummonedCreatureMovementInform(tmpSum, POINT_MOTION_TYPE, id);
}
