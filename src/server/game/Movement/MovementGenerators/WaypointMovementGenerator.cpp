...
void WaypointMovementGenerator<Creature>::LoadPath(Creature* creature)
{
    ...
    else if (!i_path)
    {
        ...
        TC_LOG_ERROR("entities.unit", "WaypointMovementGenerator::LoadPath: no dynamic waypointpath for creature %s (Entry: %u GUID: %u DB GUID: %u) ", creature->GetName().c_str(), creature->GetEntry(), creature->GetGUID().GetCounter(), creature->GetSpawnId());
        return;
    }
...
bool WaypointMovementGenerator<Creature>::StartMove(Creature* creature)
{
    ...
    if (m_isArrivalDone)
    {
        if ((i_currentNode == i_path->size() - 1) && !repeating) // If that's our last waypoint
        {
            ...
            if (!transportPath && updatehomepos)
...
bool WaypointMovementGenerator<Creature>::DoUpdate(Creature* creature, uint32 diff)
{
    ...
    {
        // If updatepos is set, set home position at place on waypoint movement.
        if ((!creature->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) || !creature->GetTransGUID()) && updatehomepos)
...
{
    *creature <<= Fire::WaypointReached(i_currentNode);

    if (i_currentNode == (i_path->size() - 1))
        *creature <<= Fire::PathEnded;
...
bool WaypointMovementGenerator<Creature>::GetResetPos(Creature* creature, float& x, float& y, float& z)
{
    ...
    if (!updatehomepos) 
    {
        ...
        x = creature->GetHomePosition().GetPositionX();
        y = creature->GetHomePosition().GetPositionY();
        z = creature->GetHomePosition().GetPositionZ();
        return true;
    }
...
void CreatureFlightPathMovementGenerator::DoInitialize(Creature* creature)
{
    creature->AddUnitState(UNIT_STATE_ROAMING | UNIT_STATE_ROAMING_MOVE);

    Movement::MoveSplineInit init(creature);

    for (auto const& point : *i_path)
    {
...
void CreatureFlightPathMovementGenerator::DoFinalize(Creature* creature)
{
    creature->ClearUnitState(UNIT_STATE_ROAMING | UNIT_STATE_ROAMING_MOVE);
    creature->SetWalk(false);
}

void CreatureFlightPathMovementGenerator::DoReset(Creature* creature)
{
    creature->AddUnitState(UNIT_STATE_IN_FLIGHT);
}

bool CreatureFlightPathMovementGenerator::DoUpdate(Creature* creature, uint32 diff)
{
    if (!i_path || i_path->empty())
        return false;

    i_currentNode = (uint32)creature->movespline->currentPathIdx();
...
void CreatureFlightPathMovementGenerator::MovementInform(Creature* creature)
{
    if (auto ai = creature->AI())
        ai->MovementInform(FLIGHT_PATH_MOTION_TYPE, i_currentNode);
}

bool CreatureFlightPathMovementGenerator::GetResetPos(Creature*, float& x, float& y, float& z)
{
    // prevent a crash at empty waypoint path.
    if (!i_path || i_path->empty())
        return false;
