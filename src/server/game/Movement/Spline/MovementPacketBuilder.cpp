...
{
    template<>
    constexpr bool IsByteBufferType<Movement::Vector3> = true;

    template<>
    struct BufferIO<Movement::Vector3>
    {
        static void write(ByteBuffer& b, Movement::Vector3 v)
        {
            b << v.x << v.y << v.z;
        }
    };
...
namespace Movement
{
    ...
    void WriteLinearPath(const Spline<int32>& spline, ByteBuffer& data)
    {
        ...
        if (last_idx > 1)
        {
            const Vector3 middle = (real_path[0] + real_path[last_idx]) / 2.f;
            // first and last points already appended
            for (uint32 i = 1; i < last_idx; ++i)
            {
                const Vector3 offset = middle - real_path[i];

                // can be used in SMSG_MONSTER_MOVE opcode
                uint32 packed = 0;
                packed |= ((int)(offset.x / 0.25f) & 0x7FF);
                packed |= ((int)(offset.y / 0.25f) & 0x7FF) << 11;
                packed |= ((int)(offset.z / 0.25f) & 0x3FF) << 22;
    ...
    void PacketBuilder::WriteCreate(const MoveSpline& move_spline, ByteBuffer& data)
    {
        ...
        //if (mov.SplineEnabled())
        {
            ...
            if(nodes != 0) {
                ...
                TC_LOG_ERROR("misc", "Build Movement Update: move_spline has no nodes");
            }
