...
MotionTransport* TransportMgr::CreateTransport(uint32 entry, uint32 guid /*= 0*/, Map* map /*= NULL*/)
{
    ...
    Position const pos{ startNode->x, startNode->y, startNode->z, tInfo->keyFrames.begin()->InitialOrientation };

    MotionTransport* trans = MotionTransport::Create(entry, mapId, pos, 255, guid);
    if (!trans)
        return nullptr;
...
bool KeyFrame::IsStopFrame() const { return Node->actionFlag == 2; }
