...
template<class TMap> class MapScript : public UpdatableScript<TMap>
{
    ...
    public:
        ...
        virtual void OnLoadGridMap(TMap* /*map*/, TerrainMap* /*gmap*/, uint32 /*gx*/, uint32 /*gy*/) { }

        // Called when a grid map is unloaded.
        virtual void OnUnloadGridMap(TMap* /*map*/, TerrainMap* /*gmap*/, uint32 /*gx*/, uint32 /*gy*/)  { }
...
class GAME_API InstanceMapScript : public ScriptObject, public MapScript<InstanceMap>
{
    protected:
...
class GAME_API CreatureScript : public UnitScript, public UpdatableScript<Creature>
{
    ...
    public:
        ...
        // Called when Item Inventory is send to a player
        virtual bool OnSendListInventory(Player* /*player*/, Creature* /*creature*/) { return true; }
...
class GAME_API GameObjectScript : public ScriptObject, public UpdatableScript<GameObject>
{
    ...
    public:
        ...
        // Called when the Gmae object is trap and is triggered
        virtual void OnTriggerTrap(GameObject* /*go*/, Unit* /*victim*/, uint32& /*spellId*/) { }
...
class GAME_API OnlyOnceAreaTriggerScript : public AreaTriggerScript
{
...
class GAME_API CommandScriptLoader : public ScriptObject
{
    protected:

        CommandScriptLoader(const char* name);

    public:

        bool IsDatabaseBound() const final override { return true; }

        virtual std::unique_ptr<chat::command::Command> CreateCommand(chat::command::CommandDefinition definition) const = 0;
};
...
class GAME_API PlayerScript : public UnitScript
{
    ...
    public:
        ...
        // Called when a player change his active spec right after the change is applied
        virtual void OnSwitchTalentSpec(Player* /*player*/, uint8 /*spec*/) { }
        ...
        virtual void OnSpellCast(Player* /*player*/, Spell* /*spell*/, SpellCastTargets& /*targets*/, bool /*skipCheck*/) { }
        ...
        virtual void OnUpdateZone(Player* /*player*/, uint32 /*newZone*/, uint32 /*newArea*/, uint32 /*oldZone*/) { }
        ...
        // Called when a quest is rewarded
        virtual void OnQuestReward(Player* /*player*/, uint32 /*questId*/) { }
};
...
class GAME_API SupportScript : public ScriptObject
{
    protected:
        SupportScript(const char* name);

    public:
        bool IsDatabaseBound() const final override { return false; }

        virtual void OnTicketCreate(support::Ticket& ticket) {}

        virtual void OnTicketFirstViewed(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketComplete(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketEscalate(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketAssign(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketUnassign(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketUpdateMessage(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketUpdateComment(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketUpdateResponse(support::Ticket& ticket, AccountId causer) {}

        virtual void OnTicketClose(support::Ticket& ticket, AccountId causer) {}
};
...
class GAME_API ScriptMgr
{
    ...
    public: /* Initialization */
        ...
        void SetScriptNameInUse(const std::string& script) { _loadedScripts.insert(script); }
    ...
    public: /* MapScript */
        ...
        void OnLoadGridMap(Map* map, TerrainMap* gmap, uint32 gx, uint32 gy);
        void OnUnloadGridMap(Map* map, TerrainMap* gmap, uint32 gx, uint32 gy);
    ...
    public: /* GameObjectScript */
        ...
        void OnGameObjectTriggerTrap(GameObject* go, Unit* victim, uint32& spellId);
    ...
    public: /* CommandScriptLoader */
        ...
        boost::optional<const CommandScriptLoader&> GetCommandScriptLoader(const std::string& name);
    ...
    public: /* PlayerScript */
        ...
        void OnPlayerSwitchTalentSpec(Player* player, uint8 spec);
        ...
        void OnPlayerSpellCast(Player* player, Spell* spell, SpellCastTargets& targets, bool skipCheck);
        ...
        void OnPlayerUpdateZone(Player* player, uint32 newZone, uint32 newArea, uint32 oldZone);
        void OnQuestStatusChange(Player* player, uint32 questId, QuestStatus status);
        void OnPlayerQuestReward(Player* player, uint32 questId);
    ...
    public: /* SupportScript */

        void OnTicketCreate(support::Ticket& ticket) const;
        void OnTicketFirstViewed(support::Ticket& ticket, AccountId causer) const;
        void OnTicketComplete(support::Ticket& ticket, AccountId causer) const;
        void OnTicketEscalate(support::Ticket& ticket, AccountId causer) const;
        void OnTicketAssign(support::Ticket& ticket, AccountId causer) const;
        void OnTicketUnassign(support::Ticket& ticket, AccountId causer) const;
        void OnTicketUpdateMessage(support::Ticket& ticket, AccountId causer) const;
        void OnTicketUpdateComment(support::Ticket& ticket, AccountId causer) const;
        void OnTicketUpdateResponse(support::Ticket& ticket, AccountId causer) const;
        void OnTicketClose(support::Ticket& ticket, AccountId causer) const;
...
template <class script_t, class additional_script_t = void>
struct SpellScriptLoaderEx : public SpellScriptLoader
{
    ...
    static_assert(std::is_base_of<SpellScript, script_t>::value || std::is_base_of<AuraScript, script_t>::value,
        "SpellScriptLoader: Given script is not a SpellScript or AuraScript!");

    static_assert(std::is_base_of<SpellScript, additional_script_t>::value ||
        std::is_base_of<AuraScript, additional_script_t>::value ||
        std::is_same<void, additional_script_t>::value,
        "SpellScriptLoader: Given additional script is not a SpellScript or AuraScript!");

    static_assert(!(std::is_base_of<SpellScript, script_t>::value &&
        std::is_base_of<SpellScript, additional_script_t>::value),
        "SpellScriptLoader: Both given scripts are a SpellScript, which is not supported!");

    static_assert(!(std::is_base_of<AuraScript, script_t>::value &&
        std::is_base_of<AuraScript, additional_script_t>::value),
        "SpellScriptLoader: Both given scripts are an AuraScript, which is not supported!");

    SpellScript *GetSpellScript() const override
    {
        return _CreateScript<SpellScript>(static_cast<script_t *>(nullptr),
            static_cast<additional_script_t *>(nullptr));
    }

    AuraScript *GetAuraScript() const override
    {
        return _CreateScript<AuraScript>(static_cast<script_t *>(nullptr), static_cast<additional_script_t *>(nullptr));
    }

private:
    template <class return_t, class first_t, class second_t>
    typename std::enable_if<std::is_base_of<return_t, first_t>::value && !std::is_base_of<return_t, second_t>::value,
        return_t>::type *
        _CreateScript(first_t *, second_t *) const
    {
        return new first_t;
    }

    template <class return_t, class first_t, class second_t>
    typename std::enable_if<std::is_base_of<return_t, second_t>::value && !std::is_base_of<return_t, first_t>::value,
        return_t>::type *
        _CreateScript(first_t *, second_t *) const
    {
        return new second_t;
    ...
    return_t *_CreateScript(...) const
    {
        return nullptr;
    }
};

GAME_API const MapEntry* lookupMapEntry(uint32 mapId);

template<class TMap>
inline MapScript<TMap>::MapScript(uint32 mapId)
    : _mapEntry(lookupMapEntry(mapId))
{
    if (!_mapEntry)
