            ...
            if (script->IsDatabaseBound())
            {
                ...
                if (id)
                {
                    ...
                    if (!existing)
                    {
                        ...
                        sScriptMgr->SetScriptNameInUse(script->GetName());
                    }
                    else
                    {
...
void ScriptMgr::Initialize()
{
    ...
    for (const std::string& name : sObjectMgr->GetScriptNames())
    {
        if (_loadedScripts.count(name) == 0)
            TC_LOG_ERROR("server.loading", "ScriptName '%s' exists in database, but no core script found!", name.c_str());
    }
...
void ScriptMgr::OnLoadGridMap(Map* map, TerrainMap* gmap, uint32 gx, uint32 gy)
{
    ASSERT(map);
    ASSERT(gmap);
...
void ScriptMgr::OnUnloadGridMap(Map* map, TerrainMap* gmap, uint32 gx, uint32 gy)
{
    ASSERT(map);
    ASSERT(gmap);
...
bool ScriptMgr::OnSendListInventory(Player* player, Creature* creature)
{
    ASSERT(player);
    ASSERT(creature);
    ...
    {
        GET_SCRIPT_RET(CreatureScript, creature->GetScriptId(), tmpscript, false);
        return tmpscript->OnSendListInventory(player, creature);
    }
    else
        return true;
}
...
void ScriptMgr::OnGameObjectTriggerTrap(GameObject* go, Unit* victim, uint32& spellId)
{
    ASSERT(go);

    GET_SCRIPT(GameObjectScript, go->GetScriptId(), tmpscript);
    tmpscript->OnTriggerTrap(go, victim, spellId);
}
...
boost::optional<const CommandScriptLoader&> ScriptMgr::GetCommandScriptLoader(const std::string& name)
{
    uint32 scriptId = sObjectMgr->GetScriptId(name.c_str());
    CommandScriptLoader* script = ScriptRegistry<CommandScriptLoader>::GetScriptById(scriptId);
...
{
    FOREACH_SCRIPT(WorldScript)->OnLoad();
}
...
void ScriptMgr::OnPlayerSwitchTalentSpec(Player* player, uint8 spec)
{
    FOREACH_SCRIPT(PlayerScript)->OnSwitchTalentSpec(player, spec);
}
...
void ScriptMgr::OnPlayerSpellCast(Player* player, Spell* spell, SpellCastTargets& targets, bool skipCheck)
{
    FOREACH_SCRIPT(PlayerScript)->OnSpellCast(player, spell, targets, skipCheck);
}
...
void ScriptMgr::OnPlayerUpdateZone(Player* player, uint32 newZone, uint32 newArea, uint32 oldZone)
{
    FOREACH_SCRIPT(PlayerScript)->OnUpdateZone(player, newZone, newArea, oldZone);
}
...
void ScriptMgr::OnPlayerQuestReward(Player* player, uint32 questId)
{
    FOREACH_SCRIPT(PlayerScript)->OnQuestReward(player, questId);
}
...
void ScriptMgr::OnTicketCreate(support::Ticket& ticket) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketCreate(ticket);
}

void ScriptMgr::OnTicketFirstViewed(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketFirstViewed(ticket, causer);
}

void ScriptMgr::OnTicketComplete(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketComplete(ticket, causer);
}

void ScriptMgr::OnTicketEscalate(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketEscalate(ticket, causer);
}

void ScriptMgr::OnTicketAssign(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketAssign(ticket, causer);
}

void ScriptMgr::OnTicketUnassign(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketUnassign(ticket, causer);
}

void ScriptMgr::OnTicketUpdateMessage(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketUpdateMessage(ticket, causer);
}

void ScriptMgr::OnTicketUpdateComment(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketUpdateComment(ticket, causer);
}

void ScriptMgr::OnTicketUpdateResponse(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketUpdateResponse(ticket, causer);
}

void ScriptMgr::OnTicketClose(support::Ticket& ticket, AccountId causer) const
{
    FOREACH_SCRIPT(SupportScript)->OnTicketClose(ticket, causer);
}
...
CommandScriptLoader::CommandScriptLoader(const char* name)
    : ScriptObject(name)
{
    ScriptRegistry<CommandScriptLoader>::AddScript(this);
}
...
SupportScript::SupportScript(const char* name)
    : ScriptObject(name)
{
    ScriptRegistry<SupportScript>::AddScript(this);
}

const MapEntry* lookupMapEntry(uint32 mapId)
{
...
template class ScriptRegistry<CommandScriptLoader>;
...
template class ScriptRegistry<SupportScript>;
