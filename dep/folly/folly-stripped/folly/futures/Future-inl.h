      ...
      [state = futures::detail::makeCoreCallbackState(
        ...
        if (!isTry && t.hasException()) {
          ...
          state.setTry(makeTryWith(
              [&] { return state.invoke(t.Try<T>::template get<isTry, Args>()...); }));
        }
      });
  ...
  setCallback_(
      [state = futures::detail::makeCoreCallbackState(
        ...
        if (!isTry && t.hasException()) {
          ...
          auto tf2 = state.tryInvoke(t.Try<T>::template get<isTry, Args>()...);
          if (tf2.hasException()) {
...
Future<T>::then(R(Caller::*func)(Args...), Caller *instance) {
  ...
  return then([instance, func](Try<T>&& t){
    return (instance->*func)(t.Try<T>::template get<isTry<FirstArg>::value, Args>()...);
  });
}
...
template <class T>
Try<T>& Future<T>::getTryVia(const std::shared_ptr<DrivableExecutor>& e) {
...
template <class T>
inline Future<T> Future<T>::via(const std::shared_ptr<Executor>& executor, int8_t priority) && {
  throwIfInvalid();

  if (executor)
    setExecutor(std::weak_ptr<Executor>(executor), priority);
  else
    setExecutor(folly::none, priority);
...
template <class Func>
auto via(const std::shared_ptr<Executor>& x, Func&& func)
...
// via
Future<Unit> via(const std::shared_ptr<Executor>& executor, int8_t priority) {
