...
class Core final {
 ...
 public:
  ...
  /// Call only from Future thread
  void setExecutor(folly::Optional<std::weak_ptr<Executor>> x, int8_t priority = Executor::MID_PRI) {
  ...
  void setExecutorNoLock(folly::Optional<std::weak_ptr<Executor>> x, int8_t priority = Executor::MID_PRI) {
    executor_ = std::move(x);
    priority_ = priority;
  }

  folly::Optional<std::weak_ptr<Executor>> getExecutor() {
    return executor_;
  }
 ...
 private:
  ...
  void doCallback() {
    folly::Optional<std::weak_ptr<Executor>> x;
    ...
    if (x) {
      if (auto executor = x.value().lock()) {
        ...
        try {
          if (LIKELY(executor->getNumPriorities() == 1)) {
            executor->add([core_ref = std::move(guard_lambda)]() mutable {
  ...
  int8_t priority_ {-1};
  folly::Optional<std::weak_ptr<Executor>> executor_{folly::none};
